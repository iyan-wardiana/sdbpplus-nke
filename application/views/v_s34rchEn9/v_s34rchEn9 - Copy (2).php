										...
										if(isset($_POST['kata_kunci']))
										{
											...
											else
											{
												...
												$results = "SELECT *, CONCAT(	IF(ISNULL(COMP_CODE),' ', COMP_CODE),
																			...
																			IF(ISNULL(COMP_NEWS),' ', COMP_NEWS)) AS keyword
															FROM tbl_company_news HAVING $where";
												...
												{
													//echo '<p>Kata kunci <b>'.$kata_kunci.'</b> mendapatkan '.$num.' hasil:</p>';
													?>
										...
										<div class="col-sm-12">
											<?php
												if(isset($_POST['kata_kunci']))
												{
													foreach($resSE as $therow) :
														$num 		= $num+1;
														$COMP_NUM 	= $therow->COMP_NUM;
														$COMP_CODE 	= $therow->COMP_CODE;
														$COMP_NAME 	= $therow->COMP_NAME ?: "-";
														$COMP_ADD 	= $therow->COMP_ADD ? "" : "-";
														$COMP_STATE	= $therow->COMP_STATE ? "" : "-";
														$COMP_PROV	= $therow->COMP_PROV ? "" : "-";
														$COMP_CITY	= $therow->COMP_CITY ? "" : "-";
														$COMP_MAIL	= $therow->COMP_MAIL ? "" : "-";
														$COMP_PHONE	= $therow->COMP_PHONE ? "" : "-";
														$DATE_LIST	= $therow->COMP_DATELIST;
														$COMP_LINK 	= $therow->COMP_LINK;
														$COMP_LINKD	= substr($COMP_LINK, 0, 30)."... ";
														$TITLE_H 	= $therow->COMP_NEWS;
														$DESC 		= "Kode : [ $COMP_CODE : $COMP_NAME ], Register Date : [ $DATE_LIST ]<br>
																		Alamat : $COMP_ADD, Prov. : $COMP_PROV, City : $COMP_CITY<br>e-Mail : $COMP_MAIL, Phone : $COMP_PHONE";
														echo '
														<p>
															<a href="'.$COMP_LINK.'" target="_blank">'.$COMP_LINKD.'</a><br>
															<b><font size="4px">'.$TITLE_H.'</font></b><br>
															'.$DESC.'...<br>
														</p>
														';	
													endforeach;
