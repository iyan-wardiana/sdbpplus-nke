...
else
{
	...
	$JOBCOST 	= 0;
	$BOQ_JOBCOST= 0;
	$sqlJlD		= "SELECT JOBPARENT, JOBDESC, JOBUNIT, JOBVOLM, PRICE, JOBCOST, BOQ_JOBCOST
			                                    ...
			                                    foreach($r_03 as $rw_03) :
			                                        ...
			                                        <tr id="tr_manldet<?php echo $noU_03; ?>" style="vertical-align: middle;">
				                                        ...
				                                        <td style="text-align:right; vertical-align: middle;"> <!-- ITM_KOEF -->
				                                        	...
				                                        	<?php } else { print number_format($ITM_KOEF, $decFormat); ?> <input type="hidden" name="ITM_KOEF<?php echo $noU_03; ?>" id="ITM_KOEF<?php echo $noU_03; ?>" value="<?php print number_format($ITM_KOEF, 4); ?>" class="form-control" style="text-align:right" onKeyPress="return isIntOnlyNew(event);" onBlur="chgKoef(this.value,<?php echo $noU_03; ?>);" > <?php } ?>
...
<script>
	...
	function chgPrc(thisVal1, row)
	{
		...
		document.getElementById('ITM_QTY'+row).value			= doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(ITMVOLM)), 3));
	...
	function chgKoef(thisVal1, row)
	{
		...
		document.getElementById('ITM_KOEF'+row).value			= doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(thisVal)), 3));
