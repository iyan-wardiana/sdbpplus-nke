...
<html>
	...
	<?php
		...
		if($LangID == 'IND')
		{
			...
			$sureUndo 	= "Anda yakin akan membatalkan komponen RAP ini?";
		}
		else
		{
				...
				<script type="text/javascript">
					...
					$(document).ready(function()
					{
				    	$('#example1').DataTable(
				    	{
							...
							"autoWidth": true,
							"filter": true,
					        "ajax": "<?php echo site_url('c_project/c_joblist/get_AllDataITM3/?id='.$PRJCODE.'&JID='.$JOB_PARCODE)?>",
					        "type": "POST",
	...
	function SyncVal()
	{
        ...
        $.ajax({
            ...
            success: function(response)
            {
				...
				document.getElementById('rapVOL').innerHTML 		= doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(TOT_V)), 2));
				document.getElementById('rapPRC').innerHTML 		= doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(AVG_P)), 2));
				document.getElementById('rapTOT').innerHTML 		= doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(TOT_C)), 2));
