...
<html>
	...
	<?php
		...
		$sql 			= "SELECT PRJNAME, PRJPERIOD, PRJ_IMGNAME, PRJCOST, RAPP_STAT, PRJADD FROM tbl_project WHERE PRJCODE = '$PRJCODE'";
    ...
    <?php
        ...
        $form_downl = site_url('c_comprof/c_bUd93tL15t/downLJL');
    ?>
			...
			<div class="row">
				<?php
					// 1. TOTAL BOQ DAN RAP
						...
						$s_00		= "SELECT SUM(ITM_BUDG) AS TOT_RAPP, SUM(RAPT_JOBCOST) AS TOT_RAPT,
										SUM(AMD_VAL-AMDM_VAL) AS TOT_ADD,
										SUM(PO_VAL - PO_CVAL + WO_VAL - WO_CVAL + VCASH_VAL + VLK_VAL + PPD_VAL) AS TREQ_VAL,
										SUM(PO_VAL_R + WO_VAL_R + VCASH_VAL_R + VLK_VAL_R + PPD_VAL_R) AS TREQ_VAL_R,
										SUM(IR_VAL + OPN_VAL + VCASH_VAL + VLK_VAL + PPD_VAL) AS TUSED_VAL,
										SUM(IR_VAL_R + OPN_VAL_R + VCASH_VAL_R + VLK_VAL_R + PPD_VAL_R) AS TUSED_VAL_R
										FROM tbl_joblist_detail_$PRJCODEVW WHERE PRJCODE = '$PRJCODE' AND ISLAST = 1";
						...
						foreach($r_00 as $rw_00):
							...
							$TREQ_VAL 	= $rw_00->TREQ_VAL;
							$TREQ_VAL_R = $rw_00->TREQ_VAL_R;
							$TOT_REQ 	= $TREQ_VAL + $TREQ_VAL_R;
							$TUSED_VAL 	= $rw_00->TUSED_VAL;
							$TUSED_VAL_R= $rw_00->TUSED_VAL_R;
							$TOT_USED 	= $TUSED_VAL + $TUSED_VAL_R;
						endforeach;
						...
						<table id="example" class="table table-bordered table-striped table-responsive search-table inner" width="100%">
					        <thead>
					            <tr>
					                ...
					                <th width="5%" style="text-align:center; vertical-align:middle" nowrap><?php echo $Requested; ?></th>
					                <th width="5%" style="text-align:center; vertical-align:middle" nowrap><?php echo $Realization; ?></th>
							...
							<tfoot>
								<?php
									...
									$TOT_REM 	= $TOT_RAPP + $TOT_ADD - $TOT_USED;
								?>
					            <tr>
					                <th colspan="4" style="text-align:center; vertical-align:middle" nowrap>T O T A L</th>
					                <th width="6%" style="text-align:right; vertical-align:middle" nowrap><?=number_format($TOT_RAPP, 0)?></th>
					                <th width="6%" style="text-align:right; vertical-align:middle" nowrap><?=number_format($TOT_BOQ, 0)?></th>
					                <th width="5%" style="text-align:right; vertical-align:middle" nowrap><?=number_format($TOT_ADD, 0)?></th>
					                <th width="6%" style="text-align:right; vertical-align:middle" nowrap><?=number_format($TOT_REQ, 0)?></th>
...
<script>
    ...
    $(document).ready(function()
    {
	    $('#example').DataTable( 
	    {
	        ...
	        "ajax": {
				        "url": "<?php echo site_url('c_project/c_joblistdet/get_AllDataJL/?id='.$PRJCODE)?>",
				        "type": "POST",
						"data": function(data) {
