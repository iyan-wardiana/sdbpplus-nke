 ...
 * File Name	= project_planning_inb.php
 * Location		= -
*/
...
<h1>
    <?php echo $h2_title; ?>
    <small>material request approval</small>
  </h1>
  <br>
		...
		<?php
			...
			if($recordcount >0)
			{
				foreach($vewproject as $row) : 
					...
					$theProjCode = $PRJCODE;
					$url_listMR	= site_url('c_project/material_request/get_last_ten_projMatReqInb/?id='.$this->url_encryption_helper->encode_url($theProjCode));
					?>    
        				<tbody>    	
                            <tr>
                                <td style="text-align:center; vertical-align:middle"> <?php print ++$i; ?>. </td>
                                <td style="text-align:center; vertical-align:middle"><?php echo anchor($url_listMR,$theProjCode);?></td>
