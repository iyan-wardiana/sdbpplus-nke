 ...
 * File Name	= material_request_inbox.php
 * Location		= -
*/
		...
		<?php
			...
			if($recordcount >0)
			{
				foreach($viewprojmatreq as $row) : 			
					...
					$secUPDurl	= site_url('c_project/material_request/update_inbox/?id='.$this->url_encryption_helper->encode_url($SPPNUM));
					?>
                        <tr <?php if($APPROVE >= 4) { ?> style="background:#CC6600" <?php } ?>>
                            <td style="text-align:center">
                            <input type="radio" name="chkDetail" id="chkDetail" value="<?php echo $SPPNUM;?>" onClick="getValueNo(this);" /></td>
                            <td nowrap><?php echo anchor($secUPDurl,$SPPNUM);?></td>
        ...
        <tr>
          <td colspan="7">
            <?php
                ...
                <input type="button" name="btnDelete" id="btnDelete" class="btn btn-primary" value="Print Document" onClick="printDocument();" />&nbsp;
            <?php
