...
<script data-cfasync="false" type="text/javascript">
	$(document).ready(function()
	{
		...
		$("#reportData").DataTable({
			...
			"paging": false,
			"ajax": {
				"url": "<?php echo site_url('c_project/c_r3p/get_AllReportData_sum');?>",
				"type": "POST",
			...
			"footerCallback": function (row, data, start, end, display) {
				...
				for(let i = 0; i < lnRow; i++) {
					...
					if(ISLASTH == 1)
					{
						GTITM_BUDG 			= parseFloat(GTITM_BUDG) + parseFloat(getData[i][5]);
						GTADD_TOTAL 		= parseFloat(GTADD_TOTAL) + parseFloat(getData[i][7]);
						GTADD_TOTAL_KOM 	= parseFloat(GTADD_TOTAL_KOM) + parseFloat(getData[i][9]);
						GTITM_BUDG2 		= parseFloat(GTITM_BUDG2) + parseFloat(getData[i][11]);
						GTREQ_AMOUNT 		= parseFloat(GTREQ_AMOUNT) + parseFloat(getData[i][13]);
						GTREQ_AMOUNT_KOM 	= parseFloat(GTREQ_AMOUNT_KOM) + parseFloat(getData[i][15]);
						GTREMREQ_AMOUNT 	= parseFloat(GTREMREQ_AMOUNT) + parseFloat(getData[i][17]);
					}
				}
				
				console.log('GTITM_BUDG = '+GTITM_BUDG);
				console.log(new Intl.NumberFormat().format(GTITM_BUDG));
				$(api.column(5).footer()).html('<div style="font-weight: bold;">'+formatter.format(GTITM_BUDG)+'</div>');
				console.log('GTADD_TOTAL = '+GTADD_TOTAL);
				$(api.column(7).footer()).html('<div style="font-weight: bold;">'+formatter.format(GTADD_TOTAL)+'</div>');
				console.log('GTADD_TOTAL_KOM = '+GTADD_TOTAL_KOM);
				$(api.column(9).footer()).html('<div style="font-weight: bold;">'+formatter.format(GTADD_TOTAL_KOM)+'</div>');
				console.log('GTITM_BUDG2 = '+GTITM_BUDG2);
				$(api.column(11).footer()).html('<div style="font-weight: bold;">'+formatter.format(GTITM_BUDG2)+'</div>');
				console.log('GTREQ_AMOUNT = '+GTREQ_AMOUNT);
				$(api.column(13).footer()).html('<div style="font-weight: bold;">'+formatter.format(GTREQ_AMOUNT)+'</div>');
				console.log('GTREQ_AMOUNT_KOM = '+GTREQ_AMOUNT_KOM);
				$(api.column(15).footer()).html('<div style="font-weight: bold;">'+formatter.format(GTREQ_AMOUNT_KOM)+'</div>');
				console.log('GTREMREQ_AMOUNT = '+GTREMREQ_AMOUNT);
				$(api.column(17).footer()).html('<div style="font-weight: bold;">'+formatter.format(GTREMREQ_AMOUNT)+'</div>');
			},
			"language": {
				"infoFiltered":"",
