					...
					if($resAMC > 0)
					{
						...
						foreach($resAM as $rowAM):
							...
							if($ITM_GROUP == 'M')
							{
								...
								$OPN_VOLM	= $rowAM->ITM_USED;
								$OPN_AMOUNT	= $rowAM->ITM_USED_AM;
								if($OPN_VOLM == 0)
									$OPN_PRICE	= 0;
								else
									$OPN_PRICE	= $OPN_AMOUNT / $OPN_VOLM;
							}
							else
							{
								$PO_VOLM	= $WO_VOLM;
							...
							// REMAIN BUDGET TO REALISASI => BUDGET - REQUEST
								$REM_VOLM2	= $ITM_VOLM - $OPN_VOLM;
								$REM_AMOUNT2= $ITM_BUDG - $OPN_AMOUNT;
								if($REM_VOLM2 == 0)
									$REM_PRICE2	= 0;
								else
							...
							// PREDICTION => REALISASI + (REM_BUDG * LASTPRICE)
								$PRED_VAL	= $OPN_AMOUNT + ($REM_VOLM2 * $ITM_LASTP);
						...
						?>
							<tr>
                                ...
                                <td nowrap style="text-align:right;"><?php echo number_format($ITM_VOLM, 2); ?></td>
                                ...
                                <td nowrap style="text-align:right;"><?php echo number_format($OPN_VOLM, 2); ?></td>
                                <td nowrap style="text-align:right;"><?php echo number_format($OPN_PRICE, 2); ?></td>
                                <td nowrap style="text-align:right; border-right-color:#000; border-right-width:2px;"><?php echo number_format($OPN_AMOUNT, 2); ?></td>
                                <td nowrap style="text-align:right;"><?php echo number_format($OPN_VOLM, 2); ?></td>
                                <td nowrap style="text-align:right;"><?php echo number_format($OPN_PRICE, 2); ?></td>
                                <td nowrap style="text-align:right; border-right-color:#000; border-right-width:2px;"><?php echo number_format($OPN_AMOUNT, 2); ?></td>
                                <td nowrap style="text-align:right;"><?php echo number_format($OPN_VOLM, 2); ?></td>
