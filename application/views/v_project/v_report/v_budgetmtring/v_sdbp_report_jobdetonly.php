        	...
        	<table id="reportData" width="100%" border="1" rules="all" style="border-color: black;">
        		<thead>
        			<tr style="background:#CCCCCC">
						...
						<th rowspan="2" width="30" style="text-align:center; font-weight:bold; border-bottom-color:#000">ISLASTH</th>
						<th rowspan="2" width="30" style="text-align:center; font-weight:bold; border-bottom-color:#000">r_isLS</th>
						<th rowspan="2" width="30" style="text-align:center; font-weight:bold; border-bottom-color:#000">IS_LEVEL</th>
						<th rowspan="2" width="30" style="text-align:center; font-weight:bold; border-bottom-color:#000">IS_LAST</th>
						<th rowspan="2" width="30" style="text-align:center; font-weight:bold; border-bottom-color:#000">secvwAMDD</th>
						<th rowspan="2" width="30" style="text-align:center; font-weight:bold; border-bottom-color:#000">secvwREQD</th>
						<th rowspan="2" width="30" style="text-align:center; font-weight:bold; border-bottom-color:#000">secvwUSED</th>
...
<script type="text/javascript">
	$(document).ready(function()
	{
		...
		$("#reportData").DataTable({
			...
			"paging": false,
			"ajax": {
				"url": "<?php echo site_url('c_project/c_r3p/get_AllReportData_jobdetonly');?>",
			...
			"createdRow": function(row, data, dataIndex) {
				let ISLASTH 	= data[2];
				let r_isLS 		= data[3];
				let RAP_REM_VOL = data[17];
				let RAP_REM_VAL	= data[18];
				if(ISLASTH == 1)
				{
					$('td:eq(2)', row).attr('colspan', 10);
				...
				{
					if(RAP_REM_VOL < 0 && r_isLS == 0)
					{
						$('td:eq(10)', row).css('background-color', 'gray');
					}

					if(RAP_REM_VAL < 0)
					{
						console.log(RAP_REM_VAL);
						$('td:eq(11)', row).css('background-color', 'gray');
					}
				}
			},
			"columnDefs": [	
				{
					...
					render: function(data, type, row, meta) {
						let ISLASTH		= row[2];
						let IS_LEVEL 	= row[4];
						let CELL_COL	= "font-weight:normal;";
				...
				},
				{
					...
					render: function(data, type, row, meta) {
						...
						// let LinkD 		= row[6];
						let indexRow 		= parseInt(meta.row) + 1;

						return '<a onclick="showITMDET_AMDD('+indexRow+')" style="cursor: pointer;"><input type="hidden" id="secvwAMDD_'+indexRow+'" name="secvwAMDD" value="'+row[6]+'">'+formatter.format(data)+'</a>';
					}
				},
				{
					targets: [16],
					...
					render: function(data, type, row, meta) {
						...
						return '<a onclick="showITMDET_REQD('+indexRow+')" style="cursor: pointer;"><input type="hidden" id="secvwREQD_'+indexRow+'" name="secvwREQD" value="'+row[7]+'">'+formatter.format(data)+'</a>';
					}
				}
			],
			"footerCallback": function (row, data, start, end, display) {
				...
				for(let i = 0; i < lnRow; i++) {
					...
					let ISLAST 		= getData[i][5];
					if(ISLAST == 1)
					{
						RAPT_VAL 	= parseFloat(RAPT_VAL) + parseFloat(getData[i][10]);
						AMDT_VAL 	= parseFloat(AMDT_VAL) + parseFloat(getData[i][12]);
						RAPT_VAL2 	= parseFloat(RAPT_VAL2) + parseFloat(getData[i][14]);
						REQT_VAL 	= parseFloat(REQT_VAL) + parseFloat(getData[i][16]);
						REMT_VAL 	= parseFloat(REMT_VAL) + parseFloat(getData[i][18]);
					}
				}
				
				console.log('RAPT_VAL = '+RAPT_VAL);
				$(api.column(10).footer()).html('<div style="font-weight: bold;">'+formatter.format(RAPT_VAL)+'</div>');
				console.log('AMDT_VAL = '+AMDT_VAL);
				$(api.column(12).footer()).html('<div style="font-weight: bold;">'+formatter.format(AMDT_VAL)+'</div>');
				console.log('RAPT_VAL2 = '+RAPT_VAL2);
				$(api.column(14).footer()).html('<div style="font-weight: bold;">'+formatter.format(RAPT_VAL2)+'</div>');
				console.log('REQT_VAL = '+REQT_VAL);
				$(api.column(16).footer()).html('<div style="font-weight: bold;">'+formatter.format(REQT_VAL)+'</div>');
				console.log('REMT_VAL = '+REMT_VAL);
				$(api.column(18).footer()).html('<div style="font-weight: bold;">'+formatter.format(REMT_VAL)+'</div>');
			},
			"language": {
				"infoFiltered":"",
	...
	function showITMDET_AMDD(row)
	{
		let LinkD = document.getElementById('secvwAMDD_'+row).value;
		w = 1000;
		h = 550;
	...
	function showITMDET_REQD(row)
	{
		let LinkD = document.getElementById('secvwREQD_'+row).value;
		w = 1000;
		h = 550;
		var left = (screen.width/2)-(w/2);
		var top = (screen.height/2)-(h/2);
