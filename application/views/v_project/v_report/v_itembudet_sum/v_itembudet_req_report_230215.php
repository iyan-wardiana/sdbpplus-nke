								...
								// get Joblist_detail
									...
									if($resJobD->num_rows() > 0)
									{
										...
										foreach($resJobD->result() as $rJobD):
											...
											// Get ADD, REQ, REALIZ PERIODE INI
												...
												if($resBUDGItem->num_rows() > 0)
												{
													foreach($resBUDGItem->result() as $rBUDGItem):
														...
														// TOTAL 
															$TADD_VOL 		= $TADD_VOL + $ADDVOLM;
															$TADD_VAL 		= $TADD_VAL + $ADD_TOTAL;

															$TREQ_VOL 		= $TREQ_VOL + $REQ_VOLM;
															$TREQ_VAL 		= $TREQ_VAL + $REQ_AMOUNT;

															$TUM_VOL 		= $TUM_VOL + $ITM_USED;
															$TUM_VAL 		= $TUM_VAL + $ITM_USED_AM;
													endforeach;
												}

											// Total => SAAT INI
												$TOT_ADDM 		= $TOT_ADDM + $TADD_VAL;
												$TOT_REQM 		= $TOT_REQM + $TREQ_VAL;
												$TOT_REALZM 	= $TOT_REALZM + $TUM_VAL;
											...
											// Get ADD, REQ, REALIZ KOMULATIF
												...
												if($resBUDGLItem->num_rows() > 0)
												{
													foreach($resBUDGLItem->result() as $rBUDGLItem):
														...
														// TOTAL 
															$TADD_VOL_KOM 		= $TADD_VOL_KOM + $ADDVOLM_KOM;
															$TADD_VAL_KOM 		= $TADD_VAL_KOM + $ADD_TOTAL_KOM;

															$TREQ_VOL_KOM 		= $TREQ_VOL_KOM + $REQ_VOLM_KOM;
															$TREQ_VAL_KOM 		= $TREQ_VAL_KOM + $REQ_AMOUNT_KOM;

															$TUM_VOL_KOM 		= $TUM_VOL_KOM + $ITM_USED_KOM;
															$TUM_VAL_KOM 		= $TUM_VAL_KOM + $ITM_USED_AM_KOM;
													endforeach;
												}
											...
											// Total => BULAN LALU
												$TOT_ADDM_KOM 		= $TOT_ADDM_KOM + $TADD_VAL_KOM;
												$TOT_REQM_KOM 		= $TOT_REQM_KOM + $TREQ_VAL_KOM;
												$TOT_REALZM_KOM 	= $TOT_REALZM_KOM + $TUM_VAL_KOM;
													...
													<!-- REQUEST : SISA BUDGET THD REQUEST --->
														...
														<td nowrap style="text-align:right; text-decoration: underline; border-right-color:#000; border-right-width:2px;">
															<a onclick="showREQDET('<?php echo $secvwREQD; ?>')" style="cursor: pointer;">
																<?php echo number_format($REMREQ_AMOUNT, 2); ?>
															</a>
														</td>
