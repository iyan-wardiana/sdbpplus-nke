							...
							foreach($resGItem->result() as $rGItem):
								...
								if($resJobD->num_rows() > 0)
								{
									...
									foreach($resJobD->result() as $rJobD):
										...
										// TOTAL REQUEST NOW
											$PRT_VOL_NOW 	= $PR_VOL_NOW + $WO_VOL_NOW + $VCASH_VOL_NOW + $VLK_VOL_NOW + $PPD_VOL_NOW + $PR_VOL_R_NOW + $WO_VOL_R_NOW + $VCASH_VOL_R_NOW + $VLK_VOL_R_NOW + $PPD_VOL_R_NOW - $PR_CVOL_NOW - $WO_CVOL_NOW;
											$PRT_VAL_NOW 	= $PO_VAL_NOW + $WO_VAL_NOW + $VCASH_VAL_NOW + $VLK_VAL_NOW + $PPD_VAL_NOW + $PO_VAL_R_NOW + $WO_VAL_R_NOW + $VCASH_VAL_R_NOW + $VLK_VAL_R_NOW + $PPD_VAL_R_NOW - $PO_CVAL_NOW - $WO_CVAL_NOW;

										// TOTAL REQUEST KOMULATIF
											$PRT_VOL_ALL	= $PR_VOL_ALL + $WO_VOL_ALL + $VCASH_VOL_ALL + $VLK_VOL_ALL + $PPD_VOL_ALL + $PR_VOL_R_ALL + $WO_VOL_R_ALL + $VCASH_VOL_R_ALL + $VLK_VOL_R_ALL + $PPD_VOL_R_ALL - $PR_CVOL_ALL - $WO_CVOL_ALL;
											$PRT_VAL_ALL	= $PO_VAL_ALL + $WO_VAL_ALL + $VCASH_VAL_ALL + $VLK_VAL_ALL + $PPD_VAL_ALL + $PO_VAL_R_ALL + $WO_VAL_R_ALL + $VCASH_VAL_R_ALL + $VLK_VAL_R_ALL + $PPD_VAL_R_ALL - $PO_CVAL_ALL - $WO_CVAL_ALL;
											$PRT_VAL_ALLV	= "$PO_VAL_ALL + $WO_VAL_ALL + $VCASH_VAL_ALL + $VLK_VAL_ALL + $PPD_VAL_ALL + $PO_VAL_R_ALL + $WO_VAL_R_ALL + $VCASH_VAL_R_ALL + $VLK_VAL_R_ALL + $PPD_VAL_R_ALL - $PO_CVAL_ALL - $WO_CVAL_ALL";
										...
										// REM TO REQUEST
											...
											$PR_VAL_REMV	= "$ITM_VAL_BG2 - $PRT_VAL_ALL";
										...
										?>
											<tr>
												...
												<!-- REALISASI : SISA BUDGET THD REALISASI --->
													<td nowrap style="text-align:right; <?=$alrtStyl3?>"><?php echo number_format($RL_VOL_REM, 2); ?></td>
