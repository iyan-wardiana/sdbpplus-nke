							...
							foreach($resGItem->result() as $rGItem):
								...
								if($resJobD->num_rows() > 0)
								{
									...
									foreach($resJobD->result() as $rJobD):
										...
										// TOTAL REQUEST NOW
											$PRT_VOL_NOW 	= $PR_VOL_NOW + $PR_VOL_R_NOW + $WO_VOL_NOW + $WO_VOL_R_NOW + $VCASH_VOL_NOW + $VCASH_VOL_R_NOW + $VLK_VOL_NOW + $VLK_VOL_R_NOW + $PPD_VOL_NOW + $PPD_VOL_R_NOW-$PR_CVOL_NOW-$WO_CVOL_NOW;
											$PRT_VAL_NOW 	= $PO_VAL_NOW + $PO_VAL_R_NOW + $WO_VAL_NOW + $WO_VAL_R_NOW + $VCASH_VAL_NOW + $VCASH_VAL_R_NOW + $VLK_VAL_NOW + $VLK_VAL_R_NOW + $PPD_VAL_NOW + $PPD_VAL_R_NOW - $PO_CVAL_NOW - $WO_CVAL_NOW;

										// TOTAL REQUEST KOMULATIF
											$PRT_VOL_ALL 	= $PR_VOL_ALL + $PR_VOL_R_ALL + $WO_VOL_ALL + $WO_VOL_R_ALL + $VCASH_VOL_ALL + $VCASH_VOL_R_ALL + $VLK_VOL_ALL + $VLK_VOL_R_ALL + $PPD_VOL_ALL + $PPD_VOL_R_ALL-$PR_CVOL_ALL-$WO_CVOL_ALL;
											$PRT_VAL_ALL 	= $PO_VAL_ALL + $PO_VAL_R_ALL + $WO_VAL_ALL + $WO_VAL_R_ALL + $VCASH_VAL_ALL + $VCASH_VAL_R_ALL + $VLK_VAL_ALL + $VLK_VAL_R_ALL + $PPD_VAL_ALL + $PPD_VAL_R_ALL - $PO_CVAL_ALL - $WO_CVAL_ALL;

										// TOTAL REALISASI NOW - IR VERSION
											// $RLT_VOL_NOW 	= $IR_VOL_NOW + $IR_VOL_R_NOW + $WO_VOL_NOW + $WO_VOL_R_NOW + $VCASH_VOL_NOW + $VCASH_VOL_R_NOW + $VLK_VOL_NOW + $VLK_VOL_R_NOW + $PPD_VOL_NOW + $PPD_VOL_R_NOW - $WO_CVOL_NOW;
											$RLT_VOL_NOW 	= $IR_VOL_NOW + $IR_VOL_R_NOW + $OPN_VOL_NOW + $OPN_VOL_R_NOW + $VCASH_VOL_NOW + $VCASH_VOL_R_NOW + $VLK_VOL_NOW + $VLK_VOL_R_NOW + $PPD_VOL_NOW + $PPD_VOL_R_NOW;
											// $RLT_VAL_NOW 	= $IR_VAL_NOW + $IR_VAL_R_NOW + $WO_VAL_NOW + $WO_VAL_R_NOW + $VCASH_VAL_NOW + $VCASH_VAL_R_NOW + $VLK_VAL_NOW + $VLK_VAL_R_NOW + $PPD_VAL_NOW + $PPD_VAL_R_NOW - $WO_CVAL_NOW;
											$RLT_VAL_NOW 	= $IR_VAL_NOW + $IR_VAL_R_NOW + $OPN_VAL_NOW + $OPN_VAL_R_NOW + $VCASH_VAL_NOW + $VCASH_VAL_R_NOW + $VLK_VAL_NOW + $VLK_VAL_R_NOW + $PPD_VAL_NOW + $PPD_VAL_R_NOW;

										// TOTAL REALISASI NOW - UM VERSION
											$UMT_VOL_NOW 	= $UM_VOL_NOW + $UM_VOL_R_NOW + $OPN_VOL_NOW + $OPN_VOL_R_NOW + $VCASH_VOL_NOW + $VCASH_VOL_R_NOW + $VLK_VOL_NOW + $VLK_VOL_R_NOW + $PPD_VOL_NOW + $PPD_VOL_R_NOW;
											$UMT_VAL_NOW 	= $UM_VAL_NOW + $UM_VAL_R_NOW + $OPN_VAL_NOW + $OPN_VAL_R_NOW + $VCASH_VAL_NOW + $VCASH_VAL_R_NOW + $VLK_VAL_NOW + $VLK_VAL_R_NOW + $PPD_VAL_NOW + $PPD_VAL_R_NOW;

										// TOTAL REALISASI KOMULATIF - IR VERSION
											...
											// $RLT_VOL_ALL 	= $IR_VOL_ALL + $IR_VOL_R_ALL + $WO_VOL_ALL + $WO_VOL_R_ALL + $VCASH_VOL_ALL + $VCASH_VOL_R_ALL + $VLK_VOL_ALL + $VLK_VOL_R_ALL + $PPD_VOL_ALL + $PPD_VOL_R_ALL - $WO_CVOL_ALL;
											//$RLT_VOL_ALL 	= $IR_VOL_ALL + $IR_VOL_R_ALL + $OPN_VOL_ALL + $OPN_VOL_R_ALL + $VCASH_VOL_ALL + $VCASH_VOL_R_ALL + $VLK_VOL_ALL + $VLK_VOL_R_ALL + $PPD_VOL_ALL + $PPD_VOL_R_ALL;
											$RLT_VOL_ALL 	= $IR_VOL_ALL + $IR_VOL_R_ALL + $OPN_VOL_ALL + $OPN_VOL_R_ALL + $VCASH_VOL_ALL + $VCASH_VOL_R_ALL + $VLK_VOL_ALL + $VLK_VOL_R_ALL + $PPD_VOL_ALL + $PPD_VOL_R_ALL;
											// $RLT_VAL_ALL 	= $IR_VAL_ALL + $IR_VAL_R_ALL + $WO_VAL_ALL + $WO_VAL_R_ALL + $VCASH_VAL_ALL + $VCASH_VAL_R_ALL + $VLK_VAL_ALL + $VLK_VAL_R_ALL + $PPD_VAL_ALL + $PPD_VAL_R_ALL - $WO_CVAL_ALL;
											//$RLT_VAL_ALL 	= $IR_VAL_ALL + $IR_VAL_R_ALL + $WO_VAL_ALL + $WO_VAL_R_ALL + $VCASH_VAL_ALL + $VCASH_VAL_R_ALL + $VLK_VAL_ALL + $VLK_VAL_R_ALL + $PPD_VAL_ALL + $PPD_VAL_R_ALL;
											$RLT_VAL_ALL 	= $IR_VAL_ALL + $IR_VAL_R_ALL + $OPN_VAL_ALL + $OPN_VAL_R_ALL + $VCASH_VAL_ALL + $VCASH_VAL_R_ALL + $VLK_VAL_ALL + $VLK_VAL_R_ALL + $PPD_VAL_ALL + $PPD_VAL_R_ALL;

										// TOTAL REALISASI KOMULATIF - UM VERSION
											$UMT_VOL_ALL 	= $UM_VOL_ALL + $UM_VOL_R_ALL + $OPN_VOL_ALL + $OPN_VOL_R_ALL + $VCASH_VOL_ALL + $VCASH_VOL_R_ALL + $VLK_VOL_ALL + $VLK_VOL_R_ALL + $PPD_VOL_ALL + $PPD_VOL_R_ALL;
											$UMT_VAL_ALL 	= $UM_VAL_ALL + $UM_VAL_R_ALL + $OPN_VAL_ALL + $OPN_VAL_R_ALL + $VCASH_VAL_ALL + $VCASH_VAL_R_ALL + $VLK_VAL_ALL + $VLK_VAL_R_ALL + $PPD_VAL_ALL + $PPD_VAL_R_ALL;
										...
										$alrtStyl3 		= "";
										$alrtStyl4 		= "";
										if(round($PR_VOL_REM, 2) < 0 && $r_isLS == 0)
										{
											$alrtStyl1 	= "background-color: gray;";
										}
										if(round($PR_VAL_REM,2) < 0)
										{
											$alrtStyl2 	= "background-color: gray;";
										}

										if(round($RL_VOL_REM, 2) < 0 && $r_isLS == 0)
										{
											$alrtStyl3 	= "background-color: gray;";
										}
										if(round($RL_VAL_REM,2) < 0)
										{
										...
										?>
											<tr>
												...
												<!-- REQUEST : SISA BUDGET THD REQUEST --->
													<td nowrap style="text-align:right; <?=$alrtStyl1?>"><?php echo number_format($PR_VOL_REM, 3); ?></td>
													<td nowrap style="text-align:right; text-decoration: underline; border-right-color:#000; border-right-width:2px; <?=$alrtStyl2?>">
												...
												<!-- REALISASI : SISA BUDGET THD REALISASI --->
													<td nowrap style="text-align:right; <?=$alrtStyl3?>"><?php echo number_format($RL_VOL_REM, 3); ?></td>
													<td nowrap style="text-align:right; border-right-color:#000; border-right-width:2px; <?=$alrtStyl4?>"><?php echo number_format($RL_VAL_REM, 2); ?></td>
											</tr>
										<?php
									endforeach;
