 ...
 * File Name  = v_itembudetrealz_report.php
 * Location   = -
*/
        ...
        <td class="style2" style="text-align:left; font-style:italic">
            <?php
                ...
                if($resAMC > 0)
                {
                    ...
                    // PERMINTAAN
                        $sqlTREALZ     = "SELECT SUM(A.ITM_VOLM) AS TREALZ_VOLM, SUM(A.JournalD_Debet) AS TREALZ_AMOUNT
                                        FROM tbl_journaldetail A 
                                        INNER JOIN tbl_journalheader B ON B.JournalH_Code = A.JournalH_Code AND B.proj_Code = B.proj_Code
                                        WHERE A.proj_Code = '$PRJCODE' $addQITM $addQJOBID AND B.JournalH_Date BETWEEN '$Start_Date' AND '$End_Date' AND B.GEJ_STAT = 3 AND A.JournalType = 'IR'
                                        UNION
                                        SELECT SUM(A.ITM_VOLM) AS TREALZ_VOLM, SUM(A.JournalD_Debet) AS TREALZ_AMOUNT
                                        FROM tbl_journaldetail A 
                                        INNER JOIN tbl_journalheader B ON B.JournalH_Code = A.JournalH_Code AND B.proj_Code = B.proj_Code
                                        WHERE A.proj_Code = '$PRJCODE' $addQITM $addQJOBID AND B.JournalH_Date BETWEEN '$Start_Date' AND '$End_Date' AND B.GEJ_STAT = 3 AND A.JournalType = 'OPN'
                                        UNION
                                        SELECT SUM(A.ITM_VOLM) AS TREALZ_VOLM, SUM(A.JournalD_Debet) AS TREALZ_AMOUNT
                                        FROM tbl_journaldetail A 
                                        INNER JOIN tbl_journalheader B ON B.JournalH_Code = A.JournalH_Code AND B.proj_Code = B.proj_Code
                                        WHERE A.proj_Code = '$PRJCODE' $addQITM $addQJOBID AND B.JournalH_Date BETWEEN '$Start_Date' AND '$End_Date' AND B.GEJ_STAT = 3 AND A.JournalType = 'VCASH'
                                        UNION
                                        SELECT SUM(A.ITM_VOLM) AS TREALZ_VOLM, SUM(A.JournalD_Debet) AS TREALZ_AMOUNT
                                        FROM tbl_journaldetail A 
                                        INNER JOIN tbl_journalheader B ON B.JournalH_Code = A.JournalH_Code AND B.proj_Code = B.proj_Code
                                        WHERE A.proj_Code = '$PRJCODE' $addQITM $addQJOBID AND B.JournalH_Date BETWEEN '$Start_Date' AND '$End_Date' AND B.GEJ_STAT = 3 AND A.JournalType = 'CPRJ'";
                        $resTREALZ     = $this->db->query($sqlTREALZ);
                        foreach($resTREALZ->result() as $rTREALZ):
                            $TREALZ_VOLM      = $rTREALZ->TREALZ_VOLM;
                            $TREALZ_AMOUNT    = $rTREALZ->TREALZ_AMOUNT;
                        endforeach;


                    // SISA BUDGET THD PERMINTAAN
                        $REM_VOLM       = $ITM_VOLM2 - $TREALZ_VOLM;
                        $REM_AMOUNT     = $ITM_BUDG2 - $TREALZ_AMOUNT;
                        ...
                        ?>
                            <table width="100%" border="0" rules="all" style="border-bottom: 1px solid;">
                              ...
                              <tr style="text-align:left; font-style:italic">
                                  <td nowrap valign="top">REALISASI</td>
                                  <td style="text-align: right;"><?php echo number_format($TREALZ_VOLM, 2); ?></td>
                                  <td style="text-align: right;"><?php echo number_format($TREALZ_AMOUNT, 2); ?></td>
                              </tr>
                              <tr style="text-align:left; font-style:italic">
                                  <td nowrap valign="top" style="font-weight: bold;">SISA BUDGET THD REALISASI</td>
        ...
        <td colspan="3" class="style2">
            <table width="100%" border="1" rules="all">
                <tr style="background:#CCCCCC">
                    ...
                    <th width="2%" nowrap style="text-align:center; font-weight:bold; border-bottom-width:2px; border-bottom-color:#000; border-top-width:2px; border-top-color:#000;">KODE</th>
                    <th width="5%" nowrap style="text-align:center; font-weight:bold; border-bottom-width:2px; border-bottom-color:#000; border-top-width:2px; border-top-color:#000;">KETERANGAN</th>
                    <th width="5%" nowrap style="text-align:center; font-weight:bold; border-bottom-width:2px; border-bottom-color:#000; border-top-width:2px; border-top-color:#000;">ITEM</th>
                    <th width="3%" nowrap style="text-align:center; font-weight:bold; border-bottom-width:2px; border-bottom-color:#000; border-top-width:2px; border-top-color:#000;">VOLUME</th>
                    <th width="3%" nowrap style="text-align:center; font-weight:bold; border-bottom-width:2px; border-bottom-color:#000; border-top-width:2px; border-top-color:#000;">HARGA</th>
                    <th width="3%" nowrap style="text-align:center; font-weight:bold; border-bottom-width:2px; border-bottom-color:#000; border-top-width:2px; border-top-color:#000;">JUMLAH</th>
                    <th width="3%" nowrap style="text-align:center; font-weight:bold; border-bottom-width:2px; border-bottom-color:#000; border-top-width:2px; border-top-color:#000;">SISA BUDGET</th>
                    <th width="2%" nowrap style="text-align:center; font-weight:bold; border-bottom-width:2px; border-bottom-color:#000; border-right-color:#000; border-right-width:2px; border-top-width:2px; border-top-color:#000;">SATUAN</th>
                </tr>
              <tr style="line-height:1px; border-left:hidden; border-right:hidden">
               ...
               <?php
                    $sqlREALZ     = "SELECT A.JournalH_Code AS REALZ_NUM, A.Manual_No AS REALZ_CODE, B.JournalH_Date AS REALZ_DATE, A.Other_Desc AS REALZ_DESC, 
                                    A.ITM_VOLM AS REALZ_VOLM, A.JournalD_Debet AS REALZ_AMOUNT, A.JOBCODEID, A.ITM_CODE, A.ITM_UNIT 
                                    FROM tbl_journaldetail A 
                                    INNER JOIN tbl_journalheader B ON B.JournalH_Code = A.JournalH_Code AND B.proj_Code = B.proj_Code
                                    WHERE A.proj_Code = '$PRJCODE' $addQITM $addQJOBID AND B.JournalH_Date BETWEEN '$Start_Date' AND '$End_Date' AND B.GEJ_STAT = 3 AND A.JournalType = 'IR'
                                    UNION
                                    SELECT A.JournalH_Code AS REALZ_NUM, A.Manual_No AS REALZ_CODE, B.JournalH_Date AS REALZ_DATE, A.Other_Desc AS REALZ_DESC, 
                                    A.ITM_VOLM AS REALZ_VOLM, A.JournalD_Debet AS REALZ_AMOUNT, A.JOBCODEID, A.ITM_CODE, A.ITM_UNIT 
                                    FROM tbl_journaldetail A 
                                    INNER JOIN tbl_journalheader B ON B.JournalH_Code = A.JournalH_Code AND B.proj_Code = B.proj_Code
                                    WHERE A.proj_Code = '$PRJCODE' $addQITM $addQJOBID AND B.JournalH_Date BETWEEN '$Start_Date' AND '$End_Date' AND B.GEJ_STAT = 3 AND A.JournalType = 'OPN'
                                    ...
                                    INNER JOIN tbl_journalheader B ON B.JournalH_Code = A.JournalH_Code AND B.proj_Code = B.proj_Code
                                    WHERE A.proj_Code = '$PRJCODE' $addQITM $addQJOBID AND B.JournalH_Date BETWEEN '$Start_Date' AND '$End_Date' AND B.GEJ_STAT = 3 AND A.JournalType = 'CPRJ'";
                    $resREALZ     = $this->db->query($sqlREALZ);
                    $TOTREALZ_AM      = 0;
                    $TOTREMREALZ_BUDG = 0;
                    if($resREALZ->num_rows() > 0) 
                    {
                        $no             = 0;
                        $REMREALZ_BUDG  = $ITM_BUDG;
                        foreach($resREALZ->result() as $rREALZ):
                            $no             = $no + 1;
                            $REALZ_NUM      = $rREALZ->REALZ_NUM;
                            $REALZ_CODE     = $rREALZ->REALZ_CODE;
                            $REALZ_DATE     = $rREALZ->REALZ_DATE;
                            $REALZ_DESC     = $rREALZ->REALZ_DESC;
                            $REALZ_VOLM     = $rREALZ->REALZ_VOLM;
                            $REALZ_AMOUNT   = $rREALZ->REALZ_AMOUNT;
                            $JOBCODEID      = $rREALZ->JOBCODEID;
                            $ITM_CODE       = $rREALZ->ITM_CODE;
                            $ITM_UNIT       = $rREALZ->ITM_UNIT;
                            
                            if($REALZ_VOLM == '' || $REALZ_VOLM == 0) 
                                $REALZ_VOLM = 1;
                                
                            $REALZ_PRICE      = $REALZ_AMOUNT / $REALZ_VOLM;
                            $REMREALZ_BUDG    = $REMREALZ_BUDG - $REALZ_AMOUNT;

                            $TOTREALZ_AM          = $TOTREALZ_AM + $REALZ_AMOUNT;
                            $TOTREMREALZ_BUDG     = $REMREALZ_BUDG;
                            ...
                            ?>
                                <tr>
                                    ...
                                    <td nowrap style="text-align:center;border-bottom-width:2px; border-bottom-color:#000;"><?php echo $REALZ_CODE; ?></td>
                                    <td width="300" style="text-align:left;border-bottom-width:2px; border-bottom-color:#000;"><?php echo $REALZ_DESC; ?></td>
                                    ...
                                    <td nowrap style="text-align:center;border-bottom-width:2px; border-bottom-color:#000;"><?php echo number_format($REALZ_VOLM, 2); ?></td>
                                    <td nowrap style="text-align:right;border-bottom-width:2px; border-bottom-color:#000;"><?php echo number_format($REALZ_PRICE, 2); ?></td>
                                    <td nowrap style="text-align:right;border-bottom-width:2px; border-bottom-color:#000;"><?php echo number_format($REALZ_AMOUNT, 2); ?></td>
                                    <td nowrap style="text-align:right;border-bottom-width:2px; border-bottom-color:#000;"><?php echo number_format($REMREALZ_BUDG, 2); ?></td>
               ...
               ?>
                <tr>
                    ...
                    <td nowrap style="text-align:center;border-bottom-width:2px; border-bottom-color:#000; text-align: right; font-weight: bold;"><?php echo number_format($TOTREALZ_AM, 2); ?></td>
