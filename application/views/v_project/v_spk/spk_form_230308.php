		                            ...
		                            if($task == 'edit')
		                            {
		                                /*$sqlDET	= "SELECT A.*,
													...
													WHERE A.WO_NUM = '$WO_NUM'
														AND B.PRJCODE = '$PRJCODE'";*/
		                                $sqlDET	= "SELECT A.*,
														C.JOBDESC, C.JOBPARENT, C.ITM_PRICE AS ITM_LASTP, C.ITM_BUDG, C.AMD_VOL, C.AMD_VAL, C.AMDM_VOL, C.AMDM_VAL
													FROM tbl_wo_detail A
														INNER JOIN tbl_joblist_detail C ON A.JOBCODEID = C.JOBCODEID
															AND C.PRJCODE = '$PRJCODE'
															AND A.ITM_CODE = C.ITM_CODE
										...
										foreach($result as $row) :
											...
											$ITM_BUDG_VOL1	= $row->ITM_BUDG_VOL;
											//$ITM_BUDG_AMN	= $row->ITM_BUDG_AMN;
											$ITM_BUDG_AMN1	= $row->ITM_BUDG;
											$AMD_VOL		= $row->AMD_VOL;
											$AMD_VAL		= $row->AMD_VAL;
											$AMDM_VOL		= $row->AMDM_VOL;
											$AMDM_VAL		= $row->AMDM_VAL;

											$ITM_BUDG_VOL 	= $ITM_BUDG_VOL1 + $AMD_VOL - $AMDM_VOL;
											$ITM_BUDG_AMN 	= $ITM_BUDG_AMN1 + $AMD_VAL - $AMDM_VAL;
											$itemConvertion	= 1;
											...
											// START : CANCEL VOL. PER ITEM
												$canShwRow 	= "$WO_NUM~$PRJCODE~$ITM_CODE~$ITM_NAME~$JOBDESCH~$WO_VOLM~$OPN_VOLM~$REM_VOLWO~$ITM_UNIT2~$WO_ID";
...
<script>
	...
	function getWOVol(thisVal1, row)
	{
		...
		{
			console.log('WO_TOTAL = '+WO_TOTAL+' > ITM_REM_AMN = '+ITM_REM_AMN)
			if(WO_TOTAL > ITM_REM_AMN)
			{
				swal('<?php echo $alertGreat; ?>',
				{
	...
	function getConvertion(thisVal1, row)
	{
		...
		// CHECK DOKUMEN KONTRAK, JIKA TOTAL OP/SPK > 500 JT WAJIB ISI DOK. KONTRAK
			var WO_REFNO 	= document.getElementById('WO_REFNO').value;
			if(parseFloat(GTOTAL_WO) > 500000000 && WO_REFNO.trim() == '')
			{
				swal('Total SPK ini lebih dari 500.000.000! Anda DIHARUSKAN mengisi no kontrak',
				{
					icon: "warning",
