...
if($task == 'add')
{
	...
	$WOYEAR			= date('y');
	$WOMONTH		= date('m');
	$WO_CODE		= "$WOCODE.$WOYEAR.$WOMONTH-PK"; // MANUAL CODE
						...
						<div class="box box-primary">
							...
							<div class="box-body">
				            	...
				            	<div class="form-group">
				                    ...
				                    <div class="col-sm-2">
				                    	<label for="inputName" class="col-sm-10 control-label">Selesai</label>
				                    </div>
				                    <div class="col-sm-4">
				                    	<div class="input-group date">
				                        <div class="input-group-addon">
				                ...
				                <script>
									function changeCODE(thisVal)
									{
										...
										var WO_CODE 	= WOCODE.substring(0, len-3);
										if(thisVal == 'MDR')
										{
											WO_CODEN	= WO_CODE+'-PK';
			                                    ...
			                                    <td style="text-align:right; vertical-align: middle;" nowrap> <!-- Item Request Now -- PR_VOLM -->
			                                        <?php
														...
														else
														{
						                                    ...
						                                    <?php } else { ?>
					                                        	<input type="text" name="WO_VOLM<?php echo $currentRow; ?>" id="WO_VOLM<?php echo $currentRow; ?>" value="<?php echo number_format($ITM_STOCK, 2); ?>" class="form-control" style="min-width:100px; max-width:100px; text-align:right" onKeyPress="return isIntOnlyNew(event);" onBlur="getWOVol(this,<?php echo $currentRow; ?>);" >
																<input type="hidden" name="data[<?php echo $currentRow; ?>][WO_VOLM]" id="data<?php echo $currentRow; ?>WO_VOLM" value="<?php echo $ITM_STOCK; ?>" class="form-control" style="max-width:300px;" >
							                                    <?php
							                                }
														}
...
<script>
	...
	function add_item(strItem)
	{
		...
		// Item Worked Now -- WO_VOLM (NOW) : WO_VOLM
			...
			{
				WOVOLM 			= parseFloat(ITM_STOCK);
				objTD.innerHTML = '<input type="text" name="WO_VOLM'+intIndex+'" id="WO_VOLM'+intIndex+'" value="'+doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(ITM_STOCK)), 2))+'" class="form-control" style="min-width:100px; max-width:100px; text-align:right" onKeyPress="return isIntOnlyNew(event);" onBlur="getWOVol(this,'+intIndex+');" ><input type="hidden" name="data['+intIndex+'][WO_VOLM]" id="data'+intIndex+'WO_VOLM" value="'+ITM_STOCK+'" class="form-control" style="max-width:300px;" >';
			}
	...
	function getWOVol(thisVal1, row)
	{
		...
		ITM_STOCK		= parseFloat(document.getElementById('ITM_STOCK'+row).value);			// Remain Budget (Vol)
		ITM_STOCK_AMN	= parseFloat(document.getElementById('ITM_STOCK_AMN'+row).value);		// Remain Budget (Amn)

		//console.log(WO_TOTAL+' > '+ITM_STOCK_AMN)
		if(WO_TOTAL > ITM_STOCK_AMN)
		{
		...
		else
		{
			...
			document.getElementById('WO_VOLM'+row).value			= doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(thisVal)), 2));
		}
		WO_VOLM		= parseFloat(document.getElementById('data'+row+'WO_VOLM').value);
		//console.log(WO_VOLM+' | '+ITM_STOCK+' * '+WO_PRICE)
		WO_TOTAL 		= parseFloat(WO_VOLM) * parseFloat(WO_PRICE);

		document.getElementById('data'+row+'WO_TOTAL').value 	= WO_TOTAL;
		document.getElementById('WO_TOTAL'+row).value			= doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(WO_TOTAL)), 2));
		document.getElementById('data'+row+'WO_TOTAL2').value 	= WO_TOTAL;
		document.getElementById('WO_TOTAL2X'+row).value			= doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(WO_TOTAL)), 2));
		getTotalWO();
	...
	function getWOPrice(thisVal1, row)
	{
		...
		//document.getElementById('data'+row+'ITM_PRICE').value	= ITM_PRICE;
	...
	function getWOPPN(TAX_NUM, row)
	{
		...
		if(TAX_NUM == '')
		{
			...
			document.getElementById('data'+row+'WO_TOTAL2').value 	= WOTOTAL;
	...
	function getTotalWO()
	{
		...
		document.getElementById('WO_VALUE').value = GTotWO;
		document.getElementById('WO_VALUE1').value = doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(GTotWO)), 2));
	}
