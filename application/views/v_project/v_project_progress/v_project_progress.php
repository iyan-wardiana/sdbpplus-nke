	...
	Highcharts.chart('line-chartx', {
		...
		series: [{
			...
			data: [<?php
						$jumTotPlanAkuma 	= '';
						$GraphicTitleText	= 3;
						$sqla   	= "SELECT A.PRJCODE, B.PRJNAME
										FROM tbl_mcheader A
										INNER JOIN tbl_project B ON B.PRJCODE = A.PRJCODE
										WHERE A.PRJCODE = '$collProject1'
										GROUP BY A.PRJCODE";
						$ressqla 	= $this->db->query($sqla)->result();
						
						foreach($ressqla as $rowa) :
							$proj_Code 	= $rowa->PRJCODE;
							$projName 	= $rowa->PRJNAME;
							$proj_Name	= "Project : $proj_Code";
							
							$getCountx		= "tbl_mcheader WHERE PRJCODE = '$collProject1'";
							$resGetCountx	= $this->db->count_all($getCountx);
							
							$NoU 		= 0;
							$sql0   	= "SELECT MAX(day(MC_DATE)) AS myDay, MAX(MONTH(MC_DATE)) AS myMonth, 
												MAX(YEAR(MC_DATE)) AS myYear, MAX(MC_DATE) AS myDate,
												MAX(MC_PROG) AS MC_PROG, MAX(MC_PROG) AS MC_PROG, 1 AS isShow, 
												1 AS isShowRA, 1 AS isShowDev
											FROM tbl_mcheader WHERE PRJCODE = '$collProject1'
											GROUP BY day(MC_DATE), month(MC_DATE), year(MC_DATE) ORDER BY MC_DATE";
							$ressql0 	= $this->db->query($sql0)->result();
							foreach($ressql0 as $row0) :
								$NoU 			= $NoU + 1;
								$DayAx 			= $row0->myDay;
								$myDate			= $row0->myDate;
								$Prg_PlanAkum	= $row0->MC_PROG;
								$Prg_RealAkum	= $row0->MC_PROG;
								$Prg_Dev2 		= $Prg_RealAkum - $Prg_PlanAkum;
								$isShow			= $row0->isShow;
								$isShowRA		= $row0->isShowRA;
								$isShowDev		= $row0->isShowDev;
