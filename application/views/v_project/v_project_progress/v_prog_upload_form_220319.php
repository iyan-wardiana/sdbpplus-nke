		...
		if($resCountPRGG == 0)
		{
			if(isset($_POST['PROGG_CODEX']))
			{
				...
				for ($i=2; $i <= $baris; $i++)
				{
					...
					endforeach;
					$proj_Code_HO 	= $PRJCODE_HO;					// C

					$progress_Type	= 3;							// D - OK - Ditetapkan ke 3 = Progress
					$Prg_Year		= substr($Prg_Date1, 0,4);		// E
					$Prg_Month 		= (int)substr($Prg_Date1, 5,2);	// F
					...
					$Prg_Real		= $data->val($i, 14);			// N - OK
					$Prg_RealAkum	= $data->val($i, 15);			// O - OK
					$Prg_Dev		= $data->val($i, 16);			// P - OK
					$Prg_ProjNotes	= "";							// Q - OK
					$Prg_PstNotes	= "";							// R - OK
					$isShow			= 1;							// S - OK
					$isShowRA		= 0;							// T - OK
					$isShowDev		= 0;							// U - OK
					$lastStepPS		= 0;							// V - OK
					$Prg_LastUpdate	= date('Y-m-d');				// W - OK
					if($Prg_LastUpdate == '')
