 ...
 * File Name	= v_document_log_frame.php
*/
?>
...
<table width="100%">
    ...
    <tr id="showLoc">
        <td width="71%" style="vertical-align:top" nowrap>
        <form id="myform" id="myform" method="post"> 
        	<input type="text" name="myIndex" id="myIndex" value=""  style="display:none" />
        	<ul id="treemenu1" class="treeview">
        	<?php
				$proj_CodeX    	= $this->session->userdata['dtSessSrc1']['selSearchproj_Code'];
				
				// LOOP 1
				$rowVal = 0;
        		$sqlA	= "SELECT * FROM tdocument WHERE doc_level = 1 AND proj_Code = '$proj_CodeX'";
				...
				foreach($resDoccumentA as $rowA) :
                                        ...
                                        <?php
                                            ...
                                            foreach($resDoccumentC as $rowC) :
																	...
																	<?php
																		...
																		foreach($resDoccumentE as $rowE) :
																								...
																								<?php
																									...
																									foreach($resDoccumentG as $rowG) :
																														...
																														<ul>
																															<?php
																																...
																																foreach($resDoccumentI as $rowI) :
																																	...
																																	$rowVal = $rowVal + 1;
																																	?>
																																	<li><a href="javascript: submitform('<?php echo $rowVal; ?>')"><?php echo $doc_nameI; ?> </a> <input type='text' name='menuID_<?php echo $rowVal; ?>' id='menuID_<?php echo $rowVal; ?>' value="<?php echo $doc_nameI; ?>" style="display:none" disabled><input type="text" name="indexMenu_<?php echo $rowVal; ?>" id="indexMenu_<?php echo $rowVal; ?>" value="<?php echo $rowVal; ?>" style="display:none" disabled /> <input type="radio" name="myFileName" id="myFileName" value="<?php echo $doc_nameI; ?>" onclick="getFileName('<?php echo $doc_nameI; ?>');" <?php if($doc_nameI == $FileUpName) { ?> checked <?php } ?> /></li>
																																	<?php
																																endforeach;
																															?>
																														</ul>
																										...
																										else
																										{
																											$rowVal = $rowVal + 1;
																											?>
																												<li> <a href="javascript: submitform('<?php echo $rowVal; ?>')"><?php echo $doc_nameG; ?> </a> <input type='text' name='menuID_<?php echo $rowVal; ?>' id='menuID_<?php echo $rowVal; ?>' value="<?php echo $doc_nameG; ?>" style="display:none" disabled><input type="text" name="indexMenu_<?php echo $rowVal; ?>" id="indexMenu_<?php echo $rowVal; ?>" value="<?php echo $rowVal; ?>" style="display:none" disabled /> <input type="radio" name="myFileName" id="myFileName" value="<?php echo $doc_nameG; ?>" onclick="getFileName('<?php echo $doc_nameG; ?>');" <?php if($doc_nameG == $FileUpName) { ?> checked <?php } ?> /></li>
																											<?php
																										}
																										// END COPY
																									endforeach;
																			...
																			else
																			{
																				$rowVal = $rowVal + 1;
																				?>
																					<li> <a href="javascript: submitform('<?php echo $rowVal; ?>')"><?php echo $doc_nameE; ?> </a> <input type='text' name='menuID_<?php echo $rowVal; ?>' id='menuID_<?php echo $rowVal; ?>' value="<?php echo $doc_nameE; ?>" style="display:none" disabled><input type="text" name="indexMenu_<?php echo $rowVal; ?>" id="indexMenu_<?php echo $rowVal; ?>" value="<?php echo $rowVal; ?>" style="display:none" disabled /> <input type="radio" name="myFileName" id="myFileName" value="<?php echo $doc_nameE; ?>" onclick="getFileName('<?php echo $doc_nameE; ?>');" <?php if($doc_nameE == $FileUpName) { ?> checked <?php } ?> /></li>
																				<?php
																			}
																			// END COPY
																		endforeach;
												...
												else
												{
													$rowVal = $rowVal + 1;
													?>
                                                        <li> <a href="javascript: submitform('<?php echo $rowVal; ?>')"><?php echo $doc_nameC; ?> </a> <input type='text' name='menuID_<?php echo $rowVal; ?>' id='menuID_<?php echo $rowVal; ?>' value="<?php echo $doc_nameC; ?>" style="display:none" disabled><input type="text" name="indexMenu_<?php echo $rowVal; ?>" id="indexMenu_<?php echo $rowVal; ?>" value="<?php echo $rowVal; ?>" style="display:none" disabled /> <input type="radio" name="myFileName" id="myFileName" value="<?php echo $doc_nameC; ?>" onclick="getFileName('<?php echo $doc_nameC; ?>');" <?php if($doc_nameC == $FileUpName) { ?> checked <?php } ?> /></li>
                                                    <?php
												}			
                                            endforeach;
                                        ?>
					...
					else
					{
						$rowVal = $rowVal + 1;
						?>
							<li> <a href="javascript: submitform('<?php echo $rowVal; ?>')"><?php echo $doc_nameA; ?> </a> <input type='text' name='menuID_<?php echo $rowVal; ?>' id='menuID_<?php echo $rowVal; ?>' value="<?php echo $doc_name; ?>" style="display:none" disabled><input type="text" name="indexMenu_<?php echo $rowVal; ?>" id="indexMenu_<?php echo $rowVal; ?>" value="<?php echo $rowVal; ?>" style="display:none" disabled /> <input type="radio" name="myFileName" id="myFileName" value="<?php echo $doc_nameA; ?>" onclick="getFileName('<?php echo $doc_nameA; ?>');" <?php if($doc_nameA == $FileUpName) { ?> checked <?php } ?> /></li>
                        <?php
					}				
				endforeach;
			?>            
        </ul>
        <input type="hidden" name="isChecking" id="isChecking" value="" />
        File yang dipilih : <input type="text" name="myTempFileName" id="myTempFileName" value="" style="display:none" /><input type="text" name="myTempFileName1" id="myTempFileName1" value="<?php echo $FileUpName; ?>" class="textbox" disabled />
        </form> 
          <script type="text/javascript">
    	...
    	<script type="text/javascript">
			function getFileName(myaValue)
			{
				document.getElementById('isChecking').value = 1;
				document.getElementById('myTempFileName').value = myaValue;
				document.getElementById('myTempFileName1').value = myaValue;
			}
