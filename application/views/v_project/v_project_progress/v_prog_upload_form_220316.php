	...
	<?php
		...
		if($resCountPRGG == 0)
		{
			if(isset($_POST['PROGG_CODEX']))
			{
				...
				$PROGG_FN		= '';
				$PROGG_CODEX	= $_POST['PROGG_CODEX'];
				$PROGG_CODE		= $_POST['PROGG_CODEX'];
				...
				$myXlsFile	= "$PROGG_FN";
				$myPath 	= "import_excel/import_proggress/$myXlsFile";
				//echo "myPath = $myPath";
				...
				for ($i=2; $i <= $baris; $i++)
				{
					$proj_Code 		= $data->val($i, 2);		// B - OK
					$proj_Code_HO	= $data->val($i, 3);		// B - OK
					$progress_Type	= 3;						// C - OK - Ditetapkan ke 3 = Progress
					$Prg_Year		= $data->val($i, 5);		// E - OK
					$Prg_Month 		= $data->val($i, 6);		// F - OK
					$Prg_Step 		= $data->val($i, 7);		// G - OK
					$Prg_Date1		= $data->val($i, 8);		// H - OK
					$Prg_Date2		= $data->val($i, 9);		// I - OK
					$Prg_Plan		= $data->val($i, 10);		// J - OK
					$Prg_PlanAkum	= $data->val($i, 11);		// K - OK
					$Prg_Real		= $data->val($i, 14);		// L - OK
					$Prg_RealAkum	= $data->val($i, 15);		// M - OK
					$Prg_Dev		= $data->val($i, 16);		// N - OK
					$Prg_ProjNotes	= $data->val($i, 17);		// O - OK
					$Prg_PstNotes	= $data->val($i, 18);		// P - OK
					$isShow			= $data->val($i, 19);		// Q - OK
					$isShowRA		= $data->val($i, 20);		// R - OK
					$isShowDev		= $data->val($i, 21);		// S - OK
					$lastStepPS		= $data->val($i, 22);		// T - OK
					$Prg_LastUpdate	= $data->val($i, 23);		// U - OK
					if($Prg_LastUpdate == '')
						$Prg_LastUpdate	= '0000-00-00 00:00:00';
