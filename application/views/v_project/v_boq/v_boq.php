...
<html>
	...
	<?php
        ...
        if($LangID == 'IND')
        {
            ...
            $alert3 = "Anda Yakin?";
            $alert4 = "Proses ini akan me-reset data BOQ sebelumnya.";
            $alert5 = "Baik! Proses akan dilanjutkan. Mohon tunggu beberapa saat.";
            $alert6 = "Baik! Proses reset master BOQ akan dibatalkan.";
        }
        else
        {
            ...
            $alert3 = "Are you sure?";
            $alert4 = "This process will reset the previous BOQ.";
            $alert5 = "Well! The process will be processed. Please wait a few moments.";
            $alert6 = "Well! The BOQ master reset process will be canceled.";
        }
		...
		if($resCountBOQ == 0)
		{
			if(isset($_POST['BOQH_CODEX']))
			{
				...
				$sqlBoQ		= "SELECT BOQH_FN FROM tbl_boq_hist WHERE BOQH_CODE = '$BOQH_CODEX'";
				...
				$myXlsFile	= "$BOQH_FN";
				$myPath 	= "import_excel/import_boq/$myXlsFile";
				...
				for ($i=2; $i <= $baris; $i++)
				{
					...
					$JOBCODE		= $data->val($i, 5);				// F
					$JOBCODEID_P	= $data->val($i, 6);				// E
					$PRJCODE		= $data->val($i, 7);				// G
					$PRJCODE_HO		= $data->val($i, 8);				// H
					$PRJPERIOD		= $data->val($i, 9);				// I
					$PRJPERIOD_P	= $data->val($i, 10);				// J
					$JOBDESC		= addslashes($data->val($i, 13));	// K
					$ITM_GROUP		= $data->val($i, 14);				// L
					$GROUP_CATEG	= $data->val($i, 15);				// M
					$ITM_CODE		= $data->val($i, 16);				// N
					$ITM_UNIT		= $data->val($i, 17);				// O
					$ITM_VOLM1		= $data->val($i, 18);				// P
					$ITM_VOLM 		= preg_replace('/[^A-Za-z0-9\.]/', '', $ITM_VOLM1);
					if($ITM_VOLM == '')
						$ITM_VOLM	= 0;
					$ITM_PRICE1		= $data->val($i, 19);				// Q
					$ITM_PRICE 		= preg_replace('/[^A-Za-z0-9\.]/', '', $ITM_PRICE1);
					if($ITM_PRICE == '')
						$ITM_PRICE	= 0;
					$ITM_LASTP1		= $data->val($i, 20);				// R
					$ITM_LASTP 		= preg_replace('/[^A-Za-z0-9\.]/', '', $ITM_LASTP1);
					if($ITM_LASTP == '')
						$ITM_LASTP	= 0;
					$ITM_AVGP		= $ITM_LASTP;						// R
					$ITM_BUDG1		= $data->val($i, 22);				// S
					$ITM_BUDG 		= preg_replace('/[^A-Za-z0-9\.]/', '', $ITM_BUDG1);
					if($ITM_BUDG == '')
						$ITM_BUDG	= 0;
					$BOQ_VOLM1		= $data->val($i, 23);				// T
					$BOQ_VOLM 		= preg_replace('/[^A-Za-z0-9\.]/', '', $BOQ_VOLM1);
					if($BOQ_VOLM == '')
						$BOQ_VOLM	= 0;
					$BOQ_PRICE1		= $data->val($i, 24);				// U
					$BOQ_PRICE 		= preg_replace('/[^A-Za-z0-9\.]/', '', $BOQ_PRICE1);
					if($BOQ_PRICE == '')
						$BOQ_PRICE	= 0;
					$BOQ_BUDG1		= $data->val($i, 25);				// V
					$BOQ_BUDG 		= preg_replace('/[^A-Za-z0-9\.]/', '', $BOQ_BUDG1);
					if($BOQ_BUDG == '')
						$BOQ_BUDG	= 0;
					$BOQ_BOBOT1		= $data->val($i, 26);				// W
					$BOQ_BOBOT 		= preg_replace('/[^A-Za-z0-9\.]/', '', $BOQ_BOBOT1);
					if($BOQ_BOBOT == '')
						$BOQ_BOBOT	= 0;
					$ISBOBOT1		= $data->val($i, 27);				// X
					$ISBOBOT 		= preg_replace('/[^A-Za-z0-9\.]/', '', $ISBOBOT1);
					if($ISBOBOT == '')
						$ISBOBOT	= 0;
					$IS_LEVEL		= $data->val($i, 52);				// AW
					$ISLAST			= $data->val($i, 54);				// AY
					$Patt_Number	= $data->val($i, 56);				// BA
					...
					// INSERT INTO BOQ
					$sqlInsBoQ		= "INSERT INTO tbl_boqlist (JOBCODEID, JOBCODEIDV, JOBPARENT, JOBCODEID_P, PRJCODE, PRJCODE_HO, PRJPERIOD, PRJPERIOD_P,
											ITM_CODE, JOBDESC, JOBGRP, JOBUNIT, JOBLEV, JOBVOLM, PRICE, JOBCOST, BOQ_VOLM, 
											BOQ_PRICE, BOQ_JOBCOST, BOQ_BOBOT, ISBOBOT, ISHEADER, ISLAST, Patt_Number)
										VALUES ('$JOBCODEID', '$JOBCODEID', '$JOBPARENT', '$JOBCODEID_P', '$PRJCODE', '$PRJCODE_HO', '$PRJPERIOD', '$PRJPERIOD_P',
											'$ITM_CODE', '$JOBDESC','$ITM_GROUP', '$ITM_UNIT', '$IS_LEVEL', '$ITM_VOLM', '$ITM_PRICE', '$ITM_BUDG', '$BOQ_VOLM', 
											'$BOQ_PRICE', '$BOQ_BUDG', '$BOQ_BOBOT', '$ISBOBOT', '$ISHEADER', '$ISLAST', '$Patt_Number')";
					$this->db->query($sqlInsBoQ);
					
					// INSERT INTO Joblist
					$sqlInsJL		= "INSERT INTO tbl_joblist (JOBCODEID, JOBCODEIDV, JOBPARENT, JOBCODEID_P, PRJCODE, PRJCODE_HO, PRJPERIOD, PRJPERIOD_P,
											ITM_CODE, JOBDESC, JOBGRP, JOBUNIT, JOBLEV, JOBVOLM, PRICE, JOBCOST, BOQ_VOLM, 
											BOQ_PRICE, BOQ_JOBCOST, BOQ_BOBOT, ISBOBOT, ISHEADER, ISLAST, Patt_Number)
										VALUES ('$JOBCODEID', '$JOBCODEID', '$JOBPARENT', '$JOBCODEID_P', '$PRJCODE', '$PRJCODE_HO', '$PRJPERIOD', '$PRJPERIOD_P', 
											'$ITM_CODE', '$JOBDESC', '$ITM_GROUP', '$ITM_UNIT', '$IS_LEVEL', '$ITM_VOLM', '$ITM_PRICE', '$ITM_BUDG', '$BOQ_VOLM',
					...
					$sqlInsJLD		= "INSERT INTO tbl_joblist_detail (JOBCODEDET, JOBCODEID, JOBPARENT, JOBCODE, JOBCODEID_P, PRJCODE, PRJCODE_HO, PRJPERIOD, PRJPERIOD_P,
											JOBDESC, ITM_GROUP, GROUP_CATEG, ITM_CODE, ITM_UNIT, ITM_VOLM, ITM_PRICE, ITM_LASTP, ITM_AVGP, 
											ITM_BUDG, BOQ_VOLM, BOQ_PRICE, BOQ_JOBCOST, BOQ_BOBOT, ISBOBOT, IS_LEVEL, ISLAST, Patt_Number)
										VALUES ('$JOBCODEDET', '$JOBCODEID', '$JOBPARENT', '$JOBCODE', '$JOBCODEID_P', '$PRJCODE', '$PRJCODE_HO', '$PRJPERIOD', '$PRJPERIOD_P',
											'$JOBDESC', '$ITM_GROUP', '$GROUP_CATEG', '$ITM_CODE', '$ITM_UNIT', '$ITM_VOLM', '$ITM_PRICE', '$ITM_LASTP', '$ITM_LASTP', 
											'$ITM_BUDG', '$BOQ_VOLM', '$BOQ_PRICE', '$BOQ_BUDG', '$BOQ_BOBOT', '$ISBOBOT', '$IS_LEVEL', '$ISLAST', '$Patt_Number')";
				                ...
				                <?php
				                    ...
				                    if($resBOQHC >0)
				                    {
										...
										foreach($resBOQH as $row) :
											...
											//$secUpd			= site_url('c_project/c_boq/update/?id='.$this->url_encryption_helper->encode_url($BOQH_CODE));
											...
											?> 
				                                    ...
				                                    <?php
				                                        $FileName 	= $BOQH_FN;
				                                        $fileAttach	= base_url('import_excel/import_boq/'.urldecode($FileName));
				                                        $collLink	= "$fileAttach~$FileName";
				                                        $linkDL1 	= site_url('c_project/c_boq/downloadFile/?id='.$this->url_encryption_helper->encode_url($collLink));
				                                        $secVWlURL	= site_url('c_project/c_boq/view_boq/?id='.$this->url_encryption_helper->encode_url($BOQH_CODE));
				                                    ?>
