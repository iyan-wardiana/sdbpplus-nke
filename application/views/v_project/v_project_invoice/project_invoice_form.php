...
if($task == 'add')
{
	...
	// START : DEFAULT VALUE
		...
		$PINV_DATE 			= "$PINV_DATED/$PINV_DATEM/$PINV_DATEY";
		...
		$PINV_ENDDATE 		= date("d/m/Y",strtotime("+$PINV_ENDDATE1 days",$PINV_ENDDATE0));
...
else
{
	...
	$JournalY 			= date('Y', strtotime($PINV_DATEX));
	$JournalM 			= date('n', strtotime($PINV_DATEX));
	...
	$PINV_ENDDATE 		= date("d/m/Y",strtotime($PINV_ENDDATEX));
...
<html>
    ...
    <body class="<?php echo $appBody; ?>" style="background-color: <?=$comp_color?>">
		<section class="content-header">
			<h1>
		    <img src="<?php echo base_url() . 'assets/AdminLTE-2.0.5/dist/img/icon/invoice.png'; ?>" style="max-width:40px; max-height:40px" >&nbsp;&nbsp;<?php echo "$h1_title ($PRJCODE)"; ?>
	                        	...
	                        	<div class="col-sm-4">
	                                <div class="form-group">
		                                ...
		                                <div class="input-group date">
		                            		...
		                            		<i class="fa fa-calendar"></i>&nbsp;</div><input type="text" name="PINV_DATE" class="form-control pull-left" id="datepicker" value="<?php echo $PINV_DATE; ?>" style="width:100px" onChange="changeFDate(this.value)">
		                            	</div>
	                                </div>
	                            </div>
		                                ...
		                                <div class="form-group">
			                                ...
			                                <div class="input-group">
			                                    <div class="input-group-btn">
			                          			...
			                          			<input type="text" class="form-control" name="PINV_SOURCE1" id="PINV_SOURCE1" value="<?php echo $PINV_SOURCE_MN; ?>" data-toggle="modal" data-target="#mdl_addMC" <?php if($task != 'add') { ?> disabled <?php } ?> />
			                          		</div>
		                        	...
		                        	<div class="col-sm-4">
		                                <div class="form-group">
		                                	<label for="inputName">PPn DP</label>
			                          		<input type="text" class="form-control" style="text-align:right;" name="PINV_DPVALPPn1" id="PINV_DPVALPPn1" value="<?php print number_format($PINV_DPVALPPn, $decFormat); ?>" onBlur="getPPNValue(this)" >
                                                ...
                                                else
                                                {
													...
													?>
														<select name="PINV_STAT" id="PINV_STAT" class="form-control select2">
															<option value="1"<?php if($PINV_STAT == 1) { ?> selected <?php } ?>>New</option>
															<option value="2"<?php if($PINV_STAT == 2) { ?> selected <?php } ?>>Confirm</option>
						                                    <option value="3"<?php if($PINV_STAT == 3) { ?> selected <?php } ?> <?php if($disabled == 1) { ?> disabled <?php } ?>>Approved</option>
			...
			$.ajax({
				...
				success: function(response)
				{
					...
					if(isLockJ == 1)
					{
						...
						$('#PINV_STAT>option[value="3"]').attr('disabled','disabled');
					...
					else
					{
						...
						document.getElementById('divAlert').style.display   = 'none';
						$('#PINV_STAT>option[value="3"]').removeAttr('disabled','disabled');
						document.getElementById('btnSave').style.display    = '';
					}
					...
					if(isLockT == 1)
					{
						if(LockCateg == 1)
						{
							$('#alrtLockJ').css('display','');
							document.getElementById('divAlert').style.display   = 'none';
							$('#PINV_STAT').removeAttr('disabled','disabled');
							$('#PINV_STAT>option[value="3"]').attr('disabled','disabled');
							document.getElementById('btnSave').style.display    = '';
						}
						else
						{
							...
							$('#PINV_STAT>option[value="3"]').removeAttr('disabled','disabled');
							$('#PINV_STAT').attr('disabled','disabled');
...
<script>
	...
	function getDetail(strItem)
	{
		...
		if(pageFrom == 'MC')
		{
			...
			// Nilai PPn Prestasi Saat ini - not use
				PINV_PAYAKUMPPn			= Math.round(parseFloat(PINV_PPNPERC * PINV_PAYAKUM / 100));
		...
		else if(pageFrom == 'MCG')
		{
			...
			// OK - Nilai Prestasi
				...
				MC_PROGVAL			= parseFloat(MC_PROGVAL);
				MC_PROGVALPPn		= parseFloat(Math.round(PINV_PPNPERC * MC_PROGVAL / 100));			// PPn Nilai Prestasi
				document.getElementById('PINV_PROGVAL1').value 		= doDecimalFormatxx(RoundNDecimal(parseFloat(Math.abs(MC_PROGVAL)),2));
			...
			// OK - Pekerjaan Tambahan
				MC_VALADD			= parseFloat(MC_VALADD);
				MC_VALADDPPn		= parseFloat(Math.round(PINV_PPNPERC * MC_VALADD / 100));			// PPn Nilai Pekerjaan Tambah
				document.getElementById('PINV_VALADD').value 		= MC_VALADD;
			...
			// OK - Nilai Termasuk Pekerjaan Tambah
				...
				PINV_TOTPROGRESSPPn		= parseFloat(Math.round(PINV_PPNPERC * PINV_TOTPROGRESS / 100));	// PPn Nilai Termasuk Pekerjaan Tambah
			...
			// OK - Nilai Uang Muka
				...
				MC_DPVALPPn			= parseFloat(Math.round(PINV_PPNPERC * MC_DPVAL / 100));
				document.getElementById('PINV_DPVALPPn1').value 	= doDecimalFormatxx(RoundNDecimal(parseFloat(Math.abs(MC_DPVALPPn)),2));
				document.getElementById('PINV_DPVALPPn').value 		= MC_DPVALPPn;
			...
			// Nilai PPn Prestasi Saat ini - not use
				PINV_PAYAKUMPPn			= Math.round(parseFloat(PINV_PPNPERC * MC_TOTVAL / 100));
				document.getElementById('PINV_PAYAKUMPPn').value 	= PINV_PAYAKUMPPn;
				//swal(PINV_PAYAKUMPPn)
		...
		if(pageFrom == 'SIC')
		{
			...
			// OK - Nilai Prestasi
				...
				SIC_PROGVALPPn		= parseFloat(Math.round(PINV_PPNPERC * SIC_PROGVAL / 100));			// PPn Nilai Prestasi
    ...
    function getPPNValue(thisVal)
    {
		MC_PPNVAL	= thisVal.value.split(",").join("");
		document.getElementById('PINV_DPVALPPn').value 		= parseFloat(MC_PPNVAL);
		document.getElementById('PINV_DPVALPPn1').value 	= doDecimalFormatxx(RoundNDecimal(parseFloat(Math.abs(MC_PPNVAL)),2));
    }
	...
	{
		var arrDate 		= thisVal.split("/");
		var validDate 		= arrDate[1]+"/"+arrDate[0]+"/"+arrDate[2];
		// alert(validDate);
	...
	function changeDueDate(thisVal)
	{
		...
		var dateDesc	=  ("0" + datey.getDate()).slice(-2) + "/" + theMD + "/" + datey.getFullYear();
		//swal(dateDesc)
