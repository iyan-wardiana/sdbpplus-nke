...
if($task == 'add')
{
	...
	$Pattern_Length = 4;
	$s_pattLn 	= "SELECT Pattern_Length FROM tbl_docpattern WHERE Pattern_Code = 'PDS'";
	$r_pattLn 	= $this->db->query($s_pattLn);
	if($r_pattLn->num_rows() > 0)
	{
		foreach($r_pattLn->result() as $rPattLn):
			$Pattern_Length = $rPattLn->Pattern_Length;
		endforeach;
	}

	$sql 		= "SELECT RIGHT(PROP_CODE, $Pattern_Length) as maxNumber FROM tbl_bprop_header
					WHERE PRJCODE = '$PRJCODE' ORDER BY PROP_CODE DESC LIMIT 1";
	$result 	= $this->db->query($sql);
	if($result->num_rows() > 0)
	{
		foreach($result->result() as $row) :
			$myMax = intval($row->maxNumber);
			$myMax = $myMax+1;
		endforeach;
	}	else	{		$myMax = 1;	}
							...
							<div class="box-body">
				                ...
				                <div class="col-sm-7">
				                  	<div class="row">
				                    	...
				                    	<div id="PROP_CODE1" class="col-md-12" style="font-style: italic;">
...
<script>
	...
	{
		var task 		= "<?=$task?>";
		var DOCNUM		= document.getElementById('PROP_NUM').value;
		var DOCCODE		= document.getElementById('PROP_CODE').value;
		...
		$.ajax({
            ...
            success: function(response)
            {
				...
				var datePROP	= new Date(DOCDATE);
				var datePROPV	= datePROP.toLocaleDateString('id-ID', { year:"numeric", month:"long", day:"numeric"}) 
            	...
            	$('#PROP_CODE').val(docCode);
				$('#PROP_CODE1').html(docCode+' / '+datePROPV);
            }
        });
	}
	...
	function add_item(strItem)					// OK
	{
		...
		validateDouble(arrItem[4],arrItem[1])
		if(validateDouble(arrItem[4],arrItem[1]))
		{
	...
	function validateDouble(vcode, jobid)
	{
		...
		console.log('g')
				...
				var elitem2= document.getElementById('data'+i+'JOBCODEID').value;
		console.log('elitem1 = '+elitem1+' == '+vcode)
				if (elitem1 == vcode && elitem2 == jobid)
				{
					duplicate = true;
					break;
				}
