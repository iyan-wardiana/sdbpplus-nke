...
<?php
	...
	if(isset($_POST['RSV_CODE']))	// MARKETING
	{
		...
		elseif($CUR_STAT == 8)		// FINISH
		{
			...
			//Status Kendaraan update to avilable
			$sqlVHSTAT	= "UPDATE tbl_vehicle SET VH_STAT = 0 WHERE VH_CODE = '$CATEG_CODE2'";
			$this->db->query($sqlVHSTAT);
			
			//Status Kendaraan update to avilable
			$sqlDR	= "UPDATE tbl_driver SET DRIVER_STAT = 0 WHERE DRIVER_CODE = '$DRIVER_CODE'";
			$this->db->query($sqlDR);
			
			// SENT MAIL TO SUBMITTER
				//$RSV_MAIL	= "iyanwardiana@gmail.com";
				$sqlRsv		= "SELECT RSV_CODE, CATEG_CODE2, DRIVER_CODE, RSV_TITLE, RSV_DESC, RSV_STARTD2, RSV_ENDD2, RSV_MAIL
				...
				foreach($resRsv as $rowRsv):
					...
					//GET VH Merek
					$VH_MEREK		= '';
					$sqlVHM			= "SELECT VH_MEREK, VH_NOPOL FROM tbl_vehicle WHERE VH_CODE = '$CATEG_CODE1'";
					$resVHM			= $this->db->query($sqlVHM)->result();
					foreach($resVHM as $rowVHM):
						$VH_MEREK	= $rowVHM->VH_MEREK;
						$VH_NOPOL	= $rowVHM->VH_NOPOL;
					endforeach;
	...
	$sqlCAg		= "tbl_reservation WHERE (RSV_STARTD2 <= '$dateNow' OR RSV_ENDD2 >= '$dateNow') AND MONTH(RSV_STARTD2) = $NowMonth
						AND RSV_EMPID = '$DefEmp_ID' AND RSV_STAT IN (3,8) AND RSV_CATEG = 'VH'";
        ...
        <div class="box-body">
            <div class="row">
            	<?php
					if($resCAg > 0)
					{
						$sqlAg		= "SELECT RSV_CODE, CATEG_CODE2, DRIVER_CODE, RSV_TITLE, RSV_DESC, RSV_STARTD2, RSV_ENDD2, RSV_STAT
										FROM tbl_reservation 
										WHERE (RSV_STARTD2 <= '$dateNow' OR RSV_ENDD2 >= '$dateNow') AND MONTH(RSV_STARTD2) = $NowMonth
											AND RSV_EMPID = '$DefEmp_ID' AND RSV_STAT IN (3,8) AND RSV_CATEG = 'VH'";
						...
						foreach($resAg as $rowAg):
							...
							$DRIVER_CODE2	= $rowAg->DRIVER_CODE;
							//GET VH Merek
							$VH_MEREK		= '';
							$sqlVHM			= "SELECT VH_MEREK, VH_NOPOL FROM tbl_vehicle WHERE VH_CODE = '$CATEG_CODE2'";
							$resVHM			= $this->db->query($sqlVHM)->result();
							foreach($resVHM as $rowVHM):
								$VH_MEREK	= $rowVHM->VH_MEREK;
								$VH_NOPOL	= $rowVHM->VH_NOPOL;
							...
							//GET Driver
							$DRIVER			= '';
							$sqlDR			= "SELECT DRIVER FROM tbl_driver WHERE DRIVER_CODE = '$DRIVER_CODE2'";
							$resDR			= $this->db->query($sqlDR)->result();
							foreach($resDR as $rowDR):
								$DRIVER	= $rowDR->DRIVER;
							endforeach;
							...
							if($STARTDD1 == $STARTDD2)
								$STARTDDV	= $STARTDD1;
							else
								$STARTDDV	= "$STARTDD1 - $STARTDD2";
							...
							$DATECOMP		= "$STARTDDV $STARTDM $STARTDY";
                                ...
                                <div class="col-md-4">
                                    <div class="box box-warning box-solid">
                                        <div class="box-header with-border">
                                            <h3 class="box-title"><?php echo "$VH_MEREK - $VH_NOPOL"; ?></h3>
                                            ...
                                            <table width="100%" border="0">
                                                <tr>
                                                    <td colspan="2" style="text-align:center; font-size:18px">
                                                        ...
                                                        <input type="hidden" name="CATEG_CODE2<?php echo $theRow; ?>" id="CATEG_CODE2<?php echo $theRow; ?>" value="<?php echo $CATEG_CODE2; ?>">
                                                        <input type="hidden" name="DRIVER_CODE<?php echo $theRow; ?>" id="DRIVER_CODE<?php echo $theRow; ?>" value="<?php echo $DRIVER_CODE2; ?>">
                                                ...
                                                <tr>
                                                    <td colspan="2" style="text-align:center; font-size:36px; color:#060"><?php echo "$DATECOMP"; ?></td>
                                                </tr>
                                                <tr>
                                                    <td colspan="2" style="text-align:center; font-size:18px"><?php echo "$HOURSCOMP WIB"; ?></td>
                                                </tr>
                                                <tr>
                                                    <td colspan="2" style="text-align:center; font-style:italic"><b><?php echo "$Driver : $DRIVER"; ?></b></td>
                                                </tr>
                                                <tr>
                ...
                <form class="form-horizontal" name="form_x" method="post" action="" style="display:none">
                    ...
                    <input type="text" name="CATEG_CODE2" id="CATEG_CODE2" value="">
                    <input type="text" name="DRIVER_CODE" id="DRIVER_CODE" value="">
...
<script>
  $(function () 
  {
	...
	function chekData_1(theRow)
	{
		...
		CATEG_CODE2		= document.getElementById('CATEG_CODE2'+theRow).value;
		DRIVER_CODE		= document.getElementById('DRIVER_CODE'+theRow).value;
		CUR_STAT		= document.getElementById('CUR_STAT'+theRow).value;
		//alert(CUR_STAT);
		
		
		document.getElementById('RSV_CODE').value = RSV_CODE;
		document.getElementById('CATEG_CODE2').value = CATEG_CODE2;
		document.getElementById('DRIVER_CODE').value = DRIVER_CODE;
