 ...
 * Create Date	= 9 Febrruari 2018
 * File Name	= v_reservation_rr_form.php
 * Location		= -
*/
...
<?php
	...
	{
		$h1_title		= 'Pemesanan';
		$h2_title		= 'Ruang Apartemen';		
		$alert1			= 'Tanggal/waktu selesai harus lebih besar dari tanggal/waktu mulai.';
		$alert2			= 'Anda harus menentukan waktu mulai penggunaan ruang rapat.';
		$alert3			= 'Anda harus menentukan waktu selesai penggunaan ruang rapat.';
		$alert4			= 'Pilih salah satu kamar.';
		$alert5			= 'Tuliskan catatan pemesanan.';
		$alert6			= 'Format waktu salah.';
		$alert7			= 'Tuliskan acara yang sedang Anda lakukan.';
	...
	else
	{
		...
		$h2_title		= 'Apartment Room';
		$alert1			= 'End date/time must be greater than Start date/time.';
		$alert2			= 'You must specify the start time of using the meeting room.';
		$alert3			= 'You must specify the end time of using themeeting room.';
		$alert4			= 'Select a room.';
		$alert5			= 'Write the reservation note of the booking room.';
		$alert6			= 'Error time format.';
		$alert7			= 'Write the activities / events that you will do.';
            ...
            <div class="box box-primary">
                ...
                <div class="box-body chart-responsive">
                	...
                	<form class="form-horizontal" name="frm" id="frm" method="post" action="<?php echo $form_action; ?>" onSubmit="return chkForm()">
                        ...
                        <div class="form-group">
                          	...
                          	<div class="col-sm-10">
                                <div class="input-group date">
                                    ...
                                    <input type="text" name="RSV_STARTD" class="form-control pull-left" id="datepicker1" value="<?php echo $RSV_STARTD; ?>" style="width:107px" onChange="changeStartD(this.value)">
                                </div>
                          	</div>
                        </div>
                        <script>
							function changeStartD(startDate)
							{
								document.getElementById('datepicker2').value = startDate;
							}
						</script>
                        <div class="form-group">
                          	<label for="inputName" class="col-sm-2 control-label"><?php echo $Time ?> </label>
                          	<div class="col-sm-10">
                                <input type="text" class="form-control" style="max-width:80px" name="RSV_STARTT" id="RSV_STARTT" value="<?php echo $RSV_STARTT; ?>" onKeyUp="toTimeString1(this.value)" onChange="chkLength1(this.value)" >
                          	</div>
                        </div>
                        <script>
							function chkLength1(dateLength)
							{
								...
								if(totTxt < 5)
								{
										...
										document.getElementById('RSV_STARTT').value = '';
										document.getElementById('RSV_STARTT').focus();
								}
							}
							...
							function toTimeString1(RSV_STARTT)
							{
								...
								if(totTxt == 1)
								{
									isHour	= document.getElementById('RSV_STARTT').value;
								...
								if(totTxt == 2)
								{
									...
									{
										document.getElementById('RSV_STARTT').value = isHour+':';
										document.getElementById('RSV_STARTT').focus();
									}
								}
                        ...
                        <div class="form-group">
                          	...
                          	<div class="col-sm-10">
                                <div class="input-group date">
                                    <div class="input-group-addon">
                                    <i class="fa fa-calendar"></i>&nbsp;</div>
                                    <input type="text" name="RSV_ENDD" class="form-control pull-left" id="datepicker2" value="<?php echo $RSV_ENDD; ?>" style="width:107px">
                                </div>
                          	</div>
                        </div>
                        <div class="form-group">
                          	<label for="inputName" class="col-sm-2 control-label"><?php echo $Time ?> </label>
                          	<div class="col-sm-10">
                                <input type="text" class="form-control" style="max-width:80px" name="RSV_ENDT" id="RSV_ENDT" value="<?php echo $RSV_ENDT; ?>" onKeyUp="toTimeString2(this.value);" onChange="chkLength2(this.value)" >
                          	</div>
                        </div>
                        <script>
							function chkLength2(dateLength)
							{
								var totTxt 	= dateLength.length;
								...
								{
										alert('<?php echo $alert6; ?>');
										document.getElementById('RSV_ENDT').value = '';
										document.getElementById('RSV_ENDT').focus();
								}
							}
							...
							function toTimeString2(RSV_ENDT)
							{
								...
								if(totTxt == 1)
								{
									isHour	= document.getElementById('RSV_ENDT').value;
									if(!isHour.match(noHour))
									{
										alert('Range no [00 - 23]');
										document.getElementById('RSV_ENDT').value = '';
								...
								if(totTxt == 2)
								{
									...
									{
										document.getElementById('RSV_ENDT').value = isHour+':';
										document.getElementById('RSV_ENDT').focus();
									}
								}
								...
								RSV_ENDT	= document.getElementById('RSV_ENDT').value;
								//document.getElementById('AU_PROCT').value = RSV_ENDT;
							}
							
							function changeFinishD(RSV_ENDD)
							{
								RSV_STARTD	= document.getElementById('datepicker1').value;
								RSV_ENDD	= document.getElementById('datepicker2').value;
								if(RSV_STARTD > RSV_ENDD)
								{
                        ...
                        <div class="form-group">
                            <label for="inputName" class="col-sm-2 control-label"><?php echo $Room; ?></label>
                            <div class="col-sm-10">
                                <select name="CATEG_CODE" id="CATEG_CODE" class="form-control" style="max-width:250px">
                                    <option value="">--- None ---</option>
                                    <?php
                                        $CountCTG 	= $this->db->count_all('tbl_apartement');
                                        $sqlCTG 	= "SELECT AR_CODE, AR_NAME, AR_ROOM, AR_FLOOR, AR_STAT, AR_STATD FROM tbl_apartement";
                                        ...
                                        if($CountCTG > 0)
                                        {
                                            foreach($resCTG as $rowCTG) :
                                                ...
                                                $AR_ROOM 	= $rowCTG->AR_ROOM;
                                                $AR_FLOOR 	= $rowCTG->AR_FLOOR;
                                                ...
                                                ?>
                                                <option value="<?php echo $AR_CODE; ?>" <?php if($AR_CODE == $CATEG_CODE) { ?>selected <?php } if($isDisabled == 1) { ?> disabled <?php } ?>>
													<?php echo "$AR_NAME$isDisDesc"; ?>
                                                </option>
                                                <?php
											endforeach;
										}
                        ...
                        <div class="form-group">
                          	<label for="inputName" class="col-sm-2 control-label"><?php echo $Event; ?> </label>
                          	<div class="col-sm-10">
                                <input type="text" class="form-control" style="max-width:350px" name="RSV_TITLE" id="RSV_TITLE" value="<?php echo $RSV_TITLE; ?>" />
                          	</div>
                        </div>
                        <div class="form-group">
                        ...
                        <div class="form-group">
                          	...
                          	<div class="col-sm-10">
                                <input type="text" class="form-control" style="max-width:50px; text-align:right" name="RSV_QTY" id="RSV_QTY" value="<?php echo $RSV_QTY; ?>" />
                          	</div>
                        </div>
                        <div class="form-group">
								...
								<?php
                                    ...
                                    ?>
                                        <select name="RSV_STAT" id="RSV_STAT" class="form-control" style="max-width:100px" onChange="selStat(this.value)">
                                            <option value="1"<?php if($RSV_STAT == 1) { ?> selected <?php } ?>>New</option>
                                            <option value="2"<?php if($RSV_STAT == 2) { ?> selected <?php } ?>>Confirm</option>
                                            <option value="6"<?php if($RSV_STAT == 6) { ?> selected <?php } ?> style="display:none">Close</option>
                                        </select>
                                    <?php
                                    }
                                ?>
  ...
  $(function () {
   ...
   //Date picker
	...
	function chkForm()
	{
		RSV_STARTD	= document.getElementById('datepicker1').value;
		RSV_ENDD	= document.getElementById('datepicker2').value;
		RSV_STARTT	= document.getElementById('RSV_STARTT').value;
		RSV_ENDT	= document.getElementById('RSV_ENDT').value;
