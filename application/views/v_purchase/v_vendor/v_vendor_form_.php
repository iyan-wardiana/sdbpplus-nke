  ...
  <head>
    ...
    <body class="<?php echo $appBody; ?>" style="background-color: <?=$comp_color?>">
		...
		<?php
			...
			$PO_T 		= $PO_T;											// TOTAL PO
			$PO_PAID 	= $DEB_P;											// TOTAL PO DIBAYAR
			$PO_REM 	= $PO_T - $PO_PAID;
	        $POP_PERC	= number_format($PO_PAID / $PO_T * 100, 2);		// PERSENTASE DIBAYAR
            ...
            // ----------- DIBAYAR
				if($PO_PAID < 1000)									// Ratusan
	            {
	            	$PO_PAIDV = number_format($PO_PAID / 1, 2);
	            	$DBPCOD	= "";
	            }
				elseif($PO_PAID < 1000000)							// Juta per Seribu
	            {
	            	$PO_PAIDV = number_format($PO_PAID / 1000, 2);
	            	$DBPCOD	= " RB";
	            }
	            elseif($PO_PAID < 1000000000)							// Miliar per Sejuta
	            {
	            	$PO_PAIDV = number_format($PO_PAID / 1000000, 2);
	            	$DBPCOD	= " JT";
	            }
	            elseif($PO_PAID < 1000000000000)						// Triliun per Semiliar
	            {
	            	$PO_PAIDV = number_format($PO_PAID / 1000000000, 2);
	            	$DBPCOD	= " M";
	            }
	            else
	            {
	            	$PO_PAIDV = number_format($PO_PAID / 1000000000000, 2);
	            	$DBPCOD	= " T";
	            }
	            $DBP_V 		= $PO_PAIDV.$DBPCOD;
						...
						<div class="box box-warning">
							...
							<div class="box-body">
		                        ...
		                        <div class="form-group">
		                            ...
		                            <div class="col-sm-5">
		                                <input type="text" class="form-control" name="SPLUSERN" id="SPLUSERN" placeholder="Username" value="<?php echo "$SPLUSERN"; ?>">
		                            </div>
		                            <div class="col-sm-4">
		                                <input type="password" class="form-control" name="SPLPASSW" id="SPLPASSW" placeholder="Password" value="<?php echo "$SPLPASSW"; ?>">
		                            </div>
		                        </div>
		                        <div class="form-group">
