...
<head>
    ...
    <?php
        ...
        $INV_TOTPOT     = $INV_AMOUNT_PPH + $INV_AMOUNT_DPB + $INV_AMOUNT_POTUM + $INV_AMOUNT_OTH;

        $INV_TOTCOST    = $INV_DPPTOT + $INV_AMOUNT_PPN - $INV_TOTPOT;
                        ...
                        <?php
                            ...
                            // START : CACATAN 29-08-2022. BY DIAN HERMANTO
                                ...
                                $s_SQL      = "SELECT A.JournalH_Code, A.proj_Code, A.Acc_Id, A.Acc_Name, A.ITM_CODE, C.ITM_NAME, A.Journal_DK,
                                                   A.Base_Debet, A.Base_Kredit, A.Manual_No, A.Other_Desc
                                                ...
                                                WHERE A.JournalH_Code IN (SELECT A.TTK_REF1_NUM FROM tbl_ttk_detail A
                                                    INNER JOIN tbl_ttk_header B ON A.TTK_NUM = B.TTK_NUM AND A.PRJCODE = B.PRJCODE
                                                WHERE A.TTK_NUM IN (SELECT TTK_NUM FROM tbl_pinv_detail WHERE INV_NUM = '$INV_NUM') AND A.PRJCODE = '$PRJCODE') AND A.GEJ_STAT = 3";
                                ...
                                if($getJournal_Hutang->num_rows() > 0)
                                {
                                    ...
                                    foreach($r_SQL as $rJ):
                                        ...
                                        if($Journal_DK == 'K')
                                        {
                                            $JournalDesc = "<span>$SPLDESC</span><div class='text-muted' style='font-style: italic;'>$Acc_Name</div>";  // Hut Supplier
                                        }
