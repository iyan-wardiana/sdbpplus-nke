 ...
 * Create Date	= 05 Desember 2017
 * File Name	= v_pinv_sel_ir.php
 * Location		= -
*/
...
<div class="box-body">
    <div class="callout callout-success" style="vertical-align:top">
        <?php echo "$PRJCODE - $PRJNAME<br>Nama Supplier : $SPLDESC"; ?>
    </div>
	<div class="search-table-outter">
        ...
        <table id="example1" class="table table-bordered table-striped table-responsive search-table inner" width="100%">
            ...
            <?php
                ...
                {
                    $totRow	= 0;
                    foreach($viewAllTTK as $row) :
						$TTK_NUM 		= $row->TTK_NUM;
						$TTK_CODE 		= $row->TTK_CODE;
						$TTK_DATE 		= $row->TTK_DATE;
						$TTK_DATE1		= date_create($TTK_DATE);
                        $TTK_DATEV 		= date_format($TTK_DATE1,"d/m/Y");
						$TTK_DUEDATE 	= $row->TTK_DUEDATE;
						$TTK_DUEDATE1	= date_create($TTK_DUEDATE);
                        $TTK_DUEDATEV 	= date_format($TTK_DUEDATE1,"d/m/Y");
						$TTK_CATEG 		= $row->TTK_CATEG;
						$TTK_NOTES		= $row->TTK_NOTES;
						$TTK_AMOUNT1	= $row->TTK_AMOUNT;
						$TTK_AMOUNT_RET	= $row->TTK_AMOUNT_RET;
						$TTK_AMOUNT_POT	= $row->TTK_AMOUNT_POT;
						$TTK_AMOUNT_PPN	= $row->TTK_AMOUNT_PPN;
						$TTK_AMOUNT_OTH	= $row->TTK_AMOUNT_OTH;
						$TTK_STAT		= $row->TTK_STAT;
						$PRJCODE	 	= $row->PRJCODE;
						$SPLCODE 		= $row->SPLCODE;
                        $SPLDESC		= $row->SPLDESC;
                        $compName 		= "";
						//$TTK_AMOUNT		= $TTK_AMOUNT1 + $TTK_AMOUNT_RET + $TTK_AMOUNT_PPN;
						//$TTK_AMOUNT		= $TTK_AMOUNT1 - $TTK_AMOUNT_RET + $TTK_AMOUNT_PPN;
						$TTK_AMOUNT		= $TTK_AMOUNT1 - $TTK_AMOUNT_RET + $TTK_AMOUNT_OTH;
						$TTK_AMOUNTV	= $TTK_AMOUNT1 + $TTK_AMOUNT_PPN + $TTK_AMOUNT_OTH - $TTK_AMOUNT_POT - $TTK_AMOUNT_RET;
						
						$TTK_AMOUNTX	= $TTK_AMOUNT1;
						if($TTK_CATEG == 'IR')
						{
							$TTK_AMOUNT		= $TTK_AMOUNT1 + $TTK_AMOUNT_PPN + $TTK_AMOUNT_OTH - $TTK_AMOUNT_RET;
							$TTK_AMOUNTV	= $TTK_AMOUNT;
						}
                        ...
                        ?>
                        <td style="text-align:center"><input type="radio" name="chk" value="<?php echo $TTK_NUM;?>" onClick="pickThis(this);" /></td>
                        <td nowrap>
                            <a href="javascript:void(null);" onClick="showItem(<?php echo $totRow; ?>)" class="link">
                            <?php echo $TTK_CODE; ?></a>
                        </td>
                        <td style="text-align:center"><?php echo date('d M Y', strtotime($TTK_DATE)); ?></td>
                        <td style="text-align:center"><?php echo date('d M Y', strtotime($TTK_DUEDATE)); ?></td>
                        <td><?php echo $TTK_NOTES; ?></td>
                        <td style="text-align:right"><?php echo number_format($TTK_AMOUNTV, $decFormat); ?></td>
                        </tr>
                    <?php
                    endforeach;
                }
