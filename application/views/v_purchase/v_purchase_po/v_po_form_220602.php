                                    ...
                                    <?php
                                        ...
                                        if($resultC > 0)
                                        {
                                            ...
                                            foreach($result as $row) :
                                                ...
                                                // GET TOTAL ORDERED VOLUME
	                                                $ORDERED_VOL	= 0;
	                                                $s_TOTPOVOL		= "SELECT SUM(A.PO_VOLM) AS ORDERED_VOL 
	                                                                    FROM tbl_po_detail A
	                                                                    ...
	                                                                    WHERE B.PRJCODE = '$PRJCODE' AND A.JOBCODEDET = '$JOBCODEDET'
	                                                                        ...
	                                                                        AND A.PRD_ID = $PRD_ID";
	                                                $r_TOTPOVOL		= $this->db->query($s_TOTPOVOL)->result();
	                                                foreach($r_TOTPOVOL as $rw_TOTPOVOL) :
	                                                    $ORDERED_VOL= $rw_TOTPOVOL->ORDERED_VOL;
	                                                    if($ORDERED_VOL == '')
	                                                    	$ORDERED_VOL 	= 0;
	                                                endforeach;

	                                           	// GET TOTAL ORDERED VALUE -> SISA NILAI DIAMBIL DARI PEKERJAAN BUKAN PER REQUEST, PER DOK. REQUEST HANYA VOLUME
	                                                $ORDERED_VAL0	= 0;
	                                                ...
	                                                $s_TOTPOVAL		= "SELECT SUM(A.Base_Debet - A.Base_Kredit) AS ORDERED_VAL 
	                                                                    ...
	                                                                    WHERE B.proj_Code = '$PRJCODE' AND A.GEJ_STAT = 3
	                                                                        AND A.JOBCODEID = '$JOBCODEID'";
	                                                $r_TOTPOVAL		= $this->db->query($s_TOTPOVAL)->result();
	                                                foreach($r_TOTPOVAL as $rw_TOTPOVAL) :
	                                                    ...
	                                                    if($ORDERED_VAL2 == '')
	                                                    	$ORDERED_VAL2 	= 0;
	                                                endforeach;
	                                                $ORDERED_VAL 	= $ORDERED_VAL0 + $ORDERED_VAL1 + $ORDERED_VAL2;
                                                
                                                // GET BUDGET
	                                                $ITM_BUDGDAL	= 0;
	                                                $s_BUDGVAL		= "SELECT ITM_BUDG FROM tbl_joblist_detail WHERE JOBCODEID = '$JOBCODEID' AND PRJCODE = '$PRJCODE'";
