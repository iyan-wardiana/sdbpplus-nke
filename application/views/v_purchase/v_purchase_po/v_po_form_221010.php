						...
						<div class="box box-primary">
							...
							<div class="box-body">
				                ...
				                <input type="hidden" name="selITM" id="selITM" value="">
		           				<input type="text" name="rowCount" id="rowCount" value="0">
                        ...
                        <div class="box box-primary">
	                        <div class="search-table-outter">
	                            <table id="tblITMDET" class="table table-bordered table-striped" width="100%">
									...
									<input type="hidden" name="totalrowPR" id="totalrowPR" value="">
                                    <input type="text" name="totalrow" id="totalrow" value="">
                                </table>
                            </div>
                        </div>
                    </div>
	    	...
	    	<!-- ============ START MODAL =============== -->
				...
				<script type="text/javascript">
					...
					$(document).ready(function()
					{
						...
						// PO_REQVOLMx
						$('#PO_REQVOLMx1').bind('blur', function(){
							alert('PO_REQVOLMx');
						})
							...
							<div class="modal-body">
								<div class="row">
									<div class="col-md-12">
										...
										<input type="text" name="rowCheck4" id="rowCheck4" value="0">
									</div>
								</div>
							</div>
						</div>
...
<script>
	...
	function add_item(strItem) 
	{
		...
		ITM_REMAIN 		= arrItem[17];
		TOTVAL_OTHDOC	= arrItem[18];
		PO_VOLM 		= arrItem[19];
		ITM_PRICE 		= 0;
		PO_DISP 		= 0;
		PO_DISC 		= 0;
		...
		objTable 		= document.getElementById('tblITMDET');
		// var totalRowCount = table.rows.length; // 5
		// var tbodyRowCount = table.tBodies[0].rows.length; // 3
		intTable 		= objTable.tBodies[0].rows.length;
		alert(intTable);
		//swal('intTable = '+intTable)
		//intIndex = parseInt(document.frm.rowCount.value) + 1;
		intIndex = parseInt(objTable.tBodies[0].rows.length) - 1;
		//intIndex = intTable;
		// document.frm.rowCount.value = intIndex;
		
		objTR = objTable.tBodies[0].insertRow(intTable);
		objTR.id = 'tr_' + intIndex;
		...
		objTD.innerHTML = '<a href="#" onClick="deleteRow('+intIndex+')" title="Delete Document" class="btn btn-danger btn-xs"><i class="fa fa-trash-o"></i></a>'+
						  '<input type="Checkbox" id="data['+intIndex+'][chk]" name="data['+intIndex+'][chk]" value="'+intIndex+'" onclick="pickThis(this,'+intIndex+')" style="display:none">'+
		...
		objTD.innerHTML = ''+ITM_CODE+'<input type="hidden" id="data'+intIndex+'ITM_CODE" name="data['+intIndex+'][ITM_CODE]" value="'+ITM_CODE+'" width="10" size="15" readonly class="form-control" style="max-width:350px;text-align:right">'+
						  ...
						  '<input type="hidden" id="data'+intIndex+'PR_NUM" name="data['+intIndex+'][PR_NUM]" value="'+PR_NUMX+'" width="10" size="15">'+
		...
		objTD = objTR.insertCell(objTR.cells.length);
		objTD.align = 'left';
		objTD.innerHTML = ''+ITM_CODE+' : '+ITM_NAME+ '<span class="text-red" style="font-style: italic; font-weight: bold;">(Hrg.RAP : '+RAP_PRICE+')</span>'+
						  ...
						  '<span style="font-style: italic; font-weight: bold;">RAP : &nbsp;&nbsp;'+ITM_BUDGVAL+'</span>'+
						  ...
						  'S : '+ITM_REMVAL+'</span>'+
						  '</div>'+
						  '<input type="hidden" name="ITM_CATEG'+intIndex+'" id="ITM_CATEG'+intIndex+'" value="'+ITM_CATEG+'>';
		//swal(CSTDESC);

		console.log(intIndex+' => 3');
		
		// <!-- ITEM BUDGET -->
		if(ITM_CATEG != 'UA') PR_VOLMv = PR_VOLM;
		else PR_VOLMv = '-';
		objTD = objTR.insertCell(objTR.cells.length);
		objTD.style.textAlign = 'center';
		...
		// <!-- ITEM REMAIN -->
		if(ITM_CATEG != 'UA') ITM_REMVOLv = ITM_REMVOL;
		else ITM_REMVOLv = '-';
		objTD = objTR.insertCell(objTR.cells.length);
		objTD.style.textAlign = 'center';
		objTD.innerHTML = ''+ITM_REMVOLv+'<input type="hidden" class="form-control" style="min-width:130px; max-width:200px; text-align:right" name="ITM_REMAIN'+intIndex+'" id="ITM_REMAIN'+intIndex+'" value="'+ITM_REMVOL+'" >'+
		...
		objTD = objTR.insertCell(objTR.cells.length);
		objTD.style.textAlign = 'center';
		objTD.innerHTML = '<input type="text" class="form-control" style="min-width:90px; max-width:300px; text-align:right" name="PO_VOLMX'+intIndex+'" id="PO_VOLMX'+intIndex+'" value="'+PO_VOLM+'" onKeyPress="return isIntOnlyNew(event);" onBlur="getValuePO('+intIndex+');">'+
		...
		objTD = objTR.insertCell(objTR.cells.length);
		objTD.style.textAlign = 'right';
		objTD.innerHTML = '<span id="ITMVOL_R'+PO_DESC_ID+''+intIndex+'" style="text-align: right;">'+ITMVOLM_R+'</span>';
		//swal(CSTPUNT);
		...
		objTD = objTR.insertCell(objTR.cells.length);
		objTD.style.textAlign = 'center';
		objTD.innerHTML = '<input type="text" class="form-control" style="text-align:right; min-width:90px" name="PO_PRICEX'+intIndex+'" id="PO_PRICEX'+intIndex+'" size="10" value="'+ITM_PRICE+'" onKeyPress="return isIntOnlyNew(event);" onBlur="getValuePO('+intIndex+');">'+
		...
		if(ITM_CATEG != 'UA') 
			PO_DISPv = '<input type="text" class="form-control" size="10"  style=" min-width:70px; max-width:150px; text-align:right" name="PO_DISPx'+intIndex+'" id="PO_DISPx'+intIndex+'" value="'+PO_DISP+'" onBlur="countDisp(this.value, '+intIndex+');" >';
		else 
		...
		// <!-- ITEM DISCOUNT -->
		if(ITM_CATEG != 'UA') 
			PO_DISCv = '<input type="text" class="form-control" style="min-width:90px; max-width:350px; text-align:right" name="PO_DISC'+intIndex+'" id="PO_DISC'+intIndex+'" value="'+PO_DISC+'" onBlur="countDisc(this.value, '+intIndex+');" >';
		else 
		...
		if(ITM_CATEG != 'UA') 
			TAXCODE1v = '<select name="data['+intIndex+'][TAXCODE1]" class="form-control select2" id="data'+intIndex+'TAXCODE1"  onChange="getValuePO('+intIndex+');" style="min-width:90px; max-width:150px;">'+optTAX1+
						'</select>';
		else 
		...
		objTD.innerHTML = '<input style="text-align:right" type="hidden" name="data['+intIndex+'][PO_COST]" id="data'+intIndex+'PO_COST" value="'+PO_COST+'">'+
						  '<input type="text" class="form-control" style="min-width:110px; max-width:350px; text-align:right" name="PO_COSTnPPn'+intIndex+'" id="PO_COSTnPPn'+intIndex+'" value="'+PO_COSTnPPn+'" size="5" >'+
