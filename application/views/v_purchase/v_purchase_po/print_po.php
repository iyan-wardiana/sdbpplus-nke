...
<head>
	...
	<style type="text/css">
		...
		.box-asign .box-column-5 span#acc {
			display: block;
			padding-top: 5px;
			/*padding-bottom: 50px;*/
			padding-bottom: 10px;
		}
		.box-asign .box-column-5 span#nama {
			display: block;
		}
		.box-asign .box-column-5 span#jab {
			display: block;
			font-size: 8pt;
			padding-bottom: 5px;
		}
	...
	<section class="page sheet custom">
		...
		<div class="cont">
			...
			<?php
            	...
            	$PO_CATEG 	= 0;
            	$PO_ADDSTEP = 0;
            	$sqlPOH 	= "SELECT PO_NUM, PO_CODE, PO_DATE, PR_NUM, PO_NOTES, PO_PLANIR, PO_PAYTYPE, PO_TENOR, PO_CONFIRMED, PO_CATEG, PO_ADDSTEP
					...
					<div class="title-header">
						<table width="100%" border="0" cellpadding="0" cellspacing="0">
							<tr>
								...
								<td width="5">:</td>
								<td width="50" nowrap>&nbsp;<b><?php echo $lastPatt_OP.$PO_ADDSTEPD; ?></b></td>
					...
					<tbody>
						<?php
				            ...
				            foreach($resPODET as $row) :
								...
								$JML_NET 		= ($TOTPO_COST - $TOTPO_CTOTAL);
								...
								$TOTPO_VOLMP 	= $TOTPO_VOLM;
								if($TOTPO_VOLMP == 0 || $TOTPO_VOLMP == '')
									$TOTPO_VOLMP = 1;

								$ITM_PRICE    	= ($JML_NET + $TOTPO_DISC) / $TOTPO_VOLMP;
								...
								if($TOTPO_DISC != 0)
				              	...
				              	?>
				              		<tr style="border-bottom: 2px solid;">
										...
										<td>
											<?php if($PO_CODE == 'OP.536021.23000001')
												{
													?>
								...
								if($TOTPO_OA > 0)
								{
									...
									$TOTPO_VOLM2P 	= $TOTPO_VOLM-$TOTPO_CVOL;
									if($TOTPO_VOLM2P == 0 || $TOTPO_VOLM2P == '')
										$TOTPO_VOLM2P = 1;

									$ITMPRICE_OA	= $TOTPO_OA / ($TOTPO_VOLM2P);
									$JML_NETOA 		= $TOTPO_OA;
									?>
										<tr style="border-bottom: 2px solid;">
											<td style="text-align: center;"><?php echo $no; ?></td>
								...
								$TOT_COST    	= $TOT_COST + $JML_NET;
								// $PO_TOTCOST 	= $PO_TOTCOST + ROUND($JML_NET,2) + ROUND($TOTTAX_PPN,2);
								// $PO_TOTCOST 	= $PO_TOTCOST + $JML_NET + $TOTPPN;
				            endforeach;
							$PO_TOTCOST 	= $TOT_COST + $TOTPPN;
						?>
					...
					<th>
						<div class="cont">
	                        ...
	                        <?php
								...
								if($PO_CATEG == 1) $COLCLS     	= "box-column-4";
	                        ?>
							<div class="box-asign">
								<div class="<?=$COLCLS?>" style="<?php if($PO_CATEG == 0) { ?> display: none; <?php } ?>">
								...
								<div class="<?=$COLCLS?>" style="<?php if($PO_CATEG == 1) { ?> display: none; <?php } ?>">
								...
								<div class="<?=$COLCLS?>">
									...
									<span id="nama" style="padding-top: 25px;">Nama: .....................</span>
