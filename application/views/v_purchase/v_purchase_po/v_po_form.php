...
else
{
	...
	if($PO_TOTCOST > 500000000 && trim($PO_CONTRNO) == '')
		$disBtn 	= 1;
}
...
<html>
	...
	<?php
		...
		if($task == 'add')
		{
			...
			$PO_PLANIR	= date('d/m/Y');
			if($PR_RECEIPTD == '0000-00-00')
			{
				$PO_PLANIR = date('d/m/Y');
			}
			elseif($PR_RECEIPTD < $CURR_DATE)
			{
				$PR_RECEIPTD = date('Y-m-d');
				$PO_PLANIR = date('d/m/Y',strtotime($PR_RECEIPTD));
			}
			elseif($PR_RECEIPTD >= $CURR_DATE)
			{
				$PO_PLANIR = date('d/m/Y',strtotime($PR_RECEIPTD));
							...
							<div class="box-body">
		                        ...
		                        <div class="form-group">
                                	...
                                	<div class="col-sm-4">
		                          	...
		                          	<div class="col-sm-3">
		                                <div class="input-group date">
		                                    <input type="text" name="RECOM_DATE" class="form-control pull-left" id="datepicker1" value="<?php echo $RECOM_DATE; ?>">
		                                </div>
		                          	</div>
		                        </div>
		                        <div class="form-group">
		                            ...
		                            <div class="col-sm-4">
		                                <input type="text" class="form-control" name="PO_CONTRNO" id="PO_CONTRNO" size="30" value="<?php echo $PO_CONTRNO; ?>" onChange="shwBtn()" />
		                            </div>
		                        </div>
		                        <div class="form-group">
				                ...
				                <script>
									...
									function shwBtn()
									{
										var PO_CONTRNO 	= document.getElementById('PO_CONTRNO').value;
										if(PO_CONTRNO.trim() == '')
										{
											swal('Total OP ini lebih dari 100.000.000! Anda DIHARUSKAN mengisi no kontrak',
											{
												icon: "warning",
											})
											.then(function()
								            {
								                ...
								                document.getElementById('PO_CONTRNO').focus();
								            });
										}
										else
										{
		                        ...
		                        <div class="form-group" >
									...
									<div class="col-sm-2" style="padding-right: 0;">
				                        <div class="pull-right" style="display: <?php if($PR_NUMX <> '' && ($PO_STAT == 1 || $PO_STAT == 4)) echo ""; else echo "none";?>;">
				                        	<a class="btn btn-sm btn-info" data-toggle="modal" data-target="#mdl_addItemPR" onclick="ref4()">
				                    ...
				                    <script type="text/javascript">
				                    	function ref4()
				                    	{
				                    		//document.getElementById('idRefresh4').click();
				                    	}
				                    </script>
		                            <!-- Update [iyan] -->
	                        	...
	                        	<?php
	                        		if($resCAPP > 0)
	                        		{
										...
										else
										{
											...
											elseif($disBtn == 0 && ($PO_STAT == 1 || $PO_STAT == 4) && $ISCREATE == 1)
											{
												?>
													<button class="btn btn-primary" id="btnSave" >
													<i class="fa fa-save"></i></button>&nbsp;
					...
					$(document).ready(function()
					{
						...
						$('#example4').DataTable(
						{
							...
							"columnDefs": [	{ targets: [0,1], className: 'dt-body-center' },
											...
											{ targets: [4,5,6,7], 
												render: function(data, type, row, meta) {
													...
													return doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(data)), 3));
													// $.ajax({
							...
							"footerCallback": function (row, data, start, end, display) {
								...
								var totalBUDQTYV = doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(totalBUDQTY)), 3));
								...
								var totalTOTPRQTYV = doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(totalTOTPRQTY)), 3));
								...
								var totalPRVOLMV = doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(totalPR_VOLM)), 3));
								...
								var totalREMORDERVOLV = doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(totalREMORDERVOL)), 3));
					...
					$(document).ready(function()
					{
						...
						if(PR_NUM != '' && task != 'add')
						{
							...
							$('#tblITMDET').DataTable(
							{
								...
								"ajax": {
									"url": "<?php echo site_url('c_purchase/c_p180c21o/get_AllDataIITMDET/?id='.$PRJCODE.'&PO_NUM='.$PO_NUM.'&PR_NUM='.$PR_NUMX.'&PO_STAT='.$PO_STAT)?>",
									"type": "POST",
									"beforeSend": function() {
			                ...
			                <div class="modal-body">
								<div class="row">
							    	<div class="col-md-12">
										...
										<div class="box-header">
											<h3 class="box-title">Pembatalan Volume OP Item : <i style='font-size: 14px;' id="itmName1"></i></h3>
										</div>
						              	<!-- <ul class="nav nav-tabs">
						                    <li id="li1" <?php //echo $Active1Cls; ?>>
												...
												<form method="post" name="frmSearch1" id="frmSearch1" action="">
													<div class="col-md-12">
														...
														<br>
														  <div class="row">
															<div class="col-md-6">
																<button type="button" id="btnproc_cnc" class="btn btn-warning" onClick="proc_cnc()"><i class="fa fa-save"></i></button>
...
<?php
	...
	$secUPOV 	= base_url().'index.php/c_purchase/c_p180c21o/chgPOVOL/?id=';
	$tblTax 	= "tbl_tax_ppn";
?>
<script>
	<?php
	...
	function getValuePO_1(row)
	{
		PRJCODE		= document.getElementById('PRJCODE').value;
		PO_NUM		= document.getElementById('PO_NUM').value;
		PR_NUM		= document.getElementById('PR_NUM').value;
		JOBCODEID 	= document.getElementById('data'+row+'JOBCODEID').value;
		PRD_ID 		= document.getElementById('data'+row+'PRD_ID').value;
		PO_VOL1		= document.getElementById('PO_VOLMX'+row);
		PO_VOL 		= parseFloat(eval(PO_VOL1).value.split(",").join(""));
		PO_PRC1		= document.getElementById('PO_PRICEX'+row);
		PO_PRC		= parseFloat(eval(PO_PRC1).value.split(",").join(""));
		// PO_OA1		= document.getElementById('PO_OAX'+row);
		// PO_OA		= parseFloat(eval(PO_OA1).value.split(",").join(""));

		var url 		= "<?php echo $secUPOV; ?>";
		// var collID		= PRJCODE+"~"+PO_NUM+"~"+PR_NUM+"~"+JOBCODEID+"~"+PRD_ID+"~"+PO_VOL+"~"+PO_PRC+"~"+PO_OA;
		var collID		= PRJCODE+"~"+PO_NUM+"~"+PR_NUM+"~"+JOBCODEID+"~"+PRD_ID+"~"+PO_VOL+"~"+PO_PRC;
		$.ajax({
            type: 'POST',
            url: url,
            data: {collID: collID},
			async: false,
            success: function(s_TOTPOVOL0C)
            {
            	getValuePO(row)
            }
        });
	...
	function getValuePO(row)
	{
		...
		if(ITM_CATEG != 'UA')
		{
			...
			// getOA			= document.getElementById('PO_OAX'+row);
			// PO_OA			= parseFloat(eval(getOA).value.split(",").join(""));
			// PO_VAL0			= parseFloat(PO_VOL) * parseFloat(POPRICE);
			// PO_VAL0			= parseFloat(PO_VOL * POPRICE) + parseFloat(PO_OA);
			PO_VAL0			= parseFloat(PO_VOL * POPRICE);
			...
			console.log('PO_VOL = '+PO_VOL)
			console.log('REM_VOL = '+REM_VOL)
			if(parseFloat(PO_VOL.toFixed(3)) > parseFloat(REM_VOL.toFixed(3)))
			{
				...
				document.getElementById('PO_VOLMX'+row).value 			= 0;
				document.getElementById('data'+row+'PO_VOLM').value 	= 0;
			...
			// START : RECHECK
					...
					if(i != row)
					{
						...
						if(theObj != null)
						{
							...
							if(JOBCODEID1 == JOBCODEID2)
							{
								...
								// PO_OA2		= parseFloat(document.getElementById('data'+i+'PO_OA').value);
								PODISC2		= parseFloat(document.getElementById('data'+i+'PO_DISC').value);
								// PO_VAL2 	= parseFloat(PO_VOL2) * parseFloat(POPRICE2);
								// PO_VAL2 	= parseFloat(PO_VOL2 * POPRICE2) + parseFloat(PO_OA2);
								PO_VAL2 	= parseFloat(PO_VOL2 * POPRICE2);
				...
				console.log('2. '+REM_RAPVAL 	+'='+ parseFloat(ITM_BUDGVAL) +'-'+ parseFloat(TOTPO_VAL2));
			// END : RECHECK REMAIN FOR THE NEXT ROW FOR THE SAME JOBCODEID AND ITM_CODE
			
			console.log('PO_VAL = '+PO_VAL.toFixed(2))
			console.log('REM_VAL = '+REM_VAL.toFixed(2))
			if(parseFloat(PO_VAL.toFixed(2)) > parseFloat(REM_VAL.toFixed(2)))
			{
				...
				.then(function()
				{
					...
					PO_PRICE 	= parseFloat(0);

					// document.getElementById('data'+row+'PO_OA').value 	= parseFloat(Math.abs(0));
					// document.getElementById('PO_OAX'+row).value			= doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(0)), 2));
			...
			else
			{
				...
				// document.getElementById('data'+row+'PO_OA').value 	= parseFloat(Math.abs(PO_OA));
				// document.getElementById('PO_OAX'+row).value 		= doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(PO_OA)), 2));
			...
			for(i=0;i<=totRow;i++)
			{
				...
				{
					var JOBCODEID2 		= document.getElementById('data'+i+'JOBCODEID').value;
					var REM_VAL_JOB2 	= parseFloat(document.getElementById('JOB_MAXREQ_AMN'+i).value);
					...
					for(j=1;j<=totRow;j++)
					{
						...
						if(theObj != null)
						{
							...
							if(JOBCODEID3 == JOBCODEID2 && i != j)
							{
								...
								// PO_OA3		= parseFloat(document.getElementById('data'+j+'PO_OA').value);
								PODISC3		= parseFloat(document.getElementById('data'+j+'PO_DISC').value);
								// PO_VAL3 	= parseFloat(PO_VOL3) * parseFloat(POPRICE3);
								// PO_VAL3 	= parseFloat(PO_VOL3 * POPRICE3) + parseFloat(PO_OA3);
								PO_VAL3 	= parseFloat(PO_VOL3 * POPRICE3);
								TOTPO_VAL3 	= parseFloat(TOTPO_VAL3) + parseFloat(PO_VAL3) - parseFloat(PODISC3);
							}
						}
					}

					ITM_RAPVAL	= parseFloat(REM_VAL_JOB2) - parseFloat(TOTPO_VAL3);
					document.getElementById('ITM_MAXREQ_AMN'+i).value 	= ITM_RAPVAL;
					document.getElementById('TOTVAL_OTHDOC'+i).value 	= parseFloat(ITM_BUDGVAL) - parseFloat(ITM_RAPVAL);
			...
			// PO TEMP
				...
				// ITMPRICE_TEMP	= ((PO_VOL * POPRICE) + PO_OA - PO_DISC);
				ITMPRICE_TEMP	= ((PO_VOL * POPRICE) - PO_DISC);
			...
			// UPDATE VOL TEMPORARY
				/*let JOBCODEID 		= document.getElementById('data'+row+'JOBCODEID').value;
				let PO_VOLM 		= document.getElementById('data'+row+'PO_VOLM').value;
				let PO_PRICE 		= document.getElementById('data'+row+'PO_PRICE').value;
	...
	function getValuePO_TAX(row)
	{
		var decFormat		= document.getElementById('decFormat').value;
		let totalrow		= document.getElementById('totalrow').value;

		ITM_TAXPRICE		= parseFloat(eval(document.getElementById('TAXPRICE1'+row)).value.split(",").join(""));
		document.getElementById('data'+row+'TAXPRICE1').value 	= parseFloat(ITM_TAXPRICE);
		document.getElementById('TAXPRICE1'+row).value 			= doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(ITM_TAXPRICE)), 2));
				...
				else
				{
					...
					$.ajax({
			            ...
			            success: function(taxPerc2)
			            {
			            	...
			            	TAX1VAL			= parseFloat(document.getElementById('data'+row+'TAXPRICE1').value);
	...
	function chgReqVol(thisVal, row)
	{
		...
		if(parseFloat(PO_VOLMX.toFixed(3)) > parseFloat(ITM_REMAIN.toFixed(3)))
		{
	            ...
	            if (willDelete) 
	            {
	                ...
	                $.ajax({
	                    ...
	                    success: function(response)
	                    {
	                        ...
	                        $('#tblITMDET').DataTable().ajax.reload();
	                    }
	                });
	            } 
	            else 
	...
	function add_item_221009(strItem) 
	{
		...
		objTD = objTR.insertCell(objTR.cells.length);
		objTD.style.textAlign = 'center';
		objTD.innerHTML = ''+unit_type_code+'<input type="hidden" id="data'+intIndex+'ITM_UNIT" name="data['+intIndex+'][ITM_UNIT]" value="'+unit_type_code+'" width="10" size="15" class="form-control" style="max-width:350px;text-align:right"><input type="hidden" id="data'+intIndex+'ITM_UNIT2" name="data['+intIndex+'][ITM_UNIT2]" value="'+unit_type_code+'" width="10" size="15" class="form-control" style="max-width:350px;text-align:right">';		
		//swal(unit_type_code);
	...
	function add_item(strItem) 
	{
		...
		ITM_UNIT 		= arrItem[20];
		ITM_UNIT2 		= arrItem[21];
		PR_DESC_ID 		= arrItem[22];
		PO_DESC_ID 		= arrItem[23];
		PO_DESC 		= arrItem[24];
		selRow 			= arrItem[25];
		ITM_GROUP 		= arrItem[26];
		...
		objTD.innerHTML = ''+ITM_CODE+' : '+ITM_NAME+ '<span class="text-red" style="font-style: italic; font-weight: bold;">(Hrg.RAP : '+RAP_PRICEvx+')</span>'+
						  ...
						  '<input type="hidden" id="data'+intIndex+'ITM_CODE" name="data['+intIndex+'][ITM_CODE]" value="'+ITM_CODE+'" width="10" size="15">'+
						  '<input type="hidden" id="data'+intIndex+'ITM_NAME" name="data['+intIndex+'][ITM_NAME]" value="'+ITM_NAME+'" width="10" size="15">'+
						  '<input type="hidden" id="data'+intIndex+'ITM_GROUP" name="data['+intIndex+'][ITM_GROUP]" value="'+ITM_GROUP+'" width="10" size="15">'+
						  '<input type="hidden" id="data'+intIndex+'ITM_CATEG" name="data['+intIndex+'][ITM_CATEG]" value="'+ITM_CATEG+'" width="10" size="15">'+
		...
		// <!-- ITEM BUDGET -->
		PR_VOLMvx = doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(PR_VOLM)), 3));
		...
		// <!-- ITEM REMAIN -->
		ITM_REMVOLvx = doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(ITM_REMVOL)), 3));
		...
		// <!-- ITEM ORDER NOW -->
		PO_VOLMvx = doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(PO_VOLM)), 3));
		objTD = objTR.insertCell(objTR.cells.length);
		objTD.style.textAlign = 'center';
		objTD.innerHTML = '<input type="text" class="form-control" style="min-width:90px; max-width:300px; text-align:right" name="PO_VOLMX'+intIndex+'" id="PO_VOLMX'+intIndex+'" value="'+PO_VOLMvx+'" onKeyPress="return isIntOnlyNew(event);" onBlur="getValuePO_1('+intIndex+');">'+
		...
		objTD = objTR.insertCell(objTR.cells.length);
		objTD.style.textAlign = 'center';
		objTD.innerHTML = '<input type="hidden" class="form-control" style="text-align:center; min-width:90px" id="data'+intIndex+'ITM_UNIT" name="data['+intIndex+'][ITM_UNIT]" value="'+ITM_UNIT+'"><input type="text" class="form-control" style="text-align:center; min-width:90px" id="data'+intIndex+'ITM_UNIT2" name="data['+intIndex+'][ITM_UNIT2]" value="'+ITM_UNIT2+'">';
		...
		objTD = objTR.insertCell(objTR.cells.length);
		objTD.style.textAlign = 'right';
		objTD.innerHTML = '<input type="text" class="form-control" style="text-align:right; min-width:90px" name="PO_PRICEX'+intIndex+'" id="PO_PRICEX'+intIndex+'" size="10" value="'+ITM_PRICEvx+'" onKeyPress="return isIntOnlyNew(event);" onBlur="getValuePO_1('+intIndex+');">'+
						  '<input style="text-align:right" type="hidden" name="data['+intIndex+'][PO_PRICE]" id="data'+intIndex+'PO_PRICE" size="6" value="'+ITM_PRICE+'">';

		/* <!-- PO Ongkos Angkut -->
		PO_OAvx = doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(PO_OA)), decFormat));
		objTD = objTR.insertCell(objTR.cells.length);
		objTD.style.textAlign = 'right';
		objTD.innerHTML = '<input type="text" class="form-control" style="text-align:right; min-width:90px" name="PO_OAX'+intIndex+'" id="PO_OAX'+intIndex+'" size="10" value="'+PO_OAvx+'" onKeyPress="return isIntOnlyNew(event);" onBlur="getValuePO_1('+intIndex+');">'+
						  '<input style="text-align:right" type="hidden" name="data['+intIndex+'][PO_OA]" id="data'+intIndex+'PO_OA" size="6" value="'+PO_OA+'">';
	...
	function getTOTOP()
	{
		...
		// CHECK DOKUMEN KONTRAK, JIKA TOTAL OP > 100 JT WAJIB ISI DOK. KONTRAK
			var PO_CONTRNO 	= document.getElementById('PO_CONTRNO').value;
			if(parseFloat(totOP) > 100000000 && PO_CONTRNO.trim() == '')
			{
				swal('Total OP ini lebih dari 100.000.000! Anda DIHARUSKAN mengisi no kontrak',
				{
					icon: "warning",
	...
	function showIRDET(row)
	{
		var url	= document.getElementById('urlIRDet'+row).value;
		w = 1000;
		h = 550;
		var left = (screen.width/2)-(w/2);
		var top = (screen.height/2)-(h/2);
