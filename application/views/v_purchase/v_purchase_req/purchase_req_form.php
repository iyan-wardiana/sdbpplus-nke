	...
	$(document).ready(function()
	{
		$('#tbl').DataTable(
    	{
			...
			"columnDefs": [	{ targets: [0,9], className: 'dt-body-center' },
							...
							{ targets: [10], visible: <?php echo $visible_R; ?>}
						  ],
        	"order": [[ 2, "desc" ]],
			...
			"footerCallback": function (row, data, start, end, display) {
				...
				// Update footer
				$(api.column(3).footer()).html('<div style="font-weight: bold;">'+totalBUDQTYV+'</div>');
				...
				// Update footer
				$(api.column(7).footer()).html('<div style="font-weight: bold;">'+totalPRVOLMV+'</div>');
			}
		});
		...
		$.ajax({
            ...
            success: function(maxPR_ID)
            {
            	...
            	$('#tbl').DataTable(
		    	{
					...
					"autoWidth": true,
					"filter": true,
			        "ajax": "<?php echo site_url('c_purchase/c_pr180d0c/get_AllDataIITMDET/?id='.$PRJCODE.'&PR_NUM='.$PR_NUM.'&task='.$task.'&SPPCAT='.$SPPCAT)?>",
			        "type": "POST",
					"lengthMenu": [[5, 10, 25, 50, 100, 200, -1], [5, 10, 25, 50, 100, 200, "All"]],
					"pageLength": -1,
					// "lengthMenu": [[500,1000], [500,1000]],
					...
					"footerCallback": function (row, data, start, end, display) {
						...
						totalPRVOLM = totalPR_VOLM - totalPR_CVOL;
						var totalPRVOLMV = doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(totalPRVOLM)), 2));
	                ...
	                $.ajax({
	                    ...
	                    success: function(response)
	                    {
							...
							$('div.toolbar').html('<form id="form-filter" class="form-horizontal">'+
												  '<select class="form-control select2" name="ITM_SRC" id="ITM_SRC" data-placeholder="Cari Item" style="width: 200px;">'+
												  '<option value=""></option></select>&nbsp;'+
												  '<select class="form-control select2" name="PRDESC_SRC" id="PRDESC_SRC" data-placeholder="Cari Deskripsi" style="width: 300px;">'+
												  '<option value=""></option></select>&nbsp;'+
												  '<button type="button" id="btn-filter" class="btn btn-primary"><i class="fa fa-filter"></i>&nbsp;Filter</button>'+
												  '</form>');
