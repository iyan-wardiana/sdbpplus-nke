				                ...
				                <div class="form-group">
									...
									<div class="col-sm-6">
										<?php
											...
											$this->db->where(['Attach_NUM' => $Attach_NUM, 'RefNumber' => $PR_NUM, 'PRJCODE' => $PRJCODE]);
											$Filesize = $this->db->get();
											if($Filesize->num_rows() > 0)
											{
												foreach($Filesize->result() as $rF):
													$totalsize = $totalsize + $rF->File_Size;
												endforeach;
											}											
										?>
										<input type="hidden" name="Attach_NUM" id="Attach_NUM" value="<?=$Attach_NUM?>">
										<input type="hidden" name="totalsize" id="totalsize" value="<?=$totalsize?>">
										<div class="custom-file">
	            							<input type="file" name="Attachment[]" id="Attachment" class="custom-file-input custom-file-control" accept="application/pdf, text/plain, audio/*, video/*, image/*, application/x-zip-compressed,.rar" multiple <?=$PR_STAT == 1 || $PR_STAT == 4 ? '' : 'disabled'?>>
	            							<label class="custom-file-label" for="Attachment">Please choose files here</label>
	            						</div>
									</div>
									<div class="col-sm-3">
									<?php
										$getDATA 	= "$PRJCODE~$PR_NUM";
										$C_Attach = $this->db->get_where('tbl_attachment', ['Attach_NUM' => $Attach_NUM, 'RefNumber' => $PR_NUM, 'PRJCODE' => $PRJCODE]);
			            				if($C_Attach->num_rows() > 0 ):
									?>
										<div class="pull-right">
		            						<a href="<?php echo base_url('index.php/c_purchase/c_pr180d0c/downloadFile/?id='.$this->url_encryption_helper->encode_url($getDATA)) ?>" class="btn btn-sm btn-default"><i class="fa fa-paperclip"></i>&nbsp;&nbsp;Download&nbsp;&nbsp;</a>
		            					</div>
		            					<div class="pull-right" style="display: none;">
		            						<a class="btn btn-sm btn-default" data-toggle="modal" data-target="#mdl_viewAttachment"><i class="fa fa-paperclip"></i>&nbsp;&nbsp;View File&nbsp;&nbsp;</a>
		            					</div>
		            				<?php endif; ?>
									</div>
                                </div>
		...
		$('#Attachment').bind('change', function (event) {
			...
			if(files.length > 0)
			{
				for(var i = 0; i < files.length; i++) {
					totalsize = parseFloat(totalsize) + parseFloat(files[i].size);
				}
				...
				if(totalsize > maxsize)
				{
					...
					$(this).val("");
					$(this).next('label').text("Please choose files here");
				}
				else
				{
					if(files.length > 1)
