...
<html>
	...
	<?php
		...
		if($LangID == 'IND')
		{
			...
			$alert6 	= "Hanya File PDF/Gambar yang bisa diunggah";
		...
		else
		{
			...
			$alert6 	= "Only file PDF/Image can be uploaded";
							...
							<div class="box-body">
								<div class="form-group">
									<div class="col-sm-4">
										...
										<input type="hidden" name="PR_DOC" id="PR_DOC" value="<?=$PR_DOC?>">
				                	</div>
									<div class="col-sm-8">
										...
										<div class="uploaded_area">
											<?php
												if($PR_DOC != null)
												{
													$dataFile 	= explode(", ", $PR_DOC);
													$jmlFile 	= count($dataFile);

													for($i=0; $i<$jmlFile; $i++) {
														?>
															<div class="itemFile_<?=$i?>">
																<?php
																	if($PR_STAT == 1 || $PR_STAT == 4)
																	{
																		?>
																			<!-- Hapus File -->
																			<a href="#" onclick="trashItemFile(<?=$i?>, '<?php echo $dataFile[$i];?>')" title="Hapus File">
																				<i class="fa fa-trash" style="color: red;"></i>
																			</a> 
																			<!-- Download -->
																			<a href="<?php echo site_url("c_purchase/c_pr180d0c/downloadFile/?file=".$dataFile[$i]."&prjCode=".$PRJCODE); ?>" title="Download File">
																				<?php echo $dataFile[$i]; ?>
																			</a>
																		<?php
																	}
...
<script>
	...
	$(function () {
		...
		$("#userfile").bind("change", (e) => {
			...
			const validExtensions 	= ["image/jpeg", "image/jpg", "image/png", "application/pdf"]; //adding some valid image extensions in array
	...
	function trashItemFile(row, fileName)
	{
		...
		swal({
			...
			if (willDelete) {
				...
				let PR_DOC 	= "<?php echo $PR_DOC; ?>";
				$.ajax({
					type: "POST",
					url: "<?php echo site_url("c_purchase/c_pr180d0c/trashFile"); ?>",
					data: {PR_NUM:PR_NUM, PRJCODE:PRJCODE, PR_DOC:PR_DOC, indexRow:row, fileName:fileName},
