...
<html>
	...
	<?php
		...
		// SETTINGAN UNTUK APPROVAL PER DIVISI
			if($PRJHO == 'KTR')
			{
				$s_DIV 	= "SELECT DISTINCT B.JOBCOD1 FROM tbl_pr_detail A INNER JOIN tbl_joblist_detail B ON A.JOBCODEID = B.JOBCODEID AND A.PRJCODE = B.PRJCODE
							WHERE A.PRJCODE = '$PRJCODE' AND A.PR_NUM = '$PR_NUM'";
				$r_DIV 	= $this->db->query($s_DIV)->result();
				foreach($r_DIV as $rw_DIV):
					$MenuApp 	= $rw_DIV->JOBCOD1;

					$s_UPD1 	= "UPDATE tbl_pr_detail SET PR_DIVID = '$MenuApp' WHERE PR_NUM = '$PR_NUM' AND PRJCODE = '$PRJCODE'";
					$this->db->query($s_UPD1);

					$s_UPD2 	= "UPDATE tbl_pr_header SET PR_DIVID = '$MenuApp' WHERE PR_NUM = '$PR_NUM' AND PRJCODE = '$PRJCODE'";
			                                            ...
			                                            <table id="mtr_perjob" class="table table-bordered table-striped table-responsive search-table inner" width="100%">
			                                                <thead>
			                                                    <tr>
			                                                        ...
			                                                        <th width="40%" style="text-align: center; vertical-align: middle;"  nowrap><?php echo $ItemName; ?> AA</th>
				...
				<script type="text/javascript">
					...
					function setType(tabType)
					{
						if(tabType == 1)
						{
					    	...
					    	{
								"dom": "<'row'<'col-sm-2'l><'col-sm-8'<'toolbarBUDG'>><'col-sm-2'f>>"+
									"<'row'<'col-sm-12'tr>>",
					    		"destroy": true,
						        "processing": true,
						        "serverSide": true,
					        	...
					        	"ajax": {
									"url": "<?php echo site_url('c_purchase/c_pr180d0c/get_AllDataITMALL/?id='.$PRJCODE.'&PRNUM='.$PR_NUM.'&ITMCAT=M')?>",
									"type": "POST",
									"data": function(data) {
										data.ITM_SRC 	= $('#ITMBUDG_SRC').val();
									},
								},
						        "type": "POST",
							...
							$('div.toolbarBUDG').html('<form id="form-filterBUDGM" class="form-horizontal">'+
												  '<select class="form-control select2" name="ITMBUDG_SRC" id="ITMBUDGM_SRC" style="width: 200px;">'+
												  '<option value=""></option></select>&nbsp;'+
												  '<input type="hidden" class="form-control" name="ITMCAT" id="ITMCAT1" value="M">'+
												  '</form>');

							let PRJCODE 	= $('#PRJCODE').val();
							let ITMBUDG_SRC = $('[name="ITMBUDG_SRC"]').val();
							let ITMCAT 		= $('[name="ITMCAT"]').val();
							// GET ITM
								$.ajax({
									url: "<?php echo site_url('c_purchase/c_pr180d0c/get_ITMBUDG'); ?>",
									type: "POST",
									data: {PRJCODE:PRJCODE,ITMCAT:ITMCAT},
									dataType: "JSON",
									beforeSend: function() {
										$('[name="ITMBUDG_SRC"]').select2({placeholder: 'Please waiting...'});
									},
									success: function(result) {
										console.log(result);
										var SRC_ITMBUDG = "<option value=''></option>";
										for(let i in result) {
											ITMCODE_SRC = result[i]['ITM_CODE'];
											ITMNAME_SRC = result[i]['ITM_NAME'];
											SRC_ITMBUDG += '<option value="'+ITMCODE_SRC+'">'+ITMNAME_SRC+'</option>';
										}
										$('[name="ITMBUDG_SRC"]').html(SRC_ITMBUDG);
									},
									complete: function() {
										$('[name="ITMBUDG_SRC"]').select2({placeholder: 'Cari Item', dropdownParent: $('#mdl_addItm')});
									}
								});

								// $('#ITMBUDG_SRC').on('change', function(e) {
								// 	// GET JOBPARENT
								// 		$.ajax({
						...
						else if(tabType == 4)
						{
							...
							$('#mtr_ovhlist').DataTable(
							{
								...
								"ajax": {
									"url": "<?php echo site_url('c_purchase/c_pr180d0c/get_AllDataITMALL/?id='.$PRJCODE.'&PRNUM='.$PR_NUM.'&ITMCAT=O')?>",
									"type": "POST",
									"data": function(data) {
										data.ITM_SRC 	= $('#ITMBUDG_SRC').val();
									},
							...
							$('div.toolbarBUDG').html('<form id="form-filterBUDGO" class="form-horizontal">'+
												  '<select class="form-control select2" name="ITMBUDG_SRC" id="ITMBUDGO_SRC" style="width: 200px;">'+
												  '<option value=""></option></select>&nbsp;'+
												  '<input type="hidden" class="form-control" name="ITMCAT" id="ITMCAT2" value="O">'+
												  '</form>');
					...
					$(document).ready(function()
					{
				    	$('#mtr_perjob').DataTable(
				    	{
					        ...
					        "ajax": {
								...
								"type": "POST",
								"data": function(data) {
									data.ITM_SRC 	= $('[name="ITMBUDG_SRC"]').val();
								},
							},
							"lengthMenu": [[5, 10, 25, 50, 100, 200, -1], [5, 10, 25, 50, 100, 200, "All"]],
							// "lengthMenu": [[10, 25, 50, 100, 200], [10, 25, 50, 100, 200]],
						...
						// GET ITM
							...
							$('[name="ITMBUDG_SRC"]').on('change', function(e) {
								$('#mtr_perjob').DataTable().ajax.reload();
							});
						// END GET ITM
					});
