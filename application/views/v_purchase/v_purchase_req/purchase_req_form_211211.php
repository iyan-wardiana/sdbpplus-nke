...
<html>
  	<head>
        ...
        ?>
        <!-- dropzonejs -->
  		<link rel="stylesheet" href="<?=base_url("assets/AdminLTE-2.0.5")?>/plugins/dropzone/min/dropzone.min.css">
    ...
    <body class="<?php echo $appBody; ?>" style="background-color: <?=$comp_color?>">
		...
		<section class="content">
						...
						<div class="box box-warning">
							...
							<div class="box-body">
				                ...
				                <div class="form-group">
				            		...
				            		<div class="col-sm-9">
				            			<?php
				            				$C_Attach = $this->db->get_where('tbl_attachment', ['RefNumber' => $PR_NUM]);
				            				if($C_Attach->num_rows() > 0 )
				            				{
				            					?>
				            						<a title="View Attachment" class="btn btn-default btn-md" data-toggle="modal" data-target="#mdl_viewAttachment"><i class="fa fa-paperclip"></i></a>
				            					<?php
				            				}
				            				else
				            				{
				            					?>
					            						<div class="custom-file">
					            							<input type="file" name="Attachment[]" id="Attachment" class="custom-file-input custom-file-control" accept="application/pdf, text/plain, audio/*, video/*, image/*, application/x-zip-compressed,.rar" multiple required>
					            							<label class="custom-file-label" for="Attachment">Please choose or drag file here</label>
	    	...
	    	<!-- ============ START MODAL =============== -->
				...
				<div class="modal fade" id="mdl_viewAttachment" tabindex="-1" role="dialog" aria-labelledby="mdl_viewAttachmentLabel" aria-hidden="true">
				  <div class="modal-dialog" role="document">
				    <div class="modal-content">
				      <div class="modal-header">
				        <h3 class="modal-title" id="mdl_viewAttachmentLabel">Attachment List</h3>
				      </div>
				      <div class="modal-body">
				      ...
				      <div class="modal-footer">
				      	<button type="button" id="idClose5" class="btn btn-info" data-dismiss="modal">
                    		<i class="glyphicon glyphicon-download-alt"></i>&nbsp;&nbsp;<?php echo "Download"; ?>
                    	</button>
				...
				<script type="text/javascript">
					...
					$(document).ready(function()
					{
						...
						$('#example5').DataTable(
						{
							...
							"autoWidth": true,
							"filter": true,
					        "ajax": "<?php echo site_url('c_purchase/c_pr180d0c/get_AllDataAttachment/?id='.$getDATA)?>",
					        "type": "POST",
							//"lengthMenu": [[10, 25, 50, 100, 200, -1], [10, 25, 50, 100, 200, "All"]],
							"lengthMenu": [[10, 25, 50, 100, 200], [10, 25, 50, 100, 200]],
		...
		$('#Attachment').bind('change', function (event) {
			...
			if(files.length > 0)
			{
				for(var i = 0; i < files.length; i++) {
					totalsize = totalsize + Number(files[i].size);
				}
				...
				{
					swal("File tidak boleh melebihi dari 10MB!");
					$(this).val("");
					$(this).next('label').text("Please choose or drag file here");
				}
				else
				{
					if(files.length > 1)
						$(this).next('label').text(files.length+" files selected");
					else
						$(this).next('label').text(files[0]['name']);
				}
			}
			else
			{
...
<!-- dropzonejs -->
<script src="<?=base_url("assets/AdminLTE-2.0.5")?>/plugins/dropzone/min/dropzone.min.js"></script>
<?php
