...
else
{
	...
	$s_sql 	= "SELECT COUNT(*) AS TOT_PR
				FROM (SELECT A.* FROM $tblPRD A WHERE A.PR_NUM = '$PR_NUM' AND A.PRJCODE = '$PRJCODE'
				UNION ALL
...
<html>
	...
	<style type="text/css">
		...
		.toolbar {
			display: <?php if($PR_STAT == 1 || $PR_STAT == 4) echo "none"; ?>;
		}
	</style>
	...
	<?php
		...
		if($LangID == 'IND')
		{
			...
			$alert6 	= "Hanya File PDF yang bisa diunggah";
		...
		else
		{
			...
			$alert6 	= "Only file PDF can be uploaded";
		...
		// SETTINGAN UNTUK APPROVAL PER DIVISI
			if($PRJHO == 'KTR')
			{
				$s_DIV 	= "SELECT B.JOBCOD1 FROM $tblPRD A INNER JOIN tbl_joblist_detail B ON A.JOBCODEID = B.JOBCODEID AND A.PRJCODE = B.PRJCODE
				...
				foreach($r_DIV as $rw_DIV):
					...
					$s_UPD1 	= "UPDATE $tblPRD SET PR_DIVID = '$MenuApp' WHERE PR_NUM = '$PR_NUM' AND PRJCODE = '$PRJCODE'";
					$this->db->query($s_UPD1);

					$s_UPD2 	= "UPDATE $tblPRH SET PR_DIVID = '$MenuApp' WHERE PR_NUM = '$PR_NUM' AND PRJCODE = '$PRJCODE'";
    ...
    <body class="<?php echo $appBody; ?>" style="background-color: <?=$comp_color?>">
		<section class="content-header">
			<h1>
			    <img src="<?php echo base_url() . 'assets/AdminLTE-2.0.5/dist/img/icon/purchase_req.png'; ?>" style="max-width:40px; max-height:40px" >&nbsp;&nbsp;<?php echo "$subTitleH ($PRJCODE)"; ?>
		...
		<section class="content">
					...
					<div class="col-md-6">
						<div class="box box-primary">
							...
							<div class="box-body">
				                ...
				                <input type="hidden" name="SPPCAT" id="SPPCAT" value="<?php echo $SPPCAT; ?>">
				                ...
				                <input type="hidden" name="PATTCODE" id="PATTCODE" value="<?php echo $PATTCODE; ?>">
				                <input type="hidden" name="PR_DIVID" id="PR_DIVID" value="<?php echo $MenuApp; ?>">
				                ...
				                <div class="form-group">
				                    ...
				                    <div class="col-sm-4">
				                        <div class="input-group date">
				                        	...
				                        	<input type="text" name="PR_DATEX" class="form-control pull-left" id="datepicker" value="<?php echo $PR_DATE; ?>" onChange="getPR_NUM(this.value)" <?php if($SPPCAT != 'FPA') echo "disabled"; ?>>
				                    ...
				                    <div class="col-sm-3">
		                                <select name="PR_CATEG" id="PR_CATEG" class="form-control select2">
                                            <option value="0"<?php if($PR_CATEG == 0) { ?> selected <?php } ?>>Bahan</option>
				                                ...
				                                <select name="PR_STAT" id="PR_STAT" class="form-control select2" onChange="chkSTAT(this.value)">
				                                    ...
				                                    if($PR_STAT != 1 AND $PR_STAT != 4)
				                                    {
				                                        ?>
				                                            ...
				                                            <?php if($isUSED == 0) { ?>
				                                            	<option value="9"<?php if($PR_STAT == 9) { ?> selected <?php } ?>>Void</option>
				                                            <?php } else { ?>
                            ...
                            <table id="tbl" class="table table-bordered table-striped table-responsive search-table inner" width="100%">
                                <thead>
                                    <tr>
										  ...
										  <th width="18%" style="text-align:center; vertical-align: middle;"><?php echo "Nama Barang" ?> </th>
										  <th width="18%" style="text-align:center; vertical-align: middle;"><?php echo "Ukuran/Spesifikasi" ?> </th>
		                ...
		                <div class="form-group">
		                    ...
		                    <div class="col-sm-9">
		                        <?php
		                            ...
		                            $backURL	= site_url('c_purchase/c_pr180d0c/pRQ_l5t_x/?SPPCAT='.$SPPCAT.'&id='.$this->url_encryption_helper->encode_url($PRJCODE));
	    	...
	    	<!-- ============ START MODAL ITEM LIST =============== -->
							    	...
							    	<div class="col-md-12">
						              	<ul class="nav nav-tabs">
						              		<?php
								            	...
								            	<?php } elseif($SPPCAT == 'FPA') { ?>
								                    <li id="li7">
								                    	<a href="#itm7" data-toggle="tab" onClick="setType(5)"><?php echo "Daftar Pekerjaan Alat"; ?></a>
								                    </li>
								            	<?php }
								            ?>
						                </ul>
							            <div class="box-body">
						              		<?php
									            	...
									            	<div class="<?php echo $Active1; ?> tab-pane" id="itm1">
				                                        <div class="form-group">
				                                        	<form method="post" name="frmSearch1" id="frmSearch1" action="">
					                                            <table id="mtr_perjob" class="table table-bordered table-striped table-responsive search-table inner" width="100%">
					                                                <thead>
					                                                    <tr>
									            	...
									            	<div class="<?php echo $Active2; ?> tab-pane" id="itm2" style="display: none;">
				                                        <div class="form-group">
				                                        	<form method="post" name="frmSearch2" id="frmSearch2" action="">
					                                            <table id="mtr_subst" class="table table-bordered table-striped table-responsive search-table inner" width="100%">
					                                                <thead>
				                                                        <tr>
					                                                        <th width="2%" style="text-align: center; vertical-align: middle;">&nbsp;</th>
					                                                        <th width="40%" style="text-align: center; vertical-align: middle;"  nowrap><?php echo $ItemName; ?></th>
															...
															<div class="box box-primary">
																...
																<div class="box-body">
						                                        	<form method="post" name="frmSearch3" id="frmSearch3" action="">
							                                            <table id="mtr_peritem" class="table table-bordered table-striped table-responsive search-table inner" width="100%">
							                                                <thead>
							                                                    <tr>
							                                                        <th width="2%">&nbsp;</th>
						                                        	...
						                                        	<form method="post" name="frmSearch3" id="frmSearch3" action="">
																		...
																		<div class="col-md-3">
															                <div class="form-group">
														                    	...
														                    	<input type="hidden" class="form-control" style="text-align:left" id="MAXREQ" name="MAXREQ" size="5" value="0.00" />
															                </div>
															            </div>
							                                            ...
							                                            <table id="mtr_peritemlist" class="table table-bordered table-striped table-responsive search-table inner" width="100%">
							                                                <thead>
							                                                    <tr>
									            	...
									            	<div class="<?php echo $Active4; ?> tab-pane" id="itm4" style="display: none;">
				                                        <div class="form-group">
				                                        	<form method="post" name="frmSearc4" id="frmSearchO" action="">
					                                            <table id="mtr_ovhlist" class="table table-bordered table-striped table-responsive search-table inner" width="100%">
					                                                <thead>
					                                                    <tr>
					                                                        <th width="2%" style="text-align: center; vertical-align: middle;">&nbsp;</th>
					                                                        <th width="40%" style="text-align: center; vertical-align: middle;"  nowrap><?php echo $ItemName; ?></th>
								            	...
								            	<?php } elseif($SPPCAT == 'FPA') { ?>
									            	<div class="<?php echo $Active1; ?> tab-pane" id="itm7">
				                                        <div class="form-group">
				                                        	<form method="post" name="frmSearch1" id="frmSearch1" action="">
					                                            <table id="alt_perjob" class="table table-bordered table-striped table-responsive search-table inner" width="100%">
					                                                <thead>
					                                                    <tr>
					                                                        <th width="2%" style="text-align: center; vertical-align: middle;">&nbsp;</th>
					                                                        <th width="40%" style="text-align: center; vertical-align: middle;"  nowrap><?php echo $ItemName; ?></th>
				...
				<script type="text/javascript">
					function rendJList()
					{
						var rowCExmpl4 = $('#mtr_peritemlist tr').length - 1;
					...
					function chgTOTREQ(thisVal1)
					{
						...
						var MAXREQ 	= parseFloat(document.getElementById('MAXREQ').value);

						if(thisVal > MAXREQ)
						{
							swal('Nilai yang dimasukan melebihi sisa yang tersedia.',
							{
								icon:"warning"
							})
							.then(function()
						...
						rowCExmpl4 	= $('#mtr_peritemlist tr').length - 1;
					...
					function setType(tabType)
					{
						if(tabType == 1)
						{
							...
							$('#mtr_perjob').DataTable(
					    	{
								...
								"autoWidth": true,
								"filter": true,
						        //"ajax": "<?php // echo site_url('c_purchase/c_pr180d0c/get_AllDataITMM/?id='.$PRJCODE.'&PRNUM='.$PR_NUM.'&ITMCAT=M')?>",
					        	"ajax": "<?php echo site_url('c_purchase/c_pr180d0c/get_AllDataITMALL/?id='.$PRJCODE.'&PRNUM='.$PR_NUM.'&ITMCAT=M')?>",
						        "type": "POST",
						...
						else if(tabType == 2)
						{
							...
							document.getElementById('itm3').style.display	= 'none';
							document.getElementById('itm4').style.display	= 'none';
							document.getElementById('itm7').style.display	= 'none';
						}
						else if(tabType == 3)
						{
							document.getElementById('itm1').style.display	= 'none';
						...
						if(tabType == 7)
						{
							...
							document.getElementById('itm3').style.display	= 'none';
							document.getElementById('itm4').style.display	= 'none';
							document.getElementById('itm7').style.display	= '';

							$('#alt_perjob').DataTable(
					    	{
					    		"destroy": true,
						        "processing": true,
						        "serverSide": true,
								...
								"autoWidth": true,
								"filter": true,
						        //"ajax": "<?php // echo site_url('c_purchase/c_pr180d0c/get_AllDataITMO/?id='.$PRJCODE.'&PRNUM='.$PR_NUM.'&ITMCAT=O')?>",
						        "ajax": "<?php echo site_url('c_purchase/c_pr180d0c/get_AllDataITMALL/?id='.$PRJCODE.'&PRNUM='.$PR_NUM.'&ITMCAT=M')?>",
						        "type": "POST",
								"lengthMenu": [[5, 10, 25, 50, 100, 200, -1], [5, 10, 25, 50, 100, 200, "All"]],
					...
					$(document).ready(function()
					{
						...
						{
							$('#mtr_subst').DataTable().ajax.reload();
						});*/

				    	$('#mtr_peritem').DataTable(
				    	{
					        "processing": true,
					        "serverSide": true,
							//"scrollX": false,
						...
						{
							$('#mtr_peritem').DataTable().ajax.reload();
						});

				    	$('#mtr_ovhlist').DataTable(
				    	{
				    		"destroy": true,
					        "processing": true,
					        "serverSide": true,
							...
							"filter": true,
					        //"ajax": "<?php // echo site_url('c_purchase/c_pr180d0c/get_AllDataITMO/?id='.$PRJCODE.'&PRNUM='.$PR_NUM.'&ITMCAT=O')?>",
					        "ajax": "<?php echo site_url('c_purchase/c_pr180d0c/get_AllDataITMALL/?id='.$PRJCODE.'&PRNUM='.$PR_NUM.'&ITMCAT=O')?>",
					        "type": "POST",
							"lengthMenu": [[5, 10, 25, 50, 100, 200, -1], [5, 10, 25, 50, 100, 200, "All"]],
							// "lengthMenu": [[10, 25, 50, 100, 200], [10, 25, 50, 100, 200]],
							"columnDefs": [	{ targets: [2,4,5], className: 'dt-body-center' },
						...
						{
							$('#mtr_ovhlist').DataTable().ajax.reload();
						});
				    	...
				    	$('#alt_perjob').DataTable(
				    	{
							...
							"filter": true,
					        //"ajax": "<?php // echo site_url('c_purchase/c_pr180d0c/get_AllDataITMO/?id='.$PRJCODE.'&PRNUM='.$PR_NUM.'&ITMCAT=O')?>",
					        "ajax": "<?php echo site_url('c_purchase/c_pr180d0c/get_AllDataITMALL/?id='.$PRJCODE.'&PRNUM='.$PR_NUM.'&ITMCAT=T')?>",
					        "type": "POST",
							"lengthMenu": [[5, 10, 25, 50, 100, 200, -1], [5, 10, 25, 50, 100, 200, "All"]],
							// "lengthMenu": [[10, 25, 50, 100, 200], [10, 25, 50, 100, 200]],
							"columnDefs": [	{ targets: [2,4,5], className: 'dt-body-center' },
						...
						{
							$('#alt_perjob').DataTable().ajax.reload();
						});
					});
					...
					function pickThis3(thisobj, row) 
					{
						/*var myarr 	= response.split("|");
						PRJCODE 	= myarr[0];
						ITM_CODE 	= myarr[1];*/
						...
						PR_NUM 		= "<?=$PR_NUM?>";
						var MAXREQ 	= document.getElementById('MAXREQ_'+row).value;
						document.getElementById('MAXREQ').value 		= parseFloat(MAXREQ);
						...
						{
							$('#mtr_peritemlist').DataTable().ajax.reload();
						});
					}
		                                        ...
		                                        <div class="form-group">
		                                        	<form class="form-horizontal" name="frm" method="post" action="" enctype="multipart/form-data" onSubmit="return checkFormDesc()">
														<div class="box-body">
											                ...
											                <div class="form-group">
											                    <div class="col-sm-12">
											                    	<label for="inputName" class="control-label"><?php echo $Specification; ?></label>
											                    	<input type="text" class="form-control" style="text-align:left" id="ITM_SPEC" name="ITM_SPEC" size="5" value="" />
											                    </div>
											                </div>
											            </div>
...
<script>
	...
	$(document).ready(function()
	{
		$('#tbl').DataTable(
    	{
	        ...
	        "ajax": {
				        "url": "<?php echo site_url('c_purchase/c_pr180d0c/get_AllDataIITMDET/?id='.$PRJCODE.'&PR_NUM='.$PR_NUM.'&task='.$task.'&SPPCAT='.$SPPCAT)?>",
				        "type": "POST",
						"beforeSend": function() {
			...
			"columnDefs": [	{ targets: [0,9], className: 'dt-body-center' },
							{ targets: [4,5,6,7,8,9,10,11], className: 'dt-body-right' },
							{ targets: [5], visible: false},
							{ targets: [7], visible: false},
							{ targets: [9], visible: false},
							{ targets: [10], visible: false},
							{ targets: [11], visible: <?php echo $visible_R; ?>}
						  ],
        	"order": [[ 2, "desc" ]],
			...
			"footerCallback": function (row, data, start, end, display) {
				...
				// Update footer
				$(api.column(6).footer()).html('<div style="font-weight: bold;">'+totalTOTPRQTYV+'</div>');
				...
				totalPRVOLM = totalPR_VOLM - totalPR_CVOL;
				var totalPRVOLMV = doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(totalPRVOLM)), 3));

				// Update footer
				$(api.column(8).footer()).html('<div style="font-weight: bold;">'+totalPRVOLMV+'</div>');
			}
		});
		
		$('div.toolbar').html('<form id="form-filter" class="form-horizontal">'+
							  '<select class="form-control select2" name="ITM_SRC" id="ITM_SRC" data-placeholder="Cari Item" style="width: 200px;">'+
							  '<option value=""></option></select>&nbsp;'+
							  '<select class="form-control select2" name="PRDESC_SRC" id="PRDESC_SRC" data-placeholder="Cari Deskripsi" style="width: 200px;">'+
							  '<option value=""></option></select>&nbsp;'+
							  '<button type="button" id="btn-filter" class="btn btn-primary"><i class="fa fa-filter"></i>&nbsp;Filter</button>&nbsp;'+
							  '<button type="button" id="btn-reset" class="btn btn-success"><i class="fa fa-refresh"></i>&nbsp;Reset</button>'+
							  '</form>');
		...
		// GET ITM_SRC
			...
			var PR_NUM 	= "<?=$PR_NUM?>";
			$.ajax({
				url: "<?php echo site_url('c_purchase/c_pr180d0c/get_ITMSRC'); ?>",
				type: "POST",
				dataType: "JSON",
		...
		$('#ITM_SRC').bind('change', function(e){
			...
			$.ajax({
				url: "<?php echo site_url('c_purchase/c_pr180d0c/get_PRDESCSRC'); ?>",
				type: "POST",
				dataType: "JSON",
	...
	function add_desc(strItem) 
	{
		...
		document.getElementById('data'+row+'PR_SPEC').value 	= ITM_SPEC;
		...
		// AUTO SAVE TO DATABASE
			...
			var collID		= PRJCODE+'~'+PR_NUM+'~'+ITM_CODE+'~'+PR_ID+'~'+DESC_ID+'~'+DESC_NOTES+'~'+ITM_SPEC;
	        var url 		= "<?=$updDESC?>";
	...
	{		
		swal({
			...
			if (willDelete) {
				let PR_NUM	= "<?php echo $PR_NUM; ?>";
				let PRJCODE	= "<?php echo $PRJCODE; ?>";
				$.ajax({
					type: "POST",
					url: "<?php echo site_url("c_purchase/c_pr180d0c/trashFile"); ?>",
					data: {PR_NUM:PR_NUM, PRJCODE:PRJCODE, fileName:fileName},
