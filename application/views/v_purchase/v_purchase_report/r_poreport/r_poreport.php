		    ...
		    <div class="box box-primary">
                <div class="box-body chart-responsive">
                	<form class="form-horizontal" method="post" name="frm" id="frm" action="<?php echo $form_action; ?>" onSubmit="return target_popup(this);" >
                        ...
                        <div class="form-group">
                            <label for="inputName" class="col-sm-2 control-label"><?php echo "No. PO"; ?></label>
                            <div class="col-sm-10">
                              	<select name="PO_NUM[]" id="PO_NUM" multiple class="form-control select2" data-placeholder="&nbsp;<?php echo "No. PO"; ?>" >
                                    <option value=""></option>
                          		</select>
                        	</div>
                        </div>
...
<script>
    ...
    $("#PO_NUM").on('change', function(){
        ...
        if(val !== null)
        {
            if(val == 1)
            {
                ...
                $("#PO_NUM option:nth-child(1)").prop("disabled", false);
            }
            else
            {
                $(this).select2({
                ...
                $("#PO_NUM option:nth-child(1)").prop("disabled", true);
            }
        }
        else
        {
    ...
    // Get PO_CODE
        $("#daterange-btn").on("change", function(e) {
            ...
            $.ajax({
                type: "POST",
                url: "<?php echo base_url("c_purchase/c_r3p0rt/getPO") ?>",
                dataType: "JSON",
                ...
                success: function(data) {
                    ...
                    let dataOpt = '';
                    if(lnData != 0)
                        dataOpt = '<option value=""></option>';

                    dataOpt = '<option value="1">Semua PO</option>';
                    for(let i=0; i<lnData; i++) {
                        dataOpt += '<option value="'+data[i].PO_NUM+'">'+data[i].PO_CODE+'</option>';
                    }
