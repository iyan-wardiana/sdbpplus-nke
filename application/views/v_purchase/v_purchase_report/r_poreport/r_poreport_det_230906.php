			              	...
			              	<tr style="line-height:1px; border-left:hidden; border-right:hidden">
			               	...
			               	<?php
								...
								if($PO_NUM[0] != 1)
								{
									...
									$addQuery 	.= " AND A1.PO_NUM IN ('$joinQPONUM')";
								}
								...
								$getPO 		= "SELECT A.PO_ID, A.PO_NUM, A1.PO_CODE, A.PO_DATE, A.PRJCODE, A.JOBCODEID, 
												...
												A.PO_PRICE, A.PO_COST, A.PO_DESC, A1.SPLCODE, A.ITM_CODE, A.PO_CVOL, A.PO_CTOTAL
												FROM tbl_po_detail A
												INNER JOIN tbl_po_header A1 ON A1.PO_NUM = A.PO_NUM AND A1.PRJCODE = A.PRJCODE
												WHERE A.PRJCODE = '$PRJCODE' AND A1.PO_STAT NOT IN (5,9)
											...
											foreach($resLPM->result() as $rLPM):
												...
												?>
													<tr>
														...
														<td style="text-align:left;border-top-width:1px; border-top-color:#000; padding: 5px;" nowrap><?php echo "$ITM_NAMEV"; ?><div class="text-mute" style="font-style: italic;"><?php echo "$PO_DESC ($JOBCODEID)"; ?></div></td>
