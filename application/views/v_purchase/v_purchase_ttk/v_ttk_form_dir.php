...
$s_INVC	= "SELECT COUNT(TOT_VOC) FROM
			(
				SELECT COUNT(*) AS TOT_VOC FROM tbl_pinv_detail A WHERE A.TTK_NUM = '$TTK_NUM' AND A.INV_STAT NOT IN (5,9) 
				UNION 
				SELECT COUNT(*) AS TOT_VOC FROM tbl_dp_header B WHERE B.TTK_NUM = '$TTK_NUM' AND B.DP_STAT NOT IN (5,9)
			) TOT_COUNTVOC";
$r_INVC	= $this->db->query($s_INVC);
if($r_INVC->num_rows() > 1)
{
	$editable 	= 0;
}

// echo $editable."=".$r_INVC->num_rows();
?>
<!DOCTYPE html>
<html>
  	<head>
	...
	<?php
		...
		if($TTK_CATEG == 'DP')
		{
			...
			else
			{
				$sqlSUPL	= "SELECT DISTINCT A.SPLCODE, B.SPLDESC, B.SPLADD1
								FROM tbl_po_header A
									INNER JOIN tbl_supplier B ON A.SPLCODE = B.SPLCODE
								WHERE A.PO_STAT IN (3,6) AND A.PO_DPSTAT = 0 AND A.PRJCODE = '$PRJCODE'
								UNION
								SELECT DISTINCT A.SPLCODE, B.SPLDESC, B.SPLADD1
								FROM tbl_wo_header A
									INNER JOIN tbl_supplier B ON A.SPLCODE = B.SPLCODE
								WHERE A.WO_STAT IN (3,6) AND A.WO_DPSTAT = 0 AND A.PRJCODE = '$PRJCODE' AND A.WO_DPPER > 0";
			}
		...
		else
		{
			...
			$sqlSUPL	= "SELECT SPLCODE, SPLDESC, SPLADD1
							FROM tbl_supplier
							WHERE SPLSTAT = 1";
			$vwSUPL		= $this->db->query($sqlSUPL);
		}
	            ...
	            <form class="form-horizontal" name="frm" id="frm" method="post" action="<?php echo $form_action; ?>" enctype="multipart/form-data" onSubmit="return checkForm()">
					<div class="col-md-6">
						<div class="box box-primary">
							...
							<div class="box-body">
			                    ...
			                    <script>
									...
									function getSUPPLIER(TTK_CATEG) 
									{
										...
										if(TTK_CATEG == 'DP')
										{
											...
											$('#TTK_AMOUNTX').attr('readonly', true);
										}
										else
										{
											$('#GuaranteeType').hide();
											...
											$('#TTK_AMOUNTX').attr('readonly', false);
										}
								...
								<div class="form-group" id="SelDoc" style="margin-top: 20px; display: <?php echo $divSelDoc; ?>">
		                            ...
		                            <div class="col-sm-4">
		                            	<div class="group-label">
											...
											<div class="label-top"><label><?php echo "Nilai OP / SPK"; ?></label></div>
					...
					<div class="col-md-6">
						<div class="box box-warning">
							...
							<div class="box-body">
								<div class="form-group" id="DPAmount" style="display: <?php echo $divDPAmount; ?>">
								...
								<div class="form-group">
		                            <label for="inputName" class="col-sm-3 control-label">Nilai Tagihan / PPn</label>
									<div class="col-sm-5">
									...
									<div class="col-sm-4">
		                                <?php if($TTK_STAT == 1 || $TTK_STAT == 4) { ?>
		                               		<input type="text" class="form-control" style="text-align:right" name="TTK_AMOUNT_PPNX" id="TTK_AMOUNT_PPNX" value="<?php echo number_format($TTK_AMOUNT_PPNH, 2); ?>" onBlur="getAmountPPn(this)" onKeyPress="return isIntOnlyNew(event);">
		                                <?php } else { ?>
								...
								<div class="form-group">
									...
									<div class="col-sm-4">
										<?php if($TTK_STAT == 1 || $TTK_STAT == 4) { ?>
		                               		<input type="text" class="form-control" style="text-align:right" name="TTK_AMOUNT_PPHX" id="TTK_AMOUNT_PPHX" value="<?php echo number_format($TTK_AMOUNT_PPHH, 2); ?>" onKeyPress="return isIntOnlyNew(event);" onchange="pphVal(this)">
		                                <?php } else { ?>
			                    ...
			                    <div class="form-group">
			                   	  <label for="inputName" class="col-sm-3 control-label"><?php echo $Notes; ?></label>
			                      	<div class="col-sm-5">
			                            <textarea class="form-control" name="TTK_NOTES"  id="TTK_NOTES" rows="5"><?php echo $TTK_NOTES; ?></textarea>
			                      	</div>
									<div class="col-sm-4">
		                          	...
		                          	<div class="col-sm-4">
		                                ...
		                                <input type="text" class="form-control" style="text-align:right; background-color: rgb(225, 193, 110); font-weight: bold;" name="TTK_TOTKWIT" id="TTK_TOTKWIT" value="<?php echo number_format($TTK_TOTKWIT, 2); ?>" onKeyPress="return isIntOnlyNew(event);" disabled >
		                          	</div>
			                    </div>
		                        ...
		                        <div class="form-group" id="voidNote" style="display: none;">
		                       	  <label for="inputName" class="col-sm-3 control-label"><?php echo $OthNotes; ?></label>
		                          	<div class="col-sm-9">
		                                <textarea class="form-control" name="TTK_NOTES1"  id="TTK_NOTES1"><?php echo $TTK_NOTES1; ?></textarea>
		                          	</div>
		                        </div>
			                    <div class="form-group">
		                                                    ...
		                                                    <tr id="trTAX_<?php echo $rowTAX; ?>">
		                                                        ...
		                                                        <td style="text-align:left;" nowrap>
		                                                        	<?php 
			                                                        	...
			                                                        	{
			                                                        		?>
			                                                            	<input type="text" id="dataTAX<?php echo $rowTAX; ?>TTKT_SPLINVDATE" name="dataTAX[<?php echo $rowTAX; ?>][TTKT_SPLINVDATE]" value="<?php echo $TTKT_SPLINVDATE; ?>" class="form-control" maxlength="100">
			                                                        		<?php 
			                                                        	} 
			                                                        	else 
			                                                        	{
			                                                        		echo $TTKT_SPLINVDATE;
			                                                        		?>
			                                                            	<input type="hidden" id="dataTAX<?php echo $rowTAX; ?>TTKT_SPLINVDATE" name="dataTAX[<?php echo $rowTAX; ?>][TTKT_SPLINVDATE]" value="<?php echo $TTKT_SPLINVDATE; ?>" class="form-control" maxlength="100">
			                                                        		<?php 
			                                                        	}
		                                                        	?>
		                                                        </td>
		                                                        <td style="text-align:right;" nowrap>
		                                                        	<?php 
			                                                        	...
			                                                        	{ 
			                                                        		...
			                                                        		?>
			                                                            	<input type="hidden" id="TTKT_SPLINVVAL<?php echo $rowTAX; ?>" value="<?php echo number_format($TTKT_SPLINVVAL, 2); ?>" class="form-control" style="min-width:100px; text-align: right">
			                                                        		<?php 
			                                                        	}
		                                                        	?>
...
<script>
	$(function () {
		...
		$('#TTK_AMOUNTX').on('blur', function(e){
			let TTK_AMOUNT		= parseFloat(eval(e.target).value.split(",").join(""));
			$('#TTK_AMOUNTX').val(doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(TTK_AMOUNT)), 2)));
			$('#TTK_AMOUNT').val(TTK_AMOUNT);
		})
	});
	...
	{
		let TTK_CATEG = document.getElementById('TTK_CATEG').value;
		console.log(strItem);
		...
		{	
			document.getElementById('TTK_AMOUNT').value 	= 0;
			document.getElementById('TTK_AMOUNTX').value = doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(0)),2));
		}
	...
	function add_TAXNO() 
	{
		...
		// START : SETTING ROWS INDEX
			...
			if(intIndex == 1)
			{
				...
				if(TTK_CATEG == 'OTH')
				{
					TTKT_AMOUNT 	= TTK_AMOUNT;
					TTKT_TAXAMOUNT 	= parseFloat(0.11 * TTKT_AMOUNT);
				}

				document.getElementById('TTK_AMOUNT_PPN').value = TTKT_TAXAMOUNT;
				document.getElementById('TTK_AMOUNT_PPNX').value = doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(TTKT_TAXAMOUNT)),2));
			}
	...
	function checkTotalTTK()
	{
		...
		let TTK_CATEG 	= document.getElementById('TTK_CATEG').value;
		// TTK_AMOUNT		= 0;
		TTK_AMOUNT		= document.getElementById('TTK_AMOUNT').value;
		TTK_AMOUNT_PPN	= 0;
		TTK_GTOTAL		= 0;
		TTK_TOTPOT 		= 0;
		...
		if (typeof varTAX !== 'undefined' && varTAX !== null)
		{
			...
			for(i=1; i<=totalrowTAX; i++)
			{
				...
				if(theObj != null)
				{
					...
					TTKT_TAXNO		= document.getElementById('dataTAX'+i+'TTKT_TAXNO').value;

					if(TTKT_TAXAMOUNT > 0 && TTKT_TAXNO == "")
					{
					...
					TTK_TOTAMOUNT 	= parseFloat(TTK_TOTAMOUNT) + parseFloat(TTKT_AMOUNT);
				}
			}
		}
		...
		// TTK AMOUNT DIGANTI DENGAN NILAI TAGIHAN DETAIL
		TTK_AMOUNT 			= parseFloat(TTK_TOTAMOUNT);

		console.log('TTK_AMOUNT_PPN-A = '+TTK_AMOUNT_PPN)
		...
		console.log('TTK_AMOUNT_PPN-B = '+TTK_AMOUNT_PPN)
		if(TTK_CATEG == 'DP')
		{
			//TTK_AMOUNT = DP_AMOUNT
		}
		...
		console.log('TTK_AMOUNT_PPN-C = '+TTK_AMOUNT_PPN)
		...
		TTK_GTOTAL			= parseFloat(TTK_AMOUNT) + parseFloat(TTK_AMOUNT_PPN) - parseFloat(TTK_AMOUNT_PPH);
	...
	function getPPH(TAX_NUM)
	{
		...
		$.ajax({
			...
			success: function(response)
			{
				...
				let PPH_VAL 		= parseFloat(TTK_AMOUNT) * parseFloat(response) / 100;
	...
	function pphVal(thisVal1)
	{
		thisVal 			= eval(thisVal1).value.split(",").join("");
				
		document.getElementById('TTK_AMOUNT_PPH').value		= thisVal;
		document.getElementById('TTK_AMOUNT_PPHX').value	= doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(thisVal)), 2));
