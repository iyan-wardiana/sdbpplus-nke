							...
							<script type="text/javascript">
								$(document).ready(function()
								{
									...
									$.ajax({
										...
										success: function(result) {
											...
											for(let i in result) {
												...
												POCODEVW 	+= '<option value="'+PONUM+'">'+POCODE+'</option>';
											}
											$('#PO_NUM').html(POCODEVW);
											$('#PO_NUMSJ').html(POCODEVW);
										}
									});

									$('#PO_NUM').change(function(e)
									{
										id = $(this).val();
										$('#PO_NUMX').val(id);
									});
							    	...
							    	<?php
								    	if($TTK_CATEG1 == 'IR' || $TTK_CATEG1 == 'SJ')
										{
									    	...
									    	$('#example3').DataTable(
									    	{
												"dom": "<'row'<'col-sm-2'l><'col-sm-8'<'toolbarSJ'>><'col-sm-2'f>>"+
												"<'row'<'col-sm-12'tr>>",
										        "processing": true,
										        "serverSide": true,
												//"scrollX": false,
												...
												"ajax": {
											        "url": "<?php echo $popURLSJ;?>",
											        "type": "POST",
													"data": function(data) {
														data.PO_NUM = $('#PO_NUMSJ').val();
													},
										        },
										        "type": "POST",
												//"lengthMenu": [[10, 25, 50, 100, 200, -1], [10, 25, 50, 100, 200, "All"]],
											...
											$('div.toolbarSJ').html('<form id="form-filterSJ" class="form-horizontal">'+
														  '<select class="form-control select2" name="PO_NUMSJ" id="PO_NUMSJ" data-placeholder="Daftar PO" style="width: 250px;">'+
														  '<option value=""></option></select>&nbsp;'+
														  '<button type="button" id="btn-filterSJ" class="btn btn-success"><i class="fa fa-check"></i>&nbsp;&nbsp;OK</button>&nbsp;'+
														  '</form>');

											$('#btn-filterSJ').bind('click', function(){
								...
								$(document).ready(function()
								{
								   	$("#btnDetail1").click(function()
								    {
								    	...
								    	$("#tbl_tax").find("tr:not(:first)").remove();
								    	$("#tbl_exp").find("tr:not(:first)").remove();
