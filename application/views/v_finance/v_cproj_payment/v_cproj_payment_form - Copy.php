...
/* 
	...
	* File Name	= v_cproj_payment_form.php
	* Location		= -
*/
...
endforeach;
// HO
$sqlHO 	= "SELECT PRJNAME FROM tbl_project WHERE PRJCODE = '$PRJCODE_HO'";
...
{	
	...
	$sql 	= "tbl_journalheader WHERE YEAR(JournalH_Date) = $yearC AND proj_Code = '$PRJCODE' AND JournalType = 'CPRJ'";
...
<h1>
    <?php echo $subTitleD; ?>
    <small><?php echo "$PRJNAME$PRJNAME1"; ?></small>  </h1>
        ...
        <form class="form-horizontal" name="frm" method="post" action="<?php echo $form_action; ?>" enctype="multipart/form-data" onSubmit="return validateInData()">
            ...
            <input type="Hidden" name="rowCount" id="rowCount" value="0">
            	...
            	<div class="form-group">
                    ...
                    <div class="col-sm-10">
                    	<input type="text" class="form-control" name="Manual_No" id="JournalH_Code" size="30" value="<?php echo $Manual_No; ?>" />
                    </div>
                </div>
            	<div class="form-group">
            	...
            	<div class="form-group" style="display:none">
                    ...
                    <div class="col-sm-10">
                    	<select name="proj_Code" id="proj_Code" class="form-control" disabled>
                          ...
                          <?php
						  	$theProjCode 	= $PRJCODE;
                        	$url_AddItem	= site_url('c_finance/c_cpa70d18/puSA0b28t18/?id=');
                            if($countPRJ > 0)
                            {
            	...
            	<div class="form-group">
                    <label for="inputName" class="col-sm-2 control-label"><?php echo $CashAccount; ?></label>
                    <div class="col-sm-10">
                    	<select name="acc_number" id="acc_number" class="form-control select2">
                          <option value="none">--- None ---</option>
                          <?php
						  	$theProjCode 	= $PRJCODE;
                        	$url_AddItem	= site_url('c_finance/c_cpa70d18/puSA0b28t18yXP/?id=');
                            if($cAllCOA>0)
							{
								$totRow		= 0;
            	...
            	<div id="idmemo" class="form-group" <?php if($JournalH_Desc2 == '') { ?> style="display:none" <?php } ?>>
            	...
            	<div class="form-group">
                    ...
                    <script>
						function selStat(thisValue)
						{
							...
							{
								if(STAT_BEFORE == thisValue)
								{
									document.getElementById('updateVoid').style.display = 'none';
									document.getElementById('idmemo').style.display 	= 'none';
								}
								else
								{
									document.getElementById('updateVoid').style.display = '';
									document.getElementById('idmemo').style.display 	= '';
								}
							}
							else
							{
                    ...
                    <div class="col-md-12">
                        ...
                        <table width="100%" border="1" id="tbl">
                        	<tr style="background:#CCCCCC">
                              ...
                              <th width="12%" style="text-align:center">Volm</th>
								...
								foreach($result as $row) :
									...
									<td width="2%" height="25" style="text-align:center">
								 	...
								 	<td width="8%" style="text-align:left;">
                                        ...
                                        <input type="text" name="data[<?php echo $currentRow; ?>][Acc_Id]" id="data<?php echo $currentRow; ?>Acc_Id" value="<?php echo $Acc_Id; ?>" class="form-control" style="max-width:100px; <?php if ($edited == 0) { ?> display:none; <?php } ?>" onClick="selectAccount(<?php echo $currentRow; ?>);" placeholder="<?php echo $SelectAccount; ?>" size="10" >
                                        <?php if ($edited == 0) { ?> 
                                        	<input type="text" name="data<?php echo $currentRow; ?>Acc_Id" id="data<?php echo $currentRow; ?>Acc_Id" value="<?php echo $Acc_Id; ?>" class="form-control" style="max-width:100px;" onClick="selectAccount(<?php echo $currentRow; ?>);" placeholder="<?php echo $SelectAccount; ?>" size="10" disabled >
										<?php } ?>
									...
									<td width="6%" nowrap style="text-align:center; display:none">
								  	...
								  	<td width="12%" style="text-align:left">
                                        <input type="text" class="form-control" style="text-align:right" name="ITM_VOLM<?php echo $currentRow; ?>" id="ITM_VOLM<?php echo $currentRow; ?>" value="<?php echo number_format($ITM_VOLM, 2); ?>" placeholder="Volume" onBlur="chgVolume(this,<?php echo $currentRow; ?>)" onKeyPress="return isIntOnlyNew(event);" <?php if ($edited == 0) { ?> disabled <?php } ?>>
								  	...
								  	<td width="11%" style="text-align:right; <?php if($disButtonR == 1) { ?> background-color:#F00<?php } ?>">
                ...
                <div class="form-group">
                    <div class="col-sm-offset-2 col-sm-10">
                        <?php
							...
							{
								if($disButton == 0 && $disButtonR == 0)
								{
									?>
                                        <button class="btn btn-primary" id="updateVoid">
								...
								{
									?>
                                        <button class="btn btn-primary" id="updateVoid" style="display:none">
                                        <i class="fa fa-save"></i>&nbsp;&nbsp;<?php echo $Update; ?>
                                        </button>&nbsp;
									<?php
								}
							...
							$backURL	= site_url('c_finance/c_cpa70d18/cp2b0d18_all/?id='.$this->url_encryption_helper->encode_url($PRJCODE));
...
<script>
  $(function () {
	...
	function add_listAcc(strItem) 
	{
		...
		objTD.noWrap = true;
		objTD.innerHTML = '<input type="text" name="data'+intIndex+'ITM_UNIT" id="ITM_UNIT'+intIndex+'" value="'+ITM_UNIT+'" class="form-control" style="max-width:150px;" readonly ><input type="hidden" name="data['+intIndex+'][ITM_UNIT]" id="data'+intIndex+'ITM_UNIT" value="'+ITM_UNIT+'" class="form-control" style="max-width:150px;" >';
	...
	function chgAmount(thisval, row)
	{
		...
		if(Acc_Amount > Rem_BudAmn)
		{
			...
			alert("Out of budget, max amount "+Rem_BudAmnV)
			Acc_Volume	= parseFloat(Rem_BudAmn / ITM_PRICE);
			Acc_Amount	= parseFloat(Rem_BudAmn);
		}
		else
	...
	function chgVolume(thisval, row)
	{
		...
		if(Acc_Volume > Rem_Budget)
		{
			...
			*/
			...
			alert("Out of budget, max volm "+Rem_BudgetV)
			Acc_Volume	= parseFloat(Rem_Budget);
		}
