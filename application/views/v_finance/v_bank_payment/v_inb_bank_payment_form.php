...
$JournalY 		= date('Y', strtotime($CB_DATE1));
$JournalM 		= date('n', strtotime($CB_DATE1));
...
<html>
	...
	<?php
		...
		if($LangID == 'IND')
		{
			...
			$alert5		= "Masukan alasan mengapa dokumen ini di-revise/reject.";
			...
			$alert9		= "Silahkan pilih status persetujuan.";
			$alert10	= "Silahkan tulis catatan alasan Anda merevisi dokumen ini.";
			$docNo		= "Dok. Sumber";
		...
		else
		{
			...
			$alert5		= "Input the reason why you revise/reject this document.";
			...
			$alert9		= "Please select approval status.";
			$alert10	= "Please write why you revising this document.";
			$docNo	= "Source Doc.";
		...
		elseif($CB_SOURCE == 'PPD')
		{
			$sql	= "SELECT DISTINCT A.PERSL_EMPID AS SPLCODE, CONCAT(B.First_Name,' ', B.Last_Name) AS SPLDESC, '' AS Vend_Address
						FROM tbl_journalheader_pd A
							INNER JOIN tbl_employee B ON A.PERSL_EMPID = B.Emp_ID
						WHERE A.GEJ_STAT = 3 AND A.GEJ_STAT_PPD = 2
						UNION ALL
						SELECT DISTINCT A.PERSL_EMPID AS SPLCODE, B.SPLDESC AS SPLDESC, '' AS Vend_Address
						FROM tbl_journalheader_pd A
							INNER JOIN tbl_supplier B ON A.PERSL_EMPID = B.SPLCODE
		...
		$TOT_AMOUNT_DP 	= 0;
		$TOTPOT_DP 		= 0;
		$s_POTDP		= "SELECT SUM(AMOUNT_DP) AS TOTPOT_DP FROM tbl_bp_detail WHERE CB_NUM = '$CB_NUM'";
		$r_POTDP		= $this->db->query($s_POTDP)->result();
		foreach($r_POTDP as $rw_POTDP):
			$TOTPOT_DP	= $rw_POTDP->TOTPOT_DP;
		endforeach;
		if($TOTPOT_DP == '')
			$TOTPOT_DP 	= 0;
		...
		$sqlTOTDP	= "SELECT SUM(A.BP_AMOUNT - A.DP_AMOUNT_USED) AS TOT_AMOUNT_DP FROM tbl_dp_header A
						WHERE A.SPLCODE = '$CB_PAYFOR' 
						AND A.DP_AMOUNT > A.DP_AMOUNT_USED AND A.BP_AMOUNT > 0";
		$resTOTDP	= $this->db->query($sqlTOTDP)->result();
		foreach($resTOTDP as $rowTOTDP):
			$TOT_AMOUNT_DP	= $rowTOTDP->TOT_AMOUNT_DP;
		endforeach;
		...
		<section class="content">
			<div class="row">
			    ...
			    <form name="frm" id="frm" method="post" action="<?php echo $form_action; ?>" enctype="multipart/form-data" onSubmit="return validateInData()">
	                        ...
	                        <div class="box-body">
	                        	...
	                        	<div class="col-sm-4"> <!-- CB_NUM -->
	                                <div class="form-group">
	                                    ...
	                                    <select name="CB_PAYTYPEX" id="CB_PAYTYPEX" class="form-control select2" disabled>
			                                <option value="CB" <?php if($CB_PAYTYPE == 'CB'){ ?> selected <?php } ?>> Kas / Bank </option>
			                                <option value="PD" <?php if($CB_PAYTYPE == 'PD'){ ?> selected <?php } ?>> Pinjaman Dinas (PD)</option>
			                            </select>
			                            <input type="hidden" name="CB_PAYTYPE" class="form-control pull-left" id="CB_PAYTYPE" value="<?php echo $CB_PAYTYPE; ?>">
	                                </div>
	                            </div>
	                        	...
	                        	<div class="col-sm-4"> <!-- CB_SOURCE -->
	                                <div class="form-group">
	                                    ...
	                                    <select name="CB_SOURCEX" id="CB_SOURCEX" class="form-control select2" onChange="selectAccount(this.value)" disabled>
			                                <option value=""> --- </option>
			                                ...
			                                <option value="PD" <?php if($CB_SOURCE == 'PD'){ ?> selected <?php } ?>>Pinjaman Dinas</option>
	                        ...
	                        <div class="box-body">
	                        	...
	                        	<div class="col-sm-12"> <!-- CB_CURRID, Acc_ID -->
	                                <div class="form-group">
			                            ...
			                            <select name="CB_ACCID1" id="CB_ACCID1" class="form-control select2" onChange="selectAccount(this.value)" disabled>
			                                ...
			                                <?php
			                                	$s_ACC	 = "SELECT DISTINCT
																Account_Number, 
																Account_Nameen as Account_Name,
																Account_Category,
																Account_Class,			
																currency_ID,
											                    Base_OpeningBalance,
												...
												foreach($r_ACC as $rw_ACC) :
													...
													$Account_Number = $rw_ACC->Account_Number;
													$Account_Name 	= cut_text ($rw_ACC->Account_Name, 50);
	                            ...
	                            <?php
									...
									// CEK TOTAL YANG AKAN DIBAYAR
			                            ...
			                            {
			                                $sqlDET	= "SELECT A.INV_AMOUNT, A.INV_AMOUNT_PPN, A.INV_AMOUNT_PPH, A.CBD_AMOUNT
	                        	...
	                        	<div class="col-sm-12"> <!-- STATUS -->
	                                <div class="form-group">
										...
										<?php
											/* Sudah ada pengecekan di voucher ---> Upd: 12 Juli 2023 ----------
											$REM_DP 		= $TOT_AMOUNT_DP-$TOTPOT_DP;
											if($REM_DP < 0)
											{
												$descApp 	= "Tidak bisa diapprove karena ada dokumen potongan uang muka yang melebihi sisa uang muka.";
												$canApprove = 0;
											}
											---------------------- End Hidden -------------------- */
			                                ...
			                                ?>
			                                    <select name="CB_STAT" id="CB_STAT" class="form-control select2" onChange="selStat(this.value)" >
			                                        <option value=""> --- </option>
			                                        <option value="3"<?php if($CB_STAT == 3) { ?> selected <?php } ?> <?php if($disabled == 1) { ?> disabled <?php } ?>>Approve</option>
								...
								<script>
			                        function selStat(thisValue)
			                        {
			                            ...
			                            if(thisValue == 3)
			                            {
			                                ...
			                                document.getElementById('bpMemo').style.display 		= 'none';
			                            }
			                            if(thisValue == 4 || thisValue == 5)
			                            {
			                                document.getElementById('btnSave').style.display 		= '';
			                                document.getElementById('bpMemo').style.display 		= '';
			                            }
			                        }
			                    </script>
	                        	<div class="col-sm-12">
			                        	...
			                        	<?php
			                        		...
			                        		elseif(($CB_PAYFOR != '') && ($CB_STAT == 1 || $CB_STAT == 4) && $CB_SOURCE == 'DP')
			                        		{
			                        			?>
													...
													<a class="btn btn-sm btn-warning" data-toggle="modal" data-target="#mdl_addDP" id="btnModal" style="display: none;">
						                        		<i class="glyphicon glyphicon-search"></i>&nbsp;&nbsp;<?php echo $DPCode; ?>
						                        	</a>
			                        			<?php
			                        		}
	                ...
	                <div class="col-sm-12" id="bpMemo" style="display: none;">
	                	<div class="box box-primary">
	                        <div class="box-header with-border">
	                            <h3 class="box-title center"><?php echo $Notes; ?> Revise / Reject</h3>
	                        </div>
	                        <div class="box-body">
                                <div class="form-group">
                                    <textarea name="CB_MEMO" class="form-control" id="CB_MEMO" cols="30" style="height: 75px"><?php echo $CB_MEMO; ?></textarea>
                                </div>
	                        </div>
	                    </div>
	                </div>
				                            ...
				                            if($CB_SOURCENO == '' && $task == 'edit')
				                            {
				                                ...
				                                if($resultCount > 0)
				                                {
				                                    foreach($result as $row) :
														...
														$OPNIR_NUM		= $row->OPNIR_NUM;
														$OPNIR_CODE		= $row->OPNIR_CODE;
														$SRC_NUM		= $row->SRC_NUM;
				                                        ...
				                                        <tr>
					                                        ...
					                                        <td style="text-align:right" > <!-- CBD_AMOUNT -->
					                                      	...
					                                      	<td style="text-align:right;"> <!-- AMOUNT_DP -->
					                                        	...
					                                        	<input type="hidden" name="data[<?php echo $currentRow; ?>][TTK_NUM]" id="data<?php echo $currentRow; ?>TTK_NUM" value="<?php echo $TTK_NUM; ?>" class="form-control" style="max-width:500px;" >
					                                        	<input type="hidden" name="data[<?php echo $currentRow; ?>][TTK_CODE]" id="data<?php echo $currentRow; ?>TTK_CODE" value="<?php echo $TTK_CODE; ?>" class="form-control" style="max-width:500px;" >
					                                        	<input type="hidden" name="data[<?php echo $currentRow; ?>][OPNIR_NUM]" id="data<?php echo $currentRow; ?>OPNIR_NUM" value="<?php echo $OPNIR_NUM; ?>" class="form-control" style="max-width:500px;" >
					                                        	<input type="hidden" name="data[<?php echo $currentRow; ?>][OPNIR_CODE]" id="data<?php echo $currentRow; ?>OPNIR_CODE" value="<?php echo $OPNIR_CODE; ?>" class="form-control" style="max-width:500px;" >
					                                        	<input type="hidden" name="data[<?php echo $currentRow; ?>][SRC_NUM]" id="data<?php echo $currentRow; ?>SRC_NUM" value="<?php echo $SRC_NUM; ?>" class="form-control" style="max-width:500px;" >
					                                        	<input type="hidden" name="data[<?php echo $currentRow; ?>][SRC_CODE]" id="data<?php echo $currentRow; ?>SRC_CODE" value="<?php echo $SRC_CODE; ?>" class="form-control" style="max-width:500px;" >
	                ...
	                <div class="col-sm-12">
	                	<div class="box box-warning">
	                        <div class="box-header with-border">
                                <h3 class="box-title center"><?php echo $payInfonOth; ?></h3>
	                        </div>
	                        <div class="box-body">
					            <div class="search-table-outter">
					            ...
					            <div class="search-table-outter">
					                <table id="tbl_pd" class="table table-bordered table-striped table-responsive search-table inner" width="100%">
			                        	<tr style="background:#CCCCCC">
			                              	...
			                              	<th width="15%" style="text-align:center"><?php echo $Amount; ?></th>
											...
											foreach($result as $row) :
												...
												// START : GET REMAIN
													$s_REM 			= "SELECT A.Journal_Amount, A.Journal_AmountTsf, A.Journal_AmountReal, A.PDPaid_Amount, A.PDRec_Amount, A.PPD_RemAmount
																		FROM tbl_journalheader_pd A
																		WHERE A.proj_Code = '$PRJCODE' AND A.JournalH_Code = '$PD_NUM'";
			                                    ...
			                                    <tr id="trpd_<?php echo $pdRow; ?>">
			                                        ...
			                                        <td style="text-align:left;" nowrap>
			                                        	<?php if($editable == 1) { ?>
			                                        		<input type="text" name="dataPD[<?=$pdRow?>][PD_NOTES]" id="dataPD<?=$pdRow?>PD_NOTES" value="<?=$PD_NOTES?>" class="form-control" style="text-align:left" placeholder="<?php echo $Remarks; ?>">
			                                        	<?php } else { ?>
			                                        		<?php echo $PD_NOTES; ?>
			    	...
			    	if(($CB_PAYFOR != '') && ($CB_STAT == 1 || $CB_STAT == 4) && $CB_SOURCE != 'DP')
			    	{
						...
						if($CB_SOURCE == 'VCASH' || $CB_SOURCE == 'PD' || $CB_SOURCE == 'PPD')
						{
				    		...
				    		$sqlSPL 	= "SELECT CONCAT(First_Name,' ', Last_Name) AS SPLDESC
				    						FROM tbl_employee WHERE Emp_ID = '$CB_PAYFOR'";
		                    $resSPL 	= $this->db->query($sqlSPL)->result();
							foreach($resSPL as $rowSPL) :
								$SPLDESC 	= $rowSPL->SPLDESC;
							endforeach;
							                                            ...
							                                            <table id="example0" class="table table-bordered table-striped table-responsive search-table inner" width="100%">
							                                                <thead>
							                                                    <tr>
															                        ...
															                        <th width="16%" style="text-align:center; vertical-align: middle;" nowrap><?php echo $InvoiceNo; ?> </th>
															                        ...
															                        <th width="8%" style="text-align:center; vertical-align: middle;" nowrap>Total</th>
															                  	</tr>
							                                                </thead>
							                                                <tbody>
							                                                </tbody>
						                                        	...
						                                        	<form method="post" name="frmSearch1" id="frmSearch1" action="">
							                                            <table id="example1" class="table table-bordered table-striped table-responsive search-table inner" width="100%">
							                                                <thead>
							                                                    <tr>
															                        <th width="2%" style="text-align:center; vertical-align: middle;">&nbsp;</th>
															                        <th width="16%" style="text-align:center; vertical-align: middle;" nowrap><?php echo $VCNo; ?> </th>
															                        <th width="37%" style="text-align:center; vertical-align: middle;"><?php echo $Description; ?> </th>
															                        <th width="6%" style="text-align:center; vertical-align: middle;" nowrap><?php echo $Date; ?>  </th>
															                        <th width="7%" style="text-align:center; vertical-align: middle;" nowrap><?php echo $Amount; ?></th>
															                        <th width="6%" style="text-align:center; vertical-align: middle;" nowrap><?php echo $PPn; ?></th>
															                        <th width="3%" style="text-align:center; vertical-align: middle;" nowrap>PPh</th>
															                        <th width="3%" style="text-align:center; vertical-align: middle;" nowrap>Retensi</th>
								...
								<script type="text/javascript">
									$(document).ready(function()
									{
								    	...
								    	$('#example1').DataTable(
								    	{
											...
											"autoWidth": true,
											"filter": true,
									        "ajax": "<?php echo site_url('c_finance/c_bp0c07180851/get_AllDataVC/?id='.$colPAYERID)?>",
									        "type": "POST",
											//"lengthMenu": [[10, 25, 50, 100, 200, -1], [10, 25, 50, 100, 200, "All"]],
											"lengthMenu": [[10, 25, 50, 100, 200], [10, 25, 50, 100, 200]],
											"columnDefs": [	{ targets: [0], className: 'dt-body-center' },
...
<script>
	...
	function validateInData()
	{
		...
		if(CB_STAT != 4 && (parseFloat(TotRemAccount) < parseFloat(GTInPay)))
		{
			// sementara di-hide dulu. Req. by Ibu Lyta 04 Maret 2022
			/*swal('<?php echo $alert8; ?>',
			{
		...
		if(CB_STAT == 4 || CB_STAT == 5)
		{
			CB_MEMO			= document.getElementById('CB_MEMO').value;
			if(CB_MEMO == '')
			{
				...
				.then(function()
				{
					document.getElementById('CB_MEMO').focus();
				});
				return false;
			}
		}
