                ...
                <table width="100%" border="1" cellpadding="0" cellspacing="0">
					...
					<tbody>
						...
						// get PRJ & SPL
							...
							if($DOC_STAT != 0)
							{
								$addQSTAT_INV 		= "AND B.INV_STAT = '$DOC_STAT'";
								$addQSTAT_DP 		= "AND A.DP_STAT = '$DOC_STAT'";
								$addQSTAT_VCASH 	= "AND A.GEJ_STAT = '$DOC_STAT'";
								$addQSTAT_PPD 		= "AND A.GEJ_STAT = '$DOC_STAT'";
							}
							else
							{
								$addQSTAT_INV 		= "";
							...
							// get HUT. Supplier
								$get_HUTSPL 	= "SELECT B.INV_NUM, B.INV_CODE, '' AS Reference_Number, B.INV_DATE, B.INV_DUEDATE, B.INV_AMOUNT_TOT, B.INV_AMOUNT_PAID,
													B.PRJCODE, B.SPLCODE, D.SPLDESC, B.STATDESC, IF(ROUND(B.INV_AMOUNT_PAID,2) = ROUND(B.INV_AMOUNT_TOT,2), 'Lunas', 'Belum Lunas') AS PAY_STAT
													...
													WHERE B.INV_STAT NOT IN (5,9)
													$ADDQRYPRJ2 $ADDQRYSPL2 $ADDQRYREPINV_STAT $addQSTAT_INV
													AND B.INV_DATE BETWEEN '$Start_Date' AND '$End_Date'
													UNION
													SELECT A.DP_NUM AS INV_NUM, A.DP_CODE AS INV_CODE, '' AS Reference_Number, A.DP_DATE AS INV_DATE, '' AS INV_DUEDATE, A.DP_AMOUN_TOT AS INV_AMOUNT_TOT, BP_AMOUNT AS INV_AMOUNT_PAID,
													A.PRJCODE, A.SPLCODE, B.SPLDESC, A.STATDESC, IF(ROUND(A.BP_AMOUNT) = ROUND(A.DP_AMOUN_TOT), 'Lunas', 'Belum Lunas') AS PAY_STAT
													FROM tbl_dp_header A
													INNER JOIN tbl_supplier B ON B.SPLCODE = A.SPLCODE
													WHERE A.DP_STAT NOT IN (5,9)
													$ADDQRYPRJ1 $ADDQRYSPL1 $ADDQRYREPDP_STAT $addQSTAT_DP
													...
													A.proj_Code AS PRJCODE, A.SPLCODE, B.SPLDESC, A.STATDESC,  IF(ROUND(A.Journal_AmountReal) = ROUND(A.GJournal_Total), 'Lunas', 'Belum Lunas') AS PAY_STAT
													FROM tbl_journalheader_vcash A
													INNER JOIN tbl_supplier B ON B.SPLCODE = A.SPLCODE
													WHERE A.GEJ_STAT NOT IN (5,9)
													$ADDQRYPRJ3 $ADDQRYSPL1 $ADDQRYREPVCASH_STAT $addQSTAT_VCASH
													...
													A.proj_Code AS PRJCODE, A.SPLCODE, B.SPLDESC, A.STATDESC,  IF(ROUND(A.Journal_AmountTsf) = ROUND(A.GJournal_Total), 'Lunas', 'Belum Lunas') AS PAY_STAT
													FROM tbl_journalheader_pd A
													INNER JOIN tbl_supplier B ON B.SPLCODE = A.SPLCODE
													WHERE A.GEJ_STAT NOT IN (5,9)
													$ADDQRYPRJ3 $ADDQRYSPL1 $ADDQRYREPPD_STAT $addQSTAT_PPD
													AND A.JournalH_Date BETWEEN '$Start_Date' AND '$End_Date'";
													/* ------------------- Hold --------------------------------
													UNION
								...
								if($res_HUTSPL->num_rows() > 0)
								{
									...
									foreach($res_HUTSPL->result() as $rHUTSPL):
										...
										$STATDESC 		= $rHUTSPL->STATDESC;
										$PAY_STAT 		= $rHUTSPL->PAY_STAT;
										...
										if($REM_INVAMOUNT != 0)
										{
											...
											?>
												<tr>
													...
													<td style="text-align: center;"><?=$STATDESC?></td>
													<td style="text-align: center;"><?=$PAY_STAT?></td>
