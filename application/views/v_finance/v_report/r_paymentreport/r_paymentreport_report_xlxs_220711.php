...
if($viewType == 1)
{
    ...
    header("Content-Disposition: attachment; filename=LAP. Pembayaran_$dtNow.xls");
        ...
        <div class="page">
            ...
            <table width="100%" border="1" rules="all" style="overflow-x:scroll">
                ...
                $sqlq3          = "SELECT DISTINCT A.JournalH_Code AS GEJ_NUM, A.CB_CODE, A.PRJCODE, A.CB_DATE, A.CB_TYPE,
                                    ...
                                    WHERE 
                                        A.PRJCODE IN ($COLREFPRJ)
                                        AND A.CB_STAT = 3 AND A.CB_DATE >= '$Start_Date' AND A.CB_DATE <= '$End_Date'
                                    UNION ALL
                                    SELECT DISTINCT B.JournalH_Code AS GEJ_NUM, B.Manual_No AS CB_CODE, B.proj_Code AS PRJCODE,
                                        ...
                                        B.acc_number AS CB_ACCID, PERSL_EMPID AS CB_PAYFOR, B.Journal_Amount AS CB_TOTAM, 0 AS CB_TOTAM_PPn, 
                                        B.JournalH_Desc AS CB_NOTES
                                    FROM
                                        tbl_journalheader B
                                    WHERE
                                        B.proj_Code IN ($COLREFPRJ) AND JournalType IN ('CPRJ','VCPRJ','CHO','VCHO')
                ...
                foreach($resq3 as $rowsqlq3) :
                    ...
                    ?>
                        <tr>
                            ...
                            <td nowrap style="text-align:right;"><?php echo $CB_TOTAM; ?></td>
                        </tr>
                    <?php
                endforeach;
                ?>
                <tr>
                    <td colspan="6" nowrap style="text-align:center; font-weight:bold;">T O T A L</td>
                  <td nowrap style="text-align:right; font-weight:bold"><?php echo $GTOTAL; ?></td>
                </tr>
            </table>
        </div>
    </body>
