						...
						<tbody>
							<?php
								// get Akun BANK
									...
									if($res_AccB->num_rows() > 0)
									{
										...
										foreach($res_AccB->result() as $rAccB):
											...
											$TOB_BANK 	= $TOB_BANK + $OB_BANK;
										endforeach;
										$arrAcc_BANK 	= implode("','", $Acc_BANK);

										// get PRJ KTR Pusat
											$get_HO = "SELECT PRJCODE FROM tbl_project WHERE isHO = 1 AND PRJSTAT = 1";
											$res_HO = $this->db->query($get_HO);
											if($res_HO->num_rows() > 0)
											{
												foreach($res_HO->result() as $rHO):
													$HO 	= $rHO->PRJCODE;
												endforeach;

												// get PRJ_HO
													$get_PRJHO = "SELECT PRJCODE FROM tbl_project_budg WHERE PRJCODE_HO = '$HO' AND PRJSTAT = 1";
													$res_PRJHO = $this->db->query($get_PRJHO);
													if($res_PRJHO->num_rows() > 0)
													{
														foreach($res_PRJHO->result() as $rPRJHO):
															$PRJCODE_HO = $rPRJHO->PRJCODE;
														endforeach;
													}
											}

										// Saat Ini: get transfer dari pusat => PRJ_HO
											$getTF_HO 	= "SELECT SUM(JournalD_Debet) AS TOT_TF 
															FROM tbl_journaldetail 
															WHERE JournalType = 'PINBUK' AND proj_Code = '$PRJCODE_HO' AND Acc_Id IN ('$arrAcc_BANK')
															AND JournalH_Date BETWEEN '$Start_Date' AND '$End_Date' AND GEJ_STAT = 3";
											$resTF_HO 	= $this->db->query($getTF_HO);
											if($resTF_HO->num_rows() > 0)
											{
												foreach($resTF_HO->result() as $rTFHO):
													$TOT_TF 	= $rTFHO->TOT_TF;
												endforeach;
											}

										// Before: get transfer dari pusat => PRJ_HO 
											$getTF_HO_BF 	= "SELECT SUM(JournalD_Debet) AS TOT_TF_BF 
															FROM tbl_journaldetail 
															WHERE JournalType = 'PINBUK' AND proj_Code = '$PRJCODE_HO' AND Acc_Id IN ('$arrAcc_BANK')
															AND JournalH_Date < '$Start_Date' AND GEJ_STAT = 3";
											$resTF_HO_BF 	= $this->db->query($getTF_HO_BF);
											if($resTF_HO_BF->num_rows() > 0)
											{
												foreach($resTF_HO_BF->result() as $rTFHO_BF):
													$TOT_TF_BF 	= $rTFHO_BF->TOT_TF_BF;
												endforeach;
											}

										// Komulatif: get transfer dari pusat => PRJ_HO
										...
										foreach($resREALZB->result() as $rRB):
											...
											// get transfer dari pusat => PRJ_HO
												$getPB_HO 	= "SELECT SUM(JournalD_Debet) AS TOT_TF 
																FROM tbl_journaldetail 
																WHERE JournalType = 'PINBUK' AND proj_Code = 'KTR-22' AND Acc_Id = '$Acc_BANK'
																AND JournalH_Date BETWEEN '$Start_Date' AND '$End_Date' AND GEJ_STAT = 3";
								...
								// KAS SAAT INI 
									...
									if($resREALZK->num_rows() > 0)
									{
										foreach($resREALZK->result() as $rRK):
											...
											// pengambilan utk KAS
												$getPB_PRYK 	= "SELECT SUM(JournalD_Debet) AS TOT_GETTFK 
																	FROM tbl_journaldetail 
										...
										foreach($resREALZB_BF->result() as $rRKB_BF):
											...
											// get opening balance BANK
												$getOBBANK_BF 	= "SELECT Base_OpeningBalance, Account_Number AS Acc_BANK FROM tbl_chartaccount 
																WHERE PRJCODE = '$PRJCODE' AND Account_Class t = 4 AND Account_Number = '$Acc_Id_BF'";
								...
								// SALDO AKHIR BANK KOMULATIF
									$SALDO_BANK_KOM 	= $OBalance_BANK_KOM + $TOT_BR_KOM - $TOT_BEXPD_KOM;
								...
								// SALDO AKHIR KAS KOMULATIF
									$SALDO_KAS_KOM 	= $OBalance_KAS_KOM + $TOT_KR_KOM - $TOT_KEXPD_KOM;
