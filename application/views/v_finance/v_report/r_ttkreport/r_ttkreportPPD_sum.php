			...
			$QVOC 		= "SELECT * FROM
							(
								SELECT A.JournalH_Code AS INV_NUM, A.Manual_No AS INV_CODE, '' AS TTK_NUM, '' AS TTK_CODE,
									...
									A.PERSL_EMPID, A.SPLCODE, A.Emp_ID AS EmpID, 'VCASH' AS JournalType
								FROM
								...
								SELECT A.JournalH_Code AS INV_NUM, A.Manual_No AS INV_CODE, '' AS TTK_NUM, '' AS TTK_CODE,
									A.JournalH_Date AS INV_DATE, A.proj_Code AS PRJCODE, A.JournalH_Desc AS INV_NOTES,
									A.GJournal_Total AS INV_AMOUNT_TOT, A.PPNH_Amount AS INV_PPN, A.PPHH_Amount AS INV_PPH,
									A.PERSL_EMPID, A.SPLCODE, A.Emp_ID AS EmpID, 'PD' AS JournalType
								...
								SELECT A.JournalH_Code AS INV_NUM, A.Manual_No AS INV_CODE, '' AS TTK_NUM, '' AS TTK_CODE,
									...
									'' AS PERSL_EMPID, '' AS SPLCODE, '' AS EmpID, 'PPD' AS JournalType
								...
								WHERE
									...
									$addQ_VOC
									AND A.JournalType = 'CHO-PD' 
									AND A.GEJ_STAT_PD = 3 AND A.JOBCODEID != ''
								UNION
									...
									'' AS PERSL_EMPID, A.SPLCODE AS SPLCODE, A.CREATER AS EmpID, 'PINV' AS JournalType
								FROM
									tbl_pinv_header A
										...
										<?php
											...
											if($query->num_rows() > 0)
											{
												...
												foreach($query->result() as $r):
													...
													if($JournalType == 'PPD')
													{
														// get SPLCODE / PERSL_EMPID & EmpID IN journalheader_pd
															$get_PPDH 	= "SELECT Emp_ID, PERSL_EMPID, SPLCODE 
																			FROM tbl_journalheader_pd
																			WHERE JournalH_Code = '$INV_NUM' AND proj_Code = '$PRJCODE'";
															$res_PPDH 	= $this->db->query($get_PPDH);
															if($res_PPDH->num_rows() > 0)
															{
																foreach($res_PPDH->result() as $rPPDH):
																	$Emp_ID 		= $rPPDH->Emp_ID;
																	$PERSL_EMPID 	= $rPPDH->PERSL_EMPID;
																	$SPLCODE 		= $rPPDH->SPLCODE;
																endforeach;
															}
													}
