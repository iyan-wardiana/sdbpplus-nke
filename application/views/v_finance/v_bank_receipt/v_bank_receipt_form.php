...
if($task == 'add')
{
	...
	$TRXTIME		= date('ymdHis');
	$BR_NUM			= "$PATTCODE$PRJCODE.$TRXTIME";
	$DocNumber 		= "";
	$BR_CODE		= $DocNumber;
...
else
{
	...
	$JournalY 		= date('Y', strtotime($default['BR_DATE']));
	$JournalM 		= date('n', strtotime($default['BR_DATE']));
...
if(isset($_POST['submit1']))
{
	...
	$MONTH 		= date('m');
	$DATE 		= date('d');
	$s_01 		= "tbl_br_header WHERE Acc_ID = '$selAccount' AND PRJCODE = '$PRJCODE' AND BR_DATE = '$CB_DATE'";
	$r_01 		= $this->db->count_all($s_01);
	$myMax 		= $r_01+1;

	$BANK_PATT 	= "";
	$s_02 		= "SELECT LEFT(Account_NameId,3) AS BANK_PATT FROM tbl_chartaccount_$PRJCODEVW WHERE Account_Number = '$selAccount' AND PRJCODE = '$PRJCODE'";
	...
	$lastPatt 	= $nol.$myMax;
	$BR_CODE	= "$BANK_PATT$YEAR$MONTH$DATE$lastPatt";
}
else
{
	$SelCurr 		= 'IDR';
...
<html>
	...
	<?php
		...
		if($LangID = 'IND')
		{
			...
			$alert4		= "Tentukan posisi Debet atau Kredit pada akun ";
		}
		else
		{
			...
			$alert4		= "Determine the Debit or Credit position on the account ";
		}
    ...
    <body class="<?php echo $appBody; ?>" style="background-color: <?=$comp_color?>">
		<section class="content-header">
			<h1>
			    <img src="<?php echo base_url() . 'assets/AdminLTE-2.0.5/dist/img/icon/bank_receipt.png'; ?>" style="max-width:40px; max-height:40px" >&nbsp;&nbsp;<?php echo "$BankReceipt ($PRJCODE)"; ?>
		...
		<section class="content">
		    <div class="row">
		        <form class="form-horizontal" name="frm1" method="post" action="" style="display: none;">
		        	...
		        	<input type="text" name="BR_CODEX" id="BR_CODEX" value="<?php echo $BR_CODE; ?>" />
							...
							<div class="box-body">
                                ...
                                <div class="form-group"> <!-- CB DOCUMENT -->
		                          	...
		                          	<div class="col-sm-9">
			                            ...
			                            <input type="hidden" name="BR_NUM" id="BR_NUM" value="<?php echo $BR_NUM; ?>">
		                          	</div>
                                </div>
		                          	...
		                          	<div class="col-sm-9">
	                                    <div class="input-group date">
			                                ...
			                                <?php
												...
												{
													?>
			                            			<input type="text" name="BR_DATE" class="form-control pull-left" id="datepicker" value="<?php echo $BR_DATE; ?>" style="width:105px" onChange="addUCODE()">
			                                        <?php
												}
												else
												{
		                    	...
		                    	<?php
									...
									elseif($BR_RECTYPE == 'PPD')
									{
										$countOWN	= "tbl_journalheader_pd A
															INNER JOIN tbl_employee B ON A.PERSL_EMPID = B.Emp_ID WHERE A.proj_Code = '$PRJCODE'
										...
										if($resCOWN > 0)
										{
											/*$sqlOWN		= "SELECT DISTINCT B.Emp_ID AS own_Code, '' AS own_Title,
															...
															INNER JOIN tbl_employee B ON A.PERSL_EMPID = B.Emp_ID WHERE A.GEJ_STAT = '3' AND A.GEJ_STAT_PD = '3' AND A.GEJ_STAT_PPD = '1'
											...
											if($task == 'edit')
											{
												/*$sqlOWN	= "SELECT DISTINCT B.Emp_ID AS own_Code, '' AS own_Title,
																CONCAT(B.First_Name, ' ', B.Last_Name) AS own_Name
															FROM tbl_journalheader_pd A
															INNER JOIN tbl_employee B ON A.PERSL_EMPID = B.Emp_ID WHERE A.GEJ_STAT IN ('3','6') AND A.GEJ_STAT_PD IN ('3','6') AND A.GEJ_STAT_PPD = '1'
																AND A.proj_Code = '$PRJCODE'";*/
												$sqlOWN	= "SELECT DISTINCT B.Emp_ID AS own_Code, '' AS own_Title,
																CONCAT(B.First_Name, ' ', B.Last_Name) AS own_Name
															FROM tbl_journalheader_pd A
                        ...
                        <div class="box box-warning">
                            ...
                            <div class="box-body">
		                        ...
		                        <?php
	                                ...
	                                $s_DETACC	= "SELECT SUM(A.Base_Kredit) AS TOT_ACC FROM tbl_journaldetail_br A
													WHERE A.GEJ_STAT IN (2)";
	                                $r_DETACC 	= $this->db->query($s_DETACC)->result();
                                    foreach($r_DETACC as $row) :
                                        $TOT_ACC 	= $row->TOT_ACC;
									
										$BR_TOTAM	= $BR_TOTAM + $TOT_ACC;
									endforeach;
			                    ...
			                    <?php
									$collID		= "$BR_RECTYPE~$BR_PAYFROM~$PRJCODE";
                                ...
                                <div class="form-group"> <!-- CHOOSE DOC. -->
		                          	...
		                          	<div class="col-sm-9">
	                                    ...
	                                    <button class="btn btn-warning" type="button" onClick="selectitem();"<?php if($BR_RECTYPE == 'OTH') { ?> disabled <?php } ?>>
	                        ...
	                        <div class="search-table-outter">
	                            <table id="tbl" class="table table-bordered table-striped" width="100%">
		                            <tr style="background:#CCCCCC">
		                              	...
		                              	<th width="33%"A.PINV_TOTVALPPh style="text-align:center; vertical-align: middle;"><?php echo $Description; ?> </th>
		                            ...
		                            if($task == 'edit')
		                            {
		                                ...
		                                if($resultCount > 0)
		                                {
		                                    foreach($result as $row) :
		                                        ...
		                                        $BR_TOTAM 		= $BR_TOTAM + $Inv_Amount;
		                                        $BR_TOTAM_PPn	= $BR_TOTAM_PPn + $Inv_Amount_PPn;
		                                        ...
		                                        <td style="text-align:right; vertical-align: middle;" nowrap>
		                                        	<?php
		                                        		echo number_format($GInv_Amount, $decFormat);
		                                        	?>
	                        ...
	                        <div class="box-body">
					            <div class="search-table-outter">
		                        	...
		                        	<tr style="background:#CCCCCC">
		                              	...
		                              	<th width="13%" style="text-align:center"><?php echo $AccountNo; ?> </th>
		                              	...
		                              	<th width="25%" style="text-align:center"><?php echo $Remarks; ?></th>
		                          	</tr>
		                            <?php
		                            $accRow 	= 0;
		                            $editable	= 0;
		            ...
		            <script type="text/javascript">
						function addAcc()
						{
							...
							// Account Position
								...
								objTD.innerHTML = '<select name="dataACC['+intIndex+'][Journal_DK]" id="dataACC'+intIndex+'Journal_DK" class="form-control select2" style="width:100%" onChange="countAmount()"><option value="">--</option><option value="D" selected>D</option><option value="K">K</option></select>';
							
							// Account Amount
							// // HASIL DISKUSI DGN BU LYTA 08 JUNI 2022 BHW SEMUA PENAMBAHAN DETIL AKUN BERSIFAT MENGURANGI NILAI KAS/BANK, ARTINYA TERSIMPAN DI SISI DEBET
								objTD = objTR.insertCell(objTR.cells.length);
						...
						function chgAmount(thisval, row)
						{
							...
							// TOTAL VOUCHER
								...
								for(i=1;i<=totRow;i++)
								{
									var Amount			= document.getElementById('data'+i+'GAmount').value;
									var Amount_PPn		= document.getElementById('data'+i+'Amount_PPn').value;
									var TOT_AMOUNT		= parseFloat(TOT_AMOUNT) + parseFloat(Amount);
									var TOT_AMOUNT_PPn	= parseFloat(TOT_AMOUNT_PPn) + parseFloat(Amount_PPn);
								}

							GTOT_AMOUNT 				= parseFloat(totAmountK) + parseFloat(TOT_AMOUNT);
							GTOT_INVREM 				= document.getElementById('totREMAMN').value;
							if((GTOT_AMOUNT > GTOT_INVREM) && BR_RECTYPE != 'OTH')
							{
								swal('Total nilai yang dimasukan melebihi nilai sisa piutang.',
								{
									icon:"warning"
								})
								.then(function()
								{
									swal.close();
									eval(thisval).value 	= "0";
									eval(thisval).focus();
								});
								return false;
							}

							console.log('GTOT_AMOUNT = '+GTOT_AMOUNT)
							document.getElementById('BR_TOTAM').value		= parseFloat(GTOT_AMOUNT);
...
<script>
	...
	{
		var task 		= "<?=$task?>";
		var DOCNUM		= document.getElementById('BR_NUM').value;
		var DOCCODE		= document.getElementById('BR_CODE').value;
		var DOCDATE		= document.getElementById('datepicker').value;
		var ACC_ID		= document.getElementById('selAccount').value;
		var PDManNo 	= "";
	...
	function add_item(strItem) 
	{
		...
		PINV_AMOUNT 		= parseFloat(arrItem[2]);
		PINV_AMOUNT_PPn		= parseFloat(arrItem[3]);
		PINV_AMOUNT_PPh		= parseFloat(arrItem[4]);
		PINV_AMOUNT_OTH		= parseFloat(arrItem[5]);
		PINV_NOTES 			= arrItem[6];
		PINV_Number 		= arrItem[7];
		PINV_MANNO 			= arrItem[8];
		GTOT_INVOICE		= parseFloat(arrItem[9]);
		GTOT_INVREM			= parseFloat(arrItem[10]);  // NILAI SISA
		GTOT_PAIDAMN		= parseFloat(arrItem[11]);  // DITERIMA

		//totREMAMN 			= parseFloat(GTOT_INVREM);
		totREMAMN 			= parseFloat(GTOT_INVOICE-GTOT_PAIDAMN);
		...
		intIndex = parseInt(objTable.rows.length); // default = 1;
		//intIndex = intTable;
		document.frm.rowCount.value = intIndex;
		...
		objTD = objTR.insertCell(objTR.cells.length);
		objTD.style.textAlign = 'right';
		objTD.innerHTML = ''+GTOT_INVOICEV+'<input type="hidden" name="Inv_Amount'+intIndex+'" id="Inv_Amount'+intIndex+'" value="'+PINV_AMOUNT+'" class="form-control" style="min-width:120px; max-width:150px; text-align:right" ><input type="hidden" name="data['+intIndex+'][Inv_Amount]" id="data'+intIndex+'Inv_Amount" size="10" value="'+PINV_AMOUNT+'" class="form-control" style="max-width:300px;" onKeyPress="return isIntOnlyNew(event);" ><input type="hidden" name="data['+intIndex+'][GInv_Amount]" id="data'+intIndex+'GInv_Amount" size="10" value="'+GTOT_INVOICE+'" class="form-control" style="max-width:300px;" onKeyPress="return isIntOnlyNew(event);" ><input type="hidden" name="data['+intIndex+'][GInv_Remain]" id="data'+intIndex+'GInv_Remain" size="10" value="'+totREMAMN+'" class="form-control" style="max-width:300px;" onKeyPress="return isIntOnlyNew(event);" >';
		
		// Invoice Realized Amount
		GTOT_PAIDAMNV	= doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(GTOT_PAIDAMN)),decFormat));
		objTD = objTR.insertCell(objTR.cells.length);
		objTD.style.textAlign = 'right';
		objTD.innerHTML = ''+GTOT_PAIDAMNV+'<input type="hidden" name="GTOT_PAIDAMN'+intIndex+'" id="GTOT_PAIDAMN'+intIndex+'" value="'+GTOT_PAIDAMN+'" class="form-control" style="min-width:120px; max-width:150px; text-align:right" >';

		// Receipt - Amount
		totREMAMNV	= doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(totREMAMN)),decFormat));
		objTD = objTR.insertCell(objTR.cells.length);
		objTD.style.textAlign = 'right';
		objTD.innerHTML = '<input type="text" name="GAmount'+intIndex+'" id="GAmount'+intIndex+'" value="'+totREMAMNV+'" class="form-control" style="min-width:150px; max-width:150px; text-align:right" onKeyPress="return isIntOnlyNew(event);" onBlur="chgRecAmount(this,'+intIndex+');" ><input type="hidden" name="data['+intIndex+'][Amount]" id="data'+intIndex+'Amount" size="10" value="'+totREMAMN+'" class="form-control" style="max-width:300px;" onKeyPress="return isIntOnlyNew(event);" ><input type="hidden" name="data['+intIndex+'][GAmount]" id="data'+intIndex+'GAmount" size="10" value="'+totREMAMN+'" class="form-control" style="max-width:300px;" onKeyPress="return isIntOnlyNew(event);" ><input type="hidden" name="Inv_Amount_PPn'+intIndex+'" id="Inv_Amount_PPn'+intIndex+'" value="'+PINV_AMOUNT_PPn+'" class="form-control" style="min-width:110px; max-width:130px; text-align:right" ><input type="hidden" name="data['+intIndex+'][Inv_Amount_PPn]" id="data'+intIndex+'Inv_Amount_PPn" size="10" value="'+PINV_AMOUNT_PPn+'" class="form-control" style="max-width:300px;" onKeyPress="return isIntOnlyNew(event);" ><input type="hidden" name="Amount_PPn'+intIndex+'" id="Amount_PPn'+intIndex+'" size="10" value="0.00" class="form-control" style="min-width:120px; max-width:150px; text-align:right" onKeyPress="return isIntOnlyNew(event);" onBlur="chgRecAmountPPn(this,'+intIndex+');" ><input type="hidden" name="data['+intIndex+'][Amount_PPn]" id="data'+intIndex+'Amount_PPn" size="10" value="0.00" class="form-control" style="max-width:300px;" onKeyPress="return isIntOnlyNew(event);" >';
		
		// Keterangan
		objTD = objTR.insertCell(objTR.cells.length);
		objTD.style.textAlign = 'center';
		objTD.innerHTML = '<input type="text" name="data['+intIndex+'][Notes]" id="data'+intIndex+'Notes" size="15" value="" class="form-control" ><input type="hidden" name="totalrow" id="totalrow" value="'+intIndex+'">';
		...
		document.getElementById('totREMAMN').value 				= totREMAMN;
	}
	...
	function chgRecAmount(thisVal1, row)
	{
		...
		for(i=1;i<=totRow;i++)
		{
			...
			var GAmount			= document.getElementById('data'+row+'GAmount').value;

			if(BR_RECTYPE == 'PPD') var TOT_AMOUNT	= parseFloat(TOT_AMOUNT) + parseFloat(GAmount);
			else var TOT_AMOUNT		= parseFloat(TOT_AMOUNT) + parseFloat(Amount);
	...
	function checkInp(value)
	{
		...
		for(i=1;i<=totrowAcc;i++)
		{
			var Amount 	= parseFloat(document.getElementById('JournalD_Amount'+i).value);
			var JRNDK 	= document.getElementById('dataACC'+i+'Journal_DK').value;
			var JRNACC	= document.getElementById('dataACC'+i+'Acc_Id').value;
			var JRNDESC	= document.getElementById('dataACC'+i+'JournalD_Desc').value;
			
			if(JRNDK == '')
			{
				swal('<?php echo $alert4; ?>'+JRNACC+' '+JRNDESC,
				{
					icon: "warning",
				});
				return false;
			...
			if(Amount == 0)
			{
				...
				{
					swal.close();
					document.getElementById('JournalD_Amount'+i).value = '0';
