						...
						<div class="box-body">
	                        <div class="col-sm-4">
                                ...
                                <select name="SPLCODE" id="SPLCODE" class="form-control select2" style="width: 100%" onChange="grpVC(this.value)">
                                    <option value=""> --- </option>
                                    <?php
                                        $s_SPL  = "SELECT DISTINCT B.SPLCODE, B.SPLDESC FROM tbl_journalheader_cprj A
													LEFT JOIN tbl_supplier B ON A.PERSL_EMPID = B.SPLCODE WHERE A.proj_Code = '$PRJCODE'
													UNION
													SELECT DISTINCT B.Emp_ID AS SPLCODE, CONCAT(B.First_Name,' ', B.Last_Name) AS SPLDESC FROM tbl_journalheader_cprj A
													LEFT JOIN tbl_employee B ON A.PERSL_EMPID = B.Emp_ID WHERE A.proj_Code = '$PRJCODE' ORDER BY SPLDESC ASC";
                                        $r_SPL  = $this->db->query($s_SPL)->result();
                                        foreach($r_SPL as $rw_SPL) :
                                            $SPLCODE    = $rw_SPL->SPLCODE;
                                            $SPLDESC    = $rw_SPL->SPLDESC;
                                            ?>
                                            <option value="<?php echo $SPLCODE; ?>"><?php echo "$SPLDESC - $SPLCODE"; ?></option>
                                            <?php
                                        endforeach;
                                    ?>
                                </select>
		    ...
		    <script type="text/javascript">
		    	function grpVC()
		    	{
		    		...
		    		$('#example').DataTable({
						...
						"autoWidth": true,
						"filter": true,
				        "ajax": "<?php echo site_url('c_finance/c_cho70d18/get_AllDataCHOGRP/?id='.$PRJCODE.'&jrnType='.$jrnType)?>"+'&SPLC='+SPLC+'&GSTAT='+GSTAT,
				        "type": "POST",
...
<script>
    ...
    $('#example').DataTable( {
		...
		"autoWidth": true,
		"filter": true,
        "ajax": "<?php echo site_url('c_finance/c_cho70d18/get_AllDataCHO/?id='.$PRJCODE.'&jrnType='.$jrnType)?>",
        "type": "POST",
		"lengthMenu": [[10, 25, 50, 100, 200, -1], [10, 25, 50, 100, 200, "All"]],
		// "lengthMenu": [[10, 25, 50, 100, 200], [10, 25, 50, 100, 200]],
		"columnDefs": [	{ targets: [0,2,5,6], className: 'dt-body-center' },
