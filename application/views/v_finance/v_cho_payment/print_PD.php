...
$moneyFormat = new moneyFormat();
    ...
    if($SPLCODE != '')
    {
        ...
        endforeach;
        $PAYTO      = "$SPLCODE - $SPLDESC / $PERSL_EMPID - $EMPNAME";
    }
    else
    {
        $EMP_NAME       = "";
...
<head>
    ...
    <?php
        ...
        if($REF_CODE != '')
        {
            $sqlOP      = "SELECT A.SPLCODE, B.SPLDESC FROM tbl_po_header A
                            INNER JOIN tbl_supplier B ON A.SPLCODE = B.SPLCODE
                            WHERE A.PO_CODE = '$REF_CODE'
                            UNION
                            SELECT A.SPLCODE, B.SPLDESC FROM tbl_wo_header A
                            INNER JOIN tbl_supplier B ON A.SPLCODE = B.SPLCODE
                            WHERE A.WO_CODE = '$REF_CODE'";
            $resOP     = $this->db->query($sqlOP)->result();
            foreach($resOP as $rowOP) :
                $SPLCODE    = $rowOP->SPLCODE;
                $SPLDESC    = " / $SPLCODE ".$rowOP->SPLDESC;
            endforeach;
        }
    ?>
</head>
        ...
        <div class="cont">
            <div class="box-header">
                ...
                <div class="box-column-title">
                    ...
                    <span style="color: red !important;">PEMBAYARAN DIMUKA (PD)</span>
                </div>
            </div>
            <div class="box-header-detail">
                <table width="100%" border="0" cellpadding="0" cellspacing="0">
                    <tr>
                        ...
                        <td width="1%">:</td>
                        <td width="10%">&nbsp;<?php echo $PRJCODE; ?></td>
                    </tr>
                    <tr>
                        <td>DATE</td>
                        <td>:</td>
                        <td><?php echo date('d-m-Y', strtotime($PD_Date)); ?></td>
                        <td>TTK NO.</td>
                        <td>:</td>
