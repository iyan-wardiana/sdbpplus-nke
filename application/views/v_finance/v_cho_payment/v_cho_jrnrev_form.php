...
/* 
	...
	* Create Date	= 26 Maret 2022
	* File Name		= v_cho_jrnrev_form.php
	* Location		= -
*/
...
if($task == 'add')
{
	// START : GENERATE DOCUMENT CODE
		...
		$parDocNo 	= array('MenuCode' 		=> $MenuCode,
							'JournalType' 	=> $jrnType,
							'tblName' 		=> 'tbl_journalheader_revers',
...
if(isset($_POST['JRN_CODE']))
{
	...
	$sqlJRN			= "SELECT JournalH_Code, JournalType, JournalH_Date, PERSL_EMPID, Journal_Amount, PPNH_Amount, PPHH_Amount, GJournal_Total, Manual_No
						FROM tbl_journalheader_revers WHERE Reference_Number = '$JRN_CODE' AND proj_Code = '$PRJCODE'";
                        ...
                        <div class="box box-success">
                            ...
                            <div class="box-body">
				            	...
				            	<div class="form-group">
				                    ...
				                    <div class="col-sm-6">
				                    	<label for="inputName">No. Voucher</label>
				                    	<div class="input-group">
		                                    <div class="input-group-btn">
												<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#mdl_addTTK"><i class="glyphicon glyphicon-search"></i></button>
		                                    </div>
		                                    ...
		                                    <input type="text" class="form-control" name="Reference_NumberX" id="Reference_NumberX" value="<?php echo $REF_CODE; ?>" data-toggle="modal" data-target="#mdl_addPR">
		                                    <div class="input-group-btn">
												<button type="button" class="btn btn-warning" onClick="procDet()"><i class="glyphicon glyphicon-check"></i></button>
		                                    </div>
		                                </div>
				                    </div>
				                </div>
						      	...
						      	<table id="tblDetil" class="table table-bordered table-striped" width="100%">
									<thead>
							            <tr>
			                              	...
			                              	<th width="21%" style="text-align:center; vertical-align: middle;"><?php echo "Item"; ?> </th>
		                            ...
		                            {
		                                $sqlDET	= "SELECT A.*
													FROM tbl_journaldetail_revers A
													WHERE JournalH_Code = '$JournalH_Code' 
														AND A.proj_Code = '$PRJCODE' AND A.Journal_DK = 'K' ";
                    ...
                    <script type="text/javascript">
                    	function procDet()
                    	{
                    		...
                    		/*$('#tblDetil').DataTable(
					    	{
								...
								"autoWidth": true,
								"filter": true,
						        "ajax": "<?php echo site_url('c_finance/c_cho70d18/get_AllDataVCDET/?id='.$PRJCODE)?>",
						        "type": "POST",
							...
							$('#tblDetil').DataTable(
					    	{
								...
								"filter": true,
						        "ajax": {
							        "url": "<?php echo site_url('c_finance/c_cho70d18/get_AllDataVCDET/?id='.$PRJCODE.'&refNum=')?>"+refNum,
							        "type": "POST",
							        "complete": function()
							        {
							        	countDis();
		                        ...
		                        <?php
									/*if($ISAPPROVE == 1)
										...
										if($task == 'add')
										{
											if($disabled == 0):
											?>
												<button type="button" class="btn btn-warning" id="btnSave" style="display: none;" onClick="ReversalJrn()">
							    	...
							    	<div class="col-md-12">
						              	<ul class="nav nav-tabs">
						                    <li id="li1" <?php echo $Active1Cls; ?>>
						                    	<a href="#itm1" data-toggle="tab"><?php echo $VCList; ?></a>
						                    </li>
						                </ul>
				...
				<script type="text/javascript">
					$(document).ready(function()
					{
				    	$('#example1').DataTable(
				    	{
							...
							"autoWidth": true,
							"filter": true,
					        "ajax": "<?php echo site_url('c_finance/c_cho70d18/get_AllDataVC/?id='.$PRJCODE)?>",
					        "type": "POST",
							//"lengthMenu": [[10, 25, 50, 100, 200, -1], [10, 25, 50, 100, 200, "All"]],
							"lengthMenu": [[10, 25, 50, 100, 200], [10, 25, 50, 100, 200]],
							"columnDefs": [	{ targets: [0], className: 'dt-body-center' },
...
<script>
	...
	// START : LOCK PROCEDURE
		...
		{
			var url         = "<?php echo site_url('lck/appStat')?>";
			let DOC_DATE 	= $('#JournalH_Date').val();
		...
		if($task == 'add')
		{
			?>
				...
				function countTask()
				{
					...
					$.ajax({
			            type: 'POST',
			            url: "<?php echo site_url('__l1y/getLastDocNumR')?>",
			            data: formData,
			            success: function(response)
			            {
			            	var arrVar 	= response.split('~');
