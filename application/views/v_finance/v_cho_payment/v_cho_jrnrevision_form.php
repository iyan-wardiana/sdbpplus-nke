...
/* 
	...
	* File Name		= v_cho_jrnrevision_form.php
	* Location		= -
*/
...
if($task == 'add')
{
	// START : GENERATE DOCUMENT CODE
		...
		$parDocNo 	= array('MenuCode' 		=> $MenuCode,
							...
							'attDate' 		=> 'JournalH_Date',
							'attPRJCODE'	=> 'proj_Code',
							'PRJCODE' 		=> $PRJCODE,
							'CREATER'		=> $DefEmp_ID);
...
else
{
	...
	$ACCID_AP			= $acc_number;
	$s_01 				= "SELECT VC_LA_PAYINV FROM tbl_vendcat WHERE VendCat_Code = (SELECT SPLCAT FROM tbl_supplier WHERE SPLCODE = '$PERSL_EMPID')";
...
if(isset($_POST['JRN_CODE']))
{
	$JRN_CODE		= $_POST['JRN_CODE'];
	$sqlJRN			= "SELECT JournalH_Code, JournalType, JournalH_Date, PERSL_EMPID, Journal_Amount, PPNH_Amount, PPHH_Amount, GJournal_Total, Manual_No
						FROM tbl_journalheader_revision WHERE Reference_Number = '$JRN_CODE' AND proj_Code = '$PRJCODE'";
	$resJRN 		= $this->db->query($sqlJRN)->result();
	foreach($resJRN as $rowJOB) :
		$JournalH_Code 		= $rowJOB->JournalH_Code;
		$JournalType 		= $rowJOB->JournalType;
		$JournalH_Date 		= $rowJOB->JournalH_Date;
		$PERSL_EMPID 		= $rowJOB->PERSL_EMPID;
		$Journal_Amount 	= $rowJOB->Journal_Amount;
		$PPNH_Amount 		= $rowJOB->PPNH_Amount;
		$PPHH_Amount 		= $rowJOB->PPHH_Amount;
		$GJournal_Total 	= $rowJOB->GJournal_Total;
		$Manual_No 			= $rowJOB->Manual_No;
		$Reference_Number 	= $rowJOB->Reference_Number;
		$REF_CODE 			= $Manual_No;
	endforeach;
}
?>
...
<html>
	...
	<?php
		...
		foreach($resTransl as $rowTransl) :
			...
			if($TranslCode == 'jrnList')$jrnList = $LangTransl;
		...
		if($LangID == 'IND')
		{
			...
			$alert17	= "Anda yakin akan membuat jurnal pembalik untuk jurnal ini?";
		}
		else
		{
			$subTitleH	= "Add";
			...
			$alert17	= "Are you sure want to reverse this journal?";
		}
    ...
    <body class="<?php echo $appBody; ?>" style="background-color: <?=$comp_color?>" onload="countTask();">
		...
		<section class="content">
	        <form class="form-horizontal" name="frm" method="post" action="">
	            <input type="hidden" name="ISDISABLED" id="ISDISABLED" value="0">
                ...
                <div class="row">
                    ...
                    <div class="col-md-6">
                        <div class="box box-success">
                            ...
                            <div class="box-body">
				            	...
				            	<div class="form-group">
				                    <div class="col-sm-6">
				                    	<label for="inputName"><?php echo "$Code"; ?></label>
				                    	...
				                    	<?php if($task == 'add') { ?>
					                    	<input type="hidden" class="form-control" style="max-width:400px;text-align:right" name="Pattern_Code" id="Pattern_Code" size="30" value="<?php echo $Patt_Code; ?>" />
					                    <?php } ?>
				                    ...
				                    <div class="col-sm-6">
				                    	<label for="inputName">No. Journal</label>
				                    	<div class="input-group">
		                                    <div class="input-group-btn">
												<button type="button" class="btn btn-primary" data-toggle="modal" data-target="#mdl_addJRN" <?=$disSel?>><i class="glyphicon glyphicon-search"></i></button>
		                                    </div>
		                                    <input type="hidden" class="form-control" name="Reference_Number" id="Reference_Number" style="max-width:160px" value="<?php echo $Reference_Number; ?>" >
		                                    <input type="text" class="form-control" name="Reference_NumberX" id="Reference_NumberX" value="<?php echo $REF_CODE; ?>" data-toggle="modal" data-target="#mdl_addPR" <?=$disSel?>>
		                                    <div class="input-group-btn">
												<button type="button" class="btn btn-warning" onClick="procDet()" <?=$disSel?>><i class="glyphicon glyphicon-check"></i></button>
		                                    </div>
		                                </div>
				                    </div>
				                </div>
				            	<div class="form-group">
				                    <div class="col-sm-5">
				                    	<label for="inputName"><?php echo $Date; ?></label>
				                    	<div class="input-group date">
				                        	<div class="input-group-addon"><i class="fa fa-calendar"></i>&nbsp;</div>
				                        	<input type="text" name="JournalH_Date" class="form-control pull-left" id="JournalH_Date" value="<?php echo $JournalH_Date; ?>" readonly>
				                       	</div>
				                       	<input type="hidden" name="ISPERSL" id="ISPERSL" value="<?php echo $ISPERSL; ?>" />
				                    </div>
				                    <div class="col-sm-7">
	                                	<label for="inputName"><?php echo $Supplier; ?></label>
				                    	<input type="hidden" class="form-control" id="PERSL_EMPID" name="PERSL_EMPID" value="<?php echo $PERSL_EMPID; ?>" />
				                    	...
				                    	<input type="text" name="PERSL_EMPIDX" class="form-control" id="PERSL_EMPIDX" value="<?php echo $SPLDESC; ?>" readonly>
		                            </div>
				                </div>
				            	<div class="form-group" style="display: none;">
                    ...
                    <div class="col-md-12" id="choDet">
                        <div class="box box-primary">
							<div class="search-table-outter">
						      	<table id="tblDetil" class="table table-bordered table-striped" width="100%">
									<thead>
							            <tr>
			                              	<th width="2%" height="25" style="text-align:left">&nbsp;</th>
			                              	<th width="21%" style="text-align:center; vertical-align: middle;"><?php echo $Description; ?> </th>
			                              	...
			                              	<th width="5%" style="text-align:center; vertical-align: middle;">PPn</th>
			                              	<th width="5%" style="text-align:center; vertical-align: middle;">PPh</th>
		                            ...
		                            if($task == 'edit')
		                            {
		                                $sqlDET	= "SELECT * FROM tbl_journaldetail_revision WHERE Manual_No = '$Manual_No' AND proj_Code = '$PRJCODE'";
										...
										foreach($result as $row) :
		                                    ...
		                                    <tr id="tr_<?php echo $currentRow; ?>">
			                                    ...
			                                    <td style="text-align:right; vertical-align: middle;">
											  	...
											  	<td style="text-align:center; vertical-align: middle;">
										 			<?php echo $Ref_Number; ?>
										 			<input type="hidden" name="data[<?php echo $currentRow; ?>][Ref_Number]" id="data<?php echo $currentRow; ?>Ref_Number" value="<?php echo $Ref_Number; ?>" class="form-control" style="min-width:150px;max-width:300px;" placeholder="<?php echo $RefNumber; ?>" >
			                                    </td>
                    ...
                    <script type="text/javascript">
                    	function procDet()
                    	{
                    		var refNum 	= document.getElementById('Reference_Number').value;
                    		...
                    		/*$('#tblDetil').DataTable(
					    	{
								...
								"autoWidth": true,
								"filter": true,
						        "ajax": "<?php echo site_url('c_finance/c_cho70d18/get_AllDataJRNDET/?id='.$PRJCODE)?>",
						        "type": "POST",
							...
							$('#tblDetil').DataTable(
					    	{
								...
								"filter": true,
						        "ajax": {
							        "url": "<?php echo site_url('c_finance/c_cho70d18/get_AllDataJRNDET/?id='.$PRJCODE.'&refNum=')?>"+refNum,
							        "type": "POST",
							        "complete": function()
							        {
							        	countDis();
								...
								"columnDefs": [	{ targets: [0,2], className: 'dt-body-center' },
												{ targets: [3,4,5,6,7,8], className: 'dt-body-right' }
											  ],
	        					"order": [[ 2, "desc" ]],
								"language": {
						            "infoFiltered":"",
                    	...
                    	{
                    		var isDistot = 0;
				        	let totalrow = document.getElementsByName('totalrow').length;
				        	for(let i = 1; i <= totalrow; i++) {
					        	isDis = document.getElementById('ISDIS'+i).value;
					        	isDistot = parseFloat(isDistot)+parseFloat(isDis);
				        	}

				        	document.getElementById('ISDISABLED').value = isDistot;
				        	/*if(isDistot > 0)
							    	...
							    	<div class="col-md-12">
						              	<ul class="nav nav-tabs">
						                    <li id="li1" <?php echo $Active1Cls; ?>>
						                    	<a href="#itm1" data-toggle="tab"><?php echo $jrnList; ?></a>
						                    </li>
						                </ul>
			                                            ...
			                                            <table id="example1" class="table table-bordered table-striped table-responsive search-table inner" width="100%">
			                                                <thead>
			                                                    <tr>
											                        ...
											                        <th width="18%" style="text-align:center; vertical-align: middle;" nowrap>No. Jurnal</th>
											                        <th width="40%" style="text-align:center; vertical-align: middle;"><?php echo $Description; ?> </th>
											                        <th width="30%" style="text-align:center; vertical-align: middle;" nowrap><?=$Supplier?></th>
				...
				<script type="text/javascript">
					$(document).ready(function()
					{
				    	$('#example1').DataTable(
				    	{
							...
							"autoWidth": true,
							"filter": true,
					        "ajax": "<?php echo site_url('c_finance/c_cho70d18/get_AllDataJRN/?id='.$PRJCODE)?>",
					        "type": "POST",
							//"lengthMenu": [[10, 25, 50, 100, 200, -1], [10, 25, 50, 100, 200, "All"]],
							"lengthMenu": [[10, 25, 50, 100, 200], [10, 25, 50, 100, 200]],
							"columnDefs": [	{ targets: [0], className: 'dt-body-center' },
							                            ...
							                            <div class="box-body">
															...
															<?php
									                            ...
									                            $s_av	= "SELECT Acc_ID, Account_Number, Account_Level,
								                                            ...
								                                            FROM tbl_chartaccount WHERE Account_Category IN (5,6) AND PRJCODE = '$PRJCODE'
								                                            	ORDER BY ORD_ID";
									                            $r_av 	= $this->db->query($s_av)->result();

									                            $s_bc	= "tbl_chartaccount WHERE Account_Category IN (5,6) AND PRJCODE = '$PRJCODE'";
										                    ...
										                    <div class="col-sm-6">
										                    	...
										                    	<input type="text" class="form-control" name="jrnDesc" id="jrnDesc" value=""/>
										                    </div>
										                    <div class="col-sm-6">
										                    	<label for="inputName" class="control-label">Status Pergantian</label>
										                    	<select name="PROC_STAT" id="PROC_STAT" class="form-control select2" onChange="chgPSTAT(this.value)" style="width: 100%">
								                        			<option value="0" > --- </option>
								                        			<option value="1" > Hanya akun terpilih </option>
								                        			<option value="2" > Dalam dokumen terpilih </option>
				...
				<script type="text/javascript">
					...
					$(document).ready(function()
					{
					   	$("#btnDetail2").click(function()
					    {
							...
							var JRNNUM		= $("#Reference_Number").val();
							var JRNDESC		= encodeURIComponent($("#jrnDesc").val());
							var JRN_ID		= $("#JRN_ID").val();
							var dataString 	= 'PRJCODE='+PRJCODE+'&ITM_CODE='+ITM_CODE+'&ACC_ID='+ACC_ID+'&ACC_ID_UM='+ACC_ID_UM+'&PROC_STAT='+PROC_STAT+'&JRNNUM='+JRNNUM+'&JRNDESC='+JRNDESC+'&JRN_ID='+JRN_ID+'&JRN_CODE='+JRN_CODE;
							...
							if(PROC_STAT == 0)
							{
		                        swal('Silahkan pilih status pergantian akun', 
		                        {
		                            icon: "warning",
		                        });
		                        return false;
							...
							$.ajax({
			                    ...
			                    success: function(response)
			                    {
			                        ...
			                        $('#tblDetil').DataTable().ajax.reload();
			                    }
			                });
...
<script>
	...
	<?php
		...
		if($task == 'add')
		{
			?>
				...
				function countTask()
				{
					var refNumb 	= document.getElementById('Reference_NumberX').value;
					var formData 	= {
										MenuCode		: "<?=$MenuCode?>",
										JournalType		: "<?=$jrnType?>",
										refNumb			: refNumb,
					...
					$.ajax({
			            type: 'POST',
			            url: "<?php echo site_url('__l1y/getLastDocNumRevision')?>",
			            data: formData,
			            success: function(response)
			            {
			            	var arrVar 	= response.split('~');
			            	...
			            	$('#JournalH_Code1').val(docNum);
			            	$('#JournalH_Code').val(docNum);
			            	$('#Manual_No').val(docNum);
			            	$('#Manual_No1').val(docNum);
	...
	function createRev(row)
	{
		var ITMDESC	= document.getElementById('ITMDESC'+row).value;
		var MAN_NO 	= document.getElementById('Manual_No1').value;
	    swal({
            text: "Yakin akan membuat jurnal pembalik untuk "+ITMDESC,
            icon: "warning",
            buttons: ["No", "Yes"],
        })
        .then((willDelete) => 
        {
            if (willDelete) 
            {
                var collID1	= document.getElementById('urlRev'+row).value;
                var collID 	= collID1+'~'+MAN_NO;
  	...
  	function ReversalJrn()
	{
		var MAN_NO 	= document.getElementById('Manual_No1').value;
	    swal({
            text: "Yakin akan membuat jurnal pembalik untuk dokumen ini?",
            icon: "warning",
            buttons: ["No", "Yes"],
        })
        .then((willDelete) => 
        {
            if (willDelete) 
            {
				...
				var refCode 	= 	document.getElementById('Reference_NumberX').value;
				var refNumb 	= 	document.getElementById('Reference_Number').value;
				var refMNumb 	= 	document.getElementById('Manual_No1').value;
				var formData 	= 	{
										JRN_NUM		: refNumb,
										JRN_MNUM	: refMNumb,
										JRN_MREF	: refCode,
				...
				$.ajax({
		            type: 'POST',
		            url: "<?php echo site_url('c_finance/c_cho70d18/jrnREVJRN')?>",
		            data: formData,
		           	beforeSend: function(){
				     $("#loading").show();
				   	},
	...
	function add_header(strItem) 
	{
		...
		JRN_DATE		= arrItem[3];
		SPLDESC			= arrItem[4];
		INV_AMOUNT		= arrItem[5];
		INV_AMOUNT_PPN	= arrItem[6];
		INV_AMOUNT_PPH	= arrItem[7];
		INV_AMOUNT_TOT	= arrItem[8];

		document.getElementById("Reference_Number").value 	= JRN_NUM;
		document.getElementById("Reference_NumberX").value 	= JRN_CODE;
		document.getElementById("JournalH_Date").value 		= JRN_DATE;
		document.getElementById("PERSL_EMPIDX").value 		= SPLDESC;
		document.getElementById("Journal_Amount").value 	= INV_AMOUNT;
		document.getElementById("Journal_AmountX").value 	= doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(INV_AMOUNT)), 2));
		document.getElementById("PPNH_Amount").value 		= INV_AMOUNT_PPN;
		document.getElementById("PPNH_AmountX").value 		= doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(INV_AMOUNT_PPN)), 2));
		document.getElementById("PPHH_Amount").value 		= INV_AMOUNT_PPH;
		document.getElementById("PPHH_AmountX").value 		= doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(INV_AMOUNT_PPH)), 2));
		document.getElementById("GJournal_Total").value 	= INV_AMOUNT_TOT;
		document.getElementById("GJournal_TotalX").value 	= doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(INV_AMOUNT_TOT)), 2));
	}
	...
	function add_listAcc(strItem) 
	{
		...
		// Account Reference
			...
			objTD.noWrap = true;
			//objTD.style.display = 'none';
			objTD.innerHTML = '<input type="text" name="data['+intIndex+'][Ref_Number]" id="data'+intIndex+'Ref_Number" value="'+Ref_Number+'" class="form-control" style="min-width:150px;max-width:300px;" placeholder="No. Seri Pajak" >';
