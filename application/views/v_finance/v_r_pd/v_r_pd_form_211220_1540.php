...
$JournalH_Date_PD	= date('d/m/Y',strtotime($JournalH_Date));
...
<html>
	...
	<?php
		...
		if($LangID == 'IND')
		{
			...
			$alert17	= "Sisa PD sudah habis. Silahkan centang checkbox pada kolom isOver untuk dapat tetap melanjutkan pengisian.";
			$alert18	= "Total detail melebihi nilai Perjalanan Dinas (PD).";
		}
		else
		{
			$subTitleH	= "Add";
			...
			$alert14	= "Total details exceed the value of PD Amount";
                ...
                <div class="row">
                            ...
                            <div class="box-body">
				                ...
				                <div class="row">
			                    	...
			                    	<?php
										...
										endforeach;
										$remRealiz 	= $Journal_Amount - $totRealiz;
			                    	?>
					                ...
					                <div class="col-md-9">
					                  	<div class="row">
					                    	...
					                    	<div class="col-md-10">
					                    		<?php echo "$Manual_No &nbsp;&nbsp;&nbsp;&nbsp; <strong>$Date :</strong> $jrnDateV"; ?>
					                    	</div>
				                    	</div>
					                  	<div class="row">
					                    	<div class="col-md-2">
					                  	...
					                  	<div class="row">
					                    	...
					                    	<div class="col-md-5">
					                    		<?php echo "<i>$pdVal:</i><br><i class='text-primary' style='font-size: 16px;'><strong>".number_format($Journal_Amount, 2)."</strong></i><br>"; ?>
					                    	</div>
					                    	<div class="col-md-5">
					                    		<?php echo "<i>$RealizationValue:</i><br><i class='text-success' style='font-size: 16px;'><strong>".number_format($totRealiz, 2)."</strong></i>"; ?>
					                    	</div>
				                    	</div>
					                  	<div class="row">
					                    	<div class="col-md-2">
					                    	...
					                    	<div class="col-md-5">
					                    		<?php echo "<i>$ovPay:</i><br><i class='text-danger' style='font-size: 16px;' id='ovPay'><strong>".number_format($OverRPD, 2)."</strong></i>"; ?>
					                    	</div>
				                    	</div>
                    ...
                    <div class="col-md-5">
                        <div class="box box-warning">
                            <div class="box-header with-border">
                                <h3 class="box-title"><?php echo $pdRealiz; ?></h3>
                            </div>
                            <div class="box-body">
				                <div class="form-group">
	                            	<div class="col-sm-7">
	                                    <label for="exampleInputEmail1"><?php echo $sourceDoc; ?></label>
	                        ...
	                        <div class="search-table-outter">
	                            <table id="tbl" class="table table-bordered table-striped" width="100%">
		                        	<tr style="background:#CCCCCC">
		                              	...
		                              	<th width="7%" style="text-align:center">Volume</th>
		                            ...
		                            if($task == 'edit')
		                            {
										...
										foreach($result as $row) :
		                                    ...
		                                    <td style="text-align:right; vertical-align: middle; <?php if($disButtonR == 1) { ?> background-color:#F00<?php } ?>">
										  	...
										  	<td style="text-align:left; vertical-align: middle;">
										 		<input type="text" name="data[<?php echo $currentRow; ?>][isExtra]" id="data<?php echo $currentRow; ?>isExtra" value="<?php echo $isExtra; ?>" class="form-control">
		                                    </td>
							          </tr>
		                              	<?php
		                             	endforeach;
...
<script>
	...
	function add_listAcc(strItem) 
	{
		...
		// Sehingga, jika ada yang kelebihan penggunaan uang (versi lapangan), maka akan membentuk Piutang Karyawan yang hrs dibayar perusahaan
			objTD = objTR.insertCell(objTR.cells.length);
			objTD.align = 'left';
			objTD.innerHTML = '<input type="hidden" name="data['+intIndex+'][isExtra]" id="data'+intIndex+'isExtra" value="0" class="form-control"><input type="checkbox" id="chkExtra'+intIndex+'" onClick="chkExtra(this, '+intIndex+')">';
	...
	{
		var decFormat		= document.getElementById('decFormat').value;
		var Journal_Amount	= document.getElementById('Journal_Amount').value;
	...
	{
		totRealizRem 		= parseFloat(document.getElementById('Journal_AmountRem').value);
		...
		var isExtra 		= document.getElementById('data'+row+'isExtra').value;
		...
		if(isExtra == 0)
		{
			if(ITM_UNIT == 'LS')
			{
				...
				{
					var ITM_REMAMNV	= doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(ITM_REMAMN)), decFormat));
					var ITM_PRICE 	= parseFloat(ITM_REMAMN / ITM_VOLM);

					swal('<?php echo $alert16; ?>'+ITM_REMAMNV,
					{
						icon: "warning",
					})
					.then(function()
		            {
						...
						var TOTPRC		= parseFloat(ITM_VOLM) * parseFloat(ITM_PRICE);
						var realizRem	= doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(totRealizRem)), decFormat));
						var totRealz 	= parseFloat(totRealizRem);

						if(parseFloat(TOTPRC) > parseFloat(totRealizRem))
						{
							...
							.then(function()
				            {
								var ITM_PRICE 	= parseFloat(totRealz / ITM_VOLM);
				            	var TOTPRC		= parseFloat(ITM_VOLM) * parseFloat(ITM_PRICE);
								...
								document.getElementById('ITM_PRICE'+row).value 			= doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(ITM_PRICE)), 2));
								document.getElementById('data'+row+'ITM_PRICE').value 	= parseFloat(Math.abs(ITM_PRICE));
								document.getElementById('JournalD_Amount'+row).value 	= doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(totRealz)), 2));
								document.getElementById('data'+row+'JournalD_Amount').value = parseFloat(Math.abs(totRealz));

								if(totRealizRem == 0)
									document.getElementById('chkExtra'+row).disabled 		= false;
								else
									document.getElementById('chkExtra'+row).disabled 		= true;
				        ...
				        else
				        {
							...
							document.getElementById('ITM_PRICE'+row).value 			= doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(ITM_PRICE)), 2));
							document.getElementById('data'+row+'ITM_PRICE').value 	= parseFloat(Math.abs(ITM_PRICE));
							document.getElementById('JournalD_Amount'+row).value 	= doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(ITM_REMAMN)), 2));
							document.getElementById('data'+row+'JournalD_Amount').value = parseFloat(Math.abs(ITM_REMAMN));
				...
				else
				{
					...
					if(parseFloat(TOTPRC) > parseFloat(totRealizRem))
					{
						...
						.then(function()
			            {
							document.getElementById('ITM_VOLM'+row).value 			= doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(1)), 2));
							document.getElementById('data'+row+'ITM_VOLM').value 	= parseFloat(Math.abs(1));
							document.getElementById('ITM_PRICE'+row).value 			= doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(totRealz)), 2));
							document.getElementById('data'+row+'ITM_PRICE').value 	= parseFloat(Math.abs(totRealz));
							document.getElementById('JournalD_Amount'+row).value 	= doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(totRealz)), 2));
							document.getElementById('data'+row+'JournalD_Amount').value = parseFloat(Math.abs(totRealz));
			        ...
			        else
			        {
						...
						document.getElementById('ITM_PRICE'+row).value 			= doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(ITM_PRICE)), 2));
						document.getElementById('data'+row+'ITM_PRICE').value 	= parseFloat(Math.abs(ITM_PRICE));
						document.getElementById('JournalD_Amount'+row).value 	= doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(TOTPRC)), 2));
					...
					.then(function()
		            {
						...
						if(parseFloat(TOTPRC) > parseFloat(totRealizRem))
						{
							...
							.then(function()
				            {
								...
								document.getElementById('ITM_PRICE'+row).value 			= doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(totRealz)), 2));
								document.getElementById('data'+row+'ITM_PRICE').value 	= parseFloat(Math.abs(totRealz));
								document.getElementById('JournalD_Amount'+row).value 	= doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(0)), 2));
						...
						{
							/*totRealizRem 	= parseFloat(totRealizRem - TOTPRC);
							document.getElementById('Journal_AmountRem').value 		= totRealizRem;*/

							document.getElementById('ITM_VOLM'+row).value 			= doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(ITM_VOLM)), 2));
							document.getElementById('data'+row+'ITM_VOLM').value 	= parseFloat(Math.abs(ITM_VOLM));
							document.getElementById('ITM_PRICE'+row).value 			= doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(TOTPRC)), 2));
							document.getElementById('data'+row+'ITM_PRICE').value 	= parseFloat(Math.abs(TOTPRC));
							document.getElementById('JournalD_Amount'+row).value 	= doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(TOTPRC)), 2));
							document.getElementById('data'+row+'JournalD_Amount').value = parseFloat(Math.abs(TOTPRC));
				...
				else
				{
					...
					if(parseFloat(TOTPRC) > parseFloat(totRealizRem))
					{
						...
						.then(function()
			            {
							...
							document.getElementById('Journal_AmountRem').value 		= 0;
							document.getElementById('ITM_VOLM'+row).value 			= doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(1)), 2));
							document.getElementById('data'+row+'ITM_VOLM').value 	= parseFloat(Math.abs(1));
							document.getElementById('ITM_PRICE'+row).value 			= doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(totRealizRem)), 2));
							document.getElementById('data'+row+'ITM_PRICE').value 	= parseFloat(Math.abs(totRealizRem));
							document.getElementById('JournalD_Amount'+row).value 	= doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(totRealizRem)), 2));
							document.getElementById('data'+row+'JournalD_Amount').value = parseFloat(Math.abs(totRealizRem));
			...
			if(ITM_UNIT == 'LS')
			{
				...
				if(TOTPRC > ITM_REMAMN)
				{
					...
					.then(function()
		            {
						...
						document.getElementById('ITM_VOLM'+row).value 			= doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(1)), 2));
						document.getElementById('data'+row+'ITM_VOLM').value 	= parseFloat(Math.abs(1));
						document.getElementById('ITM_PRICE'+row).value 			= doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(ITM_REMAMN)), 2));
						document.getElementById('data'+row+'ITM_PRICE').value 	= parseFloat(Math.abs(ITM_REMAMN));
						document.getElementById('JournalD_Amount'+row).value 	= doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(ITM_REMAMN)), 2));
						document.getElementById('data'+row+'JournalD_Amount').value = parseFloat(Math.abs(ITM_REMAMN));
	...
	function countAmnRem()
	{
		...
		for(i=1;i<=totRow;i++)
		{
			let myObj 	= document.getElementById('data'+i+'JournalD_Amount');
			var values 	= typeof myObj !== 'undefined' ? myObj : '';
			
			if(values != null)
			{
				JournalD_Amn 	= document.getElementById('data'+i+'JournalD_Amount').value;
				totRealizRem 	= parseFloat(totRealizRem - JournalD_Amn);
				realizRem 		= parseFloat(totRealizRem);
				if(totRealizRem < 0)
				{
					realizRem 	= 0;

					document.getElementById('OverRPD').value 	= RoundNDecimal(parseFloat(Math.abs(totRealizRem)), 2);
					document.getElementById('ovPay').innerHTML	= doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(totRealizRem)), 2));
				}

				document.getElementById('Journal_AmountRem').value 	= realizRem;
			}
		}
	}
				...
				else if(parseFloat(totAmountD) > parseFloat(totRealizRem))
				{
					if(isExtra == 1)
					{
				        ...
				        .then((willDelete) => 
				        {
				            if (willDelete) 
				            {
								overPay 		= parseFloat(totAmountD) - parseFloat(totRealizRem);
