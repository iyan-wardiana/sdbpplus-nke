...
// $EMPNAME = $this->db->select("CONCAT(First_Name,' ',Last_Name) AS FullName", false)->from("tbl_employee")->where(["Emp_ID" => $PERSL_EMPID, "Employee_status" => 1])->get()->row("FullName");
    ...
    // get supplier
    if($SPLCODE != '') $PERSL_EMPID = $SPLCODE;
    $EMP_NAME       = "";
    ...
    // if($isManualClose == 1)
    // {
    //     $Journal_AmountReal = $Journal_Amount;
    // }

    // GET Realisasi from voucher
    $s_VOC          = "SELECT IFNULL(SUM(Invoice_Amount),0) AS TOTINV_Amount FROM tbl_journalheader_pd_rinv WHERE JournalH_Code = '$JournalH_Code'";
    ...
    $REMREALZ_Amount = $TREALZAmount - $Journal_Amount;
    if($TREALZAmount < $Journal_Amount)
    {
        $REM_DESC = "Lebih Bayar";
        $Title_PD = "PIUTANG PEMBAYARAN DIMUKA";
        $REMREALZ_AmountD = $Journal_Amount - $TREALZAmount;

        if($isManualClose == 1)
        {
            $REM_DESC = "Pengembalian";
            $Title_PD = "PENYELESAIAN PEMBAYARAN DIMUKA";
            $REMREALZ_AmountD = $Journal_AmountTsf;
        }
    }
    elseif($TREALZAmount > $Journal_Amount)
    {
            ...
            <div class="box-header-detail">
                <table width="100%" border="0" cellpadding="0" cellspacing="0">
                    ...
                    <tr>
                        <td width="70">DATE</td>
                        <td width="5">:</td>
                        <td width="450">&nbsp;<?php echo date('d-m-Y', strtotime($JournalH_Date_PD)); ?></td>
            ...
            <div class="box-detail">
                <table width="100%" border="1" cellpadding="0" cellspacing="0">
                    ...
                    <tbody>
                        <tr>
                            <td width="100">&nbsp;</td>
                            <td>&nbsp;Pembayaran Tagihan No.&nbsp;<?php echo "$Manual_No, Referensi No. $Reference_Number"; ?></td>
                            <td colspan="2" width="100" style="text-align: right;"><?php echo number_format($Amount, 2); ?>&nbsp;</td>
                        </tr>
                        <tr style="display: none;">
                            <td width="100">&nbsp;</td>
                            <td>&nbsp;Nomor Seri Pajak : &nbsp;<?php echo $TAXNO; ?></td>
                        ...
                        <tr>
                            ...
                            <td>
                            <?php
                                // if($isManualClose == 1) echo "Penyelesaian PD Tunai";
                                // elseif($isManualClose == 2) echo "Penyelesaian PD Lainnya ($Close_Notes)";
                                if($isManualClose == 2) echo "Penyelesaian PD Lainnya ($Close_Notes)";
                                else echo "Nilai Penyelesaian PD";
                            ?>
                            </td>
                            <td colspan="2" width="100" style="text-align: right;"><?php echo number_format($Journal_AmountReal, 2); ?></td>
                        </tr>
                        <tr>
                            <td width="100">&nbsp;</td>
                            <td>Nilai Penyelesaian Voucher</td>
                            <td colspan="2" width="100" style="text-align: right;"><?php echo number_format($TOTINV_Amount, 2); ?></td>
                        </tr>
                        <tr>
                            <td width="100">&nbsp;</td>
                            <td>PPN</td>
                            <td colspan="2" width="100" style="text-align: right;">
                                <?php  
                                    if($PPNH_Amount == 0)
                        ...
                        <tr>
                            <td width="100">&nbsp;</td>
                            <td style="font-weight: bold;">Jumlah Penyelesaian</td>
                            <td colspan="2" width="100" style="text-align: right; font-weight: bold; border-top: double;"><?php echo number_format($TREALZAmount, 2); ?></td>
                        </tr>
                        <tr>
                            <td width="100">&nbsp;</td>
                            <td>&nbsp;</td>
                    ...
                    <tfoot>
                        <tr>
                            ...
                            <td style="text-align: right;">
                                <?php echo number_format(abs($REMREALZ_Amount),2); ?>
                            </td>
                        </tr>
                    </tfoot>
                </table>
                ...
                <table width="100%" border="0" cellpadding="0" cellspacing="0">
                    ...
                    <tbody>
                        <?php
                            ...
                            $this->db->select("A.JournalH_Code, A.SPLCODE, B.Acc_Id, B.proj_Code, B.Journal_DK, B.Base_Debet, B.Base_Kredit, B.PPN_Code, B.PPN_Amount, B.PPH_Code, B.PPH_Amount, B.Ref_Number, B.Other_Desc, B.TAX_NO, B.TAX_DATE, C.ITM_CODE, C.ITM_NAME");
                            // $this->db->select_sum("B.Base_Debet");
                            // $this->db->select_sum("B.Base_Kredit");
                            $this->db->from("tbl_journalheader_pd A");
                            $this->db->join("tbl_journaldetail_pd B", "B.JournalH_Code = A.JournalH_Code AND B.proj_Code = A.proj_Code");
                            $this->db->join("tbl_item C", "C.ITM_CODE = B.ITM_CODE AND C.PRJCODE = B.proj_Code", "LEFT");
                            $this->db->where(["A.proj_Code" => $PRJCODE, "A.JournalH_Code" => $JournalH_Code, "B.ISPERSL_REALIZ" => 1]);
                            ...
                            if($getJournal_Hutang->num_rows() > 0)
                            {
                                ...
                                foreach($getJournal_Hutang->result() as $rJ):
                                    ...
                                    $TAX_NO         = $rJ->TAX_NO;
                                    $TAX_DATE       = date('d-m-Y', strtotime($rJ->TAX_DATE));
