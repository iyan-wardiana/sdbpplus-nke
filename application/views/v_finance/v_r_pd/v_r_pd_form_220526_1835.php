                            ...
                            <div class="box-body">
				                ...
				                <div class="row">
			                    	...
			                    	<?php
										...
										{
											$remRealiz 	= 0;
											$OverRPD 	= $Journal_Amount - $totRealiz;
										}
			                    	?>
					                ...
					                <div class="col-md-9">
					                  	...
					                  	<div class="row">
					                    	...
					                    	<div class="col-md-5">
					                    		<?php echo "<i>$settVal:</i><br><i class='text-success' style='font-size: 16px;'><strong>".number_format($totRealiz, 2)."</strong></i>"; ?>
					                    	</div>
				                    	</div>
					                  	<div class="row">
					                    	<div class="col-md-2">
                            ...
                            <div class="box-body">
				                <div class="form-group">
	                            	<div class="col-sm-7">
				                    	...
				                    	<input type="text" class="form-control" name="Manual_No" id="Manual_No" size="30" value="<?php echo "V$Manual_No"; ?>" readonly />
		                            </div>
	                            	<div class="col-sm-5">
...
<script>
	...
	function add_listAcc(strItem) 
	{
		...
		// Sehingga, jika ada yang kelebihan penggunaan uang (versi lapangan), maka akan membentuk Piutang Karyawan yang hrs dibayar perusahaan
			objTD = objTR.insertCell(objTR.cells.length);
			objTD.align = 'left';
			objTD.innerHTML = '<input type="hidden" name="data['+intIndex+'][isExtra]" id="data'+intIndex+'isExtra" value="0" class="form-control" style="min-width: 100px"><input type="checkbox" id="chkExtra'+intIndex+'" onClick="chkExtra(this, '+intIndex+')" title="Centang jika ingin memasukan nilai lebih besar dari PD">';
	...
	function chgVolume(thisval, row)
	{
		...
		document.getElementById('data'+row+'JournalD_Amount').value 	= TOTPRC;
		
		countTot()

		totRemOri 			= parseFloat(document.getElementById('Journal_AmountRemOri').value);	// MAXIMUM REALISASI
		itmRealizRem		= parseFloat(document.getElementById('data'+row+'ITM_REMAMN').value);	// SISA REALISASI PER TIEM THD BUDGET
		itmRealiz			= parseFloat(ITM_VOLM) * parseFloat(ITM_PRICE);							// TOTAL REALISASI PER ITEM
		itmRealizAll		= parseFloat(document.getElementById('Journal_TotRealiz').value);		// TOTAL RELISASI KESELURUHAN BARIS ITEM
		totRealizRem 		= parseFloat(document.getElementById('Journal_AmountRem').value);

		if(isExtra == 0)
		{
			if(itmRealiz > itmRealizRem)
			{
				document.getElementById('btnSave').style.display 	= 'none';

				var ITM_REMAMNV	= doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(itmRealizRem)), 2));

				swal('<?php echo $alert19; ?> '+ITM_REMAMNV,
				{
					icon: "warning",
				})
				.then(function()
	            {
					...
					document.getElementById('Journal_TotRealiz').value 		= 0;

					document.getElementById('chkExtra'+row).disabled 		= false;

					document.getElementById('ITM_VOLM'+row).focus();
					//**** countAmnRem();		// tidak masuk kondisi diperbolehkan kelebihan realisasi
	            });
			}
			else if(itmRealizAll > totRemOri)
			{
				document.getElementById('btnSave').style.display 	= 'none';
		...
		else
		{
			...
			else if((ITM_UNIT != 'LS' && ITM_UNIT != 'BLN') && ITM_VOLM > ITM_REMVOL)
			{
				document.getElementById('btnSave').style.display 	= '';

				var ITM_REMVOLV	= doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(ITM_REMVOL)), 2));
	...
	function chgPrice(thisval, row)
	{
		...
		if(ITM_PRICE == 0 || isNaN(ITM_PRICE))
		{
			swal("<?=$alert6; ?>",
			{
				icon:"warning",
		...
		if(isExtra == 0)
		{
			if(itmRealiz > itmRealizRem)
			{
				...
				.then(function()
	            {
					...
					document.getElementById('ITM_PRICE'+row).value 			= doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(0)), 2));
					document.getElementById('data'+row+'ITM_PRICE').value 	= parseFloat(Math.abs(0));
					document.getElementById('JournalD_Amount'+row).value 	= doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(0)), 2));
					document.getElementById('data'+row+'JournalD_Amount').value = parseFloat(Math.abs(0));
					document.getElementById('Journal_TotRealiz').value 		= 0;
	...
	function countTot()
	{
		...
		for(i=1;i<=totRow;i++)
		{
			...
			if(values != null)
			{
				JournalD_Amn 	= parseFloat(document.getElementById('data'+i+'JournalD_Amount').value);
				console.log('i = '+i+' = '+JournalD_Amn)
				totAmount 		= parseFloat(totAmount) + parseFloat(JournalD_Amn);
			}
		}
		console.log('totRealiz = '+totAmount)
		document.getElementById('Journal_TotRealiz').value 	= totAmount;
	}
