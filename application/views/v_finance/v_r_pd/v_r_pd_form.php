    ...
    <body class="<?php echo $appBody; ?>" style="background-color: <?=$comp_color?>">
		...
		<section class="content">
	        <form class="form-horizontal" name="frm" id="frm" method="post" action="<?php echo $form_action; ?>" enctype="multipart/form-data" onSubmit="return validateInData()">
	            ...
	            <input type="hidden" name="ACC_OPBAL" id="ACC_OPBAL" value="<?php echo $ACC_OPBAL; ?>" />
                    ...
                    <div class="col-md-5">
                        <div class="box box-warning">
                            ...
                            <div class="box-body">
								...
								<div class="form-group">
									<div class="col-sm-5">
										<label for="inputName"><?php echo "&nbsp;Jumlah Penyelesaian"; ?></label>
									</div>
									<!-- Jumlah penyelesaian PD sudah termasuk PPN jika ada ppn -->
									<?php
										$Journal_AmountReal = $Journal_AmountReal + $PPNH_Amount;
									?>
									<!-- END -->
									<div class="col-sm-7" style="padding-left: 0;">
										<input type="text" class="form-control" id="Manual_PDAmount1" name="Manual_PDAmount1" value="<?php echo number_format($Journal_AmountReal, $decFormat); ?>" onKeyPress="return isIntOnlyNew(event);">
										<input type="hidden" class="form-control" id="Manual_PDAmount" name="Manual_PDAmount" value="<?php echo $Journal_AmountReal; ?>">
									</div>
								</div>
								<?php endif; ?>
                            </div>
		                            ...
		                            if($task == 'edit')
		                            {
										...
										foreach($result as $row) :
											...
											/*$sqlJOBD			= "SELECT ITM_VOLM AS ITM_VOLMBG, (ITM_BUDG + ADD_JOBCOST - ADDM_JOBCOST) AS ITM_BUDG,
																	...
																	WHERE JOBCODEID = '$JOBCODEID' AND ITM_CODE = '$ITM_CODE' LIMIT 1";
											...
											$ADDVOLM 			= 0;
											$ADD_TOTAL 			= 0;
											$s_JL 	= "SELECT 	ITM_VOLM, ITM_BUDG,	(AMD_VOL - AMDM_VOL) AS AMD_VOL, (AMD_VAL - AMDM_VAL) AS AMD_VAL,
																(PR_VOL+WO_VOL+VCASH_VOL+VLK_VOL+PPD_VOL+PR_VOL_R+WO_VOL_R+VCASH_VOL_R+VLK_VOL_R+PPD_VOL_R-PR_CVOL-WO_CVOL) AS REQ_VOLM,
																(PO_VAL+WO_VAL+VCASH_VAL+VLK_VAL+PPD_VAL+PO_VAL_R+WO_VAL_R+VCASH_VAL_R+VLK_VAL_R+PPD_VAL_R-PO_CVAL-WO_CVAL) AS REQ_AMOUNT
														FROM tbl_joblist_detail_$PRJCODEVW
														WHERE JOBCODEID LIKE '$JOBCODEID%'";
											$r_JL 	= $this->db->query($s_JL);
											foreach($r_JL->result() as $rw_JL):
												$ITM_VOLMBG1		= $rw_JL->ITM_VOLM;
												$ITM_BUDG1 			= $rw_JL->ITM_BUDG;

												// Addendum
													$ADDVOLM 		= $rw_JL->AMD_VOL;
													$ADD_TOTAL 		= $rw_JL->AMD_VAL;

												// REQUEST :
													$ITM_USEDR 		= $rw_JL->REQ_VOLM;
													$ITM_USEDR_AM 	= $rw_JL->REQ_AMOUNT;
											endforeach;
											$ITM_VOLMBG 		= $ITM_VOLMBG1 + $ADDVOLM;
											$ITM_BUDG 			= $ITM_BUDG1 + $ADD_TOTAL;
											...
											{
												$sqlITM		= "SELECT Account_NameId FROM tbl_chartaccount_$PRJCODEVW WHERE Account_Number = '$Acc_Id'";
												$resITM 	= $this->db->query($sqlITM)->result();
												foreach($resITM as $rowITM) :
											...
											$BUDG_REMVOLM	= $ITM_VOLMBG - $ITM_USEDR + $ITM_VOLM;
											$BUDG_REMAMNT	= $ITM_BUDG - $ITM_USEDR_AM+ $AmountV;
											...
											$allUSED 		= $ITM_USED_AM + $ITM_USEDR_AM+ $AmountV;
		                                    ...
		                                    <tr id="tr_<?php echo $currentRow; ?>">
												...
												<td width="6%" nowrap style="text-align:center; display:none">
											  	...
											  	<td style="text-align:right; vertical-align: middle;">
			                                        ...
			                                        <input type="hidden" name="data<?php echo $currentRow; ?>ITM_REMAMN" id="data<?php echo $currentRow; ?>ITM_REMAMN" value="<?php echo $BUDG_REMAMNT; ?>" class="form-control" style="max-width:150px;" >
			                                        <input type="hidden" id="data<?php echo $currentRow; ?>ISLS" value="<?=$r_isLS?>" class="form-control" style="max-width:150px;" >
			                                    </td>
											  	<td style="text-align:right; vertical-align: middle;">
											 		<?php if ($edited == 1) { ?>
...
<script>
  	$(function () {
		...
		$('#Manual_PDAmount1').on('blur', function(e){
			let Manual_PDAmount		= parseFloat(eval(e.target).value.split(",").join(""));
			$('#Manual_PDAmount1').val(doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(Manual_PDAmount)), 2)));
			$('#Manual_PDAmount').val(Manual_PDAmount);
		})
  	});
			...
			$.ajax({
				...
				success: function(response)
				{
					...
					if(isLockJ == 1)
					{
						...
						$('#GEJ_STAT_PD>option[value="3"]').attr('disabled','disabled');
					...
					else
					{
						...
						document.getElementById('divAlert').style.display   = 'none';
						$('#GEJ_STAT_PD>option[value="3"]').removeAttr('disabled','disabled');
						// document.getElementById('btnSave').style.display    = '';
					}
					...
					if(isLockT == 1)
					{
						if(LockCateg == 1)
						{
							$('#alrtLockJ').css('display','');
							document.getElementById('divAlert').style.display   = 'none';
							$('#GEJ_STAT_PD').removeAttr('disabled','disabled');
							$('#GEJ_STAT_PD>option[value="3"]').attr('disabled','disabled');
							// document.getElementById('btnSave').style.display    = '';
						}
						else
						{
							...
							$('#GEJ_STAT_PD>option[value="3"]').removeAttr('disabled','disabled');
							$('#GEJ_STAT_PD').attr('disabled','disabled');
	...
	function add_listAcc(strItem) 
	{
		...
		// Sehingga, jika ada yang kelebihan penggunaan uang (versi lapangan), maka akan membentuk Piutang Karyawan yang hrs dibayar perusahaan
			objTD = objTR.insertCell(objTR.cells.length);
			objTD.align = 'left';
			objTD.innerHTML = '<input type="hidden" name="data['+intIndex+'][isExtra]" id="data'+intIndex+'isExtra" value="1" class="form-control" style="min-width: 100px"><input type="checkbox" id="chkExtra'+intIndex+'" onClick="chkExtra(this, '+intIndex+')" title="Centang jika ingin memasukan nilai lebih besar dari PD" checked><input type="hidden" name="data['+intIndex+'][isVerified]" id="data'+intIndex+'isVerified" value="'+isVerified+'" class="form-control" style="min-width:150px;max-width:300px;" >';
	...
	function add_item(strItem) 
	{
		...
		document.getElementById('data'+theRow+'ISLS').value				= ISLS;
		/*if(ITM_UNIT == 'LS')
		{
	...
	function chgVolume(thisval, row)
	{
		...
		var ISLS			= document.getElementById('data'+row+'ISLS').value;
	...
	function countAmount()
	{
		...
		for(i=1;i<=totRow;i++)
		{
			let myObj 	= document.getElementById('data'+i+'JournalD_Pos');
			var values 	= typeof myObj !== 'undefined' ? myObj : '';
			let	AccId 	= document.getElementById('data'+i+'Acc_Id').value;
			var values 	= typeof myObj !== 'undefined' ? myObj : '';
	...
	function validateInData()
	{
		...
		/* ------------- Hidden => karena akun kredit masuk k UANG MUKA PINJAMAN DINAS
