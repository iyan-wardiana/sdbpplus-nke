...
<?php
	...
	if (isset($_POST['submitSrch']))
	{
		...
		$myNewNoc = 0;			
		/*$hasilC		= "SELECT COUNT(*) AS COUNTME FROM VOCHD.DBF WHERE VOCCODE = '$VOCCODE'";
		$qrhasilC	= odbc_exec($odbc, $hasilC) or die (odbc_errormsg());
		while($hasilC = odbc_fetch_array($qrhasilC))*/
		
		$odbc 		= odbc_connect ('DBaseNKE3', '', '') or die('Could Not Connect to ODBC Database!');
		$jumlah		= 0;
		//$rs 		= odbc_exec($odbc, "SELECT Count(*) AS counter FROM VOCHD1.DBF WHERE VOCCODE = '$VOCCODE'");
		...
		$jumlah		= 1;
		//$hasilC		= "VOCHD WHERE VOCCODE = '$VOCCODE'";
		//$jumlah	= $this->db->count_all($hasilC);
		...
		<div class="search-table-outter">
			<table id="example1" class="table table-bordered table-striped table-responsive search-table inner">
				...
				<?php								
					...
					if($jumlah > 0)
					{
						//$getVOCHD		= "SELECT VOCCODE, TRXDATE, PRJCODE, PRJCODE, SPLCODE FROM VOCHD.DBF WHERE VOCCODE = '$VOCCODE'";
						//$qrVOCHD		= odbc_exec($odbc, $getVOCHD) or die (odbc_errormsg());
						//while($vVOCHD	= odbc_fetch_array($qrVOCHD))
						$getVOCHD		= "SELECT VOCCODE, TRXDATE, PRJCODE, PRJCODE, SPLCODE FROM VOCHD WHERE VOCCODE = '$VOCCODE'";
						$qrVOCHD		= $this->db->query($getVOCHD)->result();
						foreach($qrVOCHD as $vVOCHD) :
						{
							$i			= $i +1;
							$VOCCODE	= $vVOCHD->VOCCODE;
							$TRXDATE	= $vVOCHD->TRXDATE;
							$PRJCODE	= $vVOCHD->PRJCODE;
							$SPLCODE	= $vVOCHD->SPLCODE;
