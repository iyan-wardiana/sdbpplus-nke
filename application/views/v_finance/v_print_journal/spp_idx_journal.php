...
<?php	
	if (isset($_POST['submitSrch']))
	{
		...
		echo $odbc;
		$hasilC			= "SELECT COUNT(*) AS COUNTME FROM SPPHD.DBF WHERE SPPCODE = '$SPPCODE'";
		...
		$qrySPP			= "SPPHD WHERE SPPCODE = '$SPPCODE'";
		$jumlah			= $this->db->count_all($qrySPP);
		echo $jumlah;
		?>
		...
		<div class="search-table-outter">
			<table id="example1" class="table table-bordered table-striped table-responsive search-table inner">
				...
				<?php
					...
					if($jumlah > 0)
					{
						/*$getSPPHD	= "SELECT SPPCODE, TRXDATE, PRJCODE FROM SPPHD.DBF WHERE SPPCODE = '$SPPCODE'";
						$qSPPHD		= odbc_exec($odbc, $getSPPHD) or die (odbc_errormsg());
						while($vSPPHD = odbc_fetch_array($qSPPHD))
						{
							$i			= $i + 1;
							$SPPCODE	= $vSPPHD['SPPCODE'];
							$TRXDATE	= $vSPPHD['TRXDATE'];
							$PRJCODE	= $vSPPHD['PRJCODE'];*/
						$qryAllSPP		= "SELECT SPPCODE, TRXDATE, PRJCODE FROM SPPHD WHERE SPPCODE = '$SPPCODE'";
						$resAllSPP		= $this->db->query($qryAllSPP)->result();
						foreach($resAllSPP as $rowAllSPP) :
							$i			= $i + 1;
							$SPPCODE	= $rowAllSPP->SPPCODE;
							$TRXDATE	= $rowAllSPP->TRXDATE;
							$PRJCODE	= $rowAllSPP->PRJCODE;
							$SPLCODE	= '';
							$SPLDESC	= '-';
