 ...
 * File Name	= asset_usage_selectasset.php
 * Location		= -
*/
              ...
              <table id="example1" class="table table-bordered table-striped table-responsive search-table inner" width="100%">
                <thead>
                    <tr>
                        ...
                        <th width="7%" style="vertical-align:middle; text-align:center" nowrap><?php echo $Code ?></th>
               	  	  <th width="23%" style="vertical-align:middle; text-align:center"><?php echo $Name ?></th>
               	  	  <th width="48%" nowrap style="text-align:center"><?php echo $Description ?></th>
                ...
                <?php
                    $i = 0;
                    $j = 0;
					$EndDateN	= date('Y-m-d', strtotime('+1 days', strtotime($EndDate))); //operasi penjumlahan tanggal sebanyak 1 hari
                    if($reCountAllAsset>0)
                    {
                        ...
                        foreach($viewAllAsset as $row) :
                            ...
                            $AS_LASTPOS		= $row->AS_LASTPOS;			// 
                            $AS_LASTJOB		= $row->AS_LASTJOB;			// 
                            $AS_LASTSTAT	= $row->AS_LASTSTAT;		// 
							...
							$sql0			= "tbl_asset_usage WHERE AU_AS_CODE = '$AS_CODE' AND AU_PROCS IN (1,2)";
							...
							{
								$PRJCODE		= "";
								/*$sqlPRJ 		= "SELECT PRJCODE FROM tbl_asset_usage WHERE AU_AS_CODE = '$AS_CODE'
												AND STR_TO_DATE(AU_STARTD, '%Y-%m-%d') >= '$StartDate' AND STR_TO_DATE(AU_ENDD, '%Y-%m-%d') <= '$EndDateN'";*/
								$sqlPRJ 		= "SELECT PRJCODE, AU_STARTD, AU_ENDD FROM tbl_asset_usage 
													WHERE AU_AS_CODE = '$AS_CODE' AND AU_PROCS IN (1,2)";
								$resultPRJ 		= $this->db->query($sqlPRJ)->result();
								foreach($resultPRJ as $rowPRJ) :
								...
								$SCHEDULED		= "Scheduled - $PRJCODE : $SD to $ED";
							...
							?> 
                            <td style="text-align:center"><input type="radio" name="chk" value="<?php echo $AS_CODE;?>|<?php echo $AS_NAME;?>|<?php echo $AS_DESC;?>|<?php echo $AS_DESC;?>|<?php echo $AS_DESC;?>|<?php echo $AS_DESC;?>|<?php echo $AS_DESC;?>" onClick="pickThis(this);" <?php if($bgColor > 0) { ?> disabled <?php } ?>/></td>
