...
$End_Date1 		= "$Start_DateM/$Start_DateD/$Start_DateY";
        ...
        <form name="frmsrch" id="frmsrch" action="" method=POST style="display:none">
          ...
          <table width="100%">
            ...
            <tr>
                ...
                <td nowrap valign="top">:</td>
                <td colspan="2" nowrap>
                    <select name="AS_CODE[]" id="AS_CODE" class="form-control select2" multiple="multiple" data-placeholder="&nbsp;&nbsp;&nbsp;Asset Name" style="max-width:280px">
						<?php
                            foreach($resAset as $row) :
								$AS_CODE 	= $row->AS_CODE;
								$AS_CODE_M	= $row->AS_CODE_M;
								$AS_NAME	= $row->AS_NAME;
								$AS_DESC	= $row->AS_DESC;
                                ?>
                                    <option value="<?php echo "$AS_CODE"; ?>" >
                                        <?php echo "$AS_NAME - $AS_DESC > $AS_CODE"; ?>
                                    </option>
                                <?php
                            endforeach;
                        ?>
...
<script>
	...
	function target_popup(form)
	{
		...
		AS_CODE	= document.getElementById('AS_CODE').value;
		if(AS_CODE == '')
		{
			alert('Please select one Asset Name');
			return false;
		}
