<?php
    ...
    // SET PROJECT
		...
		{
			$myrow    = 0;
			$sql      = "SELECT A.proj_Code, B.PRJNAME, B.PRJCOST, B.PRJDATE, B.PRJEDAT, B.PRJ_MNG
						  FROM tbl_employee_proj A
		...
		else
		{
			...
			$PRJCODEX 	= 
			$PRJNAMEX 	= $selPRJCODE;
			$sqlPRJ   	= "SELECT PRJNAME, PRJCOST, PRJDATE, PRJEDAT, PRJ_IMGNAME, PRJ_MNG FROM tbl_project WHERE PRJCODE = $myPRJCODE";
	...
	<body class="<?php echo $appBody; ?>" onLoad="taskNotifApp();taskNotif();">
		...
		<section class="content">
					...
					<div class="small-box bg-teal-gradient">
						<div class="inner">
						    <?php
								...
								$cLogEMPALL	= "tbl_trail_tracker WHERE DATE(TTR_DATE) = DATE(NOW())";
								$cLogEMPALL	= $this->db->count_all($cLogEMPALL);
							?>
							<h3><?php echo $cLogEMP; ?><sup> / <?php echo number_format($cLogEMPALL,0); ?></sup></h3>
							<p>Your Activity</p>
						</div>
						<div class="icon">
			...
			<?php
				...
				$s_MPLAN	= "SELECT SUM(TOT_BUDG) AS TOT_BUDG, SUM(TOT_ADDPLUS) AS TOT_ADDPLUS, SUM(TOT_ADDMIN) AS TOT_ADDMIN
									FROM vw_budget_m WHERE PRJCODE = '$PRJCODEX'";
				...
				foreach($r_MPLAN as $rw_MPLAN):
					...
					$MTR_PLAN		= $TOT_BUDG + $TOT_ADDPLUS - $TOT_ADDMIN;
					$MTR_PLANP 		= $MTR_PLAN;
					if($MTR_PLANP == 0) $MTR_PLANP = 1;
				endforeach;

				$s_UPLAN	= "SELECT SUM(TOT_BUDG) AS TOT_BUDG, SUM(TOT_ADDPLUS) AS TOT_ADDPLUS, SUM(TOT_ADDMIN) AS TOT_ADDMIN
									FROM vw_budget_u WHERE PRJCODE = '$PRJCODEX'";
				...
				foreach($r_UPLAN as $rw_UPLAN):
					...
					$UPH_PLAN		= $TOT_BUDG + $TOT_ADDPLUS - $TOT_ADDMIN;
					$UPH_PLANP 		= $UPH_PLAN;
					if($UPH_PLANP == 0) $UPH_PLANP = 1;
				endforeach;

				$s_SPLAN	= "SELECT SUM(TOT_BUDG) AS TOT_BUDG, SUM(TOT_ADDPLUS) AS TOT_ADDPLUS, SUM(TOT_ADDMIN) AS TOT_ADDMIN
									FROM vw_budget_ssc WHERE PRJCODE = '$PRJCODEX'";
				...
				foreach($r_SPLAN as $rw_SPLAN):
					...
					$SUB_PLAN		= $TOT_BUDG + $TOT_ADDPLUS - $TOT_ADDMIN;
					$SUB_PLANP 		= $SUB_PLAN;
					if($SUB_PLANP == 0) $SUB_PLANP = 1;
				endforeach;

				$s_TPLAN	= "SELECT SUM(TOT_BUDG) AS TOT_BUDG, SUM(TOT_ADDPLUS) AS TOT_ADDPLUS, SUM(TOT_ADDMIN) AS TOT_ADDMIN
									FROM vw_budget_t WHERE PRJCODE = '$PRJCODEX'";
				...
				foreach($r_TPLAN as $rw_TPLAN):
					...
					$ALT_PLAN		= $TOT_BUDG + $TOT_ADDPLUS - $TOT_ADDMIN;
					$ALT_PLANP 		= $ALT_PLAN;
					if($ALT_PLANP == 0) $ALT_PLANP = 1;
				endforeach;

				$s_IPLAN	= "SELECT SUM(TOT_BUDG) AS TOT_BUDG, SUM(TOT_ADDPLUS) AS TOT_ADDPLUS, SUM(TOT_ADDMIN) AS TOT_ADDMIN
									FROM vw_budget_i WHERE PRJCODE = '$PRJCODEX'";
				...
				$s_GEPLAN	= "SELECT SUM(TOT_BUDG) AS TOT_BUDG, SUM(TOT_ADDPLUS) AS TOT_ADDPLUS, SUM(TOT_ADDMIN) AS TOT_ADDMIN
									FROM vw_budget_ge WHERE PRJCODE = '$PRJCODEX'";
				$r_GEPLAN	= $this->db->query($s_GEPLAN)->result();
				foreach($r_GEPLAN as $rw_GEPLAN):
					$TOT_BUDG		= $rw_GEPLAN->TOT_BUDG;
					$TOT_ADDPLUS	= $rw_GEPLAN->TOT_ADDPLUS;
					$TOT_ADDMIN		= $rw_GEPLAN->TOT_ADDMIN;
					$BAU_PLAN		= $TOT_BUDG + $TOT_ADDPLUS - $TOT_ADDMIN;
				endforeach;

				$s_OPLAN	= "SELECT SUM(TOT_BUDG) AS TOT_BUDG, SUM(TOT_ADDPLUS) AS TOT_ADDPLUS, SUM(TOT_ADDMIN) AS TOT_ADDMIN
									FROM vw_budget_o WHERE PRJCODE = '$PRJCODEX'";
				...
				foreach($r_OPLAN as $rw_OPLAN):
					...
					$OTH_PLAN		= $TOT_BUDG + $TOT_ADDPLUS - $TOT_ADDMIN;
				endforeach;
				$OVH_PLAN 		= $BAU_PLAN + $OTH_PLAN;
				$OVH_PLANP 		= $OVH_PLAN;
				if($OVH_PLANP == 0) $OVH_PLANP = 1;

				$s_PROFLOST	= "SELECT SUM(BPP_MTR_REAL) AS MTR_REAL, SUM(BPP_UPH_REAL) AS UPH_REAL, SUM(BPP_SUBK_REAL) AS SUBK_REAL, SUM(BPP_ALAT_REAL) AS ALAT_REAL,
									SUM(BPP_I_REAL) AS I_REAL, SUM(BPP_BAU_REAL) AS BAU_REAL, SUM(BPP_OTH_REAL) AS OTH_REAL
								FROM tbl_profitloss WHERE PRJCODE = '$PRJCODEX'";
				$r_PROFLOST	= $this->db->query($s_PROFLOST)->result();
				foreach($r_PROFLOST as $rowLR):
					$MTR_REAL 	= $rowLR->MTR_REAL;
					$UPH_REAL 	= $rowLR->UPH_REAL;
					$SUB_REAL 	= $rowLR->SUBK_REAL;
					$ALT_REAL 	= $rowLR->ALAT_REAL;
					$I_REAL 	= $rowLR->I_REAL;		// LAIN-LAIN
					$BAU_REAL 	= $rowLR->BAU_REAL;
					$OTH_REAL 	= $rowLR->OTH_REAL;
					$OVH_REAL 	= $BAU_REAL+$OTH_REAL;

					$MTR_REALP 	= $MTR_REAL / $MTR_PLANP * 100;
					$UPH_REALP 	= $UPH_REAL / $UPH_PLANP * 100;
					$SUB_REALP 	= $SUB_REAL / $SUB_PLANP * 100;
					$ALT_REALP 	= $ALT_REAL / $ALT_PLANP * 100;
					$I_REALP 	= $I_REAL / $I_PLANP * 100;
					$OVH_REALP 	= $OVH_REAL / $OVH_PLANP * 100;
				endforeach;
			...
			<div class="row">
                <div class="col-xs-8" style="padding-right: 1px;">
					<div class="box box-success">
						<div class="box-header with-border">
							<h3 class="box-title"><?php echo "$PRJCODEX : $PRJNAMEX"; ?></h3>
							...
							<div class="row">
										...
										<div class="pad box-pane-right">
											<div class="row">
												...
												<div class="col-md-7" style="padding-top: 0px">
													<p class="pull-right" style="font-weight: bold;">&nbsp;</p>
												</div>
											</div>
											<div class="row">
												<div class="col-md-5">
											...
											<div class="row">
												...
												<div class="col-md-7" style="padding-top: 0px">
													<p class="pull-right" style="font-weight: bold; text-align: left;">&nbsp;</p>
												</div>
											</div>
											<div class="row">
												<div class="col-md-2">
								...
								<div class="col-md-4 col-sm-4 box2">
									<div class="pad box-pane-right bg-green">
										<div class="row">
											<div class="col-md-5">
												<p class="pull-left">Nilai Proyek</p>
											</div>
											<div class="col-md-7" style="padding-top: 0px">
												<p class="pull-right" style="font-weight: bold;"><?=number_format($TOT_BOQ)?></p>
											</div>
										</div>
										<div class="row">
											<div class="col-md-5">
												<p class="pull-left">Addendum</p>
											</div>
											<div class="col-md-7" style="padding-top: 0px">
												<p class="pull-right" style="font-weight: bold;"><?=number_format($TOT_ADD)?></p>
											</div>
										</div>
										<div class="row">
											<div class="col-md-5">
												<p class="pull-left">Nilai RAP</p>
											</div>
											<div class="col-md-7" style="padding-top: 0px">
												<p class="pull-right" style="font-weight: bold;"><?=number_format($TOT_RAP)?></p>
											</div>
										</div>
										<div class="row">
											<div class="col-md-5">
												<p class="pull-left">Progres</p>
											</div>
											<div class="col-md-7" style="padding-top: 0px">
												<p class="pull-right" style="font-weight: bold;"><?=number_format($TOT_PROG,2)." %"?></p>
											</div>
										</div>
										<div class="row">
											<div class="col-md-6">
												<p class="pull-left">Masa Pelaksanaan</p>
											</div>
											<div class="col-md-6" style="padding-top: 0px">
												<p class="pull-right" style="font-weight: bold;"><?="$hari hari"?></p>
											</div>
										</div>
										<div class="row">
											<div class="col-md-4">
											...
											<div class="col-md-8" style="padding-top: 0px">
												<p class="pull-right" style="font-weight: bold;"><?="$strD - $endD"?></p>
											</div>
										</div>
										<div class="row">
											<div class="col-md-6">
												<p class="pull-left">Sisa Pelaksanaan</p>
											</div>
											<div class="col-md-6" style="padding-top: 0px">
												<!-- <p class="pull-right" style="font-weight: bold;"><?="$hariRem (".number_format($hariRemP,2)."%)"?></p> -->
												<p class="pull-right" style="font-weight: bold;"><?="$hariRem hari"?></p>
											</div>
										</div>
										<div class="row">
											<div class="col-md-6">
												<p class="pull-left">Progres Internal</p>
											</div>
											<div class="col-md-6" style="padding-top: 0px">
												<p class="pull-right" style="font-weight: bold;"><?=number_format($TOT_PROG,2)." %"?></p>
											</div>
										...
										<div class="row">
											<div class="col-md-6">
												<p class="pull-left">Progres Eksternal</p>
											</div>
											<div class="col-md-6" style="padding-top: 0px">
												<p class="pull-right" style="font-weight: bold;"><?=number_format($TOT_PROG,2)." %"?></p>
											</div>
										...
										<?php
											$MNG_NAME 	= wordwrap($MNG_NAME, 40, "<br>", true);
										?>
										<div class="row">
											<div class="col-md-5">
												<p class="pull-left">Kepala Proyek</p>
											</div>
											<div class="col-md-7" style="padding-top: 0px">
												<p class="pull-right" style="font-weight: bold; text-align: left;"><?=$MNG_NIK?></p>
											</div>
										</div>
										<div class="row">
											<div class="col-md-2">
											...
											<div class="col-md-10" style="padding-top: 0px">
												<p class="pull-right" style="font-weight: bold; text-align: left;"><?=$MNG_NAME?></p>
											</div>
										</div>
										<div class="row">
											<div class="col-md-2">
                ...
                <div class="col-xs-4" style="padding-bottom: 1px;">
							...
							<div class="row no-gutter">
								<div class="pad box-pane-right">
									<div class="row">
										...
										<div class="col-md-6">
											<ul class="chart-legend clearfix">
												<li><i class="fa fa-circle-o text-red"></i>
													<span> M</span> <span class="pull-right" title="<?="dari ".number_format($MTR_PLANP,2)." (".number_format($MTR_REALP,2)."%)"?>"><?=number_format($MTR_REAL)." (".number_format($MTR_REALP,1)."%)";?></span>
												</li>
												<li><i class="fa fa-circle-o text-green"></i>
													<span> U</span> <span class="pull-right" title="<?="dari ".number_format($UPH_PLANP,2)." (".number_format($UPH_REALP,2)."%)"?>"><?=number_format($UPH_REAL)." (".number_format($UPH_REALP,1)."%)";?></span>
												</li>
												<li><i class="fa fa-circle-o text-yellow"></i>
													<span> S</span> <span class="pull-right" title="<?="dari ".number_format($SUB_PLANP,2)." (".number_format($SUB_REALP,2)."%)"?>"><?=number_format($SUB_REAL)." (".number_format($SUB_REALP,1)."%)";?></span>
												</li>
												<li><i class="fa fa-circle-o text-aqua"></i> 
													<span> T</span> <span class="pull-right" title="<?="dari ".number_format($ALT_PLANP,2)." (".number_format($ALT_REALP,2)."%)"?>"><?=number_format($ALT_REAL)." (".number_format($ALT_REALP,1)."%)";?></span>
												</li>
												<li><i class="fa fa-circle-o text-gray"></i>
													<span> I</span> <span class="pull-right" title="<?="dari ".number_format($I_PLANP,2)." (".number_format($I_REALP,2)."%)"?>"><?=number_format($I_REAL)." (".number_format($I_REALP,1)."%)";?></span>
												</li>
												<li><i class="fa fa-circle-o text-gray"></i>
													<span> R</span> <span class="pull-right" title="<?="dari ".number_format($I_PLANP,2)." (".number_format(0,2)."%)"?>"><?=number_format(0)." (".number_format(0,1)."%)";?></span>
												</li>
												<li><i class="fa fa-circle-o text-light-blue"></i>
													<span> O</span> <span class="pull-right" title="<?="dari ".number_format($OVH_PLANP,2)." (".number_format($OVH_REALP,2)."%)"?>"><?=number_format($OVH_REAL)." (".number_format($OVH_REALP,1)."%)";?></span>
												</li>
											</ul>
										</div>
						                ...
						                <div class="col-md-12 col-sm-12 col-xs-12" style="padding-bottom: 1px">
							                <div class="direct-chat-msg">
							                  	<div class="direct-chat-info clearfix">
							                    	<span class="direct-chat-name pull-left">&nbsp;</span>
							                    	<span class="direct-chat-timestamp pull-right"><?=$PROG_UPDAT?></span>
							                  	</div>
							                  	<img class="direct-chat-img" src="<?=$imgPRJ?>" alt="Message User Image">
							                  	<div class="direct-chat-text">
							                    	<?=$PROG_NOTES?>
							                  	</div>
							                </div>
										</div>
						                <div class="col-md-6 col-sm-6 col-xs-12" style="padding-bottom: 1px">
											<div class="small-box bg-yellow-gradient">
												<div class="inner">
													<h3><?=number_format($TOT_PROG,2)?> %</h3>
													<p>PROG. FISIK (INT)</p>
												</div>
												<div class="icon">
													<i class="fa fa-bar-chart" style="font-size: 0.50em;"></i>
												</div>
											</div>
										</div>
						                <div class="col-md-6 col-sm-6 col-xs-12" style="padding-bottom: 1px">
											<div class="small-box bg-teal-gradient">
												<div class="inner">
													<h3><?=number_format($MC_PROGAPP,2)?> %</h3>
													<p>PROG. FISIK (MC)</p>
												</div>
												<div class="icon">
													<i class="fa fa-area-chart" style="font-size: 0.50em;"></i>
												</div>
											</div>
										</div>
									</div>
		    ...
		    <!--- START : PRODUCTION GRAPH -->
				...
				<?php } ?>
		    <!--- END : PRODUCTION GRAPH -->

		    <!--- START : PURCHASING GRAPH : UPDATED ON 25-06-2022 -->
				<?php if($GRF_PURCH == 1) { ?>
			      	<div class="row">
											...
											<div class="col-md-4">
												...
												<div class="progress-group">
													...
													<span class="progress-number" title="dalam juta rupiah">
														<b><?php echo number_format($PR_APP/1000000,2); ?></b>/<?php echo number_format($PR_TOTD/1000000,2); ?> (<?php echo number_format($PR_PER); ?>%)
													</span>
												...
												<div class="progress-group">
													...
													<span class="progress-number" title="dalam juta rupiah">
														<b><?php echo number_format($PO_APP/1000000,2); ?></b>/<?php echo number_format($PO_TOTD/1000000,2); ?> (<?php echo number_format($PO_PER); ?>%)
													</span>
													<div class="progress sm">
												...
												<?php
													...
													if($PO_APPP == 0)
														$PO_APPP = 1;
													$IR_PER 	= $IR_APP / $PO_APPP * 100;
												?>
												<div class="progress-group">
													<span class="progress-text"><?php echo $MtrReceipt; ?> (LPM) / OP</span>
													<span class="progress-number" title="dalam juta rupiah">
														<!-- <b><?php echo number_format($IR_APP/1000000,2); ?></b>/<?php echo number_format($IR_TOTD/1000000,2); ?> -->
														<b><?php echo number_format($IR_APP/1000000,2); ?></b>/<?php echo number_format($PO_APP/1000000,2); ?> (<?php echo number_format($IR_PER); ?>%)
													</span>
													<div class="progress sm">
												...
												<div class="progress-group">
													...
													<span class="progress-number" title="dalam juta rupiah">
														<b><?php echo number_format($WO_APP/1000000,2); ?></b>/<?php echo number_format($WO_TOTD/1000000,2); ?> (<?php echo number_format($WO_PER); ?>%)
													</span>
													<div class="progress sm">
												...
												<?php
													...
													if($WO_APPP == 0)
														$WO_APPP = 1;
													$OPN_PER 	= $OPN_APP / $WO_APPP * 100;
												?>
												<div class="progress-group">
													<span class="progress-text"><?php echo $JobOpname; ?> (OPN) / SPK</span>
													<span class="progress-number" title="dalam juta rupiah">
														<b><?php echo number_format($OPN_APP/1000000,2); ?></b>/<?php echo number_format($WO_APP/1000000,2); ?> (<?php echo number_format($OPN_PER); ?>%)
													</span>
													<div class="progress sm">
	...
	// AREA CHART
		...
		<?php if($GRF_PURCH == 1 || $GRF_SALES == 1) {
			...
			$sql_01 = "SELECT SUM(PR_VAL-PR_VAL_M) AS TOT_PR, SUM(PO_VAL-PO_VAL_M) AS TOT_PO, SUM(IR_VAL-IR_VAL_M) AS TOT_IR, SUM(UM_VAL-UM_VAL_M) AS TOT_UM,
							SUM(WO_VAL-WO_VAL_M) AS TOT_WO, SUM(OPN_VAL-OPN_VAL_M) AS TOT_OPN, SUM(0) AS TOT_SO, SUM(0) AS TOT_JO, SUM(0) AS TOT_PRD
