...
/* 
	...
	* File Name	= v_saleinv_inb_form.php
	* Location		= -
*/
...
<html>
	...
	<?php
		...
		if($LangID == 'IND')
		{
			...
			$alert6		= "Isikan catatan persetujuan dokumen.";
		...
		else
		{
			...
			$alert6		= "Please input approval notes.";
							...
							<div class="box-body">
		                        ...
		                        <?php if($SINV_ADDRESS != '') { ?>
		                            <div class="form-group">
		                                ...
		                                <div class="col-sm-9">
		                                    <textarea name="SINV_ADDRESS" id="SINV_ADDRESS" cols="50" class="form-control"  style="height:85px;"><?php echo $SINV_ADDRESS; ?></textarea>
		                                </div>
		                            </div>
		                        <?php }
						...
						<div class="box box-primary">
							...
							<div class="box-body">
		                        ...
		                        <div class="form-group">
		                          	...
		                          	<div class="col-sm-9">
		                                <textarea class="form-control" name="SINV_NOTES"  id="SINV_NOTES" style="height:60px" disabled><?php echo $SINV_NOTES; ?></textarea>
		                          	</div>
		                        </div>
		                        <div class="form-group">
		                       	  	<label for="inputName" class="col-sm-3 control-label"><?php echo $ApproverNotes; ?></label>
		                          	<div class="col-sm-9">
		                                <textarea class="form-control" name="SINV_NOTES2"  id="SINV_NOTES2" style="height:60px"><?php echo $SINV_NOTES2; ?></textarea>
		                          	</div>
		                        </div>
		                        <div class="form-group">
										...
										<?php
											// START : FOR ALL APPROVAL FUNCTION
												if($disableAll == 0)
												{
													...
													{
														$disButton	= 0;
														$sqlCAPPHE	= "tbl_approve_hist WHERE AH_CODE = '$SINV_NUM' AND AH_APPROVER = '$DefEmp_ID'";
														$resCAPPHE	= $this->db->count_all($sqlCAPPHE);
														if($resCAPPHE > 0)
															$disButton	= 1;
														?>
															<select name="SINV_STAT" id="SINV_STAT" class="form-control select2" <?php if($disButton == 1) { ?> disabled <?php } ?> >
																<option value="0"> --- </option>
																<option value="3"<?php if($SINV_STAT == 3) { ?> selected <?php } ?>>Approved</option>
																<option value="4"<?php if($SINV_STAT == 4) { ?> selected <?php } ?>>Revised</option>
																<option value="5"<?php if($SINV_STAT == 5) { ?> selected <?php } ?>>Rejected</option>
																<!-- <option value="6"<?php if($SINV_STAT == 6) { ?> selected <?php } ?>>Closed</option> -->
																<option value="7"<?php if($SINV_STAT == 7) { ?> selected <?php } ?> disabled>Awaiting</option>
															</select>
														<?php
													}
													else
                                        ...
                                        if($task == 'edit')
                                        {
                                            ...
                                            if($resSNHC > 0)
                                            {
                                                ...
                                                foreach($resSNH as $row) :
													...
													$ITM_NM			= '';
													$ITM_ACCSAL 	= '';
													$sqlITMNM 		= "SELECT ITM_NAME, ACC_ID_SAL FROM tbl_item WHERE ITM_CODE = '$ITM_CODE'";
													$resITMNM 		= $this->db->query($sqlITMNM)->result();
													foreach($resITMNM as $rowITMNM) :														
														$ITM_NM		= $rowITMNM->ITM_NAME;													
														$ITM_ACCSAL	= $rowITMNM->ACC_ID_SAL;
													endforeach;
													...
													if($ITM_ACCSAL == '')
													{
														$ACC_SAL	= "<br><span class='btn btn-danger btn-xs' style='font-size:12px'>$ITM_CODE</span>";
														$hidBut		= 1;
													}

													$TOT_AMN	= number_format(0, 2); 
													$TOT_AMNV	= number_format(0, 2); 
													$sqlITMD 	= "SELECT SUM(QRC_VOLM) AS TOT_VOLM, SUM(QRC_VOLM * QRC_PRICE) AS TOT_AMN, ITM_UNIT
													...
													foreach($resITMD as $rowITMD) :
														...
														$TOT_AMNV	= number_format($rowITMD->TOT_AMN, 2);
													...
													// CEK SN PRICE
														$SN_PRICE 	= 0;
														$sqlSNPRC 	= "SELECT SN_PRICE FROM tbl_sn_detail WHERE SN_NUM = '$SN_NUM' AND ITM_CODE = '$ITM_CODE'";
                    ...
                    <div class="form-group">
                        <div class="col-sm-offset-2 col-sm-9">
                        	<?php
								if($disableAll == 0)
								{
									if($hidBut == 0 && (($SINV_STAT == 2 || $SINV_STAT == 7) && ($ISAPPROVE == 1 && $canApprove == 1)))
									{
										?>
								...
								$backURL	= site_url('c_sales/c_s4l3InV/gls4l3Inv1nB/?id='.$this->url_encryption_helper->encode_url($PRJCODE));
...
<script>
  $(function () {
	...
	function checkInp()
	{
		...
		SINV_STAT	= document.getElementById('SINV_STAT').value;
