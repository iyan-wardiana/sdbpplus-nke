<?php
	...
	$viewTaskList		= site_url('c_help/c_t180c2hr');
...
<script>
	// Count Task
		...
		function sendALERT()
		{
			...
			minutNow 	= new Date().getMinutes();
			secondNow 	= new Date().getSeconds();

			if(hoursNow == 16 && minutNow == 0)
			{
				var EMPID 		= "<?=$Emp_ID?>";
				...
				$.ajax({
		            type: 'POST',
		            url: "<?php echo site_url('__180c2f/sendALERT')?>",
		            data: formData,
		            success: function(response)
		            {
		            	console.log(hoursNow+' : '+minutNow)
		            }
		        });
			}
		}
					...
					<!-- Tasks: style can be found in dropdown.less -->
                        ...
                        <li class="dropdown messages-menu">
                            ...
                            <ul class="dropdown-menu">
                                        ...
                                        <ul class="menu">
                                            <?php
                                                ...
                                                foreach($resCHATH as $rowCHATH) :
                                                    ...
                                                    ?>
                                                    <li><!-- start message -->
													<a href="#" onclick="addTab_task('<?=$Read_Task?>')">
                                                            <div class="pull-left">
                                ...
                                <li class="footer"><a href="#" onclick="addTab_task('<?=$viewTaskList?>')">See All Task</a></li>
                            </ul>
                        </li>
...
<?php
	$getTaskReq	= "SELECT menu_code, menu_name_{$LangID} AS menu_name FROM tbl_menu WHERE menu_code = 'MN300'";
	$mnTask		= $this->db->query($getTaskReq)->row();
?>
<script>
	function addTab_task(urlTaskReq)
	{
		addTabs({
			id: '<?php echo $mnTask->menu_code;?>',
			title: '<?php echo $mnTask->menu_name?>',
			close: true,
			url: urlTaskReq,
