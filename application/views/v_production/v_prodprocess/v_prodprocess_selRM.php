	...
	if(isset($_POST['submit1']))
	{
		...
		if($List_Type == 1)
		{
			...
			$sqlDataItm		= "SELECT PRJCODE, ITM_CODE, ITM_GROUP, ITM_CATEG, ITM_NAME, ITM_UNIT, ITM_VOLM, ITM_PRICE,
								...
								FROM tbl_item WHERE PRJCODE = '$PRJCODE' AND (ISRM = 1 OR ISWIP = 1) AND STATUS = 1 A";
	...
	else
	{
		...
		$cDataItm		= $this->db->count_all("tbl_item A 
							INNER JOIN tbl_jo_stfdetail B ON A.ITM_CODE = B.ITM_CODE
								AND B.PRJCODE = '$PRJCODE' AND B.JOSTF_TYPE = 'IN' AND B.JOSTF_STEP = '$STFFROM'
							WHERE A.PRJCODE = '$PRJCODE' AND (A.ISWIP = 1 OR A.ISFG = 1 OR A.ISRM = 1) AND A.STATUS = 1");		
                                                ...
                                                <?php
                                                    ...
                                                    if($cDataItm>0)
													{
														...
														foreach($vDataItm as $row) :								
															...
															?>
															<td style="text-align:center"><input type="checkbox" name="chk" value="<?php echo $PRJCODE;?>|<?php echo $ITM_CODE;?>|<?php echo $ITM_GROUP;?>|<?php echo $ITM_NAME;?>|<?php echo $ITM_UNIT;?>|<?php echo $ITM_VOLM;?>|<?php echo $REM_QTY;?>|<?php echo $ITM_PRICE;?>|<?php echo $ACC_ID;?>|<?php echo $ACC_ID_UM;?>|<?php echo $ITM_STOCK;?>" onClick="pickThis(this);" /></td>
