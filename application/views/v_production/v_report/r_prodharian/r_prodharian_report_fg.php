...
/*
	...
	* File Name		= r_prodharian_report_fg.php
	* Location		= -
 */
...
if($viewType == 1)
{
	...
	header("Content-Disposition: attachment; filename=LapProdFGHarian_$dateRep.xls");
...
// Menentukan Minggu Laporan
	...
	$ADDQRY2 		= "";
	if($PRJCODE != "All")
		$ADDQRY1	= "A.Proj_Code = '$PRJCODE' AND";
	...
	<div class="page">
		...
		<table border="1" width="100%">
				...
				for($i=1;$i<=$maxRow;$i++)
				{
					if($i<=$totRow)
					{
						if($i==$totRow)							// Baris terakhir
						{
							...
							// MUTASI PENERIMAAN DIDAPATKAN DARI BARANG JADI COMPAKTOR, YAITU MESIN TERAKHIR SEBELUM PACKING
								$TOTFG_A1 	= 0;
								$sqlFG_A1 	= "SELECT SUM(QtyProd_Plus + QtySN_Plus) AS TOTFG_A1
												FROM tbl_itemhistory A
												WHERE $ADDQRY1 A.Transaction_Date = '$fDtWL'";
							...
							// MUTASI PENGELUARAN SELALU KOOSNG 
								$TOTFG_A1 	= 0;
								$sqlFG_A1 	= "SELECT SUM(QtySN_Min) AS TOTFG_A1
												FROM tbl_itemhistory A
												WHERE $ADDQRY1 A.Transaction_Date = '$fDtWL'";
								$resFG_A1 	= $this->db->query($sqlFG_A1)->result();
								foreach($resFG_A1 as $rowFG_A1) :
						...
						elseif($i==1)
						{
							...
							// SALDO AWAL - DIDAPATKAN DARI SALDO AKHIR MINGGU SEBELUMNYA
								$TOTFG_A1 	= 0;
								$sqlFG_A1 	= "SELECT SUM(QtyProd_Plus - QtyProd_Min + QtySN_Plus - QtySN_Min) AS TOTFG_A1
												FROM tbl_itemhistory A
												WHERE $ADDQRY1 A.Transaction_Date < '$fWDtS'";
								$resFG_A1 	= $this->db->query($sqlFG_A1)->result();
								foreach($resFG_A1 as $rowFG_A1) :
									$TOTFG_A1	= $rowFG_A1->TOTFG_A1;
								endforeach;
							...
							// MUTASI PENERIMAAN DIDAPATKAN DARI BARANG JADI HASIL PRODUKSI
								$TOTFG_A1 	= 0;
								$sqlFG_A1 	= "SELECT SUM(QtyProd_Plus + QtySN_Plus) AS TOTFG_A1
												FROM tbl_itemhistory A
												WHERE $ADDQRY1 (A.Transaction_Date >= '$fWDtS' AND A.Transaction_Date <= '$eDtWL')";
								$resFG_A1 	= $this->db->query($sqlFG_A1)->result();
								foreach($resFG_A1 as $rowFG_A1) :
									$TOTFG_A1	= $rowFG_A1->TOTFG_A1;
								endforeach;
							...
							// MUTASI PENGELUARAN DIDAPATKAN DARI PENGIRIMAN FG
								$TOTFG_A1 	= 0;
								$sqlFG_A1 	= "SELECT SUM(QtySN_Min) AS TOTFG_A1
												FROM tbl_itemhistory A
												WHERE $ADDQRY1 (A.Transaction_Date >= '$fWDtS' AND A.Transaction_Date <= '$eDtWL')";
						...
						else
						{
							...
							// MUTASI PENERIMAAN DIDAPATKAN DARI BARANG JADI HASIL PRODUKSI
								...
								$sqlFG_A1 	= "SELECT SUM(QtyProd_Plus + QtySN_Plus) AS TOTFG_A1
												FROM tbl_itemhistory A
												WHERE $ADDQRY1 (A.Transaction_Date >= '$fDtWL' AND A.Transaction_Date <= '$eDtWL')";
								$resFG_A1 	= $this->db->query($sqlFG_A1)->result();
								foreach($resFG_A1 as $rowFG_A1) :
									$TOTFG_A1	= $rowFG_A1->TOTFG_A1;
								endforeach;
		            	...
		            	?>
							<tr style="line-height: 25px;">
			                    ...
			                    <td style="text-align:left; vertical-align: middle;" nowrap>Barang Jadi</td>
			...
			<tr style="font-weight: bold;">
                <td style="text-align:center; vertical-align: middle;" colspan="4" rowspan="2" nowrap>Rata2 FG Out <?php echo "$dt $MONTHD $YEAR"; ?></td>
                <td style="text-align:center; vertical-align: middle;" nowrap>Total FG Out</td>
        ...
        <table width="100%" border="1" rules="all">
            ...
            <tr>
                <td style="text-align: left;">ADM GUDANG FG</td>
