...
/*
	...
	* Create Date	= 11 September 2020
	* File Name		= r_prodharian_report_returso.php
	* Location		= -
 */
...
if($viewType == 1)
{
	...
	header("Content-Disposition: attachment; filename=LapReturHarian_$dateRep.xls");
	...
	<div class="page">
		...
		<table border="1" width="100%">
				...
				for($i=1;$i<=$maxRow;$i++)
				{
					if($i<=$totRow)
					{
						if($i==$totRow)							// Baris terakhir
						{
							...
							// MUTASI PENERIMAAN DIDAPATKAN DARI RETUR COMPAKTOR, YAITU MESIN TERAKHIR SEBELUM PACKING
								$TOTRET_A1 	= 0;
								$sqlRET_A1 	= "SELECT SUM(A.ITM_QTY) AS TOTRET_A1
												FROM tbl_ret_detail A
													INNER JOIN tbl_ret_header B ON A.RET_NUM = B.RET_NUM
												WHERE $ADDQRY1 B.RET_STAT IN (3,6) AND A.RET_DATE = '$fDtWL'";
								$resRET_A1 	= $this->db->query($sqlRET_A1)->result();
								foreach($resRET_A1 as $rowRET_A1) :
									$TOTRET_A1	= $rowRET_A1->TOTRET_A1;
								endforeach;
								$tot_in		= $TOTRET_A1 ?: 0;
						...
						elseif($i==1)
						{
							...
							// MUTASI PENERIMAAN DIDAPATKAN DARI RETUR CUSTOMER
								$TOTRET_A1 	= 0;
								$sqlRET_A1 	= "SELECT SUM(A.ITM_QTY) AS TOTRET_A1
												FROM tbl_ret_detail A
													INNER JOIN tbl_ret_header B ON A.RET_NUM = B.RET_NUM
												WHERE $ADDQRY1 B.RET_STAT IN (3,6)
													AND (A.RET_DATE >= '$fWDtS' AND A.RET_DATE <= '$eDtWL')";
								$resRET_A1 	= $this->db->query($sqlRET_A1)->result();
								foreach($resRET_A1 as $rowRET_A1) :
									$TOTRET_A1	= $rowRET_A1->TOTRET_A1;
								endforeach;
							...
							// MUTASI PENERIMAAN DIDAPATKAN DARI RETUR COMPAKTOR, YAITU MESIN TERAKHIR SEBELUM PACKING
								...
								$sqlRET_A1 	= "SELECT SUM(A.ITM_QTY) AS TOTRET_A1
												FROM tbl_ret_detail A
													INNER JOIN tbl_ret_header B ON A.RET_NUM = B.RET_NUM
												WHERE $ADDQRY1 B.RET_STAT IN (3,6)
													AND (A.RET_DATE >= '$fDtWL' AND A.RET_DATE <= '$eDtWL')";
								$resRET_A1 	= $this->db->query($sqlRET_A1)->result();
								foreach($resRET_A1 as $rowRET_A1) :
									$TOTRET_A1	= $rowRET_A1->TOTRET_A1;
								endforeach;
		            	...
		            	?>
							<tr style="line-height: 25px;">
			                    ...
			                    <td style="text-align:left; vertical-align: middle;" nowrap>Retur</td>
			...
			<tr style="font-weight: bold;">
                <td style="text-align:center; vertical-align: middle;" colspan="4" rowspan="2" nowrap>Rata2 Retur <?php echo "$dt $MONTHD $YEAR"; ?></td>
                <td style="text-align:center; vertical-align: middle;" nowrap>Total Retur</td>
			...
			<tr style="font-weight: bold;">
                <td style="text-align:center; vertical-align: middle;" colspan="4" rowspan="2" nowrap>Persentase Retur <?php echo "$dt $MONTHD $YEAR"; ?></td>
                <td style="text-align:center; vertical-align: middle;" nowrap>Total Retur</td>
                <td style="text-align:center; vertical-align: middle;" nowrap>Total Kirim</td>
                <td style="text-align:center; vertical-align: middle;" nowrap>Rata2/hari</td>
			</tr>
			<?php
				...
				$TOTSN_A1 	= 0;
				$sqlSN_A1 	= "SELECT SUM(A.SN_VOLM) AS TOTSN_A1
								FROM tbl_sn_detail A
									INNER JOIN tbl_sn_header B ON A.SN_NUM = B.SN_NUM
								WHERE $ADDQRY1 B.SN_STAT IN (3,6)
									AND (A.SN_DATE >= '$dateS' AND A.SN_DATE <= '$dateE')";
				$resSN_A1 	= $this->db->query($sqlSN_A1)->result();
				foreach($resSN_A1 as $rowSN_A1) :
					$TOTSN_A1	= $rowSN_A1->TOTSN_A1;
				endforeach;
				$tot_sn		= $TOTSN_A1 ?: 0;
				$tot_snP	= $TOTSN_A1 ?: 1;
				$avgRetP	= $bal_e / $tot_snP * 100;
			?>
			<tr>
                <td style="text-align:right; vertical-align: middle;" nowrap><?=number_format($bal_e,2)?></td>
                <td style="text-align:right; vertical-align: middle;" nowrap><?=number_format($tot_sn,0)?></td>
                <td style="text-align:right; vertical-align: middle;" nowrap><?=number_format($avgRetP,2)?></td>
			</tr>
		</table>
		<br>
