...
/*
	...
	* Create Date	= 13 September 2020
	* File Name		= r_prodharian_report_greige.php
	* Location		= -
 */
...
if($viewType == 1)
{
	...
	header("Content-Disposition: attachment; filename=LapGreigeHarian_$dateRep.xls");
	...
	<div class="page">
		...
		<table border="1" width="100%">
					...
					if($i<=$totRow)
					{
						if($i==$totRow)							// Baris terakhir
						{
							...
							// MUTASI PENERIMAAN DIDAPATKAN DARI RETUR COMPAKTOR, YAITU MESIN TERAKHIR SEBELUM PACKING
								$TOTIRG_A1 	= 0;
								$sqlIRG_A1 	= "SELECT SUM(A.ITM_QTY) AS TOTIRG_A1
												FROM tbl_ir_detail A
													INNER JOIN tbl_ir_header B ON A.IR_NUM = B.IR_NUM
												WHERE $ADDQRY1 B.IR_STAT IN (3,6) AND A.IR_SOURCE = 4
													AND A.IR_DATE = '$fDtWL'";
								$resIRG_A1 	= $this->db->query($sqlIRG_A1)->result();
								foreach($resIRG_A1 as $rowIRG_A1) :
									$TOTIRG_A1	= $rowIRG_A1->TOTIRG_A1;
								endforeach;
								$tot_in		= $TOTIRG_A1 ?: 0;

							// MUTASI PENGELUARAN DARI GREIGE YANG MUTASI DARI GUDANG KE PARTAI KAIN
								$tot_out		= 0;
						...
						elseif($i==1)
						{
							...
							// SALDO AWAL - DIDAPATKAN DARI SALDO AKHIR STOCK GRIGE BULAN SEBELUMNYA
								$TOTIRG_A1 	= 0;
								$sqlIRG_A1 	= "SELECT SUM(A.ITM_QTY) AS TOTIRG_A1
												FROM tbl_ir_detail A
													INNER JOIN tbl_ir_header B ON A.IR_NUM = B.IR_NUM
												WHERE $ADDQRY1 B.IR_STAT IN (3,6) AND A.IR_SOURCE = 4 AND A.IR_DATE < '$fWDtS'";
								$resIRG_A1 	= $this->db->query($sqlIRG_A1)->result();
								foreach($resIRG_A1 as $rowIRG_A1) :
									$TOTIRG_A1	= $rowIRG_A1->TOTIRG_A1;
								endforeach;
							...
							// MUTASI PENERIMAAN DIDAPATKAN DARI GRIGE CUSTOMER
								$TOTIRG_A1 	= 0;
								$sqlIRG_A1 	= "SELECT SUM(A.ITM_QTY) AS TOTIRG_A1
												FROM tbl_ir_detail A
													INNER JOIN tbl_ir_header B ON A.IR_NUM = B.IR_NUM
												WHERE $ADDQRY1 B.IR_STAT IN (3,6) AND A.IR_SOURCE = 4
													AND (A.IR_DATE >= '$fWDtS' AND A.IR_DATE <= '$eDtWL')";
								$resIRG_A1 	= $this->db->query($sqlIRG_A1)->result();
								foreach($resIRG_A1 as $rowIRG_A1) :
									$TOTIRG_A1	= $rowIRG_A1->TOTIRG_A1;
								endforeach;
							...
							// MUTASI PENERIMAAN DIDAPATKAN DARI RETUR COMPAKTOR, YAITU MESIN TERAKHIR SEBELUM PACKING
								...
								$sqlIRG_A1 	= "SELECT SUM(A.ITM_QTY) AS TOTIRG_A1
												FROM tbl_ir_detail A
													INNER JOIN tbl_ir_header B ON A.IR_NUM = B.IR_NUM
												WHERE $ADDQRY1 B.IR_STAT IN (3,6) AND A.IR_SOURCE = 4
													AND (A.IR_DATE >= '$fDtWL' AND A.IR_DATE <= '$eDtWL')";
								$resIRG_A1 	= $this->db->query($sqlIRG_A1)->result();
								foreach($resIRG_A1 as $rowIRG_A1) :
									$TOTIRG_A1	= $rowIRG_A1->TOTIRG_A1;
								endforeach;
		            	...
		            	?>
							<tr style="line-height: 25px;">
			                    ...
			                    <td style="text-align:left; vertical-align: middle;" nowrap>Gudang Greige</td>
			...
			<tr style="font-weight: bold;">
                <td style="text-align:center; vertical-align: middle;" colspan="4" rowspan="2" nowrap>Rata2 Greige <?php echo "$dt $MONTHD $YEAR"; ?></td>
                <td style="text-align:center; vertical-align: middle;" nowrap>Total Greige Masuk</td>
        ...
        <table width="100%" border="1" rules="all">
            ...
            <tr>
                <td style="text-align: left;">ADM Gd. GREIGE</td>
