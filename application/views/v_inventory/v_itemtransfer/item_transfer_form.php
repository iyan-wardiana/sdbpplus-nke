...
/* 
	...
	* File Name	= item_transfer_form.php
	* Location		= -
*/
...
{	
	...
	$myCount = $this->db->count_all('tbl_item_tsfh');
	...
	$ITMTSF_NUM		= "$DocNumber";
		$ITMTSFCODE	= $lastPatternNumb;
		$ITMTSFYEAR	= date('y');
		$ITMTSFMONTH= date('m');
	$ITMTSF_CODE	= "$Pattern_Code.$ITMTSFCODE.$ITMTSFYEAR.$ITMTSFMONTH"; // MANUAL CODE	
	
	$ITMTSF_DATE	= date('m/d/Y');
	$ITMTSF_SENDD	= date('m/d/Y');
	...
	$dataColl 		= "$PRJCODE~$Pattern_Code~tbl_item_tsfh~$Pattern_Length";
	$dataTarget		= "ITMTSF_CODE";
}
else
{	
	$isSetDocNo 		= 1;
...
<html>
	...
	<?php
		...
		if($LangID == 'IND')
		{
			...
			$alert2		= "Pilih salah satu material.";
			$alert3		= "Silahkan pilih status persetujuan.";
			$alert4		= "Ada jumlah yang ditransfer lebih besar dari stok. Jadikan jumlah pengiriman sesuai dengan stok. Setuju?";
			...
			$alert9		= "Gudang tujuan dengan gudang awal tidak boleh sama.";
			$alert10	= "Silahkan masukan alasan mengapa Anda membatalkan dokumen ini.";
		...
		else
		{
			...
			$alert2		= "Please select material.";
			$alert3		= "Please select an approval status.";
			$alert4		= "The are Qty transferred is greater than the Qty Stock. Make that qty is same with stock qty. Agree?";
			...
			$alert9		= "Destin. warehouse and origin warehouse can not be the same.";
			$alert10	= "Please enter the reason why you void this document.";
							...
							<div class="box-body">
				                ...
				                <div class="form-group">
				                    ...
				                    <div class="col-sm-9">
				                    	...
				                    	<input type="text" class="form-control" name="ITMTSF_CODEX" id="ITMTSF_CODEX" value="<?php echo $ITMTSF_CODE; ?>" disabled >
				                    </div>
				                </div>
				                <div class="form-group" style="display:none">
				                    <label for="inputName" class="col-sm-3 control-label"><?php echo $TsfNo; ?> </label>
				            	...
				            	<div class="form-group">
				                    ...
				                    <div class="col-sm-9">
				                        ...
				                        <i class="fa fa-calendar"></i>&nbsp;</div><input type="text" name="ITMTSF_DATE" class="form-control pull-left" id="datepicker" value="<?php echo $ITMTSF_DATE; ?>" style="width:106px"></div>
				                    </div>
				                </div>
				            	<div class="form-group">
				                    ...
				                    <div class="col-sm-9">
				                    	<div class="input-group date">
				                        <div class="input-group-addon">
				                        <i class="fa fa-calendar"></i>&nbsp;</div><input type="text" name="ITMTSF_SENDD" class="form-control pull-left" id="datepicker1" value="<?php echo $ITMTSF_SENDD; ?>" style="width:106px"></div>
				                    </div>
				                </div>
				            	<div class="form-group" style="display:none">
				                ...
				                <div class="form-group">
				                    ...
				                    <div class="col-sm-9">
				                        <div class="input-group">
				                            ...
				                            <input type="text" class="form-control" name="ITMTSF_REFNOX" id="ITMTSF_REFNOX" value="<?php echo $ITMTSF_REFNO; ?>" onClick="getREFNO();" <?php if($ITMTSF_STAT != 1 && $ITMTSF_STAT != 4) { ?> disabled <?php } ?>>
				                        </div>
				                    </div>
				                </div>
				                <?php
				            	...
				            	<div class="form-group">
				                    ...
				                    <div class="col-sm-9">
				                        <select name="ITMTSF_ORIGIN1" id="ITMTSF_ORIGIN1" class="form-control select2" onChange="chgORIG(this.value)">
				                        	<option value=""> --- </option>
											<?php
				                ...
				                <script>
									function chgORIG(thisVal)
									{
										...
										var ORIGIN		= thisVal;
										var DESTIN		= document.getElementById('ITMTSF_DEST').value;
										
										if(ORIGIN == DESTIN)
										{
											swal('<?php echo $alert9; ?>');
											document.getElementById('ITMTSF_ORIGIN1').value = '';
											return false;
										}
									}
								</script>
				            	<div class="form-group">
				                    ...
				                    <div class="col-sm-9">
				                        <select name="ITMTSF_DEST1" id="ITMTSF_DEST1" class="form-control select2" onChange="chgDEST(this.value)">
				                        	<option value=""> --- </option>
											<?php
												$sqlWHC	= "tbl_warehouse WHERE PRJCODE = '$PRJCODE_HO'";
												$resWHC	= $this->db->count_all($sqlWHC);
				                                if($resWHC > 0)
				                                {
				                                    ...
				                                    foreach($resWH as $rowWH) :
				                                        ...
				                                        $WH_NAME 	= $rowWH->WH_NAME;
				                                        $WH_LOC		= $rowWH->WH_LOC;
				                                        $PRJCODEX	= $rowWH->PRJCODE;
				                                        $PRJNAME	= $rowWH->PRJNAME;
														$WHCOLL		= "$PRJCODEX~$WH_CODE";
														$TSF_DESTX	= "$PRJCODEX~$ITMTSF_DEST";
				                                        ?>
				                                        <option value="<?php echo $WHCOLL; ?>" <?php if($WH_CODE == $ITMTSF_DEST) { ?> selected <?php } ?>><?php echo "$WH_NAME - $PRJNAME"; ?></option>
				                            			<?php
				                                    endforeach;
				                                }
				                            ?>
				                ...
				                <script>
									...
									{
										varSplit	= thisVal.split('~');
										document.getElementById('PRJCODE_DEST').value	= varSplit[0];
										document.getElementById('ITMTSF_DEST').value 	= varSplit[1];
										
										var ORIGIN		= document.getElementById('ITMTSF_ORIGIN1').value;
										var DESTIN		= varSplit[1];
										
										if(ORIGIN == DESTIN)
										{
											swal('<?php echo $alert9; ?>');
											document.getElementById('ITMTSF_DEST1').value = '';
											return false;
										}
									}
								</script>
						...
						<div class="box box-primary">
							...
							<div class="box-body">
				                ...
				                <div class="form-group">
				                    ...
				                    <div class="col-sm-9">
				                    	<input type="text" class="form-control" name="ITMTSF_SENDER" id="ITMTSF_SENDER" value="<?php echo $ITMTSF_SENDER; ?>" placeholder="<?php echo $Sender; ?>" >
				                    </div>
				                </div>
				                <?php
									if($Receiver != '')
									{
				                        ...
				                        <div class="form-group">
				                            ...
				                            <div class="col-sm-9">
				                                <input type="text" class="form-control" name="ITMTSF_RECEIVER1" id="ITMTSF_RECEIVER1" value="<?php echo $ITMTSF_RECEIVER; ?>" placeholder="<?php echo $Receiver; ?>" disabled >
				                                <input type="hidden" class="form-control" style="max-width:350px" name="ITMTSF_RECEIVER" id="ITMTSF_RECEIVER" value="<?php echo $ITMTSF_RECEIVER; ?>" placeholder="<?php echo $Receiver; ?>" >
				                            </div>
				                        </div>
				                        <div class="form-group" style="display: none;">
				                            ...
				                            <div class="col-sm-9">
				                                <div class="input-group date">
				                                <div class="input-group-addon">
				                                <i class="fa fa-calendar"></i>&nbsp;</div><input type="text" name="ITMTSF_RECD1" class="form-control pull-left" id="datepicker2" value="<?php echo $ITMTSF_RECD; ?>" style="width:106px" disabled >
				                                <input type="hidden" name="ITMTSF_RECD" class="form-control pull-left" id="datepicker2" value="<?php echo $ITMTSF_RECD; ?>" style="width:106px" ></div>
				                            </div>
				                        </div>
				                		<?php
									}
				                ...
				                <div class="form-group">
				                    ...
				                    <div class="col-sm-9">
				                        <textarea name="ITMTSF_NOTE" class="form-control" id="ITMTSF_NOTE" cols="30" style="height: 60px"><?php echo $ITMTSF_NOTE; ?></textarea>                        
				                    </div>
				                </div>
								<div class="form-group">
									<label for="inputName" class="col-sm-3 control-label"><?php echo $OthNotes; ?></label>
									<div class="col-sm-9">
										<textarea name="ITMTSF_NOTE2" class="form-control" id="ITMTSF_NOTE2" style="height: 60px" disabled><?php echo $ITMTSF_NOTE2; ?></textarea>                        
									</div>
								</div>
				            	<div id="revMemo" class="form-group" <?php if($ITMTSF_REVMEMO == '') { ?> style="display:none" <?php } ?>>
				                    ...
				                    <div class="col-sm-9">
				                    	<textarea name="ITMTSF_REVMEMO" class="form-control" id="ITMTSF_REVMEMO" cols="30" disabled><?php echo $ITMTSF_REVMEMO; ?></textarea>                        
				                    </div>
				                </div>
				            	<div class="form-group">
				                    ...
				                    <div class="col-sm-9">
										...
										<?php
											// START : FOR ALL APPROVAL FUNCTION
												...
												?>
													<select name="ITMTSF_STAT" id="ITMTSF_STAT" class="form-control select2" onChange="chkSTAT(this.value)">
														<?php
														$disableBtn	= 0;
														if($ITMTSF_STAT == 5 || $ITMTSF_STAT == 6 || $ITMTSF_STAT == 9)
														{
															$disableBtn	= 1;
														}
														if($ITMTSF_STAT != 1 AND $ITMTSF_STAT != 4) 
														{
															?>
																<option value="1"<?php if($ITMTSF_STAT == 1) { ?> selected <?php } ?> disabled>New</option>
																<option value="2"<?php if($ITMTSF_STAT == 2) { ?> selected <?php } ?> disabled>Confirm</option>
																<option value="3"<?php if($ITMTSF_STAT == 3) { ?> selected <?php } ?> disabled>Approve</option>
																<option value="4"<?php if($ITMTSF_STAT == 4) { ?> selected <?php } ?> disabled>Revising</option>
																<option value="5"<?php if($ITMTSF_STAT == 5) { ?> selected <?php } ?> disabled>Rejected</option>
																<option value="6"<?php if($ITMTSF_STAT == 6) { ?> selected <?php } if($disableBtn == 1) {?> disabled <?php } ?>>Closed</option>
																<option value="7"<?php if($ITMTSF_STAT == 7) { ?> selected <?php } ?> disabled>Waiting</option>
																<option value="9"<?php if($ITMTSF_STAT == 9) { ?> selected <?php } if($disableBtn == 1) {?> disabled <?php } ?>>Void</option>
															<?php
														}
														else
														{
															?>
																<option value="1"<?php if($ITMTSF_STAT == 1) { ?> selected <?php } ?>>New</option>
																<option value="2"<?php if($ITMTSF_STAT == 2) { ?> selected <?php } ?>>Confirm</option>
															<?php
														}
														?>
													</select>
				                ...
				                <script type="text/javascript">
			                		function chkSTAT(selSTAT)
									{
										if(selSTAT == 9)
										{
											document.getElementById("ITMTSF_NOTE2").disabled = false;
		                            ...
		                            if($task == 'edit')
		                            {
										...
										/*	if ($j==1) {
											...
											<td style="text-align: right;" nowrap>
												<?php if($ITMTSF_STAT == 1 || $ITMTSF_STAT == 4) { ?>
			                                    	<input type="text" name="ITMTSF_VOLM<?php echo $currentRow; ?>" id="ITMTSF_VOLM<?php echo $currentRow; ?>" value="<?php echo number_format($ITMTSF_VOLM, 2); ?>" class="form-control" style="max-width:100px; text-align:right" onKeyPress="return isIntOnlyNew(event);" onBlur="getConvertion(this,<?php echo $currentRow; ?>);" size="10" >
			                                    <?php } else {
			                                    	echo number_format($ITMTSF_VOLM, 2);
			                                    }
			                                    ?>
										  	...
										  	<td style="text-align:left">
												<?php if($ITMTSF_STAT == 1 || $ITMTSF_STAT == 4) { ?>
		                                        	<input type="text" name="data[<?php echo $currentRow; ?>][ITMTSF_DESC]" id="data<?php echo $currentRow; ?>ITMTSF_DESC" value="<?php echo $ITMTSF_DESC; ?>" class="form-control" >
			                                    <?php } else {
			                                    	echo $ITMTSF_DESC;
			                                    	?>
	                    ...
	                    <div class="form-group">
		                    ...
		                    <div class="col-sm-9">
		                        <?php
									...
									{
										if($ISCREATE == 1 && ($ITMTSF_STAT == 1 || $ITMTSF_STAT == 4))
										{
											?>
												<button class="btn btn-primary">
												<i class="fa fa-save"></i>
...
<script>
	...
	function getConvertion(thisVal1, row)
	{
		...
		if(ITMTSF_VOLM > ITM_STOCK)
		{
			swal('<?php echo $alert8; ?>',
	            ...
	            {
	                swal.close();
	                $('#ITMTSF_VOLM'+row).val = '0';
	                $('#ITMTSF_VOLM'+row).focus();
	            });
	...
	function checkInData(value)
	{
		...
		var totrow 		= document.getElementById('totalrow').value;
		var ORIGIN		= document.getElementById('ITMTSF_ORIGIN1').value;
		var DESTIN		= document.getElementById('ITMTSF_DEST1').value;
		var TSF_STAT	= document.getElementById('ITMTSF_STAT').value;
		
		if(TSF_STAT == 9)
		{
			var NOTES	= document.getElementById('ITMTSF_NOTE2').value;
			if(NOTES == '')
			{
				...
				.then(function()
				{
					document.getElementById('ITMTSF_NOTE2').focus();
				});
				return false;
			}
		}
		else
		{
			...
			for(i=1;i<=totrow;i++)
			{
				//var ITMTSF_VOLM = parseFloat(document.getElementById('ITMTSF_VOLM'+i).value);
				var ITM_STOC 	= parseFloat(document.getElementById('data'+i+'ITM_STOCK').value);
				var ITMTSF_VOLM = parseFloat(document.getElementById('data'+i+'ITMTSF_VOLM').value);
				var ITMTSFVOLM1 = document.getElementById('ITMTSF_VOLM'+i);
				var ITMTSFVOLM	= parseFloat(eval(ITMTSFVOLM1).value.split(",").join(""));
			...
			if(gtTot > 0)
			{
		        ...
		        .then((yessAgree) => 
		        {
		            if (yessAgree) 
		            {
						...
						{
							var ITM_STOC 	= parseFloat(document.getElementById('data'+i+'ITM_STOCK').value);
			                document.getElementById('data'+i+'ITMTSF_VOLM').value 	= ITM_STOC;
							document.getElementById('ITMTSF_VOLM'+i).value			= doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(ITM_STOC)),decFormat));
						}
		            } 
		            else 
		            {
