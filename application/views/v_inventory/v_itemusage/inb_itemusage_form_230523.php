		        ...
		        <form class="form-horizontal" name="frm" id="frm" method="post" action="<?php echo $form_action; ?>" enctype="multipart/form-data" onSubmit="return validateInData()">
		                            ...
		                            <?php
										...
										foreach($result as $row) :
											...
											// GET ITEM STOCK
												$STOK_VOL	= 0;
												$STOK_VAL	= 0;
												$s_STOCK	= "SELECT SUM(ITM_QTY) AS ITM_STOCK, SUM(ITM_TOTAL) AS ITM_STOCK_VAL FROM tbl_ir_detail A
												...
												foreach($r_STOCK as $rw_STOCK) :
													$STOK_VOL 	= $rw_STOCK->ITM_STOCK;
													$STOK_VAL 	= $rw_STOCK->ITM_STOCK_VAL;
													if($STOK_VOL == '')
														$STOK_VOL	= 0;
													if($STOK_VAL == '')
														$STOK_VAL	= 0;
											...
											// GET RESERVE ITEM (NEW, CONFIRM, APPROVE, CLOSED)
												$UM_VOL		= 0;
												$UM_VAL 	= 0;
												$sqlUM_VOL	= "SELECT SUM(A.ITM_QTY) AS USED_VOL, SUM(A.ITM_TOTAL) AS USED_VAL FROM tbl_um_detail A
																INNER JOIN tbl_um_header B ON A.UM_NUM = B.UM_NUM
																WHERE A.PRJCODE = '$PRJCODE' 
																	AND A.ITM_CODE = '$ITM_CODE'
																	-- AND B.UM_STAT IN (2,7)
																	AND A.UM_NUM != '$UM_NUM'";
												$resUM_VOL	= $this->db->query($sqlUM_VOL)->result();
												foreach($resUM_VOL as $rowUM_VOL) :
													$UM_VOL	= $rowUM_VOL->USED_VOL;
													$UM_VAL	= $rowUM_VOL->USED_VAL;
													if($UM_VOL == '')
														$UM_VOL	= 0;
													if($UM_VAL == '')
														$UM_VAL	= 0;
											...
											// GET RESERVE ITEM IN THIS DOC AND OTEHR ROW
												$UM_VOL2	= 0;
												$s_UM_VOL2	= "SELECT SUM(A.ITM_QTY) AS USED_VOL2 FROM tbl_um_detail A
																INNER JOIN tbl_um_header B ON A.UM_NUM = B.UM_NUM
																WHERE A.PRJCODE = '$PRJCODE' 
																	AND A.ITM_CODE = '$ITM_CODE' 
																	AND A.UM_NUM = '$UM_NUM'
																	AND A.ID != '$UMID'";
												$r_UM_VOL2	= $this->db->query($s_UM_VOL2)->result();
												foreach($r_UM_VOL2 as $rw_UM_VOL2) :
													$UM_VOL2	= $rw_UM_VOL2->USED_VOL2;
													if($UM_VOL2 == '')
														$UM_VOL2	= 0;
												endforeach;
											
											$ITM_STOCK 		= $STOK_VOL - $UM_VOL - $UM_VOL2;
											$ITM_QTY 		= $row->ITM_QTY;

											// AVEGARE PRICE
												$ITMSTOCK 	= $STOK_VOL - $UM_VOL;
												$ITMSTOCKP 	= $ITMSTOCK;
												if($ITMSTOCKP == 0)
													$ITMSTOCKP = 1;
												$PRC_AVG 	= $UM_VAL / $ITMSTOCKP;
												...
												<td width="11%" style="text-align:right"> <!-- Item Stock -->
											 	...
											 	<td width="11%" style="text-align:right"> <!-- Item QTY Use -->
											 		<?php echo number_format($ITM_QTY, 2); ?>
			                                   		...
			                                   		<input type="text" name="data[<?php echo $currentRow; ?>][ITM_PRICE]" id="data<?php echo $currentRow; ?>ITM_PRICE" value="<?php echo $PRC_AVG; ?>" class="form-control" style="max-width:300px;" >
			                                 	</td>
	                ...
	                <div class="col-md-6">
		                <div class="form-group">
		                    ...
		                    <div class="col-sm-9">
		                    	<?php
									if(($UM_STAT == 2 || $UM_STAT == 7) && $ISAPPROVE == 1 && $isDisabled == 0)
									{
										?>
