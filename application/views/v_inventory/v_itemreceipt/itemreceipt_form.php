						...
						<div class="box box-primary">
							...
							<div class="box-body">
		                        ...
		                        <div class="form-group">
		                          	...
		                          	<div class="col-sm-2">
		                                <input type="text" name="UNIC_01" id="UNIC_01" style="font-size: 25px; font-weight: bold; text-align: center;" value="<?php echo $UNIC_01; ?>" class="form-control" readonly>
		                          	</div>
		                          	<div class="col-sm-3" style="padding-left: 0">
		                                <input type="text" name="UNIC_03" id="UNIC_03" style="font-size: 23px; font-weight: bold;" value="<?php echo $UNIC_03; ?>" class="form-control" onKeyUp="validQRR(this.value)" disabled>
		                          	</div>
		                        </div>
		                        <script type="text/javascript">
		                        	function validQRR(theVal)
		                        	{
		                        		var lenQRR 	= theVal.length;
		                        		console.log(theVal+' = '+lenQRR)
		                        		if(lenQRR == 10)
		                        		{
		                        			var UNIC_01		= document.getElementById('UNIC_01').value;
											var UNIC_03		= document.getElementById('UNIC_03').value;
											var QRIT_CODE 	= UNIC_01+'.'+UNIC_03;

											var formData 	= { QRIT_CODE 		: QRIT_CODE };

											$.ajax({
									            type: 'POST',
									            url: "<?php echo site_url('c_qritase/c_qritase/chkQRIT')?>",
									            data: formData,
									            success: function(response)
									            {
									            	var myarr 		= response.split("~");
													var statDESC 	= myarr[0];
													var isRejected 	= myarr[1];
													if(isRejected == 2)
													{
														swal(statDESC,
														{
														...
														.then(function()
											            {
											            	document.getElementById('UNIC_03').value 	= '';
											                swal.close();
											            });
														return false;
													}
		                          	...
		                          	<div class="col-sm-3" style="padding-left: 0px;">
		                                <div class="input-group date">
											...
											<?php
												...
												{
													?>
													<input type="text" name="IR_RECD" class="form-control pull-left" id="datepicker2" value="<?php echo $IR_RECD; ?>">
													<?php
												}
												else
												{
									...
									function rereshfPO()
									{
										...
										$.ajax({
								            ...
								            success: function(response)
								            {
												...
												var dtCurr1x24		= new Date(PO_PLANIR);
												var IR_Cur1x24D 	= ("0" + (dtCurr1x24.getDate() + 1)).slice(-2); // Penerimaan material max. 1x24 jam => upd. 2023-05-31
												var IR_Cur1x24M 	= ("0" + (dtCurr1x24.getMonth() + 1)).slice(-2);
												var IR_Cur1x24Y 	= dtCurr1x24.getFullYear();
												if(dtCurr1x24.getDate() === 31)
												{
													console.log('dtCurr1x24_1 = '+dtCurr1x24.getDate());
													IR_Cur1x24M 	= ("0" + (dtCurr1x24.getMonth() + 2)).slice(-2);
													IR_Cur1x24D 	= ("0" + (dtCurr1x24.getDate() - 30)).slice(-2);
												}

												var dtCurr1x24		= IR_Cur1x24Y + '-' + IR_Cur1x24M + '-' + IR_Cur1x24D;

												console.log(currDate+' > '+dtCurr1x24);
												if(currDate > dtCurr1x24)
												{
													PO_PLANIR = dtCurr1x24;
		                        ...
		                        <div class="form-group">
		                          	...
		                          	<div class="col-sm-9">
		                        		...
		                        		<select name="SPLCODEX" id="SPLCODEX" class="form-control select2" data-placeholder="&nbsp;&nbsp;&nbsp;<?php echo $SupplierName; ?>" disabled>
		                        			<option value=""> --- </option>
		                                    <?php
		                                    	$sqlSPL 	= "SELECT DISTINCT A.SPLCODE, A.SPLDESC FROM tbl_supplier A";
...
<script>
		...
		$('#datepicker2').on('change', function(e) {
			...
			$.ajax({
				...
				success: function(result) {
					...
					{
						var dtCurr1x24		= new Date(RecIRSel);
						var IR_Cur1x24D 	= ("0" + (dtCurr1x24.getDate() + 1)).slice(-2); // Penerimaan material max. 1x24 jam => upd. 2023-05-31
						var IR_Cur1x24M 	= ("0" + (dtCurr1x24.getMonth() + 1)).slice(-2);
						var IR_Cur1x24Y 	= dtCurr1x24.getFullYear();
						console.log('dtCurr1x24 = '+dtCurr1x24.getDate());
						if(dtCurr1x24.getDate() === 31)
						{
							console.log('dtCurr1x24_1 = '+dtCurr1x24.getDate());
							IR_Cur1x24M 	= ("0" + (dtCurr1x24.getMonth() + 2)).slice(-2);
	...
	function changeValue(thisVal, theRow)
	{
		...
		document.getElementById('ITM_QTY'+theRow).value 		= parseFloat(Math.abs(ITM_QTYX));
		document.getElementById('ITM_QTYX'+theRow).value 		= doDecimalFormat(RoundNDecimal(parseFloat(Math.abs(ITM_QTYX)),3));
		...
		$.ajax({
            ...
            success: function(response)
            {
            	...
            	IR_ID 		= arrItem[0];
            	IR_NUM 		= arrItem[1];
            	// QTYREM 		= arrItem[2]; // Update ITM_QTY_REM sudah diupdate di function getPO : upd: 2023-08-08
	...
	function CopyCOA_IR(row)
	{
		let collID  	= document.getElementById('urlCopyCOA_IR'+row).value;
