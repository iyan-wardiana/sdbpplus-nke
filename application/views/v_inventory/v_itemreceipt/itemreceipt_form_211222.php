														...
														<?php
															if($IR_STAT == 1 || $IR_STAT == 4) 
															{
																if($ITM_CATEG == 'UA')
																{
																	?>
																		hidden
																		<input type="hidden" class="form-control" style="text-align:right" name="ITM_QTYX<?php echo $currentRow; ?>" id="ITM_QTYX<?php echo $currentRow; ?>" value="<?php print number_format($ITM_QTY, $decFormat); ?>" onBlur="changeValue(this, <?php echo $currentRow; ?>)" onKeyPress="return isIntOnlyNew(event);" <?php if($isDisabled == 1) { ?> disabled <?php } ?> size="20" >
																	<?php
																}
																else
																{
