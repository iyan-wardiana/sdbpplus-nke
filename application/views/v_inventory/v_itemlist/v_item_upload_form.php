...
<html>
	...
	<?php
        ...
        if($LangID == 'IND')
        {
			...
			$alert6	= "Baik! Proses reset master material akan dibatalkan.";
        }
        else
        {
			...
			$alert6	= "Well! The material master reset process will be canceled.";
        }
		
		$isLoadDone_1	= 1;
		$ITMH_CODEX		= '';
		$sqlBoQC		= "tbl_item WHERE PRJCODE = '$PRJCODE'";
		$resBoQC		= $this->db->count_all($sqlBoQC);

		// CHECK STATUS
		if(isset($_POST['ITMH_CODEX']))
		{
			$ITMH_CODEY	= $_POST['ITMH_CODEX'];
		...
		if($resCountITM == 0)
		{
			if(isset($_POST['ITMH_CODEX']))
			{
				...
				$ITMH_FN	= '';
				$ITMH_CODEX	= $_POST['ITMH_CODEX'];
				$ITMH_CODE	= $_POST['ITMH_CODEX'];
				...
				$myXlsFile	= "$ITMH_FN";
				$myPath 	= "import_excel/import_item/$myXlsFile";
				...
				for ($i=2; $i <= $baris; $i++)
				{
					$PRJCODE 		= $data->val($i, 2);		// B - OK
					$PRJCODE_HO		= $data->val($i, 3);		// C - OK
					$PRJPERIOD		= $PRJCODE;					// D - OK
					$ITM_CODE 		= $data->val($i, 5);		// E - OK
					$ITM_GROUP 		= $data->val($i, 13);		// M - OK
					$ITM_CATEG		= $ITM_GROUP;
					$ITM_CLASS		= $data->val($i, 15);		// M - OK
					$ITM_NAME		= addslashes($data->val($i, 16));		// N - OK
					$ITM_DESC		= addslashes($data->val($i, 18));		// P - OK
					$ITM_TYPE		= addslashes($data->val($i, 19));		// Q - OK
					$ITM_UNIT		= addslashes($data->val($i, 20));		// R - OK
					$UMCODE			= $ITM_UNIT;				// S - OK
					$ITM_CURRENCY	= $data->val($i, 22);		// T - OK
					$ITM_VOLMBG		= $data->val($i, 23);		// W - OK
					if($ITM_VOLMBG == '')
						$ITM_VOLMBG	= 0;
					$ITM_VOLMBGR	= $ITM_VOLMBG;				// V - OK
					if($ITM_VOLMBGR == '')
						$ITM_VOLMBGR= 0;
					$ITM_VOLM		= $data->val($i, 25);		// W - OK
					if($ITM_VOLM == '')
						$ITM_VOLM	= 0;
					$ITM_PRICE		= $data->val($i, 26);		// X - OK
					if($ITM_PRICE == '')
						$ITM_PRICE	= 0;
					$ITM_REMQTY		= $ITM_VOLM;				// AA - OK
					$ITM_TOTALP		= $ITM_VOLMBG * $ITM_PRICE;	// AB - OK
					if($ITM_TOTALP == '')
						$ITM_TOTALP	= 0;


					$ITM_LASTP		= $ITM_PRICE;				// AC - OK
					$ITM_AVGP		= $ITM_PRICE;				// AD - OK
					$BOQ_ITM_VOLM	= $ITM_VOLMBG;
					$BOQ_ITM_PRICE	= $ITM_PRICE;
					$BOQ_ITM_TOTALP	= $ITM_TOTALP;
					$ACC_ID			= $data->val($i, 60);		// BE - OK
					$ACC_ID_UM		= $data->val($i, 61);		// BF - OK
					$STATUS			= $data->val($i, 69);		// BP - OK
					$ISMTRL			= $data->val($i, 70);		// BQ - OK
					$ISRENT			= $data->val($i, 71);		// BR - OK
					$ISPART			= $data->val($i, 72);		// BS - OK
					$ISFUEL			= $data->val($i, 73);		// BT - OK
					$ISLUBRIC		= $data->val($i, 74);		// BU - OK
					$ISFASTM		= $data->val($i, 75);		// BV - OK
					$ISWAGE			= $data->val($i, 76);		// BW - OK
					$ISRM			= $data->val($i, 77);		// BX - OK
					$ISWIP			= $data->val($i, 78);		// BY - OK
					$ISFG			= $data->val($i, 79);		// BZ - OK
					$ISRIB			= $data->val($i, 80);		// BZ - OK
					$ISCOST			= $data->val($i, 81);		// CA - OK
					$NEEDQRC		= $data->val($i, 82);		// CB - OK
					$ITM_KIND		= $data->val($i, 83);		// CC - OK
					$CALCTOFG		= $data->val($i, 84);		// CD - OK
					$ISMAJOR		= $data->val($i, 86);		// CC - OK
					$ISCOUNT		= $data->val($i, 87);		// CD - OK
					$LASTNO			= $data->val($i, 89);		// CF - OK
					
					// INSERT INTO ITM
					$sqlInsITM		= "INSERT INTO tbl_item (PRJCODE, PRJCODE_HO, PRJPERIOD, ITM_CODE, ITM_GROUP, ITM_CLASS, ITM_NAME, 
											...
											BOQ_ITM_PRICE, BOQ_ITM_TOTALP, ACC_ID, ACC_ID_UM, STATUS, 
											ISMTRL, ISRENT, ISPART, ISFUEL, ISLUBRIC, ISFASTM, ISWAGE, ISRM,
											ISWIP, ISFG, ISRIB, ISCOST, NEEDQRC, ISCOUNT, LASTNO)
										VALUES ('$PRJCODE', '$PRJCODE_HO', '$PRJPERIOD', '$ITM_CODE', '$ITM_GROUP', '$ITM_CLASS', '$ITM_NAME', 
											'$ITM_DESC', '$ITM_TYPE', '$ITM_UNIT', '$UMCODE', '$ITM_CURRENCY', '$ITM_VOLMBG', '$ITM_VOLMBGR', 
											'$ITM_VOLM', '$ITM_PRICE', '$ITM_REMQTY', '$ITM_TOTALP', '$ITM_LASTP', '$ITM_AVGP', '$BOQ_ITM_VOLM', 
											'$BOQ_ITM_PRICE', '$BOQ_ITM_TOTALP', '$ACC_ID', '$ACC_ID_UM', '$STATUS', 
											'$ISMTRL', '$ISRENT', '$ISPART', '$ISFUEL', '$ISLUBRIC', '$ISFASTM', '$ISWAGE', '$ISRM',
				...
				// UPDATE VOLUME FROM JOBLIST
					$sqlJLC		= "tbl_joblist_detail WHERE PRJCODE = '$PRJCODE'";
					$resJLC		= $this->db->count_all($sqlJLC);
					if($resJLC > 0)
					{
						$sqlUpdVITM		= "UPDATE tbl_item A
											SET A.ITM_VOLMBG = (
												SELECT
													sum(B.ITM_VOLM)
												FROM
													tbl_joblist_detail B
												WHERE
													B.ITM_CODE = A.ITM_CODE
												AND B.PRJCODE = '$PRJCODE'
											)
											WHERE
												A.PRJCODE = '$PRJCODE'";
						$this->db->query($sqlUpdVITM);
					}
			                    ...
			                    <form name="myformupload" id="myformupload" class="form-horizontal" enctype="multipart/form-data" method="post" action="<?php echo $form_action; ?>" onSubmit="return chcekFile();" >
			                        <div class="form-group">
			                            ...
			                            <input type="text" style="display:none" name="isUploaded" id="isUploaded" value="<?php echo $isUploaded; ?>" />
			                       		...
			                       		<input type="text" style="display:none" name="PRJPERIOD" id="PRJPERIOD" value="<?php echo $PRJPERIOD; ?>" />
			                        ...
			                        <div class="form-group">
			                            ...
			                            <div class="col-sm-10">
			                                <textarea class="form-control" name="ITMH_DESC"  id="ITMH_DESC" style="height:70px"><?php echo $ITMH_DESC; ?></textarea>
			                            </div>
			                        </div>
			                        <div class="form-group">
			                            <label for="inputName" class="col-sm-2 control-label">&nbsp;</label>
			        ...
			        <div class="box-body">
			            <div class="search-table-outter">
			                <form name="frmsrch" id="frmsrch" action="" method=POST style="display:none">
			                    <input type="text" name="ITMH_CODEX" id="ITMH_CODEX" class="textbox" value="<?php echo $ITMH_CODEX; ?>" />
			                ...
			                <table id="example1" class="table table-bordered table-striped table-responsive search-table inner" width="100%">
			                    ...
			                    <tbody>
								<?php
			                        $sqlITMH	= "tbl_item_uphist WHERE ITMH_PRJCODE = '$PRJCODE'";
			                        $resITMH	= $this->db->count_all($sqlITMH);
			                        $i = 0;
			                        $j = 0;
			                        if($resITMH >0)
			                        {
			                            $sqlITMHV	= "SELECT * FROM tbl_item_uphist WHERE ITMH_PRJCODE = '$PRJCODE'";
			                            $resITMHV	= $this->db->query($sqlITMHV)->result();
			                            foreach($resITMHV as $row) :
			                                $myNewNo 		= ++$i;
			                                $ITMH_CODE 		= $row->ITMH_CODE;
			                                $ITMH_PRJCODE 	= $row->ITMH_PRJCODE;
			                                $ITMH_DATE 		= $row->ITMH_DATE;
			                                $ITMH_DESC	 	= $row->ITMH_DESC;
			                                $ITMH_FN	 	= $row->ITMH_FN;
			                                $ITMH_STAT	 	= $row->ITMH_STAT;
			                                $ITMH_USER	 	= $row->ITMH_USER;
			                                ...
			                                ?> 
			                                        ...
			                                        <td nowrap> <?php echo $ITMH_CODE;?></td>
			                                        <td nowrap> <?php echo $ITMH_DATE; ?> </td>
			                                        <td nowrap> <?php echo $ITMH_PRJCODE; ?> </td>
			                                        <td> <?php echo $ITMH_DESC; ?> </td>
			                                        <td> <?php echo $ITMH_USER; ?> </td>
			                                        ...
			                                        <td style="text-align:center" nowrap>
			                                            <?php
			                                                $FileName 	= $ITMH_FN;
			                                                $fileAttach	= base_url('import_excel/import_item/'.urldecode($FileName));
			                                                $collLink	= "$fileAttach~$FileName";
			                                                $linkDL1 	= site_url('c_inventory/c_it180e2elst/downloadFile/?id='.$this->url_encryption_helper->encode_url($collLink));
			                                                $secVWlURL	= site_url('c_inventory/c_it180e2elst/view_itemup/?id='.$this->url_encryption_helper->encode_url($ITMH_CODE));
			                                            ?>
			                                            <input type="hidden" name="secVWlURL_<?php echo $myNewNo; ?>" id="secVWlURL_<?php echo $myNewNo; ?>" value="<?php echo $secVWlURL; ?>"/>
			                                            <input type="hidden" name="ITMH_CODE<?php echo $myNewNo; ?>" id="ITMH_CODE<?php echo $myNewNo; ?>" value="<?php echo $ITMH_CODE; ?>"/>
			                                            <a href="javascript:void(null);" onClick="getITMH_CODE(<?php echo $myNewNo; ?>);" data-skin="skin-green" class="btn btn-warning btn-xs" title="Process" <?php if($disabled1 == 1) { ?> disabled="disabled" <?php } ?>>
...
<script>
	...
	{
		/*var result = confirm("<?php echo $alert3; ?>");
		if (result)
		{
