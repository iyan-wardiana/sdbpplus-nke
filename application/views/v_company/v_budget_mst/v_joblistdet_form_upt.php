...
<?php
	...
	if(isset($_POST['JOBCODEID']))
	{
		...
		$sqlUPDJDD	= "UPDATE tbl_joblist_detailm SET JOBDESC = '$JOBDESC', ITM_GROUP = '$ITM_GROUP', GROUP_CATEG = '$ITM_GROUP',
							ITM_UNIT = '$JOBUNIT', ITM_PRICE = '$ITM_PRICE', ITM_LASTP = '$ITM_PRICE'
						WHERE JOBCODEID = '$JOBCODEID'";
		$this->db->query($sqlUPDJDD);
		
		$sqlUPDBOQ	= "UPDATE tbl_boqlistm SET JOBDESC = '$JOBDESC', JOBGRP = '$ITM_GROUP', JOBUNIT = '$JOBUNIT',
		...
		if($ITM_CODE	== '')
		{
			$sqlITM		= "SELECT ITM_CODE FROM tbl_joblist_detailm WHERE JOBCODEID = '$JOBCODEID'";
		...
		if($ITM_CODE != '')
		{			
			$sqlUPD2	= "UPDATE tbl_joblist_detailm SET ITM_CODE = '$ITM_CODE' WHERE JOBCODEID = '$JOBCODEID'";
			$this->db->query($sqlUPD2);
			
			$sqlUPD2	= "UPDATE tbl_boqlistm SET ITM_CODE = '$ITM_CODE' WHERE JOBCODEID = '$JOBCODEID'";
			$this->db->query($sqlUPD2);
		}
		$PRICE		= $ITM_PRICE;
		...
		if($ISHEADER == 1)
		{
			$sqlUPD2	= "UPDATE tbl_joblist_detailm SET ITM_CODE = '' WHERE JOBCODEID = '$JOBCODEID'";
			$this->db->query($sqlUPD2);
			$sqlUPD2	= "UPDATE tbl_boqlistm SET ITM_CODE = '' WHERE JOBCODEID = '$JOBCODEID'";
			$this->db->query($sqlUPD2);
		}
		$updated	= 1;
	}
...
$JOBDESCD	= '';
$sqlJD 		= "SELECT JOBDESC FROM tbl_joblist_detailm WHERE JOBCODEID = '$JOBCODEID'";
...
$JOBDESCP	= '';
$sqlJDP 	= "SELECT JOBDESC FROM tbl_joblist_detailm WHERE JOBCODEID = '$JOBPARENT'";
