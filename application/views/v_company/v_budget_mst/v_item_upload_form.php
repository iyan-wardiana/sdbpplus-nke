...
if($resCountITM == 0)
{
	if(isset($_POST['ITMH_CODEX']))
	{
		...
		$myXlsFile	= "$ITMH_FN";
		$myPath 	= "import_excel/import_item/period/$myXlsFile";
		...
		for ($i=2; $i <= $baris; $i++)
		{
			...
			$PRJCODE_HO		= $data->val($i, 3);		// B - OK
			$PRJPERIOD		= $data->val($i, 4);		// B - OK
			$ITM_CODE 		= $data->val($i, 5);		// C - OK
			$ITM_GROUP 		= $data->val($i, 13);		// K - OK
			$ITM_CLASS		= $data->val($i, 15);		// M - OK
			$ITM_NAME		= addslashes($data->val($i, 16));		// N - OK
			//$ITM_NAME 		= str_replace("'", "", $ITM_NAME1);
			...
			$UMCODE			= addslashes($data->val($i, 21));		// S - OK
			$ITM_CURRENCY	= $data->val($i, 22);		// T - OK
			$ITM_VOLMBG		= $data->val($i, 23);		// U - OK
			if($ITM_VOLMBG == '')
				$ITM_VOLMBG	= 0;
			$ITM_VOLMBGR	= $data->val($i, 24);		// V - OK
			if($ITM_VOLMBGR == '')
				$ITM_VOLMBGR	= 0;
			...
			if($ITM_PRICE == '')
				$ITM_PRICE	= 0;
			$ITM_REMQTY		= $data->val($i, 27);		// Y - OK
			if($ITM_REMQTY == '')
				$ITM_REMQTY	= 0;
			$ITM_TOTALP		= $data->val($i, 28);		// Z - OK
			if($ITM_TOTALP == '')
				$ITM_TOTALP	= 0;
			$ITM_LASTP		= $data->val($i, 29);		// AA - OK
			if($ITM_LASTP == '')
				$ITM_LASTP	= 0;
			$BOQ_ITM_VOLM	= $data->val($i, 30);		// AB - OK
			if($BOQ_ITM_VOLM == '')
				$BOQ_ITM_VOLM	= 0;
			$BOQ_ITM_PRICE	= $data->val($i, 31);		// AC - OK
			if($BOQ_ITM_PRICE == '')
				$BOQ_ITM_PRICE	= 0;
			$BOQ_ITM_TOTALP	= $data->val($i, 32);		// AD - OK
			if($BOQ_ITM_TOTALP == '')
				$BOQ_ITM_TOTALP	= 0;
			$ACC_ID			= $data->val($i, 59);		// BE - OK
			$ACC_ID_UM		= $data->val($i, 60);		// BF - OK
			$STATUS			= $data->val($i, 68);		// BN - OK
			$ISMAJOR		= $data->val($i, 83);		// CC - OK
			$ISCOUNT		= $data->val($i, 84);		// CD - OK
			$LASTNO			= $data->val($i, 86);		// CF - OK
			...
			// INSERT INTO ITM
			$sqlInsITM		= "INSERT INTO tbl_item (PRJCODE, PRJCODE_HO, PRJPERIOD, ITM_CODE, ITM_GROUP, ITM_CLASS, ITM_NAME, ITM_DESC, 
									ITM_TYPE, ITM_UNIT, UMCODE, ITM_CURRENCY, ITM_VOLMBG, ITM_VOLMBGR, ITM_VOLM, ITM_PRICE, 
									ITM_REMQTY, ITM_TOTALP, ITM_LASTP, BOQ_ITM_VOLM, BOQ_ITM_PRICE, BOQ_ITM_TOTALP,
									ACC_ID, ACC_ID_UM, STATUS, ISCOUNT, LASTNO)
								VALUES ('$PRJCODE', '$PRJCODE_HO', '$PRJPERIOD', '$ITM_CODE', '$ITM_GROUP', '$ITM_CLASS', '$ITM_NAME', '$ITM_DESC', 
									'$ITM_TYPE', '$ITM_UNIT', '$UMCODE', '$ITM_CURRENCY', '$ITM_VOLMBG', '$ITM_VOLMBGR', '$ITM_VOLM', '$ITM_PRICE', 
									'$ITM_REMQTY', '$ITM_TOTALP', '$ITM_LASTP', '$BOQ_ITM_VOLM', '$BOQ_ITM_PRICE', '$BOQ_ITM_TOTALP', 
									'$ACC_ID', '$ACC_ID_UM', '$STATUS', '$ISCOUNT', '$LASTNO')";
                ...
                <div class="box-body chart-responsive">
                    <form name="myformupload" id="myformupload" class="form-horizontal" enctype="multipart/form-data" method="post" action="<?php echo $form_action; ?>" onSubmit="return chcekFile();" >
                        ...
                        <div class="form-group">
                            ...
                            <div class="col-sm-10">
                                <textarea class="form-control" name="ITMH_DESC"  id="ITMH_DESC" style="max-width:350px;height:70px"><?php echo $ITMH_DESC; ?></textarea>
                            </div>
                        </div>
                        <div class="form-group">
                            <label for="inputName" class="col-sm-2 control-label">&nbsp;</label>
                            ...
                            foreach($resITMHV as $row) :
                                ...
                                ?> 
                                        ...
                                        <td style="text-align:center" nowrap>
                                            <?php
                                                $FileName 	= $ITMH_FN;
                                                $fileAttach	= base_url('import_excel/import_item/period/'.urldecode($FileName));
                                                $collLink	= "$fileAttach~$FileName";
                                                $linkDL1 	= site_url('c_comprof/c_bUd93tL15t/downloadFile/?id='.$this->url_encryption_helper->encode_url($collLink));
                                                $secVWlURL	= site_url('c_comprof/c_bUd93tL15t/view_itemup/?id='.$this->url_encryption_helper->encode_url($ITMH_CODE));
                                            ?>
