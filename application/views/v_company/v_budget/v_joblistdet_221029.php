	    	...
	    	<!-- ============ START MODAL LOCK RAPT/RAPP =============== -->
				...
				<script type="text/javascript">
					async function procLockRAPT()
					{
						...
						let DOC_REF		= $('#LOCK_DOCREF1');
						let DOC_DATE	= $('#datepicker1');
						let DOC_FILE	= $('#userfile1')[0];
						let DOC_EMP1	= $('#LOCK_EMP1');
						let DOC_DESC	= $('#LOCK_DESC1');

						if (DOC_FILE!="") 
						{
							// let file_data = $('#userfile1').prop('files')[0]; 
							let file_data 	= DOC_FILE.files[0]; 
							let form_data 	= new FormData();                  
							form_data.append('file', file_data);
							// console.log(form_data);
							// return false;
							let response = await fetch("<?php echo site_url('__l1y/lockRAPT')?>", {
								method: "POST",
								body: form_data
							});
