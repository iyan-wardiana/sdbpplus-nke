...
if(isset($_POST['IMP_CODEX']))
{
						...
						foreach ($reader->getSheetIterator() as $sheet)
						{
							...
							foreach ($sheet->getRowIterator() as $row)
							{
								...
								if($rowCellD > 1)
								{
									...
									// INSERT INTO BOQ
										$sqlInsBoQ		= "INSERT INTO tbl_boqlist (ORD_ID, JOBCODEID, JOBCODEIDV, JOBPARENT, JOBCODEID_P, 
																PRJCODE, PRJCODE_HO,
																ITM_CODE,  JOBDESC, JOBGRP, JOBUNIT, JOBLEV,
																JOBVOLM, PRICE, JOBCOST, BOQ_VOLM, BOQ_PRICE, BOQ_JOBCOST,
																PRJPERIOD, PRJPERIOD_P, BOQ_BOBOT,
																ISBOBOT, ISHEADER, ISLASTH, ISLAST, Patt_Number)
															VALUES ($ORDID, '$JOBCODEID', '$JOBCODEID', '$JOBPARENT', '$JOBCODEID_P',
																'$PRJCODE', '$PRJCODE_HO',
									...
									// INSERT INTO Joblist. SEHARUSNYA UNTUK JOBLIST HANYA MENYIMPAN HEADER
										/*if($ISHEADER == 1)
										{*/
											$sqlInsJL		= "INSERT INTO tbl_joblist (ORD_ID, JOBCODEID, JOBCODEIDV, JOBPARENT, JOBCODEID_P, 
									...
									// INSERT INTO JoblistDetail
										$sqlInsJLD		= "INSERT INTO tbl_joblist_detail (ORD_ID, JOBCODEDET, JOBCODEID, JOBPARENT, JOBCODEID_P,
																...
																IS_LEVEL, ISLASTH, ISLAST, Patt_Number)
	...
	else if($IMP_TYPE == 'RESETWBD')
	{
		...
		// START : RESET SELURUH PENGGUNAAN JOBLIS_DETAIL
			...
			// GET LAST BALANCE SAMPAI BULAN SEBELUM BULAN SAAT INI
				...
				$LS_DEBET		= 0;
				$LS_KREDIT		= 0;
				$sqlLSYNC 		= "SELECT LS_DEBET, LS_KREDIT FROM tbl_lastsync
									WHERE PRJCODE = '$PRJCODE' AND LS_YEAR = '$LS_YEAR' AND LS_MONTH = '$LS_MONTH' LIMIT 1";
				$resLSYNC		= $this->db->query($sqlLSYNC)->result();
				foreach($resLSYNC as $rowLSYNC):
					$LS_DEBET	= $rowLSYNC->LS_DEBET;
					$LS_KREDIT	= $rowLSYNC->LS_KREDIT;
				endforeach;

			// BREAK DOWN ALLL JOURNAL BY ACC_ID
			$LS_DEBET		= ;
			$LS_KREDIT		= 0;
			$sqlLSYNC 		= "SELECT DISTINCT Acc_Id FROM tbl_journaldetail WHERE proj_Code = '$PRJCODE' AND  isSyncY = 0";
			$resLSYNC		= $this->db->query($sqlLSYNC)->result();
			foreach($resLSYNC as $rowLSYNC):
				$LS_DEBET	= $rowLSYNC->LS_DEBET;
				$LS_KREDIT	= $rowLSYNC->LS_KREDIT;
			...
			$resetCOA	= "UPDATE tbl_chartaccount SET Base_Debet = $LS_DEBET, Base_Debet2 = $LS_DEBET, BaseD_$accYr = $LS_DEBET,
								Base_Kredit = $LS_KREDIT, Base_Kredit2 = $LS_KREDIT, BaseK_$accYr = $LS_KREDIT
							WHERE PRJCODE = '$PRJCODE' AND LS_YEAR = '$LS_YEAR' AND LS_MONTH = '$LS_MONTH'";
			$this->db->query($resetCOA);

			$rowRes 	= 0;
			$sqlJDet	= "SELECT A.JournalH_Code, A.JournalH_Date, A.Acc_Id, A.JOBCODEID, A.ITM_CODE, A.ITM_CATEG, A.ITM_UNIT, A.ITM_VOLM, A.ITM_PRICE,
			...
			foreach($resJDet as $rowJDet) :
				...
				$up_00 			= "UPDATE tbl_journaldetail SET isSyncY = 1 WHERE proj_Code = '$PRJCODE' AND JournalH_Code = '$JournalH_Code'";
				$this->db->query($up_00);
				...
				// START : Update to COA - Debit
					...
					if($jmD > 0)
					{
						...
						for($i=0; $i < $jmD; $i++)
						{
							...
							if($r_00 == 0)
							{
								$s_ins 	= "INSERT INTO tbl_lastsync (PRJCODE, LS_ACC_ID, LS_DEBET, LS_KREDIT)
											VALUES ('$SYNC_PRJ', '$ACC_NUM', $Base_Debet, 0)";
								$this->db->query($s_ins);
							}
							else
							{
								$s_ins 	= "UPDATE tbl_lastsync SET LS_DEBET = LS_DEBET+$Base_Debet WHERE PRJCODE = '$SYNC_PRJ' AND LS_ACC_ID = '$ACC_NUM'";
								$this->db->query($s_ins);
							}
						}
					}
				...
				// START : SENT PROGRESS BAR
					...
					echo '<script>
					parent.document.getElementById("progressbarXX").innerHTML="<div class=\"cssProgress-bar cssProgress-warning cssProgress-active\" style=\"width:'.$percent.'; text-align:center; font-weight:bold;\"><span style=\"text-align:center; font-weight:bold\">'.$percent.' ( '.$rowRes. ' / '.$r_TOTJD.') processed</span></div>";</script>';
		...
		// START : RESET OTHER JOURNAL
			...
			$rowRes 	= 0;
			$sqlJDet	= "SELECT A.JournalH_Code, A.JournalH_Date, A.Acc_Id, A.Base_Debet, A.Base_Kredit
							FROM tbl_journaldetail A
				...
				// START : Update to COA - Debit
					...
					if($jmD > 0)
					{
						...
						for($i=0; $i < $jmD; $i++)
						{
							...
							{
								$s_ins 	= "INSERT INTO tbl_lastsync (PRJCODE, LS_ACC_ID, LS_DEBET, LS_KREDIT)
											VALUES ('$SYNC_PRJ', '$ACC_NUM', $Base_Debet, $Base_Kredit)";
								$this->db->query($s_ins);
							}
							else
							{
								$s_ins 	= "UPDATE tbl_lastsync SET LS_DEBET = LS_DEBET+$Base_Debet, LS_KREDIT = LS_KREDIT+$Base_Kredit
											WHERE PRJCODE = '$SYNC_PRJ' AND LS_ACC_ID = '$ACC_NUM'";
								$this->db->query($s_ins);
							}
						}
					}
	...
	else if($IMP_TYPE == 'RECOUNTJRN')	// READY TO ADD ON
	{
		...
		$s_TOTJD	= "tbl_journaldetail WHERE GEJ_STAT = '3' AND proj_Code = '$PRJCODE' AND JournalType IN ('VCASH', 'CPRJ', 'CHO', 'OPN', 'UM', 'BP')";
		$r_TOTJD	= $this->db->count_all($s_TOTJD);
		...
		// RESET SELURUH PENGGUNAAN JOBLISDETAIL
			...
			$updJLD = "UPDATE tbl_profitloss SET BPP_UPH_REAL = 0, BPP_SUBK_REAL = 0, BPP_ALAT_REAL = 0, BPP_I_REAL = 0,
							BPP_I_REAL = 0
						WHERE PRJCODE = '$PRJCODE'";
			$this->db->query($updJLD);

		$rowRes 	= 0;
		$sqlJDet	= "SELECT A.JournalH_Date, A.Acc_Id, A.JOBCODEID, A.ITM_CODE, A.ITM_CATEG, A.ITM_UNIT, A.ITM_VOLM, A.ITM_PRICE, A.Base_Debet, A.Base_Kredit
						FROM tbl_journaldetail A
						WHERE A.proj_Code = '$PRJCODE' AND A.GEJ_STAT = 3 AND A.ITM_CODE != '' AND JournalType IN ('VCASH', 'CPRJ', 'CHO', 'OPN', 'UM')";
		$resJDet	= $this->db->query($sqlJDet)->result();
		foreach($resJDet as $rowJDet) :
		...
		$sqlJACC	= "SELECT A.JournalH_Date, A.Acc_Id, A.Base_Debet, A.Base_Kredit
						FROM tbl_journaldetail A
						WHERE A.proj_Code = '$PRJCODE' AND A.GEJ_STAT = 3 AND JournalType IN ('BP')";
		$resJACC	= $this->db->query($sqlJACC)->result();
		foreach($resJACC as $rowJACC) :
			$rowRes 		= $rowRes+1;
			$JournalH_Date	= $rowJACC->JournalH_Date;
			$PERIODM		= date('m', strtotime($JournalH_Date));
			$PERIODY		= date('Y', strtotime($JournalH_Date));
			$ACC_NUM		= $rowJACC->Acc_Id;
			$Base_Debet		= $rowJACC->Base_Debet;
			$Base_Kredit	= $rowJACC->Base_Kredit;
