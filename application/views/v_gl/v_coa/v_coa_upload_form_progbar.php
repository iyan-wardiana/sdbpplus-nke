...
<html>
	<head>
    	...
    	<link rel="stylesheet" href="<?php echo base_url() . 'assets/css/pbar/dist/css/skins/_all-skins.min.css'; ?>">
		<script language="javascript" src="<?php echo base_url() . 'assets/AdminLTE-2.0.5/plugins/jQuery/jquery-2.2.3.min.js'; ?>"></script>

	  <!-- <link rel="stylesheet" href="<?php echo base_url() . 'assets/css/pbar/fonts/font-awesome-4.5.0/css/font-awesome.mina.css'; ?>">
	  <link rel="stylesheet" href="<?php echo base_url() . 'assets/css/pbar/css/stylea.css'; ?>"> -->
...
// CHECK STATUS
if(isset($_POST['COAH_CODEX']))	// MARKETING
{
	$COAH_CODEY	= $_POST['COAH_CODEX'];
...
if($resCountCOA == 0)
{
	if(isset($_POST['COAH_CODEX']))	// MARKETING
	{
		...
		$COAH_FN	= '';
		$COAH_CODEX	= $_POST['COAH_CODEX'];
		$COAH_CODE	= $_POST['COAH_CODEX'];
		...
		for ($i=1; $i <= 100; $i++)
		{
			...
			// UPDATE STATUS
				$sqlUpdCOA		= "UPDATE tbl_coa_uphist SET TOTR = $baris WHERE COAH_CODE = '$COAH_CODEX'";
		...
		for ($i=2; $i <= $baris; $i++)
		{
			for ($j=1; $j <=$totCol; $j++)
			{
				...
				if($colNamex == 'ORD_ID')
					$ORD_ID 	= $data->val($i, $j);
				elseif($colNamex == 'PRJCODE')
					$PRJCODE 		= $data->val($i, $j);
				elseif($colNamex == 'PRJCODE_HO')
					$PRJCODE_HO 	= $data->val($i, $j);
				elseif($colNamex == 'Acc_ID')
					$Acc_ID 		= $data->val($i, $j);
				elseif($colNamex == 'Account_Class')
					$Account_Class 	= $data->val($i, $j);
				elseif($colNamex == 'Account_Number')
					$Account_Number = $data->val($i, $j);
				elseif($colNamex == 'Account_NameEn')
					$Account_NameEn = $data->val($i, $j);
				elseif($colNamex == 'Account_NameId')
					$Account_NameId = $data->val($i, $j);
				elseif($colNamex == 'Account_Category')
					$Account_Category = $data->val($i, $j);
				elseif($colNamex == 'Account_Level')
					$Account_Level 	= $data->val($i, $j);
				elseif($colNamex == 'Acc_DirParent')
					$Acc_DirParent 	= $data->val($i, $j);
				elseif($colNamex == 'Acc_ParentList')
					$Acc_ParentList = $data->val($i, $j);
				elseif($colNamex == 'Acc_StatusLinked')
					$Acc_StatusLinked = $data->val($i, $j);
				elseif($colNamex == 'Acc_Enable')
					$Acc_Enable 	= $data->val($i, $j);
				elseif($colNamex == 'Default_Acc')
					$Default_Acc 	= $data->val($i, $j);
				elseif($colNamex == 'Currency_id')
					$Currency_id 	= $data->val($i, $j);
				elseif($colNamex == 'Link_Report')
					$Link_Report 	= $data->val($i, $j);
				elseif($colNamex == 'syncPRJ')
					$syncPRJ 		= $data->val($i, $j);
				elseif($colNamex == 'isLast')
					$isLast 		= $data->val($i, $j);
			...
			// INSERT INTO ITM
				$sqlInsCOA		= "INSERT INTO tbl_chartaccount (ORD_ID, PRJCODE, PRJCODE_HO, Acc_ID, Account_Class, Account_Number, Account_NameEn,
									...
									VALUES ('$ORD_ID', '$PRJCODE', '$PRJCODE_HO', '$Acc_ID', '$Account_Class', '$Account_Number', '$Account_NameEn',
										'$Account_NameId', '$Account_Category', '$Account_Level', '$Acc_DirParent', '$Acc_ParentList', 
		...
		// CATATAN 05 AGUSTUS 2020
		// TIDAK PERLU ADA COPY BUDGET, KARENA SEHARUSNYA TIDAK BOLEH ADA IMPORT COA SETELAH DIBUATKANNYA BUDGET PERIODE
			/*$sqlPRJBudg		= "SELECT PRJCODE FROM tbl_project WHERE PRJCODE_HO = '$PRJCODE_HO'";
			$resPRJBudg		= $this->db->query($sqlPRJBudg)->result();
			foreach($resPRJBudg as $rowPRJBudg) :
				$PRJCBUD	= $rowPRJBudg->PRJCODE;

				// DELETE ALL COA BUDGET
					$delCOABUD	= "DELETE FROM tbl_chartaccount WHERE PRJCODE = '$PRJCBUD'";
					$this->db->query($delCOABUD);

				// START : COPY FOR COA BUDGET
					$sqlAcc		= "SELECT ORD_ID, Acc_ID, Account_Class, Account_Number, Account_NameEn, Account_NameId, Account_Category,
					...
					foreach($resAcc as $rowAcc) :
						...
						$sqlInsrAcc			= "INSERT INTO tbl_chartaccount 
													...
													syncPRJ, isLast) 
												VALUES 
													($ORD_ID1, '$PRJCBUD', '$PRJCODE_HO', '$Acc_ID', '$Account_Class', '$Account_Number', '$Account_NameEn', 
    ...
    <div class="row">
        <div class="col-md-12">
			...
			<div class="cssProgress">
		      	<div class="progress3">
		        	<div class="cssProgress-bar cssProgress-warning cssProgress-active" id="myBar" data-percent="0" style="width: 0%;">
		          		<span class="cssProgress-label" id="muLab">0%</span>
		        	</div>
		      	</div>
		    </div>
        </div>
        ...
        <div class="box-body">
            <div class="search-table-outter">
                <form name="frmsrch" id="frmsrch" action="" method=POST style="display:none">
                    <input type="text" name="COAH_CODEX" id="COAH_CODEX" class="textbox" value="<?php echo $COAH_CODEX; ?>" />
                        ...
                        if($resITMH >0)
                        {
                            ...
                            foreach($resITMHV as $row) :
                                ...
                                ?> 
                                        ...
                                        <td> <?php echo $COAH_PRJCODE; ?> </td>
                                        <td> <?php echo $COAH_DESC; ?> </td>
                                        <td> <?php echo $COAH_USER; ?> </td>
                                        ...
                                        <td style="text-align:center" nowrap>
                                            ...
                                            <a href="javascript:void(null);" onClick="getCOAH_CODE(<?php echo $myNewNo; ?>);" data-skin="skin-green" class="btn btn-warning btn-xs" title="Process" <?php if($disabled1 == 1) { ?> disabled="disabled" <?php } ?>>
...
<script>
	...
	function getCOAH_CODE(row)
	{
		/*var result = confirm("<?php echo $alert2; ?>");
		if (result)
		{
			COAH_CODE = document.getElementById('COAH_CODE'+row).value;
			document.getElementById('COAH_CODEX').value = COAH_CODE;
		...
		.then((willDelete) => {
			if (willDelete) 
			{
				...
				.then((value) => {
					...
					/*var urlImp	= '<?php echo $impLink; ?>';
					var collID	= urlImp+'~'+COAH_CODE;
		        	var idImp 	= collID.split("~");
			        var myurl 	= idImp[0];
			        alert(urlImp)
					$.ajax({
			            type: 'POST',
					...
					var myLab 	= document.getElementById("muLab");
					var width 	= 1;
					var id 		= setInterval(frame, 1000);
					function frame() 
					{
						var collID1	= '<?php echo $percLink; ?>';
						var collID	= collID1+'~'+COAH_CODE;
