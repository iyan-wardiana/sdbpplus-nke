							...
							<form name="myformupload" id="myformupload" class="form-horizontal" enctype="multipart/form-data" method="post" >
								<div class="box-body">
			                        ...
			                        <div class="form-group">
			                            ...
			                            <div class="col-sm-9">
			                                <textarea class="form-control" name="COAH_DESC" id="COAH_DESC" style="height:70px"></textarea>
			                            </div>
			                        </div>
...
<?php
	...
	$frmAct 	= base_url().'index.php/c_gl/c_ch1h0fbeart/do_upload/?id=';
?>
<script>
    ...
    $('#example').DataTable( {
		...
		"autoWidth": true,
		"filter": true,
        "ajax": "<?php echo site_url('c_gl/c_ch1h0fbeart/get_AllData/?id='.$PRJCODE)?>",
        "type": "POST",
	...
	$(document).ready(function() {
		$('#myformupload').submit(function(e){
			...
			var COAH_DESC = document.getElementById('COAH_DESC').value;
			...
			$.ajax({
				...
				success: function(data)
				{
					...
					document.getElementById('COAH_DESC').value	= '';
		...
		.then((willDelete) => {
			if (willDelete) 
			{
				...
				.then((value) => {
					...
					$("#myiFrame")[0].contentWindow.IMP_TYPE.value 		= 'COA';
					butSubm.submit();
				});
			} 
			else 
	...
	function dowL(thisVal)
	{
		var urlVWDoc = document.getElementById('secDWL'+thisVal).value;
		...
		return window.open(urlVWDoc, target="_parent");
	}
