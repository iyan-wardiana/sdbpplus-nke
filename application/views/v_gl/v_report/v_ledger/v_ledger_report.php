...
if($PRJCODECOL[0] == 1)
{
	...
	$sql = "SELECT A.PRJCODE, A.PRJNAME, A.PRJ_MNG, A.PRJBOQ, A.isHO
			FROM tbl_project A
			WHERE isHO = 1";
...
elseif($PRJCODECOL[0] != 1)
{
	$isAll 			= 0;
	$ArrPRJCODE 	= join("','", $PRJCODECOL);
	$accJRN 		= "PRJCODE IN ('$ArrPRJCODE')";
	$jrnPRJ 		= "A.proj_Code IN ('$ArrPRJCODE')";
...
{
	$rowACC 	= 0;
	//$sqlACC 	= "SELECT Account_Number, Account_NameId, syncPRJ FROM tbl_chartaccount WHERE $accJRN AND isLast = 1";
	$sqlACC 	= "SELECT Account_Number, Account_NameId, syncPRJ FROM tbl_chartaccount WHERE PRJCODE = 'NKE' AND isLast = 1";
...
{
	$rowACC 	= 0;
	$sqlACC 	= "SELECT Account_Number, Account_NameId, syncPRJ FROM tbl_chartaccount WHERE $accJRN AND (ID BETWEEN $sellAccount AND $sellAccount2) AND isLast = 1";
	$resACC 	= $this->db->query($sqlACC)->result();
	foreach($resACC as $rwACC) :
		$rowACC		= $rowACC + 1;
		$Acc_Num 	= $rwACC->Account_Number;
...
{
	$rowACC 	= 0;
	$sqlACC 	= "SELECT Account_Number, Account_NameId, syncPRJ FROM tbl_chartaccount WHERE $accJRN AND ID = $sellAccount AND isLast = 1";
	$resACC 	= $this->db->query($sqlACC)->result();
	foreach($resACC as $rwACC) :
		$rowACC		= $rowACC + 1;
		$Acc_Num 	= $rwACC->Account_Number;
...
if($rowACC > 1)
	$AccSelc 	= "Multi Account";
else
{
	$Account_NameId	= '';
	$sqlACC 	= "SELECT Account_Number, Account_NameId FROM tbl_chartaccount WHERE Account_Number IN ($ACCSELCOL)";
	$resACC 	= $this->db->query($sqlACC)->result();
	foreach($resACC as $rowACC) :
		$Account_Number = $rowACC->Account_Number;
		$Account_NameId	= $rowACC->Account_NameId;
	endforeach;
	$AccSelc 	= $Account_NameId;
}
		        ...
		        <td colspan="3" class="style2" style="border-left: hidden; border-right: hidden; border-bottom: hidden;">
		            <table width="100%" border="1" rules="all">
		                <tr style="line-height: 25px;">
							<td width="20" nowrap style="text-align:left; font-weight:bold; border-top: double; border-bottom: double; border-left: hidden; border-right: hidden;">No.</td>
		                ...
		                <?php
							...
							$sqlJRNDC		= "tbl_journaldetail A
												...
												WHERE 
													...
													AND A.JournalH_Code IN (SELECT DISTINCT C.JournalH_Code FROM tbl_journaldetail C $ADDQUERY_ACCID)
													$ADDQUERY_SPLCODE
							...
							foreach($resCPRJ as $rowCPRJ):
								...
								?>
								<tr>
									<td nowrap style="text-align:left; border-left: hidden; border-right: hidden; border-bottom: hidden;"><?php echo $therow; ?></td>
