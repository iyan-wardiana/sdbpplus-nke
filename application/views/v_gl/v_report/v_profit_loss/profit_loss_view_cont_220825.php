		...
		<section class="content">
	        <table width="100%" border="0" style="size:auto">
				...
				<?php
                    ...
                    // NILAI PEKERJAAN TAMBAH KURANG - CURRENT AND TOTAL
						$TOTAL_SIC 	= 0;
						$sqlLRBEF	= "SELECT SUM(SI_REAL) AS SI_REAL
										FROM tbl_profitloss WHERE PRJCODE = '$PRJCODECOL' 
	                ...
	                // RENCANA PROGRESS & REALISASI PROGRESS
		                $Prg_Plan 	= 0;
		                $Prg_Real 	= 0;
		                $s_PROGPLAN	= "SELECT Prg_PlanAkum, Prg_RealAkum FROM tbl_projprogres
		                				WHERE proj_Code = '$PRJCODECOL' AND Prg_Month = $MSELECTED ORDER BY Prg_Step DESC LIMIT 1";
                        $r_PROGPLAN	= $this->db->query($s_PROGPLAN)->result();
                        foreach($r_PROGPLAN as $rw_PROGPLAN):
                            $Prg_Plan	= $rw_PROGPLAN->Prg_PlanAkum;
                            $Prg_Real	= $rw_PROGPLAN->Prg_RealAkum;
                        endforeach;
	                    
                    // REALISASI MC / TAGIHAN
                        $TOTAL_INCOME	= 0;
	                ...
	                // ------------------------------------ A. P E N D A P A T A N ------------------------------------
						...
						// SUB TOTAL A
							$SUBTOT_AA	= $PRJCOST;
							$SUBTOT_AB	= $PROGMC_REAL_AMB + $TOTAL_SIB + $TOTAL_INB;		// BEFORE
							$SUBTOT_AC	= $PROGMC_REAL_AMC + $TOTAL_SIC + $TOTAL_INC;		// CURRENT
							$SUBTOT_AD	= $PROGMC_PROYEKSI + $TOTAL_SID + $TOTAL_IND;		// DEVIADI / PROYEKSI
	                ...
	                // ------------------------------------ C -> B. BIAYA HUTANG ------------------------------------
						...
						$SUBTOT_CD= $BPP_MTR_PROYEKSI+$BPP_UPH_PROYEKSI+$BPP_ALT_PROYEKSI+$BPP_SUBK_PROYEKSI+$BPP_BAU_PROYEKSI+$BPP_OTH_PROYEKSI+$BPP_I_PROYEKSI;
	          	...
	          	<tr>
	                <td colspan="3" class="style2" style="text-align:left;">
	                    <table width="100%" style="size:auto; font-size:12px;">
	                        ...
	                        <tr style="text-align:left;">
	                         	<td title="Rencana Progress Mingguan" nowrap>RENCANA PROGRESS</td>
	                          	...
	                          	<td style="text-align:right;">
	                            	<?php echo number_format($Prg_Plan, 4); ?> %
	                            </td>
	                          	<td style="text-align:left;">&nbsp;</td>
	                          	<td>&nbsp;</td>
	                          	<td style="text-align:right;">&nbsp;</td>
	                        ...
	                        <tr style="text-align:left;">
	                         	<td title="Realisasi Progres Mingguan" nowrap>REALISASI PROGRESS</td>
	                          	<td>:</td>
	                          	<td style="text-align:right;">
	                            	<?php echo number_format($Prg_Real, 4); ?> %
	                            </td>
	                          	<td style="text-align:left;">&nbsp;</td>
	                          	<td>&nbsp;</td>
	                          	<td style="text-align:right;">&nbsp;</td>
	                        ...
	                        <tr style="text-align:left;">
	                         	<td nowrap>REALISASI MC / TAGIHAN</td>
	                          	<td>:</td>
	                          	<td style="text-align:right;">
	                            	<?php echo number_format($PROGMC_REAL, 4); ?> %
	                            </td>
					...
					<td colspan="3" class="style2" style="text-align:left; font-size:12px">
	              	<table width="100%" border="1" style="size:auto; font-size:12px;" rules="all">
	                	...
	                	<tr style="size:auto; font-size:4px;">
	              	  	...
	              	  	<?php //--- START 	: C -> B. BIAYA HUTANG ---// ?>
		                	<tr style="font-weight:bold; font-size:12px">
								<td style="<?=$stlLine5?>" align="center">B.</td>
								<td style="<?=$stlLine5?>; font-size: 13px; text-decoration: underline;" colspan="2">BIAYA HUTANG</td>
								<td style="<?=$stlLine5?>">&nbsp;</td>
								<td style="<?=$stlLine5?> text-align:right">&nbsp;</td>
								<td style="<?=$stlLine5?> text-align:right">&nbsp;</td>
								<td style="<?=$stlLine5?>">&nbsp;</td>
