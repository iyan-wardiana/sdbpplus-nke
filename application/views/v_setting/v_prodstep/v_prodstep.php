...
<html>
    ...
    <?php
    	...
    	foreach($resTransl as $rowTransl) :
    		...
    		if($TranslCode == 'SectDesc')$SectDesc = $LangTransl;
                    ...
                    <div class="search-table-outter">
                        <table id="example1" class="table table-bordered table-striped table-responsive search-table inner" width="100%">
                            ...
                            <tr>
                                ...
                                <th width="5%" style="vertical-align:middle; text-align:center" nowrap><?php echo $INVStep ?> </th>
                                <th width="25%" style="vertical-align:middle; text-align:center"><?php echo $SectName  ?> </th>
                                <th width="38%" style="vertical-align:middle; text-align:center"><?php echo $SectDesc ?> </th>
                            ...
                            <tbody> 
                                ...
                                if($countSTP > 0)
                                {
                                    foreach($vwSTP as $row) :
                                        $myNewNo 		= ++$i;
                                        $PRODS_NUM      = $row->PRODS_NUM;
                                        $PRODS_CODE 	= $row->PRODS_CODE;
                                        $PRODS_STEP 	= $row->PRODS_STEP;
                                        $PRODS_NAME 	= $row->PRODS_NAME;
                                        $PRODS_DESC 	= $row->PRODS_DESC;
                                        $PRODS_STAT 	= $row->PRODS_STAT;
                                        $PRODS_LAST 	= $row->PRODS_LAST;
                                        $PRODS_ORDER 	= $row->PRODS_ORDER;
        								$STDCOST        = 0;

        								if($PRODS_STAT == 0)
        								{
        									$PRODS_STATD= 'Non Aktif';
        									$STATCOL	= 'danger';
        								}
        								elseif($PRODS_STAT == 1)
        								{
                                        ...
                                        $secUpd         = site_url('c_setting/c_pr0ds73p/u775o_pr0ds73p/?id='.$this->url_encryption_helper->encode_url($PRODS_NUM));
                                        if ($j==1) {
                                		...
                                		?>
                                            ...
                                            <td nowrap style="text-align:left; display:none"> <?php print $PRODS_CODE; ?> </td>                   
                                            <td nowrap style="text-align:left"> <?php print $PRODS_CODE; ?> </td>
                                            <td style="text-align:center" nowrap><?php print $PRODS_ORDER; ?> </td>
                                            <td style="text-align:left" nowrap><?php print $PRODS_NAME; ?> </td>
                                            <td style="text-align:left" nowrap><?php print $PRODS_DESC; ?> </td>
                                            <td style="text-align:right" nowrap><?php print number_format($STDCOST, $decFormat); ?> </td>
                                            <td style="text-align:center" nowrap>
                                                <a href="<?php echo $secUpd; ?>" class="btn btn-info btn-xs" title="Update">
                                                    <i class="glyphicon glyphicon-pencil"></i>
                                                </a>
                                                <a href="" class="btn btn-danger btn-xs" title="Delete" onclick="return confirm('<?php echo $sureDelete; ?>')" <?php if($PRODS_STAT > 0) { ?>disabled="disabled" <?php } ?>>
                                ...
                                $url_add 	= site_url('c_setting/c_pr0ds73p/a44_pr0ds73p/?id='.$this->url_encryption_helper->encode_url($PRJCODE));
                                //$url_addRO 	= site_url('c_setting/c_pr0ds73p/a44p180c21o_r0/?id='.$this->url_encryption_helper->encode_url($PRJCODE));
                                //$url_addDir	= site_url('c_setting/c_pr0ds73p/addDir/?id='.$this->url_encryption_helper->encode_url($PRJCODE));
                                ?> 
                            </tbody>
                            <tfoot>
                                <tr>
