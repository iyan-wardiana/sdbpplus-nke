				...
				if($AH_ISLAST == 1)
				{
					...
					if(isset($_POST['data']))
					{
						foreach($_POST['data'] as $d)
						{
							...
							elseif($CB_SOURCE == 'PPD' || $CB_SOURCE == 'PD')
							{
								$RefType= "PPD";
								...
								{
									if($CB_SOURCE == 'PD')
										$CB_NOTES = "Transfer Pinjaman Dinas (PD) $ManNOPD";
									else
										$CB_NOTES = "Transfer kurang bayar PD $ManNOPD";
								}
							}
							else
							{
					...
					// START : JOURNAL DETAIL
						...
						if(isset($_POST['data']))
						{
							foreach($_POST['data'] as $d)
							{
								...
								if($CB_SOURCE == 'VCASH')
								{
									...
									$s_ACCID 	= "SELECT Acc_Id FROM tbl_journaldetail_vcash WHERE JournalH_Code = '$CBD_DOCNO'
													AND Journal_DK = 'K'";
								...
								elseif($CB_SOURCE == 'PD')
								{
									...
									$s_ACCID 	= "SELECT DISTINCT Acc_Id FROM tbl_journaldetail_pd WHERE proj_Code = '$PRJCODE'
								...
								elseif($CB_SOURCE == 'PPD')
								{
									...
									$s_ACCID 	= "SELECT DISTINCT Acc_Id FROM tbl_journaldetail WHERE proj_Code = '$PRJCODE'
								...
								elseif($CB_SOURCE == 'DP')
								{
									...
									$s_DP	= "SELECT DP_AMOUNT, DP_AMOUNT_PPN, DP_AMOUNT_PPH, DP_AMOUN_TOT FROM tbl_dp_header WHERE DP_NUM = '$CBD_DOCNO'";
						...
						// ------------------------------- K R E D I T KAS / BANK (Total Nilai Bayar Hut. Supplier => tgl. 2022-02-18)
							...
							if($CB_NOTES == '')
								$CB_NOTES 	= "Pembayaran ke $SPLDESC";
									...
									// START : Save Journal Detail - Kredit
										$sqlGEJDK = "INSERT INTO tbl_journaldetail (JournalH_Code, Acc_Id, proj_Code, Currency_id, JournalD_Kredit,
													...
													VALUES ('$JournalH_Code', '$ACC_ID_K', '$proj_Code', 'IDR', $transacValue1, $transacValue1,
														$transacValue1, 'Default', 1, 0, 'K', '$CB_NOTES', '$Manual_No', '$Acc_Name', '$proj_CodeHO')";
  	...
  	function get_AllDataINV() // GOOD
	{
		...
		// START : FOR SERVER-SIDE
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				$INV_REMAMN 	= $INV_AMOUNT_TOT - $INV_AMOUNT_PAID - $RsvPAID;
