...
class C_cho_pd extends CI_Controller  
{
  	...
  	function get_AllData() 			// Y
	{
		...
		// START : FOR SERVER-SIDE
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				if($JournalH_Desc2 != '')
				{
					$JDESC2 	= "<div style='white-space:nowrap;'>
										<p class='text-danger' style='white-space:nowrap;'><i class='fa fa-exclamation-triangle'></i>
								  		".$JournalH_Desc2."</p>
								  	</div>";
				}
				...
				$delID 				= "$secDelIcut~tbl_journalheader_pd~tbl_journaldetail_pd~JournalH_Code~$JournalH_Code~proj_Code~$PRJCODE";
				$voidID 			= "$secVoid~tbl_journalheader_pd~tbl_journaldetail_pd~JournalH_Code~$JournalH_Code~proj_Code~$PRJCODE";
	...
	function add_process_2220604() 			// Y
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			if($GJournal_Total == '')
				$GJournal_Total = 0;
			$JOURNL_DESC	= htmlspecialchars($this->input->post('JournalH_Desc'), ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401 | ENT_HTML5);
			...
			// START - HEADER
				...
				$projGEJH 		= array('JournalH_Code' 	=> $JournalH_Code,
										...
										'JournalH_Desc3'	=> htmlspecialchars($pay_ManCode, ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401 | ENT_HTML5),
			...
			// START - DETAIL
				...
				// START : DEBET - PD TABLE
					$insSQLD	= "INSERT INTO tbl_journaldetail_pd (JournalH_Code, Acc_Id, JournalType, proj_Code, Currency_id, JournalD_Debet,
										...
										1, 1, '$Manual_No', '$JOURNL_DESC', 'D', '$JournalType',
										0, '$GEJ_STAT', '$JournalH_Date', '$proj_CodeHO', '$PRJPERIOD', '$Acc_Name1', 1, $PattNumD)";
					//$this->db->query($insSQLD);			// 04 Juni 2022 oleh DIAN HER,ANTO
				// END : DEBET - PD TABLE
				...
				// START : KREDIT - PD TABLE
					$insSQLK	= "INSERT INTO tbl_journaldetail_pd (JournalH_Code, Acc_Id, JournalType, proj_Code, Currency_id, JournalD_Kredit,
										...
										1, 1, '$Manual_No', '$JOURNL_DESC', 'K', '$JournalType',
										0, '$GEJ_STAT', '$JournalH_Date', '$proj_CodeHO', '$PRJPERIOD', '$Acc_Name2', 1, $PattNumK)";
					//$this->db->query($insSQLK);			// 04 Juni 2022 oleh DIAN HER,ANTO
				// END : KREDIT - PD TABLE
			// END - DETAIL
	...
	function update_process() 		// Y
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$AH_APPROVER	= $DefEmp_ID;
			$AH_APPROVED	= date('Y-m-d H:i:s');
			$AH_NOTES		= htmlspecialchars($this->input->post('JournalH_Desc2'), ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401 | ENT_HTML5);
			$AH_ISLAST		= $this->input->post('IS_LAST');
			...
			// START : UPDATE HEADER
				$projGEJH 	= array('JournalH_Code' 	=> $JournalH_Code,
									...
									'JournalType' 		=> $JournalType,
									'JournalH_Desc'		=> htmlspecialchars($this->input->post('JournalH_Desc'), ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401 | ENT_HTML5),
									'JournalH_Desc3'	=> htmlspecialchars($this->input->post('JournalH_Desc3'), ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401 | ENT_HTML5),
									'JournalH_Date'		=> $JournalH_Date,
									'PD_Date'			=> $PD_Date,
									'PlanRDate' 		=> $PlanRDate,
									'Company_ID'		=> $comp_init,
