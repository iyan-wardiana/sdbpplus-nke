...
class C_cho70d18 extends CI_Controller  
{
	...
	function cp2b0d18_all() // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data['title'] 		= $mnName;
			$data["MenuCode"] 	= 'MN359';
			$data['addURL'] 	= site_url('c_finance/c_cho70d18/adda70d18/?id='.$this->url_encryption_helper->encode_url($PRJCODE));
			$data['backURL'] 	= site_url('c_finance/c_cho70d18/?id='.$this->url_encryption_helper->encode_url($PRJCODE));
  	...
  	function get_AllData() // GOOD
	{
		...
		// START : FOR SERVER-SIDE
			...
			$num_rows 		= $this->m_cho_payment->get_AllDataC($PRJCODE, $search);
			$total			= $num_rows;
			$output			= array();
			$output['draw']	= $draw;
			$output['recordsTotal'] = $output['recordsFiltered']= $total;
			$output['data']	= array();
			$query 			= $this->m_cho_payment->get_AllDataL($PRJCODE, $search, $length, $start, $order, $dir);
	...
	function adda70d18() // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data['backURL'] 	= site_url('c_finance/c_cho70d18/cp2b0d18_all/?id='.$this->url_encryption_helper->encode_url($PRJCODE));
			$data['PRJCODE'] 	= $PRJCODE;
	...
	function add_process() // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			// START : DETAIL JOURNAL
				// START : DEBET
					...
					foreach($_POST['data'] as $d)
					{
						...
						$insSQL	= "INSERT INTO tbl_journaldetail (JournalH_Code, Acc_Id, JournalType, proj_Code, JOBCODEID, Currency_id, JournalD_Debet, 
										...
										('$JournalH_Code', '$Acc_Id', 'CHO', '$proj_Code', '$JOBCODEID', 'IDR', $JournalD_Debet, 
										$JournalD_Debet_tax, $JournalD_Kredit, $JournalD_Kredit_tax, $Base_Debet, $Base_Debet_tax, 
				...
				// START : KREDIT -- Insert into tbl_journal_detail (K) for Akun Kas/Bank
					$insSQLK	= "INSERT INTO tbl_journaldetail (JournalH_Code, Acc_Id, JournalType, proj_Code, Currency_id, JournalD_Kredit,
										...
										('$JournalH_Code', '$acc_number', 'CHO', '$PRJCODE', 'IDR', $Base_DebetTOT, $Base_DebetTOT_Tax,
			...
			$url			= site_url('c_finance/c_cho70d18/cp2b0d18_all/?id='.$this->url_encryption_helper->encode_url($proj_Code));
			redirect($url);
		}
		else
		{
	...
	function up0b28t18() // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data['default']['ISPERSL'] 		= $getGEJ->ISPERSL;
			$data['default']['PERSL_EMPID'] 	= $getGEJ->PERSL_EMPID;
				...
				// START : DEBET
					...
					foreach($_POST['data'] as $d)
					{
						...
						$insSQL	= "INSERT INTO tbl_journaldetail (JournalH_Code, Acc_Id, JournalType, proj_Code, Currency_id, JournalD_Debet, 
										...
										VALUE
										('$JournalH_Code', '$Acc_Id', 'CHO', '$proj_Code', 'IDR', $JournalD_Debet, $JournalD_Debet_tax, 
