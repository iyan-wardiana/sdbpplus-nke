...
class C_cho70d18 extends CI_Controller  
{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			// START - HEADER
				...
				$projGEJH 		= array('JournalH_Code' 	=> $JournalH_Code,
										...
										'Manual_No' 		=> $Manual_No,
										'JournalType' 		=> $JournalType,
										'JournalH_Desc'		=> htmlspecialchars($this->input->post('JournalH_Desc'), ENT_QUOTES),
										'JournalH_Date'		=> $JournalH_Date,
										'Company_ID'		=> $comp_init,
										'Reference_Type'	=> $JournalType,
										'Emp_ID'			=> $DefEmp_ID,
	...
	function update_process() // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			// START : UPDATE HEADER
				$projGEJH 	= array('JournalH_Code' 	=> $JournalH_Code,
									'JournalH_Desc' 	=> htmlspecialchars($AH_NOTES, ENT_QUOTES),
									'Manual_No' 		=> $Manual_No,
									'JournalType' 		=> $JournalType,
									'JournalH_Desc'		=> htmlspecialchars($this->input->post('JournalH_Desc'), ENT_QUOTES),
									'JournalH_Date'		=> $JournalH_Date,
	...
	function update_processVCASH() // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$JournalH_Desc 	= htmlspecialchars($this->input->post('JournalH_Desc'), ENT_QUOTES);
			$JOURNL_DESC	= htmlspecialchars($this->input->post('JournalH_Desc'), ENT_QUOTES);
			$JOURNL_DESC	= htmlspecialchars($this->input->post('JournalH_Desc'), ENT_QUOTES);
  	...
  	function get_AllDataITM() // GOOD
	{
		...
		// START : FOR SERVER-SIDE
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				$ITM_AMD_AM 		= 0;
				$sqlJOBDR			= "SELECT
										SUM(PR_VOL) AS TOTPRVOL, SUM(PR_VOL_R) AS TOTPRVOL_R, SUM(PR_CVOL) AS TOTPRCVOL,
										SUM(PO_VOL) AS TOTPOVOL, SUM(PO_VOL_R) AS TOTPOVOL_R, SUM(PO_CVOL) AS TOTPOCVOL,
										...
										SUM(AMD_VOL) AS TOTAMDVOL, SUM(AMD_VOL_R) AS TOTAMDVOL_R, SUM(AMDM_VOL) AS TOTAMDMVOL_R,
										SUM(AMD_VAL) AS TOTAMDAMN, SUM(AMD_VAL_R) AS TOTAMDAMN_R, SUM(AMDM_VAL) AS TOTAMDMAMN_R
				...
				foreach($resJOBDR as $rowJOBDR) :
					// USED RAP SPP/SPK/VCASH/VLK/PPD => JOBCODEID --------------------------------
						...
						$TOTPOVOL 		= $rowJOBDR->TOTPOVOL; 		// PO AMOUNT
						$TOTPOVOL_R 	= $rowJOBDR->TOTPOVOL_R; 	// PO AMOUNT_R
						$TOTPOCVOL 		= $rowJOBDR->TOTPOCVOL; 	// PO CANCEL AMOUNT
						...
						$TOTAMDVOL 		= $rowJOBDR->TOTAMDVOL; 	// AMD VOLM
						$TOTAMDVOL_R 	= $rowJOBDR->TOTAMDVOL_R; 	// AMD VOLM_R
						$TOTAMDMVOL_R 	= $rowJOBDR->TOTAMDMVOL_R; 	// AMD VOLM_R MIN
						$TOTAMDAMN 		= $rowJOBDR->TOTAMDAMN; 	// AMD AMOUNT
						$TOTAMDAMN_R 	= $rowJOBDR->TOTAMDAMN_R; 	// AMD AMOUNT_R
						$TOTAMDMAMN_R 	= $rowJOBDR->TOTAMDMAMN_R; 	// AMD AMOUNT_R
						...
						$ITM_AMD_AM 	= $TOTAMDAMN;
					// END USED RAP => JOBCODEID ----------------------------------------------------------------------
				endforeach;
				/*$TOT_VOLMBG 		= $ITM_VOLMBG + $ADD_VOLM;
				$TOT_AMOUNTBG		= ($JOBVOLM * $JOBPRICE) + ($ADD_VOLM * $ADD_PRICE);*/
				$TOT_VOLMBG 		= $ITM_VOLMBG + $ADD_VOLM;
				$TOT_AMOUNTBG		= ($JOBVOLM * $JOBPRICE) + ($ITM_AMD_AM);

				$ITM_USED_AM1V 	= "($TOTPOAMN - $TOTPOCAMN) + ($TOTWOAMN - $TOTWOCAMN) + $TOTVCAMN + $TOTVLKAMN + $TOTPPDAMN";
				...
				{
					$BUDG_VOL 		= $ITM_VOLMBG;
					$BUDG_VAL 		= $TOT_AMOUNTBG;
					$ITM_USED 		= $ITM_VOLMBG;
					$ITM_USED_AM 	= $ITM_USED_AM1;
					$ITM_USED_R		= $ITM_VOLMBG;
					$ITM_USED_R_AM 	= $ITM_USEDR_AM;
					$BUDG_REMVOLM	= $ITM_VOLMBG;
					$BUDG_REMAMNT	= $JOBCOST + $ADD_JOBCOST - $ITM_USED_AM - $ITM_USEDR_AM;
				}
				else
				{
					$BUDG_VOL 		= $TOT_VOLMBG;
					...
					$ITM_USED 		= $ITM_USED1;
					$ITM_USED_AM 	= $ITM_USED_AM1;
					$ITM_USED_R		= $ITM_USEDR;
					$ITM_USED_R_AM 	= $ITM_USEDR_AM;
					$BUDG_REMVOLM	= $TOT_VOLMBG - $ITM_USED - $ITM_USEDR;
					$BUDG_REMAMNT	= $TOT_AMOUNTBG - $ITM_USED_AM - $ITM_USEDR_AM;
				}
				...
				if($disabledB == 0)
				{
					$REM_VOL = number_format($BUDG_REMVOLM, 2);
					$REM_AMN = number_format($BUDG_REMAMNT, 2);
				}
				else
				{
					$REM_VOL = "<span class='label label-danger' style='font-size:12px'>".number_format($BUDG_REMVOLM, 2)."</span>";
					$REM_AMN = "<span class='label label-danger' style='font-size:12px'>".number_format($BUDG_REMAMNT, 2)."</span>";
				}
				...
				$output['data'][] 	= array("<div style='text-align:center;'>".$chkBox."</div>",
											...
											"<div style='text-align:right; white-space: nowrap'>".$RecItemV."&nbsp;&nbsp;&nbsp;<span ".$CELL_COL.">".$UM_AMOUNTV.$RES_VW."</span></div>",
											"<div style='text-align:right;'>".$REM_VOL."</div>",
											"<div style='text-align:right;'><span ".$CELL_COL.">".$REM_AMN."</span></div>");
	...
	function chgVOLUME()
	{
		...
		// DAPATKAN TOTAL DIMINTA
			...
			$ITM_UVAL 		= 0;
			$ITM_UVAL_R 	= 0;
			$sqlJOBD		= "SELECT PO_VOL, PO_VAL, PO_VOL_R, PO_VAL_R, PO_CVOL, PO_CVAL, WO_VOL, WO_VAL, WO_VOL_R, WO_VAL_R, WO_CVOL, WO_CVAL,
									VCASH_VOL, VCASH_VAL, VCASH_VOL_R, VCASH_VAL_R, VLK_VOL, VLK_VAL, VLK_VOL_R, VLK_VAL_R, PPD_VOL, PPD_VAL, PPD_VOL_R, PPD_VAL_R
								FROM tbl_joblist_detail
								WHERE JOBCODEID = '$JOBCODEID' AND PRJCODE = '$PRJCODE'";
			...
			$REM_VOLM 		= $TOT_BVOL - $ITM_UVOL;
			$REM_AMN 		= $TOT_BVAL - $ITM_UVAL;
