...
class C_cho70d18 extends CI_Controller  
{
  	...
  	function get_AllDataCHO() // GOOD
	{
		...
		// START : FOR SERVER-SIDE
			...
			$endord			= $start + $length;
			$search			= $_REQUEST['search']["value"];
			$num_rows 		= $this->m_cho_payment->get_AllDataC($PRJCODE, $jrnType, $length, $start);
			$total			= $num_rows;
			$output			= array();
			$output['draw']	= $draw;
			$output['recordsTotal'] = $output['recordsFiltered']= $total;
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				$sqlISHO 			= "SELECT B.Account_NameId AS Acc_Name FROM tbl_journaldetail_vcash A
										LEFT JOIN tbl_chartaccount_$PRJCODEVW B ON A.Acc_Id = B.Account_Number AND A.proj_Code = B.PRJCODE
										WHERE A.proj_Code = '$PRJCODE' AND A.JournalH_Code = '$JournalH_Code'";
				$resISHO			= $this->db->query($sqlISHO)->result();
				foreach($resISHO as $rowISHO):
					$noUAcc 		= $noUAcc+1;
				...
				if($jrnType == 'VCASH')
				{
					...
					$secVoid 		= base_url().'index.php/__l1y/voidDoc_CJRN/?id=';
					$voidID 		= "$secVoid~tbl_journalheader_vcash~tbl_journaldetail_vcash~JournalH_Code~$JournalH_Code~proj_Code~$PRJCODE";
				}
				elseif($jrnType == 'CPRJ')
				{
					$secPrint1		= site_url('c_finance/c_cho70d18/printdocument_CPRJ/?id='.$this->url_encryption_helper->encode_url($JournalH_Code));
					$secUpd			= site_url('c_finance/c_cho70d18/up0b28t18CPRJ/?id='.$this->url_encryption_helper->encode_url($JournalH_Code));
					$delID 			= "$secDelIcut~tbl_journalheader_cprj~tbl_journaldetail_cprj~JournalH_Code~$JournalH_Code~proj_Code~$PRJCODE";

					//$secVoid 		= base_url().'index.php/__l1y/trashCHO_NEW/?id=';
					$secVoid 		= base_url().'index.php/__l1y/voidDoc_CJRN/?id=';
					$voidID 		= "$secVoid~tbl_journalheader_cprj~tbl_journaldetail_cprj~JournalH_Code~$JournalH_Code~proj_Code~$PRJCODE";
				}
				...
				$getPAY		= "tbl_bp_detail WHERE PRJCODE = '$PRJCODE' AND CBD_DOCNO = '$JournalH_Code' AND CB_STAT NOT IN (5,9)";
				$resPAY		= $this->db->count_all($getPAY);
				if($resPAY > 0)
					$isLock = 1;
  	...
  	function get_AllDataCHOGRP() // GOOD
	{
		...
		// START : FOR SERVER-SIDE
			...
			$endord			= $start + $length;
			$search			= $_REQUEST['search']["value"];
			$num_rows 		= $this->m_cho_payment->get_AllDataCHOGRPC($PRJCODE, $jrnType, $SPLCODE, $GEJ_STAT, $length, $start);
			$total			= $num_rows;
			$output			= array();
			$output['draw']	= $draw;
			$output['recordsTotal'] = $output['recordsFiltered']= $total;
	...
	function add_processVCASH() // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$r_emp 			= $this->db->query($s_emp)->result();
			foreach($r_emp as $rw_emp) :
				$SPLDESC	= htmlentities($rw_emp->EMP_NAME, ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401 | ENT_HTML5);
			endforeach;
			...
			// START : ADD DOC HISTORY
				$this->load->model('m_updash/m_updash', '', TRUE);
				date_default_timezone_set("Asia/Jakarta");
				$paramTrack 	= array('REF_NUM' 		=> $JournalH_Code,
										'TBLNAME' 		=> "tbl_journalheader_vcash",
										'FLDCODE'		=> "Manual_No",
										'FLDNAME'		=> "JournalH_Code",
										'HISTTYPE'		=> "Penambahan ($GEJ_STAT)");
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			// START : DETAIL JOURNAL
				...
				foreach($_POST['data'] as $d)
				{
					...
					$Base_DebetTOT 		= $Base_DebetTOT + $JournalD_Debet;
					...
					$PPN_AmountTOT 		= $PPN_AmountTOT + $PPN_Amount;

					$PPH_Code			= $d['PPH_Code'];
					$PPH_Perc			= $d['PPH_Perc'];
					$PPH_Amount			= $d['PPH_Amount'];

					$PPH_AmountTOT 		= $PPH_AmountTOT + $PPH_Amount;

					$GJournal_GTotal 	= $Base_DebetTOT + $PPN_AmountTOT - $PPH_AmountTOT;
			...
			// START : UPDATE JOURNAL HEADER
				$upJH3	= "UPDATE tbl_journalheader SET Journal_Amount = $Base_DebetTOT,	PPNH_Amount = $PPN_AmountTOT,	PPHH_Amount = $PPH_AmountTOT,
								GJournal_Total = $GJournal_GTotal
							WHERE JournalH_Code = '$JournalH_Code'";
				$this->db->query($upJH3);

				$upJH3	= "UPDATE tbl_journalheader_cprj SET Journal_Amount = $Base_DebetTOT,	PPNH_Amount = $PPN_AmountTOT,	PPHH_Amount = $PPH_AmountTOT,
								GJournal_Total = $GJournal_GTotal
							WHERE JournalH_Code = '$JournalH_Code'";
				$this->db->query($upJH3);
			// END : UPDATE JOURNAL HEADER
	...
	function add_process() // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			if($ISPERSL == 1)								// Pembayaran Dimuka
			{
				// ADD - HEADER
				...
				// START : DETAIL JOURNAL
					// START : DEBET
						...
						foreach($_POST['data'] as $d)
						{
							...
							$curr_rate			= 1;
							$isDirect			= 1;
							$Ref_Number			= htmlentities($d['Ref_Number'], ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401 | ENT_HTML5);
							$Other_Desc			= htmlentities($d['Other_Desc'], ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401 | ENT_HTML5);
							$Journal_DK			= $JournalD_Pos;
							$Journal_Type		= $Journal_Type;
							$isTax				= $isTax;
	...
	function update_process() // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			// START : UPDATE HEADER
				$projGEJH 	= array('JournalH_Code' 	=> $JournalH_Code,
									'JournalH_Desc' 	=> htmlentities($AH_NOTES, ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401 | ENT_HTML5),
									'Manual_No' 		=> $Manual_No,
									'JournalType' 		=> $JournalType,
									'JournalH_Desc'		=> htmlentities($this->input->post('JournalH_Desc'), ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401 | ENT_HTML5),
									'JournalH_Date'		=> $JournalH_Date,
	...
	function update_processVCASH() // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$JournalH_Desc 	= htmlentities($this->input->post('JournalH_Desc'), ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401 | ENT_HTML5);
			$JOURNL_DESC	= htmlentities($this->input->post('JournalH_Desc'), ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401 | ENT_HTML5);
			$JOURNL_DESC	= htmlentities($this->input->post('JournalH_Desc'), ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401 | ENT_HTML5);
				...
				// START : DETAIL JOURNAL
							...
							// START : POSTING BEBAN
								...
								// START : INSERT INTO JOURNAL DETAIL (BEBAN)					--- DEBET
									$insSQL	= "INSERT INTO tbl_journaldetail (JournalH_Code, Acc_Id, JournalType, proj_Code, JOBCODEID,
													...
													COA_Kredit, COA_Kredit_tax, curr_rate, isDirect, ITM_CODE, ITM_CATEG,
													ITM_VOLM, ITM_UNIT, ITM_PRICE, TAX_DATE, TAX_NO, Other_Desc, Journal_DK, Journal_Type, isTax,
													GEJ_STAT, JournalH_Date, proj_CodeHO, PRJPERIOD, Acc_Name, ISPERSL, Manual_No,
													PPN_Code, PPN_Perc, PPN_Amount, PPH_Code, PPH_Perc, PPH_Amount, PattNum, SPLCODE, SPLDESC, isVerified) VALUES
													('$JournalH_Code', '$Acc_Id', '$JournalType', '$proj_Code', '$JOBCODEID',
													'IDR', $JournalD_Debet, $JournalD_Debet_tax, $JournalD_Kredit, $JournalD_Kredit_tax,
													$Base_Debet, $Base_Debet_tax, $Base_Kredit, $Base_Kredit_tax, $COA_Debet, $COA_Debet_tax,
													$COA_Kredit, $COA_Kredit_tax, 1, 1, '$ITM_CODE', '$ITM_GROUP',
													'$ITM_VOLM', '$ITM_UNIT', '$ITM_PRICE', '$TAX_DATE', '$TAX_NO', '$Other_Desc', 'D', '$Journal_Type', $isTax, 
													'$GEJ_STAT', '$JournalH_Date', '$proj_CodeHO', '$PRJPERIOD', '$Acc_Name', 2, '$Manual_No',
													'$PPN_Code', $PPN_Perc, $PPN_Amount, '$PPH_Code', $PPH_Perc, $PPH_Amount, $PattNum, '$SPLCODE', '$SPLDESC', $isVerified)";
									$this->db->query($insSQL);
							...
							// START : INSERT PPH (JIKA ADA)								--- KREDIT
								if($PPH_Amount > 0)
								{
									...
									if($isPPhFin == 0)
									{
										...
										$totNPPHFinal 		= $totNPPHFinal + $d['PPH_Amount'];
										$isTax 				= 1;
										$insSQL2	= "INSERT INTO tbl_journaldetail (JournalH_Code, Acc_Id, JournalType, proj_Code, JOBCODEID,
															Currency_id, JournalD_Debet, JournalD_Debet_tax, JournalD_Kredit, JournalD_Kredit_tax, 
															Base_Debet, Base_Debet_tax, Base_Kredit, Base_Kredit_tax, COA_Debet, COA_Debet_tax, 
					...
					// START : INSERT INTO JOURNAL DETAIL VCASH (HUTANG SUPPLIER)	--- KREDIT
						...
						*/
						...
						//$totHUTSPL 		= $Journal_Amount + $PPNH_Amount - $PPHH_Amount;
						$totHUTSPL 			= $Journal_Amount + $PPNH_Amount - $totNPPHFinal;
						$JournalD_Kredit 	= $totHUTSPL;
						$Base_Kredit		= $totHUTSPL;
						$COA_Kredit			= $totHUTSPL;
						$isTax 				= 0;
	...
	function update_processCPRJ() // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
						...
						if($AH_ISLAST == 1)
						{
							...
							*/
							...
							// GET ITM_NAME
								...
								$ITM_CATEG 		= '';
								$sqlITMNM		= "SELECT ITM_NAME, ITM_TYPE, ITM_UNIT, ACC_ID_UM AS ACC_ID, ITM_GROUP
													FROM tbl_item_$PRJCODEVW WHERE ITM_CODE = '$ITM_CODE' AND PRJCODE = '$PRJCODE' LIMIT 1";
								$resITMNM		= $this->db->query($sqlITMNM)->result();
								foreach($resITMNM as $rowITMNM) :
									$ITM_NAME	= $rowITMNM->ITM_NAME;
									$ITM_TYPE	= $rowITMNM->ITM_TYPE;
			...
			else 		// IF NEW OR CONFIRMED
			{
				...
				// START : UPDATE JOURNAL HEADER
					$upJH3	= "UPDATE tbl_journalheader SET Journal_Amount = $Base_DebetTOT,	PPNH_Amount = $PPN_AmountTOT,	PPHH_Amount = $PPH_AmountTOT,
									GJournal_Total = $GJournal_GTotal, JournalH_Desc = '$JOURNL_DESC'
								WHERE JournalH_Code = '$JournalH_Code'";
					$this->db->query($upJH3);
  	...
  	function get_AllDataITM() // GOOD
	{
		...
		// START : FOR SERVER-SIDE
			...
			$start			= $_REQUEST['start'];
			$search			= $_REQUEST['search']["value"];
			$num_rows 		= $this->m_cho_payment->get_AllDataITMC($PRJCODE, $length, $start);
			$total			= $num_rows;
			$output			= array();
			$output['draw']	= $draw;
			$output['recordsTotal'] = $output['recordsFiltered']= $total;
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				$sqlJOBPAR		= "SELECT JOBDESC from tbl_joblist_$PRJCODEVW WHERE JOBCODEID = '$JOBPARENT' AND PRJCODE = '$PRJCODE' LIMIT 1";
				...
				if($AMDM_VAL != 0)
				{
					$ADDVOLM_VW = 	"<p class='text-yellow' style='white-space:nowrap'><i class='glyphicon glyphicon-triangle-bottom'></i>
								  		".number_format($AMDM_VOL, 2)."</p>";
					$ADDVALM_VW = 	"<div>
				...
				$JONDESCRIP 	= str_replace("'"," ",$JONDESCRIP);
				...
				// OTHER SETT
					...
					if($disabledB == 0)
					{
						$chkBox		= "<input type='radio' name='chk0' id='chk0".$noU."' value='".$ACC_ID."|".$JONDESCRIP."|".$ITM_CODE."|".$ITM_GROUP."|".$JOBCODEID."|".$ITM_REMVOL."|".$ITM_REMVAL."|".$ITM_UNIT."|".$ITM_LASTP."|".$THEROW."|".$ISVERIFY."|".$r_isLS."' onClick='pickThis0(".$noU.");'/>";
					}
					else
					{
				...
				$RecItemV 	= 	"<input type='hidden' name='urlRec".$noU."' id='urlRec".$noU."' value='".$RecItem."'>
								<a onClick='recountItm(".$noU.");' title='Hitung ulang penggunaan' class='pull-left' style='cursor: pointer'><i class='fa fa-repeat'></i></a>";
				...
				$output['data'][] 	= array("<div style='text-align:center;'>".$chkBox."</div>",
											...
											"<div style='text-align:center;'><span ".$CELL_COL.">".$UNITTYPE."</span></div>",
											"<div style='text-align:right; white-space:nowrap'><span ".$CELL_COL.">".number_format($ITM_VOLM, 2).$ADDVOL_VW.$ADDVOLM_VW."</span></div>",
											"<div style='text-align:right; white-space:nowrap'><span ".$CELL_COL.">".number_format($ITM_BUDG, 2).$ADDVAL_VW.$ADDVALM_VW."</span></div>",
											"<div style='text-align:right;'><span ".$CELL_COL.">".number_format($TREQ_VOL, 2).$TREQVOL_RVW."</span></div>",
											"<div style='text-align:right; white-space: nowrap'>".$RecItemV."&nbsp;&nbsp;&nbsp;<span ".$CELL_COL.">".number_format($TREQ_VAL, 2).$TREQVAL_RVW."</span></div>",
											"<div style='text-align:right;'>".number_format($ITM_REMVOL, 2)."</div>",
											"<div style='text-align:right;'><span ".$CELL_COL.">".number_format($ITM_REMVAL, 2)."</span></div>");
	...
	function chgVOLUME()
	{
		...
		// DAPATKAN BUDGET
			...
			$TOT_UVOL 	= 0;
			$TOT_UVAL 	= 0;
			$s_00		= "SELECT (ITM_VOLM + ADD_VOLM) AS BUDG_VOLM, ITM_PRICE, (ITM_BUDG + ADD_JOBCOST) AS BUDG_AMN
		...
		// DAPATKAN TOTAL DIMINTA
			...
			$ITM_UVAL 		= 0;
			$ITM_UVAL_R 	= 0;
			$sqlJOBD		= "SELECT ITM_VOLM, ITM_BUDG, PR_VOL, PR_VOL_R, PR_CVOL, PO_VOL, PO_VOL_R, PO_CVOL, PO_VAL, PO_VAL_R, PO_CVAL,
										UM_VOL, UM_VAL, UM_VOL_R, UM_VAL_R,
										WO_VOL, WO_VAL, WO_VOL_R, WO_VAL_R, WO_CVOL, WO_CVAL, OPN_VOL, OPN_VAL, OPN_VOL_R, OPN_VAL_R,
										VCASH_VOL, VCASH_VAL, VCASH_VOL_R, VCASH_VAL_R, VLK_VOL, VLK_VAL, VLK_VOL_R, VLK_VAL_R,
										PPD_VOL, PPD_VAL, PPD_VOL_R, PPD_VAL_R,
			...
			foreach($resJOBD as $rowJOBD) :
				...
				$PR_VOL			= $rowJOBD->PR_VOL;
				$PR_CVOL		= $rowJOBD->PR_CVOL;
				$PR_VOL_R		= $rowJOBD->PR_VOL_R;
				$PO_VOL			= $rowJOBD->PO_VOL;
				$PO_VAL			= $rowJOBD->PO_VAL;
				$PO_VOL_R		= $rowJOBD->PO_VOL_R;
				$PO_VAL_R		= $rowJOBD->PO_VAL_R;
				$PO_CVOL		= $rowJOBD->PO_CVOL;
				$PO_CVAL		= $rowJOBD->PO_CVAL;
				$WO_VOL			= $rowJOBD->WO_VOL;
				$WO_VAL			= $rowJOBD->WO_VAL;
				$WO_VOL_R		= $rowJOBD->WO_VOL_R;
				$WO_VAL_R		= $rowJOBD->WO_VAL_R;
				$WO_CVOL		= $rowJOBD->WO_CVOL;
				$WO_CVAL		= $rowJOBD->WO_CVAL;
				$VCASH_VOL		= $rowJOBD->VCASH_VOL;
				$VCASH_VAL		= $rowJOBD->VCASH_VAL;
				$VCASH_VOL_R	= $rowJOBD->VCASH_VOL_R;
				$VCASH_VAL_R	= $rowJOBD->VCASH_VAL_R;
				$VLK_VOL		= $rowJOBD->VLK_VOL;
				$VLK_VAL		= $rowJOBD->VLK_VAL;
				$VLK_VOL_R		= $rowJOBD->VLK_VOL_R;
				$VLK_VAL_R		= $rowJOBD->VLK_VAL_R;
				$PPD_VOL		= $rowJOBD->PPD_VOL;
				$PPD_VAL		= $rowJOBD->PPD_VAL;
				$PPD_VOL_R		= $rowJOBD->PPD_VOL_R;
				$PPD_VAL_R		= $rowJOBD->PPD_VAL_R;
				$AMD_VOL		= $rowJOBD->AMD_VOL;
				$AMD_VAL		= $rowJOBD->AMD_VAL;
				$AMDM_VOL		= $rowJOBD->AMDM_VOL;
				$AMDM_VAL		= $rowJOBD->AMDM_VAL;

				$TOT_BVOL 		= $AMD_VOL+$ITM_VOLM-$AMDM_VOL;
				$TOT_BVAL 		= $AMD_VAL+$ITM_BUDG-$AMDM_VAL;

				/*$ITM_UVOL 	= $PO_VOL+$WO_VOL+$VCASH_VOL+$VLK_VOL+$PPD_VOL-$PO_CVOL-$WO_CVOL;
				$ITM_UVAL 		= $PO_VAL+$WO_VAL+$VCASH_VAL+$VLK_VAL+$PPD_VAL-$PO_CVAL-$WO_CVAL;

				$ITM_UVOL_R		= $PO_VOL_R+$WO_VOL_R+$VCASH_VOL_R+$VLK_VOL_R+$PPD_VOL_R;
				$ITM_UVAL_R		= $PO_VAL_R+$WO_VAL_R+$VCASH_VAL_R+$VLK_VAL_R+$PPD_VAL_R;*/

				$ITM_UVOL 		= $PR_VOL+$WO_VOL+$VCASH_VOL+$VLK_VOL+$PPD_VOL-$PR_CVOL-$WO_CVOL;
				$ITM_UVAL 		= $PO_VAL+$WO_VAL+$VCASH_VAL+$VLK_VAL+$PPD_VAL-$PO_CVAL-$WO_CVAL;

				$ITM_UVOL_R		= $PR_VOL_R+$WO_VOL_R+$VCASH_VOL_R+$VLK_VOL_R+$PPD_VOL_R;
				$ITM_UVAL_R		= $PO_VAL_R+$WO_VAL_R+$VCASH_VAL_R+$VLK_VAL_R+$PPD_VAL_R;
			endforeach;
			...
			if($ITM_UVAL_R > 0)
			{
				$RES_VW		= 	"<div style='white-space:nowrap;'>
									<span class='text-red' style='white-space:nowrap;'><i class='glyphicon glyphicon-chevron-down'></i>
							  		".number_format($ITM_UVAL_R, 2)."</span>
							  	</div>";
			}

			$REM_VOLM 		= $TOT_BVOL - $ITM_UVOL - $ITM_UVOL_R;
			$REM_AMN 		= $TOT_BVAL - $ITM_UVAL - $ITM_UVAL_R;
	...
	function vtlk_all() // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data['backURL'] 	= site_url('c_finance/c_cho70d18/vtlk/?id='.$this->url_encryption_helper->encode_url($PRJCODE));
	...
	function delRVCASH()
	{
		...
		$delSQL		= "DELETE FROM tbl_journaldetail_pd WHERE JournalD_Id = $PATT_ID AND JournalH_Code = '$JRN_NUM' AND proj_Code = '$PRJCODE'";
		$this->db->query($delSQL);
  	...
  	function get_AllDataJRNREV() // GOOD
	{
		...
		// START : FOR SERVER-SIDE
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				$secDelIcut 		= base_url().'index.php/__l1y/trashDOC/?id=';
				$secVoid 			= base_url().'index.php/__l1y/trashCHO_NEW/?id=';
				$delID 				= "$secDelIcut~tbl_journalheader_revers~tbl_journaldetail_revers~JournalH_Code~$JournalH_Code~proj_Code~$PRJCODE";
				$voidID 			= "$secVoid~tbl_journalheader~tbl_journaldetail~JournalH_Code~$JournalH_Code~proj_Code~$PRJCODE";
				...
				$isLockD 	= "<i class='fa fa-lock margin-r-5'></i>";
				$disabled 	= "";
				if($isLock == 1) $disabled = "disabled='disabled'";
				$secPrint	= "<input type='hidden' name='urlPrint".$noU."' id='urlPrint".$noU."' value='".$secPrint1."'>
								<input type='hidden' name='urlDel".$noU."' id='urlDel".$noU."' value='".$delID."'>
								<input type='hidden' name='urlViewpaid".$noU."' id='urlViewpaid".$noU."' value='".$secViewpaid."'>
								<input type='hidden' name='urlVoid".$noU."' id='urlVoid".$noU."' value='".$voidID."'>
								...
								<a href='javascript:void(null);' class='btn btn-warning btn-xs' onClick='voidDOC(".$noU.")' title='Void' $disabled>
									<i class='glyphicon glyphicon-off'></i>
								</a>
								</label>";
  	...
  	function get_AllDataJRNDET() // GOOD
	{
		...
		// START : FOR SERVER-SIDE
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				$ACC_NAMEV 		= htmlentities($ACC_NAME, ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401 | ENT_HTML5);
	...
	function getGenerateUC_VTLK()
    {
		$post 		= $this->input->post();
		$MenuCode	= $post['MenuCode'];
		$PRJCODE	= $post['PRJCODE'];
        $data 	= $this->m_cho_payment->GenerateUC($MenuCode, $PRJCODE);
	...
	function get_ITMSRC()
	{
		...
		$data 		= $this->m_cho_payment->getITM_SRC($PRJCODE)->result();
		echo json_encode($data);
	}
}