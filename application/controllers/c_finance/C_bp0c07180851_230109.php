...
class C_bp0c07180851 extends CI_Controller  
{
  	...
  	function get_AllData() // GOOD
	{
		...
		// START : FOR SERVER-SIDE
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				$CB_NOTES		= htmlspecialchars($dataI['CB_NOTES'], ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401 | ENT_HTML5);
	...
	function add_process() // USE
	{
		...
		{			
			...
			$CB_DPAMOUNT 	= 0;
			$CB_NOTES	 	= htmlspecialchars($this->input->post('CB_NOTES'), ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401 | ENT_HTML5);
			...
			if(isset($_POST['dataACC']))
			{
				...
				{
					$CB_TOTAM		= $CB_TOTAM + $d['JournalD_Amount'];
				}
			}
			...
			if(isset($_POST['dataACC']))
			{
				...
				foreach($_POST['dataACC'] as $d)
				{
					...
					$insSQL	= "INSERT INTO tbl_journaldetail_bp (JournalH_Code, Acc_Id, Acc_Id_Cross, JournalType, proj_Code,
									...
									1, '$Base_Debet', '$Ref_Number', '$Other_Desc', 'D', 'BP', 0, 
						...
						foreach($_POST['data'] as $d)
						{
							...
							elseif($CB_SOURCE == 'VCASH')
							{
								...
								$updVC2 	= "UPDATE tbl_journalheader_vcash SET GEJ_STAT_VCASH = 1, Journal_AmountReal = Journal_AmountReal + $CBD_AMOUNT,
													BP_CODE = '$CB_CODE', BP_DATE = '$CB_DATE'
												WHERE JournalH_Code = '$CBD_DOCNO'";
					...
					// START : JOURNAL DETAIL
						...
						if(isset($_POST['data']))
						{
							foreach($_POST['data'] as $d)
							{
								...
								$CB_CATEG		= $d['CB_CATEG'];
								$CBD_DESC 		= htmlspecialchars($d['CBD_DESC'], ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401 | ENT_HTML5);
								...
								if($CB_SOURCE == 'VCASH')
								{
									...
									foreach($r_ACCID as $rw_ACCID):
										...
										$REF_REFNOTES 	= htmlspecialchars($rw_ACCID->Other_Desc, ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401 | ENT_HTML5);
									endforeach;
								}
								elseif($CB_SOURCE == 'PD')
								{
								...
								elseif($CB_SOURCE == 'PINV')
								{
									...
									foreach($r_TTK as $rw_TTK):
										...
										$REF_REFNOTES 	= htmlspecialchars($rw_TTK->TTK_DESC, ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401 | ENT_HTML5);
										...
										if($REF_REFCATEG == 'OPN')
										{
											$s_PPH 	= "SELECT TAXCODE2 FROM tbl_wo_detail WHERE WO_NUM = '$REF_REFNUM2'";
										...
										{
											$s_PPH 	= "SELECT TAXCODE2 FROM tbl_po_detail WHERE PO_NUM = '$REF_REFNUM2'";
											$r_PPH 	= $this->db->query($s_PPH)->result();
											foreach($r_PPH as $rw_PPH):
												$TAXCODE2 	= $rw_PPH->TAXCODE2;
											endforeach;
  	...
  	function get_AllDataINV() // GOOD
	{
		...
		// START : FOR SERVER-SIDE
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				$chkBox			= "<input type='checkbox' name='chk0' value='".$INV_NUM."|".$INV_CODE."|".$PRJCODE."|".$INV_AMOUNT."|".$INV_AMOUNT_PPN."|".$INV_AMOUNT_PPH."|".$INV_AMOUNT_DPB."|".$INV_AMOUNT_RET."|".$INV_AMOUNT_POT."|".$INV_AMOUNT_OTH."|".$INV_AMOUNT_TOT."|".$INT_PAIDTOT."|".$INV_ACC_OTH."|".$INV_PPN."|".$PPN_PERC."|".$INV_PPH."|".$PPH_PERC."|".$INV_NOTES."|".$SPLCODE."' onClick='pickThis0(this);'/>";
		...
		// START : FOR SERVER-SIDE
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				$output['data'][] 	= array($noU,
										  	...
										  	"".$Invoice_AmountV."<input type='hidden' class='form-control' style='text-align:right' name='PD_REM_AMOUNT".$noU."' id='PD_REM_AMOUNT".$noU."' value='0'><input type='hidden' class='form-control' style='text-align:right' name='PD_PAYMENT".$noU."' id='PD_PAYMENT".$noU."' value='".$Invoice_Amount."' onBlur='chgAmountPD(this,".$noU.")' onKeyPress='return isIntOnlyNew(event);'><input type='hidden' name='dataPD[".$noU."][PD_PAYMENT]' id='dataPD".$noU."PD_PAYMENT' value='".$Invoice_Amount."' class='form-control' style='max-width:150px;' >",
  	...
  	function get_AllDataDP() // GOOD
	{
		...
		// START : FOR SERVER-SIDE
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				$chkBox				= "<input type='checkbox' name='chk0' value='".$INV_NUM."|".$INV_CODE."|".$PRJCODE."|".$INV_AMOUNT."|".$INV_AMOUNT_PPN."|".$INV_AMOUNT_PPH."|".$INV_AMOUNT_DPB."|".$INV_AMOUNT_RET."|".$INV_AMOUNT_POT."|".$INV_AMOUNT_OTH."|".$INV_TOTAL."|".$INV_AMOUNT_PAID."|".$INV_ACC_OTH."|".$INV_PPN."|".$PPN_PERC."|".$INV_PPH."|".$PPH_PERC."|".$INV_NOTES."|".$SPLCODE."' onClick='pickThis0(this);'/>";
