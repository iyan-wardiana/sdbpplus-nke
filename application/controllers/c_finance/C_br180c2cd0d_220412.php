...
class C_br180c2cd0d extends CI_Controller  
{
  	...
  	function get_AllData() // GOOD
	{
		...
		// START : FOR SERVER-SIDE
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				$secPrint	= site_url('c_finance/c_br180c2cd0d/prntD/?id='.$this->url_encryption_helper->encode_url($BR_NUM));
	...
	function add_process() // OK
	{
		...
		{			
			...
			$BR_NOTES	 	= addslashes($this->input->post('BR_NOTES'));
	...
	function inb180c2g() // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data["countBP"] 	= $this->m_bank_receipt->count_all_BR_inb();
			$data['vwBP'] 		= $this->m_bank_receipt->get_last_BR_inb()->result();
	...
	function update_process_inb() // OK
	{
		...
		{			
			...
			$AH_NOTES		= $this->input->post('BR_NOTES');
			$PR_MEMO		= '';
			$AH_ISLAST		= $this->input->post('IS_LAST');
						...
						// START : JOURNAL DETAIL
							...
							// START : KREDIT
								foreach($_POST['data'] as $d)
								{
									...
									$sqlL_D			= "SELECT Acc_Id, Other_Desc, Manual_No FROM tbl_journaldetail
														WHERE JournalH_Code = '$DocumentNo' AND  Journal_DK = 'D'";
									$resL_D 		= $this->db->query($sqlL_D)->result();					
									foreach($resL_D as $rowL_D):
										$rwDet 		= $rwDet+1;
										$ACC_NUM	= $rowL_D->Acc_Id;
							...
							// START : DEBET
								...
								$sqlGEJDD = "INSERT INTO tbl_journaldetail (JournalH_Code, JournalH_Date, Acc_Id, proj_Code, Currency_id, 
												...
												Journal_DK, Other_Desc, oth_reason, Ref_Number, Acc_Name, proj_CodeHO, JournalType, LastUpdate)
											VALUES ('$JournalH_Code', '$JournalH_Date', '$Acc_ID', '$PRJCODE', 'IDR', $TOT_RECEIPT,
												$TOT_RECEIPT, $TOT_RECEIPT, 'Default', 1, 0, 'D', '$BR_NOTES', 'Pengembalian PD $collManNo', '$Manual_No', '$Acc_Name', '$proj_CodeHO', 'BR', '$AH_APPROVED')";
