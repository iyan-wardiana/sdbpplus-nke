				...
				if($AH_ISLAST == 1)
				{
					...
					// UPDATE OTHER ORD_ID
							...
							foreach($_POST['data'] as $d)
							{
								...
								// CEK SAME ORD_ID
									...
									foreach($rORID as $rwORID):
										$nxX 	= $nxX+1;
										$JID_X 	= $rwORID->JOBCODEID;
										$sUPD 	= "UPDATE tbl_joblist_detail SET ORD_ID = $nxX WHERE JOBCODEID = '$JID_X' AND PRJCODE = '$PRJCODE'";
										$this->db->query($sUPD);
									endforeach;
						...
						elseif($AMD_CATEG == 'OTH') // Other
						{
							...
							// END : UPDATE OVER BUDGET (PLUS)

							// START : UPDATE SUBTITUTOR BUDGET (MINUS)
								foreach($_POST['dataSUB'] as $ds)
								{
									// UNTUK KONDISI ITEM PENGGANTI, SAMA DENGAN MELAKUKAN AMANDEMEN MINUS
									...
									// UPDATE TO WBS / JOBLIST AND JOBDEATIL. LAST CHECKED : 21-10-29
										$paramWBS 	= array('ADD_VOLM' 		=> $ADD_VOLM,
															...
															'AMD_CLASS'		=> $AMD_CLASS,
															'PRJCODE'		=> $PRJCODE);
										$this->m_project_amd->updateWBSMINUS($paramWBS);
								}
							// END : UPDATE SUBTITUTOR BUDGET (MINUS)
  	...
  	function get_AllDataJLD() // GOOD 
	{
		...
		// START : FOR SERVER-SIDE
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				$ITM_REMV 	= 0;
				$ITM_PRICE 	= 0;
				$s_JID 		= "SELECT JOBCODEID, (ITM_VOLM - REQ_VOLM) AS ITM_REMV, ITM_LASTP FROM tbl_joblist_detail
				...
				{
					$chkBox	= "<input type='checkbox' name='chk0' value='".$JOBCODEID."|".$JCODEH_0."|".$JDESCH_0."|".$ITM_UNIT."|".$nwJID."|".$ITM_REMV."|".$ITM_PRICE."' onClick='pickThis0(this);'/>";
					$fntBld = " style='font-weight:bold;'";
				}
  	...
  	function get_AllDataJLDSUBS() // GOOD
	{
		...
		// START : FOR SERVER-SIDE
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				{
					$chkBox	= "<input type='checkbox' name='chk3' value='".$JOBID_SEL."|".$JOBCODEID."|".$JCODEH_0."|".$JDESCH_0."|".$ITM_UNIT."|".$nwJID."|".$ITM_REMV."|".$ITM_PRICE."' onClick='pickThis3(this);'/>";
					$fntBld = " style='font-weight:bold;'";
				}
