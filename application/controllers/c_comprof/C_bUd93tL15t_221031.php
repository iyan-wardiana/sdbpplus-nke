...
class C_bUd93tL15t extends CI_Controller  
{
  	...
  	function get_AllDataJL() // GOOD
	{
        ...
        $s_RAPSTAT	= "SELECT RAPT_STAT, RAP_STAT FROM tbl_project WHERE PRJCODE = '$PRJCODE'";
        $r_RAPSTAT      = $this->db->query($s_RAPSTAT)->result();
        foreach($r_RAPSTAT as $rw_RAPSTAT) :
            $RAPT_STAT	= $rw_RAPSTAT->RAPT_STAT;
            $RAPP_STAT 	= $rw_RAPSTAT->RAP_STAT;
        endforeach;
        /*$RAPT_STAT 	= 1;
        $RAPP_STAT 	= 0;*/
		...
		// START : FOR SERVER-SIDE
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				if($ISLASTH == 1)
				{
					$s_RAPRC	= "SELECT SUM(RAPT_JOBCOST) AS TOT_RAPTITM FROM tbl_joblist_detail_$PRJCODEVW
									WHERE JOBPARENT LIKE '$JOBCODEID%' AND PRJCODE = '$PRJCODE' AND ISLAST = 1";
					$r_RAPTC 	= $this->db->query($s_RAPRC)->result();
					foreach($r_RAPTC as $rw_RAPT) :
						$TOT_RAPTITM	= $rw_RAPT->TOT_RAPTITM;
					endforeach;
					if($RAPT_JOBCOST != $TOT_RAPTITM)
					{
				...
				if($ISLAST == 1)
				{
					$s_DOBL		= "tbl_joblist_detail_$PRJCODEVW WHERE JOBPARENT = '$JOBPARENT' AND ITM_CODE = '$ITM_CODE' AND PRJCODE = '$PRJCODE'";
					...
					if($RAPP_STAT == 1)
					{
						$btnUpdRAPP		= "<a href='javascript:void(null);' class='label label-".$STATCOL."' style='font-size:12px' onClick='updRAPPVW(".$noU.")'>".number_format($RAPP_JOBCOST,2)."</a>";
					}
				}
				...
				/*$RAPT_LVW		= "<a href='javascript:void(null);' class='btn btn-danger btn-xs'>
										<i class='fa fa-unlock'></i>
									</a>";
				$RAPP_LVW		= "<a href='javascript:void(null);' class='btn btn-danger btn-xs' disabled>
										<i class='fa fa-unlock'></i>
									</a>";*/
				$RAPT_LVW		= "";
				$RAPP_LVW		= "";
				/*if($RAPT_ISLOCK == 1)
				{
					$RAPT_LVW	= "<a href='javascript:void(null);' class='btn btn-".$STATCOL." btn-xs'>
										<i class='fa fa-lock'></i>
									</a>";
				}
				if($RAPP_ISLOCK == 1)
				{
					$RAPP_LVW	= "<a href='javascript:void(null);' class='btn btn-".$STATCOL." btn-xs'>
										<i class='fa fa-lock'></i>
									</a>";
				}*/
				...
				$output['data'][] 	= array("<div style='text-align:center;'>".$secAddHV."</span>",
											...
											"<div style='text-align:right;'><span ".$CELL_COL.">".number_format($RAPP_VOLM, 2).$ADDVOL_VW."</span></div>",
	...
	function updRAPP()
	{
		...
		// START : UPDATE PROCEDURE
			$s_01 		= 	"UPDATE tbl_joblist_detail SET ITM_VOLM = '$RAPP_VOL', ITM_PRICE = '$RAPP_PRICE', ITM_LASTP = '$RAPP_PRICE', ITM_AVGP = '$RAPP_PRICE', ITM_BUDG = '$ITM_TOTAL',
								RAPP_UPDATER = '$DefEmp_ID', RAPP_UPDATED = '$UPDATED'
			...
			// START : UPDATE ALL HEADER IN HIERARKI
				...
				for($i=0;$i<$arrJIDPC;$i++)
				{
					...
					$TOTDET 		= 0;
					$s_TOTDET 		= "SELECT SUM(ITM_BUDG) AS TOTDET FROM tbl_joblist_detail_$PRJCODEVW WHERE JOBPARENT LIKE '$JOBPARENT%' AND ISLAST = 1";
					$r_TOTDET 		= $this->db->query($s_TOTDET)->result();
					foreach($r_TOTDET as $rw_TOTDET) :
