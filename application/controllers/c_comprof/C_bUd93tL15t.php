...
class C_bUd93tL15t extends CI_Controller  
{
  	...
  	function get_AllData() // GOOD
	{
		...
		// START : FOR SERVER-SIDE
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				// CEK COA
				$sqlCOAC	= "tbl_chartaccount_$PRJCODEVW WHERE PRJCODE = '$PRJCODE'";
				...
				$output['data'][] 	= array("<strong style='font-size:13px'>".$PRJCODE."</strong>",
											...
											"<strong><i class='fa fa-building margin-r-5'></i>".strtoupper($PRJNAME)." </strong><br>
		...
		// START : FOR SERVER-SIDE
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				// PRJ BUDGET DETAIL
					...
					$USED_VOL	= 0;
					$USED_VAL	= 0;
					$sqlBudg 	= "SELECT 	SUM(ITM_BUDG) AS ITM_BUDG,
											SUM(TSFRAP_VAL) AS ITM_BUDG_TSF,
											SUM(ADD_JOBCOST-ADDM_JOBCOST) AS ITM_ADD,
											SUM(OPN_VOL+VCASH_VOL+VLK_VOL+PPD_VOL) AS USED_VOL,
											SUM(OPN_VOL_R+VCASH_VOL_R+VLK_VOL_R+PPD_VOL_R) AS USED_VOL_R,
											SUM(OPN_VAL+VCASH_VAL+VLK_VAL+PPD_VAL) AS USED_VAL,
											SUM(OPN_VAL_R+VCASH_VAL_R+VLK_VAL_R+PPD_VAL_R) AS USED_VAL_R
					...
					foreach($resBudg as $rowBT) :
						$TOTBP		= $rowBT->ITM_BUDG;
						$TOTBP_TSF	= $rowBT->ITM_BUDG_TSF;
						$TOTADD		= $rowBT->ITM_ADD;
						$USED_VOL	= $rowBT->USED_VOL;
						$USED_VAL	= $rowBT->USED_VAL;
					endforeach;
					$TOTBUDG	= $TOTBP + $TOTADD + $TOTBP_TSF;
					$TOTBUDGP 	= $TOTBUDG;
					if($TOTBUDG == 0)
						$TOTBUDGP	= 1;
					
					$PERCUSEDV	= round($USED_VAL / $TOTBUDGP * 100, 2);
					$PERCUSED	= round($USED_VAL / $TOTBUDGP * 100);
					// PERCUSED akan diganti dari data persentase progress mingguan
					if($PERCUSED <= 25)
						$GRFCOL	= 'success';
					elseif($PERCUSED <= 50)
				...
				{
					$secSDOWN	= "<a href='javascript:void(null);' onClick='shutDRAP(\"$PRJCODE\")' class='btn bg-red btn-xs' title='Non Aktifkan Budget'>
				...
				$output['data'][] 	= array("<strong style='font-size:13px'>".$PRJCODE."</strong>",
											...
											<div class='row' style='white-space:nowrap'>
												...
												<div class='col-sm-5 col-xs-12'>
													<div class='description-block border-right'>
														<h5 class='description-header'>".number_format($USED_VAL,0)."</h5>
														<span class='description-text'>PENGGUNAAN (Rp)</span>
													</div>
												</div>
											...
											"<div style='text-align:center; white-space:nowrap'><span class='label label-".$STATCOL."' style='font-size:12px'>".$isActDesc."</span></div>",
											"<div style='text-align:center;'>".$secPrint.$secSDOWN."</div>");
	...
	function add_process() // OK : PEMBUATAN ANGGARAN BUKAN KANTOR / CABANG
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$PRJRAPT 		= $this->input->post('PRJRAPT');
			...
			// CREATE JOBLIST VIEW
				...
				$s_jcd 			= "CREATE VIEW tbl_jobcreate_detail_$PRJCODEVW AS
									SELECT * FROM tbl_jobcreate_detail 
									WHERE PRJCODE = '$PRJCODE'";
				$this->db->query($s_jcd);
				...
				$s_jrnH			= "CREATE VIEW tbl_journalheader_$PRJCODEVW AS
									SELECT * FROM tbl_journalheader 
									WHERE proj_Code = '$PRJCODE'";
				$this->db->query($s_jrnH);
				
				$s_jrnD			= "CREATE VIEW tbl_journaldetail_$PRJCODEVW AS
									SELECT * FROM tbl_journaldetail 
									WHERE proj_Code = '$PRJCODE'";
	...
	function u180c2gdt()
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data['default']['PRJCOST_PPN']	= $getproject->PRJCOST_PPN;
			$data['default']['PRJCOST2_PPN']= $getproject->PRJCOST2_PPN;
	...
	function gl180c21JL() // OK
	{
		...
		$data['form_downl']= site_url('c_comprof/c_bUd93tL15t/downLJL');
		if($pgFrom == 'HO')
		{
			$data['backURL']	= site_url('c_comprof/c_bUd93tL15t/i180c2gdx_MN246/?id='.$this->url_encryption_helper->encode_url($PRJCODE_HO));
			$mnCode 			= 'MN246';
	...
	function do_uploadJL() // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			elseif($BOQH_TYPE == 3)
			{
				$target_path 	= "application/xlsxfile/import_amd/".$filename;  // change this to the correct site path					
				$myPath 		= "application/xlsxfile/import_amd/$filename";
			}
	...
	function upd1d0ebb2() // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data['default']['PRJCODE'] 	= $getjoblist->PRJCODE;
			$data['default']['JOBDESC'] 	= $getjoblist->JOBDESC;
			$data['default']['JOBGRP'] 		= $getjoblist->ITM_GROUP;
			$data['default']['JOBUNIT'] 	= $getjoblist->ITM_UNIT;
			$data['default']['JOBLEV'] 		= $getjoblist->IS_LEVEL;
			$data['default']['BOQ_VOLM'] 	= $getjoblist->BOQ_VOLM;
			$data['default']['BOQ_PRICE'] 	= $getjoblist->BOQ_PRICE;
			$data['default']['JOBVOLM'] 	= $getjoblist->ITM_VOLM;
			$data['default']['PRICE'] 		= $getjoblist->ITM_PRICE;
			$data['default']['JOBCOST'] 	= $getjoblist->ITM_BUDG;
			$data['default']['ISLASTH'] 	= $getjoblist->ISLASTH;
			$data['default']['ISLAST'] 		= $getjoblist->ISLAST;
			$data['default']['ITM_GROUP'] 	= $getjoblist->ITM_GROUP;
			...
			$data['default']['GROUP_CATEG'] = $GROUP_CATEG;
			$data['default']['ISHEADER'] 	= $getjoblist->ISLASTH;
			$data['default']['Patt_Number']	= $getjoblist->Patt_Number;
	...
	{
		$JOBPARID	= $this->input->post('JOBPARID');
		$PRJCODE	= $_GET['id'];
		...
		// START : FOR SERVER-SIDE
			...
			$start			= $_REQUEST['start'];
			$search			= $_REQUEST['search']["value"];
			/*$num_rows 		= $this->m_budget->get_AllDataJLC($PRJCODE, $JOBPARID, $search);
			$total			= $num_rows;*/
			$output			= array();
			$output['draw']	= $draw;
			//$output['recordsTotal'] = $output['recordsFiltered']= $total;
			...
			$query 			= $this->m_budget->get_AllDataJLL($PRJCODE, $JOBPARID, $search, $length, $start, $order, $dir); 
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				$ISLAST			= $dataI['ISLAST'];
				$ISLASTH		= $dataI['ISLASTH'];
				$RAPT_VOLM		= $dataI['RAPT_VOLM'];
				$RAPT_PRICE		= $dataI['RAPT_PRICE'];
				$RAPT_JOBCOST	= $dataI['RAPT_JOBCOST'];
				$RAPT_JOBCOSTDET= $dataI['RAPT_JOBCOSTDET'];
				$RAPP_VOLM		= $dataI['ITM_VOLM'];
				$RAPP_PRICE		= $dataI['ITM_PRICE'];
				$RAPP_JOBCOST	= $dataI['ITM_BUDG'];
				$RAPT_ISLOCK	= $dataI['RAPT_ISLOCK'];
				$RAPT_LOCKER	= $dataI['RAPT_LOCKER'];
				$RAPT_LOCKED	= $dataI['RAPT_LOCKED'];
				$RAPP_ISLOCK	= $dataI['RAPP_ISLOCK'];
				$RAPP_LOCKER	= $dataI['RAPP_LOCKER'];
				$RAPP_LOCKED	= $dataI['RAPP_LOCKED'];

				$FPA_VOL 		= $dataI['FPA_VOL'];
				$FPA_VOL_R		= $dataI['FPA_VOL_R'];
				$PR_VOL			= $dataI['PR_VOL'];
				$PR_VOL_R 		= $dataI['PR_VOL_R'];
				$PR_CVOL		= $dataI['PR_CVOL'];
				$PO_VAL			= $dataI['PO_VAL'];
				$PO_VAL_R 		= $dataI['PO_VAL_R'];
				$PO_CVAL		= $dataI['PO_CVAL'];
				$WO_VOL			= $dataI['WO_VOL'];
				$WO_VOL_R 		= $dataI['WO_VOL_R'];
				$WO_CVOL 		= $dataI['WO_CVOL'];
				$WO_VAL			= $dataI['WO_VAL'];
				$WO_VAL_R 		= $dataI['WO_VAL_R'];
				$WO_CVAL 		= $dataI['WO_CVAL'];
				$VCASH_VOL		= $dataI['VCASH_VOL'];
				$VCASH_VOL_R 	= $dataI['VCASH_VOL_R'];
				$VCASH_VAL		= $dataI['VCASH_VAL'];
				$VCASH_VAL_R 	= $dataI['VCASH_VAL_R'];
				$VLK_VOL		= $dataI['VLK_VOL'];
				$VLK_VOL_R 		= $dataI['VLK_VOL_R'];
				$VLK_VAL		= $dataI['VLK_VAL'];
				$VLK_VAL_R 		= $dataI['VLK_VAL_R'];
				$PPD_VOL		= $dataI['PPD_VOL'];
				$PPD_VOL_R 		= $dataI['PPD_VOL_R'];
				$PPD_VAL		= $dataI['PPD_VAL'];
				$PPD_VAL_R 		= $dataI['PPD_VAL_R'];

				$TREQ_VOL 		= $FPA_VOL+$FPA_VOL_R+$PR_VOL+$PR_VOL_R-$PR_CVOL+$WO_VOL+$WO_VOL_R+$VCASH_VOL+$VCASH_VOL_R+$VLK_VOL+$VLK_VOL_R+$PPD_VOL+$PPD_VOL_R;
				$TREQ_VAL 		= $PO_VAL+$PO_VAL_R-$PO_CVAL+$WO_VAL+$WO_VAL_R+$VCASH_VAL+$VCASH_VAL_R+$VLK_VAL+$VLK_VAL_R+$PPD_VAL+$PPD_VAL_R;

				$ADD_VOLM 		= $dataI['AMD_VOL'];
				$ADD_JOBCOST	= $dataI['AMD_VAL'];
				$ADD_PRICE		= $dataI['ADD_PRICE'];
				$ADDM_VOLM 		= $dataI['AMDM_VOL'];
				$ADDM_JOBCOST	= $dataI['AMDM_VAL'];
				...
				if($ISLASTH == 1)
				{
					if($RAPT_JOBCOST != $RAPT_JOBCOSTDET)
					{
						$DEVRAPT 		= $RAPT_JOBCOST - $TOT_RAPTITM;
						if($DEVRAPT > 0)
						{
							$DEVRAPTFA 	= "<div style='white-space:nowrap;'>
											<span class='text-red' style='white-space:nowrap;'><i class='glyphicon glyphicon-chevron-up'></i>
									  		".number_format($DEVRAPT, 2)."</span>
									  	</div>";
						}
						elseif($DEVRAPT < 0)
						{
						...
						$reCountRAPT	= 	"<label style='white-space:nowrap'>
											   	<a href='javascript:void(null);' class='btn btn-warning btn-xs' onClick='syncJobRAPT(".$noU.")' title='".number_format($TOT_RAPTITM,2)."'>
				...
				//$TOT_JOBCOST	= $JOBCOST + $ADD_JOBCOST - $ADDM_JOBCOST - $ITM_USED_AM;
				$TOT_JOBCOST	= $JOBCOST + $ADD_JOBCOST - $ADDM_JOBCOST;
				$TOT_JOBCOSTP 	= $TOT_JOBCOST ?: 1;
				$percBDG		= $ITM_USED_AM / $TOT_JOBCOSTP * 100;		// Used Percentation
				$STATCOL1		= 'success';
				if($percBDG > 85)
				...
				if($ISLAST == 0)	// HEADER
				{
					$DEV_BOQRAPT= $BOQ_JOBCOST - $RAPT_JOBCOST;
					$DEV_BOQRAPP= $BOQ_JOBCOST - $RAPP_JOBCOST;
					$STATCOL	= 'primary';
					$CELL_COL	= "style='font-weight:bold; white-space:nowrap'";
					...
					$secPrint	= 	"<input type='hidden' name='urlUpdate".$noU."' id='urlUpdate".$noU."' value='".$secUpd."'>
									...
									<label style='white-space:nowrap'>
									   	...
									   	<a href='javascript:void(null);' class='btn btn-info btn-xs' onClick='syncJob(".$noU.")' style='display: none'>
											<i class='glyphicon glyphicon-refresh'></i>
										</a>
									</label>";
					$reCountRAP	= 	"";
					$reCountRAB	= 	"<label style='white-space:nowrap'>
									   	<a href='javascript:void(null);' class='btn btn-info btn-xs' onClick='syncJobRAB(".$noU.")'>
											<i class='glyphicon glyphicon-refresh'></i>
										</a>
									</label>";
				...
				else
				{
					...
					$RecItem 	= "$secRecount~$PRJCODE~$JOBCODEID~$ITM_CODE~$JOBDESC";
					$RecItemV 	= 	"<input type='hidden' name='urlRec".$noU."' id='urlRec".$noU."' value='".$RecItem."'>
									<a onClick='recountItm(".$noU.");' title='Hitung ulang' class='pull-left' style='cursor: pointer'><i class='fa fa-repeat'></i></a>";
					...
					$s_JC 	= "tbl_jobcreate_detail_$PRJCODEVW WHERE JOBCODEID = '$JOBCODEID'";
					...
					if($r_JC == 0)
					{
						...
						if($PRJ_ISLOCK == 1)
						{
							$noDet 	= "<i class='fa fa-exclamation-triangle text-red' onClick='alertLOCK()' style='cursor: pointer' title='Tidak ada di dalam Analisa Pekerjaan'></i>";
						}
					}

					$s_DOBL		= "tbl_joblist_detail_$PRJCODEVW WHERE JOBPARENT = '$JOBPARENT' AND ITM_CODE = '$ITM_CODE'";
					$r_DOBL		= $this->db->count_all($s_DOBL);
					if($r_DOBL > 1)
						$STATCOL 	= "info";
					...
					if($ADD_JOBCOST > 0)
					{
						...
						$ADDVAL_VW 	= 	"<div style='white-space:nowrap;'>
											<p class='text-yellow' style='white-space:nowrap;'><i class='glyphicon glyphicon-triangle-top'></i>
					...
					if($ADDM_JOBCOST > 0)
					{
						$ADDVOLM_VW 	= 	"<div style='white-space:nowrap;'>
											<p class='text-red' style='white-space:nowrap;'><i class='glyphicon glyphicon-triangle-bottom'></i>
									  		".number_format($ADDM_VOLM, 2)."</p>
									  	</div>";
						$ADDVALM_VW 	= 	"<div style='white-space:nowrap;'>
											<p class='text-red' style='white-space:nowrap;'><i class='glyphicon glyphicon-triangle-bottom'></i>
									  		".number_format($ADDM_JOBCOST, 2)."</p>
									  	</div>";
					}
				
					$secURL 		= site_url('c_comprof/c_bUd93tL15t/updRAPP/?id=');
					$updRAPPID 		= "$secURL~$PRJCODE~$JOBCODEID~$JOBPARENT~$JOBDESC~$RAPP_VOLM~$RAPP_PRICE~$RAPP_JOBCOST";
					$updRAPPSV 		= "$secURL~$PRJCODE~$JOBCODEID~$JOBPARENT~$JOBDESC";
				...
				if($ISLASTH == 0 && $ISLAST == 0)
				{
					...
					if($PRJ_ISLOCK == 1)
					{
						$secAddHV 	= "<label style='white-space:nowrap'>
										   	<a href='javascript:void(null);' class='btn btn-warning btn-xs' title='Tambah Pekerjaan' onClick='alertLOCK()'>
												<i class='glyphicon glyphicon-plus-sign'></i>
											</a>
										</label>";
					}
				...
				$BUDG_DEV 	= $BOQ_JOBCOST - $JOBCOST - $ADD_JOBCOST + $ADDM_JOBCOST;

				/* Requested by pengendalian
				if($RAPP_STAT == 1)
				{*/
				...
				if($PRJ_ISLOCK == 1)
				{
					$btnUpdRAPP		= "<a href='javascript:void(null);' class='label label-".$STATCOL."' style='font-size:12px' onClick='alertLOCK(".$noU.")'>".number_format($RAPP_JOBCOST,2)."</a>";
				}

				$output['data'][] 	= array("<div style='text-align:center;'>".$secAddHV."</span>",
											"<span ".$CELL_COL.">".$JobView." ".$noDet."</span>
											<input type='hidden' name='TREQ_VOL".$noU."' id='TREQ_VOL".$noU."' value='".$TREQ_VOL."'>
											<input type='hidden' name='TREQ_VAL".$noU."' id='TREQ_VAL".$noU."' value='".$TREQ_VAL."'>
											<input type='hidden' name='JOBUNIT".$noU."' id='JOBUNIT".$noU."' value='".$JOBUNIT."'>",
											"<div style='text-align:center;'><span ".$CELL_COL.">".$JOBUNIT."</span></div>",
											"<div style='text-align:right;'><label style='white-space:nowrap'>".$reCountRAB."&nbsp;<span class='label label-".$STATCOL."' style='font-size:12px'>".number_format($BOQ_JOBCOST,2)."</span></label></div>",
											/*"<div style='text-align:right;'><span ".$CELL_COL.">".number_format($RAPT_VOLM, 2).$ADDVOL_VW."</span></div>",
											"<div style='text-align:right;'><span ".$CELL_COL.">".number_format($RAPT_PRICE, 2).$ADDPRC_VW."</span></div>",*/
											"<div style='text-align:right;'><span ".$CELL_COL.">".number_format($RAPT_VOLM, 2)."</span></div>",
											"<div style='text-align:right;'><span ".$CELL_COL.">".number_format($RAPT_PRICE, 2)."</span></div>",
											"<div style='text-align:right;'><label style='white-space:nowrap'>".$reCountRAPT."&nbsp;<span class='label label-".$STATCOL."' style='font-size:12px'>".number_format($RAPT_JOBCOST,2)."</span> $DEVRAPTFA</label></div>",
											"<div style='text-align:right;'><span ".$CELL_COL.">".number_format($DEV_BOQRAPT, 2)."</span></div>",
	...
	function updRAPP()
	{
		...
		$JOBDESC   	= $colExpl[4];
		$RAPP_VOL 	= $colExpl[5];
		$RAPP_PRICE = $colExpl[6];
		$ITM_TOTAL 	= $RAPP_VOL * $RAPP_PRICE;
		...
		$ITM_CODE 		= "";
		$s_TOTDET 		= "SELECT ITM_CODE FROM tbl_joblist_detail_$PRJCODEVW WHERE JOBCODEID = '$JOBCODEID' AND PRJCODE = '$PRJCODE'";
		$r_TOTDET 		= $this->db->query($s_TOTDET)->result();
		foreach($r_TOTDET as $rw_TOTDET) :
			$ITM_CODE 	= $rw_TOTDET->ITM_CODE;
		endforeach;
		...
		// REMEMBER ......!!!! VOLUME IS LOCK

		// START : UPDATE PROCEDURE
			$s_01 		= 	"UPDATE tbl_joblist_detail SET ITM_VOLM = '$RAPP_VOL', ITM_PRICE = '$RAPP_PRICE', ITM_LASTP = '$RAPP_PRICE', ITM_AVGP = '$RAPP_PRICE', ITM_BUDG = '$ITM_TOTAL',
								UPDATER_RAPP = '$DefEmp_ID', UPDATED_RAPP = '$UPDATED', UPDFLAG = 'UP-RAPP'
			...
			// START : UPDATE ALL HEADER IN HIERARKI
				...
				for($i=0;$i<$arrJIDPC;$i++)
				{
					...
					$TOTDET 		= 0;
					$s_TOTDET 		= "SELECT SUM(ITM_BUDG) AS TOTDET FROM tbl_joblist_detail_$PRJCODEVW WHERE JOBPARENT LIKE '$JIDPN%' AND ISLAST = 1";
					$r_TOTDET 		= $this->db->query($s_TOTDET)->result();
					foreach($r_TOTDET as $rw_TOTDET) :
		...
		echo "Pekerjaan ini sudah diupdate.";
	}
		...
		// START : FOR SERVER-SIDE
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				$secAction	= 	"<input type='hidden' name='secVWlURL_".$noU."' id='secVWlURL_".$noU."' value='".$secVWlURL."'>
						   		...
						   		<input type='hidden' name='BOQH_TYPE".$noU."' id='BOQH_TYPE".$noU."' value='".$BOQH_TYPE."'>
							   	<label style='white-space:nowrap'>
    ...
    function svJL()
    {
		...
		$ISLASTH 	= $_POST['ISLASTH'];
		//$JOBDESC 	= $_POST['JOBDESC'];
		$JOBDESC 	= strtoupper(addslashes($_POST['JOBDESC']));
		$JOBUNIT 	= strtoupper(addslashes($_POST['JOBUNIT']));
		$BOQ_VOLM 	= $_POST['BOQ_VOLM'];
		$BOQ_PRICE	= $_POST['BOQ_PRICE'];
		$ITM_VOLM 	= $_POST['ITM_VOLM'];
		$ITM_PRICE	= $_POST['ITM_PRICE'];
		...
		if($JOBPARENTB == $JOBPARENT)
		{
			// BOQ
				...
				$s_upd01 	= "UPDATE tbl_boqlist SET JOBUNIT = '$JOBUNIT', JOBDESC = '$JOBDESC' WHERE JOBCODEID = '$JOBCODEIDB' AND PRJCODE = '$PRJCODE'";
			...
			// JOBLIST
				/*$s_upd02 	= "UPDATE tbl_joblist SET JOBCODEID = '$JOBCODEID', JOBDESC = '$JOBDESC', JOBUNIT = '$JOBUNIT', ISLASTH = '$ISLASTH',
								...
								BOQ_VOLM = '$BOQ_VOLM', BOQ_PRICE = '$BOQ_PRICE', BOQ_JOBCOST = '$BOQ_JOBCOST'
				 				WHERE JOBCODEID = '$JOBCODEIDB' AND PRJCODE = '$PRJCODE'";*/
				$s_upd02 	= "UPDATE tbl_joblist SET JOBUNIT = '$JOBUNIT', ISLASTH = '$ISLASTH', JOBDESC = '$JOBDESC' WHERE JOBCODEID = '$JOBCODEIDB' AND PRJCODE = '$PRJCODE'";
				$this->db->query($s_upd02);

			// JOBLISTDETAIL
				/*$s_upd03 	= "UPDATE tbl_joblist_detail SET JOBCODEDET = '$JOBCODEID', JOBCODEID = '$JOBCODEID', JOBDESC = '$JOBDESC',
								ITM_UNIT = '$JOBUNIT', ISLASTH = '$ISLASTH',
								ITM_VOLM = '$ITM_VOLM', ITM_PRICE = '$ITM_PRICE', ITM_LASTP = '$ITM_PRICE',
				 				...
				 				WHERE JOBCODEID = '$JOBCODEIDB' AND PRJCODE = '$PRJCODE'";*/
				$s_upd03 	= "UPDATE tbl_joblist_detail SET ITM_UNIT = '$JOBUNIT', JOBDESC = '$JOBDESC' WHERE JOBCODEID = '$JOBCODEIDB' AND PRJCODE = '$PRJCODE'";
	...
	function downLJL()
	{
		$PRJCODE			= $this->input->post('PRJCODE2');
		
		$data["PRJCODE"] 	= $PRJCODE;
		$this->load->view('v_company/v_budget/v_joblistdet_downl', $data);
	}

  	function get_AllDataHISTAMD() // GOOD
	{
		$PRJCODE		= $_GET['id'];
		...
		// START : FOR SERVER-SIDE
			...
			$start			= $_REQUEST['start'];
			$search			= $_REQUEST['search']["value"];
			$num_rows 		= $this->m_budget->get_AllDataHISTAMDC($PRJCODE, $search);
			$total			= $num_rows;
			$output			= array();
			$output['draw']	= $draw;
			$output['recordsTotal'] = $output['recordsFiltered']= $total;
			$output['data']	= array();
			$query 			= $this->m_budget->get_AllDataHISTAMDL($PRJCODE, $search, $length, $start, $order, $dir);
								
			$noU			= $start + 1;
			foreach ($query->result_array() as $dataI) 
			{
                $AMDF_CODE 		= $dataI['AMDF_CODE'];
                $AMDF_PRJCODE 	= $dataI['AMDF_PRJCODE'];
                $AMDF_DATE 		= $dataI['AMDF_DATE'];
                $AMDF_DESC	 	= $dataI['AMDF_DESC'];
                $AMDF_FN	 	= $dataI['AMDF_FN'];
                $AMDF_STAT	 	= $dataI['AMDF_STAT'];
                $AMDF_USER	 	= $dataI['AMDF_USER'];
                $AMDF_TYPE	 	= $dataI['AMDF_TYPE'];
                ...
                $FileName 	= $AMDF_FN;
                $secVWlURL	= site_url('c_gl/c_ch1h0fbeart/view_coaup/?id='.$this->url_encryption_helper->encode_url($AMDF_CODE));
                $urlDwl		= base_url('index.php/c_comprof/c_bUd93tL15t/dwlFileBoQAMDF/?id='.urldecode($FileName));

				$secAction	= 	"<input type='hidden' name='secVWlURL_".$noU."' id='secVWlURL_".$noU."' value='".$secVWlURL."'>
						   		<input type='hidden' name='AMDF_CODE".$noU."' id='AMDF_CODE".$noU."' value='".$AMDF_CODE."'>
							   	<label style='white-space:nowrap'>
							   	<a href='javascript:void(null);' onClick='procIMP(".$noU.");' data-skin='skin-green' class='btn btn-warning btn-xs' title='Process' ".$disabled1d.">
                                    <i class='glyphicon glyphicon-refresh'></i>
                                </a>
				...
				$output['data'][] 	= array("<label style='white-space:nowrap'>".$AMDF_DATE."</label>",
										  	$AMDF_DESC,
										  	"<span class='label label-".$STATCOL."' style='font-size:12px'>".$AMDF_STATD."</span>",
										  	$secAction);
				$noU		= $noU + 1;
			}
	...
	function do_uploadJLAMD() // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$AMDF_CODE		= "AMD$AMDF_DATEY$AMDF_DATEM$AMDF_DATED-$AMDF_DATEH$AMDF_DATEm$AMDF_DATES";
			$AMDF_DATE		= date('Y-m-d H:i:s');
			$AMDF_PRJCODE	= $this->input->post('PRJCODE');
			$AMDF_DESC		= $this->input->post('AMDF_DESC');
			$AMDF_TYPE		= $this->input->post('AMDF_TYPE');
			$AMDF_USER		= $DefEmp_ID;
			$AMDF_STAT		= 1;
			...
			if(move_uploaded_file($source, $target_path))
			{
				$s_01 	= "INSERT INTO tbl_amd_file (AMDF_CODE, AMDF_PRJCODE, AMDF_DATE, AMDF_DESC, AMDF_FN, AMDF_STAT, AMDF_USER, AMDF_TYPE)
							VALUES ('$AMDF_CODE', '$AMDF_PRJCODE', '$AMDF_DATE', '$AMDF_DESC', '$filename', '$AMDF_STAT', '$AMDF_USER', '$AMDF_TYPE')";
				$this->db->query($s_01);
			}
