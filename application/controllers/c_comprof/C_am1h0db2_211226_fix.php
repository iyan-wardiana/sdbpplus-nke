...
class C_am1h0db2 extends CI_Controller  
{
	...
	function updateamd_process_inb() // OK
	{
		...
		{			
			...
			if($AMD_CATEG == 'NB') // Not Budgeting
			{
				$JOBLEVNEW		= $JOBLEV + 1;
				/*$sqlCHLDC 		= "tbl_joblist WHERE JOBPARENT = '$AMD_JOBID'";
				$resCHLDC 		= $this->db->count_all($sqlCHLDC);*/
				
				$sqlCHLDDC 		= "tbl_joblist_detail WHERE JOBPARENT = '$JOBPARENT1'";
				...
				if($AH_ISLAST == 1)
				{
					...
					// PROCEDURE UNTUK GET LAST ORD_ID
						...
						{			
							$rowDet			= 0;
							$Pattern_Length	= 3;
							$theLastD		= $resCHLDDC;	// TOTAL ROW CHILD
					...
					// UPDATE OTHER ORD_ID
						...
						elseif($AMD_CATEG == 'NB') // Not Budgeting
						{
							foreach($_POST['data'] as $d)
							{
								...
								$JOBPARENT	= $AMD_JOBPAR;		// TO BE A PARENT
								...
								// Catatan 21-10-30 : NB tidak menambahkan header, hanya menambahkan detail.
									$nxChld				= $nxChld+1;
									...
									echo "nxChld = $nxChld<br>theLastD = $theLastD<br>JOBLEVNEW = $JOBLEVNEW<br>";
									$JOBCODEID			= $d['JOBCODEID'];
									$dt['ORD_ID']		= $nxChld;
