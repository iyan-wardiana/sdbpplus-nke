 ...
 * Edit Date	= 04 September 2017
 * File Name	= c_office_periodic_report.php
 * Location		= -
*/

if ( ! defined('BASEPATH')) exit('No direct script access allowed');
class C_office_periodic_report extends CI_Controller  
{	
    function index()
	{		
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$url			= site_url('c_asset/c_office_periodic_report/office_periodic/?id='.$this->url_encryption_helper->encode_url($appName));
			redirect($url);
		}
		else
		{
	...
	function office_periodic() // OK
	{
		...
		{
			$data['title'] 			= $appName;
			$data['h2_title']		= 'Periodic';
			$data['h3_title']		= 'Report';
			$data['form_action'] 	= site_url('c_asset/c_office_periodic_report/office_periodic_view/?id='.$this->url_encryption_helper->encode_url($appName));
			
			$this->load->view('v_asset/v_office_periodic_report/v_office_periodic', $data);
		}
		else
		{
			redirect('Auth');
	...
	function office_periodic_view() // OK
	{
		$sqlApp 		= "SELECT * FROM tappname";
		$resultaApp = $this->db->query($sqlApp)->result();
		foreach($resultaApp as $therow) :
		...
		{			
			...
			{
				$this->load->view('v_asset/v_office_periodic_report/v_office_periodic_report_adm', $data);
			}
			else
			{
				$this->load->view('v_asset/v_office_periodic_report/v_office_periodic_report', $data);
			}
		}
		else
		{
