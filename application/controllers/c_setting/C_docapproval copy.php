...
class C_docapproval  extends CI_Controller
{
	...
	function update_process()
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			// CEK LAST STEP
				$s_MAX		= "SELECT MAX(MAX_STEP) AS MAXSTEP FROM tbl_docstepapp_det WHERE PRJCODE = '$PRJCODE' AND MENU_CODE = '$MENU_CODE'";
				$r_MAX		= $this->db->query($s_MAX)->result();
				foreach($r_MAX as $rw_MAX) :		
					$MAXSTEP	= $rw_MAX->MAXSTEP;
			...
			if($resDocStepAPPH->num_rows() > 0)
			{
				...
				foreach($resDocStepAPPH->result() as $rAPP):
					...
					if($rowSTEP > 1)
					{
						...
						date_add($date, date_interval_create_from_date_string("+2 seconds")); // ADD 1 second
				...
				// COPY STEP APP HEADER & DETAIL
					...
					// NEW COPY STEP APP HEADER
						$newSTEPAPPH = "INSERT INTO tbl_docstepapp (DOCCODE, PRJCODE, DOCAPP_TYPE, DOCAPP_NAME, MENU_CODE, 
										APPROVER_1, APPROVER_2, APPROVER_3, APPROVER_4, APPROVER_5, MAX_STEP) 
										SELECT '$NEWDOCCODE', '$DSTPRJCODE', DOCAPP_TYPE, DOCAPP_NAME, MENU_CODE, 
										APPROVER_1, APPROVER_2, APPROVER_3, APPROVER_4, APPROVER_5, MAX_STEP
										FROM tbl_docstepapp
										WHERE MENU_CODE = '$MENU_CODE' AND PRJCODE = '$SRCPRJCODE'";
						$this->db->query($newSTEPAPPH);
					// END NEW COPY STEP APP HEADER

					// NEW COPY STEP APP DETAIL
						$newSTEPAPPD = "INSERT INTO tbl_docstepapp_det (DOCCODE, PRJCODE, DOCAPP_TYPE, DOCAPP_NAME, MENU_CODE, APPROVER_1, APP_STEP, MAX_STEP) 
										SELECT '$NEWDOCCODE', '$DSTPRJCODE', DOCAPP_TYPE, DOCAPP_NAME, MENU_CODE, APPROVER_1, APP_STEP, MAX_STEP
										FROM tbl_docstepapp_det
										WHERE MENU_CODE = '$MENU_CODE' AND PRJCODE = '$SRCPRJCODE'";
						$this->db->query($newSTEPAPPD);
					// END NEW COPY STEP APP DETAIL
				// END COPY HEADER STEP APP

				echo json_encode($this->db->get_where('tbl_docstepapp',['PRJCODE' => $DSTPRJCODE, 'MENU_CODE' => $MENU_CODE])->result());
			}
		// END GET docstepapp HEADER & DETAIL
	}
}