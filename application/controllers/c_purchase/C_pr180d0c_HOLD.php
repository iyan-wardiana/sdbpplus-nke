...
class C_pr180d0c extends CI_Controller  
{
	...
	function pRQ_l5t_x() // G
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			{
				$data['addURL'] 	= site_url('c_purchase/c_pr180d0c/add_fpa/?id='.$this->url_encryption_helper->encode_url($PRJCODE));
				$data['backURL'] 	= site_url('c_purchase/c_pr180d0c/pL15t_fpa/?id='.$this->url_encryption_helper->encode_url($PRJCODE));
			}
  	...
  	function get_AllDataPRGRP() // GOOD
	{
		...
		$DefEmp_ID 		= $this->session->userdata['Emp_ID'];
		$cSHC			= "tusermenu WHERE menu_code = 'MN516' and emp_id = '$DefEmp_ID'";
		$cSH 			= $this->db->count_all($cSHC);
		...
		// START : FOR SERVER-SIDE
			...
			if($SPPCAT == 'SPP')
			{
				$tblPRH 	= "tbl_pr_header";
				$tblPRD 	= "tbl_pr_detail";
				$num_rows 	= $this->m_purchase_req->get_AllDataGRPC($PRJCODE, $PO_NUM, $PR_STAT, $search, $length, $start);
			}
			elseif($SPPCAT == 'FPA')
			{
				$tblPRH 	= "tbl_pr_header_fpa";
				$tblPRD 	= "tbl_pr_detail_fpa";
				$num_rows 	= $this->m_purchase_req->get_AllDataGRPC_fpa($PRJCODE, $PO_NUM, $PR_STAT, $search, $length, $start);
			}
			...
			if($SPPCAT == 'SPP')
				...
				$query 		= $this->m_purchase_req->get_AllDataGRPL_fpa($PRJCODE, $PO_NUM, $PR_STAT, $search, $length, $start, $order, $dir);
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				$MenuApp	= $dataI['PR_DIVID'];
				$MNCODE		= $dataI['PR_DIVID'];
				$CREATERNM	= $dataI['CREATERNM'];
				...
				if($PRJHO == 'KTR')
				{
					...
					elseif($MenuApp == 'MN438')
						$PRDIVD 	= "Audit Internal";
					elseif($MenuApp == 'MN439')
						$PRDIVD 	= "Corp. Legal";
					elseif($MenuApp == 'MN440')
						$PRDIVD 	= "Corp. Adm. Kontrak";
				...
				$CollCode	= "$PRJCODE~$PR_NUM";
				$secUpd		= site_url('c_purchase/c_pr180d0c/update/?SPPCAT='.$SPPCAT.'&id='.$this->url_encryption_helper->encode_url($CollCode));
				$secPPOList	= site_url('c_purchase/c_pr180d0c/pRn_P0l/?id='.$this->url_encryption_helper->encode_url($PR_NUM));
				if($SPPCAT == 'SPP')
					$secPrint	= site_url('c_purchase/c_pr180d0c/printdocument/?id='.$this->url_encryption_helper->encode_url($PR_NUM));
				elseif($SPPCAT == 'FPA')
					$secPrint	= site_url('c_purchase/c_pr180d0c/printdocument_fpa/?id='.$this->url_encryption_helper->encode_url($PR_NUM));

				$CollID		= "$SPPCAT~$PR_NUM~$PRJCODE";
				$secDel_DOC = base_url().'index.php/__l1y/trash_DOC/?id='.$CollID;
				$secDelIcut = base_url().'index.php/__l1y/trashDOC/?id=';
				$delID 		= "$secDelIcut~$tblPRH~$tblPRD~PR_NUM~$PR_NUM~PRJCODE~$PRJCODE";
				$secVoid 	= base_url().'index.php/__l1y/trashPR/?id=';
				$voidID 	= "$secVoid~$tblPRH~$tblPRD~PR_NUM~$PR_NUM~PRJCODE~$PRJCODE";

				// CEK PO
					if($SPPCAT == 'SPP')
						$sqlPOC	= "tbl_po_header WHERE PRJCODE = '$PRJCODE' AND PR_NUM = '$PR_NUM' AND PO_STAT NOT IN (5,9)";
					elseif($SPPCAT == 'FPA')
						$sqlPOC	= "tbl_wo_header WHERE PRJCODE = '$PRJCODE' AND FPA_NUM = '$PR_NUM' AND WO_STAT NOT IN (5,9)";
				...
				elseif($PR_STAT == 2 || $PR_STAT == 3 || $PR_STAT == 6)
				{
					$secAction	= 	"<input type='hidden' name='urlPOList".$noU."' id='urlPOList".$noU."' value='".$secPPOList."'>
								   	<input type='hidden' name='urlPrint".$noU."' id='urlPrint".$noU."' value='".$secPrint."'>
									<input type='hidden' name='urlVoid".$noU."' id='urlVoid".$noU."' value='".$voidID."'>
				...
				if($cSH > 0)
				{
					$urlHist	= site_url('c_purchase/c_pr180d0c/showDocHist/?id='.$this->url_encryption_helper->encode_url($PR_NUM));
					$secHist	= 	"<input type='hidden' name='urlHist".$noU."' id='urlHist".$noU."' value='".$urlHist."'>
										<a href='javascript:void(null);' class='pull-right' onClick='showDHist(".$noU.")'>
				...
				$output['data'][] 	= array("$noU.",
										  	...
										  	$PR_DATEV,
										  	"<strong><i class='fa fa-comments margin-r-5'></i>Keterangan</strong>
									  		<div style='margin-left: 20px'>
										  		<p class='text-muted'>
										  			".$JOBDESC."
										  		</p>
										  	</div>",
										  	"<strong><i class='fa fa-user margin-r-5'></i>$empName ".$secHist."</strong><br>
	...
	function add() // G
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data["MenuCode"] 	= 'MN017';
			$data["SPPCAT"] 	= 'SPP';
			$data["tblPRH"] 	= 'tbl_pr_header';
			$data["tblPRD"] 	= 'tbl_pr_detail';
  	...
  	function get_AllDataIITMDET_230306() // GOOD
	{
		$PRJCODE	= $_GET['id'];
		$PR_NUM		= $_GET['PR_NUM'];
		$task		= $_GET['task'];
		...
		// START : FOR SERVER-SIDE
			...
			$start			= $_REQUEST['start'];
			$search			= $_REQUEST['search']["value"];
			$num_rows 		= $this->m_purchase_req->get_AllDataITMDETC($PRJCODE, $PR_NUM, $task, $search, $length, $start);
			$total			= $num_rows;
			$output			= array();
			$output['draw']	= $draw;
			$output['recordsTotal'] = $output['recordsFiltered']= $total;
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				// TOTAL PR CONFIRMED AND APPROVED FROM ANOTHER DOCUMENT
		        	...
		        	$TOTPRAMOUNT2CNC= 0;
		        	$test 			= 0;
					$sqlQTY			= "SELECT SUM((PR_VOL - PR_CVOL) + (WO_VOL - PO_CVOL) + VCASH_VOL + VLK_VOL + PPD_VOL + PR_VOL_R + WO_VOL_R + VCASH_VOL_R + VLK_VOL_R + PPD_VOL_R) AS TOTPRQTY2,
											SUM((PO_VAL - PO_CVAL) + (WO_VAL - WO_CVAL) + VCASH_VAL + VLK_VAL + PPD_VAL + PO_VAL_R + WO_VAL_R + VCASH_VAL_R + VLK_VAL_R + PPD_VAL_R) AS TOTPRAMOUNT2, SUM(0) AS TOTPRAMOUNT2CNC
				...
				$TOTPRQTY 		= $TOTPRQTY1 + $TREQ_VOL + $TREQ_VOL_R;
				$TOTPRAMOUNT 	= $TOTPRAMOUNT1 + $TREQ_VAL + $TREQ_VAL_R;
				...
				$output['data'][] 	= array("<div style='white-space:nowrap'>
											...
											$ITM_R,
									  		"<div style='white-space:nowrap; text-align: center'>
									  			<input type='text' name='data[".$noU."][ITM_UNIT2]' id='data".$noU."ITM_UNIT2' value='".$ITM_UNIT."' class='form-control' style='text-align:center; width:90px; text-transform:uppercase;' >
  	...
  	function get_AllDataIITMDET() // GOOD
	{
		...
		// START : FOR SERVER-SIDE
			...
			$s_PRSTAT 	= "SELECT PR_STAT FROM $tblPRH WHERE PRJCODE = '$PRJCODE' AND PR_NUM = '$PR_NUM'";
			$r_PRSTAT 	= $this->db->query($s_PRSTAT)->result();
			foreach($r_PRSTAT as $rw_PRSTAT):
				$PR_STAT 	= $rw_PRSTAT->PR_STAT;
			endforeach;
			...
			$start			= $_REQUEST['start'];
			$search			= $_REQUEST['search']["value"];
			$num_rows 		= $this->m_purchase_req->get_AllDataITMDETC($PRJCODE, $PR_NUM, $tblPRH, $tblPRD, $task, $search, $length, $start);
			$total			= $num_rows;
			$output			= array();
			$output['draw']	= $draw;
			$output['recordsTotal'] = $output['recordsFiltered']= $total;
			$output['data']	= array();
			$query 			= $this->m_purchase_req->get_AllDataITMDETL($PRJCODE, $PR_NUM, $tblPRH, $tblPRD, $task, $search, $length, $start, $order, $dir);
			...
			foreach ($query->result_array() as $dataI) 
			{
	        	...
	        	$TPRVOL_CUR 	= 0;
	        	$TPRVAL_CUR 	= 0;
				$s_01 			= "SELECT IFNULL(SUM(PR_VOLM),0) AS TPRVOL_CUR
									FROM tbl_pr_detail_tmp
									WHERE JOBCODEID = '$JOBCODEID' AND PR_NUM = '$PR_NUM' AND PR_ID != $PR_ID AND PRJCODE = '$PRJCODE'";
				$r_01 			= $this->db->query($s_01)->result();
				foreach($r_01 as $rw_01):
					$TPRVOL_CUR = $rw_01->TPRVOL_CUR;
				endforeach;
				...
				$TOTPRQTY 		= $TPRVOL_CUR + $TREQ_VOL + $TREQ_VOL_R;
				$TOTPRAMOUNT 	= $TPRVAL_CUR + $TREQ_VAL + $TREQ_VAL_R;
                ...
                if($ITM_TYPE == 'SUBS')
                {
                    $REQ_VOLM	= 0;
                    $REQ_AMOUNT	= 0;
                    $sqlREQ		= "SELECT PO_VOL, PO_VAL FROM tbl_joblist_detail_$PRJCODEVW 
                                    WHERE PRJCODE = '$PRJCODE' AND ITM_CODE = '$ITM_CODE_H'";
                    $resREQ		= $this->db->query($sqlREQ)->result();
                    foreach($resREQ as $rowREQ) :
                        $TOTPRQTY		= $rowREQ->PO_VOL;
                        $TOTPRAMOUNT	= $rowREQ->PO_VAL;
                    endforeach;
                }
				...
				if($PR_STAT == 1 || $PR_STAT == 4)
				{
					...
					$BTN_COPYVW		= "<a onClick='copyRow(".$noU.")' title='Copy' class='btn btn-warning btn-xs'><i class='glyphicon glyphicon-copy'></i></a>";
					$BTN_CNCVW		= "";
					$ITM_VOLVW 		= "<input type='text' class='form-control' style='min-width:80px; max-width:110px; text-align:right' name='ITM_VOLMBGx".$noU."' id='ITM_VOLMBGx".$noU."' value='".number_format($TBUDG_VOL, 3)."' disabled >";
					$ITM_TOTPRQTYVW = "<input type='text' class='form-control' style='min-width:80px; max-width:110px; text-align:right' name='TOTPRQTYx".$noU."' id='TOTPRQTYx".$noU."' value='".number_format($TOTPRQTY, 3)."' disabled >";
					$ITM_PRVOLMVW 	= "<input type='text' name='PR_VOLM".$noU."' id='PR_VOLM".$noU."' value='".number_format($PR_VOLM, 3)."' class='form-control' style='min-width:80px; max-width:110px; text-align:right' onKeyPress='return isIntOnlyNew(event);' onBlur='chgVOL(this,".$noU.");'>";
					$ITM_DESCVW 	= 	"<div class='input-group'>
		                                    <div class='input-group-btn'>
												<button type='button' class='btn btn-primary' onClick='getDL(".$noU.")'><i class=' fa fa-commenting'></i></button>
		                                    </div>
					...
					$ITM_SPECVW		= "<input type='text' name='data[".$noU."][PR_SPEC]' id='data".$noU."PR_SPEC' value='".$PR_SPEC."' class='form-control' style='min-width:110px; max-width:500px; text-align:left; readonly>";
					$ITM_R 			= "<span id='ITMVOL_R'".$PR_DESC_ID."".$noU." style='text-align: right;'>".number_format($ITMVOLM_R, 3)."</span>";
					$ITM_UNITVw 	= "<input type='text' name='data[".$noU."][ITM_UNIT2]' id='data".$noU."ITM_UNIT2' value='".$ITM_UNIT2."' class='form-control' style='text-align:center; width:90px; text-transform:uppercase;' >
										<input type='hidden' name='data[".$noU."][ITM_UNIT]' id='data".$noU."ITM_UNIT' value='".$ITM_UNIT."' class='form-control' style='text-align:center; width:90px; text-transform:uppercase;' >";
				}
				elseif($PR_STAT == 3)
				{
					$BTN_DELVW 		= "";
					$BTN_COPYVW		= "";
					$BTN_CNCVW		= "<a onClick='cancelRow(".$noU.")' title='Batalkan Volume SPP' class='btn btn-danger btn-xs'><i class='fa fa-repeat'></i></a>";
					$ITM_VOLVW 		= "".number_format($TBUDG_VOL, 3)." <input type='hidden' class='form-control' style='min-width:110px; max-width:300px; text-align:right' name='ITM_VOLMBGx".$noU."' id='ITM_VOLMBGx".$noU."' value='".number_format($TBUDG_VOL, 3)."' disabled >";
					$ITM_TOTPRQTYVW = "".number_format($TOTPRQTY, 3)." <input type='hidden' class='form-control' style='min-width:110px; max-width:300px; text-align:right' name='TOTPRQTYx".$noU."' id='TOTPRQTYx".$noU."' value='".number_format($TOTPRQTY, 3)."' disabled >";
					$ITM_PRVOLMVW 	= "".number_format($PR_VOLM, 3)." <input type='hidden' name='PR_VOLM".$noU."' id='PR_VOLM".$noU."' value='".number_format($PR_VOLM, 3)."' class='form-control' style='min-width:110px; max-width:300px; text-align:right' onKeyPress='return isIntOnlyNew(event);' onBlur='getConvertion(this,".$noU.");'>";
					$ITM_DESCVW 	= "".$PR_DESC."
	                                    <input type='hidden' name='data[".$noU."][PR_DESC_ID]' id='data".$noU."PR_DESC_ID' size='20' value='".$PR_DESC_ID."' class='form-control' style='min-width:110px; max-width:500px; text-align:left'>
	                                    <input type='hidden' name='data[".$noU."][PR_DESC]' id='data".$noU."PR_DESC' size='20' value='".$PR_DESC."' class='form-control' style='min-width:110px; max-width:500px; text-align:left'>";
					$ITM_SPECVW 	= "".$PR_SPEC."
	                                	<input type='hidden' name='data[".$noU."][PR_SPEC]' id='data".$noU."PR_SPEC' value='".$PR_SPEC."' class='form-control' style='min-width:110px; max-width:500px; text-align:left'>";
					$ITM_R 			= "<span id='ITMVOL_R'".$PR_DESC_ID."".$noU." style='text-align: right;'>".number_format($ITMVOLM_R, 3)."</span>";
					$ITM_UNITVw 	= $ITM_UNIT2;
				}
				else
				{
					$BTN_DELVW 		= "";
				...
				$output['data'][] 	= array("<div style='white-space:nowrap'>
											...
											$ITM_SPECVW,
										  	"$ITM_VOLVW
										  		<input type='hidden' style='text-align:right' name='data[".$noU."][ITM_VOLMBG]' id='ITM_VOLMBG".$noU."' value='".$TBUDG_VOL."' >
										  		<input type='hidden' id='data".$noU."ITM_BUDG' name='data[".$noU."][ITM_BUDG]' value='".$ITM_BUDG."'>",
											$TBUDG_VOL,
										  	"$ITM_TOTPRQTYVW
										  		<input type='hidden' class='form-control' style='min-width:110px; max-width:300px; text-align:right' name='ITM_REQUESTED".$noU."' id='ITM_REQUESTED".$noU."' value='".$TOTPRAMOUNT."' >
  	...
  	function get_AllDataIITMDET_230717() // GOOD
	{
		$PRJCODE	= $_GET['id'];
		$PR_NUM		= $_GET['PR_NUM'];
		$task		= $_GET['task'];
	...
	function copyROW()
	{
		...
		// INSERT DATA
			$s_02 	= 	"INSERT INTO tbl_pr_detail_tmp
							...
							ITM_CODE, ITM_NAME, ITM_UNIT, ITM_UNIT2, PR_VOLM, PR_PRICE, PR_TOTAL, PR_DESC,
							ITM_VOLMBG, ITM_BUDG, PR_STAT)
						VALUES
							('$MAX_PRIDN', '$PR_NUM', '$PR_CODE', '$PRJCODE', '$DEPCODE', '$JOBCODEDET', '$JOBCODEID', '$JOBPARENT', '$JOBPARDESC',
							'$ITM_CODE', '$ITM_NAME', '$ITM_UNIT', '$ITM_UNIT2', 0, '$PR_PRICE', 0, '',
	...
	function chgVol()
	{
		...
		$TPRVOL_OTH1 	= 0;
		$s_BUDGVAL		= "SELECT (PR_VOL - PR_CVOL + WO_VOL - PO_CVOL + VCASH_VOL + VLK_VOL + PPD_VOL + PR_VOL_R + WO_VOL_R + VCASH_VOL_R + VLK_VOL_R + PPD_VOL_R) AS TPRVOL_OTH
							FROM tbl_joblist_detail_$PRJCODEVW WHERE JOBCODEID = '$JOBCODEID' AND PRJCODE = '$PRJCODE'";
		$r_BUDGVAL		= $this->db->query($s_BUDGVAL)->result();
		foreach($r_BUDGVAL as $rowJOBDR) :
			$TPRVOL_OTH1 	= $rowJOBDR->TPRVOL_OTH;
		endforeach;

    	$TPRVOL_OTH2 	= 0;
		$s_01 			= "SELECT IFNULL(SUM(PR_VOLM-PR_CVOL),0) AS TPRVOL_OTH
							FROM tbl_pr_detail_tmp
							WHERE JOBCODEID = '$JOBCODEID' AND PR_NUM != '$PR_NUM' AND PRJCODE = '$PRJCODE'";
		$r_01 			= $this->db->query($s_01)->result();
		foreach($r_01 as $rw_01):
			$TPRVOL_OTH2 	= $rw_01->TPRVOL_OTH;
		endforeach;

		$TOT_REQVOL 	= $REQ_NOW + $TPRVOL_CUR + $TPRVOL_OTH1 + $TPRVOL_OTH2; 			// TOTAL ALL ROW REQUEST IN DOC. ACTIVE

		$s_isLS 	= "tbl_unitls WHERE ITM_UNIT = '$ITM_UNIT'";
		$r_isLS 	= $this->db->count_all($s_isLS);

		//if(($TOTPRQTY3 > $ITM_BUDGV || $TOTPRAMOUNT3 > $ITM_BUDGA) && $ITM_UNIT != 'LS')
		if(($TOT_REQVOL > $ITM_BUDGV) && $r_isLS == 0)
		{
			$REM_VOLPR 	= $ITM_BUDGV - ($TPRVOL_CUR + $TPRVOL_OTH1 + $TPRVOL_OTH2);
		...
		else
		{
			...
			$alert1 	= "";
			echo "0~$alert1~$REQ_NOW";
		}
		//echo "($REQ_NOW + $TPRVOL_CUR + $TPRVOL_OTH1 + $TPRVOL_OTH2)~$ITM_BUDGV~$JOBCODEID";
	}
	...
	function chgVol_fpa()
	{
		...
		if(($TOT_REQVOL > $ITM_BUDGV) && $r_isLS == 0)
		{
			...
			// UPDATE DATA 1
				$s_03 	= 	"UPDATE tbl_pr_detail_fpa SET PR_VOLM = '$REM_VOLPR', PR_TOTAL = PR_PRICE * $REM_VOLPR
								WHERE PR_ID = $PR_ID AND PR_NUM = '$PR_NUM' AND PRJCODE = '$PRJCODE'";
				$this->db->query($s_03);
		...
		else
		{
			...
			// UPDATE DATA 1
				$s_03 	= 	"UPDATE tbl_pr_detail_fpa SET PR_VOLM = '$REQ_NOW', PR_TOTAL = PR_PRICE * $REQ_NOW
								WHERE PR_ID = $PR_ID AND PR_NUM = '$PR_NUM' AND PRJCODE = '$PRJCODE'";
				$this->db->query($s_03);
	...
	function add_process() // G
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$maxNo	= 0;
			$sqlMax = "SELECT MAX(PR_ID) AS maxNo FROM $tblPRD WHERE PR_NUM = '$PR_NUM' AND PRJCODE = '$PRJCODE'";
			$resMax = $this->db->query($sqlMax)->result();
			foreach($resMax as $rowMax) :
				$maxNo = $rowMax->maxNo;		
			endforeach;
			...
			foreach($_POST['data'] as $d)
			{
				...
				// GET HEADER
					...
					$JOBIDH		= "";
					$JOBDSH 	= "";
						$sqlJID = "SELECT JOBCODEID AS JOBPARENT, JOBDESC FROM tbl_joblist_detail_$PRJCODEVW 
                					WHERE JOBCODEID = (SELECT JOBPARENT FROM tbl_joblist_detail_$PRJCODEVW 
				...
				if($PRJHO == 'KTR')
				{
					$s_MN 	= "SELECT B.JOBCOD1 FROM $tblPRD A INNER JOIN tbl_joblist_detail_$PRJCODEVW B ON A.JOBCODEID = B.JOBCODEID AND A.PRJCODE = B.PRJCODE
								WHERE A.PRJCODE = '$PRJCODE' AND A.PR_NUM = '$PR_NUM'
								ORDER BY A.PR_ID DESC LIMIT 1";
			...
			$JOBDESC		= '';
			$sqlJOBDESC		= "SELECT JOBDESC FROM tbl_joblist_$PRJCODEVW WHERE JOBCODEID = '$JOBIDH' AND PRJCODE = '$PRJCODE' LIMIT 1";
				...
				for($i = 0; $i < $count; $i++) {
					if(!empty($_FILES['userfile']['name'][$i])) {
						...
						if ($this->upload->do_upload('userfile'))
						{
							...
							$source		= "assets/AdminLTE-2.0.5/doc_center/uploads/PR_Document/$PRJCODE/$fileName";
							...
							if($srvURL == '10.0.0.144')
							{
								...
								$destination = "/assets/AdminLTE-2.0.5/doc_center/uploads/PR_Document/$PRJCODE/$fileName";

								if($this->ftp->list_files("./assets/AdminLTE-2.0.5/doc_center/uploads/PR_Document/$PRJCODE/") == FALSE) 
								{
									$this->ftp->mkdir("./assets/AdminLTE-2.0.5/doc_center/uploads/PR_Document/$PRJCODE/", 0777);
								}
			...
			else
			{
				...
				if($PRJHO == 'KTR')
				{
					// $s_DIV 	= "SELECT DISTINCT PR_DIVID FROM tbl_pr_detail WHERE PRJCODE = '$PRJCODE' AND PR_NUM = '$PR_NUM' AND PR_DIVID != '' AND PR_DIVID IS NOT NULL";
					$s_DIV 	= "SELECT B.JOBCOD1 FROM $tblPRD A INNER JOIN tbl_joblist_detail_$PRJCODEVW B ON A.JOBCODEID = B.JOBCODEID AND A.PRJCODE = B.PRJCODE
								WHERE A.PRJCODE = '$PRJCODE' AND A.PR_NUM = '$PR_NUM'
								ORDER BY A.PR_ID DESC LIMIT 1";
					$r_DIV 	= $this->db->query($s_DIV)->result();
					foreach($r_DIV as $rw_DIV):
						// $MenuApp 	= $rw_DIV->PR_DIVID;
			...
			$projMatReqH 	= array('PR_NUM' 			=> $PR_NUM,
									...
									'PR_NOTE'			=> htmlspecialchars($this->input->post('PR_NOTE'),ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401 | ENT_HTML5),
									'PR_STAT'			=> $this->input->post('PR_STAT'),
									'JOBCODE'			=> $JOBIDH,
									'JOBDESC'			=> $JOBDESC,
									'PR_REFNO'			=> $COLPRREFNO,
			...
			$this->m_purchase_req->addH($projMatReqH, $tblPRH);
			
			// UPDATE DETAIL
				$this->m_purchase_req->updatePRDet($PR_NUM, $PRJCODE, $PR_DATE, $tblPRD);
			...
			// START : UPDATE TO TRANS-COUNT
				...
				$parameters 	= array('DOC_CODE' 		=> $PR_NUM,			// TRANSACTION CODE
										...
										'TR_TYPE'		=> $DOCTYPE,		// TRANSACTION TYPE
										'TBL_NAME' 		=> $tblPRH,			// TABLE NAME
			...
			// START : ADD DOC HISTORY
				$this->load->model('m_updash/m_updash', '', TRUE);
				date_default_timezone_set("Asia/Jakarta");
				$paramTrack 	= array('REF_NUM' 		=> $PR_NUM,
			...
			// START : UPDATE QTY_COLLECTIVE
				$parameters 	= array('TR_TYPE'		=> $DOCTYPE,
										'TR_DATE' 		=> $PR_DATE,
										'PRJCODE'		=> $PRJCODE);
			...
			if($PR_STAT == 2)
			{
				$UPD_USR 	= $this->session->userdata['Emp_ID'];
				$UPD_TIME 	= date('Y-m-d H:i:s');
				$s_UPD 		= "UPDATE $tblPRH SET PR_CONFIRMER = '$UPD_USR', PR_CONFIRMED = '$UPD_TIME' WHERE PR_NUM = '$PR_NUM' AND PRJCODE = '$PRJCODE'";
				$this->db->query($s_UPD);
			}

			// START : UPDATE ITEM_LOG
				$parIL 	= array('DOC_CATEG'		=> $DOCTYPE,
								'DOC_NUM'		=> $PR_NUM,
								'DOC_STAT'		=> $PR_STAT,
								'PRJCODE'		=> $PRJCODE);
			...
			$url			= site_url('c_purchase/c_pr180d0c/pRQ_l5t_x/?SPPCAT='.$SPPCAT.'&id='.$this->url_encryption_helper->encode_url($PRJCODE));
			redirect($url);
		}
		else
		{
	...
	function update() // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$getpurreq 						= $this->m_purchase_req->get_MR_by_number($PR_NUM, $tblPRH)->row();
			...
			$data['default']['PR_DIVID'] 	= $getpurreq->PR_DIVID;
	...
	function update_process() // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			foreach($_POST['data'] as $d)
			{
				...
				$JOBID 		= $d['JOBCODEID'];
				$JOBIDH		= "";
					$sqlJID = "SELECT JOBPARENT FROM tbl_joblist_detail_$PRJCODEVW WHERE JOBCODEID = '$JOBID' LIMIT 1";
					$reJID	= $this->db->query($sqlJID)->result();
					foreach($reJID as $rowID) :
						$JOBIDH	= $rowID->JOBPARENT;
					endforeach;
			...
			else
			{
				...
				if($PRJHO == 'KTR')
				{
					// $s_DIV 	= "SELECT DISTINCT PR_DIVID FROM tbl_pr_detail 
					// 			WHERE PRJCODE = '$PRJCODE' AND PR_NUM = '$PR_NUM' AND PR_DIVID != '' AND PR_DIVID IS NOT NULL";
					$s_DIV 	= "SELECT B.JOBCOD1 FROM $tblPRD A INNER JOIN tbl_joblist_detail_$PRJCODEVW B ON A.JOBCODEID = B.JOBCODEID AND A.PRJCODE = B.PRJCODE
								WHERE A.PRJCODE = '$PRJCODE' AND A.PR_NUM = '$PR_NUM'
								ORDER BY A.PR_ID DESC LIMIT 1";
					$r_DIV 	= $this->db->query($s_DIV)->result();
			...
			$this->m_purchase_req->updateH($PR_NUM, $projMatReqH, $tblPRH);
			...
			if($PR_STAT == 1 || $PR_STAT == 2)
			{
				...
				$this->m_purchase_req->deleteDetailD($PR_NUM, $tblPRD);
				
				$maxNo	= 0;
				$sqlMax = "SELECT MAX(PR_ID) AS maxNo FROM $tblPRD";
				$resMax = $this->db->query($sqlMax)->result();
				foreach($resMax as $rowMax) :
					$maxNo = $rowMax->maxNo;		
				endforeach;
			...
			elseif($PR_STAT == 6)
			{
				foreach($_POST['data'] as $d)
				{
					$PR_NUM		= $d['PR_NUM'];
					$ITM_CODE	= $d['ITM_CODE'];
					$this->m_purchase_req->updateVolBud2($PR_NUM, $PRJCODE, $ITM_CODE, $tblPRH, $tblPRD);
				}
			}
			elseif($PR_STAT == 9)
			{
  	...
  	function get_AllData_1n2() // GOOD
	{
		...
		// START : FOR SERVER-SIDE
			...
			$start			= $_REQUEST['start'];
			$search			= $_REQUEST['search']["value"];
			$num_rows 		= $this->m_purchase_req->get_AllDataC_1n2($PRJCODE, $search, $length, $start);
			$total			= $num_rows;
			$output			= array();
			$output['draw']	= $draw;
			$output['recordsTotal'] = $output['recordsFiltered']= $total;
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				$output['data'][] 	= array("$noU.",
										  	...
										  	"<strong><i class='fa fa-user margin-r-5'></i>$empName</strong><br>
  	...
  	function get_AllData_1n24lt() // GOOD
	{
		...
		// START : FOR SERVER-SIDE
			...
			$start			= $_REQUEST['start'];
			$search			= $_REQUEST['search']["value"];
			$num_rows 		= $this->m_purchase_req->get_AllDataC_1n24lt($PRJCODE, $search, $length, $start);
			$total			= $num_rows;
			$output			= array();
			$output['draw']	= $draw;
			$output['recordsTotal'] = $output['recordsFiltered']= $total;
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				$JOBCODE	= $dataI['JOBCODE'];
				$JOBDESC	= $dataI['JOBDESC'];
				$PR_NOTE	= stripslashes($dataI['PR_NOTE']);
				//$JOBDESC	= "$JOBDESC - $PR_NOTE";
				$JOBDESC	= "$PR_NOTE";
				$PR_STAT	= $dataI['PR_STAT'];
				$STATDESC	= $dataI['STATDESC'];
	...
	function update_inb()
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$getpurreq 						= $this->m_purchase_req->get_MR_by_number_inb($PR_NUM)->row();
			$data['default']['PR_NUM'] 		= $getpurreq->PR_NUM;
			$data['default']['PR_CODE'] 	= $getpurreq->PR_CODE;
			$data['default']['PR_DATE'] 	= $getpurreq->PR_DATE;
			$data['default']['PR_RECEIPTD'] = $getpurreq->PR_RECEIPTD;
	...
	function update_process_inb() // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$PRJCODE 		= $this->input->post('PRJCODE');
			$PR_STAT		= $this->input->post('PR_STAT');
			$PR_NOTE2		= htmlspecialchars($this->input->post('PR_NOTE2'), ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401 | ENT_HTML5);
			
			$AH_CODE		= $PR_NUM;
			$AH_APPLEV		= $this->input->post('APP_LEVEL');
			$AH_APPROVER	= $DefEmp_ID;
			$AH_APPROVED	= $PR_APPROVED;
			$AH_NOTES		= htmlspecialchars($this->input->post('PR_NOTE2'), ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401 | ENT_HTML5);
			$PR_MEMO		= $this->input->post('PR_MEMO');
			...
			if($PR_STAT == 3)
			{
				...
				if($AH_ISLAST == 1 && $yesAPP == 1)
				{
					...
					// START : CREATE ALERT TO OP CREATER
						$AS_EMPNAME = "-";
						$AS_EMPNICK = "";
						$AS_MPHONE 	= "";
						$s_EMP		= "SELECT CONCAT(TRIM(First_Name),IF(Last_Name = '','',' '),TRIM(Last_Name)) AS COMPLNAME, Middle_Name,
										...
										FROM tbl_employee WHERE Emp_ID IN (SELECT DISTINCT emp_id FROM tusermenu WHERE ISCREATE = 1 AND menu_code = 'MN019' AND emp_id IN (SELECT DISTINCT Emp_ID FROM tbl_employee_proj WHERE proj_Code = '$PRJCODE')) AND Emp_Status = 1 AND Employee_status = 1 AND writeEMP = 1";
						...
						foreach($r_EMP as $rw_EMP) :
							...
							// START : ALERT WA PROCEDURE
								$JSON_DATA = '{"token":"1f0e316f043fce1b5a52242463d88ae08a8aafe1","priority ":0,"application":"2","sleep":0,"globalmessage":"","globalmedia":"","data":[{"number":"'.$AS_MPHONE.'","message":"Halo Bapak/Ibu *'.$AS_EMPNAME.'*,\n\nAda dokumen SPP yang baru saja disetujui dan harus segera Anda buatkan PO. \n\n'.$PR_CODE.'\n\nMohon segera untuk ditindaklanjuti.\nhttps://sdbpplus.nke.co.id/\n\n Terimakasih. \n *_NKE Smart System_*"}]}';
						...
						endforeach;
					// END : CREATE ALERT TO OP CREATER
				}
				...
				{*/
					...
					else
					{
						...
						{
							// $s_DIV 	= "SELECT DISTINCT PR_DIVID FROM tbl_pr_detail WHERE PRJCODE = '$PRJCODE' AND PR_NUM = '$PR_NUM' AND PR_DIVID != '' AND PR_DIVID IS NOT NULL";
							$s_DIV 	= "SELECT B.JOBCOD1 FROM tbl_pr_detail A INNER JOIN tbl_joblist_detail B ON A.JOBCODEID = B.JOBCODEID AND A.PRJCODE = B.PRJCODE
										WHERE A.PRJCODE = '$PRJCODE' AND A.PR_NUM = '$PR_NUM'
										ORDER BY A.PR_ID DESC LIMIT 1";
							$r_DIV 	= $this->db->query($s_DIV)->result();
							foreach($r_DIV as $rw_DIV):
				...
				// START : CREATE QRCODE
					...
					if (!file_exists("assets/AdminLTE-2.0.5/qrcodelist/ACC SPP/$PRJCODE")) {
						mkdir("assets/AdminLTE-2.0.5/qrcodelist/ACC SPP/$PRJCODE", 0777, true);
					}

					$config["cacheable"]    = true; //boolean, the default is true
					$config["cachedir"]     = "assets/AdminLTE-2.0.5/qrcodelist/cache/"; //string, the default is application/cache/
					$config["errorlog"]     = "assets/AdminLTE-2.0.5/qrcodelist/logs/"; //string, the default is application/logs/
					$config["imagedir"]     = "assets/AdminLTE-2.0.5/qrcodelist/ACC_SPP/$PRJCODE/"; //direktori penyimpanan qr code
					...
					$QRCODE_DESC = "$AH_APPROVER"; 
					$image_name=$QRCODE_DESC.'.png';

					$params['data'] = $QRCODE_DESC; //data yang akan di jadikan QR CODE
	...
	function pRn_P0l()
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$PR_DATE 		= "";
			$PR_NOTE 		= "";
			$sqlPR 			= "SELECT PRJCODE, PR_CODE, PR_DATE, PR_NOTE FROM tbl_pr_header WHERE PR_NUM = '$PR_NUM'";
			$resultaPR 		= $this->db->query($sqlPR)->result();
			foreach($resultaPR as $rowPR) :
				$PRJCODE 	= $rowPR->PRJCODE;	
  	...
  	function get_AllDataITMM() // GOOD
	{
		...
		// START : FOR SERVER-SIDE
			...
			$start			= $_REQUEST['start'];
			$search			= $_REQUEST['search']["value"];
			$num_rows 		= $this->m_purchase_req->get_AllDataITMC($PRJCODE, $search, $length, $start);
			$total			= $num_rows;
			$output			= array();
			$output['draw']	= $draw;
			$output['recordsTotal'] = $output['recordsFiltered']= $total;
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				$JOBPARENT 		= $dataI['JOBPARENT'];
				$JOBCODE 		= $dataI['JOBCODE'];
				$JOBDESC		= htmlspecialchars($dataI['JOBDESC'], ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401 | ENT_HTML5);
				$ITM_GROUP		= $dataI['ITM_GROUP'];
				$ITM_CODE		= $dataI['ITM_CODE'];
				$ITM_UNIT		= $dataI['ITM_UNIT'];
		        ...
		        // GET TOTAL SPP AND REMAIN IN THIS DOCUMENT
		        	...
		        	$TOT_PRAMN 		= 0;
		        	$TOT_PRAMNCNC	= 0;
					$s_01 			= "SELECT SUM(PR_VOL + PR_VOL_R - PR_CVOL) AS TOT_PRVOL, SUM(PO_VAL + PO_VAL_R) AS TOT_PRAMN
										FROM tbl_joblist_detail_$PRJCODEVW
										WHERE ITM_CODE = '$ITM_CODE' AND JOBCODEID = '$JOBCODEID' AND PRJCODE = '$PRJCODE'
										UNION
										SELECT SUM(PR_VOLM) AS TOT_PRVOL, SUM(PO_AMOUNT) AS TOT_PRAMN
										FROM tbl_pr_detail_tmp
										WHERE ITM_CODE = '$ITM_CODE' AND JOBCODEID = '$JOBCODEID' AND PRJCODE = '$PRJCODE'";
					$r_01 			= $this->db->query($s_01)->result();
					foreach($r_01 as $rw_01):
				...
				$TOT_REQVOL 	= $TREQ_VOL + $TREQ_VOL_R;
				$TOT_REQVAL 	= $TREQ_VAL + $TREQ_VAL_R;

				$TOT_UM_VOL 	= $TUSED_VOL+$TUSED_VOL_R;
				$TOT_UM_VAL 	= $TUSED_VAL+$TUSED_VAL_R;

				$s_isLS 		= "tbl_unitls WHERE ITM_UNIT = '$JOBUNIT'";
				$r_isLS 		= $this->db->count_all($s_isLS);
				if($r_isLS == 1)
				{
					$MAX_REQVOL		= $TBUDG_VOL - $TOT_REQVOL;
					$MAX_REQVAL		= $TBUDG_VAL - $TOT_REQVAL;
					$MAX_REQPR 		= round($TBUDG_VAL - $TOT_REQVAL,2);

					$TOT_UM_VOLV 	= $TOT_UM_VAL;

					$disabledB		= 0;
					if($MAX_REQPR <= 0)
						$disabledB	= 1;

					$REM_BUDGET 	= $MAX_REQPR;
				}
				else
				{
					$MAX_REQVOL		= $TBUDG_VOL - $TOT_REQVOL;
					$MAX_REQVAL		= $TBUDG_VAL - $TOT_REQVAL;
					$MAX_REQPR 		= round($TBUDG_VOL - $TOT_REQVOL,2);

					$TOT_UM_VOLV 	= $TOT_UM_VOL;
				...
				// IS LAST SETT
					...
					else
					{
						...
						$JOBUNITV 	= $JOBUNIT;
						$JOBVOLMV	= number_format($TBUDG_VOL, 2);
						$TOT_REQVOLV= number_format($TOT_REQVOL, 2);
						$TOT_UM_VOLV= number_format($TOT_REQVOL, 2);
						$PO_VOLMV	= number_format($PO_VOLM, 2);
						if($disabledB == 0)
							$statRem = number_format($REM_BUDGET, 2);
						else
							$statRem = "<span class='label label-danger' style='font-size:12px'>".number_format($REM_BUDGET, 2)."</span>";

						if($r_isLS == 1)
						{
							$JOBVOLMV	= number_format($TBUDG_VAL, 2);
							$TOT_REQVOLV= number_format($TOT_REQVAL, 2);
							$TOT_UM_VOLV= number_format($TOT_UM_VOL, 2);
							if($disabledB == 0)
								$statRem = number_format($REM_BUDGET, 2);
							else
							{
								//$statRem = "<span class='label label-danger' style='font-size:12px'>".number_format($REM_BUDGET, 2)."</span>";
								$statRem = "<span class='label label-danger' style='font-size:12px' title='".number_format($REM_BUDGET, 2)."'>".number_format(0, 2)."</span>";
							}
						}
					}
				...
				// OTHER SETT
					if($disabledB == 0)
					{
						$chkBox		= "<input type='checkbox' name='chk1' value='".$JOBCODEDET."|".$JOBCODEID."|".$JOBCODE."|".$PRJCODE."|".$ITM_CODE."|".$JOBDESC1."|".$serialNumber."|".$ITM_UNIT."|".number_format($ITM_PRICE, 2, '.', '')."|".number_format($TBUDG_VOL, 2, '.', '')."|".number_format($ITM_STOCK, 2, '.', '')."|".number_format($ITM_USED, 2, '.', '')."|".$itemConvertion."|".$TBUDG_VAL."|".$MAX_REQVOL."|".$PO_AMOUNT."|".$TBUDG_VOL."|".$TOT_REQVOL."|".$ITM_TYPE."|".$JOBDESCH."' onClick='pickThis1(this);'/>";
					}
					else
					{
				...
				$RecItemV 	= 	"<input type='hidden' name='urlRec".$noU."' id='urlRec".$noU."' value='".$RecItem."'>
								<a onClick='recountItm(".$noU.");' title='Hitung ulang permintaan' class='pull-left' style='cursor: pointer'><i class='fa fa-repeat'></i></a>";
				...
				$output['data'][] 	= array("<div style='text-align:center;'>".$chkBox."</div>",
											...
											"<div style='text-align:center;'><span ".$CELL_COL.">".$JOBUNITV."</span></div>",
											"<div style='text-align:right;'><span ".$CELL_COL.">".$JOBVOLMV.$ADDVOL_VW."</span></div>",
											"<div style='text-align:right;'>".$RecItemV."&nbsp;&nbsp;&nbsp;<span ".$CELL_COL.">".$TOT_REQVOLV."</span></div>",
											"<div style='text-align:right;'><span ".$CELL_COL.">".$PO_VOLMV."</span></div>",
											"<div style='text-align:right;'>".$statRem."</div>");
  	...
  	function get_AllDataITMT() // GOOD
	{
		...
		// START : FOR SERVER-SIDE
			...
			$start			= $_REQUEST['start'];
			$search			= $_REQUEST['search']["value"];
			$num_rows 		= $this->m_purchase_req->get_AllDataITMTC($PRJCODE, $search, $length, $start);
			$total			= $num_rows;
			$output			= array();
			$output['draw']	= $draw;
			$output['recordsTotal'] = $output['recordsFiltered']= $total;
			...
			$query 			= $this->m_purchase_req->get_AllDataITMTL($PRJCODE, $search, $length, $start, $order, $dir);
  	...
  	function get_AllDataITMS() // GOOD
	{
		...
		// START : FOR SERVER-SIDE
			...
			$start			= $_REQUEST['start'];
			$search			= $_REQUEST['search']["value"];
			$num_rows 		= $this->m_purchase_req->get_AllDataITMSC($PRJCODE, $search, $length, $start);
			$total			= $num_rows;
			$output			= array();
			$output['draw']	= $draw;
			$output['recordsTotal'] = $output['recordsFiltered']= $total;
  	...
  	function get_AllDataITMSCUT_230303() // GOOD
	{
		//$PRJCODE		= $_GET['id'];
		...
		// START : FOR SERVER-SIDE
			...
			$start			= $_REQUEST['start'];
			$search			= $_REQUEST['search']["value"];
			$num_rows 		= $this->m_purchase_req->get_AllDataITMSCUTC($PRJCODE, $search, $length, $start);
			$total			= $num_rows;
			$output			= array();
			$output['draw']	= $draw;
			$output['recordsTotal'] = $output['recordsFiltered']= $total;
  	...
  	function get_AllDataITMSCUT() // GOOD
	{
		...
		// START : FOR SERVER-SIDE
			...
			foreach ($query->result_array() as $dataI) 
			{
				// RENDER
					...
					$ITM_RVAL 		= 0;
					$ITM_RVAL_R 	= 0;
					$sqlJOBD		= "SELECT 	SUM(AMD_VOL - AMDM_VOL) AS TOT_AMD_VOL,
												SUM(AMD_VAL - AMDM_VAL) AS TOT_AMD_VAL,
												SUM(PR_VOL 	+ WO_VOL 	+ VCASH_VOL 	+ VLK_VOL 	+ PPD_VOL	- PR_CVOL	- WO_CVOL) AS ITM_RVOL,
												SUM(PO_VAL 	+ WO_VAL 	+ VCASH_VAL 	+ VLK_VAL 	+ PPD_VAL	- PO_CVAL	- WO_CVAL) AS ITM_RVAL,
												SUM(PR_VOL_R 	+ WO_VOL_R 	+ VCASH_VOL_R 	+ VLK_VOL_R + PPD_VOL_R) AS ITM_RVOL_R,
												SUM(PO_VAL_R 	+ WO_VAL_R 	+ VCASH_VAL_R 	+ VLK_VAL_R + PPD_VAL_R) AS ITM_RVAL_R
					...
					foreach($resJOBD as $rowJOBD) :
						//$TOT_BVOL		= $rowJOBD->TOT_BVOL;
						//$TOT_BVAL		= $rowJOBD->TOT_BVAL;
						$TOT_AMD_VOL	= $rowJOBD->TOT_AMD_VOL;
						$TOT_AMD_VAL	= $rowJOBD->TOT_AMD_VAL;
						$ITM_RVOL		= $rowJOBD->ITM_RVOL;
						$ITM_RVAL		= $rowJOBD->ITM_RVAL;
						$ITM_RVOL_R		= $rowJOBD->ITM_RVOL_R;
						$ITM_RVAL_R		= $rowJOBD->ITM_RVAL_R;
					endforeach;

					$JOBPRICE		= $dataI['ITM_PRICE'];
					$ITM_PRICE		= $dataI['ITM_PRICE'];
					$ITM_LASTP		= $dataI['ITM_PRICE'];
					$JOBCOST		= $TOT_BVOL * $JOBPRICE;
					$TOT_AMD_VOL 	= $TOT_AMD_VOL;
					$TOT_AMD_VOLP 	= $TOT_AMD_VOL;
					if($TOT_AMD_VOL == 0)
						$TOT_AMD_VOLP 	= 1;

					$ADD_PRICE		= $TOT_AMD_VAL / $TOT_AMD_VOLP;
					$IS_LEVEL		= 0;
					$ISLAST			= 1;

					$serialNumber	= '';
					$itemConvertion	= 1;
					$TOT_RAP_VOL 	= $TOT_BVOL + $TOT_AMD_VOL;
					$TOT_RAP_VOLV 	= "$TOT_BVOL + $TOT_AMD_VOL";
					$TOT_RAP_VAL	= $TOT_BVAL + $TOT_AMD_VAL;
					...
					if($r_isLS == 1)
					{
						$MAX_REQ	= $TOT_RAP_VAL - $ITM_RVAL - $ITM_RVAL_R;		// 14
						if($MAX_REQ <= 0)
							$disabledB	= 1;

						/*$ITM_PRICE		= 1;
						$TOT_RAP_VOL	= $TOT_RAP_VAL;*/
					}
					else
					{
						$MAX_REQ	= $TOT_RAP_VOL - $ITM_RVOL - $ITM_RVOL_R;		// 14
						$MAX_REQV	= "$TOT_RAP_VOL - $ITM_RVOL - $ITM_RVOL_R";		// 14
						if($MAX_REQ <= 0)
							$disabledB	= 1;
					}
				...
				// IS LAST SETT
					if($ISLAST == 0)	// HEADER
					{
						...
						$STATCOL	= 'primary';
						$CELL_COL	= "style='font-weight:bold; white-space:nowrap'";
						$statRem 	= "<input type='hidden' id='MAXREQ_".$noU."' size='20' value='".$MAX_REQ."' class='form-control' style='min-width:110px; max-width:500px;'>";
						$JOBUNITV 	= "";
						$TOT_BVOLV	= number_format(0, 2);
						$ITM_RVOLV	= number_format(0, 2);
					}
					else
					{
						$strLEN 	= strlen($JOBDESC);
						...
						$JOBUNITV 	= $JOBUNIT;
						$TOT_BVOLV	= number_format($TOT_BVOL, 2);
						$ITM_RVOLV	= number_format($ITM_RVOL, 2);
						if($disabledB == 0)
							$statRem = number_format($MAX_REQ, 2)."<input type='hidden' id='MAXREQ_".$noU."' size='20' value='".$MAX_REQ."' class='form-control' style='min-width:110px; max-width:500px;'>";
						else
							$statRem = "<input type='hidden' id='MAXREQ_".$noU."' size='20' value='".$MAX_REQ."' class='form-control' style='min-width:110px; max-width:500px;'><span class='label label-danger' style='font-size:12px'>".number_format($MAX_REQ, 2)."</span>";
					}

				// OTHER SETT
					if($disabledB == 0)
						$chkBox		= "<input type='radio' name='chkRad' value='".$PRJCODE."|".$ITM_CODE."' onClick='pickThis3(this, ".$noU.");'/>";
					else
						$chkBox		= "<input type='radio' name='chkRad' value='' style='display: none' />";
					...
					$ADDVOL_VW 		= "";
					if($TOT_AMD_VOL > 0 || $TOT_AMD_VAL > 0)
					{
						$ADDVOL_VW 	= 	"<div>
											<p class='text-yellow'><i class='glyphicon glyphicon-triangle-top'></i>
									  		".number_format($TOT_AMD_VOL, 2)."</p>
									  	</div>";
					}
				...
				$output['data'][] 	= array("<div style='text-align:center;'>".$chkBox."</div>",
											"<div class='text-wrap'>
										  	...
										  	<i class='text-muted fa fa-cube'></i>&nbsp;&nbsp;$ITM_CODE ($JOBUNITV)
									  		<div style='margin-left: 17px; font-style: italic;'>$BudgetQty : ".$TOT_BVOLV." $Requested : ".$ITM_RVOLV."
									  		</div>",
											"<div style='text-align:right;'>".$statRem."</div>");
				$noU			= $noU + 1;
			}
  	...
  	function get_AllDataITMSCUT4_230303() // GOOD
	{
		//setlocale(LC_ALL, 'id-ID', 'id_ID');
		setlocale(LC_ALL, 'en_EN');
		...
		// START : FOR SERVER-SIDE
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				// START : GET PR DETAIL
					...
					$REQ_VOL	= $dataI['REQ_VOL'];
					$REQ_VAL	= $dataI['REQ_VAL'];
					$REM_ITM_VOL= $dataI['REM_ITM_VOL'];
					$REM_ITM_VAL= $dataI['REM_ITM_VAL'];
					$TOT_PRVOL	= $REQ_VOL;
					$TOT_PRAMN	= $REQ_VAL;
				// END : GET PR DETAIL
  	...
  	function get_AllDataITMSCUT4() // GOOD
	{
  	...
  	function get_AllDataITMALL() // GOOD
	{
		//setlocale(LC_ALL, 'id-ID', 'id_ID');
		setlocale(LC_ALL, 'en_EN');
		...
		// START : FOR SERVER-SIDE
			...
			$start			= $_REQUEST['start'];
			$search			= $_REQUEST['search']["value"];
			$num_rows 		= $this->m_purchase_req->get_AllDataITMALLC($PRJCODE, $ITMCAT, $search, $length, $start);
			$total			= $num_rows;
			$output			= array();
			$output['draw']	= $draw;
			$output['recordsTotal'] = $output['recordsFiltered']= $total;
			...
			$query 			= $this->m_purchase_req->get_AllDataITMALLL($PRJCODE, $ITMCAT, $search, $length, $start, $order, $dir);
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				// START : GET BUDGET
					...
					foreach($r_BUDGVAL as $rowJOBDR) :
						...
						$TREQ_VOL 		= ($PR_VOL - $PR_CVOL) + ($WO_VOL - $WO_CVOL) + $VCASH_VOL + $VLK_VOL + $PPD_VOL;
						$TREQ_VAL 		= ($PO_VAL - $PO_CVAL) + ($WO_VAL - $WO_CVAL) + $VCASH_VAL + $VLK_VAL + $PPD_VAL;
				...
				$ADD_VOLM 	= $AMD_VOL - $AMDM_VOL;
				$ADD_VALM 	= $AMD_VAL - $AMDM_VAL;
				...
				// IS LAST SETT
					...
					else
					{
						...
						if($r_isLS == 1)
						{
							$JOBVOLMV	= number_format($TBUDG_VAL, 2);
							$TOT_REQVOLV= number_format($TOT_REQVAL, 2);
							$TOT_UM_VOLV= number_format($TOT_UM_VAL, 2);
							if($disabledB == 0)
								$statRem = number_format($REM_BUDGET, 2);
							else
							{
				...
				// OTHER SETT
					...
					$ADDVOL_VW 		= "";
					if($ADD_VOLM > 0 || $ADD_VALM > 0)
					{
						$ADDVOL_VW 	= 	"<div>
											<p class='text-yellow'><i class='glyphicon glyphicon-triangle-top'></i>
									  		".number_format($ADDVAL, 2)."</p>
									  	</div>";
					}
				...
				$output['data'][] 	= array("<div style='text-align:center;'>".$chkBox."</div>",
											...
											"<div style='text-align:right;'><span ".$CELL_COL.">".$JOBVOLMV.$ADDVOL_VW."</span></div>",
											"<div style='text-align:right;'><span ".$CELL_COL.">".$TOT_REQVOLV."</span></div>",
											"<div style='text-align:right;'><span ".$CELL_COL.">".$TOT_UM_VOLV."</span></div>",
											"<div style='text-align:right;'>".$RecItemV."&nbsp;&nbsp;&nbsp;".$statRem."</div>");

				$noU			= $noU + 1;
			}

			/*$output['data'][] 	= array("A = $ITMCAT",
											"A",
											"A",
											"A",
											"A",
  	...
  	function get_AllDataITMO() // GOOD
	{
		...
		// START : FOR SERVER-SIDE
			...
			$start			= $_REQUEST['start'];
			$search			= $_REQUEST['search']["value"];
			$num_rows 		= $this->m_purchase_req->get_AllDataITMOC($PRJCODE, $search, $length, $start);
			$total			= $num_rows;
			$output			= array();
			$output['draw']	= $draw;
			$output['recordsTotal'] = $output['recordsFiltered']= $total;
	...
	function addItmTmp()
	{
		...
		$PRJCODE 		= $arrItem[3];
		$ITM_CODE 		= $arrItem[4];
		$ITM_NAME 		= htmlspecialchars($arrItem[5], ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401 | ENT_HTML5);
		$ITM_SN			= $arrItem[6];
		$ITM_UNIT 		= $arrItem[7];
		$ITM_PRICE 		= $arrItem[8];
		$ITM_VOLM 		= $arrItem[9];
		...
		$ITM_TYPE		= $arrItem[18];
		$JOBDESCH 		= htmlspecialchars($arrItem[19], ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401 | ENT_HTML5);
		$PR_NUM 		= $arrItem[20];
		$PR_CODE 		= $arrItem[21];
		...
		// TOTAL YANG DIREQUEST ADALAH TOTAL YANG SUDAH DI PR (1,2,4) + TEMP
			/*$TOT_PR 	= 0;
			$s_PRT 		= "SELECT SUM(PR_VOL) AS TOT_PR
							FROM tbl_joblist_detail_$PRJCODEVW
							WHERE ITM_CODE = '$ITM_CODE' AND JOBCODEID = '$JOBCODEID' AND PRJCODE = '$PRJCODE'
							UNION
			...
			$s_TMPPRT 	= "SELECT SUM(PR_VOLM) AS TOT_PR FROM tbl_pr_detail_tmp WHERE ITM_CODE = '$ITM_CODE' AND JOBCODEID = '$JOBCODEID'
							AND PRJCODE = '$PRJCODE' AND PR_ID != $PR_ID AND PR_NUM = '$PR_NUM'";
        ...
        foreach($resJPAR as $rowJPAR) :
            ...
            $JOBPARDESC	= htmlspecialchars($rowJPAR->JOBDESC, ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401 | ENT_HTML5);
        endforeach;
		...
		$s_00 	= 	"INSERT INTO tbl_pr_detail_tmp
						...
						VALUES
						('$PR_ID', '$PR_NUM', '$PR_CODE', '$PRJCODE', '$DEPCODE', '$JOBCODEDET', '$JOBCODEID', '$JOBPARENT', '$JOBPARDESC',
						'$ITM_CODE', '$ITM_NAME', '$ITM_UNIT', '$ITM_UNIT', '$tempTotMax', '$ITM_PRICE', '$PR_TOTAL', '',
						'$ITM_VOLMBG', '$ITM_BUDG', 1)";
		$this->db->query($s_00);
  	...
  	function get_AllDataITMSCUT4SV() // GOOD
	{
		...
		$s_00 	= 	"INSERT INTO tbl_pr_detail_tmp
						...
						ITM_CODE, ITM_NAME, ITM_UNIT, ITM_UNIT2, PR_VOLM, PR_PRICE, PR_TOTAL, PR_DESC_ID, PR_DESC,
						ITM_VOLMBG, ITM_BUDG, PR_STAT)
						VALUES
						('$PR_ID', '$PR_NUM', '$PR_CODE', '', '$PRJCODE', '$DEPCODE', '$JOBCODEDET', '$JOBCODEID', '$JOBPARENT', '$JOBPARDESC',
						'$ITM_CODE', '$ITM_NAME', '$ITM_UNIT', '$ITM_UNIT', '$TOT_ITMREM', '$ITM_PRICE', '$PR_TOTAL', '', '',
						'$ITM_VOLMBG', '$ITM_BUDG', 1)";
		$this->db->query($s_00);
	...
	function cancelItem()
	{
		...
		$s_01 			= "SELECT PR_NUM, PR_CODE, JOBCODEID, PR_VOLM, PR_PRICE, PR_TOTAL, PO_VOLM, PO_AMOUNT
							FROM tbl_pr_detail
		...
		foreach($r_01 as $rw_01) :
			...
			// ADD HISTORY
				...
				$s_05 	= 	"INSERT INTO tbl_pr_detail_canc (PR_ID, PR_NUM, PR_CODE, PR_DATE, PRJCODE, DEPCODE, PR_REFNO,
								JOBCODEDET, JOBCODEID, JOBPARENT, JOBPARDESC, ITM_CODE, SNCODE, ITM_UNIT, ITM_UNIT2, PR_VOLM, PR_VOLM2, PR_PRICE,
			...
			// ADD HISTORY ITEM VOID
				...
				$s_06 	= 	"INSERT INTO tbl_item_void (V_NUM, V_CODE, V_DATE, V_CATEG, V_DESC, PRJCODE, REF_NUM, REF_CODE, JOBCODEID, JOBDESC,
								...
								VALUES ('$V_NUM', '$PR_CODE', '$CANC_TIME', 'PR', '', '$PRJCODE', '$PR_NUM', '$PR_CODE', '$JOBCODEID', '$ITM_NAME',
  	...
  	function get_AllDataDESC() // GOOD
	{
		...
		// START : FOR SERVER-SIDE
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				$ITM_SPEC 		= $dataI['ITM_SPEC'];

				$chkBox			= "<input type='radio' name='chk5' value='".$DESC_ID."|".$DESC_NOTES."|".$ITM_SPEC."' onClick='pickThis5();'/>";
	...
	function updDESC()
	{
		...
		$s_00 		= "UPDATE tbl_pr_detail SET PR_DESC_ID = '$DESC_ID', PR_DESC = '$DESC_NOTES', PR_SPEC = '$ITM_SPEC'
						WHERE PR_NUM = '$PR_NUM' AND PR_ID = $PR_ID AND PRJCODE = '$PRJCODE'";
		$this->db->query($s_00);

		$s_01 		= "UPDATE tbl_pr_detail_tmp SET PR_DESC_ID = '$DESC_ID', PR_DESC = '$DESC_NOTES', PR_SPEC = '$ITM_SPEC'
						WHERE PR_NUM = '$PR_NUM' AND PR_ID = $PR_ID AND PRJCODE = '$PRJCODE'";
		$this->db->query($s_01);
		echo $s_00;
	}
	...
	function addDesc()
	{
		...
		$ITMCODE 	= $arrItem[0];
		$ITMDESC 	= nl2br(htmlspecialchars($arrItem[1], ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401 | ENT_HTML5));
	...
	function get_ITMSRC()
	{
		...
		$data 		= $this->m_purchase_req->getITM_SRC($PRJCODE, $PR_NUM)->result();
		echo json_encode($data);
	}
	...
	function get_PRDESCSRC()
	{
		...
		$data 		= $this->m_purchase_req->getPRDESC_SRC($PRJCODE, $PR_NUM, $ITM_CODE)->result();
		echo json_encode($data);
	}
 	...
 	function fpa()
	{
		...
		$url			= site_url('c_purchase/c_pr180d0c/pL15t_fpa/?id='.$this->url_encryption_helper->encode_url($appName));
		redirect($url);
	}
	
	function pL15t_fpa() // G
	{
		$this->load->model('m_projectlist/m_projectlist', '', TRUE);
		$this->load->model('m_purchase/m_purchase_req/m_purchase_req', '', TRUE);
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data["SPPCAT"] 	= 'FPA';
			$data['viewPRJ'] 	= $this->m_projectlist->get_all_project($DefEmp_ID)->result();
			//$data["secURL"] 	= "c_purchase/c_pr180d0c/pRQ_l5t_x_fpa/?id=";
			$data["secURL"] 	= "c_purchase/c_pr180d0c/pRQ_l5t_x/?id=";
	...
	function pRQ_l5t_x_fpa() // G
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data['title'] 		= $appName;
			$data['addURL'] 	= site_url('c_purchase/c_pr180d0c/add_fpa/?id='.$this->url_encryption_helper->encode_url($PRJCODE));
			$data['backURL'] 	= site_url('c_purchase/c_pr180d0c/fpa/?id='.$this->url_encryption_helper->encode_url($PRJCODE));
			$data['PRJCODE'] 	= $PRJCODE;
			...
			$this->load->view('v_purchase/v_purchase_req/purchase_req_fpa', $data);
		}
		else
		{
			redirect('__l1y');
  	...
  	function get_AllDataPRGRP_fpa() // GOOD
	{
		$PRJCODE		= $_GET['id'];
		$SPLCODE		= $_GET['SPLC'];
		$PO_NUM			= $_GET['REFNO'];
		...
		// START : FOR SERVER-SIDE
			...
			{							
				...
				$CollCode	= "$PRJCODE~$PR_NUM";
				$secUpd		= site_url('c_purchase/c_pr180d0c/update_fpa/?id='.$this->url_encryption_helper->encode_url($CollCode));
				$secPPOList	= site_url('c_purchase/c_pr180d0c/pRn_P0l/?id='.$this->url_encryption_helper->encode_url($PR_NUM));
				$secPrint	= site_url('c_purchase/c_pr180d0c/printdocument_fpa/?id='.$this->url_encryption_helper->encode_url($PR_NUM));
				$CollID		= "PR~$PR_NUM~$PRJCODE";
				$secDel_DOC = base_url().'index.php/__l1y/trash_DOC/?id='.$CollID;
				$secDelIcut = base_url().'index.php/__l1y/trashDOC/?id=';
				$delID 		= "$secDelIcut~tbl_pr_header_fpa~tbl_pr_detail_fpa~PR_NUM~$PR_NUM~PRJCODE~$PRJCODE";
				$secVoid 	= base_url().'index.php/__l1y/trashPR/?id=';
				$voidID 	= "$secVoid~tbl_pr_header_fpa~tbl_pr_detail_fpa~PR_NUM~$PR_NUM~PRJCODE~$PRJCODE";
				...
				if($PR_STAT == 1 || $PR_STAT == 4)
				{
					$secAction	= 	"<input type='hidden' name='urlPOList".$noU."' id='urlPOList".$noU."' value='".$secPPOList."'>
								   	...
								   	<a href='javascript:void(null);' style='display: none' class='btn btn-warning btn-xs' title='Tampilkan OP' disabled='disabled'>
										<i class='glyphicon glyphicon-list'></i>
									</a>
									<a href='javascript:void(null);' class='btn btn-primary btn-xs' disabled='disabled'>
										<i class='glyphicon glyphicon-print'></i>
				...
				elseif($PR_STAT == 2 || $PR_STAT == 3 || $PR_STAT == 6)
				{
					$secAction	= 	"<input type='hidden' name='urlPOList".$noU."' id='urlPOList".$noU."' value='".$secPPOList."'>
									...
									<input type='hidden' name='urlVoid".$noU."' id='urlVoid".$noU."' value='".$voidID."'>
								   	...
								   	<a href='javascript:void(null);' style='display: none' class='btn btn-warning btn-xs' title='Tampilkan OP' onClick='pRn_P0l(".$noU.")'>
										<i class='glyphicon glyphicon-list'></i>
									</a>
									<a href='javascript:void(null);' class='btn btn-primary btn-xs' onClick='printD(".$noU.")'>
										<i class='glyphicon glyphicon-print'></i>
	...
	function add_fpa_230303() // G
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data['title'] 		= $appName;
			$data['task'] 		= 'add';
			$data['form_action']= site_url('c_purchase/c_pr180d0c/add_process_fpa');
			$data['backURL'] 	= site_url('c_purchase/c_pr180d0c/pRQ_l5t_x_fpa/?id='.$this->url_encryption_helper->encode_url($PRJCODE));
			$data['PRJCODE'] 	= $PRJCODE;
			...
			$this->load->view('v_purchase/v_purchase_req/purchase_req_form_fpa', $data);
		}
		else
		{
			redirect('__l1y');
	...
	function add_fpa() // G
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data['task'] 		= 'add';
			$data["SPPCAT"] 	= 'FPA';
			$data["tblPRH"] 	= 'tbl_pr_header_fpa';
			$data["tblPRD"] 	= 'tbl_pr_detail_fpa';
			$data['form_action']= site_url('c_purchase/c_pr180d0c/add_process_fpa');
			$data['backURL'] 	= site_url('c_purchase/c_pr180d0c/pRQ_l5t_x/?id='.$this->url_encryption_helper->encode_url($PRJCODE));
			$data['PRJCODE'] 	= $PRJCODE;
	...
	function add_process_fpa() // G
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$maxNo	= 0;
			$sqlMax = "SELECT MAX(PR_ID) AS maxNo FROM tbl_pr_detail_fpa WHERE PR_NUM = '$PR_NUM' AND PRJCODE = '$PRJCODE'";
			$resMax = $this->db->query($sqlMax)->result();
			foreach($resMax as $rowMax) :
				$maxNo = $rowMax->maxNo;		
			endforeach;
			...
			foreach($_POST['data'] as $d)
			{
				...
				if($PRJHO == 'KTR')
				{
					...
					$s_MN 	= "SELECT B.JOBCOD1 FROM tbl_pr_detail_fpa A INNER JOIN tbl_joblist_detail B ON A.JOBCODEID = B.JOBCODEID AND A.PRJCODE = B.PRJCODE
								WHERE A.PRJCODE = '$PRJCODE' AND A.PR_NUM = '$PR_NUM'
								ORDER BY A.PR_ID DESC LIMIT 1";
					$r_MN	= $this->db->query($s_MN)->result();
					foreach($r_MN as $rw_MN) :
				...
				$this->db->insert('tbl_pr_detail_fpa',$d);
			...
			else
			{
				...
				if($PRJHO == 'KTR')
				{
					// $s_DIV 	= "SELECT DISTINCT PR_DIVID FROM tbl_pr_detail_fpa WHERE PRJCODE = '$PRJCODE' AND PR_NUM = '$PR_NUM' AND PR_DIVID != '' AND PR_DIVID IS NOT NULL";
					$s_DIV 	= "SELECT B.JOBCOD1 FROM tbl_pr_detail_fpa A INNER JOIN tbl_joblist_detail B ON A.JOBCODEID = B.JOBCODEID AND A.PRJCODE = B.PRJCODE
								WHERE A.PRJCODE = '$PRJCODE' AND A.PR_NUM = '$PR_NUM'
								ORDER BY A.PR_ID DESC LIMIT 1";
					$r_DIV 	= $this->db->query($s_DIV)->result();
					foreach($r_DIV as $rw_DIV):
			...
			$projMatReqH 	= array('PR_NUM' 			=> $PR_NUM,
									...
									'Patt_Date'			=> $Patt_Date,
									'Patt_Number'		=> $Patt_Number);
			$this->m_purchase_req->add_fpa($projMatReqH);
			
			// UPDATE DETAIL
				$this->m_purchase_req->updateDet_fpa($PR_NUM, $PRJCODE, $PR_DATE);
			...
			// START : UPDATE TO TRANS-COUNT
				...
				$parameters 	= array('DOC_CODE' 		=> $PR_NUM,			// TRANSACTION CODE
										...
										'TBL_NAME' 		=> "tbl_pr_header_fpa",	// TABLE NAME
										'KEY_NAME'		=> "PR_NUM",		// KEY OF THE TABLE
										'STAT_NAME' 	=> "PR_STAT",		// NAMA FIELD STATUS
										'STATDOC' 		=> $PR_STAT,		// TRANSACTION STATUS
										'STATDOCBEF'	=> $STAT_BEFORE,	// TRANSACTION STATUS
			...
			$url			= site_url('c_purchase/c_pr180d0c/pRQ_l5t_x_fpa/?id='.$this->url_encryption_helper->encode_url($PRJCODE));
			redirect($url);
		}
		else
		{
	...
	function update_fpa() // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data['title'] 		= $appName;
			$data['task'] 		= 'edit';
			$data['form_action']= site_url('c_purchase/c_pr180d0c/update_process_fpa');
			
			$data['countPRJ']	= $this->m_projectlist->count_all_project($DefEmp_ID);
			$data['vwPRJ'] 		= $this->m_projectlist->get_all_project($DefEmp_ID)->result();			
			
			$getpurreq 						= $this->m_purchase_req->get_MR_by_number_fpa($PR_NUM)->row();
			$data['default']['PR_NUM'] 		= $getpurreq->PR_NUM;
			$data['default']['PR_CODE'] 	= $getpurreq->PR_CODE;
			$data['default']['PR_DATE'] 	= $getpurreq->PR_DATE;
			$data['default']['PR_RECEIPTD'] = $getpurreq->PR_RECEIPTD;
  	...
  	function get_AllDataIITMDET_fpa_230303() // GOOD
	{
		$PRJCODE	= $_GET['id'];
		$PR_NUM		= $_GET['PR_NUM'];
		$task		= $_GET['task'];
		...
		// START : FOR SERVER-SIDE
			...
			$s_PRSTAT 	= "SELECT PR_STAT FROM tbl_pr_header_fpa WHERE PRJCODE = '$PRJCODE' AND PR_NUM = '$PR_NUM'";
			$r_PRSTAT 	= $this->db->query($s_PRSTAT)->result();
			foreach($r_PRSTAT as $rw_PRSTAT):
				$PR_STAT 	= $rw_PRSTAT->PR_STAT;
			endforeach;
			...
			$start			= $_REQUEST['start'];
			$search			= $_REQUEST['search']["value"];
			$num_rows 		= $this->m_purchase_req->get_AllDataITMDETC_fpa($PRJCODE, $PR_NUM, $task, $search, $length, $start);
			$total			= $num_rows;
			$output			= array();
			$output['draw']	= $draw;
			$output['recordsTotal'] = $output['recordsFiltered']= $total;
			$output['data']	= array();
			$query 			= $this->m_purchase_req->get_AllDataITMDETL_fpa($PRJCODE, $PR_NUM, $task, $search, $length, $start, $order, $dir);
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				$secChgVol 	= base_url().'index.php/c_purchase/c_pr180d0c/chgVol_fpa/?id=';
				$chgVOL 	= "$secChgVol~$PR_NUM~$PR_ID~$ITM_CODE~$PR_PRICE~$PRJCODE";
  	...
  	function get_AllDataIITMDET_fpa() // GOOD
	{
		$PRJCODE	= $_GET['id'];
		$PR_NUM		= $_GET['PR_NUM'];
		$task		= $_GET['task'];
	...
	function update_process_fpa() // OK
	{
		$this->load->model('m_purchase/m_purchase_req/m_purchase_req', '', TRUE);
		$sqlApp 		= "SELECT * FROM tappname";
		$resultaApp 	= $this->db->query($sqlApp)->result();
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$projMatReqH 	= array('PR_NUM' 		=> $this->input->post('PR_NUM'),
								...
								'Patt_Date'			=> $Patt_Date,
								'Patt_Number'		=> $this->input->post('Patt_Number'));										
			$this->m_purchase_req->update_fpa($PR_NUM, $projMatReqH);
			...
			if($PR_STAT == 1 || $PR_STAT == 2)
			{
				...
				$this->m_purchase_req->deleteDetail_fpa($PR_NUM);
				
				$maxNo	= 0;
				$sqlMax = "SELECT MAX(PR_ID) AS maxNo FROM tbl_pr_detail_fpa";
				$resMax = $this->db->query($sqlMax)->result();
				foreach($resMax as $rowMax) :
					$maxNo = $rowMax->maxNo;		
				endforeach;
			...
			elseif($PR_STAT == 6)
			{
				foreach($_POST['data'] as $d)
				{
					$PR_NUM		= $d['PR_NUM'];
					$ITM_CODE	= $d['ITM_CODE'];
					$this->m_purchase_req->updateVolBud_fpa($PR_NUM, $PRJCODE, $ITM_CODE);
				}
			}
			elseif($PR_STAT == 9)
			{
 	...
 	function inbox_fpa() // OK
	{
		...
		$url			= site_url('c_purchase/c_pr180d0c/pR7_l5t_x1_fpa/?id='.$this->url_encryption_helper->encode_url($appName));
		redirect($url);
	}
	
	function pR7_l5t_x1_fpa() // G
	{
		$this->load->model('m_projectlist/m_projectlist', '', TRUE);
		$this->load->model('m_purchase/m_purchase_req/m_purchase_req', '', TRUE);
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data['viewPRJ'] 	= $this->m_projectlist->get_all_project($DefEmp_ID)->result();
			$data['viewPRJ'] 	= $this->m_projectlist->get_all_project($DefEmp_ID)->result();			
			$data["secURL"] 	= "c_purchase/c_pr180d0c/iN20_x1_fpa/?id=";
	...
	function iN20_x1_fpa() // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data['title'] 		= $appName;
			$data["MenuCode"] 	= 'MN469';
			$data['backURL'] 	= site_url('c_purchase/c_pr180d0c/inbox_fpa/');
			$data['PRJCODE'] 	= $PRJCODE;
			...
			$this->load->view('v_purchase/v_purchase_req/purchase_req_inb_fpa', $data);
		}
		else
		{
			redirect('__l1y');
  	...
  	function get_AllData_1n2_fpa() // GOOD
	{
		$PRJCODE		= $_GET['id'];
		...
		// START : FOR SERVER-SIDE
			...
			$start			= $_REQUEST['start'];
			$search			= $_REQUEST['search']["value"];
			$num_rows 		= $this->m_purchase_req->get_AllDataC_1n2_fpa($PRJCODE, $search, $length, $start);
			$total			= $num_rows;
			$output			= array();
			$output['draw']	= $draw;
			$output['recordsTotal'] = $output['recordsFiltered']= $total;
			$output['data']	= array();
			$query 			= $this->m_purchase_req->get_AllDataL_1n2_fpa($PRJCODE, $search, $length, $start, $order, $dir);
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				$CollCode	= "$PRJCODE~$PR_NUM";
				$secUpd		= site_url('c_purchase/c_pr180d0c/update_inb_fpa/?id='.$this->url_encryption_helper->encode_url($CollCode));
				$secPPOList	= site_url('c_purchase/c_pr180d0c/pRn_P0l/?id='.$this->url_encryption_helper->encode_url($PR_NUM));
                $secPrint	= site_url('c_purchase/c_pr180d0c/printdocument/?id='.$this->url_encryption_helper->encode_url($PR_NUM));
				$CollID		= "PR~$PR_NUM~$PRJCODE";
				$secDel_DOC = base_url().'index.php/__l1y/trash_DOC/?id='.$CollID;
	...
	function update_inb_fpa()
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data['title'] 		= $appName;
			$data['task'] 		= 'edit';
			$data['form_action']= site_url('c_purchase/c_pr180d0c/update_process_inb_fpa');
			...
			$this->load->view('v_purchase/v_purchase_req/purchase_req_inb_form_fpa', $data);
		}
		else
		{
			redirect('__l1y');
	...
	function update_process_inb_fpa() // OK
	{
		$this->load->model('m_purchase/m_purchase_req/m_purchase_req', '', TRUE);
		$sqlApp 		= "SELECT * FROM tappname";
		$resultaApp 	= $this->db->query($sqlApp)->result();
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			// START : ADD DOC HISTORY
				...
				$paramTrack 	= array('REF_NUM' 		=> $PR_NUM,
										...
										'FLDCODE'		=> "PR_CODE",
										'FLDNAME'		=> "PR_NUM",
										'HISTTYPE'		=> "Update approve ($PR_STAT)");
				$this->m_updash->uDocH($paramTrack);
			// END : ADD DOC HISTORY
			...
			$url			= site_url('c_purchase/c_pr180d0c/iN20_x1_fpa/?id='.$this->url_encryption_helper->encode_url($PRJCODE));
	...
	function printdocument_fpa()
	{
		...
		{		
			...
			$this->load->view('v_purchase/v_purchase_req/print_matreq_fpa', $data);
		}
		else
		{
			redirect('__l1y');
	...
	function showDocHist()
	{
		...
		{		
			...
			$getpurreq = $this->m_purchase_req->get_MR_by_number($PR_NUM, "tbl_pr_header")->row();
			
			$data['PR_NUM'] 	= $getpurreq->PR_NUM;
			$data['PR_STAT'] 	= $getpurreq->PR_STAT;
			$data['PR_CODE'] 	= $getpurreq->PR_CODE;
			$data['PR_DATE'] 	= $getpurreq->PR_DATE;
			$data['PRJCODE'] 	= $getpurreq->PRJCODE;
			$data['SPLCODE']	= $getpurreq->SPLCODE;
			$data['PR_DEPT']	= $getpurreq->PR_DEPT;
			$data['PR_NOTE'] 	= $getpurreq->PR_NOTE;
			$data['PR_NOTE2']	= $getpurreq->PR_NOTE2;
			$data['PR_PLAN_IR'] = $getpurreq->PR_RECEIPTD;
			$data['PR_MEMO'] 	= $getpurreq->PR_MEMO;
			$data['PR_CREATER'] = $getpurreq->PR_CREATER;
							
			$this->load->view('v_purchase/v_purchase_report/r_prdochist/r_prdochist_perdoc', $data);
		}
		else
		{
			redirect('__l1y');
	...
	function prc_syncAltAPP_221222HOLD()
	{
		$PRJCODE = $this->input->post('PRJCODE');
		$MenuApp = $this->input->post('MenuApp');

		// GET DOC PR
			$getDOC 	= "SELECT PR_NUM, PR_CODE, PR_CATEG, PR_STAT FROM tbl_pr_header WHERE PRJCODE = '$PRJCODE' 
							AND PR_STAT != 1";
			$resDOC 	= $this->db->query($getDOC);
			if($resDOC->num_rows() > 0)
			{
				foreach($resDOC->result() as $rDOC):
					$PR_NUM 	= $rDOC->PR_NUM;
					$PR_CODE 	= $rDOC->PR_CODE;
					$PR_CATEG 	= $rDOC->PR_CATEG;
					$PR_STAT 	= $rDOC->PR_STAT;
					...
					if($PR_STAT == 2)
					{
						// GET DOC STEP APP
							$APPROVER = "";
							$getSAPP 	= "SELECT APPROVER_1 FROM tbl_docstepapp_det
											WHERE MENU_CODE = '$MenuApp' AND PRJCODE = '$PRJCODE' AND APP_STEP = 1
											LIMIT 1";
							$resSAPP 	= $this->db->query($getSAPP);
							if($resSAPP->num_rows() > 0)
							{
								foreach($resSAPP->result() as $rSAPP):
									$EmpID 		= $rSAPP->APPROVER_1;
									$EmpName	= $this->db->select('CONCAT(First_Name," ",Last_Name) AS EmpName', FALSE)
													->get_where('tbl_employee',['Emp_ID' => $EmpID, 'Emp_Status' => 1])->row('EmpName');
								endforeach;
							}
						// END GET DOC STEP APP

						// UPDATE tbl_alert_schedzule
							$updAlt 	= "UPDATE tbl_alert_schedzule SET AS_APPEMP = '$EmpID',	
											AS_APPEMPNM = '$EmpName', AS_APPEMP_1 = '$EmpID'
											WHERE PRJCODE = '$PRJCODE' AND AS_MNCODE = '$MenuApp' 
											AND AS_CATEG = 'PR' AND AS_DOCNUM = '$PR_NUM'";
							$this->db->query($updAlt);
						// END UPDATE
					}

					if($PR_STAT == 3 || $PR_STAT == 4 || $PR_STAT == 5 || $PR_STAT == 6 || $PR_STAT == 9)
					{
						// DELETE Hist Alt sch
							$delAlt 	= "DELETE FROM tbl_alert_schedzule 
											WHERE PRJCODE = '$PRJCODE' AND AS_MNCODE = '$MenuApp'
											AND AS_CATEG = 'PR' AND AS_DOCNUM = '$PR_NUM'";
							$this->db->query($delAlt);
						// END DELETE Hist Alt sch
					}

					if($PR_STAT = 7)
					{
						// GET Hist APP
							$getHistAPP 	= "SELECT PRJCODE, AH_APPROVER, AH_ISLAST FROM tbl_approve_hist
												WHERE PRJCODE = '$PRJCODE' AND AH_CODE = '$PR_NUM'tes";
							$resHistAPP 	= $this->db->query($getHistAPP);
							if($resHistAPP->num_rows() > 0)
							{
								foreach($resHistAPP->result() as $rHAPP):
									$AH_PRJCODE 	= $rHAPP->PRJCODE;
									$AH_APPROVER 	= $rHAPP->AH_APPROVER;
									$AH_ISLAST 		= $rHAPP->AH_ISLAST;

									// CHK DOC STEP APP
										$chkSAPP 	= "SELECT APP_STEP, MAX_STEP FROM tbl_docstepapp_det 
														WHERE MENU_CODE = '$MenuApp' AND PRJCODE = '$PRJCODE'
														AND APPROVER_1 = '$AH_APPROVER'tes";
										$resCHKSAPP = $this->db->query($chkSAPP);
										if($resCHKSAPP->num_rows() > 0)
										{
											foreach($resCHKSAPP->result() as $rCSAPP):
												$APP_STEP 	= $rCSAPP->APP_STEP;
												$MAX_STEP 	= $rCSAPP->MAX_STEP;

												if($APP_STEP == $MAX_STEP)
												{
													// UPDATE AH_ISLAST = 1
														$updHistAPP = "UPDATE tbl_approve_hist SET AH_ISLAST = 1, AH_APPLEV = '$APP_STEP'
																		WHERE PRJCODE = '$AH_PRJCODE' AND AH_CODE = '$PR_NUM'
																		AND AH_APPROVER = '$AH_APPROVER'";
														$this->db->query($updHistAPP);
													// END UPDATE AH_ISLAST = 1

													// UPDATE DOC Approve
														$updDOCAPP = "UPDATE tbl_pr_header SET PR_STAT = 3,
																		STATDESC = 'Approved', STATCOL = 'success'
																		WHERE PRJCODE = '$PRJCODE' AND PR_NUM = '$PR_NUM'
																		AND PR_STAT = 7";
														$this->db->query($updDOCAPP);
													// END UPDATE DOC Approve
												}
												else
												{
													$updHistAPP = "UPDATE tbl_approve_hist SET AH_ISLAST = 0, AH_APPLEV = '$APP_STEP'
																	WHERE PRJCODE = '$AH_PRJCODE' AND AH_CODE = '$PR_NUM'
																	AND AH_APPROVER = '$AH_APPROVER'";
													$this->db->query($updHistAPP);
												}
										...
										{
											// DELETE Hist APP
												$delHistAPP 	= "DELETE FROM tbl_approve_hist 
																	WHERE PRJCODE = '$AH_PRJCODE' AND AH_CODE = '$PR_NUM'
																	AND AH_APPROVER = '$AH_APPROVER'";
												$this->db->query($delHistAPP);
											// END DELETE Hist APP
										}
									// END CHK DOC STEP APP
								endforeach;
							}
						// END GET Hist APP
					}
		...
		echo "Sync. Alert schedule whatsapp telah selesai";
	}
}