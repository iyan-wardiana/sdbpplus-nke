		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			foreach($_POST['data'] as $d)
			{
				...
				if($PRJHO == 'KTR')
				{
					...
					$s_MN 	= "SELECT B.JOBCOD1 FROM tbl_pr_detail A INNER JOIN tbl_joblist_detail B ON A.JOBCODEID = B.JOBCODEID AND A.PRJCODE = B.PRJCODE
								WHERE A.PRJCODE = '$PRJCODE' AND A.PR_NUM = '$PR_NUM'
			...
			if($PR_STAT == 2)
			{
				...
				$s_UPD 		= "UPDATE tbl_pr_header SET PR_CONFIRMER = '$UPD_USR', PR_CONFIRMED = '$UPD_TIME' WHERE PR_NUM = '$PR_NUM' AND PRJCODE = '$PRJCODE'";
				$this->db->query($s_UPD);
			}
