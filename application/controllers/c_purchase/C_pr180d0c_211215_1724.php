...
class C_pr180d0c extends CI_Controller  
{
	...
	function add_process() // G
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			// ============================= Start Upload File Attachment ========================================== //
			$Attach_NUM = $this->input->post('Attach_NUM');
			$this->load->library('upload');
		    $this->upload->initialize(["upload_path" => "assets/AdminLTE-2.0.5/doc_center/uploads/Attachment",
									   "allowed_types" => "*",
									   // "max_size" => 1024,
									   "overwrite" => false
		  	]);
		  	...
		  	if ($this->upload->do_multi_upload('Attachment'))
			{
			    ...
			    if($C_Attach->num_rows() > 0 )
			    {
			    	...
			    	$RowFile 	= $this->db->order_by('Attach_ID', 'DESC')->limit(1)->get()->row();
			    	$lastRow 	= intval($RowFile->Attach_ID);
			    }
			  	else
			  	{
			  		$Attach_NUM = "PR-".date('YmdHis');
			  		$lastRow = 0;
			  	}
				...
				for($i = 0; $i < $lnFiles; $i++) {
					$filedata = $data[$i];
					$dAttach 	= array("Attach_NUM" => $Attach_NUM,
										"Attach_ID"	=> $i+1+$lastRow,
										"RefNumber" => $PR_NUM,
										"PRJCODE"	=> $PRJCODE,
					...
					$this->db->insert('tbl_attachment',$dAttach);
				}
			}

			// ============================= End Upload File Attachment ========================================== //
			...
			$projMatReqH 	= array('PR_NUM' 		=> $PR_NUM,
									...
									'Attach_NUM' 	=> $Attach_NUM, //Field File Upload
									'PR_STAT'		=> $this->input->post('PR_STAT'),
									'JOBCODE'		=> $COLPR_REFNO,
									'JOBDESC'		=> $JOBDESC,
									'PR_REFNO'		=> $COLPRREFNO,
	...
	function update() // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data['default']['Attach_NUM']	= $getpurreq->Attach_NUM; //Field File Upload
	...
	function downloadFile()
	{
		...
		$myarr		= explode("~", $this->url_encryption_helper->decode_url($_GET['id']));
		$PRJCODE 	= $myarr[0];
		$PR_NUM 	= $myarr[1];

		$Attach = $this->db->get_where('tbl_attachment', ['RefNumber' => $PR_NUM, 'PRJCODE' => $PRJCODE]);
		foreach($Attach->result() as $rA):
			$File_Path 	= $rA->File_Path;
			$File_Name 	= $rA->File_Name;
			$loc_file 	= "assets/AdminLTE-2.0.5/doc_center/uploads/Attachment/$File_Name";
			$new_path 	= "Attachment/$File_Name";
			$this->zip->read_file($loc_file, $new_path);
		endforeach;

		if($Attach->num_rows() > 1)
		{
			date_default_timezone_set("Asia/Jakarta");
		  	$datetime	= date('Ymd_h:i:s');
		    $this->zip->download("$datetime.zip");
		}
		else
		{
			force_download($loc_file,NULL);
		}
	}
}