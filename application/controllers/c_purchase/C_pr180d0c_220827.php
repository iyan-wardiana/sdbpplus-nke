			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				if($PR_STAT == 1 || $PR_STAT == 4)
				{
					...
					$ITM_DESCVW 	= 	"<div class='input-group'>
		                                    ...
		                                    <input type='text' name='data[".$noU."][PR_DESC]' id='data".$noU."PR_DESC' size='20' value='".$PR_DESC."' class='form-control' style='min-width:110px; max-width:500px; text-align:left' readonly>
			                            </div>";
				}
				elseif($PR_STAT == 3)
				{
	...
	function add_process() // G
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			foreach($_POST['data'] as $d)
			{
				...
				// UPDATE tbl_joblist_report, tbl_joblist_detail
                    $ITM_CODE   = $d['ITM_CODE'];
                    $PR_VOLM	= $d['PR_VOLM'];
                    $PR_PRICE	= $d['PR_PRICE'];
                    ...
                    // GET JOBDESC
                        $JOBDESC = "";
                        $getJOBDESC = "SELECT JOBDESC FROM tbl_joblist_detail_$PRJCODE WHERE JOBCODEID = '$JOBID' AND ITM_CODE = '$ITM_CODE'";
                        $resJOBDESH = $this->db->query($getJOBDESC);
                        if($resJOBDESH->num_rows() > 0)
                        {
                            foreach($resJOBDESH->result() as $rJOBD):
                                $JOBDESC = $rJOBD->JOBDESC;
                            endforeach;
                        }

                    $s_00 		= "tbl_joblist_report WHERE PERIODE = '$PR_DATE' AND JOBCODEID = '$JOBID' 
                    ...
                    if($r_00 == 0)
                    {
                        $s_01	= "INSERT INTO tbl_joblist_report (PRJCODE, PERIODE, JOBCODEID, ITM_CODE, JOBDESC, PR_VOL_R, PR_VAL_R)
                                    VALUES ('$PRJCODE', '$PR_DATE', '$JOBID', '$ITM_CODE', '$JOBDESC', $PR_VOLM, $PR_VAL)";
                        $this->db->query($s_01);
                    }
                    else
                    {
                        $s_01	= "UPDATE tbl_joblist_report SET PR_VOL_R = PR_VOL_R+$PR_VOLM, PR_VAL_R = PR_VAL_R+$PR_VAL
                                    WHERE PRJCODE = '$PRJCODE' AND PERIODE = '$PR_DATE' AND JOBCODEID = '$JOBID' AND ITM_CODE = '$ITM_CODE'";
                        $this->db->query($s_01);
                    }

                    $s_UPDJLD 	= "UPDATE tbl_joblist_detail SET REQ_VOLM_R = REQ_VOLM_R+$PR_VOLM, REQ_AMOUNT_R = REQ_AMOUNT_R+$PR_VAL
                                    WHERE JOBCODEID = '$JOBID' AND ITM_CODE = '$ITM_CODE' AND PRJCODE = '$PRJCODE'";
                    $this->db->query($s_UPDJLD);
                
                // End UPDATE tbl_joblist_report, tbl_joblist_detail
			...
			if($PR_STAT == 3)
			{
				...
				if($AH_ISLAST == 1 && $yesAPP == 1)
				{
                    ...
                    foreach($_POST['data'] as $d)
                    {
                        ...
                        $s_01	= "UPDATE tbl_joblist_report SET PR_VOL_R = PR_VOL_R-$PR_VOLM, PR_VAL_R = PR_VAL_R-$PR_VAL,
                                    PR_VOL = PR_VOL+$PR_VOLM, PR_VAL = PR_VAL+$PR_VAL
                                    WHERE PRJCODE = '$PRJCODE' AND PERIODE = '$PR_DATE' 
                                    AND JOBCODEID = '$JOBID' AND ITM_CODE = '$ITM_CODE'";
                        $this->db->query($s_01);

                        $s_UPDJLD 	= "UPDATE tbl_joblist_detail SET REQ_VOLM_R = REQ_VOLM_R-$PR_VOLM, REQ_AMOUNT_R = REQ_AMOUNT_R-$PR_VAL,
                                        REQ_VOLM = REQ_VOLM+$PR_VOLM, REQ_AMOUNT = REQ_AMOUNT+$PR_VAL
                                        WHERE JOBCODEID = '$JOBID' AND ITM_CODE = '$ITM_CODE' AND PRJCODE = '$PRJCODE'";
                        $this->db->query($s_UPDJLD);
                    }
				}
