...
class C_pr180d0c extends CI_Controller  
{
		...
		// START : FOR SERVER-SIDE
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				if($PR_STAT == 1 || $PR_STAT == 4)
				{
					...
					$BTN_COPYVW		= "<a onClick='copyRow(".$noU.")' title='Copy' class='btn btn-warning btn-xs'><i class='glyphicon glyphicon-copy'></i></a>";
					$BTN_CNCVW		= "";
					$ITM_VOLVW 		= "<input type='text' class='form-control' style='min-width:80px; max-width:110px; text-align:right' name='ITM_VOLMBGx".$noU."' id='ITM_VOLMBGx".$noU."' value='".number_format($ITM_BUDQTY, 3)."' disabled >";
				...
				elseif($PR_STAT == 3)
				{
					...
					$BTN_COPYVW		= "";
					$BTN_CNCVW		= "<a onClick='cancelRow(".$noU.")' title='Batalkan Volume SPP' class='btn btn-danger btn-xs'><i class='fa fa-repeat'></i></a>";
					$ITM_VOLVW 		= "".number_format($ITM_BUDQTY, 3)." <input type='hidden' class='form-control' style='min-width:110px; max-width:300px; text-align:right' name='ITM_VOLMBGx".$noU."' id='ITM_VOLMBGx".$noU."' value='".number_format($ITM_BUDQTY, 3)."' disabled >";
	...
	function chgVol_fpa()
	{
        ...
        // GET VOL SPP
        	$TOTPRQTY0 		= 0;
        	$TOTPRAMOUNT0 	= 0;
			$s_00 			= "SELECT SUM(PR_VOLM) AS TOTPRQTY0, SUM(PR_TOTAL) AS TOTPRAMOUNT0 FROM tbl_pr_detail_fpa
								WHERE ITM_CODE = '$ITM_CODE' AND PR_ID = $PR_ID AND PR_NUM = '$PR_NUM' AND PRJCODE = '$PRJCODE'
								UNION
								SELECT SUM(PR_VOLM) AS TOTPRQTY0, SUM(PR_TOTAL) AS TOTPRAMOUNT0 FROM tbl_pr_detail_tmp
								WHERE ITM_CODE = '$ITM_CODE' AND PR_ID = $PR_ID AND PR_NUM = '$PR_NUM' AND PRJCODE = '$PRJCODE'";
