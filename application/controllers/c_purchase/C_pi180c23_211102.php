...
class C_pi180c23 extends CI_Controller  
{
	...
	function update_process() // OK
	{
		...
		{			
			...
			if($INV_STAT == 3)
			{
				// RESET DETAIL INVOICE. DI BLOCK KARENA PEMILIHAN STATUS 3 BERARTI DETAIL SUDAH FIX
					/*
						$this->m_purchase_inv->deleteINVDet($INV_NUM);
						$ITEM_TOT	= 0;
						$ITEM_PPN	= 0;
						...
						foreach($_POST['data'] as $d)
						{
							$d['INV_NUM']	= $INV_NUM;
							$d['INV_CODE']	= $INV_CODE;
							$ITM_UNITP		= $d['ITM_UNITP'];
							$ITM_AMOUNT_RET	= $d['ITM_AMOUNT_RET'];
							$ITM_AMOUNT_POT	= $d['ITM_AMOUNT_POT'];
							$ITM_AMOUNT_PPN	= $d['TAX_AMOUNT_PPn1'];
							$ITEM_TOT		= $ITEM_TOT + $ITM_UNITP;
							$ITEM_PPN		= $ITEM_PPN + $ITM_AMOUNT_PPN;
							$ITEM_RET		= $ITEM_RET + $ITM_AMOUNT_RET;
							$ITEM_POT		= $ITEM_POT + $ITM_AMOUNT_POT;
							$this->db->insert('tbl_pinv_detail',$d);
					}*/
				...
				// START : JOURNAL DETAIL : Hutang Lain(D) dan Hutang Supplier (K)
					...
					// MENGURANGI ATAS TTK
						foreach($_POST['data'] as $d)
						{
							$TTK_NUM	= $d['TTK_NUM'];
							$TTK_NUM	= $d['TTK_NUM'];
							$sqlTTK		= "SELECT TTK_DATE, TTK_DUEDATE, PRJCODE, ITM_AMOUNT
											FROM tbl_ttk_header
							...
							foreach($resTTK as $rowTTK):
								...
								$TTK_AMOUNTX	= $rowTTK->ITM_AMOUNTX;
				...
				// UPDATE DP HEADER
					if($DP_NUM != '')
					{
						...
						$this->m_purchase_inv->updateDP($DP_NUM, $updDP);
					}
				...
				*/
				...
				// END : JOURNAL DETAIL DEBIT - KHUSUS PPn Masukan
				
				// START : JOURNAL DETAIL KREDIT - KHUSUS Hutang Retensi
					if($INV_AMOUNT_RET > 0)
					{
						...
						// START : JOURNAL DETAIL
							...
							$JSource		= 'PINV_RET';
							$PRJCODE		= $PRJCODE;
							$Notes			= "Hutang Retensi $INV_CODE";
							
							$ITM_CODE 		= $INV_NUM;
							$ACC_ID 		= '';
							$ITM_UNIT 		= '';
							$ITM_QTY 		= 1;
							$ITM_PRICE 		= $INV_AMOUNT_RET;
							$ITM_DISC 		= 0;
							$TAXCODE1 		= "";
							$TAXPRICE1		= 0;
							
							$TRANS_CATEG 	= "PINV_RET~$SPLCAT";				// PERHATIKAN DI SINI. PENTING
				...
				// END : JOURNAL DETAIL DEBIT - KHUSUS Hutang Retensi
				...
				// START : JOURNAL DETAIL - KHUSUS BEBAN LAINNYA
					if($INV_AMOUNT_OTH > 0)
					{
						$JournalH_Code	= $INV_NUM;
						$JournalType	= 'PINV4';
						...
						$JSource		= 'PINV4';
						$PRJCODE		= $PRJCODE;
						$Notes			= "$INV_NOTES- $INV_CODE (Other exp. _p.inv)";
						
						$ITM_CODE 		= $INV_NUM;
						$ACC_ID 		= $INV_ACC_OTH;
						$ITM_UNIT 		= '';
						$ITM_QTY 		= 1;
						$ITM_PRICE 		= $INV_AMOUNT_OTH;
						$ITM_DISC 		= 0;
						$TAXCODE1 		= '';
						$TAXPRICE1		= 0;
						
						$TRANS_CATEG 	= "PINV4";									// PERHATIKAN DI SINI. PENTING
				...
				// END : JOURNAL DETAIL - KHUSUS BEBAN LAINNYA
				
				// START : JOURNAL DETAIL - KHUSUS POTONGAN
					echo "INV_AMOUNT_POT = $INV_AMOUNT_POT";
					...
					if($INV_AMOUNT_POT > 0)
					{
						...
						$JSource		= 'PINV5';
						$PRJCODE		= $PRJCODE;
						$Notes			= "$INV_NOTES- $INV_CODE (Other disc. _p.invoice";
						
						$ITM_CODE 		= $INV_NUM;
						$ACC_ID 		= $INV_ACC_OTH;
						$ITM_UNIT 		= '';
						$ITM_QTY 		= 1;
						$ITM_PRICE 		= $INV_AMOUNT_POT;
						$ITM_DISC 		= 0;
						$TAXCODE1 		= '';
						$TAXPRICE1		= 0;
						
						$TRANS_CATEG 	= "PINV5";									// PERHATIKAN DI SINI. PENTING
				...
				// END : JOURNAL DETAIL - KHUSUS POTONGAN
					...
					if($INV_AMOUNT_RET > 0)
					{
						...
						// START : CREATE INVOICE
							$insertINV 	= array('INV_NUM' 			=> $INV_NUMRET,
												...
												'INV_AMOUNT_BASE'	=> $INV_AMOUNT_RET,
												'INV_AMOUNT_RET'	=> 0,
												'INV_LISTTAXVAL'	=> 0,
												'INV_PPH'			=> '',
												'INV_PPHVAL'		=> 0,
							...
							$sqlL_K		= "SELECT LA_ACCID FROM tbl_link_account
											WHERE LA_ITM_CODE = '$SPLCAT' AND LA_CATEG = 'RET' AND LA_DK = 'K'";
							...
							foreach($_POST['data1'] as $dc1)
							{
								...
								$this->db->insert('tbl_pinv_detail',$dc1);
							}
						// END : CREATE INVOICE
					}
