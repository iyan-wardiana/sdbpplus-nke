...
class C_p180c21o extends CI_Controller  
{
	...
	function get_AllDataIITMDET_221216()
	{
		$PRJCODE	= $_GET['id'];
		$PO_NUM		= $_GET['PO_NUM'];
		$PR_NUMX 	= $_GET['PR_NUM'];
	...
	function get_AllDataIITMDET_230119()
	{
		$PRJCODE	= $_GET['id'];
		$PO_NUM		= $_GET['PO_NUM'];
		$PR_NUMX 	= $_GET['PR_NUM'];
		...
		// START : FOR SERVER-SIDE
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				// 1. FROM CURRENT DOCUMENT AND OTHERS ROW
					...
					$s_TOTPOVOL0C		= "SELECT IFNULL(SUM(A.PO_VOLM), 0) AS ORDERED_VOL, IFNULL(SUM(A.PO_COST), 0) AS ORDERED_VAL,
											...
											WHERE B.PRJCODE = '$PRJCODE'
												...
												AND A.ITM_CODE = '$ITM_CODE'
												AND A.PRD_ID = $PRD_ID
												AND A.PO_STAT NOT IN (5,9)";
					$r_TOTPOVOL0C		= $this->db->query($s_TOTPOVOL0C)->result();
					foreach($r_TOTPOVOL0C as $rw_TOTPOVOL0C) :
						$ORDERED_VOLC	= $rw_TOTPOVOL0C->ORDERED_VOL;
						$ORDERED_VALC	= $rw_TOTPOVOL0C->ORDERED_VAL;
				...
				// START : SISA VOL AND VAL SETELAH DIKURANGI DENGAN DOKUMEN AKTIF (TEMPORARY)
					// $ITM_REMVOL	= $ITM_REMVOLA - $ORDERED_VOL - $ORDERED_TMP_VOL0 + $ORDERED_VOLC;
					// $ITM_REMVAL	= $ITM_REMVALA - $ORDERED_VAL - $ORDERED_TMP_VAL0 + $ORDERED_VALC;
					$ITM_REMVOL		= $ITM_REMVOLA - $ORDERED_TMP_VOL0 + $ORDERED_VOLC;
					$ITM_REMVAL		= $ITM_REMVALA - $ORDERED_TMP_VAL0 + $ORDERED_VALC;
				// END : SISA VOL AND VAL SETELAH DIKURANGI DENGAN DOKUMEN AKTIF (TEMPORARY)

				// START : SISA BUDGET VOLUME SPP PER PR ID
					$REM_SPP_VOL 	= $PR_VOLM - $ORDERED_VOL - $ORDERED_TMP_VOL0 + $ORDERED_VOLC;
				...
				$REM_VOLPO 		= $PO_VOLM - $ORDERED_CVOLC - $IR_VOLM;
				// START : CANCEL VOL. PER ITEM
					$secShwD 	= base_url().'index.php/c_purchase/c_pr180d0c/cancelItem/?id=';
					$canShwRow 	= "$secShwD~$PO_NUM~$PRJCODE~$ITM_CODE~$ITM_NAME~$JOBPARDESC~$PO_VOLM~$IR_VOLM~$REM_VOLPO~$ITM_UNIT";
					$secDelD 	= base_url().'index.php/__l1y/cancelItem/?id=';
	...
	function get_AllDataIITMREQDET_221216() // GOOD
	{
		$PRJCODE	= $_GET['id'];
		$PO_NUM		= $_GET['PO_NUM'];
		$PO_CODE	= $_GET['PO_CODE'];
		...
		// START : FOR SERVER-SIDE
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				// START : SISA VOL AND VAL SETELAH DIKURANGI DENGAN DOKUMEN AKTIF (TEMPORARY)
					...
					$ITM_REMVALV	= "(($ITM_BUDG + $AMD_VAL - $AMDM_VAL) - $TREQ_VAL - $TREQ_VAL_R) - $ORDERED_TMP_VAL0";
				// END : SISA VOL AND VAL SETELAH DIKURANGI DENGAN DOKUMEN AKTIF (TEMPORARY)
				...
				//{
					$output['data'][] 	= array("$chkBox",
												...
												$ORDERED_VOL,
												$REM_SPP_VOL,
												"<input type='text' class='form-control' style='min-width:80px; max-width:110px; text-align:right' name='PO_REQVOLMX".$noU."' id='PO_REQVOLMX".$noU."' value='".number_format($REM_SPP_VOL, 2)."' onBlur='chgReqVol(this.value, ".$noU.")' disabled>
