...
class C_pr180d0c extends CI_Controller  
{
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				// START : GET BUDGET
					...
					foreach($r_BUDGVAL as $rowJOBDR) :
						...
						$TREQ_VOL 		= ($PR_VOL - $PR_CVOL) + ($PO_VOL - $PO_CVOL) + $VCASH_VOL + $VLK_VOL + $PPD_VOL;
						$TREQ_VAL 		= ($PO_VAL - $PO_CVAL) + ($WO_VAL - $WO_CVAL) + $VCASH_VAL + $VLK_VAL + $PPD_VAL;
				...
				$TPRVOL_REQR 	= 0;
	        	$TPRVAL_REQR 	= 0;
				$s_02 			= "SELECT IFNULL(SUM(PR_VOLM),0) AS TPRVOL_REQR
									FROM tbl_pr_detail
									WHERE JOBCODEID = '$JOBCODEID' AND PR_NUM = '$PR_NUM' AND PR_ID = $PR_ID AND PRJCODE = '$PRJCODE'";
				$r_02 			= $this->db->query($s_02)->result();
				foreach($r_02 as $rw_02):
					$TPRVOL_REQR = $rw_02->TPRVOL_REQR;
				endforeach;
				...
				$TOTPRQTY 		= $TPRVOL_CUR + $TREQ_VOL + ($TREQ_VOL_R - $TPRVOL_REQR);
	...
	function chgVol()
	{
		...
		$TOT_REQVOL 	= $REQ_NOW + $TPRVOL_CUR + ($TPRVOL_OTH1 - $TPRVOL_REQR) + $TPRVOL_OTH2; 			// TOTAL ALL ROW REQUEST IN DOC. ACTIVE
