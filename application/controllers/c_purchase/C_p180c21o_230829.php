...
class C_p180c21o extends CI_Controller  
{
  	...
  	function get_AllData() // GOOD 
	{
		...
		// START : FOR SERVER-SIDE
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				$s_histP 	= "SELECT PDOC_EmpNM, PDOC_DATE FROM tbl_print_hist WHERE PDOC_NUM = '$PO_NUM' AND isPrint = 0 ORDER BY ID DESC LIMIT 1";
				$r_histP 	= $this->db->query($s_histP);
				$userPrint 	= "";
				$printDate 	= "";
				if($r_histP->num_rows() > 0)
				{
					foreach($r_histP->result() as $rw_histP):
						$userPrint = $rw_histP->PDOC_EmpNM;
						$printDate = $rw_histP->PDOC_DATE;
					endforeach;
				}
				...
				elseif($PO_STAT == 2 || $PO_STAT == 3)
				{
					$secAction	= 	"<input type='hidden' name='urlIRList".$noU."' id='urlIRList".$noU."' value='".$secPIRList."'>
									...
									<a href='javascript:void(null);' class='btn btn-primary btn-xs' title='User Print : ".$userPrint."\nDate Print : ".$printDate."' onClick='printDocument(".$noU.")'>
										<i class='glyphicon glyphicon-print'></i>
									</a>
  	...
  	function get_AllDataPOGRP() // GOOD 
	{
		...
		// START : FOR SERVER-SIDE
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				$PO_CREATER	= $dataI['PO_CREATER'];
				$PO_CAT		= $dataI['PO_CAT'];

				$CREATERNM 	= "-";
				$s_Empl		= "SELECT CONCAT(First_Name,' ', Last_Name) AS compName FROM tbl_employee WHERE Emp_ID = '$PO_CREATER' LIMIT 1";
				$r_Empl 	= $this->db->query($s_Empl);
				if($r_Empl->num_rows() > 0)
				{
					foreach($r_Empl->result() as $rw_Empl):
						$CREATERNM 	= $rw_Empl->compName;
					endforeach;
				}
				$CREATERNM	= cut_text2 ("$CREATERNM", 15);
				...
				elseif($PO_STAT == 2 || $PO_STAT == 3)
				{
					$secAction	= 	"<input type='hidden' name='urlIRList".$noU."' id='urlIRList".$noU."' value='".$secPIRList."'>
									...
									<a href='javascript:void(null);' class='btn btn-".$PRINTCOL." btn-xs' title='User Print : ".$userPrint."\nDate Print : ".$printDate."' onClick='printDocument(".$noU.")'>
										<i class='glyphicon glyphicon-print'></i>
									</a>
									<a href='javascript:void(null);' class='btn bg-purple btn-xs' onClick='".$disCl."(".$noU.")' title='Batal' ".$disV.">
										<i class='glyphicon glyphicon-off'></i>
											...
											<div style='margin-left: 20px; font-style: italic'>
									  		...
									  		<div>
										  		<div class='text-muted'>
										  			<i class='fa fa-gear margin-r-5'></i> ".$PODIVD."
										  		</div>
										  		<div class='text-muted'>
										  			<i class='fa fa-user margin-r-5'></i> ".$CREATERNM."
										  		</div>
										  	</div>",
										  	"<div style='white-space:nowrap'>
												<span class='label label-".$STATCOL."' style='font-size:12px'>".$STATDESC."</span>
	...
	function add_process() // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$RECOM_DATE		= date('Y-m-d',strtotime(str_replace('/', '-', $this->input->post('RECOM_DATE'))));
			...
			$PO_NOTES 		= htmlspecialchars($this->input->post('PO_NOTES'), ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401 | ENT_HTML5);
				...
				for($i = 0; $i < $count; $i++) {
					if(!empty($_FILES['userfile']['name'][$i])) {
						...
						if($this->upload->do_upload('userfile'))
						{
							...
							$source		= "assets/AdminLTE-2.0.5/doc_center/uploads/PO_Document/$PRJCODE/$fileName";
							...
							if($srvURL == '10.0.0.144')
							{
								...
								$destination = "/assets/AdminLTE-2.0.5/doc_center/uploads/PO_Document/$PRJCODE/$fileName";
	
								if($this->ftp->list_files("./assets/AdminLTE-2.0.5/doc_center/uploads/PO_Document/$PRJCODE/") == FALSE) 
								{
									$this->ftp->mkdir("./assets/AdminLTE-2.0.5/doc_center/uploads/PO_Document/$PRJCODE/", 0777);
								}
			...
			$task	= 'add'; 
			$this->m_purchase_po->updatePODet($PO_NUM, $PRJCODE, $PR_NUM, $ISDIRECT, $task); // UPDATE JOB DETAIL DAN PR

			// START : PROCEDURE UPDATE JOBLISTDETAIL
				$UPD_CATEG	= "PO";
				$DOC_NUM	= $PO_NUM;
				// $this->m_updash->updJD($PRJCODE, $UPD_CATEG, $DOC_NUM);
			...
			// START : ADD DOC HISTORY
				$this->load->model('m_updash/m_updash', '', TRUE);
				date_default_timezone_set("Asia/Jakarta");
				$paramTrack 	= array('REF_NUM' 		=> $PO_NUM,
										'TBLNAME' 		=> "tbl_po_header",
										'FLDCODE'		=> "PO_CODE",
										'FLDNAME'		=> "PO_NUM",
										'HISTTYPE'		=> "Penambahan ($PO_STAT)");
			...
			if($PO_STAT == 2)
			{
				...
				$s_UPD 		= "UPDATE tbl_po_header SET PO_CONFIRMER = '$UPD_USR', PO_CONFIRMED = '$UPD_TIME' WHERE PO_NUM = '$PO_NUM' AND PRJCODE = '$PRJCODE'";
				$this->db->query($s_UPD);
			}
	...
	function u77p180c21o_p0() // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data['default']['RECOM_DATE'] 	= $getPO->RECOM_DATE;
	...
	function update_process() // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			elseif($PO_STAT == 6)
			{
				...
				$this->m_purchase_po->updateVolBudNew($PO_NUM, $PRJCODE);
			}
				...
				foreach($_POST['data'] as $d)
				{
					...
					// UPDATE LASTP
						/*if($PO_STAT == 2)
						{
							...
							$ITMC  	= $d['ITM_CODE'];
							$LASTP 	= $d['PO_PRICE'];
							$updLP 	= "UPDATE tbl_joblist_detail_$PRJCODEVW SET ITM_LASTP = $LASTP WHERE JOBCODEID = '$JOBID' AND ITM_CODE = '$ITMC' AND PRJCODE = '$PRJCODE'";
							$this->db->query($updLP);
						}*/
				}
	...
	function up180djinb() // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data['default']['SPLCODE'] 	= $getPO->SPLCODE;
			$data['default']['PR_NUM'] 		= $getPO->PR_NUM;
			$data['default']['PR_CODE'] 	= $getPO->PR_CODE;
			$data['default']['PO_DPPER'] 	= $getPO->PO_DPPER;
			$data['default']['PO_DPVAL'] 	= $getPO->PO_DPVAL;
			$data['default']['PO_CURR'] 	= $getPO->PO_CURR;
			$data['default']['PO_CURRATE'] 	= $getPO->PO_CURRATE;
	...
	function update_process_inb() // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$PO_NOTES1 		= htmlspecialchars($this->input->post('PO_NOTES1'),ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401 | ENT_HTML5);
			$PO_MEMO 		= htmlspecialchars($this->input->post('PO_MEMO'),ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401 | ENT_HTML5);
			...
			// START : SAVE APPROVE HISTORY
				...
				$AH_NOTES		= htmlspecialchars($this->input->post('PO_NOTES1'), ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401 | ENT_HTML5);
			...
			// START : ADD DOC HISTORY
				...
				$paramTrack 	= array('REF_NUM' 		=> $PO_NUM,
										...
										'FLDCODE'		=> "PO_CODE",
										'FLDNAME'		=> "PO_NUM",
										'HISTTYPE'		=> "Update approve ($PO_STAT)");
				$this->m_updash->uDocH($paramTrack);
			// END : ADD DOC HISTORY
	...
	function watermark_upd()
	{
		...
		$compName 	= "-";
		$s_Empl		= "SELECT CONCAT(First_Name,' ', Last_Name) AS compName
						FROM tbl_employee
						WHERE Emp_ID = '$DefEmp_ID' AND Emp_Status = 1";
		$r_Empl 	= $this->db->query($s_Empl);
		if($r_Empl->num_rows() > 0)
		{
			foreach($r_Empl->result() as $rw_Empl):
		...
		// Insert print History
			date_default_timezone_set("Asia/Jakarta");
			$PDOC_DATE 		= date('Y-m-d H:i:s');
			$inPrintHist 	= ["PDOC_NUM" => $PO_NUM, 
							   "PDOC_CODE" => $PO_CODE, 
							   "PDOC_DATE" => $PDOC_DATE,
							   "isPrint" => $isPrint,
							   "PDOC_EmpID" => $DefEmp_ID,
							   "PDOC_EmpNM" => $compName];
			$this->db->insert('tbl_print_hist', $inPrintHist);
		// END print History
	}
	...
	function get_AllDataIITMDET()
	{
		...
		// START : FOR SERVER-SIDE
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				$ITM_UNIT2 		= strtoupper($dataI['ITM_UNIT2']);
				$ITM_PRICE 		= $dataI['ITM_PRICE'];
				...
				// START : TOTAL VOL/VAL YANG SUDAH DIBUATKAN PO ATAS SPP YANG TERPILIH PER PR_ID TERMASUK PO AKTIF
					$TOT_PO_VOL		= 0;
					$TOT_PO_VAL		= 0;
					$TEMP_PO_VOL	= 0;
					$TEMP_PO_VAL	= 0;
					...
					foreach($r_TOTPOVOL0 as $rw_TOTPOVOL0) :
						$TOTPO_VOL		= $rw_TOTPOVOL0->ORDERED_VOL;
						$TOTPO_VAL		= $rw_TOTPOVOL0->ORDERED_VAL;
						$TEMP_PO_VOL	= $rw_TOTPOVOL0->ORDERED_TMP_VOL;
						$TEMP_PO_VAL	= $rw_TOTPOVOL0->ORDERED_TMP_VAL;
					endforeach;*/

					$s_TOTPOVOL0C		= "SELECT IFNULL(SUM(A.PO_VOLM), 0) AS ORDERED_VOL, IFNULL(SUM(A.PO_COST), 0) AS ORDERED_VAL,
					...
					foreach($r_TOTPOVOL0C as $rw_TOTPOVOL0C) :
						$TOTPO_VOL		= $rw_TOTPOVOL0C->ORDERED_VOL;
						$TOTPO_VAL		= $rw_TOTPOVOL0C->ORDERED_VAL;
						$ORDERED_CVOL	= $rw_TOTPOVOL0C->ORDERED_CVOL;
						$ORDERED_CVAL	= $rw_TOTPOVOL0C->ORDERED_CVAL;
					endforeach;

					$TOT_PO_VOL 	= $TOTPO_VOL;
					$TOT_PO_VAL 	= $TOTPO_VAL;
				// END : TOTAL VOL/VAL YANG SUDAH DIBUATKAN PO ATAS SPP YANG TERPILIH PER PR_ID TERMASUK PO AKTIF

				// START : FROM CURRENT DOCUMENT AND CURRENT ROW
					$TOT_PO_VOLC		= 0;
					$TOT_PO_VALC		= 0;
					$TOT_PO_CVOLC		= 0;
					$TOT_PO_CVALC		= 0;
					$s_TOTPOVOL0C		= "SELECT IFNULL(SUM(A.PO_VOLM), 0) AS TOT_PO_VOL, IFNULL(SUM(A.PO_COST), 0) AS TOT_PO_VAL,
												IFNULL(SUM(A.PO_CVOL), 0) AS TOT_PO_CVOL, IFNULL(SUM(A.PO_CTOTAL), 0) AS TOT_PO_CVAL
												FROM tbl_po_detail A
												INNER JOIN tbl_po_header B ON A.PO_NUM = B.PO_NUM
											WHERE B.PRJCODE = '$PRJCODE'
												AND A.JOBCODEID = '$JOBCODEID'
					...
					foreach($r_TOTPOVOL0C as $rw_TOTPOVOL0C) :
						...
						$TOT_PO_CVOLC 	= $rw_TOTPOVOL0C->TOT_PO_CVOL;
						$TOT_PO_CVALC 	= $rw_TOTPOVOL0C->TOT_PO_CVAL;
					endforeach;
				// END : FROM CURRENT DOCUMENT AND CURRENT ROW

				// START : FROM CURRENT DOCUMENT AND OTHERS ROW
					$TOT_PO_VOLO		= 0;
					$TOT_PO_VALO		= 0;
					$TOT_PO_CVOLO		= 0;
					$TOT_PO_CVALO		= 0;
					...
					$r_TOTPOVOL0C		= $this->db->query($s_TOTPOVOL0C)->result();
					foreach($r_TOTPOVOL0C as $rw_TOTPOVOL0C) :
						$TOT_PO_VOLO	= $rw_TOTPOVOL0C->TOT_PO_VOL;
						$TOT_PO_VALO	= $rw_TOTPOVOL0C->TOT_PO_VAL;
						$TOT_PO_CVOLO 	= $rw_TOTPOVOL0C->TOT_PO_CVOL;
						$TOT_PO_CVALO 	= $rw_TOTPOVOL0C->TOT_PO_CVAL;
					endforeach;
				// END : FROM CURRENT DOCUMENT AND OTHERS ROW

				//$ITM_REMVAL	= $ITM_REMBUDVAL - $TEMP_PO_VAL + $TOT_PO_VALC;
				$ITM_REMVAL		= $ITM_REMBUDVAL + $TOT_PO_VALC;

				// START : SISA VOLUME SPP PER PR ID
					/*$REM_SPP_VOL 	= $PR_VOLM - $TOT_PO_VOL + $TOT_PO_CVOLC;
					$REM_SPP_VAL 	= $PR_VOLM - $TOT_PO_VAL + $TOT_PO_CVALC;*/

					$REM_SPP_VOL 	= $PR_VOLM - $TOT_PO_VOL + $TOT_PO_CVOLC + $TOT_PO_VOLC;
					//$REM_SPP_VOLVW = "$PR_VOLM - $TOT_PO_VOL + $TOT_PO_CVOLC + $TOT_PO_VOLC";
					$REM_SPP_VAL 	= $PR_VOLM - $TOT_PO_VAL + $TOT_PO_CVALC + $TOT_PO_VALC;
				// END : SISA VOLUME SPP PER PR ID
				...
				if($PO_STAT == 1 || $PO_STAT == 4)
				{
					...
					$vWPO_ORDER 	= "<input type='text' class='form-control' style='min-width:90px; max-width:90px; text-align:right' name='PO_VOLMX".$noU."' id='PO_VOLMX".$noU."' value='".number_format($PO_VOLM, 2)."' onKeyPress='return isIntOnlyNew(event);' onBlur='getValuePO_1(".$noU.");' ".$isDisabled.">";
					$vWITM_PRICE 	= "<input type='text' class='form-control' style='text-align:right; min-width:90px' name='PO_PRICEX".$noU."' id='PO_PRICEX".$noU."' size='10' value='".number_format($ITM_PRICE, 2)."' onKeyPress='return isIntOnlyNew(event);' onBlur='getValuePO_1(".$noU.");'>";
					$vWPO_OA 		= "<input type='text' class='form-control' style='text-align:right; min-width:90px' name='PO_OAX".$noU."' id='PO_OAX".$noU."' size='10' value='".number_format($PO_OA, 2)."' onKeyPress='return isIntOnlyNew(event);' onBlur='getValuePO_1(".$noU.");'>";
					...
					if($ITM_CATEG != 'UA') $ITM_TAX1v = "<select name='data[".$noU."][TAXCODE1]' class='form-control select2' id='data".$noU."TAXCODE1'  onChange='getValuePO_1(".$noU.");' style='width:150px;'><option value='0'> --- </option>".$opt_TAXv1."</select>";
					else $ITM_TAX1v = "-<input type='hidden' name='data[".$noU."][TAXCODE1]' id='data".$noU."TAXCODE1'  value='0'>";

					$vWTAX_PRICE 	= "<input type='text' class='form-control' style='text-align:right; min-width:90px' name='TAXPRICE1".$noU."' id='TAXPRICE1".$noU."' size='10' value='".number_format($TAXPRICE1, 2)."' onKeyPress='return isIntOnlyNew(event);' onBlur='getValuePO_TAX(".$noU.");'>";
				...
				elseif($PO_STAT == 3)
				{
					...
					if($TOT_PO_CVOLC != 0) $PO_CVOLv = "<div style='color:red;'><i class='fa fa-sort-down (alias)'></i>".number_format($TOT_PO_CVOLC, 2)."</div>";
					else $PO_CVOLv = "";
					...
					$vWTAX_PRICE 	= number_format($TAXPRICE1, 2);
					if($ITM_CATEG != 'UA') 
					{
				...
				$REM_VOLPO 		= $PO_VOLM - $TOT_PO_CVOLC - $IR_VOLM;
				...
				$vwIRDET	= "$PRJCODE~$PO_NUM~$PO_CODE~$JOBCODEID~$JOBDESC~$JOBPARENT~$JOBPARDESC~$ITM_CODE~$PO_ID~$PO_VOLM~$TOT_PO_CVOLC";
				$secvwIRD	= site_url('c_purchase/c_p180c21o/shwIR_H15tDET/?id='.$this->url_encryption_helper->encode_url($vwIRDET));

				$VW_IRDET	= "<a onClick='showIRDET(".$noU.")' title='Vol. LPM' style='cursor: pointer; text-decoration:none;'><i class='fa fa-truck'></i>&nbsp;".number_format($IR_VOLM, 2)."</a>";
				$output['data'][] 	= ["$BTN_CNCVW
										...
										<input type='hidden' name='urlIRDet".$noU."' id='urlIRDet".$noU."' value='".$secvwIRD."'>
										...
										$ITM_CODE,
										"$ITM_CODE ($JOBCODEID) : $ITM_NAME <span class='text-red' style='font-style: italic; font-weight: bold;'>(Hrg.RAP : ".number_format($RAP_PRICE,2)." )</span>
										<div style='font-style: italic;'>
										...
										"$ITM_REMVOLv
										<div style='font-style: italic;'>
											<i class='text-muted fa fa-chevron-circle-down'></i>&nbsp;&nbsp; ".number_format($TOT_PO_VOLO,2)."
										</div>
										<input type='hidden' class='form-control' style='min-width:130px; max-width:200px; text-align:right' name='ITM_REMAIN".$noU."' id='ITM_REMAIN".$noU."' value='".$REM_SPP_VOL."' >
										...
										<input type='hidden' class='form-control' style='min-width:130px; max-width:200px; text-align:right' name='TOTVAL_OTHDOC".$noU."' id='TOTVAL_OTHDOC".$noU."' value='".$TOT_PO_VAL."' >
										...
										<input type='hidden' class='form-control' style='min-width:130px; max-width:200px; text-align:right' name='JOB_MAXREQ_AMN".$noU."' id='JOB_MAXREQ_AMN".$noU."' value='".$ITM_REMVAL."' >",
										"$vWPO_ORDER
										<div>
											<p class='text-red'>".$VW_IRDET."
									  		</p>
									  	</div>
										...
										"<input type='hidden' class='form-control' style='text-align:center; max-width:90px' id='data".$noU."ITM_UNIT' name='data[".$noU."][ITM_UNIT]' value='".$ITM_UNIT."'>
										<input type='text' class='form-control' style='text-align:center; max-width:90px' id='data".$noU."ITM_UNIT2' name='data[".$noU."][ITM_UNIT2]' value='".$ITM_UNIT2."'>",
										"$vWITM_PRICE
										<input style='text-align:right' type='hidden' name='data[".$noU."][PO_PRICE]' id='data".$noU."PO_PRICE' size='6' value='".$ITM_PRICE."'>",
										// "$vWPO_OA
										// <input style='text-align:right' type='hidden' name='data[".$noU."][PO_OA]' id='data".$noU."PO_OA' size='6' value='".$PO_OA."'>",
										"$PO_DISPv
	...
	function chgPOVOL()
	{
        ...
        $PO_PRC 	= $colExpl[6];
        // $PO_OA 		= $colExpl[7];
        // $PO_VAL 	= ($PO_VOL * $PO_PRC) + $PO_OA;
        $PO_VAL 	= ($PO_VOL * $PO_PRC);

        $s_TMP		= "UPDATE tbl_po_detail SET PO_VOLM = $PO_VOL, PO_PRICE = $PO_PRC, PO_COST = $PO_VAL
						WHERE PO_NUM 		= '$PO_NUM'
							AND PR_NUM 		= '$PR_NUM'
		...
		$r_TOTPOVOL0C		= $this->db->query($s_TOTPOVOL0C)->result();
		foreach($r_TOTPOVOL0C as $rw_TOTPOVOL0C) :
			$TOT_PO_VOL		= $rw_TOTPOVOL0C->TOT_PO_VOL;
			$TOT_PO_VAL		= $rw_TOTPOVOL0C->TOT_PO_VAL;
			$TOT_PO_CVOL	= $rw_TOTPOVOL0C->TOT_PO_CVOL;
			$TOT_PO_CVAL	= $rw_TOTPOVOL0C->TOT_PO_CVAL;
			$PO_VOL			= $TOT_PO_VOL - $TOT_PO_CVOL;
			$PO_VAL 		= $TOT_PO_VAL - $TOT_PO_CVAL;

	        $s_TMP			= "UPDATE tbl_pr_detail SET PO_VOLM = $PO_VOL, PO_AMOUNT = $PO_VAL
								WHERE PR_NUM 		= '$PR_NUM'
									AND JOBCODEID 	= '$JOBCODEID'
									AND PR_ID 		= $PRD_ID";
			$this->db->query($s_TMP);
		...
		foreach($r_TOTPOVOL0C as $rw_TOTPOVOL0C) :
	        ...
	        $s_TMP			= "UPDATE tbl_joblist_detail SET PO_VOL_R = $PO_VOL, PO_VAL_R = $PO_VAL WHERE PRJCODE = '$PRJCODE' AND JOBCODEID = '$JOBCODEID'";
			$this->db->query($s_TMP);
		endforeach;
		...
		// START : ADD DOC HISTORY
			...
			$paramTrack 	= array('REF_NUM' 		=> $PO_NUM,
									...
									'FLDCODE'		=> "PO_CODE",
									'FLDNAME'		=> "PO_NUM",
									'HISTTYPE'		=> "Perubahan vol $PO_VOL prc $PO_PRC");
			$this->m_updash->uDocH($paramTrack);
		// END : ADD DOC HISTORY
	...
	function updPOVolTemp()
	{
		...
		$PO_TMP_VAL = $PO_VOLM * $PO_PRICE;
		
		$s_TMP		= "UPDATE tbl_pr_detail SET PO_TMP_VOL = $PO_VOLM, PO_TMP_VAL = $PO_PRICE
						WHERE PR_NUM = '$PR_NUM'
							AND JOBCODEID = '$JOBCODEID'
							AND PR_ID = $PRD_ID";
		$this->db->query($s_TMP);
	...
	function get_AllDataIITMREQDET() 		// GOOD - AFTER SELECT SPP
	{
		$PRJCODE	= $_GET['id'];
		$PO_NUM		= $_GET['PO_NUM'];
		...
		// START : FOR SERVER-SIDE
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				$JOBCODEID		= $dataI['JOBCODEID'];
				$JOBPARENT 		= $dataI['JOBPARENT'];
				$JOBPARDESC		= htmlspecialchars($dataI['JOBPARDESC'], ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401 | ENT_HTML5);
				$PRD_ID 		= $dataI['PRD_ID'];
				$ITM_CODE 		= $dataI['ITM_CODE'];
				$ITM_NAME 		= htmlspecialchars($dataI['ITM_NAME'], ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401 | ENT_HTML5);
				$ITM_NAME 		= wordwrap($ITM_NAME, 60, "<br>", TRUE);
				$ITM_GROUP 		= $dataI['ITM_GROUP'];
				$ITM_UNIT 		= strtoupper($dataI['ITM_UNIT']);
				$ITM_UNIT2 		= strtoupper($dataI['ITM_UNIT2']);
				...
				// START : GET BUDGET
					// *Noted : Volume yg dibatalkan di PO, volume yg kembali ke SPP sudah terpotong oleh pembatalan PO
					$ITM_PRICE 		= $ITM_PRICE;
					$s_BUDGVAL		= "SELECT
										ITM_VOLM, ITM_PRICE, ITM_LASTP, ITM_BUDG,
										AMD_VOL, AMD_VOL_R, AMD_VAL, AMD_VAL_R, AMDM_VOL, AMDM_VAL,
										-- (PR_VOL - PR_CVOL + WO_VOL - PO_CVOL + VCASH_VOL + VLK_VOL + PPD_VOL) AS TREQ_VOL,
										(PR_VOL - PR_CVOL + WO_VOL + VCASH_VOL + VLK_VOL + PPD_VOL) AS TREQ_VOL,
										...
										(UM_VOL + OPN_VOL + VCASH_VOL + VLK_VOL + PPD_VOL) AS TUSED_VOL,
										(UM_VOL_R + OPN_VOL_R + VCASH_VOL_R + VLK_VOL_R + PPD_VOL_R) AS TUSED_VOL_R,
										(UM_VAL + OPN_VAL + VCASH_VAL + VLK_VAL + PPD_VAL) AS TUSED_VAL,
										(UM_VAL_R + OPN_VAL_R + VCASH_VAL_R + VLK_VAL_R + PPD_VAL_R) AS TUSED_VAL_R
										FROM tbl_joblist_detail_$PRJCODEVW WHERE JOBCODEID = '$JOBCODEID' AND PRJCODE = '$PRJCODE' LIMIT 1";
					$r_BUDGVAL		= $this->db->query($s_BUDGVAL)->result();
					foreach($r_BUDGVAL as $rowJOBDR) :
						$ITM_VOLM 		= $rowJOBDR->ITM_VOLM;		// BUDG VOL
						...
						$TREQ_VOL 		= $rowJOBDR->TREQ_VOL;
						$TREQ_VAL 		= $rowJOBDR->TREQ_VAL;
						$TREQ_VOL_R		= $rowJOBDR->TREQ_VOL_R;	// VOL REQUEST NEW, CONFIRMED &  WAITING
						$TREQ_VAL_R		= $rowJOBDR->TREQ_VAL_R;	// VAL REQUEST NEW, CONFIRMED &  WAITING

						$TUSED_VOL 		= $rowJOBDR->TUSED_VOL;
						$TUSED_VOL_R	= $rowJOBDR->TUSED_VOL_R;	// VAL PENGGUNAAN NEW, CONFIRMED &  WAITING
						$TUSED_VAL 		= $rowJOBDR->TUSED_VAL;
						$TUSED_VAL_R	= $rowJOBDR->TUSED_VAL_R;	// VAL PENGGUNAAN NEW, CONFIRMED &  WAITING
					endforeach;
					...
					$ITM_REMVOLA	= $TBUDG_VOL - $TREQ_VOL - $TREQ_VOL_R;
					$ITM_REMVALA	= $TBUDG_VAL - $TREQ_VAL - $TREQ_VAL_R;
				// END : GET BUDGET
				
				// START : GET TOTAL ORDERED VOL AND VAL SAAT INI YANG AKTIF (TEMPORARY)
					$ORDERED_VOL0		= 0;
					$ORDERED_VAL0		= 0;
					$ORDERED_TMP_VOL0	= 0;
					$ORDERED_TMP_VAL0	= 0;
					...
					foreach($r_TOTPOVOL0 as $rw_TOTPOVOL0) :
						...
						$ORDERED_TMP_VOL0	= $rw_TOTPOVOL0->ORDERED_TMP_VOL;
						$ORDERED_TMP_VAL0	= $rw_TOTPOVOL0->ORDERED_TMP_VAL;
					endforeach;

					$ORDERED_VOL 	= $ORDERED_VOL0;
					$ORDERED_VAL 	= $ORDERED_VAL0;
					$TOTVAL_OTHDOC 	= $ORDERED_VAL;
				// END : GET TOTAL ORDERED VOL AND VAL SAAT INI YANG AKTIF (TEMPORARY)

				// START : SISA VOL AND VAL SETELAH DIKURANGI DENGAN DOKUMEN AKTIF (TEMPORARY)
					// $ITM_REMVOL	= $ITM_REMVOLA - $ORDERED_VOL - $ORDERED_TMP_VOL0;
					// $ITM_REMVAL	= $ITM_REMVALA - $ORDERED_VAL - $ORDERED_TMP_VAL0;
					$ITM_REMVOL		= $ITM_REMVOLA - $ORDERED_TMP_VOL0;
					$ITM_REMVAL		= $ITM_REMVALA - $ORDERED_TMP_VAL0;
				// END : SISA VOL AND VAL SETELAH DIKURANGI DENGAN DOKUMEN AKTIF (TEMPORARY)

				// START : SISA BUDGET VOLUME SPP PER PR ID
					$REM_SPP_VOL 	= $PR_VOLM - $ORDERED_VOL - $ORDERED_TMP_VOL0;
				...
				// ADA REGULASI BARU UNTUK OP OVERHEAD
					...
					if($REM_SPP_VOL > 0 && $ITM_REMVAL > 0) 			// ITEM LS AND NON-LS TYPE, VOL AND VAL MUST BE GREATHER
						$ITM_AVAIL	= 1;
				...
				$PO_DESC		= htmlspecialchars($dataI['PO_DESC'], ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401 | ENT_HTML5);
				$TAXCODE1		= $dataI['TAXCODE1'];
				$TAXCODE2		= $dataI['TAXCODE2'];
				$TAXPRICE1		= $dataI['TAXPRICE1'];
				$TAXPRICE2		= $dataI['TAXPRICE2'];
				...
				$disabledB 	= 0;
				if($REM_SPP_VOL <= 0 || $ITM_AVAIL == 0) $disabledB = 1;
				
				$selRow 	= $noU;

				if($disabledB == 0)
				{
					$chkBox	= "<input type='checkbox' name='chk4' id='chkRow_".$noU."' value='".$PO_ID."|".$PRD_ID."|".$ITM_CODE."|".$ITM_NAME."|".$JOBCODEDET."|".$JOBCODEID."|".$JOBPARENT."|".$JOBPARDESC."|".$JOBDESC."|".$ITM_PRICE."|".$TBUDG_VAL."|".$ORDERED_VAL."|".$REM_SPP_VOL."|".$ITM_REMVAL."|".$ITM_CATEG."|".$PR_VOLM."|".$PR_AMOUNT."|".$TOTVAL_OTHDOC."|".$PO_VOLM."|".$ORDERED_VOL0."|".$ITM_UNIT."|".$ITM_UNIT2."|".$PR_DESC_ID."|".$PO_DESC_ID."|".$PO_DESC."|".$selRow."|".$ITM_GROUP."' onChange='pickThis4(this);'/>";
				}
				else
				{
				...
				$vw 		= "$PO_ID :  $PRD_ID :  $ITM_CODE :  $ITM_NAME :  $JOBCODEDET :  $JOBCODEID :  $JOBPARENT :  $JOBPARDESC :  $JOBDESC :  $ITM_PRICE :  $TBUDG_VAL :  $ORDERED_VAL :  $ITM_REMVOL :  $ITM_REMVAL :  $ITM_CATEG :  $PR_VOLM :  $PR_AMOUNT :  $TOTVAL_OTHDOC :  $PO_VOLM :  $ORDERED_VOL0 :  $ITM_UNIT :  $PR_DESC_ID :  $PO_DESC_ID :  $PO_DESC : $selRow";
				...
				//{
					$output['data'][] 	= array("$chkBox",
												...
												$ORDERED_VOL,
												$REM_SPP_VOL,
												"<input type='text' class='form-control' style='min-width:80px; max-width:110px; text-align:right' name='PO_REQVOLMX".$noU."' id='PO_REQVOLMX".$noU."' value='".number_format($REM_SPP_VOL, 3)."' onBlur='chgReqVol(this.value, ".$noU.")' disabled>
												<input type='hidden' class='form-control' style='min-width:80px; max-width:110px; text-align:right' name='PO_REQVOLM".$noU."' id='PO_REQVOLM".$noU."' value='".$REM_SPP_VOL."'>
												<input type='hidden' class='form-control' style='min-width:80px; max-width:110px; text-align:right' name='PO_REMVOLM".$noU."' id='PO_REMVOLM".$noU."' value='".$REM_SPP_VOL."'>");
					$noU		= $noU + 1;
				//}
    ...
    function shwIR_H15tDET() 
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data['JOBPARENT'] 		= $JOBPARENT;
			$data['JOBPARDESC'] 	= $JOBPARDESC;
			$data['ITM_CODE'] 		= $ITM_CODE;
			$data['PO_ID'] 			= $PO_ID;
			$data['PO_VOLM'] 		= $PO_VOLM;
			$data['PO_CVOL'] 		= $PO_CVOL;
			$data['viewType'] 		= 0;
			
			$this->load->view('v_purchase/v_purchase_po/v_ir_hist_itm', $data);
		}
		else
		{
			redirect('__l1y');
