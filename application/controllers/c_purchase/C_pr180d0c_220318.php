...
class C_pr180d0c extends CI_Controller  
{
		...
		// START : FOR SERVER-SIDE
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				$output['data'][] 	= array("<div style='white-space:nowrap'>
		                                        ...
		                                        <input type='hidden' name='urldelD".$noU."' id='urldelD".$noU."' value='".$canclRow."'>
	...
	function chgVol()
	{
		...
		if($TOTPRQTY3 > $ITM_BUDGV || $TOTPRAMOUNT3 > $ITM_BUDGA)
		{
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				// LS PROCEDURE 1
					if($JOBUNIT == 'LS')
					{
						...
						$MAX_REQ	= $TOT_AMOUNTBG - $TOT_REQ;		// 14
						$PO_VOLM	= $PO_AMOUNT;
						$TOT_BUDG	= $TOT_AMOUNTBG;				// not used in form*/
						$TOT_REQ 	= $TOT_PRAMN;
						$MAX_REQ	= $TOT_AMOUNTBG - $TOT_REQ;		// 14
						$PO_VOLM	= $PO_AMOUNT;
						$TOT_BUDG	= $TOT_AMOUNTBG;				// not used in form
					...
					else
					{
						...
						$MAX_REQ	= $TOT_VOLMBG - $TOT_REQ;		// 14
						$PO_VOLM	= $PO_VOLM;
						$TOT_BUDG	= $TOT_VOLMBG;					// not used in form*/
						$TOT_REQ 	= $TOT_PRVOL;
						$MAX_REQ	= $TOT_VOLMBG - $TOT_REQ;		// 14
						$PO_VOLM	= $PO_VOLM;
						$TOT_BUDG	= $TOT_VOLMBG;					// not used in form
					...
					$REMREQ_QTY		= $TOT_VOLMBG - $TOT_PRVOL;
					$REMREQ_AMN		= $TOT_AMOUNTBG - $TOT_PRAMN;
				...
				// IS LAST SETT
					...
					else
					{
						...
						$JOBUNITV 	= $JOBUNIT;
						$JOBVOLMV	= number_format($TOT_VOLMBG, 2);
						$TOT_REQV	= number_format($TOT_REQ, 2);
						$PO_VOLMV	= number_format($PO_VOLM, 2);
						if($disabledB == 0)
							$statRem = number_format($REMREQ_QTY, 2);
				...
				// OTHER SETT
					if($disabledB == 0)
					{
						$chkBox		= "<input type='checkbox' name='chk1' value='".$JOBCODEDET."|".$JOBCODEID."|".$JOBCODE."|".$PRJCODE."|".$ITM_CODE."|".$JOBDESC1."|".$serialNumber."|".$ITM_UNIT."|".number_format($ITM_PRICE, 2, '.', '')."|".number_format($TOT_VOLMBG, 2, '.', '')."|".number_format($ITM_STOCK, 2, '.', '')."|".number_format($ITM_USED, 2, '.', '')."|".$itemConvertion."|".$TOT_AMOUNTBG."|".$tempTotMax."|".$PO_AMOUNT."|".$TOT_BUDG."|".$TOT_REQ."|".$ITM_TYPE."|".$JOBDESCH."' onClick='pickThis1(this);'/>";
					}
					else
					{
						$chkBox		= "<input type='checkbox' name='chk' value='".$JOBCODEDET."|".$JOBCODEID."|".$JOBCODE."|".$PRJCODE."|".$ITM_CODE."|".$JOBDESC1."|".$serialNumber."|".$ITM_UNIT."|".$ITM_PRICE."|".$TOT_VOLMBG."|".$ITM_STOCK."|".$ITM_USED."|".$itemConvertion."|".$TOT_AMOUNTBG."|".$tempTotMax."|".$PO_AMOUNT."|".$TOT_BUDG."|".$TOT_REQ."|".$ITM_TYPE."' style='display: none' />";
	...
	function addItmTmp()
	{
		...
		// TOTAL YANG DIREQUEST ADALAH TOTAL YANG SUDAH DI PR (1,2,3,4,6) + TEMP
			$TOT_PR 	= 0;
			$s_PRT 		= "SELECT SUM(PR_VOLM) AS TOT_PR FROM tbl_pr_detail WHERE PR_STAT IN (1,2,3,4,6) AND ITM_CODE = '$ITM_CODE' AND PRJCODE = '$PRJCODE'";
			$r_PRT 		= $this->db->query($s_PRT)->result();
			foreach($r_PRT as $rw_PRT):
				$TOT_PR = $rw_PRT->TOT_PR;
			endforeach;
			if($TOT_PR == '')
				$TOT_PR = 0;
	...
	function cancelItem()
	{
		...
		$s_01 			= "SELECT JOBCODEID, PR_VOLM, PR_PRICE, PR_TOTAL, PO_VOLM, PO_AMOUNT
							FROM tbl_pr_detail
