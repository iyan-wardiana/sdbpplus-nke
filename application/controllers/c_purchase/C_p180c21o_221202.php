		...
		// START : FOR SERVER-SIDE
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				// START : GET TOTAL ORDERED VALUE
					...
					// 2. FROM OTHERS DOCUMENT AND PR YANG SAMA
						$ORDERED_VAL1	= 0;
						$s_TOTPOVAL		= "SELECT SUM(A.PO_COST) AS ORDERED_VAL,
											SUM(A.PO_CTOTAL) AS ORDERED_CVAL
											FROM tbl_po_detail A
					...
					// 2A. FROM OTHERS DOCUMENT AND PR YANG BERBEDA
						$ORDERED_VAL1A	= 0;
						$s_TOTPOVAL		= "SELECT SUM(A.PO_COST) AS ORDERED_VAL,
											SUM(A.PO_CTOTAL) AS ORDERED_CVAL
											FROM tbl_po_detail A
						...
						$r_TOTPOVAL		= $this->db->query($s_TOTPOVAL)->result();
						foreach($r_TOTPOVAL as $rw_TOTPOVAL) :
							$ORDERED_VAL1A	= $rw_TOTPOVAL->ORDERED_VAL;
							$ORDERED_CVAL1A	= $rw_TOTPOVAL->ORDERED_CVAL;
							if($ORDERED_VAL1A == '')
								$ORDERED_VAL1A 	= 0;
					...
					// 4. TOTAL VOLUME PO
						$ORDERED_VAL 	= ($ORDERED_VAL0 - $ORDERED_CVAL0) + ($ORDERED_VAL1 + $ORDERED_VAL1A - $ORDERED_CVAL1) + $ORDERED_VAL2 + $ORDERED_VAL3 + $ORDERED_VAL4;
				...
				if($PO_STAT == 1 || $PO_STAT == 4)
				{
					...
					$vWPO_ORDER 	= "<input type='text' class='form-control' style='min-width:90px; max-width:300px; text-align:right' name='PO_VOLMX".$noU."' id='PO_VOLMX".$noU."' value='".number_format($PO_VOLM, 2)."' onKeyPress='return isIntOnlyNew(event);' onBlur='getValuePO(".$noU.");' ".$isDisabled.">";
				...
				/* ---------------------------------- Hold --------------------------
					$output['data'][] 	= array("$BTN_CNCVW
												...
												"<span id='ITMVOL_R".$PO_DESC_ID."".$noU."' style='text-align: right;'>	
													".number_format($ITMVOLM_R, 2)."
												</span>",
												"$ITM_UNIT
												<input type='hidden' id='data".$noU."ITM_UNIT' name='data[".$noU."][ITM_UNIT]' value='".$ITM_UNIT."'>",
												"$vWITM_PRICE
				...
				$output['data'][] 	= ["$BTN_CNCVW
										...
										"$ITM_REMVOLv
										<input type='hidden' class='form-control' style='min-width:130px; max-width:200px; text-align:right' name='ITM_REMAIN".$noU."' id='ITM_REMAIN".$noU."' value='".$ITM_REMVOL."' >
										<input type='hidden' class='form-control' style='min-width:130px; max-width:200px; text-align:right' name='ITM_BUDGVAL".$noU."' id='ITM_BUDGVAL".$noU."' value='".$ITM_BUDGVAL."' >
