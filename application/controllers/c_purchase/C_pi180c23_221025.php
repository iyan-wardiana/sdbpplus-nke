...
class C_pi180c23 extends CI_Controller  
{
				...
				if($AH_ISLAST == 1)
				{
					...
					// START : JOURNAL DETAIL : Hutang Lain(D) dan Hutang Supplier (K)
						...
						foreach($_POST['data'] as $d)
						{
							...
							$TTK_REF1_CODE 	= '';
							$TTK_REF2_CODE 	= '';
							$s_01 	= "SELECT TTK_REF1_CODE, TTK_REF2_CODE FROM tbl_ttk_detail WHERE TTK_NUM = '$TTK_NUM'";
	...
	function printdocument()
	{
		...
		{		
			...
			else $this->load->view('v_purchase/v_purchase_inv/print_inv', $data);
		}
		else
		{
			redirect('__l1y');
	...
	function get_AllDataIRSJ() // GOOD
	{
		...
		// START : FOR SERVER-SIDE
			...
			$start			= $_REQUEST['start'];
			$search			= $_REQUEST['search']["value"];
			$num_rows 		= $this->m_purchase_inv->get_AllDataIRSJC($PRJCODE, $SPLCODE, $search);
			$total			= $num_rows;
			$output			= array();
			$output['draw']	= $draw;
			$output['recordsTotal'] = $output['recordsFiltered']= $total;
			$output['data']	= array();
			$query 			= $this->m_purchase_inv->get_AllDataIRSJL($PRJCODE, $SPLCODE, $search, $length, $start, $order, $dir);
  	...
  	function get_AllDataTTKL() // GOOD
	{
		...
		// START : FOR SERVER-SIDE
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				$TTK_NOTES		= htmlspecialchars($dataI['TTK_NOTES'], ENT_QUOTES);
	...
	function s3l4llW_0L() // GOOD
	{
		...
		// START : FOR SERVER-SIDE
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				$chkBox	= "<input type='radio' name='chk1' id='chk1' value='".$WO_NUM."|".$WO_CODE."|".$REF_TYPE."|".$WO_VALUE."|".$DP_PERC."' onClick='pickThisWO(this);' />";
