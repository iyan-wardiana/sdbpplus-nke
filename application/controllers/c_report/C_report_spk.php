 ...
 * File Name	= c_report_spk.php
 * Location		= -
*/

if ( ! defined('BASEPATH')) exit('No direct script access allowed');
class C_report_spk extends CI_Controller  
{	
    function index()
	{		
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$url			= site_url('c_report/c_report_spk/spk/?id='.$this->url_encryption_helper->encode_url($appName));
			redirect($url);
		}
		else
		{
...
/*	function r_assetlist_view() // OK
	...
	function spk() // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data['h2_title']		= 'SPK';
			$data['h3_title']		= 'Report';
			$data['form_action'] 	= site_url('c_report/c_report_spk/spk_view/?id='.$this->url_encryption_helper->encode_url($appName));
			
			$this->load->view('v_report/v_spk/v_spk', $data);
		}
		else
		{
			redirect('Auth');
	...
	function spk_view() // OK
	{
		...
		{			
			...
			{
				$this->load->view('v_report/v_spk/v_spk_report_adm', $data);
			}
			else
			{
				$this->load->view('v_report/v_spk/v_spk_report', $data);
			}
		}
		else
		{
