...
class C_j0b0rd3r extends CI_Controller  
{
 	...
 	function index() // G
	{
		...
		$url			= site_url('c_production/c_j0b0rd3r/prj180c21l/?id='.$this->url_encryption_helper->encode_url($appName));
		redirect($url);
	}
	...
	function prj180c21l() // G
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data['viewPRJ'] 	= $this->m_projectlist->get_all_project($DefEmp_ID)->result();			
			$data["secURL"] 	= "c_production/c_j0b0rd3r/glj0b0rd3r/?id=";
	...
	function glj0b0rd3r() // G
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			// -------------------- START : SEARCHING METHOD --------------------
				...
				$data["url_search"] = site_url('c_production/c_j0b0rd3r/f4n7_5rcH/?id='.$this->url_encryption_helper->encode_url($PRJCODE));
			
				//$num_rows 			= $this->m_joborder->count_all_JO($PRJCODE, $key);
				//$data["cData"] 		= $num_rows;	 
				//$data['vData']		= $this->m_joborder->get_all_JO($PRJCODE, $start, $end, $key)->result();
			...
			$data['addURL'] 	= site_url('c_production/c_j0b0rd3r/a44_j0b0rd3r/?id='.$this->url_encryption_helper->encode_url($PRJCODE));
			$data['backURL'] 	= site_url('c_production/c_j0b0rd3r/?id='.$this->url_encryption_helper->encode_url($PRJCODE));
			...
			$this->load->view('v_production/v_joborder/v_joborder', $data);
		}
		else
		{
			redirect('__l1y');
	...
	// -------------------- START : SEARCHING METHOD --------------------
		function f4n7_5rcH()
		{
			...
			$url			= site_url('c_production/c_j0b0rd3r/glj0b0rd3r/?id='.$this->url_encryption_helper->encode_url($collDATA));
			redirect($url);
		}
	// -------------------- END : SEARCHING METHOD --------------------
  	...
  	function get_AllData() // GOOD
	{
		...
		// START : FOR SERVER-SIDE
			...
			$num_rows 		= $this->m_joborder->get_AllDataC($PRJCODE, $search);
			$total			= $num_rows;
			$output			= array();
			$output['draw']	= $draw;
			$output['recordsTotal'] = $output['recordsFiltered']= $total;
			$output['data']	= array();
			$query 			= $this->m_joborder->get_AllDataL($PRJCODE, $search, $length, $start, $order, $dir);
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				$secUpd		= site_url('c_production/c_j0b0rd3r/u77_j0b0rd3r/?id='.$this->url_encryption_helper->encode_url($CollID));
				$secCMR		= site_url('c_production/c_j0b0rd3r/add_processMR/?id='.$this->url_encryption_helper->encode_url($CollID));
				$secPrint	= site_url('c_production/c_j0b0rd3r/prnt180d0bdoc/?id='.$this->url_encryption_helper->encode_url($JO_NUM));
				$secPrintQR	= site_url('c_production/c_j0b0rd3r/printQR/?id='.$this->url_encryption_helper->encode_url($JO_NUM));
				...
				$delID 		= "$secDelIcut~tbl_jo_header~tbl_jo_detail~JO_NUM~$JO_NUM~PRJCODE~$PRJCODE";
				...
				if($JO_STAT == 1) 
				{
					$secAction	= "<input type='hidden' name='urlPrint".$noU."' id='urlPrint".$noU."' value='".$secPrint."'>
								   ...
								   <a href='".$secUpd."' class='btn btn-info btn-xs' title='Update'>
									...
									<a href='".$secCMR."' class='btn btn-".$colMR." btn-xs' title='Creat MR' ".$isDis.">
				...
				else
				{
					$secAction	= "<input type='hidden' name='urlPrint".$noU."' id='urlPrint".$noU."' value='".$secPrint."'>
								   ...
								   <a href='".$secUpd."' class='btn btn-info btn-xs' title='Update'>
									...
									<a href='#' class='btn btn-".$colMR." btn-xs' ".$isDis." title='Creat MR' onClick='procMR(".$noU.")'>
										<i class='glyphicon glyphicon-duplicate'></i>
									</a>
	...
	function a44_j0b0rd3r() // G
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data['form_action']	= site_url('c_production/c_j0b0rd3r/add_process');
			$cancelURL				= site_url('c_production/c_j0b0rd3r/glj0b0rd3r/?id='.$this->url_encryption_helper->encode_url($PRJCODE));
			...
			$MenuCode 				= 'MN370';
			$data["MenuCode"] 		= 'MN370';
			$data['viewDocPattern'] = $this->m_joborder->getDataDocPat($MenuCode)->result();
			...
			$this->load->view('v_production/v_joborder/v_joborder_form', $data);
		}
		else
		{
			redirect('__l1y');
	...
	function s3l4llW1p() // GOOD
	{
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data['PRJCODE'] 		= $PRJCODE;
			$data['PRODS_STEP'] 	= $PRODS_STEP;
			$data['cAllItemPrm']	= $this->m_joborder->count_all_prim($PRJCODE, $SO_NUM, $PRODS_STEP);
			$data['vwAllItemPrm'] 	= $this->m_joborder->view_all_prim($PRJCODE, $SO_NUM, $PRODS_STEP)->result();
			
			$data['cAllItemSubs']	= $this->m_joborder->count_all_subs($PRJCODE, $SO_NUM, $PRODS_STEP);
			$data['vwAllItemSubs'] 	= $this->m_joborder->view_all_subs($PRJCODE, $SO_NUM, $PRODS_STEP)->result();
	...
	function s3l4llit3m() // OK
	{
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data['PRJCODE'] 		= $PRJCODE;
			$data['secShowAll']		= site_url('c_production/c_j0b0rd3r/s3l4llit3m/?id='.$this->url_encryption_helper->encode_url($PRJCODE));
			
			$data['countAllItem'] 	= $this->m_joborder->count_all_item($PRJCODE);
			$data['vwAllItem'] 		= $this->m_joborder->get_all_item($PRJCODE)->result();
					
			$this->load->view('v_production/v_joborder/v_joborder_selitem', $data);
		}
		else
		{
			redirect('__l1y');
	...
	function add_process() // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$JO_DESC 		= addslashes($this->input->post('JO_DESC'));
			//$JO_VOLM 		= $this->input->post('JO_VOLM');
			$JO_VOLM 		= 0;
			$JO_NOTES 		= addslashes($this->input->post('JO_NOTES'));
			...
			// START : INSERT DETAIL FG
				...
				$this->m_joborder->updateJOH($JO_NUM, $updJOH);
				...
				foreach($_POST['selSTEP'] as $dSTEP)
				{
					...
					if(isset($_POST["dataRM$P_STEP"]))
					{
						foreach($_POST["dataRM$P_STEP"] as $dRM1)
						{
							...
							$JOSTFTYPE			= $dRM1['ITM_TYPE'];
							$ITMCODEX			= $dRM1['ITM_CODE'];
							$ITM_CATEG 			= "";
							$sqlITMCAT			= "SELECT ITM_CATEG FROM tbl_item WHERE PRJCODE = '$PRJCODE' AND ITM_CODE = '$ITMCODEX'";
							$resITMCAT			= $this->db->query($sqlITMCAT)->result();
							foreach($resITMCAT as $rowITM):
								$ITM_CATEG		= $rowITM->ITM_CATEG;
							endforeach;
							$dRM['ITM_CATEG']	= $ITM_CATEG;

							$dRM['ITM_PRICE']	= $dRM1['ITM_PRICE'];
							$ITMPRICE 			= $dRM1['ITM_PRICE'];
							if($JOSTFTYPE == 'IN' AND $ITM_CATEG == 'WIP')	// JIKA IN = WIP, MAKA HARGA SET TO 0
							{
								$dRM['ITM_PRICE']	= 0;
								$ITMPRICE			= 0;
							}
			...
			// UPDATE HARGA WIP IN DARI HARGA PRODUKSI WIP SEBELUMNYA
				$TOTINAMNT	= 0;
				$TOTIN_AMNT	= 0;
				$TOTINAMNT2	= 0;
				$sqlJD 		= "SELECT DISTINCT JO_NUM, ITM_CODE, JOSTF_ORD, JOSTF_STEP, JOSTF_TYPE, ITM_QTY, ITM_PRICE
								FROM tbl_jo_stfdetail
								WHERE JO_NUM = '$JO_NUM' ORDER BY JOSTF_ORD, JOSTF_TYPE";
				$resJD 		= $this->db->query($sqlJD)->result();
				foreach($resJD as $rowJD) :
					$JO_NUM 	= $rowJD->JO_NUM;
					$ITMCODE 	= $rowJD->ITM_CODE;
					$JOSTFORD 	= $rowJD->JOSTF_ORD;
					$JOSTFSTEP 	= $rowJD->JOSTF_STEP;
					$STEPNEXT	= $JOSTFORD + 1;
					$JOSTFTYPE 	= $rowJD->JOSTF_TYPE;
					$ITMQTY 	= $rowJD->ITM_QTY;
					$ITMPRICE 	= $rowJD->ITM_PRICE;

					if($JOSTFTYPE == 'IN')
					{
						$TOTINAMNT 	= $ITMQTY * $ITMPRICE;
						$TOTIN_AMNT = $TOTIN_AMNT + $TOTINAMNT;
					}

					$ITMWIP		= '';
					if($JOSTFTYPE == 'OUT')
					{
						$sqlUJD 	= "UPDATE tbl_jo_stfdetail SET ITM_PRICE = $TOTIN_AMNT
										WHERE JO_NUM = '$JO_NUM' AND ITM_CODE = '$ITMCODE'
										 AND JOSTF_ORD = $JOSTFORD AND JOSTF_TYPE = 'OUT'";
						$this->db->query($sqlUJD);

						$STEP_NEXTC	= 0;
						$sqlJDNC	= "tbl_jo_stfdetail WHERE JO_NUM = '$JO_NUM' AND ITM_CODE = '$ITMCODE' AND ITM_GROUP = 'WIP'
										 AND JOSTF_ORD = $STEPNEXT AND JOSTF_TYPE = 'IN'";
						$resJDNC 	= $this->db->count_all($sqlJDNC);

						if($resJDNC > 0)
						{
							$sqlUJD1 = "UPDATE tbl_jo_stfdetail SET ITM_PRICE = $TOTIN_AMNT
										WHERE JO_NUM = '$JO_NUM' AND ITM_CODE = '$ITMCODE' AND ITM_GROUP = 'WIP'
										 AND JOSTF_ORD = $STEPNEXT AND JOSTF_TYPE = 'IN'";
							$this->db->query($sqlUJD1);
						}
					}
				endforeach;
			...
			// UPDATE DETAIL
				//$this->m_joborder->updateDet($JO_NUM, $PRJCODE, $JO_DATE, $JO_PRODD);
			...
			$url			= site_url('c_production/c_j0b0rd3r/glj0b0rd3r/?id='.$this->url_encryption_helper->encode_url($PRJCODE));
			redirect($url);
		}
		else
		{
	...
	function u77_j0b0rd3r() // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data['form_action']	= site_url('c_production/c_j0b0rd3r/update_process');
			$cancelURL				= site_url('c_production/c_j0b0rd3r/glj0b0rd3r/?id='.$this->url_encryption_helper->encode_url($PRJCODE));
			$data['link'] 			= array('link_back' => anchor("$cancelURL",'<input type="button" name="btnCancel" id="btnCancel" class="btn btn-danger" value="Cancel" />', array('style' => 'text-decoration: none;')));
			...
			$getJO 							= $this->m_joborder->get_jo_by_number($JO_NUM)->row();
	...
	function update_process() // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$updJO			= array('JO_CODE' 		=> $JO_CODE,
									...
									'Patt_Date'		=> $Patt_Date,
									'Patt_Number'	=> $Patt_Number);
			$this->m_joborder->updateJO($JO_NUM, $updJO);
			...
			if($JO_STAT == 6) // HOLD
			{
				foreach($_POST['data'] as $d)
				{
					$JO_NUM		= $d['JO_NUM'];
					$ITM_CODE	= $d['ITM_CODE'];
					//$this->m_joborder->updateVolBud($SO_NUM, $PRJCODE, $ITM_CODE); // HOLD
				}
			}
			elseif($JO_STAT == 9)	// VOID
			{
				foreach($_POST['data'] as $d)
				{
					...
					$prm 		= array('SO_NUM' 	=> $SO_NUM,
										...
										'ITM_TOTAL' => $ITM_TOTAL,
										'PRJCODE'	=> $PRJCODE);
					$this->m_joborder->updVOID($prm);
				}
				
				// START : UPDATE TO DOC. COUNT
					$parameters 	= array('DOC_CODE' 		=> $JO_NUM,
											'PRJCODE' 		=> $PRJCODE,
											'DOC_TYPE'		=> "JO",
											'DOC_QTY' 		=> "DOC_JOQ",
											'DOC_VAL' 		=> "DOC_JOV",
			...
			{
				$this->m_joborder->deleteJODetail($JO_NUM);
				...
				// START : INSERT DETAIL PROCESS
					$this->m_joborder->deleteJOSTFDetail($JO_NUM);
					...
					foreach($_POST['selSTEP'] as $dSTEP)
					{
						...
						if(isset($_POST["dataFG$P_STEP"]))
						{
							foreach($_POST["dataFG$P_STEP"] as $dFG1)
							{
								...
								// UPDATE WIP NEXT FROM WIP BEFORE
								$updWIP	= "UPDATE tbl_jo_stfdetail SET ITM_PRICE = $ITM_PRICEFG
											WHERE JO_NUM = '$JO_NUM' AND ITM_CODE = '$ITMCODEWIP' AND PRJCODE = '$PRJCODE'";
								$this->db->query($updWIP);
							}
						}
					}
				// END : INSERT DETAIL PROCESS
			...
			// UPDATE HARGA WIP IN DARI HARGA PRODUKSI WIP SEBELUMNYA
				...
				// UPDATE DETAIL JO
					//$this->m_joborder->updateDet($JO_NUM, $PRJCODE, $JO_DATE, $JO_PRODD);
					$this->m_joborder->updateDetPCost($JO_NUM, $PRJCODE, $GTOTIN_AMN);
			}
	...
	function iNb0x() // OK
	{
		...
		$url			= site_url('c_production/c_j0b0rd3r/p07_l5t_x1/?id='.$this->url_encryption_helper->encode_url($appName));
		redirect($url);
	}
	...
	function p07_l5t_x1() // G
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data['viewPRJ'] 	= $this->m_projectlist->get_all_project($DefEmp_ID)->result();
			$data['viewPRJ'] 	= $this->m_projectlist->get_all_project($DefEmp_ID)->result();			
			$data["secURL"] 	= "c_production/c_j0b0rd3r/glj0b0rd3r_1Nb/?id=";
	...
	function glj0b0rd3r_1Nb() // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			// -------------------- START : SEARCHING METHOD --------------------
				...
				$data["url_search"] = site_url('c_production/c_j0b0rd3r/f4n7_5rcH1nB/?id='.$this->url_encryption_helper->encode_url($PRJCODE));
			
				$num_rows 			= $this->m_joborder->count_all_JOInb($PRJCODE, $key, $DefEmp_ID);
				$data["cData"] 		= $num_rows;	 
				$data['vData']		= $this->m_joborder->get_all_JOInb($PRJCODE, $start, $end, $key, $DefEmp_ID)->result();
			// -------------------- END : SEARCHING METHOD --------------------
			...
			$data['backURL'] 	= site_url('c_production/c_j0b0rd3r/iNb0x/?id='.$this->url_encryption_helper->encode_url($PRJCODE));
			$MenuCode 			= 'MN371';
			$data["MenuCode"] 	= 'MN371';
			/*$MaxLimitApp		= $this->m_joborder->getMyMaxLimit($MenuCode, $DefEmp_ID);
	...
	// -------------------- START : SEARCHING METHOD --------------------
		function f4n7_5rcH1nB()
		{
			...
			$PRJCODE		= $this->url_encryption_helper->decode_url($PRJCODE);
			$collDATA		= "$gEn5rcH~$PRJCODE~$mxLS~$mxLF";
			$url			= site_url('c_production/c_j0b0rd3r/glj0b0rd3r_1Nb/?id='.$this->url_encryption_helper->encode_url($collDATA));
			redirect($url);
		}
	// -------------------- END : SEARCHING METHOD --------------------
	...
	function up180djinb() // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data['form_action']	= site_url('c_production/c_j0b0rd3r/update_process_inb');
			...
			$data['default']['JO_CODE'] 	= $getJO->JO_CODE;
			$data['default']['PRJCODE'] 	= $getJO->PRJCODE;
			$data['default']['JO_CATEG'] 	= $getJO->JO_CATEG;
	...
	function update_process_inb() // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
				...
				if($JO_STAT == 3)
				{
					...
					foreach($_POST['data1'] as $d)
					{
						...
						$prm 			= array('SO_NUM' 	=> $SO_NUM,
												...
												'ITM_TOTAL' => $ITM_TOTAL,
												'PRJCODE'	=> $PRJCODE);
						$this->m_joborder->updSODET($prm);
					}
					
					// UPDATE SO HEADER
						$this->m_joborder->updateSOH($SO_NUM, $PRJCODE);
			...
			$url	= site_url('c_production/c_j0b0rd3r/glj0b0rd3r_1Nb/?id='.$this->url_encryption_helper->encode_url($PRJCODE));
			redirect($url);
		}
		else
		{
	...
	function popupallPR()
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$this->load->view('v_production/v_joborder/v_joborder_sel_req', $data);
		}
		else
		{
			redirect('__l1y');
	...
	function printirlist()
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data['countIR']	= $this->m_joborder->count_all_IR($SO_NUM);
			$data['vwIR'] 		= $this->m_joborder->get_all_IR($SO_NUM)->result();	
	...
	function prnt180d0bdoc()
	{
		...
		{		
			...
			/*$getSO 			= $this->m_joborder->get_SO_by_number($SO_NUM)->row();
	...
	function m0N7_j0() // G
	{
		...
		$url			= site_url('c_production/c_j0b0rd3r/prj1_m0N7j0/?id='.$this->url_encryption_helper->encode_url($appName));
		redirect($url);
	}
	...
	function prj1_m0N7j0() // G
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data['viewPRJ'] 	= $this->m_projectlist->get_all_project($DefEmp_ID)->result();
			$data['viewPRJ'] 	= $this->m_projectlist->get_all_project($DefEmp_ID)->result();			
			$data["secURL"] 	= "c_production/c_j0b0rd3r/m0N7_j0X/?id=";
	...
	function m0N7_j0X() // G
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$this->load->view('v_production/v_joborder/v_joborder_mnt', $data);
		}
		else
		{
			redirect('__l1y');
	...
	function add_processMR() // G
	{
		...
		// START : UPDATE TO T-TRACK
			...
			$this->m_updash->updateTrack($paramTrack);
		// END : UPDATE TO T-TRACK
			echo "No. Permintaan telah dibuat : $MR_CODE";
	}
