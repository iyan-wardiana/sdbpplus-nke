...
class C_r3p0r77l  extends CI_Controller  
{
    ...
    function getAccNumber()
    {
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			if($PRJCODE[0] != 1)
			{
				...
				// endforeach;
				//$addQPRJACC 	= "";
				// $addQPRJACC 	= "AND A.PRJCODE = '$ISHO_PRJ'";
			}
			else
			{
				$sqlPRJ 	= "SELECT PRJCODE FROM tbl_project WHERE isHO = 1 LIMIT 1";
    ...
    function idxpr0f17l005vwD() 
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			{
				$data['title'] 		= $appName;
				$data['h1_title'] 	= 'Detail Transaction Report';
			}
			...
			$this->load->view('v_gl/v_report/v_profit_loss/profit_loss_viewDASH', $data);
		}
		else
		{
			redirect('__l1y');
	...
	function l3dg3r_4llJrn() // OK
	{
  	...
  	function get_AllDataJRN_230627() // OK
	{
		...
		// START : FOR SERVER-SIDE
			...
			$start			= $_REQUEST['start'];
			$search			= $_REQUEST['search']["value"];
			$num_rows 		= $this->m_gej->get_AllDataAC($PRJCODE, $ACCID, $search, $STARTD, $ENDD, $GEJSTAT);
			$total			= $num_rows;
			$output			= array();
			$output['draw']	= $draw;
			$output['recordsTotal'] = $output['recordsFiltered']= $total;
			$output['data']	= array();
			$query 			= $this->m_gej->get_AllDataAL($PRJCODE, $ACCID, $search, $length, $start, $order, $dir, $STARTD, $ENDD, $GEJSTAT);
			...
			foreach ($query->result_array() as $dataI)
			{
				...
				$sqlJD 		= "SELECT A.JournalD_Id, A.JournalH_Code, A.Manual_No, A.JournalH_Date, A.JournalType,
									A.TAX_NO, A.Faktur_Code, A.TAX_DATE, A.Kwitansi_No, A.Kwitansi_Date,
									A.SPLCODE, A.ITM_CODE, A.SPLDESC, A.Other_Desc, A.Revise_Desc, A.GEJ_STAT, 
									A.Acc_Id, A.Acc_Name, SUM(A.JournalD_Debet ) AS JournalD_Debet, SUM(A.JournalD_Kredit) AS JournalD_Kredit, A.Creater
								FROM tbl_journaldetail_$PRJCODEVW A
								WHERE A.proj_Code = '$PRJCODE' AND A.JournalH_Code = '$JournalH_Code' $QRYPRJACC
				...
				foreach($resJD as $rowJD):
					...
					$ITM_NAME 			= "";
					$s_ITM 				= "SELECT ITM_NAME FROM tbl_item WHERE PRJCODE = '$PRJCODE' AND ITM_CODE = '$ITM_CODE'";
					$r_ITM 				= $this->db->query($s_ITM);
					if($r_ITM->num_rows() > 0)
					{
						foreach($r_ITM->result() AS $rw_ITM):
							$ITM_NAME 	= $rw_ITM->ITM_NAME;
						endforeach;
						$ITM_NAME 		= "$ITM_NAME ($ITM_CODE)";
					}
					...
					if($JournalH_Code2 != $JournalH_Code)
					{
						...
						$delID 		= "$secDelIcut~$JournalH_Code~$Manual_No~$JournalType~$PRJCODE";
					...
					/*if($AccNameId == '')
					{
						$sqlAccNm		= "SELECT Account_NameId, Account_NameEn FROM tbl_chartaccount_$PRJCODEVW
	                ...
	                if($Revise_Desc != '')
	                {
						$revDesc 	= 	"<br><strong><i class='fa  fa-bell margin-r-5'></i>Revise Note </strong>
								  		<div style='margin-left: 15px'>
									  		<p class='text-muted' style='font-style: italic;'>".$Revise_Desc."</p>
									  	</div>";
	                }
					...
					$urlsvDoc 	= site_url('c_gl/c_r3p0r77l/vwDocRel/?id=');
					$dtColDoc 	= "$JournalType~$JournalH_Code~$PRJCODE~$Acc_Id~$ACC_NAMEV~$Other_Desc~$Manual_No";
					$svItmDoc	= site_url('c_gl/c_r3p0r77l/vwDocRel/?id='.$this->url_encryption_helper->encode_url($dtColDoc));
					$updACC		= "<i class='fa fa-book margin-r-5 text-red' onClick='setAccUM(\"".$svItmAcc."\")' style='cursor: pointer' title='Ganti Akun'></i>";
					$vwDOC		= "<i class='fa fa-eye margin-r-5 text-blue' onClick='vwDOCDET(\"".$svItmDoc."\")' style='cursor: pointer' title='Tampilkan Dokumen'></i>";

					$Other_DescD= wordwrap($Other_Desc, 50, "<br>", true);
				...
				$output['data'][] 	= array("<div style='vertical-align:middle; text-align:center;'>&nbsp;</div>",
											...
											"<div style='vertical-align:middle; text-align:center;'>&nbsp;</div>",
											"<div style='vertical-align:middle; text-align:center;'>&nbsp;</div>",
										  	"<div style='white-space:nowrap'><b>".number_format($totD,2)."$unBalD</b></div>",
										  	"<div style='white-space:nowrap'><b>".number_format($totK,2)."$unBalK</b></div>",
										  	"<div style='vertical-align:middle; text-align:center;'>&nbsp;</div>",
											"<div style='vertical-align:middle; text-align:center;'>&nbsp;</div>",
											"<div style='vertical-align:middle; text-align:center;'>&nbsp;</div>",
											"<div style='vertical-align:middle; text-align:center;'>&nbsp;</div>",
			...
			$output['data'][] 	= array("<div style='vertical-align:middle; text-align:center;'>&nbsp;</div>",
										...
										"<div style='vertical-align:middle; text-align:center;'>&nbsp;</div>",
										"<div style='vertical-align:middle; text-align:center;'>&nbsp;</div>",
									  	"<div style='white-space:nowrap'><b>".number_format($GtotD,2)."</b></div>",
									  	"<div style='white-space:nowrap'><b>".number_format($GtotK,2)."</b></div>",
									  	"<div style='vertical-align:middle; text-align:center;'>&nbsp;</div>",
										"<div style='vertical-align:middle; text-align:center;'>&nbsp;</div>",
										"<div style='vertical-align:middle; text-align:center;'>&nbsp;</div>",
										"<div style='vertical-align:middle; text-align:center;'>&nbsp;</div>",
	...
	function get_AllDataJRN() // OK
	{
		...
		$GEJSTAT	= $_GET['GEJ_STAT'];
		$STARTD		= date('Y-m-d',strtotime(str_replace('/', '-', $_GET['STARTD'])));
		$ENDD		= date('Y-m-d',strtotime(str_replace('/', '-', $_GET['ENDD'])));
		...
		// START : FOR SERVER-SIDE
			...
			$start			= $_REQUEST['start'];
			$search			= $_REQUEST['search']["value"];
			$num_rows 		= $this->m_gej->get_AllDataAC($search, $STARTD, $ENDD, $GEJSTAT);
			$total			= $num_rows;
			$output			= array();
			$output['draw']	= $draw;
			$output['recordsTotal'] = $output['recordsFiltered']= $total;
			$output['data']	= array();
			$query 			= $this->m_gej->get_AllDataAL($search, $length, $start, $order, $dir, $STARTD, $ENDD, $GEJSTAT);
    ...
    function vwDocRel() 
	{
		...
		$data['JRN_TYPE'] 		= $JRN_TYPE;
		$data['PRJCODE'] 		= $PRJCODE;
		$data['JRN_NUM'] 		= $JRN_NUM;
		$data['JRN_CODE'] 		= $JRN_CODE;
		
		$this->load->view('v_gl/v_report/v_jrnlist/v_jrnlist_hist', $data);
    }
