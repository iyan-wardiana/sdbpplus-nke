...
class Profit_loss  extends CI_Controller  
{
	...
	function get_all_stock_opn() // U
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data['backURL'] 		= site_url('c_gl/profit_loss/');
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			if($SOPNH_STAT == 3)
			{
				...
				// UPDATE MATERIAL
					...
					if($resCM > 0)
					{
						...
						else
						{
							...
							$this->m_profit_loss->addCon($IConcl);
						}
					}
				...
				// UPDATE TOOLS
					...
					if($resCT > 0)
					{
						...
						else
						{
							...
							$this->m_profit_loss->addCon($IConclT);
						}
					}
				...
				foreach($_POST['data'] as $d)
				{
					...
					$sqlUpStok	= "UPDATE tbl_item SET ITM_IN = ITM_IN + $ITEM_IN, ITM_INP = ITM_INP + $ITEM_INP, ITM_OUT = ITM_OUT + $ITEM_OUT,
									ITM_OUTP = ITM_OUTP + $ITEM_OUTP, ITM_PRICE = $ITEM_PRICE, ITM_TOTALP = $ITM_TOTALP,
									ITM_REMQTY = $ITEM_REM, LAST_TRXNO = '$SOPNH_CODE'									
			...
			if($SOPNH_STAT == 3)
			{
				...
				foreach($_POST['data'] as $d)
				{
					...
					$sqlUpStok	= "UPDATE tbl_item SET ITM_IN = $ITEM_IN, ITM_INP = $ITEM_INP, ITM_OUT = $ITEM_OUT, ITM_OUTP = $ITEM_OUTP,
									ITM_PRICE = $ITEM_PRICE, ITM_TOTALP = $ITM_TOTALP, ITM_REMQTY = $ITEM_REM, LAST_TRXNO = '$SOPNH_CODE'									
									WHERE PRJCODE = '$PRJCODED' AND ITM_CODE = '$ITEM_CODE'";
					$this->db->query($sqlUpStok);
				}
			}
	...
	function asset_exp_idx() // U
	{
		...
		$url			= site_url('c_gl/profit_loss/asset_exp_prjlist/?id='.$this->url_encryption_helper->encode_url($appName));
		redirect($url);
	}
	...
	function get_all_asset_exp() // U
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			//$data['link'] 			= array('link_back' => anchor('c_gl/profit_loss/asset_exp_idx','<input type="button" name="btnCancel" id="btnCancel" class="btn btn-danger" value="Back" />', array('style' => 'text-decoration: none;')));
			$data['backURL'] 		= site_url('c_gl/profit_loss/asset_exp_idx');
			$data['PRJCODE'] 		= $PRJCODE;
			$data['MenuCode'] 		= 'MN303';
			
			$num_rows 				= $this->m_profit_loss->count_all_asset_exp($PRJCODE);
			$data["recCount"] 		= $num_rows;
	 
			$data['vwAssetExp']		= $this->m_profit_loss->get_all_asset_exp($PRJCODE)->result();
	...
	function addAssExp() // U
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$backURL			= site_url('c_gl/profit_loss/get_all_asset_exp/?id='.$this->url_encryption_helper->encode_url($PRJCODE));
			...
			$data['h2_title']	= 'Add';
			$data['h3_title']	= 'asset cost';
			$data['form_action']= site_url('c_gl/profit_loss/addAssExp_process');
			//$data['link'] 		= array('link_back' => anchor("$backURL",'<input type="button" name="btnCancel" id="btnCancel" class="btn btn-danger" value="Cancel" />', array('style' => 'text-decoration: none;')));
			$data['backURL'] 	= $backURL;
	...
	function popupallAsset() // U
	{
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data['recCountAsset'] 	= $this->m_profit_loss->count_Asset($PRJCODE);
			$data['vwAsset'] 		= $this->m_profit_loss->viewAsset($PRJCODE)->result();
	...
	function addAssExp_process() // U
	{
		...
		{	
			...
			$RASSET_DATE 	= date('Y-m-d',strtotime($this->input->post('RASSET_PERIOD')));
			...
			$this->m_profit_loss->addAstExp($InpAssetExp);
			...
			if($RASSET_STAT == 3)
			{
				...
				else
				{
					$IConcl	= array('RASTXP_PRJCODE'=> $RASSET_PROJECT,
									...
									'RASTXP_REMP'	=> $GTPrice);
					$this->m_profit_loss->addConAst($IConcl);
				}
			...
			$url			= site_url('c_gl/profit_loss/get_all_asset_exp/?id='.$this->url_encryption_helper->encode_url($PRJCODE));
			redirect($url);
		}
		else
		{
	...
	function updateAstExp() // U
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data['h2_title']				= 'Update';
			$data['h3_title']				= 'stock opname';
			$data['form_action']			= site_url('c_gl/profit_loss/updateAstExp_process');
			$data['MenuCode'] 				= 'MN303';
			
			$data['recCountPrj']			= $this->m_profit_loss->count_all_project($DefEmp_ID);
			$data['vwProject'] 				= $this->m_profit_loss->get_all_project($DefEmp_ID)->result();
			
			$getAssetExp						= $this->m_profit_loss->get_stock_astExp_det($RASSET_CODE)->row();
	...
	function updateAstExp_process() // U
	{
		...
		{	
			...
			$this->m_profit_loss->updAstExp($RASSET_CODE, $UpdAssetExp);
			$this->m_profit_loss->deleteAstDetail($RASSET_CODE);
	...
	function overhead_idx() // U
	{
		...
		$url			= site_url('c_gl/profit_loss/overhead_prjlist/?id='.$this->url_encryption_helper->encode_url($appName));
		redirect($url);
	}
	...
	function get_all_overhead() // U
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data['h3_title'] 	= 'overhead';		
			//$data['link'] 		= array('link_back' => anchor('c_gl/profit_loss/overhead_idx','<input type="button" name="btnCancel" id="btnCancel" class="btn btn-danger" value="Back" />', array('style' => 'text-decoration: none;')));
			$data['backURL'] 	= site_url('c_gl/profit_loss/overhead_idx');
			$data['PRJCODE'] 	= $PRJCODE;
			$data['MenuCode']	= 'MN311';
			
			$num_rows 			= $this->m_profit_loss->count_all_overhead($PRJCODE);
			$data["recCount"] 	= $num_rows;
	 
			$data['vwOverhead']	= $this->m_profit_loss->get_all_overhead($PRJCODE)->result();
	...
	function addOVH() // U
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$backURL			= site_url('c_gl/profit_loss/get_all_overhead/?id='.$this->url_encryption_helper->encode_url($PRJCODE));
			...
			$data['h2_title']	= 'Add';
			$data['h3_title']	= 'overhead';
			$data['form_action']= site_url('c_gl/profit_loss/addOVH_process');
			//$data['link'] 		= array('link_back' => anchor("$backURL",'<input type="button" name="btnCancel" id="btnCancel" class="btn btn-danger" value="Cancel" />', array('style' => 'text-decoration: none;')));
			$data['backURL'] 	= $backURL;
	...
	function addOVH_process() // U
	{
		...
		{	
			...
			$InpOVH 		= array('OVH_CODE' 		=> $OVH_CODE,
									...
									'Patt_Date'		=> $Patt_Date,
									'Patt_Number'	=> $Patt_Number);							
			$this->m_profit_loss->addOVH($InpOVH);
			...
			$url			= site_url('c_gl/profit_loss/get_all_overhead/?id='.$this->url_encryption_helper->encode_url($PRJCODE));
			redirect($url);
		}
		else
		{
	...
	function updateOVH() // U
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data['h2_title']				= 'Update';
			$data['h3_title']				= 'overhead';
			$data['form_action']			= site_url('c_gl/profit_loss/updateOVH_process');
			$data['MenuCode']				= 'MN311';
			
			$data['recCountPrj']			= $this->m_profit_loss->count_all_project($DefEmp_ID);
			$data['vwProject'] 				= $this->m_profit_loss->get_all_project($DefEmp_ID)->result();
			
			$getOverhead					= $this->m_profit_loss->get_ovh_det($OVH_CODE)->row();
	...
	function updateOVH_process() // U
	{
		...
		{	
			...
			$UpdOVH 		= array('OVH_CODE' 		=> $OVH_CODE,
									...
									'Patt_Date'		=> $Patt_Date,
									'Patt_Number'	=> $Patt_Number);
			$this->m_profit_loss->updOVH($OVH_CODE, $UpdOVH);
