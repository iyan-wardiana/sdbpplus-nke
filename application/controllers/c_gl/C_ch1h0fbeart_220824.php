...
class C_ch1h0fbeart extends CI_Controller
{
	...
	function add_process() // G
	{
		...
		// GET PROJECT PARENT FOR PRJ ACITVE
			$SELPRJ_P	= '';
			$sqlPRJ_PC 	= "tbl_project_budg WHERE PRJCODE = '$PRJCODE'";
			$resPRJ_PC 	= $this->db->count_all($sqlPRJ_PC);
			if($resPRJ_PC > 0)
			{
				$sqlPRJ_P 	= "SELECT PRJCODE_HO FROM tbl_project_budg WHERE PRJCODE = '$PRJCODE'";
		...
		// COUNT ALL PROJECT RELATION
			...
			if($Account_Class == 3 || $Account_Class == 4)
			{
				$row		= 0;
				$PRJCODE1A	= '';
				$sqlGetPRJ 	= "SELECT PRJCODE FROM tbl_project_budg WHERE PRJCODE_HO IN ('$PRJSEL')";
				$resGetPRJ 	= $this->db->query($sqlGetPRJ)->result();
				foreach($resGetPRJ as $rowGPRJ) :
					$row	= $row + 1;	
					$SELPRJ = $rowGPRJ->PRJCODE;
					if($row == 1)
					{
						$PRJCODE1A	= $SELPRJ;
					}
					...
					{
						$PRJCODE1A	= "$PRJCODE1A~$SELPRJ";
					}
				endforeach;
				// GABUNGKAN SEMUA HEADER DAN PERIODE
				$PRJCODE1	= "$PRJCODE1~$PRJCODE1A";
			}
		...
		// SAVE TO ALL PROJECT
			...
			foreach($resPRJ as $rowPRJ) :
				...
				// MAX ORD_ID IN COA
					$MAXORDID	= 0;
					//$sqlMAXCOA 	= "SELECT MAX(ORD_ID) AS MAXORDID FROM tbl_chartaccount WHERE Account_Number = '$Acc_DirParent' AND PRJCODE = '$PRJCODE'";
					$sqlMAXCOA 	= "SELECT MAX(ORD_ID) AS MAXORDID FROM tbl_chartaccount WHERE Acc_DirParent = '$Acc_DirParent' AND PRJCODE = '$PRJCODE'";
					...
					$PRJCODE_HO 	= $PRJCODE;
					$s_PRJHO 		= "SELECT PRJCODE_HO FROM tbl_project WHERE PRJCODE = '$PRJCODE' LIMIT 1";
					$r_PRJHO 		= $this->db->query($s_PRJHO)->result();
					foreach($r_PRJHO as $rw_PRJHO) :	
						$PRJCODE_HO = $rw_PRJHO->PRJCODE_HO;	
					endforeach;

					$upd_ordid 	= "UPDATE tbl_chartaccount SET ORD_ID = ORD_ID+1 WHERE ORD_ID > $MAXORDID AND PRJCODE = '$PRJCODE'";
		...
		// SAVE TO ALL PROJECT BUDGET
			/*$sqlGetPRJB	= "SELECT PRJCODE, PRJCODE_HO FROM tbl_project_budg WHERE PRJCODE_HO IN ('$PRJSEL')";
			$resGetPRJB	= $this->db->query($sqlGetPRJB)->result();
			foreach($resGetPRJB as $rowPRJB) :
				$PRJCODE 	= $rowPRJB->PRJCODE;
				$PRJCODEHO	= $rowPRJB->PRJCODE_HO;
				$isHO 		= 0;
				// MAX ORD_ID IN COA
					$MAXORDID	= 0;
					$sqlMAXCOA 	= "SELECT MAX(ORD_ID) AS MAXORDID FROM tbl_chartaccount WHERE Account_Number = '$Acc_DirParent' AND PRJCODE = '$PRJCODE'";
				...
				$coaIns = array('Acc_ID'				=> $Acc_ID,
								'ORD_ID'				=> $MAXORDID,
								'Account_Class'			=> $Account_Class,
								'PRJCODE'				=> $PRJCODE,
								'PRJCODE_HO'			=> $PRJCODEHO,
								'Account_Number'		=> $Account_Number,
								'Account_NameEn'		=> $this->input->post('Account_NameEn'),
								'Account_NameId'		=> $this->input->post('Account_NameId'),
								'Acc_DirParent'			=> $Acc_DirParent,
		...
		// UPDATE ALL CASH BANK DSYNC
			$sqlUPCBD 	= "UPDATE tbl_chartaccount SET syncPRJ = '$PRJCODE1' WHERE Account_Class IN (3,4)";
			$this->db->query($sqlUPCBD);
