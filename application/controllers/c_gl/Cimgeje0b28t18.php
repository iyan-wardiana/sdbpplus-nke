...
/*
	...
	* Create Date	= 09 April 2022
	* File Name		= Cimgeje0b28t18.php
	* Location		= -
*/

class Cimgeje0b28t18 extends CI_Controller  
{
  	function __construct() // GOOD
	{
		parent::__construct();
		
		$this->load->model('m_gl/m_gej_entry/m_imgej_entry', '', TRUE);
 	...
 	function index() // OK
	{
		...
		$url			= site_url('c_gl/cimgeje0b28t18/prjl0b28t18/?id='.$this->url_encryption_helper->encode_url($appName));
		redirect($url);
	}
	...
	function prjl0b28t18() // OK - project list
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data["secURL"] 	= "c_gl/cimgeje0b28t18/gej0b28t18/?id=";
	...
	function gej0b28t18() // OK - gej list
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			// -------------------- START : SEARCHING METHOD --------------------				
				...
				$data["url_search"] = site_url('c_gl/cimgeje0b28t18/f4n7_5rcH/?id='.$this->url_encryption_helper->encode_url($PRJCODE));
			...
			$data['addURL'] 	= site_url('c_gl/cimgeje0b28t18/add0b28t18/?id='.$this->url_encryption_helper->encode_url($COLLDATA));
			$data['backURL'] 	= site_url('c_gl/cimgeje0b28t18/?id='.$this->url_encryption_helper->encode_url($PRJCODE));
			...
			$this->load->view('v_gl/v_imgej_entry/imgej_entry', $data);
		}
		else
		{
			redirect('__l1y');
	...
	// -------------------- START : SEARCHING METHOD --------------------
		function f4n7_5rcH()
		{
			...
			$url			= site_url('c_gl/cimgeje0b28t18/gej0b28t18/?id='.$this->url_encryption_helper->encode_url($collDATA));
			redirect($url);
		}
	// -------------------- END : SEARCHING METHOD --------------------
  	...
  	function get_AllData() // GOOD
	{
		...
		// START : FOR SERVER-SIDE
			...
			$num_rows 		= $this->m_imgej_entry->get_AllDataC($PRJCODE, $search);
			$total			= $num_rows;
			$output			= array();
			$output['draw']	= $draw;
			$output['recordsTotal'] = $output['recordsFiltered']= $total;
			$output['data']	= array();
			$query 			= $this->m_imgej_entry->get_AllDataL($PRJCODE, $search, $length, $start, $order, $dir);
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				$secUpd				= site_url('c_gl/cimgeje0b28t18/up0b28t18/?id='.$this->url_encryption_helper->encode_url($CollCode));
				$secPrint1			= site_url('c_gl/cimgeje0b28t18/prN7_d0c/?id='.$this->url_encryption_helper->encode_url($CollCode));
	...
	function add0b28t18() // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data['title'] 		= $appName;
			$data['task'] 		= 'add';
			$data['form_action']= site_url('c_gl/cimgeje0b28t18/do_upload');
			$data['backURL'] 	= site_url('c_gl/cimgeje0b28t18/gej0b28t18/?id='.$this->url_encryption_helper->encode_url($PRJCODE));
			...
			$MenuCode 			= 'MN028';
			$data["MenuCode"] 	= 'MN028';
			$data['vwDocPatt'] 	= $this->m_imgej_entry->getDataDocPat($MenuCode)->result();
	...
	function puSA0b28t18() // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data['form_action']	= site_url('c_gl/cimgeje0b28t18/update_process');
			$data['PRJCODE'] 		= $PRJCODE;
			$data['THEROW'] 		= $THEROW;
			$data['secShowAll']		= site_url('c_gl/cimgeje0b28t18/puSA0b28t18/?id='.$this->url_encryption_helper->encode_url($PRJCODE));
			
			$data['countAllCOA']	= $this->m_imgej_entry->count_all_COA($PRJCODEX, $DefEmp_ID);
			$data['vwAllCOA'] 		= $this->m_imgej_entry->view_all_COA($PRJCODEX, $DefEmp_ID)->result();
			
			$data['countAllItem']	= $this->m_imgej_entry->count_all_Account($PRJCODEX, $PRJPERIOD);
			$data['vwAllItem'] 		= $this->m_imgej_entry->view_all_Account($PRJCODEX, $PRJPERIOD)->result();
					
			$this->load->view('v_gl/v_imgej_entry/imgej_entry_selacc', $data);
		}
		else
		{
			redirect('__l1y');
	...
	function add_process() // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$projGEJH 		= array('JournalH_Code' 	=> $JournalH_Code,
									...
									'SPLDESC'			=> $SPLDESC,
									'GEJ_STAT'			=> $GEJ_STAT);
			$this->m_imgej_entry->add($projGEJH);
			...
			// START : DETAIL JOURNAL
				...
				foreach($_POST['data'] as $d)
				{
					...
					$insSQL	= "INSERT INTO tbl_journaldetail_imp (JournalH_Code, JournalType, Acc_Id, proj_Code, proj_CodeHO, PRJPERIOD, JOBCODEID, Currency_id,
			...
			// UPDATE AMOUNT JOURNAL HEADER
				$sqlUpdJH	= "UPDATE tbl_journalheader_imp SET Journal_Amount = $Base_DebetTOT
			...
			// START : UPDATE TO TRANS-COUNT
				...
				$parameters 	= array('DOC_CODE' 		=> $JournalH_Code,	// TRANSACTION CODE
										...
										'TBL_NAME' 		=> "tbl_journalheader_imp",	// TABLE NAME
			...
			$url			= site_url('c_gl/cimgeje0b28t18/gej0b28t18/?id='.$this->url_encryption_helper->encode_url($PRJCODE));
			redirect($url);
		}
		else
		{
	...
	function up0b28t18() // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$getGEJ 		= $this->m_imgej_entry->get_GEJ_by_number($JournalH_Code)->row();
			...
			$data['default']['REF_NUM'] 		= $getGEJ->Reference_Number;
	...
	function update_process() // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$projGEJH 		= array('JournalH_Code' 	=> $JournalH_Code,
									...
									'JournalType' 		=> 'GEJ',	// Cash Project
									'JournalH_Desc'		=> addslashes($this->input->post('JournalH_Desc')),
									'JournalH_Desc2'	=> addslashes($this->input->post('JournalH_Desc2')),
									'JournalH_Date'		=> $JournalH_Date,
									'Company_ID'		=> $comp_init,
									'Reference_Type'	=> 'GEJ',
									'Emp_ID'			=> $DefEmp_ID,
									...
									'SPLDESC'			=> $SPLDESC,
									'GEJ_STAT'			=> $GEJ_STAT);
			$this->m_imgej_entry->updateGEJ($JournalH_Code, $projGEJH);
			...
			if($GEJ_STAT == 3)
			{
				// UPDATE DEFAULT STATUS
					$upJH		= "UPDATE tbl_journalheader_imp SET GEJ_STAT = 7 WHERE JournalH_Code = '$JournalH_Code'";
					$this->db->query($upJH);
				...
				if($AH_ISLAST == 1)
				{
					$upJH2	= "UPDATE tbl_journalheader_imp SET GEJ_STAT = '$GEJ_STAT' WHERE JournalH_Code = '$JournalH_Code'";
					$this->db->query($upJH2);
				...
				// RESET DETAIL
					$this->m_imgej_entry->deleteGEJDetail($JournalH_Code);
				...
				// START : DETAIL JOURNAL
					...
					foreach($_POST['data'] as $d)
					{
								...
								if($ITM_CODE != '')
								{
									...
									// START : ITEM HISTORY
										$sqlHist 		= "INSERT INTO tbl_itemhistory (JournalH_Code, proj_Code, Transaction_Date, Item_Code, Qty_Plus, Qty_Min, 
																QtyRR_Plus, QtyRR_Min, Transaction_Type, Transaction_Value, Company_ID, Currency_ID,
																JOBCODEID, GEJ_STAT, ItemPrice, ItemCategoryType, MEMO)
															VALUES ('$JournalH_Code', '$PRJCODE', '$JournalH_Date', '$ITM_CODE', 0, $Base_Kredit, 
																0, 0, 'GEJ', $Base_Debet, '$comp_init', 'IDR', 
																'$JOBCODEID', 3, '$Base_Debet', '$ITM_CATEG', '$AH_NOTES')";
										$this->db->query($sqlHist);
									// END : ITEM HISTORY
						...
						if($Ref_Number != '' && $JournalD_Pos == 'K')
						{
							...
							$this->db->where(["TLK_NUM" => $Ref_Number, "PRJCODE" => $PRJCODE, "TLK_STATUS" => 3]);
							...
							if($getREFNUM->num_rows() > 0)
							{
								...
								$sqlUpdTLK	= "UPDATE tbl_tsflk SET TLK_REALIZ = $TTLK_REALIZ, TLK_USTATUS = $TLK_USTATUS
										WHERE TLK_NUM = '$Ref_Number' AND PRJCODE = '$PRJCODE' AND TLK_STATUS = 3";
			...
			{
				// UPDATE DEFAULT STATUS
					$upJH		= "UPDATE tbl_journalheader_imp SET GEJ_STAT = '$GEJ_STAT' WHERE JournalH_Code = '$JournalH_Code'";
					$this->db->query($upJH);
			...
			// START : UPDATE AKUN LAWAN
				$Acc_Cr 		= "";
				$sqlACC_Cr 		= "SELECT Acc_Id FROM tbl_journaldetail_imp WHERE proj_Code = '$PRJCODE' AND Journal_DK = 'K' AND JournalH_Code = '$JournalH_Code'";
				$resACC_Cr		= $this->db->query($sqlACC_Cr)->result();
				foreach($resACC_Cr as $row_Cr):
					$Acc_Cr		= $row_Cr->Acc_Id;
				endforeach;

				$updAcc_Db		= "UPDATE tbl_journaldetail_imp SET Acc_Id_Cross = '$Acc_Cr' WHERE proj_Code = '$PRJCODE' AND JournalH_Code = '$JournalH_Code'";
			...
			$url			= site_url('c_gl/cimgeje0b28t18/gej0b28t18/?id='.$this->url_encryption_helper->encode_url($proj_Code));
			redirect($url);
		}
		else
		{
  	...
  	function get_AllDataITMS() // GOOD
	{
		...
		// START : FOR SERVER-SIDE
			...
			$start			= $_REQUEST['start'];
			$search			= $_REQUEST['search']["value"];
			$num_rows 		= $this->m_imgej_entry->get_AllDataITMSC($PRJCODE, $search);
			$total			= $num_rows;
			$output			= array();
			$output['draw']	= $draw;
			$output['recordsTotal'] = $output['recordsFiltered']= $total;
			...
			$query 			= $this->m_imgej_entry->get_AllDataITMSL($PRJCODE, $search, $length, $start, $order, $dir);
			...
			foreach ($query->result_array() as $dataI) 
			{
                ...
                $JOBDESCPR      = "";
                $sqlJOBPAR      = "SELECT JOBDESC FROM tbl_joblist_detail WHERE JOBCODEID = '$JOBPARENT'";
  	...
  	function get_AllDataCOA() // GOOD
	{
		...
		// START : FOR SERVER-SIDE
			...
			$start			= $_REQUEST['start'];
			$search			= $_REQUEST['search']["value"];
			$num_rows 		= $this->m_imgej_entry->get_AllDataCOAC($PRJCODEX, $DefEmp_ID, $search);
			$total			= $num_rows;
			$output			= array();
			$output['draw']	= $draw;
			$output['recordsTotal'] = $output['recordsFiltered']= $total;
			...
			$query 			= $this->m_imgej_entry->get_AllDataCOAL($PRJCODEX, $DefEmp_ID, $search, $length, $start, $order, $dir);
	...
	function get_AllDataTLK()
	{
		...
		// START : FOR SERVER-SIDE
			...
			$endord			= $start + $length;
			$search			= $_REQUEST['search']["value"];
			$num_rows 		= $this->m_imgej_entry->get_AllData_VTLKC($PRJCODE, $search);
			$total			= $num_rows;
			$output			= array();
			$output['draw']	= $draw;
			$output['recordsTotal'] = $output['recordsFiltered']= $total;
			$output['data']	= array();
			$query 			= $this->m_imgej_entry->get_AllData_VTLKL($PRJCODE, $search, $length, $start, $order, $dir);
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				$this->db->from("tbl_journalheader_imp A");
				$this->db->join("tbl_journaldetail_imp B", "B.JournalH_Code = A.JournalH_Code AND B.proj_Code = A.proj_Code", "INNER");
	...
	function iNdJrnl_0x() // GEJ
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data["MenuCode"] 	= 'MN107';
			$data['viewPRJ'] 	= $this->m_projectlist->get_all_project($DefEmp_ID)->result();			
			$data["secURL"] 	= "c_gl/cimgeje0b28t18/iNdJrnl_zx/?id=";
	...
	function iNdJrnl_zx() // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$SOURCEDOC			= "";
			$COLLDATA			= "$PRJCODE~$SOURCEDOC";
			/*$data['addURL'] 	= site_url('c_gl/cimgeje0b28t18/iNdJrnl_0x/?id='.$this->url_encryption_helper->encode_url($COLLDATA));*/
			$data['backURL'] 	= site_url('c_gl/cimgeje0b28t18/'.$BackFunct.'/?id='.$this->url_encryption_helper->encode_url($PRJCODE));
			$data['PRJCODE'] 	= $PRJCODE;
  	...
  	function get_AllDataJRN() // OK
	{
		...
		// START : FOR SERVER-SIDE
			...
			$start			= $_REQUEST['start'];
			$search			= $_REQUEST['search']["value"];
			$num_rows 		= $this->m_imgej->get_AllDataC($PRJCODE, $search, $jrnCat);
			$total			= $num_rows;
			$output			= array();
			$output['draw']	= $draw;
			$output['recordsTotal'] = $output['recordsFiltered']= $total;
			$output['data']	= array();
			$query 			= $this->m_imgej->get_AllDataL($PRJCODE, $search, $length, $start, $order, $dir, $jrnCat);
			...
			foreach ($query->result_array() as $dataI)
			{
				...
				/*$sqlJD 		= "SELECT A.JournalH_Code, A.Other_Desc, A.Acc_Id, A.Acc_Name, A.JournalD_Debet, A.JournalD_Kredit,
								...
								FROM tbl_journaldetail_imp A
									INNER JOIN tbl_journalheader_imp B ON B.JournalH_Code = A.JournalH_Code
	...
	function getManualNo()
	{
		...
		$this->db->limit(1);
		$this->db->from("tbl_journaldetail_imp A");
		$this->db->join("tbl_journalheader_imp B", "B.JournalH_Code = A.JournalH_Code AND B.proj_Code =  A.proj_Code", "INNER");
	...
	function do_upload() // G
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$target_path = "application/xlsxfile/import_trx/".$filename;  // change this to the correct site path
				
			$myPath 	= "application/xlsxfile/import_trx/$filename";
			...
			if($task == 'add')
			{
				$projGEJH 	= array('JournalH_Code' 	=> $JournalH_Code,
									...
									'JournalType' 		=> 'IMP-TRX',
									'JournalH_Desc'		=> addslashes($this->input->post('JournalH_Desc')),
									'JournalH_Date'		=> date('Y-m-d'),
									'Company_ID'		=> $comp_init,
									'Reference_Type'	=> 'IMP-TRX',
									'Emp_ID'			=> $DefEmp_ID,
			...
			$data['JournalH_Code'] 	= $JournalH_Code;
  	...
  	function get_AllDataDetil() // GOOD
	{
		$this->load->model('m_gl/m_coa/m_coa', '', TRUE);

		$JournalHCode = $_GET['id'];
		...
		// START : FOR SERVER-SIDE
			...
			$start			= $_REQUEST['start'];
			$search			= $_REQUEST['search']["value"];
			$num_rows 		= $this->m_imgej_entry->get_AllDataDetC($JournalHCode, $search);
			$total			= $num_rows;
			$output			= array();
			$output['draw']	= $draw;
			$output['recordsTotal'] = $output['recordsFiltered']= $total;
			$output['data']	= array();
			$query 			= $this->m_imgej_entry->get_AllDataDetL($JournalHCode, $search, $length, $start, $order, $dir);
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				if($r_PRJC == 0)
				{
					...
					$ItmCol2	= '</span>';
					$noPRJ 		= "- Kode proyek $proj_Code belum terdapat di sistem SdBP+ <br>";
			...
			$s_00		= "SELECT count(*) AS TOTR, SUM(Base_Debet) AS GTOT_D, SUM(Base_Kredit) AS GTOT_K
							FROM tbl_journaldetail_imp WHERE JournalH_Code = '$JournalHCode'";
