...
class C_ir180c15  extends CI_Controller
{
		...
		// START : FOR SERVER-SIDE
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				if($IR_STAT == 1 || $IR_STAT == 4)
				{
					...
					$disAble 	= "";
					$hiDden 	= "";
					$btnCpyDel 	= "<a onClick='deleteRow(".$noU.")' title='Delete' class='btn btn-danger btn-xs' style='".$hiDden."'><i class='fa fa-trash-o'></i></a><a onClick='copyRow(".$noU.")' title='Copy' class='btn btn-warning btn-xs' style='".$hiDden."'><i class='glyphicon glyphicon-copy'></i></a>";
				...
				$output['data'][] 	= array(
											"<label style='white-space:nowrap'>
										  	...
										  	"$textSJ",
										  	"$ITMQTY
										  		<input type='text' style='text-align:right' name='data[".$noU."][ITM_QTY]' id='ITM_QTY".$noU."' size='10' value='".$ITM_QTY."' >
												<input type='hidden' style='text-align:right' id='REMAINQTY".$noU."' size='10' value='".$REMAINQTY."' >
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				$output['data'][] 	= array("<label style='white-space:nowrap'>
										  	...
										  	"$ITMQTY
										  		<input type='text' style='text-align:right' name='data[".$noU."][ITM_QTY]' id='ITM_QTY".$noU."' size='10' value='".$ITM_QTY."' >
												<input type='hidden' style='text-align:right' id='REMAINQTY".$noU."' size='10' value='".$REMAINQTY."' >
												<input type='text' style='text-align:right' name='data[".$noU."][ITM_QTY_REM]' id='ITM_QTY_REM".$noU."' size='10' value='".$ITM_QTY_REM."' >",
										  	"$ITMUNIT
										  		<input type='hidden' class='form-control' style='max-width:350px; text-align:right' name='data[".$noU."][ITM_UNIT]' id='ITM_UNIT".$noU."' size='10' value='".$ITM_UNIT."' >
	                                            <input type='hidden' class='form-control' style='max-width:350px; text-align:right' name='data[".$noU."][ITM_GROUP]' id='ITM_GROUP".$noU."' size='10' value='".$ITM_GROUP."' >
	                                            <input type='hidden' class='form-control' style='max-width:350px; text-align:right' id='ITM_CATEG".$noU."' size='10' value='".$ITM_CATEG."' >
	...
	function copyROW()
	{
		...
		if($resJDP == 0)
		{
    		$sql_01 	= 	"INSERT INTO tbl_ir_detail_tmp
							...
							($IR_ID, '$PRJCODE', '$DEPCODE', '$IR_NUM', '$IR_CODE', '$ACC_ID', 3, '$PR_NUM', '$PO_NUM', '$JOBCODEDET', '$JOBCODEID',
								'$JOBPARENT', '$JOBPARDESC', '$POD_ID', '$ITM_CODE', '$ITM_NAME', '$ITM_UNIT', '$ITM_TYPE', '$ITM_GROUP',
								'$ITM_QTY_REM2', '$ITM_QTY_REM2', '$ITM_PRICE', '$ITM_TOTAL', '$ITM_DISP', '$ITM_DISC', '$TAXCODE1', '$TAXCODE2',
								'$TAXPRICE1', '$TAXPRICE2', '$IR_AMOUNTNET', '$ITM_DESC')";
			$this->db->query($sql_01);
		}
	...
	function getPO()
	{
		...
		$SPLCODE	= '';
		$TERM_PAY	= 0;
		$s_01		= "SELECT PO_CODE, PR_NUM, SPLCODE, PO_TENOR FROM tbl_po_header WHERE PO_NUM = '$PO_NUM'";
		$r_01 	= $this->db->query($s_01)->result();
		foreach($r_01 as $rw_01):
		...
		endforeach;
		echo "$PO_CODE~$PR_NUM~$SPLCODE~$TERM_PAY";
	}
	...
	function updSJ()
	{
		...
		$s_02		= "UPDATE tbl_ir_detail_tmp SET SJ_NUM = '$SJ_NUM', ITM_QTY_REM = $QTYREM, ITM_QTY = '$IR_VOL'
						WHERE IR_NUM = '$IR_NUM' AND IR_ID = $IR_ID";
		$this->db->query($s_02);

		$s_02a		= "UPDATE tbl_ir_detail SET SJ_NUM = '$SJ_NUM', ITM_QTY_REM = $QTYREM, ITM_QTY = '$IR_VOL'
						WHERE IR_NUM = '$IR_NUM' AND IR_ID = $IR_ID";
		$this->db->query($s_02a);

		$s_03		= "UPDATE tbl_ir_detail_tmp SET SJ_NUM = '$SJ_NUM', ITM_QTY_REM = $QTYREM, ITM_QTY = '$IR_VOL'
						WHERE IR_NUM = '$IR_NUM' AND IR_ID = $IR_ID";
		$this->db->query($s_03);

		$s_03a		= "UPDATE tbl_ir_detail SET SJ_NUM = '$SJ_NUM', ITM_QTY_REM = $QTYREM, ITM_QTY = '$IR_VOL'
						WHERE IR_NUM = '$IR_NUM' AND IR_ID = $IR_ID";
		$this->db->query($s_03a);
	...
	function updVOL()
	{
		...
		// GET TOTAL VOLUME
			...
			$s_01 	= "SELECT SUM(ITM_QTY) AS TOT_VOL FROM tbl_ir_detail_tmp
						WHERE ITM_CODE = '$ITM_CODE' AND IR_NUM = '$IR_NUM'";
			$r_01 	= $this->db->query($s_01)->result();
			foreach($s_01 as $rw_01):
				$TOT_VOL = $rw_01->TOT_VOL;
			endforeach;
			$QTYREM = $ITM_REM - $TOT_VOL;

		$s_01		= "UPDATE tbl_ir_detail SET SJ_NUM = '$SJ_NUM', ITM_QTY_REM = $QTYREM, ITM_QTY = '$IR_VOL'
						WHERE IR_NUM = '$IR_NUM' AND IR_ID = $IR_ID";
		$this->db->query($s_01);

		$s_02		= "UPDATE tbl_ir_detail SET SJ_NUM = '$SJ_NUM', ITM_QTY_REM = $QTYREM, ITM_QTY = '$IR_VOL'
						WHERE IR_NUM = '$IR_NUM' AND IR_ID = $IR_ID";
		$this->db->query($s_02);
	}
}