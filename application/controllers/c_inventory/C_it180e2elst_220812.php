  	...
  	function get_AllData() // GOOD
	{
		...
		// START : FOR SERVER-SIDE
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				$ITM_NAMEV 		= addslashes($ITM_NAME2);
				...
				$sqlAccIN 		= "SELECT Account_NameId FROM tbl_chartaccount WHERE Account_Number = '$ACC_ID' LIMIT 1";
				$resAccIN		= $this->db->query($sqlAccIN)->result(); 
				foreach($resAccIN as $rowAccIN):
					$Nm_IN		= $rowAccIN->Account_NameId;
				endforeach;
				
				$sqlAccOUT 		= "SELECT Account_NameId FROM tbl_chartaccount WHERE Account_Number = '$ACC_ID_UM' LIMIT 1";
				$resAccOUT		= $this->db->query($sqlAccOUT)->result();
				foreach($resAccOUT as $rowAccOUT):
					$Nm_OUT		= $rowAccOUT->Account_NameId;
				endforeach;
				
				$sqlAccSAL 		= "SELECT Account_NameId FROM tbl_chartaccount WHERE Account_Number = '$ACC_ID_SAL' LIMIT 1";
