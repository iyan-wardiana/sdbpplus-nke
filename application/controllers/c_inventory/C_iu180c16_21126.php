	...
	class C_iu180c16  extends CI_Controller
	{
	  	...
	  	function get_AllDataSRV() // GOOD
		{
			...
			// START : FOR SERVER-SIDE
				...
				$start			= $_REQUEST['start'];
				$search			= $_REQUEST['search']["value"];
				$num_rows 		= $this->m_itemusage->get_AllDataSRVC($PRJCODE, $search);
				$total			= $num_rows;
				$output			= array();
				$output['draw']	= $draw;
				$output['recordsTotal'] = $output['recordsFiltered']= $total;
				...
				$query 			= $this->m_itemusage->get_AllDataSRVL($PRJCODE, $search, $length, $start, $order, $dir);
				...
				foreach ($query->result_array() as $dataI) 
				{
					...
					$ITM_PRICE1		= $dataI['ITM_PRICE'];				// 8
					$ADD_PRICE 		= $dataI['ADD_PRICE'];
					$ITM_PRICE 		= $ITM_PRICE1;						// 8
					$ITM_VOLM 		= $dataI['ITM_VOLM'];				// 9
					$ITM_BUDG 		= $dataI['ITM_BUDG'];				// 9
					...
					$WO_QTY			= $dataI['WO_QTY'];					// 18
					$WO_AMOUNT		= $dataI['WO_AMOUNT'];				// 19
					$OPN_QTY		= $dataI['OPN_QTY'];				// 20
					$OPN_AMOUNT		= $dataI['OPN_AMOUNT'];				// 21
					$ISLAST			= $dataI['ISLAST'];
					...
					// CEK APAKAH PEKERJAAN MEMILIKI TURUNAN DENGAN ISLAST = 1
						$getLAST	= "tbl_joblist_detail WHERE PRJCODE = '$PRJCODE' AND JOBPARENT = '$JOBCODEID' AND ISLAST = 1";
						$resLAST	= $this->db->count_all($getLAST);

						if($resLAST == 0)
							$disabledB	= 1;

					// IS LAST SETT
						if($disabledB == 0)	// HEADER
						{
							$JOBDESC1 	= $JOBDESC;
							$STATCOL	= 'primary';
					...
					// OTHER SETT
						if($disabledB == 0)
						{
							$chkBox	= "<input type='radio' name='chk' value='".$JOBCODEID."|".$PRJCODE."|".$JOBDESC."' onClick='pickThis(this);'/>";
						}
						else
						{
					...
					$output['data'][] 	= array($chkBox,
												...
												/*"<div style='text-align:right;'><span ".$CELL_COL.">".number_format($ITM_BUDG, 2)."</span></div>",
												...
												"<div style='text-align:right;'><span ".$CELL_COL.">".number_format($ITM_STOCK, 2)."</span></div>"*/
											);
