...
class C_it180e2elst  extends CI_Controller  
{
  	...
  	function get_AllData() // GOOD
	{
		...
		// START : FOR SERVER-SIDE
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				$sqlAccIN 		= "SELECT Account_NameId FROM tbl_chartaccount WHERE Account_Number = '$ACC_ID'";
				$resAccIN		= $this->db->query($sqlAccIN)->result();
				foreach($resAccIN as $rowAccIN):
					$Nm_IN		= $rowAccIN->Account_NameId;
				endforeach;
				
				$sqlAccOUT 		= "SELECT Account_NameId FROM tbl_chartaccount WHERE Account_Number = '$ACC_ID_UM'";
				$resAccOUT		= $this->db->query($sqlAccOUT)->result();
				foreach($resAccOUT as $rowAccOUT):
					$Nm_OUT		= $rowAccOUT->Account_NameId;
				endforeach;
				
				$sqlAccSAL 		= "SELECT Account_NameId FROM tbl_chartaccount WHERE Account_Number = '$ACC_ID_SAL'";
				$resAccSAL		= $this->db->query($sqlAccSAL)->result();
				foreach($resAccSAL as $rowAccSAL):
					$Nm_SAL		= $rowAccSAL->Account_NameId;
				endforeach;

				$ACC_IN			= "<span class='btn btn-success btn-xs' style='font-size:12px' title='".$Nm_IN."'><i class='glyphicon glyphicon-import'></i></span>";
				$ACC_OUT		= "<span class='btn btn-success btn-xs' style='font-size:12px' title='".$Nm_OUT."'><i class='glyphicon glyphicon-export'></i></span>";
				$ACC_SAL		= "<span class='btn btn-success btn-xs' style='font-size:12px' title='".$Nm_SAL."'><i class='fa fa-opencart'></i></span>";

				if($ACC_ID == '')
					$ACC_IN		= "<span class='btn btn-danger btn-xs' style='font-size:12px'><i class='glyphicon glyphicon-import'></i></span>";

				if($ACC_ID_UM == '')
					$ACC_OUT	= "<span class='btn btn-danger btn-xs' style='font-size:12px'><i class='glyphicon glyphicon-export'></i></span>";

				if($ACC_ID_SAL == '')
					$ACC_SAL	= "<span class='btn btn-danger btn-xs' style='font-size:12px'><i class='fa fa-opencart'></i></span>";

				if($ACC_ID != '' && $Nm_IN == '')
					$ACC_IN		= "<span class='btn btn-warning btn-xs' style='font-size:12px'><i class='glyphicon glyphicon-import'></i></span>";

				if($ACC_ID_UM != '' && $Nm_OUT == '')
					$ACC_OUT	= "<span class='btn btn-warning btn-xs' style='font-size:12px'><i class='glyphicon glyphicon-export'></i></span>";

				if($ACC_ID_SAL != '' && $Nm_SAL == '')
					$ACC_SAL	= "<span class='btn btn-warning btn-xs' style='font-size:12px'><i class='fa fa-opencart'></i></span>";
		...
		{	
			...
			foreach($resPRJ as $rowPRJ):
				...
				$itemPar 	= array('PRJCODE' 		=> $PRJCODE,
									...
									'ITM_CATEG'		=> $this->input->post('ITM_CATEG'),
									'ITM_NAME'		=> addslashes($this->input->post('ITM_NAME')),
									'ITM_DESC'		=> addslashes($this->input->post('ITM_DESC')),
									'ITM_TYPE'		=> $this->input->post('ITM_TYPE'),
									'ITM_UNIT'		=> addslashes($this->input->post('ITM_UNIT')),
									'UMCODE'		=> addslashes($this->input->post('ITM_UNIT')),
									'ITM_CURRENCY'	=> addslashes($this->input->post('ITM_CURRENCY')),
									'ITM_PRICE'		=> $this->input->post('ITM_PRICE'),
	...
	function update_process()
	{
		...
		{	
			...
			/*$upWBSD	= "UPDATE tbl_joblist_detail SET ITM_GROUP = '$ITM_GROUP', ITM_UNIT = '$ITM_UNIT'
