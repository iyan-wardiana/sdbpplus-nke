...
class C_ir180c15  extends CI_Controller
{
		...
		// START : FOR SERVER-SIDE
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				$output['data'][] = array("$noU.",
										  "<label style='white-space:nowrap'>$isLockD".$dataI['IR_CODE']."</div>",
										  $IR_DATEV,
										  $SPLDESC,
										  $dataI['IR_NOTE'],
  	...
  	function get_AllDataGRP() // GOOD
	{
		...
		// START : FOR SERVER-SIDE
			...
			$start			= $_REQUEST['start'];
			$search			= $_REQUEST['search']["value"];
			$num_rows 		= $this->m_itemreceipt->get_AllDataGRPC($PRJCODE, $SPLCODE, $PO_NUM, $IR_STAT, $IR_CATEG, $search);
			$total			= $num_rows;
			$output			= array();
			$output['draw']	= $draw;
			$output['recordsTotal'] = $output['recordsFiltered']= $total;
			$output['data']	= array();
			$query 			= $this->m_itemreceipt->get_AllDataGRPL($PRJCODE, $SPLCODE, $PO_NUM, $IR_STAT, $IR_CATEG, $search, $length, $start, $order, $dir);
	...
	function copyROW()
	{
        ...
        $s_01		= "SELECT A.PRJCODE, A.IR_ID, A.DEPCODE, A.IR_NUM, A.IR_CODE, A.JOBCODEDET, A.JOBCODEID,
							A.ACC_ID, A.PR_NUM, A.PO_NUM, A.PO_CODE, A.ITM_CODE, A.ITM_UNIT, A.ITM_UNIT2,
		...
		if($resJDP == 0)
		{
    		$sql_03 	= 	"INSERT INTO tbl_ir_detail_tmp
							...
							($IR_ID, '$PRJCODE', '$DEPCODE', '$IR_NUM', '$IR_CODE', '$ACC_ID', 3, '$PR_NUM', '$PO_NUM', '$PO_CODE', '$JOBCODEDET', '$JOBCODEID',
								...
								'$TAXPRICE1', '$TAXPRICE2', '$ITM_TOTAL', '$ITM_DESC')";
