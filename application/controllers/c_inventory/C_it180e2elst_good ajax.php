...
class C_it180e2elst  extends CI_Controller  
{
 	function index() // OK
	{
		...
		$url			= site_url('c_inventory/c_it180e2elst/projectlist/?id='.$this->url_encryption_helper->encode_url($appName));
		redirect($url);
	}
	...
	function projectlist() // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data['viewPRJ'] 	= $this->m_projectlist->get_all_project($DefEmp_ID)->result();			
			$data["secURL"] 	= "c_inventory/c_it180e2elst/get_it180e2elst_lti/?id=";
	...
	function get_it180e2elst_ltixx() // OK
	{
		$this->load->model('m_inventory/m_itemlist/m_itemlist', '', TRUE);
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data['backURL']		= site_url('c_inventory/c_it180e2elst/projectlist/?id='.$this->url_encryption_helper->encode_url($appName));
	...
	function get_it180e2elst_lti() // OK
	{
		$this->load->model('m_inventory/m_itemlist/m_itemlist', '', TRUE);
  	...
  	function get_data_item()
	{
		...
		// try 01
			/*$list = $this->m_itemlist->get_datatables();
			$data = array();
			$no = $_POST['start'];
			...
			$output = array(
				"draw" => $_POST['draw'],
				"recordsTotal" => $this->m_itemlist->count_all(),
				"recordsFiltered" => $this->m_itemlist->count_filtered(),
				"data" => $data,
			);
		...
		// try 02
			...
			$total			= $this->db->count_all_results("tbl_item");
			$output			= array();
			$output['draw']	= $draw;
			...
			if($search!="")
			{
				$this->db->like("ITM_NAME", $search);
			}
	
	
			/*Lanjutkan pencarian ke database*/
			$this->db->limit($length,$start);
			$this->db->order_by('ITM_NAME','DESC');
			...
			*/
			...
			{
				$output['data'][]=array($nomor_urut,$desa['ITM_NAME']);
				$nomor_urut++;
			}
	...
	function it180e2elst_upl() // OK
	{
		...
		{		
			...
			$backURL	= site_url('c_inventory/c_it180e2elst/get_it180e2elst_lti/?id='.$this->url_encryption_helper->encode_url($PRJCODE));
	...
	function do_upload() // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$backURL				= site_url('c_inventory/c_it180e2elst/get_it180e2elst_lti/?id='.$this->url_encryption_helper->encode_url($ITMH_PRJCODE));
			$data['isProcess'] 		= 1;
	...
	function add_process() // OK
	{
		...
		{	
			...
			$url			= site_url('c_inventory/c_it180e2elst/get_it180e2elst_lti/?id='.$this->url_encryption_helper->encode_url($PRJCODE));
			redirect($url);
		}
		else
		{
    ...
    function inbox($offset=0)
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data["recordcount"] = $num_rows;
			$config = array();
			$config['base_url'] = site_url('c_inventory/c_it180e2elst/get_it180e2elst_lti');
