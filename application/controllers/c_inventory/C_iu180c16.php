	...
	class C_iu180c16  extends CI_Controller
	{
				...
				foreach ($query->result_array() as $dataI) 
				{
					...
					// GET isLock in Journal
						...
						$getJ 	= "SELECT IFNULL(isLock,0) AS isLock FROM tbl_journalheader_$PRJCODEVW 
									WHERE GEJ_STAT = 3 AND JournalH_Code = '$UM_NUM'";
					...
					$delID 		= "$secDelIcut~tbl_um_header~tbl_um_detail~UM_NUM~$UM_NUM~PRJCODE~$PRJCODE";
					$secVoid 	= base_url().'index.php/__l1y/voidDoc_CJRN/?id=';
					$voidID 	= "$secVoid~tbl_um_header~tbl_um_detail~UM_NUM~$UM_NUM~PRJCODE~$PRJCODE";
					...
					elseif($UM_STAT == 3)
					{
						$secAction	= 	"<input type='hidden' name='urlPrint".$noU."' id='urlPrint".$noU."' value='".$secPrint."'>
										...
										<a href='javascript:void(null);' class='btn btn-danger btn-xs' onClick='deleteDOC(".$noU.")' disabled='disabled'>
											<i class='fa fa-trash-o'></i>
										</a>
										</label>";
					}                                      
		...
		function add_process() // G
		{
			...
			if ($this->session->userdata('login') == TRUE)
			{
				...
				$PRJDEST		= $this->input->post('PRJDEST');
				$UM_NOREF		= $this->input->post('UM_NOREF');
				$UM_SPLNOTES	= htmlspecialchars($this->input->post('UM_SPLNOTES'), ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401 | ENT_HTML5);
				// $UM_DATE		= date('Y-m-d',strtotime($this->input->post('UM_DATE')));
				$UM_DATE		= date('Y-m-d',strtotime(str_replace('/', '-', $this->input->post('UM_DATE'))));
				...
				$UM_NOTE		= htmlspecialchars($this->input->post('UM_NOTE'), ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401 | ENT_HTML5);
			...
			// END : MEMBUAT LIST NUMBER / tbl_doclist
				...
				foreach($_POST['data'] as $d)
				{
					...
					// $ITM_PRICE 		= $this->m_updash->get_itmAVG($PRJCODE, $ITM_CODE); // sudah dihitung saat di view form update tgl. 28-Agust-2023
				...
				// START : ADD DOC HISTORY
					$this->load->model('m_updash/m_updash', '', TRUE);
					date_default_timezone_set("Asia/Jakarta");
					$paramTrack 	= array('REF_NUM' 		=> $UM_NUM,
											'TBLNAME' 		=> "tbl_um_header",
											'FLDCODE'		=> "UM_CODE",
											'FLDNAME'		=> "UM_NUM",
											'HISTTYPE'		=> "Penambahan ($UM_STAT)");
		...
		function update() // G
		{
			...
			if ($this->session->userdata('login') == TRUE)
			{
				...
				$data['default']['UM_CATEG']	= $getrr->UM_CATEG;
				...
				$data['default']['PRJDEST'] 	= $getrr->PRJDEST;
		...
		function update_inbox_process() // G
		{
			...
			if ($this->session->userdata('login') == TRUE)
			{
				...
				$UM_STAT		= $this->input->post('UM_STAT'); // 1 = New, 2 = Awaiting, 3 = Approve, 4 = Revise, 5 = Reject
				$UM_NOTE		= htmlspecialchars($this->input->post('UM_NOTE'), ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401 | ENT_HTML5);
				$UM_NOTE2		= htmlspecialchars($this->input->post('UM_NOTE2'), ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401 | ENT_HTML5);
				$WH_CODE		= $this->input->post('WH_CODE');
				...
				// START : SAVE APPROVE HISTORY
					...
					$AH_APPROVED	= $APPDATE;
					$AH_NOTES		= htmlspecialchars($this->input->post('UM_NOTE2'), ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401 | ENT_HTML5);
					...
					if($UM_STAT == 3)
					{
						...
						// START : JOURNAL DETAIL
							foreach($_POST['data'] as $d)
							{
								...
								$ITM_CATEG 		= "";
								$ITM_LR 		= "";
								$sqlL_DK		= "SELECT ACC_ID, ACC_ID_UM, ITM_GROUP, ITM_LR FROM tbl_item WHERE PRJCODE = '$PRJCODE' AND ITM_CODE = '$ITM_CODE'";
								$resL_DK 		= $this->db->query($sqlL_DK)->result();					
								foreach($resL_DK as $rowL_DK):
									$ACC_ID_IR	= $rowL_DK->ACC_ID;
									$ACC_ID_UM	= $rowL_DK->ACC_ID_UM;
									$ITM_CATEG	= $rowL_DK->ITM_GROUP;
									$ITM_LR		= $rowL_DK->ITM_LR;
								endforeach;
								...
								if($UM_TYPE == 1)
								{
									$parameters = array('JournalH_Code' 	=> $JournalH_Code,
														...
														'PRJCODE'			=> $PRJCODE,
														'JSource'			=> $JSource,
														'TRANS_CATEG' 		=> 'UM',	// UM = Use Material, UM-SUB = Penggunaan Material oleh Pihak 3
														'ITM_CODE' 			=> $ITM_CODE,
														'ACC_ID' 			=> $ACC_ID,
														'ITM_UNIT' 			=> $ITM_UNIT,
														'ITM_GROUP' 		=> $ITM_GROUP,
									...
									// START : UPDATE PROFIT AND LOSS
										...
										$parameters1 = array('PERIODED' 	=> $PERIODED,
															...
															'ITM_CODE'		=> $ITM_CODE,
															'ITM_TYPE'		=> $ITM_TYPE);
										$this->m_updash->updateLR_UM($PRJCODE, $parameters1);	// CONTRACTOR
									...
									// START : UPDATE STOCK
										$parameters1 = array('PRJCODE' 	=> $PRJCODE,
															...
															'ITM_QTY' 	=> $ITM_QTY,
															'ITM_PRICE' => $ITM_PRICE);
										$this->m_itemusage->updateITM_UM($parameters1);
									// START : UPDATE STOCK
								}
								elseif($UM_TYPE == 2)
								{
									$parameters = array('JournalH_Code' 	=> $JournalH_Code,
														...
														'PRJCODE'			=> $PRJCODE,
														'JSource'			=> $JSource,
														'TRANS_CATEG' 		=> 'UM-SUB',	// UM = Use Material, UM-SUB = Penggunaan Material oleh Pihak 3
														'ITM_CODE' 			=> $ITM_CODE,
														'ACC_ID' 			=> $ACC_ID,
														'ITM_UNIT' 			=> $ITM_UNIT,
														'ITM_GROUP' 		=> $ITM_GROUP,
								...
								elseif($UM_TYPE == 3)
								{
									...
									$parameters = array('JournalH_Code' 	=> $JournalH_Code,
													...
													'PRJCODE2'			=> $PRJDEST,
													'JSource'			=> $JSource,
													'TRANS_CATEG' 		=> "UM_TSF",	// UM = Use Material, UM-SUB = Penggunaan Material oleh Pihak 3
													'ITM_CODE' 			=> $ITM_CODE,
													'ACC_ID' 			=> $ACC_ID,
													'ITM_UNIT' 			=> $ITM_UNIT,
													'ITM_GROUP' 		=> $ITM_GROUP,
				...
				// START : ADD DOC HISTORY
					...
					$paramTrack 	= array('REF_NUM' 		=> $UM_NUM,
											...
											'FLDCODE'		=> "UM_CODE",
											'FLDNAME'		=> "UM_NUM",
											'HISTTYPE'		=> "Update approve ($UM_STAT)");
					$this->m_updash->uDocH($paramTrack);
				// END : ADD DOC HISTORY
	  	...
	  	function get_AllDataSRV() // GOOD
		{
			...
			// START : FOR SERVER-SIDE
				...
				$start			= $_REQUEST['start'];
				$search			= $_REQUEST['search']["value"];
				$num_rows 		= $this->m_itemusage->get_AllDataSRVCX($PRJCODE, $search, $start, $length);
				$total			= $num_rows;
				$output			= array();
				$output['draw']	= $draw;
				$output['recordsTotal'] = $output['recordsFiltered']= $total;
	  	...
	  	function get_AllDataITM() // GOOD
		{
			...
			// START : FOR SERVER-SIDE
				...
				$start			= $_REQUEST['start'];
				$search			= $_REQUEST['search']["value"];
				$num_rows 		= $this->m_itemusage->get_AllDataITMC($PRJCODE, $JOBPAR, $search, $start, $length);
				$total			= $num_rows;
				$output			= array();
				$output['draw']	= $draw;
				$output['recordsTotal'] = $output['recordsFiltered']= $total;
				...
				foreach ($query->result_array() as $dataI) 
				{
					...
					$PO_VOL 		= $dataI['PO_VOL'];	
					$PO_VOL_R 		= $dataI['PO_VOL_R'];
					$IR_VOL 		= $dataI['IR_VOL'];	
					$IR_VOL_R 		= $dataI['IR_VOL_R'];
					$UM_VOL 		= $dataI['UM_VOL']; 
					$UM_VOL_R 		= $dataI['UM_VOL_R'];

					$WO_VOL 		= $dataI['WO_VOL'];
					$WO_VOL_R 		= $dataI['WO_VOL_R'];
					$WO_CVOL 		= $dataI['WO_CVOL'];
					$OPN_VOL 		= $dataI['OPN_VOL'];
					$OPN_VOL_R 		= $dataI['OPN_VOL_R'];
					...
					$itemConvertion	= 1;
					$REQ_VOLM 		= $PR_VOL + $PR_VOL_R + $WO_VOL + $WO_VOL_R + $VCASH_VOL  + $VCASH_VOL_R + $VLK_VOL + $VLK_VOL_R + $PPD_VOL + $PPD_VOL_R - $PR_CVOL - $WO_CVOL;
					$PO_VOLM 		= $PO_VOL + $PO_VOL_R + $OPN_VOL + $OPN_VOL_R + $VCASH_VOL  + $VCASH_VOL_R + $VLK_VOL + $VLK_VOL_R + $PPD_VOL + $PPD_VOL_R;
					$IR_VOLM 		= $IR_VOL + $OPN_VOL + $VCASH_VOL  + $VLK_VOL + $PPD_VOL;
					$IR_VOLM 		= $IR_VOL + $OPN_VOL + $VCASH_VOL  + $VLK_VOL + $PPD_VOL;
					$IR_VOLM_R 		= $IR_VOL_R + $OPN_VOL_R + $VCASH_VOL_R  + $VLK_VOL_R + $PPD_VOL_R;
					$IR_VOLMV 		= $IR_VOLM + $IR_VOLM_R;
					$UM_VOLM 		= $UM_VOL + $OPN_VOL + $VCASH_VOL  + $VLK_VOL + $PPD_VOL;
					$UM_VOLM_R 		= $UM_VOL_R + $OPN_VOL_R + $VCASH_VOL_R  + $VLK_VOL_R + $PPD_VOL_R;
					...
					//$ITM_STOCK	= $IR_VOLM - $UM_VOLM - $UM_VOLM_R;
					$ITM_STOCK		= $IR_VOL - $UM_VOL - $UM_VOL_R;
					$ITM_STOCKV		= "$IR_VOL - $UM_VOL - $UM_VOL_R";
					...
					$ITM_UVOL_R		= 0;
					$ITM_UVAL_R		= 0;
					$s_PRC	= "SELECT 	SUM(IR_VOL) AS ITM_IR_VOL, SUM(IR_VAL) AS ITM_IR_VAL,
										SUM(UM_VOL) AS ITM_UM_VOL, SUM(UM_VOL_R) AS ITM_UM_VOL_R,
										SUM(UM_VAL) AS ITM_UM_VAL, SUM(UM_VAL_R) AS ITM_UM_VAL_R
								FROM tbl_item_logbook_$PRJCODEVW WHERE DOC_CATEG IN ('IR','UM') AND ITM_CODE = '$ITM_CODE' AND JOBCODEID = '$JOBCODEID'";
					$r_PRC	= $this->db->query($s_PRC)->result();
					foreach($r_PRC as $rw_PRC) :
						$ITM_IR_VOL 	= $rw_PRC->ITM_IR_VOL;
						$ITM_IR_VAL		= $rw_PRC->ITM_IR_VAL;
						$ITM_UM_VOL		= $rw_PRC->ITM_UM_VOL;
						$ITM_UM_VOL_R	= $rw_PRC->ITM_UM_VOL_R;
						$ITM_UM_VAL		= $rw_PRC->ITM_UM_VAL;
						$ITM_UM_VAL_R	= $rw_PRC->ITM_UM_VAL_R;
					endforeach;
					$ITM_STOCK 		= $ITM_IR_VOL - $ITM_UM_VOL - $ITM_UVOL_R;


					// OTHER SETT
						if($ITM_STOCK > 0)
						{
							$chkBox		= "<input type='checkbox' name='chk1' value='".$JOBCODEDET."|".$JOBCODEID."|".$JOBCODE."|".$PRJCODE."|".$ITM_CODE."|".$ITM_NAME."|".$ITM_UNIT."|".$ITM_PRICE."|".$ITM_VOLM."|".$ITM_STOCK."|".$ACC_ID."|".$ACC_ID_UM."|".$ITM_STOCK."' onClick='pickThis1(this);'/>";
						}
						else
						{
					...
					$output['data'][] 	= array($chkBox,
												"<div>
											  		<span style='white-space:nowrap'>$ITM_CODE - $ITM_NAME</span>
											  	</div>
											  	<div style='font-style: italic;'>
											  		<i class='text-muted'>$JOBPARENT : $JOBDESC
											  	</div>",
												number_format($ITM_VOLM, 2),
												number_format($REQ_VOLM, 2),
												number_format($IR_VOL, 2),
												number_format($UM_VOL, 2),
												"<span title='Total reserved dari seluruh pekerjaan'>".number_format($ITM_UM_VOL_R, 2)."</span>",
												"<span title='Total stok dari seluruh pekerjaan'>".number_format($ITM_STOCK, 2)."</span>",
												"$ITM_UNIT"
											);
	  	...
	  	function get_AllDataITM2() // GOOD
		{
			...
			// START : FOR SERVER-SIDE
				...
				$start			= $_REQUEST['start'];
				$search			= $_REQUEST['search']["value"];
				$num_rows 		= $this->m_itemusage->get_AllDataITM2C($PRJCODE, $JOBPAR, $ITMCODE, $search, $start, $length);
				$total			= $num_rows;
				$output			= array();
				$output['draw']	= $draw;
				$output['recordsTotal'] = $output['recordsFiltered']= $total;
				...
				$query 			= $this->m_itemusage->get_AllDataITM2L($PRJCODE, $JOBPAR, $ITMCODE, $search, $length, $start, $order, $dir);
				...
				foreach ($query->result_array() as $dataI) 
				{
					...
					$itemConvertion	= 1;
					//$REQ_VOLM 	= $PR_VOL + $PR_VOL_R + $WO_VOL + $WO_VOL_R + $VCASH_VOL  + $VCASH_VOL_R + $VLK_VOL + $VLK_VOL_R + $PPD_VOL + $PPD_VOL_R - $PR_CVOL - $WO_CVOL;
					$REQ_VOLM 		= $PO_VOL + $PO_VOL_R + $WO_VOL + $WO_VOL_R + $VCASH_VOL  + $VCASH_VOL_R + $VLK_VOL + $VLK_VOL_R + $PPD_VOL + $PPD_VOL_R - $PO_CVOL - $WO_CVOL;
					$PO_VOLM 		= $PO_VOL + $PO_VOL_R + $OPN_VOL + $OPN_VOL_R + $VCASH_VOL  + $VCASH_VOL_R + $VLK_VOL + $VLK_VOL_R + $PPD_VOL + $PPD_VOL_R;
					$IR_VOLM 		= $IR_VOL + $OPN_VOL + $VCASH_VOL  + $VLK_VOL + $PPD_VOL;
					$IR_VOLM 		= $IR_VOL + $OPN_VOL + $VCASH_VOL  + $VLK_VOL + $PPD_VOL;
					$IR_VOLM_R 		= $IR_VOL_R + $OPN_VOL_R + $VCASH_VOL_R  + $VLK_VOL_R + $PPD_VOL_R;
					...
					$TIR_VOL 		= $IR_VOL + $OPN_VOL + $VCASH_VOL  + $VLK_VOL + $PPD_VOL;
					$TUM_VOL 		= $UM_VOL + $OPN_VOL + $VCASH_VOL  + $VLK_VOL + $PPD_VOL + $UM_VOL_R + $OPN_VOL_R + $VCASH_VOL_R  + $VLK_VOL_R + $PPD_VOL_R;
					$TREM_VOL 		= $TIR_VOL - $TUM_VOL;
					...
					foreach($r_PRC as $rw_PRC) :
						...
						$ITM_UM_VAL_R	= $rw_PRC->ITM_UM_VAL_R;
					endforeach;
					$ITM_STOCK 		= round($ITM_IR_VOL - $ITM_UM_VOL - $ITM_UM_VOL_R,3);


					// OTHER SETT
						if($ITM_STOCK > 0)
						{
							$chkBox		= "<input type='checkbox' name='chk2' id='chk2_".$noU."' value='".$JOBCODEDET."|".$JOBCODEID."|".$JOBCODE."|".$PRJCODE."|".$ITM_CODE."|".$ITM_NAME."|".$ITM_UNIT."|".$ITM_PRICE."|".$ITM_VOLM."|".$ITM_STOCK."|".$ACC_ID."|".$ACC_ID_UM."|".$ITM_STOCK."' onClick='pickThis2(this);'/><input type='checkbox' name='chk2a' id='chk2a_".$noU."' value='".$JOBCODEDET."|".$JOBCODEID."|".$JOBCODE."|".$PRJCODE."|".$ITM_CODE."|".$ITM_NAME."|".$ITM_UNIT."|".$ITM_PRICE."|".$ITM_VOLM."|".$ITM_STOCK."|".$ACC_ID."|".$ACC_ID_UM."' style='display: none'/>";
						}
						else
						{
							$chkBox		= "<input type='checkbox' name='chk' value='' style='display: none' />";
					...
					$output['data'][] 	= array($chkBox,
												...
												number_format($ITM_VOLM, 2),
												number_format($REQ_VOLM, 2),
												number_format($ITM_IR_VOL, 2),
												number_format($ITM_UM_VOL, 2),
												"<span title='Total reserved dari seluruh pekerjaan'>".number_format($ITM_UM_VOL_R, 2)."</span>",
												"<span title='Total stok dari seluruh pekerjaan'>
													".number_format($ITM_STOCK, 2)."
													<input type='hidden' name='ITM_STOCK_".$noU."' id='ITM_STOCK_".$noU."' value='".$ITM_STOCK."'>
												</span>",
												"$ITM_UNIT"
											);
