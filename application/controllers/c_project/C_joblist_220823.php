...
class C_joblist extends CI_Controller  
{
  	...
  	function get_AllDataRAPDET() // GOOD
	{
		...
		// START : FOR SERVER-SIDE
			...
			$s_00 			= "tbl_joblist_detail_$PRJCODEVW WHERE JOBPARENT = '$JOBPARENT' AND PRJCODE = '$PRJCODE'";
		...
		// START : FOR SERVER-SIDE
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				if($ISLASTH == 1)
				{
					...
					$BOQ_JOBCOSTV	= number_format($dataI['BOQ_JOBCOST'], 2);
		...
		// START : FOR SERVER-SIDE
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				if($ISLASTH == 1)
				{
					...
					$chkBox		= "<input type='checkbox' name='chk0' value='".$JOBCODEID."|".$PRJCODE."|".$JOBDESC."|".$ITM_UNIT."|".$BOQ_JOBCOST."' onClick='pickThis0(this);'/>";
				}
				else
				{
					// $JOBCOSTV	= "-";
	...
	function addJAnl_process() // G
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$this->db->trans_begin();
				...
				$JOBUNIT 		= $this->input->post('JOBUNIT');
				// $JOBCOST 		= $this->input->post('JOBCOST');
				$BOQ_JOBCOST	= $this->input->post('BOQ_JOBCOST');
				$MAN_NUM 		= $this->input->post('MAN_NUM');
				$MAN_CODE 		= $this->input->post('MAN_CODE');
