...
class C_r3ps70ck0p extends CI_Controller  
{	
    function index()
	{		
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$url			= site_url('c_project/c_r3ps70ck0p/s70ck0p/?id='.$this->url_encryption_helper->encode_url($appName));
			redirect($url);
		}
		else
		{
	...
	function s70ck0p() // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data['form_action'] 	= site_url('c_project/c_r3ps70ck0p/s70ck0p_view/?id='.$this->url_encryption_helper->encode_url($appName));
	...
	function s70ck0p_view() // OK
	{
		...
		{			
			...
			$data['ITM_GROUP'] 		= $this->input->post('ITM_GROUP');	// View or Excel
