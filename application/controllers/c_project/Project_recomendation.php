...
class Project_recomendation extends CI_Controller  
{
	...
	{	
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data['backURL'] 			= site_url('c_project/project_recomendation/');
			$MenuCode 					= 'MN267';
			$data["MenuCode"] 			= 'MN267';
	...
	function add_process() // OK
	{
		...
		{		
			...
			$projectrecom = array('REC_CODE' 		=> $this->input->post('REC_CODE'),
							...
							'REC_FUNDSRC_LOAN'		=> $this->input->post('REC_FUNDSRC_LOAN'),
	...
	{	
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data['default']['REC_FUNDSRC_LOAN']= $getprojrecom->REC_FUNDSRC_LOAN;
			...
			$REC_USR_MRK_STAT					= $getprojrecom->REC_USR_MRK_STAT;
			//echo "$REC_USR_MRK_STAT";
