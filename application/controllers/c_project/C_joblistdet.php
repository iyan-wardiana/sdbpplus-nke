...
class C_joblistdet extends CI_Controller  
{
		...
		// START : FOR SERVER-SIDE
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				$s_BUDGVAL		= "SELECT ITM_VOLM, ITM_PRICE, ITM_LASTP, ITM_BUDG,
										AMD_VOL 																AS AMD_VOL,
										AMD_VAL 																AS AMD_VAL,
										AMDM_VOL 																AS AMDM_VOL,
										AMDM_VAL 																AS AMDM_VAL,
										(PR_VOL - PR_CVOL + WO_VOL - WO_CVOL + VCASH_VOL + VLK_VOL + PPD_VOL) 	AS REQ_VOL,
										(PR_VOL_R + WO_VOL_R + VCASH_VOL_R + VLK_VOL_R + PPD_VOL_R) 			AS REQ_VOL_R,
										(PO_VAL - PO_CVAL + WO_VAL - WO_CVAL + VCASH_VAL + VLK_VAL + PPD_VAL) 	AS REQ_VAL,
										(PO_VAL_R + WO_VAL_R + VCASH_VAL_R + VLK_VAL_R + PPD_VAL_R) 			AS REQ_VAL_R,
										-- (UM_VOL + OPN_VOL + VCASH_VOL + VLK_VOL + PPD_VOL) 						AS USED_VOL,
										(IR_VOL + OPN_VOL + VCASH_VOL + VLK_VOL + PPD_VOL) 						AS USED_VOL,
										-- (UM_VOL_R + OPN_VOL_R + VCASH_VOL_R + VLK_VOL_R + PPD_VOL_R) 			AS USED_VOL_R,
										(IR_VOL_R + OPN_VOL_R + VCASH_VOL_R + VLK_VOL_R + PPD_VOL_R) 			AS USED_VOL_R,
										-- (UM_VAL + OPN_VAL + VCASH_VAL + VLK_VAL + PPD_VAL) 						AS USED_VAL,
										(IR_VAL + OPN_VAL + VCASH_VAL + VLK_VAL + PPD_VAL) 						AS USED_VAL,
										-- (UM_VAL_R + OPN_VAL_R + VCASH_VAL_R + VLK_VAL_R + PPD_VAL_R) 			AS USED_VAL_R
										(IR_VAL_R + OPN_VAL_R + VCASH_VAL_R + VLK_VAL_R + PPD_VAL_R) 			AS USED_VAL_R
									FROM tbl_joblist_detail_$PRJCODEVW WHERE JOBCODEID = '$JOBCODEID' AND PRJCODE = '$PRJCODE'";
				...
				foreach($r_BUDGVAL as $rowJOBDR) :
					...
					$AMD_VOL 		= $rowJOBDR->AMD_VOL; 		// AMD PLUS VOL
					$AMD_VAL 		= $rowJOBDR->AMD_VAL; 		// AMD PLUS VAL
					$AMDM_VOL 		= $rowJOBDR->AMDM_VOL; 		// AMD MIN VOL
					$AMDM_VAL 		= $rowJOBDR->AMDM_VAL; 		// AMD MIN VAL

					$REQ_VOL 		= $rowJOBDR->REQ_VOL; 		// TOTAL REQUEST VOL
					$REQ_VOL_R		= $rowJOBDR->REQ_VOL_R; 	// TOTAL REQUEST VOL RESERVE
					$REQ_VAL 		= $rowJOBDR->REQ_VAL; 		// TOTAL REQUEST VAL
					$REQ_VAL_R		= $rowJOBDR->REQ_VAL_R; 	// TOTAL REQUEST VAL RESERVE

					$USED_VOL 		= $rowJOBDR->USED_VOL; 	// TOTAL DIGUNAKAN VOL
					$USED_VOL_R		= $rowJOBDR->USED_VOL_R; 	// TOTAL DIGUNAKAN VOL RESERVE
					$USED_VAL 		= $rowJOBDR->USED_VAL;		// TOTAL DIGUNAKAN VAL
					$USED_VAL_R		= $rowJOBDR->USED_VAL_R; 	// TOTAL DIGUNAKAN VAL RESERVE

					$TREQ_VOL 		= $REQ_VOL + $REQ_VOL_R;
					$TREQ_VAL 		= $REQ_VAL + $REQ_VAL_R;
				endforeach;
				...
				if($ISLAST == 0)	// HEADER
				{
					...
					$CELL_COL	= "style='font-weight:bold; white-space:nowrap'";
						...
						$REMAIN			= $ITM_BUDG + $AMD_VAL - $AMDM_VAL - $TREQ_VAL;
				...
				else
				{
					...
					// COUNT PROJECTION COMPLETED
						$TOTUSED_01	= $TREQ_VAL;
						if($JOBUNIT == 'LS')
							$TOTREMA_01	= ($ITM_BUDG + $AMD_VAL - $TOTUSED_01);
						else
					...
					$LinkRlz	= "$PRJCODE~$JOBCODEID~$ITM_CODE";
					$ViewRlz	= site_url('c_project/c_joblistdet/idxpr0f17l005vwRlz/?id='.$this->url_encryption_helper->encode_url($LinkRlz));
				...
				$TOT_JOBCOST	= $ITM_BUDG + $AMD_VAL - $AMDM_VAL - $TREQ_VAL;
				$TOT_JOBCOSTP 	= $TOT_JOBCOST ?: 1;
				$percBDG		= $TREQ_VAL / $TOT_JOBCOSTP * 100;		// Used Percentation
				$STATCOL1		= 'success';
				if($percBDG > 85)
					$STATCOL	= 'danger';
				elseif($percBDG > 60)
				...
				if($ISLAST == 1)
				{
					if($ADD_VOLM > 0 && $AMD_VAL > 0)
					{
						$ADD_PRICEV = $AMD_VAL / $ADD_VOLM;
					...
					if($AMDM_VAL > 0)
					{
						...
						$ADDVAL_VWM	= 	"<div style='white-space:nowrap;'>
					...
					if($ITM_CODE == '')
					{
						...
						$svItmAcc 	= "$urlsvAcc~$PRJCODE~$JOBCODEID~$JOBDESC";
					...
					if($TREQ_VAL > 0)
						$VWICON 	= "<span class='label label-info' style='font-size:12px; cursor: pointer;' onClick='showDetTrx(\"".$ViewTrx."\")'>
												<i class='fa fa-eye'></i></span>";
					if($USED_VAL > 0)
						$VWICONRlz 	= "<span class='label label-info' style='font-size:12px; cursor: pointer;' onClick='showDetRlz(\"".$ViewRlz."\")'>
												<i class='fa fa-eye'></i></span>";
				}
				...
				$output['data'][] 	= array("<span ".$CELL_COL."><div style='margin-left: ".$spaceLev."px;'>$secPrint &nbsp;".$JobView."</div></span>",
											...
											"<div style='text-align:right; white-space:nowrap'><span class='label label-".$STATCOL."' style='font-size:12px'>".number_format($ITM_BUDG,2)."</span></div>",
											...
											"<div style='text-align:right; white-space:nowrap'>
												<span class='label label-".$STATCOL."' style='font-size:12px; cursor: pointer;' onClick='showDetAmd(\"".$ViewAmd."\")'>
													".number_format($AMD_VAL,2)."
												</span>".$ADDVAL_VWM."
											</div>",
											"<div style='text-align:right; white-space:nowrap'>
												<span class='label label-".$STATCOL."' style='font-size:12px;'>
													".number_format($TREQ_VAL,2)."
												</span>&nbsp;
												".$VWICON."
											</div>",
											"<div style='text-align:right; white-space:nowrap'>
												<span class='label label-".$STATCOL."' style='font-size:12px;'>
													".number_format($USED_VAL,2)."
												</span>&nbsp;
												".$VWICONRlz."
											</div>",
    ...
    function idxpr0f17l005vwRlz() 
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$this->load->view('v_project/v_joblistdet/v_joblistdet_viewRlz', $data);
		}
		else
		{
			redirect('__l1y');
