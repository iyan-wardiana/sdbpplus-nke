...
class C_joblist extends CI_Controller  
{
				...
				if($ISLASTH == 1)
				{
					...
					$secAddDetV		= 	"<input type='hidden' name='urlAddJDet".$noU."' id='urlAddJDet".$noU."' value='".$secAddDet."'>
										<label class='pull-right' style='white-space:nowrap'>
											<a href='".$secAddDet."' class='btn btn-info btn-xs' title='Create'>
  	...
  	function get_AllDataITM2() // GOOD
	{
		...
		// START : FOR SERVER-SIDE
			...
			$start			= $_REQUEST['start'];
			$search			= $_REQUEST['search']["value"];
			$num_rows 		= $this->m_joblist->get_AllDataITMC($PRJCODE, $search);
			$total			= $num_rows;
			$output			= array();
			$output['draw']	= $draw;
			$output['recordsTotal'] = $output['recordsFiltered']= $total;
			...
			$query 			= $this->m_joblist->get_AllDataITML($PRJCODE, $search, $length, $start, $order, $dir);
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				$chkBox			= "<input type='checkbox' name='chk1' value='".$PRJCODE."|".$JOBCODEID."|".$ITM_CODE."|".$ITM_NAME."|".$ITM_UNIT."|".$ITM_GROUP."' onClick='pickThis1(this);'/>";
