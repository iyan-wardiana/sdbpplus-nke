...
class C_amandemen_report extends CI_Controller  
{	
    function index()
	{		
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$url			= site_url('c_project/c_amandemen_report/amandemen/?id='.$this->url_encryption_helper->encode_url($appName));
			redirect($url);
		}
		else
		{
	...
	function amandemen() // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data['form_action'] 	= site_url('c_project/c_amandemen_report/amandemen_view/?id='.$this->url_encryption_helper->encode_url($appName));
	...
	function amandemen_view() // OK
	{
		...
		{			
			...
			{
				$this->load->view('v_project/v_report/v_amandemen/v_amandemen_report_adm', $data);
			}
			else
			{
