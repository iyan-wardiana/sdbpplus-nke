 ...
 * File Name	= c_si180c2ecer.php
 * Notes		= -
*/

class C_si180c2ecer  extends CI_Controller  
{
 	public function index() // OK
	{
		...
		$url			= site_url('c_project/c_si180c2ecer/prj180c2cls/?id='.$this->url_encryption_helper->encode_url($appName));
		redirect($url);
	}
	...
	function prj180c2cls() // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			if($LangID == 'IND')
			{
				$data["h1_title"] 	= "Sertifikat SI";
			}
			else
			{
				$data["h1_title"] 	= "SI Certificate";
			}
			...
			$data['viewPRJ'] 	= $this->m_projectlist->get_all_project($DefEmp_ID)->result();			
			$data["secURL"] 	= "c_project/c_si180c2ecer/gall180c2csic/?id=";
	...
	function gall180c2csic() // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data['backURL'] 			= site_url('c_project/c_si180c2ecer/');
	...
	function a180c2edd() // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$cancel_url 				= site_url('c_project/c_si180c2ecer/gall180c2csic/?id='.$this->url_encryption_helper->encode_url($PRJCODE));
			...
			$data['form_action']		= site_url('c_project/c_si180c2ecer/add_process');
	...
	function pall180c2esi() // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data['form_action']		= site_url('c_project/c_si180c2ecer/add_process');
			$data['PRJCODE'] 			= $PRJCODE;
			$data['secShowAll']			= site_url('c_project/c_si180c2ecer/popupallitem/?id='.$this->url_encryption_helper->encode_url($PRJCODE));
	...
	function add_process() // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$dataSIC 	= array('SIC_CODE' 		=> $SIC_CODE,
								...
								'SIC_APPSTAT'	=> $SIC_APPSTAT,
								'SIC_APPDATE'	=> $SIC_APPDATE,
			...
			$back	= site_url('c_project/c_si180c2ecer/gall180c2csic/?id='.$this->url_encryption_helper->encode_url($PRJCODE));
			redirect($back);
		}
		else
		{
	...
	function update() // OK
	{
		...
		if ($this->session->userdata('login') == TRUE)
		{
			...
			$data['h2_title']			= 'Add SI Certificate';
			$data['h3_title']			= 'SI Certificate';
			$data['form_action']		= site_url('c_project/c_si180c2ecer/update_process');
			$data['MenuCode'] 			= 'MN254';
			$data['backURL'] 			= $cancel_url;
