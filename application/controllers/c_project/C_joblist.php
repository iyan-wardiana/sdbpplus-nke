...
class C_joblist extends CI_Controller  
{
  	...
  	function get_AllDataJL() // GOOD
	{
		...
		// START : FOR SERVER-SIDE
			...
			$num_rows 		= $this->m_joblist->get_AllDataBQC($PRJCODE, $JOBPARID, $search);
			$total			= $num_rows;
			$output			= array();
			$output['draw']	= $draw;
			...
			$query 			= $this->m_joblist->get_AllDataBQL($PRJCODE, $JOBPARID, $search, $length, $start, $order, $dir);
  	...
  	function get_AllDataRAPDET() // GOOD 
	{
		...
		else if($RAPT_STAT == 1 && $RAPP_STAT == 0)		// RAPT LOCKED BUT RAPP OPENED
		{
			$ANLCAT 	= 2;
			if($ISAPPROVE == 1)
				$CANEDIT 	= 1;
		...
		// START : FOR SERVER-SIDE
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				$ITM_NAME		= htmlentities($dataI['ITM_NAME'], ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401 | ENT_HTML5);
				$ITM_UNIT		= $dataI['ITM_UNIT'];
				$ITM_GROUP		= $dataI['ITM_GROUP'];
				...
				$CREATED_STAT	= $dataI['CREATED_STAT'];
							...
							if($TOT_TRX == 0)
							{
								$secAction	= 	"<input type='hidden' name='urlDel".$noU."' id='urlDel".$noU."' value='".$delID."'>
											   	...
											   	<label style='white-space:nowrap'>
													...
													<a href='javascript:void(null);' class='btn bg-yellow btn-xs' id='lockITM".$noU."' onClick='lockRow(".$noU.")' title='Kunci'>
						...
						elseif($ISLOCK == 1 && $CANEDIT == 1)
						{
							...
							if($TOT_TRX == 0)
							{
								$btnUndo= "<input type='hidden' name='urlUndo".$noU."' id='urlUndo".$noU."' value='".$undoID."'>
											<a href='javascript:void(null);' class='btn bg-yellow btn-xs' id='undoITM".$noU."' onClick='undoRow(".$noU.")' title='Batalkan RAP'>
						...
						if($ISLOCK2 == 0 && $CANEDIT == 1)
						{
							if($TOT_TRX == 0)
							{
								$secAction	= 	"<input type='hidden' name='urlDel".$noU."' id='urlDel".$noU."' value='".$delID."'>
											   	<input type='hidden' name='urlLock".$noU."' id='urlLock".$noU."' value='".$lockID."'>
											   	<label style='white-space:nowrap'>
													<a href='javascript:void(null);' class='btn bg-red btn-xs' onClick='delRow(".$noU.")' title='Hapus' ".$isReadOnly.">
														<i class='glyphicon glyphicon-trash'></i>
													</a>
							...
							$output['data'][] 	= array($secAction,
														...
														<input type='hidden' id='data".$noU."ITM_NAME' name='data[".$noU."][ITM_NAME]' value='".$ITM_NAME."' class='form-control' style='max-width:300px;'></div>",
														"<input type='text' name='ITM_RAPVX".$noU."' id='ITM_RAPVX".$noU."' value='".number_format($ITM_RAPV, 2)."' class='form-control' style='min-width:80px; max-width:90px; text-align:right' onKeyPress='return isIntOnlyNew(event);' onBlur='chgVOL(this,".$noU.");' ".$isReadOnly." >
														<input type='hidden' name='data[".$noU."][ITM_RAPV]' id='data".$noU."ITM_RAPV' value='".$ITM_RAPV."' class='form-control' style='max-width:300px;' >",
														"<input type='text' name='ITM_KOEFX".$noU."' id='ITM_KOEFX".$noU."' value='".number_format($ITM_KOEF, 4)."' class='form-control' style='min-width:60px; max-width:70px; text-align:right' onKeyPress='return isIntOnlyNew(event);' onBlur='chgKOEF(this,".$noU.");' >
														<input type='hidden' name='data[".$noU."][ITM_KOEF]' id='data".$noU."ITM_KOEF' value='".$ITM_KOEF."' class='form-control' style='max-width:300px;' readonly >",
														"<input type='text' name='ITM_RAPPX".$noU."' id='ITM_RAPPX".$noU."' value='".number_format($ITM_RAPP, 2)."' class='form-control' style='min-width:110px; max-width:120px; text-align:right' onKeyPress='return isIntOnlyNew(event);' onBlur='chgPRICE(this,".$noU.");' >
				...
				else
				{
					...
					$secAction	= 	"<input type='hidden' name='urlDel".$noU."' id='urlDel".$noU."' value='".$delID."'>
								   	<input type='hidden' name='urlLock".$noU."' id='urlLock".$noU."' value='".$lockID."'>
								   	<label style='white-space:nowrap'>
										<a href='javascript:void(null);' class='btn bg-red btn-xs' onClick='alertLOCK()' title='Locked'>
											<i class='fa fa-lock'></i>
										</a>
										".$btnUndo."
									</label>";
		...
		// START : FOR SERVER-SIDE
			...
			foreach ($query->result_array() as $dataI) 
			{
			    ...
			    foreach($resJlD as $rowJLD) :
				    $JOBPARDESC = htmlentities($rowJLD->JOBDESC, ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401 | ENT_HTML5);
				endforeach;
  	...
  	function get_AllDataITM4() // GOOD
	{
		...
		// START : FOR SERVER-SIDE
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				$ITM_NAME		= htmlentities($dataI['ITM_NAME'], ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401 | ENT_HTML5);;
				$ITM_NAMEV		= wordwrap($ITM_NAME, 50, "<br>", TRUE);
				$ITM_UNIT		= htmlentities($dataI['ITM_UNIT'], ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401 | ENT_HTML5);
				$ITM_GROUP		= $dataI['ITM_GROUP'];
				$ITM_VOLM		= $dataI['ITM_VOLM'];
				$ITM_PRICE		= $dataI['ITM_PRICE'];
				$ITM_BOQV		= $dataI['BOQ_ITM_VOLM'];
  	...
  	function get_AllDataSRV() // GOOD
	{
		...
		// START : FOR SERVER-SIDE
			...
			foreach ($query->result_array() as $dataI) 
			{
				...
				$JOBDESC		= htmlentities($dataI['JOBDESC'], ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401 | ENT_HTML5);
				$ITM_UNIT		= $dataI['JOBUNIT'];
				$JOBCOST		= $dataI['JOBCOST'];
				$JOBLEV			= $dataI['JOBLEV'];
				$ISLASTH		= $dataI['ISLASTH'];
	...
	function delComp()
	{
        ...
        $sqlDel01		= "DELETE FROM tbl_jobcreate_detail
        				WHERE JOBD_ID = '$JOBD_ID' AND JOB_NUM = '$JOB_NUM' AND ITM_CODE = '$ITM_CODE'
        					AND JOBPARENT = '$JOBPARENT' AND PRJCODE = '$PRJCODE'";
        $this->db->query($sqlDel01);

		$sqlDel02	= "DELETE FROM tbl_item
        				WHERE ITM_CODE = '$ITM_CODE' AND PRJCODE = '$PRJCODE' AND PRJCODE NOT IN ('NKE')";
        $this->db->query($sqlDel02);
	...
	function lockComp()
	{
        ...
        $JOBPARENT	= $colExpl[4];
        $ITM_CODE	= $colExpl[5];
        $ITM_NAME	= htmlentities($colExpl[6], ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401 | ENT_HTML5);
        $JOBD_ID	= $colExpl[7];
        $JOBDESCP	= $colExpl[8];
		$DefEmp_ID 	= $this->session->userdata['Emp_ID'];
			...
			if($ANLCAT == 1)						// UPDATE RAPT
			{
				...
				if($r_JID > 0)
				{
					// START : UPDATE JOBLISTDETAIL
						$s_00		= "SELECT ITM_UNIT, ITM_GROUP, ITM_KOEF, ITM_RAPV, ITM_RAPP, ITM_TOTAL, CREATED_STAT
						...
						foreach($r_00 as $rw_00) :
							...
							$s_01 		= 	"UPDATE tbl_joblist_detail SET ITM_VOLM = '$ITM_RAPV', ITM_PRICE = '$ITM_RAPP', JOBDESCP = '$JOBDESCP',
												...
												RAPT_VOLM = '$ITM_RAPV', RAPT_PRICE = '$ITM_RAPP', RAPT_JOBCOST = '$ITM_TOTAL', CREATED_STAT = $JOBCREAT,
						...
						// START : INSERT INTO JOBLIST DETAIL
							...
							/*$s_ITM	= "SELECT ITM_UNIT, ITM_GROUP, ITM_KOEF, ITM_RAPV, ITM_RAPP, ITM_TOTAL
											FROM tbl_jobcreate_detail_$PRJCODEVW WHERE JOB_NUM = '$JOB_NUM' AND ITM_CODE = '$ITM_CODE'
												AND JOBPARENT = '$JOBPARENT'AND PRJCODE = '$PRJCODE'";*/
							$s_ITM		= "SELECT ITM_UNIT, ITM_GROUP, ITM_KOEF, ITM_RAPV, ITM_RAPP, ITM_TOTAL, CREATED_STAT
							...
							// INSERT INTO JOBDET
								$s_05	= "INSERT INTO tbl_joblist_detail (ORD_ID, JOBCODEDET, JOBCODEID, JOBPARENT, JOBCODEID_P,
												...
												ITM_PRICE, ITM_LASTP, PRJPERIOD, PRJPERIOD_P, ITM_BUDG, BOQ_VOLM, BOQ_PRICE, BOQ_JOBCOST, IS_LEVEL, CREATED_STAT,
											...
											VALUES ($ORD_IDN, '$JOBID_DET', '$JOBID_DET', '$JOBPARENT', '$JOBPARENT',
												...
												'$ITM_RAPP', '$ITM_RAPP', '$PRJCODE', '$PRJ_HO', '$ITM_TOTAL', '0', '0', '0', '$JOBLEV_DET', $JOBCREAT,
									...
									foreach($r_05b as $rw_05b):
										...
										$sqlInsITM	= "INSERT INTO tbl_item (PRJCODE, PRJCODE_HO, PRJPERIOD, ITM_CODE, ITM_GROUP, ITM_CATEG, ITM_NAME, 
														...
														VALUES ('$PRJCODE', '$PRJ_HO', '$PRJCODE', '$ITM_CODE', '$ITM_GROUP', '$ITM_CATEG', '$ITM_NAME', 
															...
															'$ISMTRL', '0', '0', '0', '0', '1', '$nNo', '$ITM_LR', '$CREATED', '$CREATER', 'BoQ Manual')";
				...
				if($r_JID > 0)
				{
					// START : UPDATE JOBLISTDETAIL
						...
						foreach($r_00 as $rw_00) :
							...
							$s_01 		= 	"UPDATE tbl_joblist_detail SET ITM_VOLM = '$ITM_RAPV', ITM_PRICE = '$ITM_RAPP', JOBDESCP = '$JOBDESCP',
												ITM_LASTP = '$ITM_RAPP', ITM_AVGP = '$ITM_RAPP', ITM_BUDG = '$ITM_TOTAL', CREATED_STAT = $JOBCREAT,
						...
						// START : INSERT INTO JOBLIST DETAIL
							...
							// INSERT INTO JOBDET
								$s_05	= "INSERT INTO tbl_joblist_detail (ORD_ID, JOBCODEDET, JOBCODEID, JOBPARENT, JOBCODEID_P,
												...
												ITM_PRICE, ITM_LASTP, ITM_BUDG, IS_LEVEL, ISLAST, oth_reason, Patt_Number, CREATED_STAT,
											...
											VALUES ($ORD_IDN, '$JOBID_DET', '$JOBID_DET', '$JOBPARENT', '$JOBPARENT',
												...
												'$ITM_RAPP', '$ITM_RAPP', '$ITM_TOTAL', '$JOBLEV_DET', 1, '$ITM_NOTES', '$nNo', $JOBCREAT,
	...
	function addItmTmp()
	{
		...
		$ITM_CODE 		= $arrItem[3];
		$ITM_NAME 		= htmlentities($arrItem[4], ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401 | ENT_HTML5);
		$ITM_UNIT 		= $arrItem[5];
		$ITM_GROUP		= $arrItem[6];
		$JOB_NUM		= $arrItem[7];
		$JOB_NOTE		= $arrItem[8];
		...
		// START : PARENT JOB DETAIL
			...
			$sqlJlD			= "SELECT JOBDESC, JOBUNIT, JOBLEV, JOBVOLM, PRICE, JOBCOST, BOQ_VOLM, BOQ_PRICE, BOQ_JOBCOST
								FROM tbl_joblist_$PRJCODEVW WHERE JOBCODEID = '$JOBPARENT' LIMIT 1";
			$resJlD			= $this->db->query($sqlJlD)->result();
			foreach($resJlD as $rowJLD) :
			    $JOBDESC 	= $rowJLD->JOBDESC;
		...
		// START : SAVE DETAIL
			$s_02	= "INSERT INTO tbl_jobcreate_detail (PRJCODE, JOB_NUM, JOBCODEID, JOBPARENT, ITM_CODE, ITM_NAME, ITM_UNIT, ITM_GROUP, ITM_BOQV, ITM_BOQP,
							ITM_KOEF, ITM_RAPV, ITM_RAPP, ITM_TOTAL, ITM_NOTES, ISLOCK, CREATED, CREATED_STAT)
						VALUES ('$PRJCODE', '$JOB_NUM', '', '$JOBPARENT', '$ITM_CODE', '$ITM_NAME', '$ITM_UNIT', '$ITM_GROUP', '$BOQ_VOLM', '$BOQ_PRICE',
							0, 0, 0, 0, '', 0, '$CREATED', $ANLCAT)";
			$this->db->query($s_02);
		// END : SAVE DETAIL

		// START : COPY ITEM FROM MASTER NKE
			$s_03 	= "INSERT INTO tbl_item (PRJCODE, PRJCODE_HO, PRJPERIOD, PITM_CODE, ITM_CODE, ITM_CODE_H, ITM_CODE_H5, 
	...
	function copyRAPDET()
	{
		$PRJCODE 	= $this->input->post('PRJCODE');
		$JOB_NUM 	= $this->input->post('JOB_NUM');
		$JOBPARENT 	= $this->input->post('JOB_PARCODE');
		$PRJCODEVW 	= strtolower(preg_replace("/[^a-zA-Z0-9\s]/", "", $PRJCODE));

		// GET JOBLIST DETAIL
			$getJOBD 	= "tbl_joblist_detail_$PRJCODEVW 
							WHERE JOBPARENT = '$JOBPARENT'
							AND JOBCODEID NOT IN (SELECT JOBCODEID FROM tbl_jobcreate_detail_$PRJCODEVW
													WHERE JOBPARENT = '$JOBPARENT')";
			$countJOBD	= $this->db->count_all($getJOBD);
			if($countJOBD > 0)
			{
				$insJOBC 	= "INSERT INTO tbl_jobcreate_detail (PRJCODE, JOB_NUM, JOBCODEID, JOBPARENT, ITM_CODE, ITM_NAME, ITM_UNIT, ITM_GROUP, 
																ITM_BOQV, ITM_BOQP, ITM_RAPV, ITM_RAPP, ITM_TOTAL, ISLOCK)
								SELECT PRJCODE, '$JOB_NUM', JOBCODEID, JOBPARENT, ITM_CODE, JOBDESC, ITM_UNIT, ITM_GROUP,
								BOQ_VOLM, BOQ_PRICE, ITM_VOLM, ITM_PRICE, ITM_BUDG, 0
								FROM tbl_joblist_detail
								WHERE PRJCODE = '$PRJCODE' AND JOBPARENT = '$JOBPARENT' 
								AND JOBCODEID NOT IN (SELECT JOBCODEID FROM tbl_jobcreate_detail_$PRJCODEVW
														WHERE JOBPARENT = '$JOBPARENT')";
				$this->db->query($insJOBC);
			}
