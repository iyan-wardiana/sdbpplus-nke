...
class M_mail extends CI_Model
{
	...
	function get_last_mail($DefEmp_ID) // OK
	{
		$sql = "SELECT * from tbl_mailgroup_header ORDER BY MG_CODE";
	...
	{		
		$sql		= "select Emp_ID, First_Name, Email from tbl_employee ORDER BY Emp_ID";
		return $this->db->query($sql);
	}
	
	function add($projMatReqH) // OK
	{
		$this->db->insert('tbl_mailgroup_header', $projMatReqH);
	}
	
	function get_Mail_by_number($MG_CODE) // OK
	{
		$sql = "SELECT * FROM tbl_mailgroup_header
				WHERE MG_CODE = '$MG_CODE'";
		return $this->db->query($sql);
	...
	function update($MG_CODE, $projMatReqH) // OK
	{
		$this->db->where('MG_CODE', $MG_CODE);
		$this->db->update('tbl_mailgroup_header', $projMatReqH);
	}
	
	function deleteDetail($MG_CODE) // OK
	{
		//$this->db->where('SPPNUM', $SPPNUM);
		$this->db->where('MG_CODE', $MG_CODE);
		$this->db->delete('tbl_mailgroup_detail');
	}
