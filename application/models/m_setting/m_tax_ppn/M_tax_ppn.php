 ...
 * File Name	= M_tax_ppn.php
 * Location		= -
*/

class M_tax_ppn extends CI_Model
{
	...
	{
		return $this->db->count_all('tbl_tax_ppn');
	}
	...
	{
		$this->db->insert('tbl_tax_ppn', $InsLA);
	}
	
	function get_tax_la($TAXLA_NUM) // G
	{		
		$sql = "SELECT * FROM tbl_tax_ppn WHERE TAXLA_NUM = '$TAXLA_NUM'";
		return $this->db->query($sql);
	}
	
	function t4x_l4_update($TAXLA_NUM) // G
	{
		$sql	= "tbl_tax_ppn WHERE TAXLA_NUM = '$TAXLA_NUM'";
		return $this->db->count_all($sql);
	}
	...
	function update($TAXLA_NUM, $UpdTLA) // G
	{
		...
		$this->db->update('tbl_tax_ppn', $UpdTLA);
	}
}
?>