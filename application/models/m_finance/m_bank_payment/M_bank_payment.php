...
{	
	...
	function updateDP($DocumentNo, $updDP) // G
	{
		...
		$BP_CODE		= "";
		if(isset($updDP['BP_CODE'])) 
			$BP_CODE 	= $updDP['BP_CODE'];

		$BP_DATE		= "";
		if(isset($updDP['BP_DATE'])) 
			$BP_DATE 	= $updDP['BP_DATE'];

		$BP_AMOUNT		= 0;
		if(isset($updDP['BP_AMOUNT'])) 
			$BP_AMOUNT 	= $updDP['BP_AMOUNT'];
		
		$s_DP 			= "UPDATE tbl_dp_header SET BP_AMOUNT = BP_AMOUNT+$BP_AMOUNT, BP_CODE = '$BP_CODE', BP_DATE = '$BP_DATE', DP_PAID = IF(BP_AMOUNT=DP_AMOUN_TOT,2,1) WHERE DP_NUM = '$DocumentNo'";
		$this->db->query($s_DP);
	}
	...
	function get_AllDataVOCPDC($SPLCODE, $INV_NUM, $search) // GOOD =>  // Multiple Proyek Pembayaran Voucher PD
	{
		$sql 	= "tbl_journalheader_pd A INNER JOIN tbl_journalheader_pd_rinv B ON A.JournalH_Code = B.JournalH_Code
						WHERE A.GEJ_STAT = 3 AND A.Journal_AmountTsf > 0
							AND B.Invoice_No IN ('$INV_NUM')
							AND (A.Manual_No LIKE '%$search%' ESCAPE '!' OR A.JournalH_Desc LIKE '%$search%' ESCAPE '!')";
		return $this->db->count_all($sql);
	}
	
	// function get_AllDataVOCPDL($PRJCODE, $SPLCODE, $INV_NUM, $search, $length, $start, $order, $dir) // GOOD
	function get_AllDataVOCPDL($SPLCODE, $INV_NUM, $search, $length, $start, $order, $dir) // GOOD =>  // Multiple Proyek Pembayaran Voucher PD
	{
		if($length == -1)
		{
			if($order !=null)
			{
				$sql = "SELECT DISTINCT A.JournalH_Code, A.Manual_No, A.JournalH_Desc, A.JournalH_Date,
							A.PERSL_EMPID, A.SPLCODE, A.Journal_Amount, A.Reference_Number,
							A.Journal_Amount, A.Journal_AmountTsf, A.Journal_AmountReal, A.PDPaid_Amount, A.PDRec_Amount, A.PPD_RemAmount,
							B.Invoice_No, B.Invoice_Code, B.Invoice_Date, B.Journal_Amount AS REM_PD_Amount, B.Invoice_Amount
		...
		if($length == -1)
		{
			if($order !=null)
			{
				$sql = "SELECT DISTINCT A.DP_NUM AS INV_NUM, A.DP_CODE AS INV_CODE, A.DP_DATE AS INV_DUEDATE, '' AS PO_NUM, A.PRJCODE,
							...
							'DP' AS INV_CATEG, 0 AS INV_AMOUNT_RET, 0 AS INV_AMOUNT_OTH, A.BP_AMOUNT AS INV_AMOUNT_PAID
