 ...
 * File Name	= M_material_request.php
 * Location		= -
*/
?>
<?php
class M_material_request extends CI_Model
{
	...
	function get_last_ten_projMatReq($PRJCODE) // OK
	{
	...
	{
		$this->db->insert('tbl_spp_header', $projMatReqH);
	}
	
	function get_MR_by_number($SPPNUM) // OK
	{
		$sql = "SELECT A.SPPNUM, A.SPPCODE, A.TRXDATE, A.PRJCODE, A.TRXOPEN, A.TRXUSER, A.APPROVE, A.APPRUSR, A.JOBCODE, A.SPPNOTE, A.SPPSTAT, A.REVMEMO,
				A.SPP_VALUE, A.SPP_VALUEAPP,
				A.Patt_Year, A.Patt_Month, A.Patt_Date, A.Patt_Number,
				B.proj_Number, B.PRJCODE, B.PRJNAME
				FROM tbl_spp_header A
				INNER JOIN 	tbl_project B ON A.PRJCODE = B.PRJCODE
				WHERE A.SPPNUM = '$SPPNUM'";
		return $this->db->query($sql);
	}
	
	function update($SPPNUM, $projMatReqH) // OK
	{
