 ...
 * File Name	= m_project_mc.php
 * Notes		= -
*/

class m_project_mc extends CI_Model
{
	...
	function get_AllDataL($PRJCODE, $search, $length, $start, $order, $dir) // GOOD
	{
		if($length == -1)
		{
			if($order !=null)
			{
				$sql = "SELECT A.MC_CODE, A.MC_MANNO, A.MC_DOC, A.MC_STAT, A.PRJCODE, A.MC_DATE, A.MC_STARTD, A.MC_ENDDATE, A.MC_STEP, A.MC_NOTES, A.MC_ISINV, A.MC_PROGAPP
						FROM tbl_mcheader A
							LEFT JOIN tbl_employee B ON A.MC_EMPID = B.Emp_ID
						WHERE A.PRJCODE = '$PRJCODE'
							AND (A.MC_MANNO LIKE '%$search%' ESCAPE '!' OR A.MC_NOTES LIKE '%$search%' ESCAPE '!') ORDER BY A.MC_STAT, $order $dir";
			}
			else
			{
				$sql = "SELECT A.MC_CODE, A.MC_MANNO, A.MC_DOC, A.MC_STAT, A.PRJCODE, A.MC_DATE, A.MC_STARTD, A.MC_ENDDATE, A.MC_STEP, A.MC_NOTES, A.MC_ISINV, A.MC_PROGAPP
	...
	function get_AllDataSIC($PRJCODE, $search) // GOOD
	{
		$sql = "tbl_siheader
				WHERE PRJCODE = '$PRJCODE'
					AND (SI_MANNO LIKE '%$search%' ESCAPE '!' OR SI_DESC LIKE '%$search%' ESCAPE '!'
					OR SI_VALUE LIKE '%$search%' ESCAPE '!')";
		return $this->db->count_all($sql);
	}
		...
		if($length == -1)
		{
			if($order !=null)
			{
				$sql = "SELECT *
						...
						WHERE PRJCODE = '$PRJCODE'
							AND (SI_MANNO LIKE '%$search%' ESCAPE '!' OR SI_DESC LIKE '%$search%' ESCAPE '!'
							OR SI_VALUE LIKE '%$search%' ESCAPE '!') ORDER BY $order $dir";
			}
			else
			{
				$sql = "SELECT *
		...
		else
		{
			...
			else
			{
				$sql = "SELECT *
						...
						WHERE PRJCODE = '$PRJCODE'
							AND (SI_MANNO LIKE '%$search%' ESCAPE '!' OR SI_DESC LIKE '%$search%' ESCAPE '!'
							OR SI_VALUE LIKE '%$search%' ESCAPE '!') LIMIT $start, $length";
			}
			return $this->db->query($sql);
		}
	}
	...
	function count_all_num_rowsProjMC($PRJCODE, $GETFROM) // OK
	{
		$sql	= "tbl_mcheader A
					LEFT JOIN  	tbl_employee B ON A.MC_EMPID = B.Emp_ID
					INNER JOIN 	tbl_project C ON A.PRJCODE = C.PRJCODE
					WHERE A.PRJCODE = '$PRJCODE' AND A.GETFROM = '$GETFROM'";
		return $this->db->count_all($sql);
	}
	
	function get_last_ten_projmc($PRJCODE, $GETFROM) // OK
	{
	...
	function count_all_num_rowsProject_byID($PRJCODE) // OK
	{
		return $this->db->count_all("tbl_project where PRJCODE = '$PRJCODE'");
	}

	function viewProject_byID($PRJCODE) // OK
	{
		$sql = "SELECT proj_Number, PRJCNUM, PRJCODE, PRJNAME, PRJDATE, PRJCOST
	...
	function add($dataMCH, $PRJCODE) // OK
	{
		$this->db->insert('tbl_mcheader', $dataMCH);
		...
		$res_A	= 0;
		$res_CL	= 0;
		$sql_N	= "tbl_mcheader WHERE PRJCODE = '$PRJCODE' AND MC_STAT = 1";
		$res_N	= $this->db->count_all($sql_N);
		$sql_C	= "tbl_mcheader WHERE PRJCODE = '$PRJCODE' AND MC_STAT = 2";
		$res_C	= $this->db->count_all($sql_C);
		$sql_A	= "tbl_mcheader WHERE PRJCODE = '$PRJCODE' AND MC_STAT = 3";
		$res_A	= $this->db->count_all($sql_A);
		$sql_CL	= "tbl_mcheader WHERE PRJCODE = '$PRJCODE' AND MC_STAT = 6";
		...
		if($resPRJ == 0)
		{
			...
			// SAVE TO PROFITLOSS
				$insPL	= "INSERT INTO tbl_dash_transac (PRJ_CODE, PRJ_VALUE, TOT_MC_N, TOT_MC_C, TOT_MC_A, TOT_MC_CL)
		...
		{
			// SAVE TO PROFITLOSS
			$sqlUPD	= "UPDATE tbl_dash_transac SET TOT_MC_N = $res_N, TOT_MC_C = $res_C, TOT_MC_A = $res_A, TOT_MC_CL = $res_CL
						WHERE PRJ_CODE = '$PRJCODE'";
			$this->db->query($sqlUPD);
		}
	}
	
	function addConc($dataMCH) // OK
	{
		$this->db->insert('tbl_mc_conc', $dataMCH);
	}
	...
	function get_MC_by_number($MC_CODE) // OK
	{
		$sql = "SELECT * FROM tbl_mcheader
				WHERE MC_CODE = '$MC_CODE'";
		return $this->db->query($sql);
	}
	
	function update($MC_CODE, $dataMCH, $PRJCODE) // OK
	{
		$this->db->where('MC_CODE', $MC_CODE);
		$this->db->update('tbl_mcheader', $dataMCH);
	...
	function updatePRJ($dataPRJ, $PRJCODE) // OK
	{
		$this->db->where('PRJCODE', $PRJCODE);
		$this->db->update('tbl_project', $dataPRJ);
	}
