...
class M_cashout_upah extends CI_Model
{
	function count_all_prj($DefEmp_ID) // U
	{
		$sql	= "tbl_project WHERE PRJCODE IN (SELECT proj_Code FROM tbl_employee_proj WHERE Emp_ID = '$DefEmp_ID')";
		return $this->db->count_all($sql);
	}
	
	function get_all_prj($DefEmp_ID) // U
	{
	...
	function count_all_num_rowsCOU($PRJCODE) // U
	{
		$sql	= "tbl_co_upah A
	...
	function get_all_COU($PRJCODE) // U
	{
		$sql = "SELECT A.*,
				...
				INNER JOIN 	tbl_project B ON A.PRJCODE = B.PRJCODE
				WHERE A.PRJCODE = '$PRJCODE'
				ORDER BY A.COU_PeriodeCode ASC";
		return $this->db->query($sql);
	}
	...
	function add($projcou) // OK
	{
		$this->db->insert('tbl_co_upah', $projcou);
	}
	
	function get_COU_by_number($COU_PeriodeCode) // OK
	{
		$sql = "SELECT * FROM tbl_co_upah
				WHERE COU_PeriodeCode = '$COU_PeriodeCode'";
		return $this->db->query($sql);
	}
	
	function update($COU_PeriodeCode, $projcou) // OK
	{
		$this->db->where('COU_PeriodeCode', $COU_PeriodeCode);
		$this->db->update('tbl_co_upah', $projcou);
	}
	
	function deleteDetail($SPPNUM, $SPPCODE) // OK
	{
		$this->db->where('SPPNUM', $SPPNUM);
		$this->db->where('SPPCODE', $SPPCODE);
		$this->db->delete('tbl_spp_detail');
	}
	...
	function count_all_num_rowsPR_Inb($PRJCODE) // USED
	{
		$sql	= "tbl_pr_header A
						INNER JOIN 	tbl_project B ON A.PRJCODE = B.PRJCODE
					WHERE A.PRJCODE = '$PRJCODE'
	...
	function count_all_num_rowsPR_SrcInb_MR($PRJCODE, $txtSearch) // USED
	{
		$sql	= "tbl_pr_header A
						INNER JOIN 	tbl_project B ON A.PRJCODE = B.PRJCODE
					WHERE A.PRJCODE = '$PRJCODE'
	...
	function count_all_num_rowsPR_SrcInb_SPP($PRJCODE, $txtSearch) // USED
	{
		$sql	= "tbl_pr_header A
						INNER JOIN 	tbl_project B ON A.PRJCODE = B.PRJCODE
					WHERE A.PRJCODE = '$PRJCODE'
	...
	function get_all_PRInb_MRNo($PRJCODE, $limit, $offset, $txtSearch) // USED
	{
	...
	function get_all_PRInb_SPP($PRJCODE, $limit, $offset, $txtSearch) // USED
	{
		$sql = "SELECT A.SPPNUM, A.SPPCODE, A.TRXDATE, A.PRJCODE, A.TRXOPEN, A.TRXUSER, A.APPROVE, A.APPRUSR, A.JOBCODE, A.SPPNOTE, A.SPPSTAT, REVMEMO,
				A.Patt_Year, A.Patt_Month, A.Patt_Date, A.Patt_Number,
				B.proj_Number, B.PRJCODE, B.PRJNAME
	...
	function get_all_PRInb($PRJCODE, $limit, $offset) // USED
	{
		$sql = "SELECT A.SPPNUM, A.SPPCODE, A.TRXDATE, A.PRJCODE, A.TRXOPEN, A.TRXUSER, A.APPROVE, A.APPRUSR, A.JOBCODE, A.SPPNOTE, A.SPPSTAT, REVMEMO,
				A.Patt_Year, A.Patt_Month, A.Patt_Date, A.Patt_Number,
				B.proj_Number, B.PRJCODE, B.PRJNAME
	...
	function get_all_prj_PNo($limit, $offset, $txtSearch, $DefEmp_ID) // USED
	{
		$sql = "SELECT A.proj_Number, A.PRJCODE, A.PRJCNUM, A.PRJNAME, A.PRJLOCT, A.PRJCOST, A.PRJDATE, A.PRJEDAT, A.PRJSTAT
				FROM tbl_project A
	...
	function get_all_prj_PNm($limit, $offset, $txtSearch, $DefEmp_ID)
	{
		$sql = "SELECT A.proj_Number, A.PRJCODE, A.PRJCNUM, A.PRJNAME, A.PRJLOCT, A.PRJCOST, A.PRJDATE, A.PRJEDAT, A.PRJSTAT
				FROM tbl_project A
				WHERE A.PRJNAME LIKE '%$txtSearch%' AND A.PRJCODE IN (SELECT proj_Code FROM tbl_employee_proj WHERE Emp_ID = '$DefEmp_ID')
	...
	function get_all_PR_MRNo($limit, $offset, $txtSearch)
	{
	...
	function get_all_PR_PNm($limit, $offset, $txtSearch)
	{
		$sql = "SELECT A.MRH_ID, A.MR_Number, A.MR_Date, A.req_date, A.latest_date, A.MR_Class, A.MR_Type, A.proj_ID, A.proj_Code, A.MR_DepID, A.MR_EmpID, A.MR_Notes, A.MR_Status, 
				A.Approval_Status, A.Approve_Date, A.Patt_Year, A.Patt_Month, A.Patt_Date, A.Patt_Number, B.First_Name, B.Middle_Name, B.Last_Name, 
				C.proj_ID,C.proj_Number, C.proj_Code, C.proj_Name
