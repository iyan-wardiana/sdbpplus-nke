		...
		*/
		...
		if($TRANS_CATEG == 'IR')				// IR = Item Receipt // no notes
		{
			...
			// START : UPDATE L/R
				...
				$ITM_NOTES 	= htmlentities($ITM_NOTES, ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401 | ENT_HTML5);
				$IR_NOTE 	= htmlentities($IR_NOTE, ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401 | ENT_HTML5);
					...
					// START : 04-07-2018
						...
						if($resCL_D > 0)
						{
							...
							foreach($resL_D as $rowL_D):
								...
								$Other_Desc2 	= "$SPLDESC. $ITM_NOTES. $REFNUM";
								$Other_Desc 	= htmlentities($Other_Desc2, ENT_QUOTES | ENT_SUBSTITUTE | ENT_HTML401 | ENT_HTML5);
									...
									if($resCGEJ == 0)
									{
											$sqlGEJDD = "INSERT INTO tbl_journaldetail (JournalH_Code, Acc_Id, proj_Code, Currency_id, 
														...
														VALUES ('$JournalH_Code', '$ACC_NUM', '$proj_Code', 'IDR', 
															...
															'$JOBCODEID', $JRNPatt, '$Other_Desc', '$IR_NOTE', '$Acc_Name', '$proj_CodeHO')";
		...
		elseif($TRANS_CATEG == 'IR-OVH')		// IR = Item Receipt Overhead
		{
			...
			// ---------------- START : Pembuatan Journal Detail ----------------		
				...
				// GET PATT NUMBER
					$sqlJRN_C	= "tbl_journaldetail_$PRJCODEVW";
					$resJRN_C	= $this->db->count_all($sqlJRN_C);
					$JRNPatt	= $resJRN_C + 1;
				
				// ------------------------------- D E B I T : BEBAN -------------------------------
					// START : 04-30-2022
							$sqlL_D	= "SELECT ACC_ID_UM FROM tbl_item_$PRJCODEVW WHERE PRJCODE = '$proj_Code' AND ITM_CODE = '$Item_Code'";
							$resL_D = $this->db->query($sqlL_D)->result();
							foreach($resL_D as $rowL_D):
		...
		elseif($TRANS_CATEG == 'BP-NEW')		// BP = Bank Payment
		{
			...
			$INV_AMOUNT			= $parameters['INV_AMOUNT'];
			$INV_AMOUNT_PPN 	= $parameters['TAXPRICE1'];
			...
			// ---------------- START : Pembuatan Journal Detail ----------------
				...
				$PPn_PAYSTAT 	= 0;
				$PPhFin_PAYSTAT = 0;
				if($INV_AMOUNT_PPH > 0 || $INV_AMOUNT_PPN > 0)
				{
					$TAXCODE_PPH 	= "NOT_SET_ACCPPH";
					if($CB_SOURCE == 'VCASH')
					{
						// $s_PPH	= "SELECT PPn_PAYSTAT, PPhFin_PAYSTAT, PPH_Code FROM tbl_journaldetail_vcash WHERE JournalH_Code = '$Faktur_No' AND (PPH_Code IS NOT NULL AND PPH_Code != '')";
						$s_PPH	= "SELECT PPn_PAYSTAT, PPhFin_PAYSTAT, PPH_Code FROM tbl_journaldetail_vcash WHERE JournalH_Code = '$Faktur_No' AND PPH_Code != ''";
						$r_PPH 	= $this->db->query($s_PPH)->result();					
						foreach($r_PPH as $rw_PPH):
							$PPn_PAYSTAT	= $rw_PPH->PPn_PAYSTAT;
							$PPhFin_PAYSTAT	= $rw_PPH->PPhFin_PAYSTAT;
							$TAXCODE_PPH	= $rw_PPH->PPH_Code;
						endforeach;
						if($TAXCODE_PPH == '') $TAXCODE_PPH = $TAXCODE2;
					}
					elseif($CB_SOURCE == 'PPD')
					{
						// $s_PPH	= "SELECT PPn_PAYSTAT, PPhFin_PAYSTAT, PPH_Code FROM tbl_journaldetail_pd WHERE JournalH_Code = '$Faktur_No' AND (PPH_Code IS NOT NULL AND PPH_Code != '')";
						$s_PPH	= "SELECT PPn_PAYSTAT, PPhFin_PAYSTAT, PPH_Code FROM tbl_journaldetail_pd WHERE JournalH_Code = '$Faktur_No' AND PPH_Code != ''";
						$r_PPH 	= $this->db->query($s_PPH)->result();					
						foreach($r_PPH as $rw_PPH):
							$PPn_PAYSTAT	= $rw_PPH->PPn_PAYSTAT;
							$PPhFin_PAYSTAT	= $rw_PPH->PPhFin_PAYSTAT;
					...
					{
						$s_PPH	= "SELECT PPn_PAYSTAT, PPhFin_PAYSTAT, TAXCODE_PPH FROM tbl_pinv_detail WHERE INV_NUM = '$Faktur_No'";
						$r_PPH 	= $this->db->query($s_PPH)->result();					
						foreach($r_PPH as $rw_PPH):
							$PPn_PAYSTAT	= $rw_PPH->PPn_PAYSTAT;
							$PPhFin_PAYSTAT	= $rw_PPH->PPhFin_PAYSTAT;
					...
					elseif($CB_SOURCE == 'DP')
					{
						$s_PPH	= "SELECT PPn_PAYSTAT, PPhFin_PAYSTAT, TAXCODE_PPH FROM tbl_dp_header WHERE DP_NUM = '$Faktur_No'";
						$r_PPH 	= $this->db->query($s_PPH)->result();					
						foreach($r_PPH as $rw_PPH):
							$PPn_PAYSTAT	= $rw_PPH->PPn_PAYSTAT;
							$PPhFin_PAYSTAT	= $rw_PPH->PPhFin_PAYSTAT;
					...
					else
					{
						...
						// $s_PPH	= "SELECT TAXCODE_PPN, PPn_PAYSTAT, TAXCODE_PPH, PPhFin_PAYSTAT FROM tbl_pinv_detail WHERE INV_NUM = '$Faktur_No'";
						// $r_PPH 	= $this->db->query($s_PPH)->result();					
						// foreach($r_PPH as $rw_PPH):
						// 	$TAXCODE_PPN	= $rw_PPH->TAXCODE_PPN;
						// 	$PPn_PAYSTAT	= $rw_PPH->PPn_PAYSTAT;
						// 	$TAXCODE_PPH	= $rw_PPH->TAXCODE_PPH;
						// 	$PPhFin_PAYSTAT	= $rw_PPH->PPhFin_PAYSTAT;
						// endforeach;
				...
				// $transacValue 	= $transacValueD+$POT_PPHFIN+$AMOUNT_POT_DP;
				// $transacValue 	= $transacValueD-$INV_AMOUNT_PPN+$POT_PPHFIN;
				// UPDATED ON 22/05/02023 - MENGHILANGKAN FAKTOR PENGURANG PPN
				$transacValue 	= $transacValueD+$POT_PPHFIN;
				if($PPn_PAYSTAT == 0) $transacValue 	= $transacValueD-$INV_AMOUNT_PPN+$POT_PPHFIN;
				...
				// ------------------------------- D E B I T -------------------------------
					...
					if($resCL_D > 0)
					{
						...
						foreach($resL_D as $rowL_D):
							...
							// CEK APAKAH SUDAH ADA JOURNAL SEBELUMNYA DENGAN KODE = JournalH_Code
								...
								if($INV_AMOUNT_PPN > 0 && $PPn_PAYSTAT == 0)
								{
									$transacValue = $INV_AMOUNT_PPN;
									...
									$sqlGEJDD = "INSERT INTO tbl_journaldetail (JournalH_Code, Acc_Id, proj_Code, Currency_id, 
												...
												VALUES ('$JournalH_Code', '$ACC_NUM', '$proj_Code', 'IDR', 
													$transacValue, $transacValue, $transacValue, 'Default', 1, 0,
													'D', 'Hut. PPn - $Other_Desc', '$Manual_No', '$Ref_Number', '$Faktur_No', '$Faktur_Code', '$Acc_Name', '$proj_CodeHO',
													'$SPLCODE', '$SPLDESC')";
									$this->db->query($sqlGEJDD);

									// START: UPDATE PPn_PAYSTAT
										if($CB_SOURCE == 'VCASH')
										{
											$s_01c		= "UPDATE tbl_journaldetail_vcash SET PPn_PAYSTAT = 1 
															WHERE JournalH_Code = '$Faktur_No' AND PPN_Code = '$TAXCODE1'";
											$this->db->query($s_01c);
										}
										elseif($CB_SOURCE == 'PPD')
										{
											$s_01c		= "UPDATE tbl_journaldetail_pd SET PPn_PAYSTAT = 1 
															WHERE JournalH_Code = '$Faktur_No' AND PPN_Code = '$TAXCODE1'";
											$this->db->query($s_01c); 
										}
										elseif($CB_SOURCE == 'PINV')
										{
											$s_01b		= "UPDATE tbl_pinv_detail SET PPn_PAYSTAT = 1 WHERE INV_NUM = '$Faktur_No'";
											$this->db->query($s_01b);
										}
										elseif($CB_SOURCE == 'DP')
										{
											$s_01b		= "UPDATE tbl_dp_header SET PPn_PAYSTAT = 1 WHERE DP_NUM = '$Faktur_No'";
											$this->db->query($s_01b);
										}
									// END: UPDATE PPn_PAYSTAT
					...
					if($AMOUNT_POT_DP > 0)
					{
						...
						// Pengeualian hasil migrasi berdasarkan hasil diskusi dengan pak luki tgl. 03-03-2023
						if($PRJCODE == '478020' || $PRJCODE == '480020' || $PRJCODE == '535021' || $PRJCODE == '536021' || $PRJCODE == '524D90' || $PRJCODE == '533020' || $PRJCODE == '532020' || $PRJCODE == '361090')
							$ACC_IDUM = "1107.01.001";
						// End -----
				...
				// ------------------------------- K R E D I T -------------------------------
					/* START : POT. UM => Nilai DPP sudah dipotong UM (hidden) 06-03-2023 --------------------------- 
						if($AMOUNT_POT_DP > 0)
						{
							$DP_CODE 	= "";
							$OPSPK_CODE = "";
							...
							$sqlGEJDD = "INSERT INTO tbl_journaldetail (JournalH_Code, Acc_Id, proj_Code, Currency_id, 
											JournalD_Kredit, Base_Kredit, COA_Debet, CostCenter, curr_rate, isDirect, 
											Journal_DK, Other_Desc, Manual_No, Ref_Number, Faktur_No, Faktur_Code, Acc_Name, proj_CodeHO,
											SPLCODE, SPLDESC)
										VALUES ('$JournalH_Code', '$ACC_IDUM', '$proj_Code', 'IDR', 
											$AMOUNT_POT_DP, $AMOUNT_POT_DP, $AMOUNT_POT_DP, 'Default', 1, 0,
											'K', '$Other_Desc Potongan DP $DP_CODE - $OPSPK_CODE', '$Manual_No', '$Ref_Number', '$Faktur_No', '$Faktur_Code', '$Acc_Name', '$proj_CodeHO',
											'$SPLCODE', '$SPLDESC')";
							$this->db->query($sqlGEJDD);
							...
							// START : Update to COA - Debit
								...
								if($jmD > 0)
								{
									...
									{
										$SYNC_PRJ	= $dataPecah[$i];
										$sqlUpdCOA	= "UPDATE tbl_chartaccount SET Base_Kredit = Base_Kredit+$AMOUNT_POT_DP,
															Base_Kredit2 = Base_Kredit2+$AMOUNT_POT_DP, BaseK_$accYr = BaseK_$accYr+$AMOUNT_POT_DP
														WHERE PRJCODE = '$SYNC_PRJ' AND Account_Number = '$ACC_IDUM'";
										$this->db->query($sqlUpdCOA);
									}
								}
					...
					// START : 03-08-2022
						if($INV_AMOUNT_PPH > 0 && $isPPhFin == 1 && $PPhFin_PAYSTAT == 0)			// 0 PPH NON-FINAL 1 PPH FINAL
						{
							// CEK APAKAH SUDAH ADA JOURNAL SEBELUMNYA DENGAN KODE = JournalH_Code
								$sqlCGEJ	= "tbl_journaldetail WHERE JournalH_Code = '$JournalH_Code' AND Journal_DK = 'K'
								...
								//{
										...
										// START: UPDATE PPhFin_PAYSTAT
											if($CB_SOURCE == 'VCASH')
											{
												$s_01c		= "UPDATE tbl_journaldetail_vcash SET PPhFin_PAYSTAT = 1 
																WHERE JournalH_Code = '$Faktur_No' AND PPH_Code = '$TAXCODE_PPH'";
												$this->db->query($s_01c);
											}
											elseif($CB_SOURCE == 'PPD')
											{
												$s_01c		= "UPDATE tbl_journaldetail_pd SET PPhFin_PAYSTAT = 1 
																WHERE JournalH_Code = '$Faktur_No' AND PPH_Code = '$TAXCODE_PPH'";
												$this->db->query($s_01c); 
											}
											elseif($CB_SOURCE == 'PINV')
											{
												$s_01b		= "UPDATE tbl_pinv_detail SET PPhFin_PAYSTAT = 1 WHERE INV_NUM = '$Faktur_No'";
												$this->db->query($s_01b);
											}
											elseif($CB_SOURCE == 'DP')
											{
												$s_01b		= "UPDATE tbl_dp_header SET PPhFin_PAYSTAT = 1 WHERE DP_NUM = '$Faktur_No'";
												$this->db->query($s_01b);
											}
										// END: UPDATE PPhFin_PAYSTAT
								/*}
								else
								{
										$sqlUpdCOAD	= "UPDATE tbl_journaldetail SET 
		...
		elseif($TRANS_CATEG == 'BP-ACC_230301')		// BP = Bank Payment PPD
		{
			$SPLCATEG			= $data[1];
			$Manual_No 			= $parameters['Manual_No'];
			$Ref_Number			= $parameters['Ref_Number'];
			...
			// ---------------- START : Pembuatan Journal Detail ----------------
				...
				// ------------------------------- START : D E B I T - K R E D I T -------------------------------
					...
					if($Journal_DK == 'D')
					{
						...
						// CEK APAKAH SUDAH ADA JOURNAL SEBELUMNYA DENGAN KODE = JournalH_Code
						/*
						$sqlCGEJ	= "tbl_journaldetail WHERE JournalH_Code = '$JournalH_Code' AND Journal_DK = 'D' AND Acc_Id = '$ACC_NUM'";
						$resCGEJ	= $this->db->count_all($sqlCGEJ);
						...
						------------------ Pembentukan jurnal di creat 1x pada saat approve ------------------- */
					...
					{
						$isTax = 0;
						$s_tax = "tbl_tax_la WHERE TAXLA_LINKOUT = '$ACC_NUM'";
						$r_tax = $this->db->count_all($s_tax);
						if($r_tax > 0) $isTax = 1;

						$insSQL	= "INSERT INTO tbl_journaldetail (JournalH_Code, Acc_Id, Acc_Id_Cross, JournalType, proj_Code,
										Currency_id, JournalD_Kredit, Base_Kredit, COA_Kredit, curr_rate, isDirect, Manual_No,
										ITM_VOLM, ITM_PRICE, Ref_Number, Other_Desc, Journal_DK, Journal_Type, isTax,
										proj_CodeHO, Acc_Name, ISPERSL) VALUES
										('$JournalH_Code', '$ACC_NUM', '$ACC_BANK', 'BP', '$proj_Code',
										'IDR', $transacValue, $transacValue, $transacValue, 1, 1, '$Manual_No',
										1, '$transacValue', '$Ref_Number', '$Other_Desc', 'K', 'BP', $isTax, 
										'$proj_CodeHO', '$Acc_Name', 0)";
						$this->db->query($insSQL);
						...
						else
						{
								$sqlUpdCOAD	= "UPDATE tbl_journaldetail SET JournalD_Kredit = JournalD_Kredit+$transacValue,
												...
												WHERE JournalH_Code = '$JournalH_Code' AND Journal_DK = 'K' AND Faktur_No = '$Faktur_No'
													AND Journal_Type = 'NTAX' AND Acc_Id = '$ACC_NUM'";
								$this->db->query($sqlUpdCOAD);
						}
						------------------ Pembentukan jurnal di creat 1x pada saat approve ------------------- */
		...
		elseif($TRANS_CATEG == 'BPDP')		// BPDP = Pembayaran Down Payment
		{
			...
			$DP_CODE 		= "";
			if(isset($parameters['Faktur_Code']))
				$DP_CODE 	= $parameters['Faktur_Code'];
			...
			$TTKT_SPLINVDATE 	= "";
			$TTKT_SPLINVNO 		= "";
			$s_DP	= "SELECT DP_DATE, DP_REFCODE, DP_NOTES, PPn_PAYSTAT, PPhFin_PAYSTAT, TTK_NUM FROM tbl_dp_header WHERE DP_NUM = '$DP_NUM'";
			$r_DP = $this->db->query($s_DP)->result();					
			foreach($r_DP as $rw_DP):
				$DP_DATE		= $rw_DP->DP_DATE;
				$DP_REFCODE		= $rw_DP->DP_REFCODE;
				$DP_NOTES		= $rw_DP->DP_NOTES;
				$PPn_PAYSTAT 	= $rw_DP->PPn_PAYSTAT;
				$PPhFin_PAYSTAT = $rw_DP->PPhFin_PAYSTAT;
			...
			// $TTKT_SPLINVDATE 	= "";
			// $TTKT_SPLINVNO 		= "";
			// $s_DP 			= "SELECT TTK_NUM, PPn_PAYSTAT FROM tbl_dp_header WHERE DP_NUM = '$DP_NUM' AND PRJCODE = '$proj_Code'";
			// $r_DP 			= $this->db->query($s_DP)->result();
			// foreach($r_DP as $rw_DP) :
			// 	$TTK_NUM 		= $rw_DP->TTK_NUM;
			// 	$PPn_PAYSTAT 	= $rw_DP->PPn_PAYSTAT;
			...
			// ---------------- START : Pembuatan Journal Detail ----------------
				...
				// $transacValue	= $transacValuex + $AMOUNT_PPN - $AMOUNT_PPH;
				$transacValuex 	= $TOT_PAYMENT;
				if($PPn_PAYSTAT == 0 || $PPhFin_PAYSTAT == 0) $transacValuex 	= $TOT_PAYMENT+$AMOUNT_PPH-$AMOUNT_PPN;
				...
				// ------------------------------- START : D E B I T -------------------------------
					...
					if($resCL_D > 0)
					{
						...
						foreach($resL_D as $rowL_D):
							...
							$sqlGEJDD = "INSERT INTO tbl_journaldetail (JournalH_Code, Acc_Id, proj_Code, Currency_id, 
											JournalD_Debet, Base_Debet, COA_Debet, Faktur_No, Faktur_Code, Faktur_Date, CostCenter, curr_rate, isDirect, 
											Journal_DK, Other_Desc, Acc_Name, proj_CodeHO,
											TAX_NO, TAX_DATE, Kwitansi_No, Kwitansi_Date, SPLCODE, SPLDESC)
										VALUES ('$JournalH_Code', '$ACC_NUM', '$proj_Code', 'IDR', $transacValuex, 
											$transacValuex, $transacValuex, '$DP_NUM', '$DP_CODE', '$DP_DATE', 'Default', 1, 0, 'D', '$SPLDESC. $DP_NOTES ($DP_REFCODE)', '$Acc_Name', '$proj_CodeHO',
				...
				// ------------------------------- START : K R E D I T -------------------------------
				// PPh Final & non Final terbentuk saat pembayaran DP => PPhFin = PPhNonFin
				if($AMOUNT_PPH > 0 && $PPhFin_PAYSTAT == 0)
				{
					$ACC_NUM 	= '';
					$s_ACC 		= "SELECT TAXLA_LINKIN FROM tbl_tax_la WHERE TAXLA_NUM = '$ACCCODE_PPH' LIMIT 1";
					$r_ACC		= $this->db->query($s_ACC)->result();
					...
					if($ACC_NUM != '')
					{
						...
						// START : Save Journal Detail - Kredit
							$sqlGEJDK = "INSERT INTO tbl_journaldetail (JournalH_Code, Acc_Id, proj_Code, Currency_id, JournalD_Kredit,
											Base_Kredit, COA_Kredit, isTax, CostCenter, curr_rate, isDirect, Journal_DK, Acc_Name, proj_CodeHO,
											Other_Desc, SPLCODE, SPLDESC)
										VALUES ('$JournalH_Code', '$ACC_NUM', '$proj_Code', 'IDR', $AMOUNT_PPH, $AMOUNT_PPH,
											$AMOUNT_PPH, 1, 'Default', 1, 0, 'K', '$Acc_Name', '$proj_CodeHO',
											'$Other_Desc','$SPLCODE','$SPLDESC')";
							$this->db->query($sqlGEJDK);
						// END : Save Journal Detail - Kredit
		...
		elseif($TRANS_CATEG == 'UM')			// UM = Use Material
		{
			...
			// START : UPDATE L/R
				...
				$ITM_TYPE 	= $parameters['ITM_TYPE'];
				$Notes 		= $parameters['Notes'];
				$ACC_ID_IR 	= $parameters['ACC_ID_IR'];
				$ACC_ID_UM 	= $parameters['ACC_ID_UM'];
				$ITM_CATEG 	= $parameters['ITM_CATEG'];
				$ITM_LR 	= $parameters['ITM_LR'];
			...
			// PEREKAMAN JEJAK KE tbl_itemhistory MIN
				$sqlHist 		= "INSERT INTO tbl_itemhistory (JournalH_Code, proj_Code, Transaction_Date, Item_Code, Qty_Plus, Qty_Min, 
										QtyRR_Plus, QtyRR_Min, Transaction_Type, Transaction_Value, Company_ID, Currency_ID,
										JOBCODEID, GEJ_STAT, ItemPrice, ItemCategoryType, MEMO)
			...
			// ---------------- START : Pembuatan Journal Detail ----------------
				...
				// ------------------------------- D E B I T -------------------------------
					...
					// START : INSERT DEBET
						$Acc_Name 	= "-";
						$sqlNm 		= "SELECT Account_NameId FROM tbl_chartaccount_$PRJCODEVW WHERE Account_Number = '$ACC_ID_UM' LIMIT 1";
						$resNm		= $this->db->query($sqlNm)->result();
						foreach($resNm as $rowNm):
							$Acc_Name	= $rowNm->Account_NameId;
						endforeach;
						...
						$sqlGEJDD = "INSERT INTO tbl_journaldetail (JournalH_Code, Acc_Id, proj_Code, 
									...
									VALUES ('$JournalH_Code', '$ACC_ID_UM', '$proj_Code', 
				...
				// ------------------------------- K R E D I T -------------------------------
					// START : INSERT KREDIT
						$Acc_Name 	= "-";
						$sqlNm 		= "SELECT Account_NameId FROM tbl_chartaccount_$PRJCODEVW WHERE Account_Number = '$ACC_ID_IR' LIMIT 1";
						$resNm		= $this->db->query($sqlNm)->result();
						foreach($resNm as $rowNm):
							$Acc_Name	= $rowNm->Account_NameId;
						endforeach;
						...
						// CEK APAKAH SUDAH ADA JOURNAL SEBELUMNYA DENGAN KODE = JournalH_Code
							$sqlCGEJ	= "tbl_journaldetail_$PRJCODEVW WHERE JournalH_Code = '$JournalH_Code' AND Journal_DK = 'K' 
											AND Journal_Type = 'NTAX' AND Acc_Id = '$ACC_ID_IR'";
							$resCGEJ	= $this->db->count_all($sqlCGEJ);
							...
							if($resCGEJ == 0)
							{
									$sqlGEJDD = "INSERT INTO tbl_journaldetail (JournalH_Code, Acc_Id, proj_Code, Currency_id,
												...
												VALUES ('$JournalH_Code', '$ACC_ID_IR', '$proj_Code', 'IDR', $transacValue, 
													$transacValue,$transacValue, 'Default', 1, 0, 'K',
		...
		elseif($TRANS_CATEG == 'UM_TSF')		// UM = Use Material by Other Project
		{
			$curr_rate = 1; // Default IDR ke IDR
			// START : UPDATE L/R
				$PRJCODE1 	= $parameters['PRJCODE1'];
				$PRJCODE 	= $parameters['PRJCODE2'];
				$PRJCODEVW 	= strtolower(preg_replace("/[^a-zA-Z0-9\s]/", "", $PRJCODE));
				$PERIODM	= date('m', strtotime($JournalH_Date));
				$PERIODY	= date('Y', strtotime($JournalH_Date));
				$JRNH_CODE 	= $parameters['JournalH_Code']."IR";
				$JRNH_DATE 	= $parameters['JournalH_Date'];
				$ITM_CODE 	= $parameters['ITM_CODE'];
				$ITM_VOLM 	= $parameters['ITM_QTY'];
				$ITM_PRICE 	= $parameters['ITM_PRICE'];
				$JOBCODEID 	= $parameters['JOBCODEID'];
				...
				$ITM_CATEG 	= $parameters['ITM_CATEG'];
				$ITM_LR 	= $parameters['ITM_LR'];
				$WHCODE 	= $parameters['WHCODE'];
				$TSF_VALUE 	= $ITM_VOLM * $ITM_PRICE;
			...
			// PEREKAMAN JEJAK KE tbl_itemhistory PLUS
				$sqlHist 		= "INSERT INTO tbl_itemhistory (JournalH_Code, proj_Code, Transaction_Date, Item_Code, Qty_Plus, Qty_Min, 
										QtyRR_Plus, QtyRR_Min, Transaction_Type, Transaction_Value, Company_ID, Currency_ID,
										JOBCODEID, GEJ_STAT, JournalD_Id, ItemPrice, ItemCategoryType, MEMO)
									VALUES ('$JRNH_CODE', '$PRJCODE', '$JRNH_DATE', '$ITM_CODE', $ITM_VOLM, 0, 
										$ITM_VOLM, 0, '$JSource', $TSF_VALUE, '$Company_ID', '$Currency_ID', 
										'$JOBCODEID', 3, $JRNPatt, '$ITM_PRICE', '$ITM_CATEG', 'Transfer Proyek. $Notes - $Manual_No')";
				$this->db->query($sqlHist);
			
			// UPDATE DETAIL
				$sqlUpDet	= "UPDATE tbl_item SET ITM_VOLM = ITM_VOLM + $ITM_VOLM, ITM_PRICE = $ITM_PRICE, ITM_TOTALP = ITM_TOTALP + $TSF_VALUE,
									ITM_REMQTY = ITM_REMQTY + $ITM_VOLM, ITM_LASTP = $ITM_PRICE, 
									ITM_IN = ITM_IN + $ITM_VOLM, ITM_INP = ITM_INP + $TSF_VALUE,
									TSFMTR_VOL = TSFMTR_VOL + $ITM_VOLM, TSFMTR_VAL = TSFMTR_VAL + $TSF_VALUE, LAST_TRXNO = '$JRNH_CODE'
								WHERE ITM_CODE = '$ITM_CODE' AND PRJCODE = '$PRJCODE'";	
				$this->db->query($sqlUpDet);
				
			// UPDATE WH_QTY
				$sqlWHC	= "tbl_item_whqty WHERE ITM_CODE = '$ITM_CODE' AND PRJCODE = '$PRJCODE' AND WH_CODE = '$WHCODE'";
				$resWHC	= $this->db->count_all($sqlWHC);
				
				if($resWHC > 0)
				{
					$sqlUpWH	= "UPDATE tbl_item_whqty SET ITM_VOLM = ITM_VOLM + $ITM_VOLM,
										ITM_IN = ITM_IN + $ITM_VOLM, ITM_INP = ITM_INP + $TSF_VALUE,
										TSFMTR_VOL = TSFMTR_VOL + $ITM_VOLM, TSFMTR_VAL = TSFMTR_VAL + $TSF_VALUE
									WHERE ITM_CODE = '$ITM_CODE' AND PRJCODE = '$PRJCODE' AND WH_CODE = '$WHCODE'";
					$this->db->query($sqlUpWH);
				}

			// UPDATE JOBD DETAIL
				$sqlUpDetJ		= "UPDATE tbl_joblist_detail SET ITM_STOCK = ITM_STOCK + $ITM_VOLM, ITM_STOCK_AM = ITM_STOCK_AM + $TSF_VALUE,
										TSFMTR_VOL = TSFMTR_VOL + $ITM_VOLM, TSFMTR_VAL = TSFMTR_VAL + $TSF_VALUE
			...
			// ---------------- START : Pembuatan Journal Detail ----------------
				...
				// ------------------------------- D E B I T -------------------------------
					...
					// START : INSERT DEBET
						...
						$sqlGEJDD = "INSERT INTO tbl_journaldetail (JournalH_Code, JournalH_Date, JournalType, Acc_Id, proj_Code, 
										Currency_id, JournalD_Debet, Base_Debet, COA_Debet, CostCenter, 
										curr_rate, isDirect, Journal_DK, ITM_CODE, ITM_GROUP, ITM_CATEG, ITM_VOLM, ITM_PRICE,
										JOBCODEID, Acc_Name, proj_CodeHO, GEJ_STAT, Manual_No, Other_Desc)
									VALUES ('$JRNH_CODE', '$JRNH_DATE', 'IR-TSF', '$ACC_ID_IR', '$PRJCODE', 
										'IDR', $TSF_VALUE, $TSF_VALUE, $TSF_VALUE, 'Default',
										1, 0, 'D', '$ITM_CODE', '$ITM_GROUP','$ITM_CATEG', $ITM_VOLM, $ITM_PRICE,
										'$JOBCODEID', '$Acc_Name', '$proj_CodeHO', 3, '$Manual_No', '$Notes. Penerimaan transfer material dari proyek $PRJCODE1')";
						$this->db->query($sqlGEJDD);
						...
						// START : Update to COA - Debit
							...
							if($jmD > 0)
							{
								...
								{
									$SYNC_PRJ	= $dataPecah[$i];
									$sqlUpdCOA	= "UPDATE tbl_chartaccount SET Base_Debet = Base_Debet+$TSF_VALUE,
														Base_Debet2 = Base_Debet2+$TSF_VALUE, BaseD_$accYr = BaseD_$accYr+$TSF_VALUE
													WHERE PRJCODE = '$SYNC_PRJ' AND Account_Number = '$ACC_ID_IR'";
									$this->db->query($sqlUpdCOA);
								}
							}
				...
				// ------------------------------- K R E D I T -------------------------------
					// START : INSERT KREDIT
						// SEMENTARA DISETT KE 7105.03 PENDAPATAN LAIN DILUAR JASA KONSTRUKSI KARENA JURNAL INI NANTINYA TIDAK AKAN DIBUATKAN JURNAL PEMBALIK
						$Acc_Name 	= "PENDAPATAN LAIN DILUAR JASA KONSTRUKSI";
						$ACC_ID_IRK = "7105.03";
						/*$sqlNm 		= "SELECT Account_NameId FROM tbl_chartaccount_$PRJCODEVW WHERE Account_Number = '$ACC_ID_IR' LIMIT 1";
						$resNm		= $this->db->query($sqlNm)->result();
						foreach($resNm as $rowNm):
						...
						// CEK APAKAH SUDAH ADA JOURNAL SEBELUMNYA DENGAN KODE = JournalH_Code
							$sqlCGEJ	= "tbl_journaldetail_$PRJCODEVW WHERE JournalH_Code = '$JRNH_CODE' AND Journal_DK = 'K' 
											AND Journal_Type = 'NTAX' AND Acc_Id = '$ACC_ID_IRK'";
							$resCGEJ	= $this->db->count_all($sqlCGEJ);
							
							if($resCGEJ == 0)
							{
									$sqlGEJDD = "INSERT INTO tbl_journaldetail (JournalH_Code, JournalH_Date, JournalType, Acc_Id, proj_Code, Currency_id,
													JournalD_Kredit, Base_Kredit, COA_Kredit, CostCenter, curr_rate, 
													isDirect, Journal_DK, ITM_CODE, ITM_GROUP, ITM_CATEG, Other_Desc, Acc_Name, proj_CodeHO)
												VALUES ('$JRNH_CODE', '$JRNH_DATE', 'IR-TSF', '$ACC_ID_IRK', '$PRJCODE', 'IDR',
													$TSF_VALUE, $TSF_VALUE,$TSF_VALUE, 'Default', 1, 0, 'K',
													'$Item_Code', '$ITM_GROUP', '$ITM_CATEG', '$Notes. Penerimaan transfer material dari proyek $PRJCODE1', '$Acc_Name', '$proj_CodeHO')";
									$this->db->query($sqlGEJDD);
							}
							else
							{
									$sqlUpdCOAD	= "UPDATE tbl_journaldetail SET JournalD_Kredit = JournalD_Kredit+$TSF_VALUE,
														Base_Kredit = Base_Kredit+$TSF_VALUE, 
														COA_Kredit = COA_Kredit+$TSF_VALUE
													WHERE JournalH_Code = '$JRNH_CODE' AND Journal_DK = 'K' 
														AND Journal_Type = 'NTAX' AND Acc_Id = '$ACC_ID_IRK'";
									$this->db->query($sqlUpdCOAD);
							}
						...
						// START : Update to COA - Debit
							...
							$syncPRJ		= '';
							$sqlISHO 		= "SELECT isHO, syncPRJ FROM tbl_chartaccount_$PRJCODEVW
												WHERE PRJCODE = '$PRJCODE' AND Account_Number = '$ACC_ID_IRK' LIMIT 1";
							$resISHO		= $this->db->query($sqlISHO)->result();
							foreach($resISHO as $rowISHO):
								$isHO		= $rowISHO->isHO;
								$syncPRJ	= $rowISHO->syncPRJ;
							...
							if($jmD > 0)
							{
								...
								{
									$SYNC_PRJ	= $dataPecah[$i];
									$sqlUpdCOA	= "UPDATE tbl_chartaccount SET Base_Kredit = Base_Kredit+$TSF_VALUE, 
														Base_Kredit2 = Base_Kredit2+$TSF_VALUE, BaseK_$accYr = BaseK_$accYr+$TSF_VALUE
													WHERE PRJCODE = '$SYNC_PRJ' AND Account_Number = '$ACC_ID_IRK'";
									$this->db->query($sqlUpdCOA);
								}
							}
						// END : Update to COA - Debit
			...
			// SYNC STAT AND DATE
				$sqlSYNCHD 	= "UPDATE tbl_journaldetail A, tbl_journalheader B SET A.JournalH_Date = B.JournalH_Date,
									A.GEJ_STAT = B.GEJ_STAT, A.LastUpdate = B.LastUpdate, A.JournalType = B.JournalType
								WHERE A.JournalH_Code  = B.JournalH_Code AND A.JournalH_Code = '$JRNH_CODE'";
				$this->db->query($sqlSYNCHD);
		}
