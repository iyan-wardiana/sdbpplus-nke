...
{	
	...
	function get_AllDataGRPC($PRJCODE, $SPLCODE, $search) // GOOD
	{
		$SPLQRY 	= "";
		if($SPLCODE != '')
			$SPLQRY = "AND A.SPLCODE = '$SPLCODE'";

		$sql = "tbl_pinv_header A
					INNER JOIN tbl_supplier B ON A.SPLCODE = B.SPLCODE
				WHERE A.PRJCODE = '$PRJCODE' $SPLQRY
					AND (A.INV_NUM LIKE '%$search%' ESCAPE '!' OR A.INV_CODE LIKE '%$search%' ESCAPE '!' 
					OR A.INV_DATE LIKE '%$search%' ESCAPE '!' OR A.INV_NOTES LIKE '%$search%' ESCAPE '!' 
					OR B.SPLDESC LIKE '%$search%' ESCAPE '!')";
		return $this->db->count_all($sql);
	...
	function get_AllDataGRPL($PRJCODE, $SPLCODE, $search, $length, $start, $order, $dir) // GOOD
	{
		$SPLQRY 	= "";
		if($SPLCODE != '')
			$SPLQRY = "AND A.SPLCODE = '$SPLCODE'";
	...
	{
		## Custom Field value
		$PRJCODEA 	= $this->input->get('PRJCODEA');
		$SPLCODE 	= $this->input->get('SPLCODE');
		$DOC_NUM 	= $this->input->get('DOC_NUM');
		...
		if($SPLCODE != '')
		{
			$addQuery .= " AND B.SPLCODE = '".$SPLCODE."'";
		}

		if($DOC_NUM != '')
		{
			$TTKNUM 	= "";
			$s_DOCREF 	= "SELECT DISTINCT TTK_NUM FROM tbl_ttk_detail WHERE TTK_REF1_NUM = '$DOC_NUM'";
			$r_DOCREF 	= $this->db->query($s_DOCREF);
			if($r_DOCREF->num_rows() > 0)
			{
				$arrTTKNUM 	= [];
				foreach($r_DOCREF->result() as $rw_DOCREF):
					$arrTTKNUM[] = $rw_DOCREF->TTK_NUM;
				endforeach;
				$TTKNUM 	= join("','", $arrTTKNUM);
				$addQuery .= " AND TTK_NUM IN ('$TTKNUM')";
			}
		}

		$sql = "tbl_ttk_header A
					LEFT JOIN tbl_supplier B ON A.SPLCODE = B.SPLCODE
				WHERE A.PRJCODE = '$PRJCODE' $addQuery
	...
	function get_AllDataTTKGRPC($PRJCODE, $SPLCODE, $search) // GOOD
	{
		$SPLQRY 	= "";
		if($SPLCODE != '')
			$SPLQRY = "AND A.SPLCODE = '$SPLCODE'";
	...
	function get_AllDataTTKGRPL($PRJCODE, $SPLCODE, $search, $length, $start, $order, $dir) // GOOD
	{
		$SPLQRY 	= "";
		if($SPLCODE != '')
			$SPLQRY = "AND A.SPLCODE = '$SPLCODE'";
	...
	function get_AllDataCTTK($PRJCODE, $SPLCODE, $search) // GOOD
	{
		$sql = "tbl_ttk_header A
					INNER JOIN tbl_supplier B ON A.SPLCODE = B.SPLCODE
				WHERE A.PRJCODE = '$PRJCODE' AND A.SPLCODE = '$SPLCODE' AND A.INV_CREATED = 0 AND A.TTK_STAT NOT IN (5,9)
					AND (A.TTK_CODE LIKE '%$search%' ESCAPE '!' OR A.TTK_DATE LIKE '%$search%' ESCAPE '!' 
					OR A.TTK_NOTES LIKE '%$search%' ESCAPE '!' OR B.SPLDESC LIKE '%$search%' ESCAPE '!'
					OR A.STATDESC LIKE '%$search%' ESCAPE '!')";
		return $this->db->count_all($sql);
	...
	function updOPN($OPN_NUM, $updOPN)
	{
		...
		$this->db->where('OPNH_TYPE', 0);
		$this->db->update('tbl_opn_header', $updOPN);
	}

	function updOPNRET($OPN_NUM, $updOPN)
	{
		$this->db->where('OPNH_NUM', $OPN_NUM);
		$this->db->where('OPNH_TYPE', 1);
		$this->db->update('tbl_opn_header', $updOPN);
	}
	...
	function get_AllDataIRC($PRJCODE, $SPLCODE, $PONUM, $search) // GOOD
	{
		$sql 	= "tbl_ir_header A
					WHERE A.IR_STAT = 3
						AND A.SPLCODE = '$SPLCODE'
	...
	function get_AllDataIRL($PRJCODE, $SPLCODE, $PONUM, $search, $length, $start, $order, $dir) // GOOD
	{
		if($length == -1)
		{
			if($order !=null)
	...
	function get_AllDataIRSJC($PRJCODE, $SPLCODE, $PO_NUM, $search) // GOOD
	{
		$sql 	= "tbl_ir_detail A
	...
	function get_AllDataIRSJL($PRJCODE, $SPLCODE, $PO_NUM, $search, $length, $start, $order, $dir) // GOOD
	{
		if($length == -1)
		{
			if($order !=null)
			{
				$sql = "SELECT A.SJ_NUM, A.IR_NUM, A.IR_CODE, A.IR_DATE, B.IR_DUEDATE, B.IR_REFER, B.SPLCODE, A.PO_NUM, 
						A.ITM_TOTAL AS IR_AMOUNT, B.IR_NOTE, B.IR_NOTE2, B.SPLDESC, A.TAXCODE1 AS TAXCODE_PPN, A.TAXCODE2 AS TAXCODE_PPH,
						A.TAXPRICE1, A.TAXPRICE2, A.ITM_DISC
						FROM tbl_ir_detail A
						INNER JOIN tbl_ir_header B ON B.IR_NUM = A.IR_NUM AND B.PRJCODE = A.PRJCODE
						WHERE B.IR_STAT = 3
							AND B.SPLCODE = '$SPLCODE'
	...
	function get_AllDataWOC($PRJCODE, $SPLCODE, $search) // GOOD
	{
		$sql 	= "tbl_wo_header A
						INNER JOIN tbl_supplier B ON A.SPLCODE = B.SPLCODE
					WHERE PRJCODE = '$PRJCODE' AND A.SPLCODE = '$SPLCODE' AND A.TTK_CREATED = 0 AND A.WO_DPPER > 0
	...
	function get_AllDataPOC($PRJCODE, $SPLCODE, $search) // GOOD
	{
		$sql 	= "tbl_po_header A
						INNER JOIN tbl_supplier B ON A.SPLCODE = B.SPLCODE
					WHERE PRJCODE = '$PRJCODE' AND A.SPLCODE = '$SPLCODE' AND A.PO_DPPER > 0
	...
	function get_AllDataSPL_PDC($PRJCODE, $SPLCODE, $search) // GOOD
	{
		$sql 	= "tbl_journalheader_pd A
					WHERE A.Journal_AmountReal < A.Journal_Amount AND A.Journal_AmountTsf > 0
						AND A.GEJ_STAT = 3 AND A.SPLCODE = '$SPLCODE'
						AND (A.JournalH_Desc LIKE '%$search%' ESCAPE '!' OR A.Reference_Number LIKE '%$search%' ESCAPE '!' 
						OR A.PERSL_EMPID LIKE '%$search%' ESCAPE '!' OR A.Manual_No LIKE '%$search%' ESCAPE '!'
						OR A.REF_CODE LIKE '%$search%' ESCAPE '!' OR A.REF_PINV LIKE '%$search%' ESCAPE '!')";
		return $this->db->count_all($sql);
	}
	
	function get_AllDataSPL_PDL($PRJCODE, $SPLCODE, $search, $length, $start, $order, $dir) // GOOD
	{
		if($length == -1)
		{
			if($order !=null)
			{
				$sql = "SELECT A.*
						...
						WHERE A.Journal_AmountReal < A.Journal_Amount AND A.Journal_AmountTsf > 0
							...
							AND (A.JournalH_Desc LIKE '%$search%' ESCAPE '!' OR A.Reference_Number LIKE '%$search%' ESCAPE '!' 
							OR A.PERSL_EMPID LIKE '%$search%' ESCAPE '!' OR A.Manual_No LIKE '%$search%' ESCAPE '!'
							OR A.REF_CODE LIKE '%$search%' ESCAPE '!' OR A.REF_PINV LIKE '%$search%' ESCAPE '!') ORDER BY $order $dir";
			}
			else
			{
				$sql = "SELECT A.*
			...
			else
			{
				$sql = "SELECT A.*
						...
						WHERE A.Journal_AmountReal < A.Journal_Amount AND A.Journal_AmountTsf > 0
							...
							AND (A.JournalH_Desc LIKE '%$search%' ESCAPE '!' OR A.Reference_Number LIKE '%$search%' ESCAPE '!' 
							OR A.PERSL_EMPID LIKE '%$search%' ESCAPE '!' OR A.Manual_No LIKE '%$search%' ESCAPE '!'
							OR A.REF_CODE LIKE '%$search%' ESCAPE '!' OR A.REF_PINV LIKE '%$search%' ESCAPE '!') LIMIT $start, $length";
			}
			return $this->db->query($sql);
		}
	}
