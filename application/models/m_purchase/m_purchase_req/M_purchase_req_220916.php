...
class M_purchase_req extends CI_Model
{
	...
	function get_MR_by_number($PR_NUM) // G
	{
		$sql = "SELECT A.PR_NUM, A.PR_CODE, A.PR_DATE, A.PR_RECEIPTD, A.PRJCODE, A.SPLCODE, A.PR_DEPT, A.JOBCODE, A.JOBDESC,
					A.PR_NOTE, A.PR_NOTE2, A.PR_STAT, PR_MEMO, A.PR_VALUE, A.PR_VALUEAPP, A.PR_REFNO, A.PR_PLAN_IR, A.PR_DOC,
	...
	function updPR_DOC($PR_NUM, $PRJCODE, $getarr)
	{
		$PR_DOC = join(", ", $getarr);
		$this->db->set("PR_DOC", $PR_DOC);
		$this->db->where(["PR_NUM" => $PR_NUM, "PRJCODE" => $PRJCODE]);
		$this->db->update("tbl_pr_header");
	}
}
?>