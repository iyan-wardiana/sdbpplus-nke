...
class M_purchase_req extends CI_Model
{
			...
			if($order !=null)
			{
				$sql = "SELECT DISTINCT A.ORD_ID, A.JOBCODEDET, A.JOBCODEID, A.JOBPARENT, A.JOBCODE, A.JOBDESC,
						...
						WHERE A.ITM_CODE = '$ITM_CODE' AND A.PRJCODE = '$PRJCODE' AND A.ISLAST = '1'
							...
							OR A.JOBDESC LIKE '%$search%' ESCAPE '!') ORDER BY A.JOBCODEID ASC";
			}
			else
			{
				$sql = "SELECT DISTINCT A.ORD_ID, A.JOBCODEDET, A.JOBCODEID, A.JOBPARENT, A.JOBCODE, A.JOBDESC,
		...
		else
		{
			if($order !=null)
			{
				$sql = "SELECT DISTINCT A.ORD_ID, A.JOBCODEDET, A.JOBCODEID, A.JOBPARENT, A.JOBCODE, A.JOBDESC,
						...
						WHERE A.ITM_CODE = '$ITM_CODE' AND A.PRJCODE = '$PRJCODE' AND A.ISLAST = '1'
							AND (A.ITM_CODE LIKE '%$search%' ESCAPE '!' OR A.JOBCODEID LIKE '%$search%' ESCAPE '!'
							OR A.JOBDESC LIKE '%$search%' ESCAPE '!') ORDER BY A.JOBCODEID ASC LIMIT $start, $length";
			}
			else
			{
				$sql = "SELECT DISTINCT A.ORD_ID, A.JOBCODEDET, A.JOBCODEID, A.JOBPARENT, A.JOBCODE, A.JOBDESC,
	...
	function get_AllDataITMDETC($PRJCODE, $PR_NUM, $task, $search, $length, $start) // GOOD
	{
		## Custom Field value
	...
	{
		if($ITMCAT == 'O')
			$QRY01 	= "ITM_GROUP NOT IN ('M','T') AND";
		elseif($ITMCAT == 'M')
