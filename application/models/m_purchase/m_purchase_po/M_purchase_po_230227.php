...
class M_purchase_po extends CI_Model
{
	...
	function updatePODet($PO_NUM, $PRJCODE, $PR_NUM, $ISDIRECT, $task) // OK
	{
		...
		$s_TPO	= "SELECT A.PRD_ID, A.JOBCODEID, A.ITM_CODE, SUM(A.PO_VOLM) AS TOT_PO_VOL, SUM(PO_CVOL) AS TOT_PO_CVOL, SUM(A.PO_COST) AS TOT_PO_VAL, SUM(PO_CTOTAL) AS TOT_PO_CVAL FROM tbl_po_detail A
