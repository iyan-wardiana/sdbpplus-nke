...
class M_purchase_po extends CI_Model
{
	...
	function get_AllDataITMREQDETC($PRJCODE, $PR_NUM, $task, $search, $length, $start) // GOOD
	{
		## Custom Field value
		...
		if($length == -1)
		{
			$s_sql 	= "SELECT COUNT(*) AS TOT_PR
						FROM (SELECT A.* FROM tbl_pr_detail A WHERE A.PR_NUM = '$PR_NUM' AND A.PRJCODE = '$PRJCODE' $searchQuery
		...
		if($length == -1)
		{
			if($order !=null)
			{
				$sql = "SELECT DISTINCT A.PR_ID, A.PR_NUM, A.PR_CODE, A.PR_DATE, A.PRJCODE, A.DEPCODE, A.JOBCODEDET, A.JOBCODEID, A.JOBPARENT, A.JOBPARDESC,
							...
							A.PR_CVOL
						FROM tbl_pr_detail A WHERE A.PR_NUM = '$PR_NUM' AND A.PRJCODE = '$PRJCODE' $searchQuery
							AND (A.ITM_CODE LIKE '%$search%' ESCAPE '!' OR A.ITM_NAME LIKE '%$search%' ESCAPE '!'
							OR A.PR_DESC LIKE '%$search%' ESCAPE '!')
						UNION ALL
