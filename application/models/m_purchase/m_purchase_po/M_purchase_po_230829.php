...
class M_purchase_po extends CI_Model
{
	...
	{			
		$sql = "SELECT A.*, B.proj_Number, B.PRJCODE, B.PRJNAME, C.PR_CODE
				FROM tbl_po_header A
					INNER JOIN 	tbl_project B ON A.PRJCODE = B.PRJCODE
	...
	function updatePODet($PO_NUM, $PRJCODE, $PR_NUM, $ISDIRECT, $task) // OK
	{
		// task add tidak ada update PO_STAT => upd. 2022-12-22
		if($task != 'add')
		{
			$UPD_STAT 	= "UPDATE tbl_po_detail A, tbl_po_header B SET A.PO_STAT = B.PO_STAT 
							WHERE A.PO_NUM = B.PO_NUM AND A.PRJCODE = '$PRJCODE'
							AND A.PO_NUM = '$PO_NUM'";
			$this->db->query($UPD_STAT);
		}

		/*$s_TPO	= "SELECT A.PRD_ID, A.JOBCODEID, A.ITM_CODE, SUM(A.PO_VOLM) AS TOT_PO_VOL, SUM(PO_CVOL) AS TOT_PO_CVOL, SUM(A.PO_COST) AS TOT_PO_VAL, SUM(PO_CTOTAL) AS TOT_PO_CVAL
					FROM tbl_po_detail A
						WHERE A.PO_NUM IN (SELECT Z.PO_NUM FROM tbl_po_header Z WHERE Z.PR_NUM = '$PR_NUM' AND Z.PRJCODE = '$PRJCODE'
							AND Z.PO_STAT NOT IN (5,9)) GROUP BY A.PR_NUM, A.PRD_ID, A.JOBCODEID";*/
		$s_TPO	= "SELECT A.PRD_ID, A.JOBCODEID, A.ITM_CODE, SUM(A.PO_VOLM) AS TOT_PO_VOL, SUM(PO_CVOL) AS TOT_PO_CVOL, SUM(A.PO_COST) AS TOT_PO_VAL, SUM(PO_CTOTAL) AS TOT_PO_CVAL
					FROM tbl_po_detail A
						WHERE A.PO_NUM IN ('$PO_NUM') GROUP BY A.PR_NUM, A.PRD_ID, A.JOBCODEID";
		$r_TPO	= $this->db->query($s_TPO)->result();
		foreach($r_TPO as $rw_TPO) :
			$PRD_ID		= $rw_TPO->PRD_ID;
			$PO_JID		= $rw_TPO->JOBCODEID;
			$ITM_CODE	= $rw_TPO->ITM_CODE;
			$TOT_VOL	= $rw_TPO->TOT_PO_VOL;
			$TOT_CVOL 	= $rw_TPO->TOT_PO_CVOL;
			$TOT_VAL	= $rw_TPO->TOT_PO_VAL;
			$TOT_CVAL 	= $rw_TPO->TOT_PO_CVAL;

			$s_UPPR		= "UPDATE tbl_pr_detail SET PO_VOLM = ($TOT_VOL-$TOT_CVOL), PO_AMOUNT = ($TOT_VAL-$TOT_CVAL) WHERE PR_NUM = '$PR_NUM' AND JOBCODEID = '$PO_JID' AND PR_ID = $PRD_ID";
			$this->db->query($s_UPPR);
		endforeach;
	...
	function updateVolBudNew($PO_NUM, $PRJCODE) // G
	{
		$PR_VOLM 	= 0;
		$PR_AMOUNT 	= 0;
		$IR_VOLM 	= 0;
		$IR_AMOUNT 	= 0;			
		$sqlGetPO	= "SELECT PO_ID, JOBCODEID, ITM_CODE, PO_DATE, PO_VOLM, PO_COST, IR_VOLM, IR_AMOUNT, PR_NUM, PRD_ID
						FROM tbl_po_detail
		...
		foreach($resGetPO as $rowPO) :
			...
			$s_02		= "UPDATE tbl_joblist_detail SET PR_VOL = PR_VOL-$REM_VOL, PR_VAL = PR_VAL-$REM_VAL, PO_VOL = PO_VOL-$REM_VOL, PO_VAL = PO_VAL-$REM_VAL
							WHERE PRJCODE = '$PRJCODE' AND JOBCODEID = '$JOBCODEID' AND ITM_CODE = '$ITM_CODE'";
			$this->db->query($s_02);
			
			$s_03		= "UPDATE tbl_item SET PR_VOL = PR_VOL-$REM_VOL, PR_VAL = PR_VAL-$REM_VAL, PO_VOL = PO_VOL-$REM_VOL, PO_VAL = PO_VAL-$REM_VAL
							WHERE PRJCODE = '$PRJCODE' AND ITM_CODE = '$ITM_CODE'";
			$this->db->query($s_03);
			...
			$s_05		= "UPDATE tbl_po_detail SET PO_CVOL = PO_CVOL+$REM_VOL, PO_CTOTAL = PO_CTOTAL+$REM_VAL
							WHERE PRJCODE = '$PRJCODE' AND PO_NUM = '$PO_NUM' AND ITM_CODE = '$ITM_CODE' AND PO_ID = '$PO_ID'";
			$this->db->query($s_05);
		endforeach;
	}
	...
	function get_AllDataITMDETC($PRJCODE, $PO_NUM, $length, $start) // GOOD
	{
		$PRJCODEVW 	= strtolower(preg_replace("/[^a-zA-Z0-9\s]/", "", $PRJCODE));

		if($length == -1)
		{
			/*$sql 	= "SELECT A.* FROM tbl_po_detail A
						...
						WHERE B.PO_NUM = '$PO_NUM' AND B.PRJCODE = '$PRJCODE'";*/
			$sql 	= "SELECT A.* FROM tbl_po_detail A
	...
	function get_AllDataITMDETL($PRJCODE, $PO_NUM, $search, $length, $start, $order, $dir) // GOOD
	{
		...
		if($length == -1)
		{
			if($order !=null)
			{
				/*$sql = "SELECT DISTINCT A.PO_ID, A.PO_NUM, A.PO_DATE, A.PR_NUM, A.JOBCODEDET, A.JOBCODEID, 
							A.ITM_CODE, A.ITM_UNIT, A.ITM_UNIT2, 
							A.PO_PRICE AS ITM_PRICE, A.PR_VOLM, A.PO_VOLM, A.PO_PRICE,
							A.IR_VOLM, A.IR_AMOUNT, A.PO_DISP, A.PO_CVOL, A.PO_CTOTAL,
							A.PO_COST, A.PO_DISC, A.PO_DESC_ID, A.PO_DESC, A.PR_DESC_ID, A.TAXCODE1, 
							A.TAXCODE2, A.TAXPRICE1,
							A.TAXPRICE2, A.PRD_ID, A.JOBPARENT, A.JOBPARDESC, 
							B.PO_CODE, B.PRJCODE, C.ITM_NAME, C.ITM_GROUP, C.ITM_CATEG
						FROM tbl_po_detail A
						INNER JOIN tbl_po_header B ON B.PO_NUM = A.PO_NUM AND B.PRJCODE = A.PRJCODE
						INNER JOIN tbl_item_$PRJCODEVW C ON C.ITM_CODE = A.ITM_CODE
						WHERE B.PO_NUM = '$PO_NUM' AND B.PRJCODE = '$PRJCODE'
						AND (A.ITM_CODE LIKE '%$search%' ESCAPE '!' OR C.ITM_NAME LIKE '%$search%' ESCAPE '!'
						OR A.PO_DESC LIKE '%$search%' ESCAPE '!')
						ORDER BY A.ITM_CODE, A.PO_DESC_ID, $order $dir";*/
				$sql = "SELECT DISTINCT A.PO_ID, A.PO_NUM, A.PO_DATE, A.PR_NUM, A.JOBCODEDET, A.JOBCODEID, 
							A.ITM_CODE, A.ITM_UNIT, A.ITM_UNIT2, 
							A.PO_PRICE AS ITM_PRICE, A.PO_OA, A.PR_VOLM, A.PO_VOLM, A.PO_PRICE,
							A.IR_VOLM, A.IR_AMOUNT, A.PO_DISP, A.PO_CVOL, A.PO_CTOTAL,
							A.PO_COST, A.PO_DISC, A.PO_DESC_ID, A.PO_DESC, A.PR_DESC_ID, A.TAXCODE1, 
							A.TAXCODE2, A.TAXPRICE1,
							A.TAXPRICE2, A.PRD_ID, A.JOBPARENT, A.JOBPARDESC, 
							A.PO_CODE, A.PRJCODE, A.ITM_NAME, A.ITM_GROUP, A.ITM_CATEG
						FROM tbl_po_detail A
						WHERE A.PO_NUM = '$PO_NUM' AND A.PRJCODE = '$PRJCODE'
		...
		else
		{
			if($order !=null)
			{
				$sql = "SELECT DISTINCT A.PO_ID, A.PO_NUM, A.PO_DATE, A.PR_NUM, A.JOBCODEDET, A.JOBCODEID, 
						...
						WHERE A.PO_NUM = '$PO_NUM' AND A.PRJCODE = '$PRJCODE'
						AND (A.ITM_CODE LIKE '%$search%' ESCAPE '!' OR A.ITM_NAME LIKE '%$search%' ESCAPE '!'
						OR A.PO_DESC LIKE '%$search%' ESCAPE '!') ORDER BY A.ITM_CODE, A.PO_DESC_ID, $order $dir
						LIMIT $start, $length";
			}
			else
			{
	...
	function get_AllDataITMPRC($PRJCODE, $search) // GOOD
	{
		/*$sql = "tbl_joblist_detail A
	...
	function get_AllDataITMPRL($PRJCODE, $search, $length, $start, $order, $dir) // GOOD
	{
		if($length == -1)
		{
			if($order !=null)
	...
	function get_AllDataITMREQDETC($PRJCODE, $PR_NUM, $task, $length, $start) // GOOD
	{
		$PRJCODEVW 	= strtolower(preg_replace("/[^a-zA-Z0-9\s]/", "", $PRJCODE));
		...
		if($searchByPRDESC != '')
		{
			$searchQuery .= " AND A.PR_DESC_ID = '".$searchByPRDESC."'";
		}
		...
		if($length == -1)
		{
			$s_sql 	= "SELECT A.*
						...
						-- INNER JOIN tbl_item_$PRJCODEVW C ON C.ITM_CODE = A.ITM_CODE
						WHERE B.PR_NUM = '$PR_NUM' AND B.PRJCODE = '$PRJCODE' $searchQuery
						AND (A.PR_VOLM - A.PR_CVOL) > 0 AND B.PR_STAT NOT IN (5,9)";
		}
		else
		{
			$s_sql 	= "SELECT A.*
						...
						-- INNER JOIN tbl_item_$PRJCODEVW C ON C.ITM_CODE = A.ITM_CODE
						WHERE B.PR_NUM = '$PR_NUM' AND B.PRJCODE = '$PRJCODE' $searchQuery
						AND (A.PR_VOLM - A.PR_CVOL) > 0 AND B.PR_STAT NOT IN (5,9) LIMIT $start, $length";
		}

		return $this->db->query($s_sql)->result();
	}
	
	function get_AllDataITMREQDETL($PRJCODE, $PR_NUM, $task, $search, $length, $start, $order, $dir) // GOOD
	{
		$PRJCODEVW 	= strtolower(preg_replace("/[^a-zA-Z0-9\s]/", "", $PRJCODE));
		...
		if($length == -1)
		{
			if($order !=null)
			{
				$sql = "SELECT DISTINCT A.PR_ID, A.PR_NUM, A.PR_CODE, A.JOBCODEDET, A.JOBCODEID, A.ITM_CODE, A.SNCODE, 
						A.ITM_UNIT, A.ITM_UNIT2, (A.PR_VOLM - A.PR_CVOL) AS PR_VOLM,
						A.PO_VOLM, A.IR_VOLM, A.IR_AMOUNT, 0 AS PO_DISP, 0 AS PO_DISC, 
						A.PO_VOLM AS ORDERED_VOL, 
						A.PR_PRICE AS ITM_PRICE, A.PR_PRICE AS PO_PRICE, A.PR_TOTAL, A.PR_DESC_ID,
						A.PR_DESC_ID AS PO_DESC_ID, A.PR_DESC AS PO_DESC, A.TAXCODE1,
						A.TAXCODE2, A.TAXPRICE1, A.TAXPRICE2, A.PR_ID AS PRD_ID, A.JOBPARENT, A.JOBPARDESC,
						C.ITM_NAME, C.ITM_GROUP, C.ITM_CATEG
						FROM tbl_pr_detail A
						INNER JOIN tbl_pr_header B ON B.PR_NUM = A.PR_NUM AND B.PRJCODE = A.PRJCODE
						INNER JOIN tbl_item_$PRJCODEVW C ON C.ITM_CODE = A.ITM_CODE
						WHERE B.PR_NUM = '$PR_NUM' AND B.PRJCODE = '$PRJCODE' $searchQuery
						AND (A.PR_VOLM - A.PR_CVOL) > 0 AND B.PR_STAT NOT IN (5,9)
						AND (A.ITM_CODE LIKE '%$search%' ESCAPE '!' OR C.ITM_NAME LIKE '%$search%' ESCAPE '!'
						OR A.PR_DESC LIKE '%$search%' ESCAPE '!') ORDER BY A.ITM_CODE, A.PR_DESC_ID, $order $dir";
			}
			else
			{
				$sql = "SELECT DISTINCT A.PR_ID, A.PR_NUM, A.PR_CODE, A.JOBCODEDET, A.JOBCODEID, A.ITM_CODE, A.SNCODE, 
		...
		else
		{
			...
			else
			{
				$sql = "SELECT DISTINCT A.PR_ID, A.PR_NUM, A.PR_CODE, A.JOBCODEDET, A.JOBCODEID, A.ITM_CODE, A.SNCODE, 
						...
						AND (A.PR_VOLM - A.PR_CVOL) > 0 AND B.PR_STAT NOT IN (5,9)
						AND (A.ITM_CODE LIKE '%$search%' ESCAPE '!' OR C.ITM_NAME LIKE '%$search%' ESCAPE '!'
						OR A.PR_DESC LIKE '%$search%' ESCAPE '!') LIMIT $start, $length";
			}
