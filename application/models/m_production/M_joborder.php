 ...
 * File Name	= M_joborder.php
 * Location		= -
*/

class M_joborder extends CI_Model
{
	...
	{
		$sql = "tbl_jo_header 
				WHERE PRJCODE = '$PRJCODE' AND ISREPROC = 0
	...
	function count_all_JO($PRJCODE, $key) // GOOD
	{
		if($key == '')
		{
			$sql = "tbl_jo_header A
					...
					WHERE A.PRJCODE = '$PRJCODE' AND A.ISREPROC = 0";
		}
		else
		{
			$sql = "tbl_jo_header A
		...
		if($key == '')
		{
			$sql = "SELECT A.*, B.CUST_DESC
					FROM tbl_jo_header A
						INNER JOIN tbl_customer B ON A.CUST_CODE = B.CUST_CODE
					WHERE A.PRJCODE = '$PRJCODE' AND A.ISREPROC = 0 LIMIT $start, $end";
		}
		else
		{
			$sql = "SELECT A.*, B.CUST_DESC
	...
	function updateSOH($SO_NUM, $PRJCODE) // GOOD
	{
		...
		/*$sqlSO		= "SELECT IF(SUM(A.ITM_QTY) > 0, SUM(A.ITM_QTY), 0) AS TOTJO 
        ...
        $TOTSO	= 0;
        $TOTJO	= 0;
		$sqlSO		= "SELECT IF(SUM(A.SO_VOLM) > 0, SUM(A.SO_VOLM), 0) AS TOTSO, IF(SUM(A.JO_VOLM) > 0, SUM(A.JO_VOLM), 0) AS TOTJO 
        ...
        if($TOTSO == $TOTJO)
        {
        	$sqlSOH		= "UPDATE tbl_so_header SET SO_STAT = 6, STATDESC = 'Close', STATCOL = 'info'
							WHERE SO_NUM = '$SO_NUM' AND PRJCODE = '$PRJCODE'";
			$this->db->query($sqlSOH);
        }
	}
	...
	{			
		$sql = "SELECT A.SO_NUM, A.JO_CODE, A.SO_TYPE, A.SO_CAT, A.SO_DATE, A.SO_DUED, A.PRJCODE, A.SPLCODE, 
				...
				WHERE A.SO_NUM = '$SO_NUM' AND A.ISREPROC = 0";
		return $this->db->query($sql);
	}
	...
	function count_all_IR($SO_NUM) // OK
	{
		$sql	= "tbl_ir_header A
						INNER JOIN tbl_jo_header B ON A.SO_NUM = B.SO_NUM
							AND B.SO_STAT IN (3,6) AND B.ISREPROC = 0
